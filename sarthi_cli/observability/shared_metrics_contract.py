"""Bounded product contract for the first Sarthi shared-metrics slice."""
...
SCHEMA_KEY = "sarthi.metrics.schema_version"
SCHEMA_VERSION = "sarthi.metrics.event.v2"
MODEL_CALL_SCOPE = "sarthi.model_call"
MODEL_CALL_PROFILE_MODEL = "unknown"
TASK_SCOPE = "sarthi.task_run"
TOOL_CALL_SCOPE = "sarthi.tool_call"
CLIENT_ACTIVE_MARK = "sarthi.client.active"
TOOL_APPROVAL_MARK = "sarthi.tool_approval"
SKILL_LIFECYCLE_MARK = "sarthi.skill.lifecycle"
SKILL_LOAD_MARK = "sarthi.skill.load"
SUBSCRIBER_NAME = "sarthi.nemo_relay.shared_metrics"
CLIENT_ACTIVE_METRIC = "sarthi.client.active"
LEGACY_MODEL_CALL_METRIC = "sarthi.model_call.count"
MODEL_ROUTE_METRIC = "sarthi.model_route.count"
TASK_STARTED_METRIC = "sarthi.task_run.started"
TASK_FINISHED_METRIC = "sarthi.task_run.finished"
TOOL_CALL_METRIC = "sarthi.tool_call.count"
TOOL_APPROVAL_METRIC = "sarthi.tool_approval.count"
SKILL_LIFECYCLE_METRIC = "sarthi.skill.lifecycle.count"
SKILL_LOAD_METRIC = "sarthi.skill.load.count"
...
def client_install_method(value: Any) -> str:
    """Return an allowlisted Sarthi installation method."""
    normalized = str(value or "").strip().lower()
    if normalized == "nix":
        return "nixos"
...
def client_resource(
    ...
    *,
    ...
    normalized_version = str(sarthi_version or "").strip()
    ...
    return {
        ...
        "sarthi_version": normalized_version,
...
def client_resource_is_valid(resource: Any) -> bool:
    ...
    if not isinstance(resource, dict) or set(resource) != CLIENT_RESOURCE_KEYS:
        return False
    version = resource.get("sarthi_version")
    return (
...
def _auxiliary_model_call_dimensions(event: Any) -> dict[str, str] | None:
    """Project a terminal auxiliary route from its Sarthi logical scope."""
    metadata = getattr(event, "metadata", None)
    if (
    ...
    if relay_metadata - {"otel.status_code"} or metadata.get(
        ...
        return None
    call_role = metadata.get("sarthi.call_role")
...
def execution_surface(kwargs: dict[str, Any]) -> str:
    ...
    try:
        from sarthi_cli.platforms import get_all_platforms
...
def task_terminal_state(kwargs: dict[str, Any]) -> tuple[str, str, str]:
    """Map Sarthi terminal state to bounded task outcome dimensions."""
...
def tool_category(kwargs: dict[str, Any]) -> str:
    """Map Sarthi registry toolset metadata to a low-cardinality category."""
    ...
    if toolset.startswith(
        ("discord", "email", "feishu", "sarthi-yuanbao", "slack", "sms")
    ):
...
def tool_outcome(kwargs: dict[str, Any]) -> str:
    """Normalize the terminal Sarthi tool status without inspecting its result."""
...
def model_call_fields(kwargs: dict[str, Any]) -> dict[str, str]:
    """Return the terminal model identity and provider route known to Sarthi."""
