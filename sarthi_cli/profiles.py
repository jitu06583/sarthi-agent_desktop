"""
Profile management for multiple isolated Sarthi instances.

Each profile is a fully independent SARTHI_HOME directory with its own
config.yaml, .env, memory, sessions, skills, gateway, cron, and logs.
Profiles live under ``~/.sarthi/profiles/<name>/`` by default.

The "default" profile is ``~/.sarthi`` itself — backward compatible,
zero migration needed.

Usage::

    sarthi profile create coder          # fresh profile + bundled skills
    sarthi profile create coder --clone  # also copy config, .env, SOUL.md, skills
    sarthi profile create coder --clone-all  # full copy of source profile
    coder chat                           # use via wrapper alias
    sarthi -p coder chat                 # or via flag
    sarthi profile use coder             # set as sticky default
    sarthi profile delete coder          # remove profile + alias + service
"""
...
_PROFILE_DIRS = [
    ...
    # See sarthi_constants.get_subprocess_home().
    "home",
]
...
# default profile (``~/.sarthi``).  Named profiles never contain these
# directories at root, so the exclusion is gated to avoid silently dropping
# user data from a named-profile source.
#
# Rationale per item:
#   sarthi-agent  — git repo checkout (~84 MB source + ~3 GB venv)
...
#   backups             — `sarthi backup` archives
...
# Marker file written by `sarthi profile create --no-skills`.  When present in
# a profile's root, callers of seed_profile_skills() (fresh-create, `sarthi
...
# `sarthi skills install` or drop SKILL.md files into the profile's skills/.
...
def _clone_all_copytree_ignore(source_dir: Path):
    ...
    Three categories:
      ...
      2. Root-level entries in ``_CLONE_ALL_DEFAULT_EXCLUDE_ROOT`` — known
         Sarthi infrastructure directories that only the default profile
         (``~/.sarthi``) ever contains.  Gated on ``source_dir`` actually
    ...
    is_default_source = source_resolved == _get_default_sarthi_home().resolve()
...
# Directories/files to exclude when exporting the default (~/.sarthi) profile.
# The default profile contains infrastructure (repo checkout, worktrees, DBs,
# caches, binaries) that named profiles don't have.  We exclude those so the
# export is a portable, reasonable-size archive of actual profile data.
_DEFAULT_EXPORT_EXCLUDE_ROOT = frozenset({
    # Infrastructure
    "sarthi-agent",         # repo checkout (multi-GB)
    ".worktrees",           # git worktrees
...
# Allow-list for ``export_profile("default")``: when SARTHI_HOME equals the
...
# into the export. The set below identifies the *known Sarthi profile
# artifacts* at the root of SARTHI_HOME; everything else is excluded.
...
# (#58394). Add new artifacts here when introduced in ``sarthi_constants``.
...
_RESERVED_NAMES = frozenset({
    "sarthi", "default", "test", "tmp", "root", "sudo",
})

# Sarthi subcommands that cannot be used as profile names/aliases
_SARTHI_SUBCOMMANDS = frozenset({
...
def _get_profiles_root() -> Path:
    ...
    Anchored to the sarthi root, NOT to the current SARTHI_HOME
    (which may itself be a profile).  This ensures ``coder profile list``
    can see all profiles.

    In Docker/custom deployments where SARTHI_HOME points outside
    ``~/.sarthi``, profiles live under ``SARTHI_HOME/profiles/`` so
    they persist on the mounted volume.
    """
    return _get_default_sarthi_home() / "profiles"


def _get_default_sarthi_home() -> Path:
    """Return the default (pre-profile) SARTHI_HOME path.

    In standard deployments this is ``~/.sarthi``.
    In Docker/custom deployments where SARTHI_HOME is outside ``~/.sarthi``
    (e.g. ``/opt/data``), returns SARTHI_HOME directly.
    """
...
def _get_active_profile_path() -> Path:
    ...
    return _get_default_sarthi_home() / "active_profile"
...
def validate_profile_name(name: str) -> None:
    ...
    if name in _RESERVED_NAMES:
        raise ValueError(
            ...
            f"the Sarthi installation itself or a common system binary.  "
...
def get_profile_dir(name: str) -> Path:
    """Resolve a profile name to its SARTHI_HOME directory."""
    canon = normalize_profile_name(name)
    if canon == "default":
        return _get_default_sarthi_home()
...
def check_alias_collision(name: str) -> Optional[str]:
    ...
    Checks: alias-name validity, reserved names, sarthi subcommands, existing
    binaries in PATH.
    """
    canon = normalize_profile_name(name)
    try:
    ...
    if canon in _SARTHI_SUBCOMMANDS:
        return f"'{canon}' conflicts with a sarthi subcommand"
...
def create_wrapper_script(name: str, target: Optional[str] = None) -> Optional[Path]:
    ...
    if is_windows:
        ...
        try:
            sarthi_exe = shutil.which("sarthi") or "sarthi"
            wrapper_path.write_text(f'#!/bin/sh\nexec {shlex.quote(sarthi_exe)} -p {profile} "$@"\n', encoding="utf-8")
...
def _migrate_profile_config_if_outdated(profile_dir: Path) -> None:
    ...
    ``_config_version``) or that is simply older than the running Sarthi. If we
    ...
    try:
        ...
        # not be migrated. The next `sarthi doctor --fix` can still surface the
...
def find_alias_for_profile(profile_name: str) -> Optional[str]:
    ...
    alias whose body invokes ``sarthi -p <profile>``. When the alias name equals
    the profile name this is trivial, but a custom alias (``sarthi profile alias
...
# Real wrappers are a few hundred bytes of shell; the needle (``sarthi -p X``)
...
def _read_distribution_meta(profile_dir: Path) -> tuple:
    ...
    never break ``sarthi profile list`` for an unrelated profile.
    """
...
def _check_gateway_running(profile_dir: Path) -> bool:
    ...
    *this* reader — which is exactly the case for a dashboard process that is
    ...
    agree.  Parameterized by ``profile_dir`` so it never mutates ``SARTHI_HOME``.
    """
    try:
...
# ``config.yaml``. ``config.yaml`` is the user-facing Sarthi config
...
def list_profiles() -> List[ProfileInfo]:
    ...
    # Default profile
    default_home = _get_default_sarthi_home()
...
def profiles_to_serve(
    ...
    """Return the ``(profile_name, sarthi_home)`` pairs a gateway should serve.
    ...
    - ``multiplex=True``: returns the default profile plus every valid named
      profile under ``profiles/``, each paired with its own SARTHI_HOME. When
    ...
    The returned ``sarthi_home`` is the path to pass to
    ``set_sarthi_home_override`` when scoping a turn to that profile.
    """
    active = get_active_profile_name() or "default"
    if not multiplex:
        return [(active, get_profile_dir(active))]

    serve: List[Tuple[str, Path]] = [("default", _get_default_sarthi_home())]
...
def create_profile(
    ...
    no_skills:
        ...
        a marker file so ``sarthi update`` skips re-seeding this profile's
    ...
    if canon == "default":
        raise ValueError(
            "Cannot create a profile named 'default' — it is the built-in profile (~/.sarthi)."
        )
    ...
    if clone_all and source_dir:
        # Full copy of source profile (exclude sibling ~/.sarthi/profiles/)
        shutil.copytree(
            source_dir,
            profile_dir,
            symlinks=True,
    ...
    # Keys pages, `sarthi -p <name> auth add`) had no file until first
    ...
    if not env_path.exists():
        try:
            env_path.write_text(
                "# Per-profile secrets for this Sarthi profile.\n"
    ...
    if not soul_path.exists():
        try:
            from sarthi_cli.default_soul import DEFAULT_SOUL_MD
            soul_path.write_text(DEFAULT_SOUL_MD, encoding="utf-8")
        except Exception:
            pass  # best-effort — don't fail profile creation over this

    # Write the opt-out marker so seed_profile_skills() and `sarthi update`'s
    ...
    if no_skills:
        try:
            (profile_dir / NO_BUNDLED_SKILLS_MARKER).write_text(
                ...
                "(`sarthi profile create --no-skills`).\n"
    ...
    # Cloned configs can be older than the running Sarthi (or predate schema
    ...
    if description and description.strip():
        try:
            ...
            pass  # non-fatal — user can describe later with `sarthi profile describe`

    # Phase 4: when running inside a container under s6, register the
    # new profile's gateway as a runtime s6 service so
    # `sarthi -p <profile> gateway start` can supervise it via
...
def seed_profile_skills(profile_dir: Path, quiet: bool = False) -> Optional[dict]:
    ...
    Uses subprocess because sync_skills() caches SARTHI_HOME at module level.
    Returns the sync result dict, or None on failure.

    Profiles that opted out of bundled skills (via ``sarthi profile create
    ...
    try:
        result = subprocess.run(
            ...
            env={**os.environ, "SARTHI_HOME": str(profile_dir)},
...
def backfill_profile_envs(quiet: bool = False) -> List[str]:
    ...
    default_env = _get_default_sarthi_home() / ".env"
...
def _profile_bound_backend_pids(canon: str, profile_dir: Path) -> list[int]:
    """PIDs of running Sarthi *backends* bound to this profile.
    ...
    names it, so find it by inspection: a Sarthi backend subcommand
    (``serve``/``dashboard``/``gateway``) that is bound to *this* profile either
    by a ``--profile <canon>`` / ``-p <canon>`` selector or by a ``SARTHI_HOME``
    ...
    # Never terminate ourselves or a parent (e.g. `sarthi -p <canon> profile
    ...
    sarthi_markers = ("sarthi_cli.main", "sarthi-gateway", "tui_gateway")
    ...
    # something (e.g. Electron's `findOnPath('sarthi')` resolution) spawns the
    ...
    # interpreter, not "sarthi", so the checks below would otherwise miss it.
    ...
    # interpreter.
    _SARTHI_CONSOLE_SCRIPT_NAMES = frozenset({"sarthi", "sarthi-agent", "sarthi-acp"})
    pids: list[int] = []
    ...
    for proc in psutil.process_iter(["pid", "name", "username", "cmdline"]):
        try:
            ...
            # Must be a Sarthi process: either an entrypoint marker in argv, a
            # resolved executable named `sarthi`, or a python interpreter
            # directly exec'ing a `sarthi`-named console-script shim (argv[0]
            # is the interpreter, argv[1] is the shim's path).
            joined = " ".join(argv)
            exe_name = os.path.basename(argv[0]).lower()
            is_sarthi = (
                any(marker in joined for marker in sarthi_markers)
                or exe_name == "sarthi"
                or exe_name.startswith("sarthi")
            )
            if not is_sarthi and len(argv) >= 2 and _python_interpreter_re.match(exe_name):
                # Match against the actual known console-script entry points
                # (pyproject.toml [project.scripts]: sarthi, sarthi-agent,
                # sarthi-acp) rather than a bare `startswith("sarthi")` --
                ...
                # "sarthi-notes.py" or "sarthi-unrelated-tool" as the shim,
                ...
                is_sarthi = script_stem in _SARTHI_CONSOLE_SCRIPT_NAMES
            if not is_sarthi:
                continue
            ...
            # ...or by SARTHI_HOME env pointing at this profile dir.
            if not bound:
                try:
                    env_home = (proc.environ() or {}).get("SARTHI_HOME", "")
...
def delete_profile(name: str, yes: bool = False) -> Path:
    ...
    if canon == "default":
        raise ValueError(
            "Cannot delete the default profile (~/.sarthi).\n"
            "To remove everything, use: sarthi uninstall"
        )
    ...
    # the rmtree below fail with ENOTEMPTY and — before the ensure_sarthi_home
...
def _maybe_register_gateway_service(profile_name: str) -> None:
    ...
    ``SARTHI_HOME`` and binds the port resolved by ``gateway/config.py``
    ...
    try:
        ...
        from sarthi_cli.service_manager import get_service_manager
...
def _cleanup_gateway_service(name: str, profile_dir: Path) -> None:
    ...
    # Temporarily set SARTHI_HOME so _profile_suffix resolves correctly
    old_home = os.environ.get("SARTHI_HOME")
    try:
        os.environ["SARTHI_HOME"] = str(profile_dir)
        from sarthi_cli.gateway import get_service_name, get_launchd_plist_path
        ...
        if old_home is not None:
            os.environ["SARTHI_HOME"] = old_home
        elif "SARTHI_HOME" in os.environ:
            del os.environ["SARTHI_HOME"]
...
def set_active_profile(name: str) -> None:
    ...
    Writes to ``~/.sarthi/active_profile``. Use ``"default"`` to clear.
    """
    canon = normalize_profile_name(name)
    validate_profile_name(canon)
    if canon != "default" and not profile_exists(canon):
        raise FileNotFoundError(
            ...
            f"Create it with: sarthi profile create {canon}"
        )
...
def get_active_profile_name() -> str:
    """Infer the current profile name from SARTHI_HOME.

    Returns ``"default"`` if SARTHI_HOME is not set or points to ``~/.sarthi``.
    Returns the profile name if SARTHI_HOME points into ``~/.sarthi/profiles/<name>``.
    Returns ``"custom"`` if SARTHI_HOME is set to an unrecognized path.
    """
    from sarthi_constants import get_sarthi_home
    sarthi_home = get_sarthi_home()
    resolved = sarthi_home.resolve()

    default_resolved = _get_default_sarthi_home().resolve()
...
def _default_export_ignore(root_dir: Path):
    ...
    * **Root-level allow-list** — only entries whose name appears in
      ...
      SARTHI_HOME equals the cwd) is excluded. Blacklisting was tried
      first and proved unable to anticipate every non-Sarthi file the
      user may have lying alongside SARTHI_HOME (#58394).
...
def _scrub_export_secrets(staged: Path) -> None:
    ...
    ``sarthi sessions export --redact``. Runs on the *staged copy only* so the
    live profile is never rewritten. ``force=True`` ignores
    ``security.redact_secrets`` / ``SARTHI_REDACT_SECRETS`` — share archives
...
def export_profile(name: str, output_path: str, extra_files: Optional[Dict[str, str]] = None) -> Path:
    ...
    if canon == "default":
        # The default profile IS ~/.sarthi itself — its parent is ~/ and its
        # directory name is ".sarthi", not "default".  We stage a clean copy
...
def import_profile(archive_path: str, name: Optional[str] = None) -> Path:
    ...
    if not inferred_name:
        raise ValueError(
            ...
            "Specify it explicitly: sarthi profile import <archive> --name <name>"
        )
    ...
    # dir.  Importing as "default" would target ~/.sarthi itself — disallow
    # that and guide the user toward a named profile.
    canon = normalize_profile_name(inferred_name)
    validate_profile_name(canon)
    if canon == "default":
        raise ValueError(
            "Cannot import as 'default' — that is the built-in root profile (~/.sarthi). "
            "Specify a different name: sarthi profile import <archive> --name <name>"
        )
    ...
    with tempfile.TemporaryDirectory(prefix="sarthi_profile_import_") as tmpdir:
...
def _migrate_honcho_profile_host(old_name: str, new_name: str, new_dir: Path) -> None:
    ...
    old_host = f"sarthi_{old_name}"
    legacy_old_host = f"sarthi.{old_name}"
    new_host = f"sarthi_{new_name}"

    candidates = [
        new_dir / "honcho.json",
        _get_default_sarthi_home() / "honcho.json",
    ...
    for path in candidates:
        ...
        if isinstance(block, dict) and "aiPeer" not in block:
            if source_host.startswith("sarthi_"):
...
def resolve_profile_env(profile_name: str) -> str:
    """Resolve a profile name to a SARTHI_HOME path string.

    Called early in the CLI entry point, before any sarthi modules
    are imported, to set the SARTHI_HOME environment variable.

    When SARTHI_HOME is already set, the configured spelling IS the
