"""Shell completion script generation for sarthi CLI.
...
def generate_bash(parser: argparse.ArgumentParser) -> str:
    ...
    for cmd in sorted(tree["subcommands"]):
        ...
        if cmd == "profile" and info["subcommands"]:
            ...
            cases.append(
                ...
                f"                    COMPREPLY=($(compgen -W \"$(_sarthi_profiles)\" -- \"$cur\"))\n"
                f"                    return\n"
                f"                    ;;\n"
                f"            esac\n"
                f"            ;;"
    ...
    return f"""# Sarthi Agent bash completion
# Add to ~/.bashrc:
#   eval "$(sarthi completion bash)"

_sarthi_profiles() {{
    local profiles_dir="$HOME/.sarthi/profiles"
...
_sarthi_completion() {{
    ...
    if [[ "$prev" == "-p" || "$prev" == "--profile" ]]; then
        COMPREPLY=($(compgen -W "$(_sarthi_profiles)" -- "$cur"))
        return
    fi
...
complete -F _sarthi_completion sarthi
"""
...
def generate_zsh(parser: argparse.ArgumentParser) -> str:
    ...
    return f"""#compdef sarthi
# Sarthi Agent zsh completion
# Add to ~/.zshrc:
...
_sarthi_profiles() {{
    local -a profiles
    profiles=(default)
    if [[ -d "$HOME/.sarthi/profiles" ]]; then
        profiles+=($HOME/.sarthi/profiles/*(N/:t))
    fi
...
_sarthi() {{
    ...
    _arguments -C \\
        ...
        '(-)'{{-p,--profile}}'[Profile name]:profile:_sarthi_profiles' \\
        '1:command:->commands' \\
        '*::arg:->args'
...
{top_cmds_str}
            ...
            _describe 'sarthi command' subcmds
            ;;
        args)
            case ${{line[1]}} in
{sub_cases_str}
...
def generate_fish(parser: argparse.ArgumentParser) -> str:
    ...
    lines: list[str] = [
        "# Sarthi Agent fish completion",
        "# Add to your config:",
        "#   sarthi completion fish | source",
        "",
        "# Helper: list available profiles",
        "function __sarthi_profiles",
        "    echo default",
        "    if test -d $HOME/.sarthi/profiles",
        "        for d in $HOME/.sarthi/profiles/*/",
        "            basename $d",
        "        end",
        "    end",
        "end",
        ...
        "",
        "# Complete profile names after -p / --profile",
        "complete -c sarthi -f -s p -l profile"
        " -d 'Profile name' -xa '(__sarthi_profiles)'",
        "",
        ...
        if cmd == "profile":
            for action in sorted(profile_name_actions):
                lines.append(
                    ...
                    f"-a '(__sarthi_profiles)' -d 'Profile name'"
                )
