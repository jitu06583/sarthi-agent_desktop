"""Profile distributions — shareable, packaged Sarthi profiles via git.

A distribution is a Sarthi profile published as a git repository (or
...
* ``sarthi profile export/import`` — local backup / restore for a profile
  on your own machine. NOT a distribution format. Stays as-is.
* ``sarthi skills install <url>`` — the URL install pattern we're mirroring,
  but at the profile granularity.

Subcommands (all live under ``sarthi profile``, not a parallel tree):

    sarthi profile install <source> [--name N] [--alias] [--force] [--yes]
...
USER_OWNED_EXCLUDE: frozenset = frozenset({
    ...
    "backups", "cache",
    # Infrastructure
    "sarthi-agent", ".worktrees", "profiles", "bin", "node_modules",
...
class DistributionManifest:
    ...
    def from_dict(cls, data: Any) -> "DistributionManifest":
        ...
        return cls(
            ...
            description=str(data.get("description") or ""),
            sarthi_requires=str(data.get("sarthi_requires") or ""),
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        if self.sarthi_requires:
            out["sarthi_requires"] = self.sarthi_requires
        if self.author:
...
def check_sarthi_requires(spec: str, current_version: str) -> None:
    ...
    if not ok:
        raise DistributionError(
            f"This distribution requires Sarthi {op}{target}, "
...
def _stage_source(source: str, workdir: Path) -> Tuple[Path, str]:
    ...
    installed manifest's ``source:`` field so ``sarthi profile update`` can
    ...
    if _looks_like_git_url(src_str):
        ...
        if not (cloned / MANIFEST_FILENAME).is_file():
            raise DistributionError(
                ...
                "This repository is not a Sarthi profile distribution."
            )
...
def plan_install(
    ...
    if manifest is None:
        raise DistributionError(
            ...
            "this source is not a Sarthi distribution."
        )

    # Version check up-front so we fail fast
    check_sarthi_requires(manifest.sarthi_requires, sarthi_version)
    ...
    if canon == "default":
        raise DistributionError(
            ...
            "root profile (~/.sarthi).  Pass --name <name> to install under a "
            "new profile."
        )
    manifest.name = canon
...
def install_distribution(
    ...
    with tempfile.TemporaryDirectory(prefix="sarthi_dist_install_") as tmp:
        ...
        if plan.existing and not force:
            raise DistributionError(
                ...
                "Use `sarthi profile update` to upgrade in place, "
...
def update_distribution(
    ...
    if not existing_manifest.source:
        raise DistributionError(
            ...
            "`sarthi profile install <source> --name {canon} --force`."
        )

    with tempfile.TemporaryDirectory(prefix="sarthi_dist_update_") as tmp:
        plan = plan_install(
