"""Unified provider-credential lifecycle across every store Sarthi reads.

A provider API key can live in up to THREE stores at once:

    1. ``~/.sarthi/.env``                     — the canonical secret store
    2. ``~/.sarthi/auth.json`` →
       ``credential_pool.<provider>[*]``      — env-seeded pool entries
       (``source == "env:<VAR>"``) persisted by the pool loader
    3. ``~/.sarthi/config.yaml``              — inline mirrors written by the
...
def purge_env_credential_references(
    ...
    # Make the removal sticky the same way `sarthi auth remove` does: a
...
def save_provider_env_credential(env_var: str, value: str) -> Dict[str, Any]:
    ...
    re-add through the UI behaves like ``sarthi auth add``.
    """
    from sarthi_cli.config import load_env, save_env_value
