"""Windows ConPTY bridge for the `sarthi dashboard` chat tab.

Drop-in counterpart to ``sarthi_cli.pty_bridge.PtyBridge`` for native
Windows. Mirrors the exact public surface the ``/api/pty`` WebSocket
handler in ``sarthi_cli.web_server`` consumes: ``spawn``, ``read``,
