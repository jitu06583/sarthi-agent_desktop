...
late-binding seam in :mod:`sarthi_cli.web_deps` (``late`` for callables,
...
from sarthi_cli.web_deps import late, LateState
from sarthi_cli.web_models import (
...
_log = logging.getLogger("sarthi_cli.web_server")
...
_spawn_sarthi_action = late("_spawn_sarthi_action")
...
async def toggle_toolset(name: str, body: ToolsetToggle, profile: Optional[str] = None):
    ...
    # same background install `sarthi tools` runs interactively. Without this,
    ...
    # dead-end that forced users to discover `sarthi computer-use install`
