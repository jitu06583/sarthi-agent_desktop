...
late-binding seam in :mod:`sarthi_cli.web_deps` so tests that
...
from sarthi_cli.web_deps import late
...
def get_profiles_projects_tree(preview_limit: int = 3, session_limit: int = 2000):
    ...
    the repo-scan policy, the SARTHI_HOME junk filters — through the
...
async def update_profile_soul(name: str, body: ProfileSoulUpdate):
    ...
    try:
        ...
        # profiles seed SOUL.md at the umask default (sarthi_cli.profiles
...
# ── Export / Import ──────────────────────────────────────────────────────────
# Profile sharing for the desktop: wraps sarthi_cli.profiles.export_profile /
# import_profile (the same machinery behind `sarthi profile export|import`).
...
async def export_profile_endpoint(name: str, body: ProfileExport):
    ...
    if not output:
        ...
        staging = get_sarthi_home() / "profile-exports"
        try:
