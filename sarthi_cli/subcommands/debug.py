"""``sarthi debug`` subcommand parser.
...
def build_debug_parser(subparsers, *, cmd_debug: Callable) -> None:
    ...
    debug_parser = subparsers.add_parser(
        ...
        description="Debug utilities for Sarthi Agent. Use 'sarthi debug share' to "
        "upload a debug report (system info + recent logs) to a paste "
        "service and get a shareable URL.",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""\
Examples:
    sarthi debug share              Upload debug report (asks for confirmation)
    sarthi debug share --yes        Skip confirmation (for scripts/CI)
    sarthi debug share --lines 500  Include more log lines
    sarthi debug share --expire 30  Keep paste for 30 days
    sarthi debug share --local      Print report locally (no upload)
    sarthi debug share --no-redact  Disable upload-time secret redaction
    sarthi debug share --nous       Upload to Nous-internal storage (private)
    sarthi debug delete <url>       Delete a previously uploaded paste
""",
    )
    ...
    delete_parser = debug_sub.add_parser(
        "delete",
        help="Delete a paste uploaded by 'sarthi debug share'",
    )
