"""``sarthi sync`` subcommand parser — Skill Sync.

Cloned from ``sarthi_cli/subcommands/cron.py`` — same injected-handler shape
...
Skill Sync covers two surfaces, both under this one command for launch:
  ...
  Personal — your own skills, across your own devices:
    sarthi sync status                 show gate/opt-in/head state
    sarthi sync pull                   pull and materialize opted-in skills
    sarthi sync push                   push opted-in skills
    sarthi sync now                    reconcile: pull then push
    sarthi sync enable <skill>         opt a skill into sync
    sarthi sync disable <skill>        opt a skill out of sync
    sarthi sync device [--name]        show or set this device's label

  Organisation — skills shared with your team:
    sarthi sync propose <skill>        share a skill with your organisation
...
def build_sync_parser(subparsers, *, cmd_sync: Callable) -> None:
    ...
    sync_parser = subparsers.add_parser(
        ...
        epilog=(
            "Examples:\n"
            "  sarthi sync status            what is synced, and from where\n"
            "  sarthi sync enable my-skill   include a skill in your sync\n"
            "  sarthi sync now               pull, then push\n"
            "  sarthi sync propose my-skill  share a skill with your team\n"
        ),
