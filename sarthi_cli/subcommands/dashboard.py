"""``sarthi dashboard`` / ``sarthi serve`` subcommand parsers.
...
``sarthi_cli/main.py:main()`` (god-file Phase 2); handler injected to avoid
importing ``main``.
"""
...
def _add_server_runtime_args(parser) -> None:
    ...
    # `sarthi dashboard` / `sarthi serve` cmdlines and SIGTERM them directly —
    # the same path `sarthi update` uses to clean up stale servers.
    parser.add_argument(
        "--stop",
        action="store_true",
        help="Stop all running Sarthi web server processes and exit",
    )
    parser.add_argument(
        "--status",
        action="store_true",
        help="List running Sarthi web server processes and exit",
    )
...
def build_dashboard_parser(
    ...
    dashboard_parser = subparsers.add_parser(
        ...
        description="Launch the Sarthi Agent web dashboard for managing config, API keys, and sessions",
    )
    _add_server_runtime_args(dashboard_parser)
    dashboard_parser.add_argument(
        "--no-open", action="store_true", help="Don't open browser automatically"
    )
    # Backward-compat shim: older Sarthi desktop app shells (<= 0.15.x) spawn the
    # backend as `sarthi dashboard --no-open --tui --host ... --port ...`. The
    ...
    # showed "Sarthi couldn't start" with no actionable cause. Accept and silently
    ...
    # browser. It exists so the Sarthi Desktop app (and headless remote
    ...
    serve_parser = subparsers.add_parser(
        "serve",
        help="Start the Sarthi backend server (headless; powers the desktop app and remote backends)",
        description=(
            "Run the Sarthi backend server — the JSON-RPC/WebSocket gateway the "
    ...
    # (cmd_dashboard exports SARTHI_SERVE_HEADLESS=1). `dashboard` leaves it
    # unset and serves the browser UI as before.
    serve_parser.set_defaults(func=cmd_dashboard, no_open=True, headless_backend=True)

    # `sarthi dashboard register` — register a self-hosted dashboard OAuth
    # client with Nous Portal and write the client_id into ~/.sarthi/.env.
    # Nested subparser so bare `sarthi dashboard` keeps launching the server
    ...
    dashboard_register_parser = dashboard_subparsers.add_parser(
        ...
        description=(
            ...
            "SARTHI_DASHBOARD_OAUTH_CLIENT_ID into ~/.sarthi/.env, and prints "
            "how to engage the login gate. Requires being logged in (sarthi setup)."
        ),
    )
    ...
    dashboard_register_parser.add_argument(
        ...
        help=(
            ...
            "https://sarthi.example.com/auth/callback. Omit for localhost-only use."
        ),
    )
    dashboard_register_parser.add_argument(
        "--portal-url",
        ...
        help=(
            ...
            "portal. Also settable via SARTHI_DASHBOARD_PORTAL_URL. Mainly for "
