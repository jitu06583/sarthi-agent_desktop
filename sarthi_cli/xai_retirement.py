...
Pure logic: walks a Sarthi config dict, returns issues for any reference
to a retired xAI model. No I/O, no CLI dependencies — testable in isolation
and reusable from both `sarthi doctor` and a future `sarthi migrate xai`.
"""
...
class RetirementIssue:
    """A reference to a retired xAI model found in a Sarthi config."""
...
def find_retired_xai_refs(config: Dict[str, Any]) -> List[RetirementIssue]:
    """Walk all model slots in a Sarthi config and return retirement issues.
...
def apply_migration(
    ...
    from sarthi_cli.config import require_readable_config_before_write
    ...
    # None``, so the next `sarthi migrate xai` reports nothing to migrate
