"""
Top-level argparse construction for the sarthi CLI.
...
# argparse runs (it sets ``SARTHI_HOME`` and strips itself from ``sys.argv``),
...
def _inherited_flag(parser, *args, **kwargs):
    """Register a flag that ``sarthi_cli.relaunch`` should carry over when
...
_EPILOGUE = """
Examples:
    sarthi                        Start interactive chat
    sarthi chat -q "Hello"        Single query mode
    sarthi --tui                  Launch the modern TUI (or set display.interface: tui)
    sarthi --cli                  Force the classic REPL (overrides display.interface: tui)
    sarthi -c                     Resume the most recent session
    sarthi -c "my project"        Resume a session by name (latest in lineage)
    sarthi --resume <session_id>  Resume a specific session by ID
    sarthi --resume latest        Resume the most recent session (same as -c)
    sarthi --tui --resume latest --in ./dir   Resume ./dir's latest session in the TUI
    sarthi setup                  Run setup wizard
    sarthi logout                 Clear stored authentication
    sarthi auth add <provider>    Add a pooled credential
    sarthi auth list              List pooled credentials
    sarthi auth remove <p> <t>    Remove pooled credential by index, id, or label
    sarthi auth reset <provider>  Clear exhaustion status for a provider
    sarthi model                  Select default model
    sarthi fallback [list]        Show fallback provider chain
    sarthi fallback add           Add a fallback provider (same picker as `sarthi model`)
    sarthi fallback remove        Remove a fallback provider from the chain
    sarthi config                 View configuration
    sarthi config edit            Edit config in $EDITOR
    sarthi config set model gpt-4 Set a config value
    sarthi gateway                Run messaging gateway
    sarthi -s sarthi-agent-dev,github-auth
    sarthi -w                     Start in isolated git worktree
    sarthi gateway install        Install gateway background service
    sarthi sessions list          List past sessions
    sarthi sessions browse        Interactive session picker
    sarthi sessions rename ID T   Rename/title a session
    sarthi logs                   View agent.log (last 50 lines)
    sarthi logs -f                Follow agent.log in real time
    sarthi logs errors            View errors.log
    sarthi logs --since 1h        Lines from the last hour
    sarthi debug share             Upload debug report for support
    sarthi console                Open the safe Sarthi command console
    sarthi update                 Update to latest version
    sarthi dashboard              Start web UI dashboard (port 9119)
    sarthi dashboard --stop       Stop running dashboard processes
    sarthi dashboard --status     List running dashboard processes
...
def build_top_level_parser():
    ...
    parser = argparse.ArgumentParser(
        prog="sarthi",
        description="Sarthi Agent - AI assistant with tool-calling capabilities",
    ...
    # Mirrors `sarthi chat --model ... --provider ...` semantics.
    _inherited_flag(
        parser,
        "-m",
        "--model",
        ...
        help=(
            ...
            "Applies to -z/--oneshot and --tui. Also settable via SARTHI_INFERENCE_MODEL env var."
        ),
    )
    _inherited_flag(
        parser,
        ...
        help=(
            ...
            "under model.provider — use `sarthi setup` or edit the file to change it."
        ),
    )
    _inherited_flag(
        parser,
    ...
    _inherited_flag(
        ...
        help=(
            ...
            "without a TTY prompt.  Equivalent to SARTHI_ACCEPT_HOOKS=1 or "
    ...
    _inherited_flag(
        ...
        action="store_true",
        default=False,
        help="Ignore ~/.sarthi/config.yaml and fall back to built-in defaults (credentials in .env are still loaded)",
    )
    _inherited_flag(
        parser,
        "--ignore-rules",
    ...
    chat_parser = subparsers.add_parser(
        ...
        description="Start an interactive chat session with Sarthi Agent",
    )
    ...
    # top-level parser: when the user writes `sarthi -m foo chat`, argparse
    ...
    # `-w/--worktree`, `--yolo`, etc. (see tests/sarthi_cli/
    ...
    _inherited_flag(
        ...
        help=(
            "Auto-approve any unseen shell hooks declared in config.yaml "
            "without a TTY prompt (see also SARTHI_ACCEPT_HOOKS env var and "
    ...
    _inherited_flag(
        ...
        action="store_true",
        default=argparse.SUPPRESS,
        help="Ignore ~/.sarthi/config.yaml and fall back to built-in defaults (credentials in .env are still loaded). Useful for isolated CI runs, reproduction, and third-party integrations.",
    )
    _inherited_flag(
        chat_parser,
        "--ignore-rules",
    ...
    _inherited_flag(
        ...
        action="store_true",
        default=argparse.SUPPRESS,
        help="Troubleshooting mode: disable ALL customizations — user config, AGENTS.md/memory injection, plugins, and MCP servers (implies --ignore-user-config and --ignore-rules). Use to isolate whether a problem comes from your setup or from Sarthi itself.",
    )
    chat_parser.add_argument(
        "--source",
        default=None,
