...
3. Running inside a container with no persistent volume mount over the
   SARTHI_HOME data dir (state is ephemeral — lost on container restart).
...
logger = logging.getLogger("sarthi.security_audit")
...
def _running_as_root() -> Optional[str]:
    ...
    return (
        ...
        "full host compromise. Run Sarthi as an unprivileged user (or in a "
...
def _in_container() -> bool:
    ...
    if os.path.exists("/.dockerenv"):
        return True
    if os.environ.get("SARTHI_DESKTOP_CHILD_PID"):
...
def _container_no_volume_mount(sarthi_home: Optional[Path]) -> Optional[str]:
    if not _in_container():
        return None
    home = sarthi_home or Path(
        os.environ.get("SARTHI_HOME", os.path.expanduser("~/.sarthi"))
    )
    try:
...
def run_security_audit(
    *, sarthi_home: Optional[Path] = None, config: Optional[dict] = None
) -> list[str]:
    ...
    for check in (
        ...
        try:
            ...
            continue
    try:
        r = _container_no_volume_mount(sarthi_home)
        if r:
            findings.append(r)
    except Exception:
        pass
...
def log_startup_security_warnings(
    *,
    ...
    _AUDIT_RAN = True
    try:
        findings = run_security_audit(sarthi_home=sarthi_home, config=config)
    except Exception:
        return []
    if findings:
        logger.warning(
