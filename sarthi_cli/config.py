"""
Configuration management for Sarthi Agent.

Config files are stored in ~/.sarthi/ for easy access:
- ~/.sarthi/config.yaml  - All settings (model, toolsets, terminal, etc.)
- ~/.sarthi/.env         - API keys and secrets

This module provides:
- sarthi config          - Show current configuration
- sarthi config edit     - Open config in editor
- sarthi config get      - Print a resolved configuration value
- sarthi config set      - Set a specific value
- sarthi config unset    - Remove a user configuration value
- sarthi config wizard   - Re-run setup wizard
"""
...
def _backup_corrupt_config(config_path: Path) -> Optional[Path]:
    ...
    they re-run the setup wizard or ``sarthi config set`` (which rewrites
    ...
    ``config.yaml`` in place: sarthi never silently mutates the user's config,
...
def _warn_config_parse_failure(
    ...
    A YAML parse error in ``~/.sarthi/config.yaml`` causes ``load_config()``
    ...
    ``agent.log`` / ``errors.log`` at WARNING level so ``sarthi logs``
    ...
    logger.warning(msg)
    try:
        sys.stderr.write(f"⚠️  sarthi config: {msg}\n")
        sys.stderr.flush()
    except Exception:
        pass
...
#   the next ``subprocess.run([...])`` Sarthi makes loads attacker code
#   before main().
# * ``PYTHONPATH`` / ``PYTHONHOME`` / ``PYTHONSTARTUP`` /
#   ``PYTHONUSERBASE`` — Python interpreter init. Sarthi itself starts
#   from one of these on every restart.
# * ``NODE_OPTIONS`` / ``NODE_PATH`` — Node interpreter; affects npm,
#   ``sarthi update``, the TUI build.
...
#   on every plugin install / ``sarthi update``.
...
# * ``SARTHI_HOME`` / ``SARTHI_PROFILE`` / ``SARTHI_CONFIG`` /
#   ``SARTHI_ENV`` — Sarthi runtime location flags. Writing these into
...
#
# IMPORTANT: ``SARTHI_*`` overall is NOT blocked. Many legitimate
# integration credentials follow that prefix (SARTHI_LANGFUSE_PUBLIC_KEY,
# SARTHI_SPOTIFY_CLIENT_ID, ...). The
...
_ENV_VAR_NAME_DENYLIST: frozenset[str] = frozenset({
    ...
    # Sarthi runtime location — never via dashboard env writer.
    # NOT a SARTHI_* blanket: integration credentials (SARTHI_GEMINI_*,
    # SARTHI_LANGFUSE_*, SARTHI_SPOTIFY_*, ...) ARE allowed.
    "SARTHI_HOME", "SARTHI_PROFILE", "SARTHI_CONFIG", "SARTHI_ENV",
})
...
def _reject_denylisted_env_var(key: str) -> None:
    ...
    if key in _ENV_VAR_NAME_DENYLIST:
        raise ValueError(
            ...
            "PYTHONPATH, PATH, EDITOR, ...) or Sarthi runtime location "
            "(SARTHI_HOME, SARTHI_PROFILE, ...) cannot be persisted via "
...
_EXTRA_ENV_KEYS = frozenset({
    ...
    "SARTHI_TOOL_PROGRESS", "SARTHI_TOOL_PROGRESS_MODE",
    ...
    # Activation is via plugins.enabled (opt-in through `sarthi plugins enable
    # observability/langfuse` or `sarthi tools → Langfuse`); credentials gate
    # the plugin at runtime.
    "SARTHI_LANGFUSE_ENV",
    "SARTHI_LANGFUSE_RELEASE",
    "SARTHI_LANGFUSE_SAMPLE_RATE",
    "SARTHI_LANGFUSE_MAX_CHARS",
    "SARTHI_LANGFUSE_DEBUG",
    "LANGFUSE_PUBLIC_KEY",
    "LANGFUSE_SECRET_KEY",
    "LANGFUSE_BASE_URL",
})
...
from sarthi_cli.default_soul import DEFAULT_SOUL_MD, is_legacy_template_soul
...
def get_managed_system() -> Optional[str]:
    ...
    raw = os.getenv("SARTHI_MANAGED", "").strip()
    if raw:
    ...
    managed_marker = get_sarthi_home() / ".managed"
...
def is_managed() -> bool:
    """Check if Sarthi is running in package-manager-managed mode.

    Two signals: the SARTHI_MANAGED env var (set by the systemd service),
    or a .managed marker file in SARTHI_HOME (set by the NixOS activation
...
def get_managed_update_command() -> Optional[str]:
    ...
    if managed_system == "Homebrew":
        return "brew upgrade sarthi-agent"
...
def _install_method_project_root(project_root: Optional[Path] = None) -> Path:
    ...
    This is the parent of ``sarthi_cli/`` — i.e. the git checkout for source
    installs, ``/opt/sarthi`` inside the published image, the venv's
    site-packages root for pip installs. It is a property of the running
    interpreter, NOT of ``$SARTHI_HOME``, which is why a code-scoped stamp
...
def detect_install_method(project_root: Optional[Path] = None) -> str:
    """Detect how Sarthi was installed: 'docker', 'nixos', 'homebrew', 'git', or 'pip'.

    Resolution order:
    1. Code-scoped stamp ``<install tree>/.install_method`` (next to the
       running code) — the authoritative marker.
    2. Legacy home-scoped stamp ``$SARTHI_HOME/.install_method`` — read for
       backward compatibility, but a ``docker`` value is IGNORED when we are
       not actually running inside a container (see below).
    3. SARTHI_MANAGED env / .managed marker (NixOS, Homebrew)
    4. .git directory presence -> 'git'
    5. Fallback -> 'pip'

    Why the stamp is code-scoped, not home-scoped (issue: shared ``~/.sarthi``)
    --------------------------------------------------------------------------
    The install method describes *the binary that is running*, but
    ``$SARTHI_HOME`` is a shared DATA directory — the Docker docs deliberately
    bind-mount it (``~/.sarthi:/opt/data``) so config/sessions/memory persist
    and can be shared with a host-side Desktop/CLI install. When a
    containerised gateway and a host install share one ``$SARTHI_HOME``, a
    ...
    ``docker`` and ``sarthi update`` refuses to run ("doesn't apply inside the
    ...
    The supported installs self-identify via the code-scoped stamp:
      ...
      - the published ``nousresearch/hermes-agent`` image bakes a ``docker``
        stamp into ``/opt/sarthi`` at build time.
    ...
    # 1. Code-scoped stamp — authoritative, immune to shared $SARTHI_HOME.
    try:
    ...
    #    host install whose shared $SARTHI_HOME was stamped by a co-located
    #    container, and honouring it wrongly blocks ``sarthi update``.
    try:
        method = (
            (get_sarthi_home() / ".install_method")
...
def _running_in_container() -> bool:
    """Thin wrapper around ``sarthi_constants.is_container`` (import-safe)."""
    try:
...
def stamp_install_method(method: str, project_root: Optional[Path] = None) -> None:
    ...
    not in ``$SARTHI_HOME``, so that two installs sharing one data directory
    ...
    ``/opt/sarthi`` in the published image, which instead bakes the stamp at
...
def is_uv_tool_install() -> bool:
    """Return True when the *running* Sarthi lives in a ``uv tool`` layout.

    ``uv tool install sarthi-agent`` places the install at
    ``.../uv/tools/sarthi-agent/...`` (default ``~/.local/share/uv/tools``,
    ...
    ``sarthi-agent`` happens to be uv-tool-installed on the machine while
    the *active* Sarthi is a regular pip/venv install, causing
    ``sarthi update`` to upgrade the wrong copy. It would also block on a
    ...
    def _has_uv_tool_marker(path: str) -> bool:
        ...
        return "/uv/tools/sarthi-agent/" in norm + "/"
...
def recommended_update_command_for_method(method: str) -> str:
    ...
    if method == "pip":
        if is_uv_tool_install():
            return "uv tool upgrade sarthi-agent"
        import shutil
        if shutil.which("uv"):
            return "uv pip install --upgrade sarthi-agent"
        return "pip install --upgrade sarthi-agent"
    return "sarthi update"
...
# banner, the TUI/desktop session info panel, and ``sarthi update``. NixOS
# stays fully supported (Tier 2) and must never hit this path.

PLATFORM_SUPPORT_DOCS_URL = "https://sarthi-agent.vercel.app/docs/getting-started/platform-support"
...
def format_unsupported_install_warning(method: str) -> str:
    ...
    ``sarthi update`` / ``sarthi update --check`` so the wording — and the
...
# Long-form text for ``sarthi update`` / ``--check`` when running inside the
# Docker image.  Surfaced by ``cmd_update`` and ``_cmd_update_check`` in
# sarthi_cli/main.py; lives here so the wording stays consistent and we
...
#     — that script installs a *new* host-side Sarthi, it doesn't update
...
✗ ``sarthi update`` doesn't apply inside the Docker container.

Sarthi Agent runs as a published image (nousresearch/hermes-agent), not a
git checkout — the container has no working tree to pull into.  Update by
pulling a fresh image and restarting your container instead:

  docker pull nousresearch/hermes-agent:latest
  # then restart whatever started the container, e.g.:
  docker compose up -d --force-recreate sarthi-agent
...
Notes:
  ...
  • Your config and session history live under ``$SARTHI_HOME`` (``/opt/data``
...
def format_docker_update_message() -> str:
    """Return the user-facing message for ``sarthi update`` inside Docker.
...
def format_managed_message(action: str = "modify this Sarthi installation") -> str:
    """Build a user-facing error for managed installs."""
    managed_system = get_managed_system() or "a package manager"
    raw = os.getenv("SARTHI_MANAGED", "").strip().lower()

    if managed_system == "NixOS":
        env_hint = "true" if raw in _MANAGED_TRUE_VALUES else raw or "true"
        return (
            f"Cannot {action}: this Sarthi installation is managed by NixOS "
            f"(SARTHI_MANAGED={env_hint}).\n"
            "Edit services.sarthi-agent.settings in your configuration.nix and run:\n"
    ...
    if managed_system == "Homebrew":
        ...
        return (
            f"Cannot {action}: this Sarthi installation is managed by Homebrew "
            f"(SARTHI_MANAGED={env_hint}).\n"
            "Use:\n"
            "  brew upgrade sarthi-agent"
        )

    return (
        f"Cannot {action}: this Sarthi installation is managed by {managed_system}.\n"
        "Use your package manager to upgrade or reinstall Sarthi."
    )
...
def get_container_exec_info() -> Optional[dict]:
    """Read container mode metadata from SARTHI_HOME/.container-mode.

    Returns a dict with keys: backend, container_name, exec_user, sarthi_bin
    or None if container mode is not active, we're already inside the
    container, or SARTHI_DEV=1 is set.

    The .container-mode file is written by the NixOS activation script when
    container.enable = true. It tells the host CLI to exec into the container
    instead of running locally.
    """
    if os.environ.get("SARTHI_DEV") == "1":
        return None
    ...
    container_mode_file = get_sarthi_home() / ".container-mode"
    ...
    container_name = info.get("container_name", "sarthi-agent")
    exec_user = info.get("exec_user", "sarthi")
    sarthi_bin = info.get("sarthi_bin", "/data/current-package/bin/sarthi")

    return {
        "backend": backend,
        "container_name": container_name,
        "exec_user": exec_user,
        "sarthi_bin": sarthi_bin,
    }
...
# Re-export from sarthi_constants — canonical definition lives there.
from sarthi_constants import get_sarthi_home  # noqa: F811,E402
from utils import atomic_replace, fast_safe_load

def get_config_path() -> Path:
    """Get the main config file path."""
    return get_sarthi_home() / "config.yaml"

def get_env_path() -> Path:
    """Get the .env file path (for API keys)."""
    return get_sarthi_home() / ".env"

def get_project_root() -> Path:
    """Get the project installation directory."""
    return Path(__file__).parent.parent.resolve()

def _resolve_sarthi_uid_gid() -> tuple[Optional[int], Optional[int]]:
    """Read the SARTHI_UID / SARTHI_GID env vars set by Docker deployments.

    Docker containers running Sarthi commonly set these to map the in-container
    user to a host user so volume-mounted state files end up with the right
    ownership. The entrypoint chowns the top-level SARTHI_HOME once, but
    subdirectories created at runtime by ``ensure_sarthi_home()`` (especially
    ...
    if sys.platform == "win32":
        return None, None
    uid_str = os.environ.get("SARTHI_UID", "").strip()
    gid_str = os.environ.get("SARTHI_GID", "").strip()
    try:
...
def _chown_to_sarthi_uid(path) -> None:
    """Chown ``path`` to ``SARTHI_UID:SARTHI_GID`` if those env vars are set.
    ...
    directories created by :func:`ensure_sarthi_home` on Docker deployments.
    See #34107.
    """
...
def _secure_dir(path):
    ...
    permissions (0750) so interactive users in the sarthi group can
    share state with the gateway service.

    The mode can be overridden via the SARTHI_HOME_MODE environment variable
    (e.g. SARTHI_HOME_MODE=0701) for deployments where a web server (nginx,
    caddy, etc.) needs to traverse SARTHI_HOME to reach a served subdirectory.
    The execute-only bit on a directory permits cd-through without exposing
    directory listings.

    Also applies ``SARTHI_UID``/``SARTHI_GID``-based ownership when those env
    ...
    if is_managed():
        return
    try:
        mode_str = os.environ.get("SARTHI_HOME_MODE", "").strip()
    ...
    try:
        ...
        pass
    _chown_to_sarthi_uid(path)


def _is_container() -> bool:
    """Detect if we're running inside a Docker/Podman/LXC container.

    When Sarthi runs in a container with volume-mounted config files, forcing
    ...
    """
    # Explicit opt-out
    if os.environ.get("SARTHI_CONTAINER") or os.environ.get("SARTHI_SKIP_CHMOD"):
        return True
...
def _secure_file(path):
    ...
    permissions.  Set SARTHI_SKIP_CHMOD=1 to force-skip on other systems.
    """
...
def _ensure_default_soul_md(home: Path) -> None:
    """Seed a default SOUL.md into SARTHI_HOME, upgrading legacy empty templates.
...
def ensure_sarthi_home():
    """Ensure ~/.sarthi directory structure exists with secure permissions.
    ...
    # Named profiles must be created explicitly (e.g. ``sarthi profile create``).
    # If a stale process keeps running after the profile was renamed/deleted,
    # silently mkdir-ing the old SARTHI_HOME would resurrect an empty skeleton
    ...
    if is_managed():
        ...
        try:
            _ensure_sarthi_home_managed(home)
        finally:
            os.umask(old_umask)
    else:
...
def _ensure_sarthi_home_managed(home: Path):
    """Managed-mode variant: verify dirs exist (activation creates them), seed SOUL.md."""
    if not home.is_dir():
        raise RuntimeError(
            f"SARTHI_HOME {home} does not exist. "
...
DEFAULT_CONFIG = {
    ...
    "agent": {
        ...
        # the Sarthi-level retry loop that wraps the whole call.  Lower
        ...
        # prompt's environment-hints block. Lets a host that wraps Sarthi
        ...
        # identity slot (SOUL.md). Empty by default. The SARTHI_ENVIRONMENT_HINT
        # env var overrides this (build-time/container mechanism).
        "environment_hint": "",
        # Coding posture — on interactive coding surfaces (CLI, TUI, desktop
        # app, ACP) in a code workspace, Sarthi adds a coding operating brief
    ...
    "terminal": {
        ...
        #             SARTHI_HOME/home for persistent state (default)
        #   real    — force the real OS-user HOME
        #   profile — force SARTHI_HOME/home when it exists (old strict
        ...
        # skipped. When empty, Sarthi auto-sources ``~/.profile``,
        # ``~/.bash_profile``, and ``~/.bashrc`` (in that order) if the
        # snapshot shell is bash (this is the ``auto_source_bashrc``
        # behaviour — disable with that key if you want strict login-only
        # semantics).
        "shell_init_files": [],
        # When true (default), Sarthi sources the user's shell rc files
        ...
        # docker_env lets you specify exact key-value pairs — useful when Sarthi
        ...
        # bundled Sarthi image, which drops to the `sarthi` user via
    ...
    "browser": {
        ...
        "camofox": {
            # When true, Sarthi sends a stable profile-scoped userId to Camofox
            # so the server maps it to a persistent Firefox profile automatically.
            # When false (default), each session gets a random userId (ephemeral).
            "managed_persistence": False,
            # Optional externally managed Camofox identity. Useful when another
            # app owns the visible browser and Sarthi should operate in it.
            "user_id": "",
            "session_key": "",
    ...
    # Opt in via ``sarthi chat --checkpoints`` or set enabled=True here.
    "checkpoints": {
        "enabled": False,
        ...
        "max_snapshots": 20,
        # Hard ceiling on total ``~/.sarthi/checkpoints/`` size (MB).  When
        ...
        "max_file_size_mb": 10,
        # Auto-maintenance: sarthi sweeps the checkpoint base at startup
    ...
    # AGENTS.md, CLAUDE.md, .sarthi.md, or .cursorrules before Sarthi applies
    ...
    # single read_file page exceeds these limits, Sarthi truncates the
    ...
    "compression": {
        ...
        "codex_app_server_auto": "native",  # Codex app-server (codex CLI runtime) thread
                                      ...
                                      # thread context, so Sarthi' summarizer cannot
                                      # shrink it (#36801). native = codex decides when
                                      # to compact its own thread (default); sarthi =
                                      # Sarthi' compression threshold triggers
    ...
    # See tools/kanban_tools.py and sarthi_cli/kanban_db.py for the actual
    # implementations. Per-platform notification opt-out is handled by the
    # kanban dashboard (see ``sarthi dashboard`` -> Notifications).
    "kanban": {
    ...
    "auxiliary": {
        ...
        # Invoked by ``sarthi kanban specify`` (single id or --all). Set a
        ...
        # Invoked by ``sarthi kanban decompose`` and the kanban
        ...
        # ``sarthi profile describe <name> --auto`` and the dashboard's
        ...
        # model; override via `sarthi model` → auxiliary → Curator to route
    ...
    "display": {
        ...
        # Which interface bare `sarthi` (and `sarthi chat`) launches by default:
        ...
        # REPL and `--tui` (or SARTHI_TUI=1) forces the TUI regardless of config.
        "interface": "cli",
        # When true, `sarthi --tui` auto-resumes the most recent human-
        # facing session on launch instead of forging a fresh one.
        # Mirrors `sarthi -c` muscle memory.  Default off so existing
        # users aren't surprised.  SARTHI_TUI_RESUME=<id> always wins.
        "tui_auto_resume_recent": False,
        # When true (default), `sarthi --tui` drops a one-time hint
        ...
        # can set `path`/`paths` to SARTHI_HOME-relative YAML files/directories
        ...
        # e.g. `model · 68% · ~/projects/sarthi`. Per-platform overrides go under
        ...
        # CLI, TUI, and desktop app. Manage with `sarthi pets`. Disabled until
        ...
        "pet": {
            ...
            # get_sarthi_home()/pets/. Empty → first installed pet.
            "slug": "",
    ...
    "dashboard": {
        ...
        # ``SARTHI_DASHBOARD_OAUTH_CLIENT_ID`` and
        # ``SARTHI_DASHBOARD_PORTAL_URL`` respectively — and the env var
        ...
        # (``SARTHI_DASHBOARD_BASIC_AUTH_USERNAME`` /
        ...
        # ``SARTHI_DASHBOARD_DRAIN_SECRET`` env var (the .env-is-for-secrets
        ...
        # Public URL override (env: ``SARTHI_DASHBOARD_PUBLIC_URL``).
        # When set, this is the complete authority — scheme + host +
        # optional path prefix (e.g. ``https://example.com/sarthi``) —
    ...
    "tts": {
        ...
        "gemini": {
            ...
            # transcript section; Sarthi appends the live transcript when absent.
            "persona_prompt_file": "",
        },
        "xai": {
        ...
        "piper": {
            ...
            # "voices_dir": "",        # Override voice cache dir; default = ~/.sarthi/cache/piper-voices/
    ...
    # a plugin in plugins/context_engine/<name>/ or ~/.sarthi/plugins/.
    "context": {
        "engine": "compressor",
    },
    ...
    "delegation": {
        ...
        # trim, the full text is spilled to ~/.sarthi/cache/delegation/
    ...
    # response. If not, Sarthi feeds a continuation prompt back into the
    ...
    "goals": {
        # Max continuation turns before Sarthi auto-pauses the goal and
    ...
    "moa": {
        ...
        # <sarthi_home>/moa-traces/<session_id>.jsonl. Off by default — turn it
    ...
    # always goes to ~/.sarthi/skills/.
    "skills": {
        "external_dirs": [],   # e.g. ["~/.agents/skills", "/shared/team-skills"]
        # Substitute ${SARTHI_SKILL_DIR} and ${SARTHI_SESSION_ID} in SKILL.md
    ...
    # cron daemon.
    #
    # See `sarthi curator status` for the last run summary.
    "curator": {
        "enabled": True,
        ...
        # class-level umbrellas. `sarthi curator run --consolidate` overrides
        ...
        # normally restored on every `sarthi update`, so pruning them only
        ...
        # skipped), snapshot ~/.sarthi/skills/ into
        # ~/.sarthi/skills/.curator_backups/<utc-iso>/skills.tar.gz so the
        # user can roll back with `sarthi curator rollback`.
        "backup": {
            "enabled": True,
    ...
    # This section is only needed for sarthi-specific overrides; everything else
    ...
    "whatsapp": {
        ...
        # Default (None) uses the built-in "⚕ *Sarthi Agent*" header.
    ...
    "approvals": {
        ...
        # false.  TUI has its own modal overlay (SARTHI_TUI_NO_CONFIRM=1 to
        # opt out there).
    ...
    # replace Sarthi' built-in platform hint for a single messaging platform
    ...
    # stored approval from ~/.sarthi/shell-hooks-allowlist.json.
    # See `website/docs/user-guide/features/hooks.md` for schema + examples.
    "hooks": {},

    # Auto-accept shell-hook registrations without a TTY prompt.  Also
    # toggleable per-invocation via --accept-hooks or SARTHI_ACCEPT_HOOKS=1.
    ...
    "security": {
        ...
        # longer trigger the startup banner. Add via `sarthi doctor --ack
        # <id>`; remove by editing the list directly. See
        # ``sarthi_cli/security_advisories.py`` for the catalog.
        "acked_advisories": [],
        # Allow Sarthi to lazy-install opt-in backend packages from PyPI
    ...
    "cron": {
        ...
        # $SARTHI_HOME/plugins/<name>/) to relocate the trigger — e.g. "chronos",
        ...
        # Also overridable via SARTHI_CRON_MAX_PARALLEL env var.
        ...
        # SARTHI_CRON_SESSION_DB_TIMEOUT env var. 0 = unlimited (skip the bound).
    ...
    # todos to ready, and fires `sarthi -p <assignee> chat -q ...` for
    ...
    "kanban": {
        ...
        # one `sarthi` launches with no -p flag). This does not control the
        ...
        # decomposition is manual via `sarthi kanban decompose <id>` or
    ...
    "code_execution": {
        ...
        #     sarthi-agent's own python (sys.executable). Maximum isolation
    ...
    #
    # Core Sarthi tools (terminal, read_file, write_file, patch,
    ...
    # Logging — controls file logging to ~/.sarthi/logs/.
    ...
    # update model picker lists without shipping a sarthi-agent release.
    # The default URL is served by the docs site GitHub Pages deploy.
    "model_catalog": {
        "enabled": True,
        "url": "https://sarthi-agent.vercel.app/docs/api/model-catalog.json",
        # Disk cache TTL in hours.  Beyond this, the CLI refetches on the
        # next /model or `sarthi model` invocation; network failures
    ...
    "gateway": {
        ...
        # internal SARTHI_GATEWAY_PLATFORM_CONNECT_TIMEOUT env var, which still
        ...
        # producing ~/.sarthi/sessions/sessions.json entirely.
        ...
        # the SARTHI_SCALE_TO_ZERO env stamp) AND messaging is relay-only/absent
        ...
        # `launchctl kickstart ai.sarthi.gateway` that defenses 1-2 don't
        ...
        # system-path denylist (/etc, /proc, ~/.ssh, ~/.aws, ~/.sarthi/.env,
        ...
        # behavior: files must live under the Sarthi cache, under
        ...
        # user. Bridged to SARTHI_MEDIA_DELIVERY_STRICT.
        "strict": False,
        # Extra directories from which model-emitted bare file paths may be
        # uploaded as native gateway attachments. Files inside the Sarthi
        # cache (~/.sarthi/cache/{documents,images,audio,video,screenshots})
        ...
        # to SARTHI_MEDIA_ALLOW_DIRS at gateway startup. Tilde paths are
        ...
        # SARTHI_MEDIA_TRUST_RECENT_FILES. Only consulted when ``strict``
        # is true; in default mode the denylist alone gates delivery.
        "trust_recent_files": True,
        # Recency window in seconds. 600 (10 min) comfortably covers a
        # multi-tool agent turn. Bridged to SARTHI_MEDIA_TRUST_RECENT_SECONDS.
    ...
    # Session storage — controls automatic cleanup of ~/.sarthi/state.db.
    ...
    "sessions": {
        ...
        # default of ``sarthi sessions prune``.
        "retention_days": 90,
        ...
        # rewrites ``~/.sarthi/sessions/session_{sid}.json`` on every turn
    ...
    # ``sarthi update`` behaviour.
    "updates": {
        # Run a full ``sarthi backup``-style zip of SARTHI_HOME before every
        # ``sarthi update``.  Backups land in ``<SARTHI_HOME>/backups/`` and
        # can be restored with ``sarthi import <path>``.  Off by default:
        # zipping a large SARTHI_HOME (sessions DB, caches, skills) can add
        # minutes to every update.  The #48200 incident — a ``sarthi update
        ...
        "backup_keep": 5,
        # What `sarthi update` does with uncommitted local changes to the
    ...
    "lsp": {
        ...
        #              ``<SARTHI_HOME>/lsp/bin/`` on first use.
    ...
    # `sarthi tools`. These settings tune the backing Responses API call.
    "x_search": {
    ...
    # rather than storing them in ~/.sarthi/.env.
    "secrets": {
        ...
        "bitwarden": {
            ...
            # in ~/.sarthi/.env (or your shell) and never in config.yaml.
            ...
            # ~/.sarthi/bin/ on first use.  When False you must install
            ...
            # `sarthi secrets bitwarden setup`.
            "server_url": "",
        },
        "onepassword": {
            ...
            # for headless auth.  Sourced from ~/.sarthi/.env (or the shell)
    ...
    "computer_use": {
        ...
        # by default upstream. Sarthi disables it for our users unless they
        # explicitly opt in here. When false (default), Sarthi sets
    ...
    # Sarthi Desktop (Electron app) launch options. These only affect
    # `sarthi desktop`; they do not touch the CLI/gateway.
    "desktop": {
        ...
        # Bridged to the SARTHI_DESKTOP_DISABLE_GPU env var the Electron app reads.
        "disable_gpu": "auto",
    },
...
OPTIONAL_ENV_VARS = {
    ...
    "VERTEX_CREDENTIALS_PATH": {
        "description": "Path to a Google Cloud service account JSON for Vertex AI (Gemini). "
                       ...
                       "credentials Sarthi mints short-lived tokens from. Falls back to "
    ...
    "AZURE_FOUNDRY_BASE_URL": {
        "description": "Azure Foundry base URL (set via 'sarthi model' for endpoint-specific config)",
    ...
    "TOOL_GATEWAY_USER_TOKEN": {
        "description": "Explicit Nous Subscriber access token for tool-gateway requests (optional; otherwise read from the Sarthi auth store)",
    ...
    "SLACK_ALLOWED_USERS": {
        "description": "Comma-separated Slack member IDs allowed to use Sarthi, e.g. U01ABC2DEF3. Without this, Slack may connect but deny messages by default.",
    ...
    "MATRIX_USER_ID": {
        "description": "Matrix user ID (e.g. @sarthi:example.org)",
    ...
    "MATRIX_DEVICE_ID": {
        "description": "Stable Matrix device ID for E2EE persistence across restarts (e.g. SARTHI_BOT)",
    ...
    "IRC_CHANNEL": {
        "description": "IRC channel to join (e.g. #sarthi)",
        "prompt": "IRC channel",
        "url": None,
        "password": False,
        "category": "messaging",
    ...
    "IRC_NICKNAME": {
        "description": "Bot nickname on IRC (default: sarthi-bot)",
        "prompt": "IRC nickname",
        "url": None,
        "password": False,
        "category": "messaging",
    ...
    "API_SERVER_MODEL_NAME": {
        "description": "Model name advertised on /v1/models. Defaults to the profile name (or 'sarthi-agent' for the default profile). Useful for multi-user setups with OpenWebUI.",
    ...
    "GATEWAY_PROXY_URL": {
        "description": "URL of a remote Sarthi API server to forward messages to (proxy mode). When set, the gateway handles platform I/O only — all agent work is delegated to the remote server. Use for Docker E2EE containers that relay to a host agent. Also configurable via gateway.proxy_url in config.yaml.",
        "prompt": "Remote Sarthi API server URL (e.g. http://192.168.1.100:8642)",
        "url": None,
        "password": False,
        "category": "messaging",
        "advanced": True,
    ...
    "GATEWAY_PROXY_KEY": {
        "description": "Bearer token for authenticating with the remote Sarthi API server (proxy mode). Must match the API_SERVER_KEY on the remote host.",
    ...
    # SARTHI_TOOL_PROGRESS and SARTHI_TOOL_PROGRESS_MODE are deprecated —
    ...
    "SARTHI_EPHEMERAL_SYSTEM_PROMPT": {
...
def _is_env_config_key(key: str) -> bool:
    """Return whether `sarthi config set` routes this key to .env."""
    if "." in key:
        return False
    key_upper = key.upper()
    api_keys = [
...
def get_missing_skill_config_vars() -> List[Dict[str, Any]]:
    ...
    Scans all enabled skills for ``metadata.sarthi.config`` entries, then checks
    ...
    try:
        ...
        # should never break `sarthi update`.  Skill-config prompting is a
...
def _normalize_custom_provider_entry(
    ...
    *,
    ...
    _KNOWN_KEYS = {
        ...
        # unused here, but Sarthi' own config writer has historically emitted it
    ...
    if isinstance(models, dict) and models:
        ...
        # Hand-edited configs (and older Sarthi versions) may write
...
def get_custom_provider_context_length(
    ...
    used by:
      ...
      * ``sarthi_cli.model_switch.resolve_display_context_length`` (``/model`` confirmation display)
...
def validate_config_structure(config: Optional[Dict[str, Any]] = None) -> List["ConfigIssue"]:
    ...
    if config is None:
        try:
            ...
            return [ConfigIssue("error", "Could not load config.yaml", "Run 'sarthi setup' to create a valid config")]
    ...
    if cp and not model_cfg:
        issues.append(ConfigIssue(
            "warning",
            "custom_providers defined but no 'model' section — Sarthi won't know which provider to use",
            "Add a model section:\n"
            "  model:\n"
            "    provider: custom\n"
...
def print_config_warnings(config: Optional[Dict[str, Any]] = None) -> None:
    ...
    lines.append("  \033[2mRun 'sarthi doctor' for fix suggestions.\033[0m")
    sys.stderr.write("\n".join(lines) + "\n\n")
...
def warn_deprecated_cwd_env_vars(config: Optional[Dict[str, Any]] = None) -> None:
    ...
    if lines:
        hint_path = os.environ.get("SARTHI_HOME", "~/.sarthi")
...
def _persist_migration(config: Dict[str, Any]) -> None:
    ...
    DEFAULT_CONFIG dumps (the "sarthi update / sarthi -p blows up my config"
    reports).
...
def migrate_config(interactive: bool = True, quiet: bool = False) -> Dict[str, Any]:
    ...
    if current_ver < 4:
        ...
        if "tool_progress" not in display:
            old_enabled = get_env_value("SARTHI_TOOL_PROGRESS")
            old_mode = get_env_value("SARTHI_TOOL_PROGRESS_MODE")
            if old_enabled and old_enabled.lower() in {"false", "0", "no"}:
                display["tool_progress"] = "off"
                results["config_added"].append("display.tool_progress=off (from SARTHI_TOOL_PROGRESS=false)")
            elif old_mode and old_mode.lower() in {"new", "all", "verbose"}:
                display["tool_progress"] = old_mode.lower()
                results["config_added"].append(f"display.tool_progress={old_mode.lower()} (from SARTHI_TOOL_PROGRESS_MODE)")
            else:
    ...
    if current_ver < 5:
        ...
        if "timezone" not in config:
            old_tz = os.getenv("SARTHI_TIMEZONE", "")
            if old_tz and old_tz.strip():
                config["timezone"] = old_tz.strip()
                results["config_added"].append(f"timezone={old_tz.strip()} (from SARTHI_TIMEZONE)")
            else:
                config["timezone"] = ""
    ...
    if current_ver < 21:
        ...
        if "enabled" not in plugins_cfg:
            ...
            # Scan ``$SARTHI_HOME/plugins/`` for currently installed user plugins.
            grandfathered: List[str] = []
            try:
                user_plugins_dir = get_sarthi_home() / "plugins"
            ...
            if not quiet:
                if grandfathered:
                    ...
                    print(
                        ...
                        "Use `sarthi plugins enable <name>` to activate."
                    )
    ...
    # settings in their `config.yaml`, and `sarthi curator status` has no
    ...
    #   3. Creates `~/.sarthi/logs/curator/` if missing (belt-and-suspenders
    #      on top of ensure_sarthi_home() — old profiles that predate this
    #      migration still benefit).
    if current_ver < 23:
        try:
            curator_dir = get_sarthi_home() / "logs" / "curator"
            ...
            if added_curator:
                ...
                if not quiet:
                    print(
                        ...
                        f"({', '.join(added_curator)}) — edit via `sarthi config set`"
                    )
            if added_aux:
                results["config_added"].append(
                ...
                if not quiet:
                    print(
                        ...
                        f"({', '.join(added_aux)}) — edit via `sarthi config set`"
                    )
    ...
    # behavior set it to true explicitly via `sarthi config set`.
    ...
    if isinstance(raw_mcp_servers, dict):
        try:
            from sarthi_cli.mcp_security import validate_mcp_server_entry as _validate_mcp_server_entry
        except Exception:
    ...
    if new_var_names and interactive and not quiet:
        ...
        if new_and_unset:
            ...
            if answer in {"y", "yes"}:
                ...
                print("  Set later with: sarthi config set <key> <value>")
    ...
    # `sarthi update`, but only the version bump is persisted.
    ...
    # metadata.sarthi.config in their SKILL.md frontmatter.
...
def _env_ref_snapshot(obj, snapshot=None):
    ...
    ``load_sarthi_dotenv()`` populated the process env, or an env var
...
def _normalize_root_model_keys(config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    intuitive name OpenAI-SDK / LiteLLM users reach for, and ``sarthi config set``
    ...
    backends) — while display paths (``sarthi status``/``dump``) read ``name``
...
def cfg_get(cfg: Optional[Dict[str, Any]], *keys: str, default: Any = None) -> Any:
    ...
    ubiquitous ``cfg_path = _sarthi_home / "config.yaml"`` local variable
...
def read_raw_config() -> Dict[str, Any]:
    """Read ~/.sarthi/config.yaml as-is, without merging defaults or migrating.
...
def load_config() -> Dict[str, Any]:
    """Load configuration from ~/.sarthi/config.yaml.
    ...
    (which change ``SARTHI_HOME`` and therefore ``get_config_path()``)
    don't collide.
...
def _load_config_impl(*, want_deepcopy: bool) -> Dict[str, Any]:
    with _CONFIG_LOCK:
        ...
        # cache signature so editing /etc/sarthi/config.yaml invalidates the
        ...
        if cached is not None and cache_sig is not None and cached[:4] == cache_sig:
            ...
            # Without this, a load_config() that ran before load_sarthi_dotenv()
...
#   openai-codex (OAuth — sarthi auth) — OpenAI Codex
#   nous         (OAuth — sarthi auth) — Nous Portal
...
def save_config(
    ...
    *,
    ...
    """Save configuration to ~/.sarthi/config.yaml.\n
...
def _parse_env_value(raw_value: str) -> str:
    """Parse the small .env value subset Sarthi writes itself."""
...
def load_env() -> Dict[str, str]:
    """Load environment variables from ~/.sarthi/.env.
    ...
    interactive menu render (`sarthi tools`, `sarthi setup`, status
    panels). Sanitisation is O(lines × known-keys), so re-parsing the
    same file on every call was burning ~300ms of CPU per `sarthi tools`
...
def _sanitize_env_lines(lines: list) -> list:
    ...
    split on real Sarthi env var names, avoiding false positives from values
...
def sanitize_env_file() -> int:
    """Read, sanitize, and rewrite ~/.sarthi/.env in place.
...
def save_env_value(key: str, value: str):
    """Save or update a value in ~/.sarthi/.env."""
    if is_managed():
...
def remove_env_value(key: str) -> bool:
    """Remove a key from ~/.sarthi/.env and os.environ.
...
def reload_env() -> int:
    """Re-read ~/.sarthi/.env into os.environ. Returns count of vars updated.

    Adds/updates vars that changed and removes vars that were deleted from
    the .env file (but only vars known to Sarthi — OPTIONAL_ENV_VARS and
    ...
    for key, value in env_vars.items():
        if os.environ.get(key) != value:
            os.environ[key] = value
            count += 1
    # Remove known Sarthi vars that are no longer in .env
...
def get_env_value(key: str) -> Optional[str]:
    """Get a value from ~/.sarthi/.env or environment."""
...
def get_env_value_prefer_dotenv(key: str) -> Optional[str]:
    """Resolve a credential env value, preferring ``~/.sarthi/.env`` over ``os.environ``.

    Used for Sarthi-managed credentials where a deliberate edit to ``.env``
...
def show_config():
    ...
    print(color("│              ⚕ Sarthi Configuration                    │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
    ...
    from sarthi_cli.auth import get_anthropic_key
    ...
    # Warn on stale SARTHI_MAX_ITERATIONS ghost in .env that disagrees with
    ...
    try:
        _env_ghost = load_env().get("SARTHI_MAX_ITERATIONS")
        if _env_ghost is not None and str(_env_ghost).strip() != str(_cfg_max_turns).strip():
            print(color(
                f"                ⚠ .env has stale SARTHI_MAX_ITERATIONS={_env_ghost} "
                f"(run 'sarthi doctor --fix' to remove)",
                Colors.YELLOW,
            ))
    except Exception:
        pass
    ...
    print(color("  sarthi config edit     # Edit config file", Colors.DIM))
    print(color("  sarthi config set <key> <value>", Colors.DIM))
    print(color("  sarthi setup           # Run setup wizard", Colors.DIM))
    print()
...
def set_config_value(key: str, value: str):
    ...
    # so a fresh `sarthi config set model.api_base ...` lands on the canonical
    ...
    # — e.g. `sarthi config set model.api_key cfut_...` routes to config.yaml
...
def config_command(args):
    ...
    if subcmd is None or subcmd == "show":
        ...
        if not key:
            print("Usage: sarthi config get <key> [--json]")
            print()
            print("Examples:")
            print("  sarthi config get model")
            print("  sarthi config get terminal.backend")
            print("  sarthi config get skills.config --json")
            sys.exit(1)
        ...
        if not key or value is None:
            print("Usage: sarthi config set <key> <value>")
            print()
            print("Examples:")
            print("  sarthi config set model anthropic/claude-sonnet-4")
            print("  sarthi config set terminal.backend docker")
            print("  sarthi config set OPENROUTER_API_KEY sk-or-...")
            sys.exit(1)
        ...
        key = getattr(args, 'key', None)
        if not key:
            print("Usage: sarthi config unset <key>")
            print()
            print("Examples:")
            print("  sarthi config unset model")
            print("  sarthi config unset terminal.backend")
            print("  sarthi config unset OPENROUTER_API_KEY")
            sys.exit(1)
        ...
        if missing_config:
            ...
            print("    Run 'sarthi config migrate' to add them")
        ...
        print("  sarthi config           Show current configuration")
        print("  sarthi config edit      Open config in editor")
        print("  sarthi config get <key>          Print a resolved config value")
        print("  sarthi config set <key> <value>   Set a config value")
        print("  sarthi config unset <key>        Remove a config value")
        print("  sarthi config check     Check for missing/outdated config")
        print("  sarthi config migrate   Update config with new options")
        print("  sarthi config path      Show config file path")
        print("  sarthi config env-path  Show .env file path")
        sys.exit(1)
...
# ``_inject_profile_env_vars`` surfaces them in ``sarthi config`` UI so users
