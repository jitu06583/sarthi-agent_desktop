...
from sarthi_cli.route_identity import normalize_route_base_url
...
_EXTRA_ENV_KEYS = frozenset({
    ...
    # SARTHI_TOOL_PROGRESS_MODE is deprecated (replaced by display.tool_progress
    ...
    "SARTHI_ACP_AUTH_METHOD",
    "SARTHI_ACP_AUTO_APPROVE",
    "SARTHI_COPILOT_ACP_COMMAND",
    "SARTHI_COPILOT_ACP_ARGS",
    "COPILOT_CLI_PATH",
...
# from `nix run` / `nix profile install` (which don't set SARTHI_MANAGED).
...
def get_managed_system() -> Optional[str]:
    ...
    if raw:
        ...
        # SARTHI_MANAGED variable of the service. A marker with content
...
_NIX_UPDATE_MSG = (
    "Update Sarthi through the Nix source that installed it "
...
def _install_method_project_root(project_root: Optional[Path] = None) -> Path:
    ...
    installs, ``/opt/sarthi`` inside the published image. It is a property of
    the running interpreter, NOT of ``$SARTHI_HOME``, which is why a
...
def detect_install_method(project_root: Optional[Path] = None) -> str:
    """Detect how Sarthi was installed: 'apt', 'docker', 'nix', 'nixos',
    ...
    3. SARTHI_MANAGED env / .managed marker (NixOS managed mode)
    ...
    # detect Nix installs that don't set SARTHI_MANAGED (e.g. ``nix run``,
...
def recommended_update_command_for_method(method: str) -> str:
    ...
    if method == "apt":
        ...
        return "pkg upgrade sarthi-agent"
...
from sarthi_constants import get_sarthi_home, get_process_sarthi_home  # noqa: F811,E402
...
# ensure_sarthi_home(). Only successful passes are recorded, so a raised
# managed-mode/missing-profile error keeps re-checking on later loads.
_SARTHI_HOME_ENSURED: set = set()
...
def ensure_sarthi_home():
    ...
    After the first successful pass for a given ``SARTHI_HOME`` we only re-run
    ...
    ``get_sarthi_home()`` and therefore re-run for the new path.
    """
    home = get_sarthi_home()
    key = str(home)

    if key in _SARTHI_HOME_ENSURED and home.is_dir():
        return
...
from sarthi_cli.config_defaults import DEFAULT_CONFIG, OPTIONAL_ENV_VARS  # noqa: F401
...
def _normalize_custom_provider_entry(
    ...
    *,
    ...
    # Sarthi (``_save_discovered_models_to_config``), not hand-curated.
    # Older Sarthi versions wrote an in-mapping ``__discovered_model_catalog__``
...
_EXTRA_KNOWN_ROOT_KEYS = {
    ...
    "plugins",           # plugin enable/disable lists (sarthi_cli/plugins_cmd.py)
    "smart_model_routing",   # written by the setup wizard (sarthi_cli/setup.py)
    "platform_toolsets",     # written by the setup wizard (sarthi_cli/setup.py)
    "known_plugin_toolsets", # written/read by sarthi_cli/tools_config.py toolset-save flow
...
def validate_config_structure(config: Optional[Dict[str, Any]] = None) -> List["ConfigIssue"]:
    ...
    # scalars are bridged into os.environ (gateway/run.py, sarthi send) so
...
def migrate_config(interactive: bool = True, quiet: bool = False) -> Dict[str, Any]:
    ...
    from sarthi_cli.config_migrations import (
    ...
    if floor_refused:
        ...
        sys.stderr.write(f"⚠ sarthi config: {msg}\n")
        if not quiet:
            print(f"  ⚠ {msg}")
    else:
        # ── Versioned migration ladder (table-driven) ──
        # The per-version steps live in sarthi_cli.config_migrations as a
    ...
    * ``${env:VAR}`` — Cursor-style SecretRef, same resolution after the
    ...
    if inner.startswith("env:"):
        ...
        if val is not None:
            return val
        logger.warning(
            "Config ref %r: %s is not set (check ~/.sarthi/.env); "
...
def split_model_config_default(raw_default: Any) -> tuple[str, str]:
    ...
    together through ``SarthiCLI`` construction: the model becomes a plain
...
#   - survives the ``str() → int()`` round-trip through the SARTHI_MAX_ITERATIONS
...
# Back-compat alias — canonical set lives in sarthi_cli.personality.
from sarthi_cli.personality import NEUTRAL_PERSONALITY_NAMES as _NEUTRAL_PERSONALITY_NAMES  # noqa: F401


def _prompt_text(value: Any) -> str:
    """Normalize config prompt values from YAML before handing them to AIAgent.

    Delegates to :mod:`sarthi_cli.personality` — the single owner of
    personality/overlay semantics. Kept as a re-export for existing importers.
    """
    from sarthi_cli.personality import prompt_text
...
def render_personality_prompt(value: Any) -> str:
    ...
    from sarthi_cli.personality import render_personality_prompt as _render
...
def resolve_ephemeral_system_prompt_from_config(cfg: Optional[Dict[str, Any]]) -> str:
    ...
    still prefer ``SARTHI_EPHEMERAL_SYSTEM_PROMPT`` when that env var is set.

    Delegates to :mod:`sarthi_cli.personality` (single owner).
    """
    from sarthi_cli.personality import resolve_ephemeral_system_prompt
...
def read_user_config_raw(config_path: Optional[Path] = None) -> Dict[str, Any]:
    ...
    ``_sarthi_home``, tui profile override, multi-profile probes).
    """
...
def _is_ssh_remote_tilde_cwd(backend: str, cwd: str) -> bool:
    ...
    Expanding ``~`` on the Sarthi host rewrites it to the host or container
...
def custom_endpoint_key_env(identity: str) -> str:
    ...
    - The fixed ``SARTHI_CUSTOM_`` prefix keeps the result a valid POSIX name
    ...
    return f"SARTHI_CUSTOM_{slug}_API_KEY" if slug else "SARTHI_CUSTOM_API_KEY"
...
def get_env_value(key: str) -> Optional[str]:
    """Get a value from ``os.environ`` or ``~/.sarthi/.env``, scope-aware.
...
def show_config():
    ...
    try:
        from sarthi_cli.personality import active_personality_name
...
def resolve_cron_model_drift_defaults(
    ...
    *,
    ...
    wins ``SARTHI_MODEL``; the environment is only a fallback. Per-job and cron
    ...
    provider = ""
    model = _model_assignment_text(env.get("SARTHI_MODEL", ""))
...
def warn_unpinned_cron_jobs_after_model_config_change(
    ...
    print(
        ...
        "changed model/provider. Inspect with `sarthi cron list`, then pin the "
        "intended values with `sarthi cron edit <job_id> --provider <provider> "
        "--model <model>`."
    )
...
_SCHEMA_DEFINED_DICT_KEYS = frozenset({
    ...
    # (sarthi_cli/plugins_cmd.py, sarthi_cli/plugin_index.py). Absent from
    # DEFAULT_CONFIG (written only when used), so listed here for
    # `sarthi config set plugins.index_url ...` validation.
    "plugins",
})
...
def _known_top_level_keys() -> set[str]:
    ...
    to decide whether a ``sarthi config set`` invocation is targeting a
    known shape.
    """
...
def _suggest_closest_key(key: str, candidates: set[str], cutoff: float = 0.6) -> Optional[str]:
    ...
    similar enough to ``key``, else None.  Used by ``sarthi config set``
...
def set_config_value(key: str, value: str, force: bool = False):
    ...
    Args:
        ...
        force: When True, skip the unknown-key warning — useful for scripted
            ...
            CLI exposes this via ``sarthi config set --force``.
    """
    if is_managed():
    ...
    if config_path.exists():
        try:
            ...
            print(
                ...
                f"  (sarthi config edit will open it in your editor.)",
                file=sys.stderr,
            )
            sys.exit(1)
    ...
    if not isinstance(_default_value_for_key(key), str):
        ...
        if _lower in {'true', 'yes', 'on'}:
            ...
            #   sarthi config set platform_toolsets.line '["file","web"]'
            # or a multi-line YAML block:
            #   sarthi config set custom_providers '- name: foo
            #     base_url: https://...'
    ...
    # ``sarthi config set model.provider openai`` doesn't silently
    ...
    # (e.g. ``sarthi config set model gpt-5.6-sol`` when model already
    ...
    if "." not in key:
        ...
        if isinstance(_existing, dict):
            if key == "model":
                ...
                print(
                    f"    sarthi config set {key}.<sub-key> <value>",
                    file=sys.stderr,
                )
                print(
                ...
                print(
                    f"    sarthi config set --force {key} {value!r}",
                    file=sys.stderr,
                )
                sys.exit(1)
    ...
    if key == "display.skin" and isinstance(value, str) and value:
        try:
            skin_file = get_sarthi_home() / "skins" / f"{value}.yaml"
            if skin_file.exists():
                skin_file.touch()
        except Exception:
...
def unset_config_value(key: str):
    ...
    if _is_env_config_key(key):
        ...
        # model-cache rows too, so `sarthi config unset <KEY>` fully removes
...
def config_command(args):
    ...
    if subcmd is None or subcmd == "show":
        ...
        if not key or value is None:
            print("Usage: sarthi config set [--force] <key> <value>")
            print()
            print("Examples:")
