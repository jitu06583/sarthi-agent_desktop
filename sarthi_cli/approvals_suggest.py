"""``sarthi approvals suggest`` — mine approval history into allowlist proposals.

Sarthi has no dedicated approval-decision ledger: ``always`` answers land in
...
is the session DB (``~/.sarthi/state.db``): every assistant ``terminal`` tool
...
* **Destructive / privilege / credential / obfuscation classes are never
  ...
  restarts, sarthi self-management, …) are eligible.
...
def normalize_command(command: str) -> str:
    """Fold user/sarthi home prefixes and collapse whitespace.

    Reuses tools.approval's home-folding machinery so proposals are portable
    across machines/users (``/home/alice/x`` -> ``~/x``).
    """
    from tools.approval import (
        _rewrite_resolved_sarthi_home,
        _rewrite_resolved_user_home,
    )

    folded = _rewrite_resolved_user_home(_rewrite_resolved_sarthi_home(command))
...
def _render_text(proposals: list[Proposal], days: int) -> None:
    ...
    print(
        ...
        "  sarthi approvals suggest --apply 1,3\n"
        "Entries are merged into command_allowlist in ~/.sarthi/config.yaml."
    )


def suggest_command(args) -> int:
    """Entry point for ``sarthi approvals suggest``."""
...
def approvals_command(args) -> int:
    """Dispatch ``sarthi approvals <subcommand>``."""
    sub = getattr(args, "approvals_command", None)
    if sub == "suggest":
        return suggest_command(args)
    if sub == "test":
        from sarthi_cli.approvals_test import approvals_test_command
        return approvals_test_command(args)
    print(
        "usage: sarthi approvals <subcommand>\n"
        "\n"
        "subcommands:\n"
        ...
        "\n"
        "Run `sarthi approvals <subcommand> -h` for details."
    )
    return 1
