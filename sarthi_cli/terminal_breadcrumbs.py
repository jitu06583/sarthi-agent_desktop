"""Per-terminal session breadcrumbs for ``sarthi -c`` / ``--continue``.

Each CLI session writes a tiny breadcrumb file
``$SARTHI_HOME/terminal-sessions/<terminal-id>`` containing
``{"session_id": ..., "cwd": ..., "ts": ...}``.  A bare ``sarthi -c`` then
...
def _breadcrumbs_dir() -> Path:
    ...
    return get_sarthi_home() / "terminal-sessions"
