"""
Dump command for sarthi CLI.

Outputs a compact, plain-text summary of the user's Sarthi setup
...
from sarthi_cli.config import get_sarthi_home, get_env_path, get_project_root, load_config
...
def _dotenv_key_names() -> set[str]:
    """Return the set of env-var names assigned a non-empty value in ~/.sarthi/.env.
    ...
    exports. ``sarthi debug share`` runs in a terminal, so ``os.getenv`` reflects
...
def _get_git_commit(project_root: Path) -> str:
    ...
    to ``<project_root>/.sarthi_build_sha`` by the Dockerfile's
    ``SARTHI_GIT_SHA`` build-arg (see ``sarthi_cli/build_info.py``).
...
def _redact(value: str) -> str:
    ...
    ``sarthi dump`` formats empty values as blank, not as ``"(not set)"``).
    """
...
def _gateway_status() -> str:
    ...
    try:
        from sarthi_cli.gateway import get_gateway_runtime_snapshot
...
def _count_skills(sarthi_home: Path) -> int:
...
def _cron_summary(sarthi_home: Path) -> str:
    """Return cron jobs summary."""
    jobs_file = sarthi_home / "cron" / "jobs.json"
...
def run_dump(args):
    ...
    lines.append(f"sarthi_home:      {display_sarthi_home()}")
    ...
    for env_var, label in api_keys:
        ...
        # Set in this (shell) process but absent from ~/.sarthi/.env: a managed
        ...
        # A credential added via `sarthi auth add openrouter` lives in the
        # credential pool, not as an env var — surface it so the dump doesn't
        # misleadingly read "not set" while `sarthi auth list` shows it (#42130).
    ...
    toolsets = config.get("toolsets", ["sarthi-cli"])
    ...
    lines.append(f"  cron_jobs:          {_cron_summary(sarthi_home)}")
    lines.append(f"  skills:             {_count_skills(sarthi_home)}")
