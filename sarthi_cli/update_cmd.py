"""Sarthi update pipeline — extracted from ``sarthi_cli/main.py``.
...
change is that references to helpers/constants that STAY in ``sarthi_cli.main``
(and to moved-but-test-patched siblings) are routed through ``_m()`` — a lazy
``sarthi_cli.main`` reference — so existing call sites and test monkeypatches
that target ``sarthi_cli.main.<name>`` (``PROJECT_ROOT``, ``_is_windows``,
...
the test-patch surface still resolve on ``sarthi_cli.main``.
...
Imports are one-way: ``sarthi_cli.main`` imports this module, never the reverse
...
def _m():
    """Lazy ``sarthi_cli.main`` reference.

    Lets callers keep patching ``sarthi_cli.main.<helper>`` (the historical
    test surface) and have those patches reach this code path, and defers the
    import so ``sarthi_cli.main`` -> ``sarthi_cli.update_cmd`` stays one-way
    at import time.
    """
    from sarthi_cli import main
...
#: ``_purge_stale_sarthi_modules`` so any LATER import chain resolves against
...
_STALE_PURGE_PROTECTED = frozenset(
    {
        ...
        "sarthi_cli.sarthi_logging",
    }
)


def _purge_stale_sarthi_modules() -> None:
    """Evict every cached Sarthi module after the checkout changed in-place.

    ``sarthi update`` keeps running in the pre-pull Python process. The
    gateway auto-restart phase that follows does function-level
    ``from sarthi_cli.gateway import ...`` — executing NEW source inside an
    ...
    field failure: freshly-pulled ``sarthi_cli.gateway`` does
    ``from sarthi_cli.cli_output import line_input``, but ``cli_output`` was
    ...
    export. This is the class fix: drop EVERY cached module under the Sarthi
    ...
    try:
        ...
        if purged:
            logger.debug(
                "Purged %d stale Sarthi module(s) after checkout update", len(purged)
            )
    except Exception as exc:
        logger.debug("Could not purge stale Sarthi modules: %s", exc)


def _reload_updated_runtime_modules() -> None:
    """Reload update-sensitive modules after the checkout changes in-place.

    ``sarthi update`` keeps running in the pre-pull Python process. After a
...
def _reload_config_modules() -> None:
    ...
    ``sarthi update`` runs in the PRE-pull Python process. After ``git pull``
    ...
    This function force-reloads ``sarthi_cli.config_defaults``,
    ``sarthi_cli.config``, and ``sarthi_cli.config_migrations`` from disk
    so subsequent imports read the UPDATED code.

    It also reloads ``sarthi_cli._subprocess_compat`` and
    ``sarthi_cli.dashboard_procs`` so that post-update dashboard cleanup
    ...
    importlib.invalidate_caches()
    for mod_name in (
        "sarthi_cli.config_defaults",
        "sarthi_cli.config",
        "sarthi_cli.config_migrations",
        "sarthi_cli._subprocess_compat",
...
def _run_config_check_fresh() -> tuple:
    ...
    from sarthi_cli.config import check_config_version
...
def _run_migrate_config_fresh(*, interactive: bool = False, quiet: bool = False) -> dict:
    ...
    """
    _reload_config_modules()
    from sarthi_cli.config import migrate_config
...
def _editable_install_is_current(git_cmd, cwd, pre_pull_sha: str | None) -> bool:
    ...
    On Windows that rewrite is the only reason the running ``sarthi.exe`` has
    ...
    Skipping is safe because Sarthi pins its editable finder to a *static*
...
def _validate_critical_modules_import(root) -> tuple[bool, str | None, str | None]:
    ...
    ``_venv_core_imports_healthy``): ``sarthi update`` can be driven by a
    ...
    """
    from sarthi_constants import FIRST_PARTY_MODULE_ROOTS
    ...
    probe = (
        ...
        # The root set is injected from sarthi_constants so this can't drift
        # from the hint the user is shown (they disagreed once already).
        "        missing = (getattr(exc, 'name', '') or '').split('.')[0]\n"
        "        if missing in %r or missing.startswith('sarthi_'):\n"
...
def _print_fts_optimize_available_notice() -> None:
    """Advertise the opt-in v23 search-index optimization after `sarthi update`.
    ...
    try:
        ...
        return
    db_path = get_sarthi_home() / "state.db"
    ...
    if interrupted:
        ...
        print(
            "  A previous `sarthi sessions optimize-storage` run was "
            "interrupted. Search still works; re-run the command to resume "
            "and finish reclaiming disk:"
        )
        print("    sarthi sessions optimize-storage")
        return
    ...
    print("  Run when convenient:  sarthi sessions optimize-storage")
    print(
...
def _reload_process_scan_modules() -> None:
    ...
    ``from sarthi_cli._subprocess_compat import bounded_probe_run``. If the
...
def _atomic_replace_dir(src: str, dst: str) -> None:
    ...
    ``sarthi_cli.main`` re-export surface and guards the #49145 regression.
    """
...
def _discard_staged(staged) -> None:
    ...
    the tree. The user then follows the "re-run `sarthi update`" advice with
...
def _commit_staged_replacements(staged) -> None:
    ...
    first-party modules (``run_agent.py``, ``cli.py``, ``sarthi_constants.py``
...
def _assess_parked_branch_switch(
    ...
    stale feature branch left behind by earlier tooling; ``sarthi update``
...
def _print_parked_branch_skip_warning(
    ...
    print(f"    git -C {cwd} checkout {target_branch} && sarthi update")
    print(
...
def _print_update_completion(message: str) -> None:
    ...
    action_id = os.environ.get("SARTHI_ACTION_ID", "")
    if len(action_id) == 32 and all(char in "0123456789abcdef" for char in action_id):
        print(f"=== sarthi-update completed {action_id} ===")
...
def _refuse_update_for_contended_shims(exc: BaseException) -> None:
    ...
    """
    print("✗ Cannot continue the update: live Sarthi launcher(s) could not be")
    print("  moved aside:")
    for name in getattr(exc, "failed_shims", []) or ["sarthi.exe"]:
        print(f"    {name}")
    print("  Another process is holding this install's venv — typically Sarthi")
    print("  Desktop, a gateway, or another sarthi REPL — and mutating the venv")
    print("  now would strand it half-updated.")
    print("  The dependency install has been deferred: close the process(es)")
    print("  above, then run any `sarthi` command to finish it automatically.")
...
def _should_zip_fallback_on_update_error(exc: BaseException) -> bool:
    ...
    A dependency-install failure (locked ``sarthi.exe`` / ``uv pip install``
...
def _zip_overlay_block_reason(
    ...
    two-phase replace creates ``*.sarthi-update-staging`` siblings inside the
...
_ZIP_STAGING_ARTIFACT_SUFFIXES = (".sarthi-update-staging", ".sarthi-update-old")
...
def _abort_zip_update_if_dirty_tree() -> None:
    ...
    print("  Stash or commit your changes, then rerun `sarthi update`.")
...
def _update_via_zip(args, *, had_desktop_app_before_update: bool = False) -> bool:
    ...
    try:
        ...
        try:
            for item in entries:
                ...
                # Sarthi.exe) exists only in the LIVE tree. Swapping `apps`
        ...
        print(
            "  Re-run `sarthi update` to retry; if the agent won't start, "
    ...
    if not import_ok:
        ...
        print("  Re-run `sarthi update` to complete it.")
        _m().sys.exit(1)
    ...
    try:
        from sarthi_cli.backup import _quick_snapshot_root, verify_sqlite_integrity

        _state_path = get_sarthi_home() / "state.db"
        if _state_path.exists():
            ...
            if not _state_ok.get("valid"):
                ...
                _snap_root = _quick_snapshot_root(get_sarthi_home())
    ...
    try:
        from sarthi_cli.update_receipt import finalize_update_receipt
...
def _park_stashed_changes(stash_ref: str) -> None:
    ...
    Used by ``sarthi update --keep-stash`` (the desktop updater's mode): the
...
def _capture_active_tool_dependencies() -> list[str]:
    """Snapshot Python dependencies installed explicitly through ``sarthi tools``."""
    try:
        from sarthi_cli import tools_config

        return tools_config.active_restorable_python_tool_dependencies()
    except Exception as exc:
        logger.debug("Could not snapshot active Sarthi Tools dependencies: %s", exc)
        return []
...
def _restore_active_tool_dependencies(
    ...
    *,
    ...
    """Restore allowlisted ``sarthi tools`` dependencies into a rebuilt venv.
    ...
    try:
        ...
        logger.debug("Sarthi Tools dependency restore skipped (import failed): %s", exc)
        return
    ...
    print()
    print(f"→ Restoring {len(missing)} Sarthi Tools dependency set(s)...")
    restored: list[str] = []
...
def _refresh_active_lazy_features(
    ...
    *,
    ...
    if install_cmd_prefix is None:
        print("  ⚠ Lazy refresh failed; rerun `sarthi update` once resolved.")
        return False
    ...
    if status == "healthy":
        ...
        print("  Rerun `sarthi update` once the upstream issue is resolved.")
        return True
...
def _refresh_active_memory_provider_dependencies() -> None:
    ...
    ``hindsight-all``), NOT in Sarthi' editable-install extras or
    ...
    try:
        from sarthi_cli.memory_setup import _install_dependencies
    except Exception as exc:
...
def _npm_lockfile_changed(sarthi_root: Path) -> bool:
    ...
    # landed must NOT skip the reinstall — otherwise every later `sarthi
...
def _repair_node_deps_on_current_checkout(print_completion) -> None:
    ...
    sarthi update" — but the early return never reached the Node refresh,
    ...
    if node_failures:
        ...
        print("    Fix npm and re-run `sarthi update`.")
        print_completion(
...
def _update_node_dependencies() -> list[str]:
    ...
    # `sarthi update` case. Synchronous and can block ~11s on a true cold
    ...
    if not _m()._npm_lockfile_changed(shared_sarthi_root):
    ...
    nixos_env = with_sarthi_node_path(_m()._nixos_build_env())
    ...
    # `sarthi update` comes from the *desktop* build, not this step; that
...
def _cmd_update_check(branch: str = "main", *, branch_explicit: bool = False):
    ...
    from sarthi_cli.gitlock import clear_stale_git_locks
    ...
    if is_shallow:
        ...
        if head_sha and target_sha and head_sha == target_sha:
            ...
            from sarthi_cli.banner import _github_compare_behind
...
def _ensure_acp_launcher() -> None:
    ...
    On Windows, restore missing ``sarthi.exe`` / ``sarthi-acp.exe`` copies in
    the dedicated ``<install>\\bin`` directory.  Existing files are not
    overwritten because ``bin\\sarthi.exe`` may be the currently running
    update launcher.

    Mirrors the launcher block in ``scripts/install.sh`` so existing installs
    gain the ACP command on ``sarthi update`` without a reinstall.  ACP hosts
    (Zed, JetBrains, Buzz Desktop) spawn the agent by resolving the
    ``sarthi-acp`` command name against the login-shell PATH; the console
    script of that name lives inside the install's venv, which is not on that
    PATH, so those hosts report Sarthi as not installed even when it is.

    The shim simply delegates to the sibling ``sarthi`` launcher with the
    ...
    On POSIX, the ACP shim is skipped wherever a ``sarthi-acp`` is already
    present next to the ``sarthi`` command.  Unwritable POSIX directories
    (e.g. ``/usr/local/bin`` as non-root) are skipped silently.  Idempotent.
    """
    if _m().sys.platform == "win32":
        from sarthi_cli._install_repair import _sync_windows_cli_launchers
    ...
    for bin_dir in (Path.home() / ".local" / "bin", Path("/usr/local/bin")):
        sarthi_cmd = bin_dir / "sarthi"
        acp_cmd = bin_dir / "sarthi-acp"
        try:
            if not (sarthi_cmd.is_file() or sarthi_cmd.is_symlink()):
                continue
...
def _run_pre_update_backup(args) -> Optional[str]:
    ...
    - ``full``  — the quick snapshot PLUS a full zip of SARTHI_HOME under
      ``backups/`` (restorable via ``sarthi import``; the #48200 wrong-path
    ...
    snapshot_id = None
    try:
        from sarthi_cli.backup import (
            _quick_snapshot_root,
            create_quick_snapshot,
            verify_sqlite_integrity,
        )

        # NOTE: this function later does `from sarthi_constants import
        # get_sarthi_home`, which makes the name function-local — the
        # module-level import is shadowed and unbound here. Alias explicitly.
        from sarthi_cli.config import get_sarthi_home as _get_home
        ...
        try:
            from sarthi_cli.backup import create_pre_update_snapshots_all_profiles
            ...
            if _sibling_snaps:
                ...
                try:
                    from sarthi_cli.update_receipt import record_step
    ...
    # Human-readable size
    from sarthi_cli.sizefmt import format_bytes
...
def _venv_core_imports_healthy() -> tuple[bool, str]:
    ...
    if not venv_python.exists():
        ...
        managed_markers = (
            _m().PROJECT_ROOT / ".sarthi-bootstrap-complete",
...
def _detect_venv_python_processes(
    *, exclude_pids: set[int] | None = None
    ...
    for proc in proc_iter:
        ...
        # the `-m sarthi_cli.main gateway run` argv, so autostarted gateways
...
# ``cryptography`` is the canonical case: ``sarthi_cli.main`` used to import
...
def _abort_dependency_sync_if_self_locked(gateway_resume=None) -> None:
    ...
    - The ``sarthi.exe`` console shim we were launched from (#88838, #89599).
      The marker cannot help here — every future ``sarthi`` launch is also the
...
def _defer_update_for_self_lock(loaded: list[str]) -> None:
    ...
    print("  sync has been deferred: the next `sarthi` launch will complete it")
...
def _sarthi_holder_subcommand(cmdline: str) -> str | None:
    """The actual Sarthi SUBCOMMAND a venv-holder argv runs, or None.

    Token-based, never substring (#90778: ``kanban --preserve-cache``
    contained \"serve\" and got labeled as the Desktop backend). Finds the
    ``sarthi_cli.main`` / ``sarthi(.exe)`` entry token, then returns the
    ...
    for i, token in enumerate(tokens):
        ...
        if low.endswith("sarthi_cli.main") and i > 0 and tokens[i - 1] == "-m":
            entry_idx = i
            break
        base = low.rsplit("\\", 1)[-1].rsplit("/", 1)[-1]
        if base in ("sarthi", "sarthi.exe"):
            entry_idx = i
            break
...
def _format_venv_python_holders_message(matches: list[tuple[int, str, str]]) -> str:
    ...
    (#90778): a standalone ``sarthi dashboard`` must not be labeled as the
    ...
    hint_by_subcommand = {
        "serve": "  ← Sarthi backend (if the Desktop app is open, close it)",
        "dashboard": "  ← sarthi dashboard (stop it: sarthi dashboard stop, or close that terminal)",
        "gateway": "  ← gateway",
    }
    for pid, name, cmdline in matches[:6]:
        sub = _sarthi_holder_subcommand(cmdline)
...
def _venv_launcher_ancestors(pids: list[int]) -> list[int]:
    ...
    # Never return ourselves or our own ancestry: a CLI ``sarthi update``
...
def _leftover_pausable_gateway_pids(
    ...
    """
    from sarthi_cli._scan_venv_blockers import _is_pausable_gateway
...
def _orphaned_desktop_backend_pids(
    ...
    shim before spawning sarthi-setup, and the update-in-progress marker
    ...
    the update with "Sarthi is still running" while the user stares at zero
    open windows (ryanc's 2026-08-09 01:59/02:17 failures).

    A holder qualifies only when BOTH hold:

    - its cmdline is a Sarthi backend (``sarthi_cli.main`` + ``serve`` /
      ``dashboard``), and
    ...
    managed-runtime descendants (the ``.sarthi-runtime`` interpreter child)
    ...
    def _is_backend(argv_low: str) -> bool:
        return "sarthi_cli.main" in argv_low and (
...
def _ledger_reapable_backend_pids(
    ...
    (``sarthi_cli.process_identity``). A holder qualifies when ALL of:
...
def _handoff_reapable_backend_pids(
    ...
    """PIDs of Sarthi ``serve``/``dashboard`` backends safe to reap during a
    ...
    run AND no live Desktop shim (``sarthi.exe``) is open, NOTHING legitimate
    ...
    - Only Sarthi backends (``sarthi_cli.main`` + ``serve``/``dashboard``)
    ...
    - Only runs when the CALLER has confirmed the hand-off context
      ...
      ``sarthi.exe`` shim) — outside that gate this function is never called
...
def _stop_process_trees(pids: list[int]) -> None:
    ...
    ``.sarthi-runtime`` interpreter child alive and holding the install open
...
def _for_each_systemd_gateway_unit(
    ...
    *,
    ...
    """Process each ``sarthi-gateway*.service``/``sarthi-serve*.service`` unit
    ...
    for line in (list_units_stdout or "").strip().splitlines():
        ...
        # ``unit.startswith("sarthi-serve")`` alone would also accept the
        # unrelated ``sarthi-server.service`` — require the exact base unit
        # or the hyphenated profile family instead (review on #83595).
        if not (
            unit == "sarthi-gateway.service"
            or unit.startswith("sarthi-gateway-")
            or unit == "sarthi-serve.service"
            or unit.startswith("sarthi-serve-")
        ):
            continue
...
def _service_unit_supports_graceful_sigusr1_restart(svc_name: str) -> bool:
    ...
    Only ``sarthi-gateway*`` units run ``gateway/run.py``, which installs the
    SIGUSR1 handler. ``sarthi-serve*`` units (#83438) don't, so sending them
    ...
    (``sarthi-gateway-helper`` is fine — profile units are
    ``sarthi-gateway-<profile>`` — but ``sarthi-gatewayd``-style names are
    not) can't be sent a SIGUSR1 it doesn't handle.
    """
    return svc_name == "sarthi-gateway" or svc_name.startswith("sarthi-gateway-")
...
def _warn_incomplete_gateway_fleet_restart(failed_units: list) -> None:
    ...
    print("    sarthi gateway status")
    if any(not name.startswith("ai.sarthi.") for name in ordered):
        print("    systemctl --user restart <unit>   # user-scope")
        print("    sudo systemctl restart <unit>     # system-scope")
    if any(name.startswith("ai.sarthi.") for name in ordered):
...
def _restart_macos_launchd_gateways(
    ...
    ``ai.sarthi.gateway*`` LaunchAgent must reload it — restarting only the
...
def _surviving_gateway_pids_after_failed_restart():
    ...
    when ``sarthi_cli.gateway`` itself no longer imports, which is one of the
...
def _warn_gateway_restart_phase_aborted(exc: BaseException, pids) -> None:
    ...
    (e.g. importing ``sarthi_cli.gateway`` from the freshly pulled checkout)
...
def _refresh_windows_gateway_launchers() -> None:
    ...
    ``sarthi update`` never touched them, so installs created before the
...
def _refresh_bootstrap_cache_scripts(branch: str = "main") -> None:
    ...
    The Desktop GUI updater (``sarthi-setup.exe``) executes
    ``$SARTHI_HOME/bootstrap-cache/install-<ref>.ps1`` (or ``.sh``) for its
    ...
    try:
        ...
        cache_dir = Path(_m().get_sarthi_home()) / "bootstrap-cache"
...
def _normalize_managed_eol(git_cmd, repo_root):
    ...
    ``install.ps1`` forever — so ``sarthi update``, which ships with the checkout
...
def _rebuild_desktop_after_update(
    ...
    # `sarthi desktop --build-only` subprocess re-imports the whole CLI stack
    ...
    desktop_build_cmd = [sys.executable, "-m", "sarthi_cli.main", "desktop", "--build-only"]
    ...
    #
    # Start the build subprocess with the Sarthi-managed Node on PATH: when
    # `sarthi update` runs inside the desktop updater chain (Desktop →
    # sarthi-setup → sarthi update), the shell PATH customizations are lost,
    ...
    if build_result.returncode != 0:
        print("  ⚠ Desktop build failed (run `sarthi desktop` to retry)")
...
def _cmd_update_impl(args, gateway_mode: bool):
    ...
    try:
        from sarthi_cli.update_receipt import begin_update_receipt
    ...
    # running Sarthi runtime, its supervisor, and its running code version —
    ...
    if _m()._is_windows() and not getattr(args, "force", False):
        ...
        if scripts_dir is not None:
            concurrent = _m()._detect_concurrent_sarthi_instances(scripts_dir)
            if concurrent:
    ...
    if _m()._is_windows() and not getattr(args, "force_venv", False):
        ...
        if _venv_holders:
            ...
            # the spawn ledger proves are orphaned Sarthi backends — the
            ...
            if _ledger_backends:
                print(
                    ...
                    "Sarthi backend process(es) hold the venv; stopping their trees"
                )
        ...
        if _venv_holders:
            ...
            if _orphan_backends:
                ...
                # its backend (and any .sarthi-runtime child) holding the
                # venv. Nothing will respawn an orphan, so reap the tree and
                # re-check instead of dead-ending with "Sarthi is still
        ...
        if _venv_holders:
            ...
            # the hand-off context those surviving Sarthi backends are leaks,
            ...
            try:
                ...
                if _scripts_dir is not None:
                    _no_live_shim = not _m()._detect_concurrent_sarthi_instances(_scripts_dir)
            except Exception:
                _no_live_shim = False
            if _handoff and _no_live_shim:
                ...
                if _handoff_backends:
                    print(
                        f"  ⚠ {len(_handoff_backends)} Sarthi backend process(es) "
    ...
    # process by design (a CLI `sarthi update` IS the venv python), and an
    ...
    try:
        ...
        if commit_count == 0:
            ...
            if handed_off_sync:
                print("→ Finishing the dependency install handed off by sarthi.exe...")
            elif not healthy:
            ...
            if runtime_repaired is not None and not _m()._is_windows():
                ...
                print(
                    "  Any running Sarthi gateways, Desktop backends, or other "
        ...
        # Before this guard, ``sarthi update`` printed "✓ Code updated!" and
        # reinstalled deps + rebuilt the desktop app against the stale tree —
        # no error, no warning, ``sarthi doctor`` healthy. Compare pre-pull
        ...
        if pre_pull_sha and post_pull_sha == pre_pull_sha:
            ...
            print(
                ...
                f"git -C {_m().PROJECT_ROOT} checkout {branch} && sarthi update"
            )
            _m()._resume_windows_gateways_after_update(_windows_gateway_resume)
            sys.exit(1)
        ...
        # marker survives and the next ``sarthi`` launch finishes the install
        ...
        # symbols in sarthi_constants or lazy_deps. The dependency install
        ...
        if lazy_ok:
            ...
            print(
                "  ⚠ Lazy-refresh recovery incomplete — run `sarthi` again "
        ...
        if not import_ok:
            ...
            print("    Run `sarthi update` again — if it persists, reinstall:")
        ...
        try:
            ...
            if _state_path.exists():
                ...
                if _state_ok.get("valid"):
                    ...
                    if _pre_snap_id:
                        _snap_state = (
                            _quick_snapshot_root(get_sarthi_home())
                            / _pre_snap_id
                            / "state.db"
                        )
                        if _snap_state.exists():
        ...
        # ``sarthi update`` process is the PRE-pull Python process — its
        # ``sys.modules`` cache holds the OLD ``sarthi_cli.config`` and
        # ``sarthi_cli.config_migrations`` from before ``git pull`` updated
        # the source files. A function-level ``from sarthi_cli.config import
            ...
            if assume_yes:
                ...
                try:
                    ...
                    print(
                        ...
                        "Run 'sarthi config migrate' manually to configure."
                    )
                    response = "n"
        ...
        try:
            from sarthi_cli.backup import restore_cron_jobs_all_profiles
        ...
        # Self-heal the launchers exposed on PATH: the POSIX sarthi-acp shim
        # and missing copies in Windows' dedicated bin directory.
        try:
            _ensure_acp_launcher()
        except Exception as e:
            logger.debug("sarthi-acp launcher self-heal failed: %s", e)
        ...
        try:
            ...
            if (
                ...
                # driver) keeps the installed version — `sarthi update`
                # must stay fast; `sarthi computer-use install --upgrade`
        ...
        #
        # Purge stale cached Sarthi modules FIRST: the import below pulls
        ...
        _m()._purge_stale_sarthi_modules()
        try:
            from sarthi_cli.gateway import (
                is_macos,
            ...
            try:
                from sarthi_cli.gateway import _get_restart_exit_wait_budget
            ...
            # Discover all sarthi-gateway* units (default + profiles) plus
            # sarthi-serve* units (the Desktop app's backend, #83438).
            if supports_systemd_services():
                ...
                for scope, scope_cmd in [
                    ...
                    def _restart_one_systemd_gateway_unit(svc_name: str) -> None:
                        ...
                        # sarthi-serve has no such handler (it isn't
            ...
            # Restart EVERY ai.sarthi.gateway* LaunchAgent, not only the
            ...
            try:
                from sarthi_cli.update_receipt import record_gateway_restart
        ...
        #
        # Forward the systemd units restarted above (includes sarthi-serve*,
        ...
        try:
            from sarthi_cli.update_receipt import (
            ...
            if _called_process_error_is_python_dep_install(e):
                ...
                if _m()._is_windows():
                    ...
                    print(
                        ...
                        '"from sarthi_cli.main import main; main()" update --yes'
                    )
            try:
                from sarthi_cli.update_receipt import finalize_update_receipt
...
def _restart_phase_failure_is_incomplete(surviving, pre_restart_pids) -> bool:
    ...
    * ``surviving is None`` — the survivor probe could not determine state
      (typically the freshly-pulled ``sarthi_cli.gateway`` no longer imports,
