...
# coverage category (sarthi_cli.nous_account.TOOL_COVERAGE_CATEGORIES). Lets the
# `sarthi tools` picker scope its entitlement gate to the selected backend, so a
...
def _selected_provider(section: object, name_key: str = "provider") -> Optional[str]:
    ...
    with the runtime resolver is what stops ``sarthi status`` from lying.
    """
    if not isinstance(section, dict):
        return None
...
def _has_agent_browser() -> bool:
    ...
    try:
        ...
        # Sarthi-managed Node dirs (Windows installer / POSIX $SARTHI_HOME/node)
        ...
        managed_path = with_sarthi_node_path().get("PATH", "")
        if managed_path:
...
def _local_stt_backend_available() -> bool:
    ...
    """
    if get_env_value("SARTHI_LOCAL_STT_COMMAND"):
        return True
    try:
...
def get_nous_subscription_features(
    ...
    *,
    ...
    # Keyless Tavily is opt-in: selecting it in `sarthi tools` / setup writes
    ...
    try:
        ...
        local_stt_available = bool(_HAS_FASTER_WHISPER) or bool(
            get_env_value("SARTHI_LOCAL_STT_COMMAND")
        )
    except Exception:
        local_stt_available = bool(get_env_value("SARTHI_LOCAL_STT_COMMAND"))
...
def prompt_enable_tool_gateway(
    ...
    *,
    ...
    try:
        from sarthi_cli.setup import prompt_checklist
    except Exception:
        return set()
...
# ---------------------------------------------------------------------------
# Inline Nous Portal login for the Tool Gateway picker (`sarthi tools`)
# ---------------------------------------------------------------------------
...
def ensure_nous_portal_access(
    *,
    ...
    Used by ``sarthi tools`` when a user selects a Nous-managed Tool Gateway
    backend (e.g. "Firecrawl (Nous Portal)").  Unlike ``sarthi model``'s Nous
    login, this:
