"""Pydantic request/response models for the Sarthi dashboard web server.

Extracted verbatim from ``sarthi_cli/web_server.py`` (pure schema move).
``web_server`` re-exports every name here, so existing imports like
``from sarthi_cli.web_server import ConfigUpdate`` keep working.
"""
...
class ModelAssignment(BaseModel):
    ...
    # ``sarthi model`` custom flow collects.
    api_key: str = ""
...
class ProfileExport(BaseModel):
    ...
    # Where to write the archive. Empty → a staging path under SARTHI_HOME.
    output: str = ""
