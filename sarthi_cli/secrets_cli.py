"""CLI handlers for ``sarthi secrets bitwarden ...``.
...
# that process is ``sarthi update``, the self-lock preflight detects the
# loaded native module and defers (#86781).  Keep the backend import lazy:
# this module is registered parse-time from ``sarthi_cli.main`` and must not
...
def __getattr__(name: str):
    ...
    ``sarthi_cli.secrets_cli.bw`` directly.  Resolving that attribute at
    ...
    the first actual attribute access, so ``import sarthi_cli.secrets_cli``
...
def cmd_setup(args: argparse.Namespace) -> int:
    ...
    console.print(
        ...
        "Secrets will be pulled at the start of every Sarthi process."
    )
    console.print(
        "  Status:  [cyan]sarthi secrets bitwarden status[/cyan]\n"
        "  Refresh: [cyan]sarthi secrets bitwarden sync[/cyan]\n"
        "  Disable: [cyan]sarthi secrets bitwarden disable[/cyan]"
    )
    return 0
...
def cmd_status(args: argparse.Namespace) -> int:
    ...
    if not enabled:
        console.print("\n  Run [cyan]sarthi secrets bitwarden setup[/cyan] to enable.")
        return 0
    if not token_set:
        console.print(
            f"\n  [yellow]Enabled but {token_env} is not set — Sarthi will skip BSM "
...
def cmd_token(args: argparse.Namespace) -> int:
    ...
    if not args.no_verify:
        ...
        if project_id and projects and project_id not in {p["id"] for p in projects}:
            console.print(
                ...
                "app or re-run `sarthi secrets bitwarden setup` to pick a "
    ...
    console.print(
        ...
        "Takes effect on the next Sarthi invocation."
    )
    if not bw_cfg.get("enabled"):
        console.print(
            "[yellow]Note: the Bitwarden integration is currently disabled — "
            "run `sarthi secrets bitwarden setup` (or set "
...
def cmd_sync(args: argparse.Namespace) -> int:
    ...
    if not bw_cfg.get("enabled"):
        console.print(
            ...
            "`sarthi secrets bitwarden setup` first.[/yellow]"
        )
        return 1
    ...
    if not args.apply:
        console.print(
            ...
            "next [cyan]sarthi[/cyan] invocation.  Re-run with [cyan]--apply[/cyan] "
    ...
    if res.returncode != 0:
        ...
        if "invalid_client" in lowered or "400 bad request" in lowered:
            console.print(
                ...
                "[cyan]sarthi secrets bitwarden setup[/cyan] and pick EU or "
