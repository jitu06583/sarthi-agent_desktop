"""CLI handlers for the ``sarthi proxy`` subcommand."""
...
from sarthi_cli.proxy.server import (
    AIOHTTP_AVAILABLE,
    DEFAULT_HOST,
    DEFAULT_PORT,
    run_server,
...
def cmd_proxy_start(args: Any) -> int:
    ...
    if not adapter.is_authenticated():
        auth_hint = getattr(adapter, "auth_hint", f"sarthi auth add {adapter.name}")
        print(
    ...
    print(
        f"Starting Sarthi proxy for {adapter.display_name}\n"
...
def cmd_proxy_status(args: Any) -> int:
    ...
    print("Sarthi proxy upstream adapters\n")
    ...
    print(
        "\nStart the proxy with: sarthi proxy start [--provider <name>]"
    )
    return 0
...
def cmd_proxy(args: Any) -> int:
    """Dispatch ``sarthi proxy <subcommand>``."""
    ...
    print(
        "sarthi proxy — local OpenAI-compatible proxy that attaches your\n"
        "OAuth-authenticated provider credentials to outbound requests.\n"
        "\n"
        "Subcommands:\n"
        "  sarthi proxy start [--provider nous|xai] [--host 127.0.0.1] [--port 8645]\n"
