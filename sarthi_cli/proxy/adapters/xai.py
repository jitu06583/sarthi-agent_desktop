...
# xAI's public API is OpenAI-compatible for the endpoints Sarthi commonly
# uses. The Responses endpoint is included because Sarthi' native xAI runtime
...
class XAIGrokAdapter(UpstreamAdapter):
    """Proxy upstream for xAI Grok via Sarthi-managed OAuth credentials."""

    auth_hint = "sarthi auth add xai-oauth --type oauth"
        ...
        with self._lock:
            ...
            if pool is None or not pool.has_credentials():
                raise RuntimeError(
                    ...
                    "`sarthi auth add xai-oauth --type oauth` first."
                )
            ...
            if entry is None:
                raise RuntimeError(
                    ...
                    "`sarthi auth reset xai-oauth` or re-authenticate with "
                    "`sarthi auth add xai-oauth --type oauth`."
                )
    ...
    def _credential_from_entry(self, entry: PooledCredential) -> UpstreamCredential:
        ...
        if not bearer:
            raise RuntimeError(
                ...
                "Re-authenticate with `sarthi auth add xai-oauth --type oauth`."
            )
