"""Default configuration data for Sarthi Agent.

Pure-data leaf module: DEFAULT_CONFIG and OPTIONAL_ENV_VARS, extracted
verbatim from sarthi_cli/config.py. Must not import from sarthi_cli.config.
"""
...
DEFAULT_CONFIG = {
    ...
    "toolsets": ["sarthi-cli"],
    # SQLite journal mode used by every Sarthi database opener. WAL is the
    ...
    # Soft file-descriptor limit for long-running Sarthi server processes.
    ...
    "session": {
        # Per-terminal `sarthi -c`: each CLI session drops a breadcrumb file
        # under $SARTHI_HOME/terminal-sessions/<terminal-id>, and a bare
    ...
    "agent": {
        ...
        # all normalized by sarthi_cli.config.resolve_turn_limit.
        "max_turns": None,
        ...
        # budget. CLI one-shot equivalent: `sarthi chat --run-budget N`.
        ...
        # an interactive `sarthi gateway restart` must never block for hours
        ...
        # `sarthi status` / fleet monitoring). Retries never stop — this is a
        ...
        # ``SARTHI_LOCAL_STREAM_STALE_TIMEOUT`` overrides for escape-hatch use.
    ...
    "web": {
        ...
        # firecrawl, keenable). Set by the `sarthi tools` picker's
    ...
    "checkpoints": {
        ...
        # ``sarthi checkpoints prune`` command (add ``--keep-orphans`` to
    ...
    # Single-query (``sarthi -q/-z "..."``) variant of mcp_discovery_timeout.
    ...
    "compression": {
        ...
        "codex_responses_native": False,  # Opt in to OpenAI's server-side compaction
                                      ...
                                      # route/model is unaffected. Sarthi' local
        ...
        "codex_responses_compact_threshold": 200000,  # Server-side compaction trigger
                                      ...
                                      # the server compacts before Sarthi does.
    ...
    "display": {
        ...
        # model — see sarthi_cli/focus_view.py.
        "focus_view": False,
    ...
    # "Hey Sarthi" hands-free wake word. Always-on, on-device hotword
    ...
    "wake_word": {
        ...
        "phrase": "hey sarthi",       # for "sherpa" this IS the detected phrase (any text works); for other engines it's a cosmetic label — detection is keyed by the model/keyword below
        ...
        "openwakeword": {
            # "hey_sarthi" (the bundled, works-out-of-the-box default) OR a
    ...
    "skills": {
        ...
        # checkout, ``<root>/.sarthi/skills/`` and ``<root>/.agents/skills/``
        ...
        # with ``sarthi skills trust`` (run from inside it). Set to false to
        ...
        # Managed by ``sarthi skills trust`` / ``sarthi skills untrust``.
        ...
        # (`sarthi skills install`). Runs ALONGSIDE the built-in skills
        ...
        # ~/.sarthi/skills/.curator_ledger.jsonl with before/after file
        # hashes; file contents are stored content-addressed (deduped) under
        # ~/.sarthi/.curator_backups/blobs/. Enables `sarthi curator ledger`
        # and single-mutation `sarthi curator rollback <entry-id>`.
    ...
    "curator": {
        ...
        # skills are kept forever. When > 0, `sarthi curator purge` deletes
    ...
    # dangerous command. -q runs export SARTHI_INTERACTIVE=1 (for interactive
    ...
    "approvals": {
        ...
        # modal; SARTHI_TUI_NO_CONFIRM=1 force-skips that modal regardless of
    ...
    "security": {
        ...
        # .cursorrules, project-local .sarthi config) always require human
    ...
    "cron": {
        ...
        # SARTHI_CRON_SCRIPT_TIMEOUT. Keep this in sync with
        ...
        # overridable via SARTHI_CRON_MEDIA_SEND_TIMEOUT env var. Keep in
    ...
    # #8731).
    #
    # Provider keys accept the Sarthi provider id (as used elsewhere in
    ...
    "gateway": {
        ...
        # `sarthi update` hang because it can never drain the gateway.
        "restart_loop_guard": {
            "max_restarts": 3,
            "window_seconds": 60,
            "max_gap_seconds": 300,
        ...
        # ``SARTHI_GATEWAY_MAX_STARTS`` / ``SARTHI_GATEWAY_START_WINDOW_S``
    ...
    "sessions": {
        ...
        # the default of ``sarthi sessions prune``.
        "retention_days": 90,
        ...
        # user runs `sarthi sessions optimize-storage`, because the rebuild is
        # disk-heavy and long on large DBs (see that command's disk preflight).
        #
        #   "advise" (default): `sarthi update` prints a one-line notice with
        ...
        # ~/.sarthi/lib/libfts5_cjk.so), 1-2 char CJK terms (일본, 项目, ...)
        ...
        # serve the cjk index. Bridged to SARTHI_CJK_FTS (internal carrier).
        "cjk_fts": True,
        ...
        # SARTHI_SEARCH_SLOW_MS (internal carrier).
        "search_slow_ms": 1000,
        ...
        # (non-streaming) export such as `sarthi sessions export`. Checked
    ...
    # ``sarthi doctor`` behaviour.
    "doctor": {
        # Per-probe timeout (seconds) for the opt-in `sarthi doctor --live`
    ...
    "updates": {
        ...
        #     DBs) into <SARTHI_HOME>/state-snapshots/ before the update.
        ...
        #     jobs) safety net.
        #   full — the quick snapshot PLUS a full ``sarthi backup``-style zip
        #     of SARTHI_HOME into <SARTHI_HOME>/backups/, restorable with
        #     ``sarthi import``. Can add minutes on large homes. This is the
        ...
        # When `sarthi update` finds the source checkout parked on a feature
        ...
        #                         merge. `sarthi update --switch-branch`
    ...
    "secrets": {
        ...
        "bitwarden": {
            ...
            # material under ~/.sarthi/cache/. If a later startup cannot reach
            # Bitwarden due to NETWORK/TIMEOUT, Sarthi may use this encrypted
    ...
    "computer_use": {
        ...
        # cua-driver ≥ 0.6.x supports --no-overlay; Sarthi also calls
        ...
        "no_overlay": None,
        # cua-driver permission mode for each Sarthi computer-use runtime.
        ...
        # manifest used when permission_mode is "bounded". Sarthi passes the
    ...
    #
    # Configure with `sarthi egress setup`.  Disabled by default — the rest of
    # Sarthi works exactly as before with `enabled: false`.
    "proxy": {
        ...
        "tunnel_port": 9090,
        # Auto-download the pinned iron-proxy binary into ~/.sarthi/bin/ on
    ...
    "desktop": {
        ...
        # Ignored on macOS/Windows. Bridged to the SARTHI_DESKTOP_PASSWORD_STORE
...
OPTIONAL_ENV_VARS = {
    ...
    "PORCUPINE_ACCESS_KEY": {
        "description": "Picovoice access key for the Porcupine 'Hey Sarthi' wake word engine (optional; openWakeWord is the free default)",
    ...
    # SARTHI_TOOL_PROGRESS_MODE is deprecated — tool progress is configured via
    # display.tool_progress in config.yaml (off|new|all|verbose|log). The
    # gateway still falls back to SARTHI_TOOL_PROGRESS_MODE for backward
    ...
    # SARTHI_TOOL_PROGRESS is fully unsupported since the v12 config support
