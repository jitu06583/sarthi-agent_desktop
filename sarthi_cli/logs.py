"""``sarthi logs`` — view and filter Sarthi log files.
...
Usage examples::
    ...
    sarthi logs                    # last 50 lines of agent.log
    sarthi logs -f                 # follow agent.log in real time
    sarthi logs errors             # last 50 lines of errors.log
    sarthi logs gateway -n 100    # last 100 lines of gateway.log
    sarthi logs gui -f            # follow gui.log (dashboard/pty/ws)
    sarthi logs desktop -f        # follow desktop.log (Electron app boot/backend)
    sarthi logs --level WARNING    # only WARNING+ lines
    sarthi logs --session abc123   # filter by session ID substring
    sarthi logs --component tools  # only tool-related lines
    sarthi logs --since 1h         # lines from the last hour
    sarthi logs --since 30m -f     # follow, starting 30 min ago
"""
...
def tail_log(
    ...
    *,
    ...
    log_path = get_sarthi_home() / "logs" / filename
    if not log_path.exists():
        print(f"Log file not found: {log_path}")
        print("(Logs are created when Sarthi runs — try 'sarthi chat' first)")
        sys.exit(1)
    ...
    if follow:
        print(f"--- {display_sarthi_home()}/logs/{filename}{filter_desc} (Ctrl+C to stop) ---")
    else:
        print(f"--- {display_sarthi_home()}/logs/{filename}{filter_desc} (last {num_lines}) ---")
...
def list_logs() -> None:
    ...
    if not log_dir.exists():
        print(f"No logs directory at {display_sarthi_home()}/logs/")
        return

    print(f"Log files in {display_sarthi_home()}/logs/:\n")
    found = False
    ...
    if not found:
        print("  (no log files yet — run 'sarthi chat' to generate logs)")
