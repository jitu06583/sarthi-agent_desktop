"""
Single source of truth for provider identity in Sarthi Agent.
...
2. **Sarthi overlays** — transport type, auth patterns, aggregator flags,
...
# -- Sarthi overlay ----------------------------------------------------------
# Sarthi-specific metadata that models.dev doesn't provide.

@dataclass(frozen=True)
class SarthiOverlay:
    """Sarthi-specific provider metadata layered on top of models.dev."""
...
SARTHI_OVERLAYS: Dict[str, SarthiOverlay] = {
    "moa": SarthiOverlay(
        transport="openai_chat",
        auth_type="virtual",
        base_url_override="moa://local",
    ),
    "openrouter": SarthiOverlay(
        transport="openai_chat",
        is_aggregator=True,
    ...
    "openai-codex": SarthiOverlay(
        transport="codex_responses",
        auth_type="oauth_external",
        base_url_override="https://chatgpt.com/backend-api/codex",
    ),
    "openai-api": SarthiOverlay(
        transport="codex_responses",
        base_url_override="https://api.openai.com/v1",
        base_url_env_var="OPENAI_BASE_URL",
    ),
    "xai-oauth": SarthiOverlay(
        transport="codex_responses",
        auth_type="oauth_external",
        base_url_override="https://api.x.ai/v1",
        base_url_env_var="XAI_BASE_URL",
    ),
    "qwen-oauth": SarthiOverlay(
        transport="openai_chat",
        auth_type="oauth_external",
        base_url_override="https://portal.qwen.ai/v1",
        base_url_env_var="SARTHI_QWEN_BASE_URL",
    ),
    "lmstudio": SarthiOverlay(
        transport="openai_chat",
        auth_type="api_key",
    ...
    "copilot-acp": SarthiOverlay(
        transport="codex_responses",
        auth_type="external_process",
        base_url_override="acp://copilot",
        base_url_env_var="COPILOT_ACP_BASE_URL",
    ),
    "github-copilot": SarthiOverlay(
        transport="openai_chat",
        extra_env_vars=("COPILOT_GITHUB_TOKEN", "GH_TOKEN"),
    ),
    "anthropic": SarthiOverlay(
    ...
    "kimi-for-coding": SarthiOverlay(
        transport="openai_chat",
        base_url_env_var="KIMI_BASE_URL",
    ),
    "stepfun": SarthiOverlay(
        transport="openai_chat",
        extra_env_vars=("STEPFUN_API_KEY",),
        base_url_override="https://api.stepfun.ai/step_plan/v1",
        base_url_env_var="STEPFUN_BASE_URL",
    ),
    "minimax": SarthiOverlay(
        transport="anthropic_messages",
        base_url_env_var="MINIMAX_BASE_URL",
    ),
    "minimax-oauth": SarthiOverlay(
        transport="anthropic_messages",
        auth_type="oauth_external",
        base_url_override="https://api.minimax.io/anthropic",
    ),
    "minimax-cn": SarthiOverlay(
        transport="anthropic_messages",
        base_url_env_var="MINIMAX_CN_BASE_URL",
    ),
    "deepseek": SarthiOverlay(
        transport="openai_chat",
        base_url_env_var="DEEPSEEK_BASE_URL",
    ),
    "alibaba": SarthiOverlay(
        transport="openai_chat",
        base_url_env_var="DASHSCOPE_BASE_URL",
    ),
    "alibaba-coding-plan": SarthiOverlay(
        transport="openai_chat",
        base_url_env_var="ALIBABA_CODING_PLAN_BASE_URL",
    ),
    "vercel": SarthiOverlay(
        transport="openai_chat",
        is_aggregator=True,
    ),
    "opencode": SarthiOverlay(
        transport="openai_chat",
        is_aggregator=True,
        base_url_env_var="OPENCODE_ZEN_BASE_URL",
    ),
    "opencode-go": SarthiOverlay(
        transport="openai_chat",
        is_aggregator=True,
        base_url_env_var="OPENCODE_GO_BASE_URL",
    ),
    "opencode-free": SarthiOverlay(
        transport="openai_chat",
        is_aggregator=True,
    ...
    "huggingface": SarthiOverlay(
        transport="openai_chat",
        is_aggregator=True,
        base_url_env_var="HF_BASE_URL",
    ),
    "novita": SarthiOverlay(
        transport="openai_chat",
        is_aggregator=True,
    ...
    "nvidia": SarthiOverlay(
        transport="openai_chat",
        base_url_override="https://integrate.api.nvidia.com/v1",
        base_url_env_var="NVIDIA_BASE_URL",
    ),
    "xiaomi": SarthiOverlay(
        transport="openai_chat",
        base_url_env_var="XIAOMI_BASE_URL",
    ),
    "tencent-tokenhub": SarthiOverlay(
        transport="openai_chat",
    ...
    "azure-foundry": SarthiOverlay(
        transport="openai_chat",  # default; overridden by api_mode in config
        base_url_env_var="AZURE_FOUNDRY_BASE_URL",
    ),
    "bedrock": SarthiOverlay(
...
class ProviderDef:
    ...
    doc: str = ""
    source: str = ""                      # "models.dev", "sarthi", "user-config"
...
def get_provider(name: str, *, allow_network: bool = True) -> Optional[ProviderDef]:
    ...
    Resolution order:
      1. Sarthi overlays (for providers not in models.dev: nous, openai-codex, etc.)
      2. models.dev catalog + Sarthi overlay
    ...
    overlay = SARTHI_OVERLAYS.get(canonical)
    ...
    if mdev_info is not None:
        ...
        # Combine env vars: models.dev env + sarthi extra
    ...
    if overlay is not None:
        # Sarthi-only provider (not in models.dev)
        return ProviderDef(
            id=canonical,
    ...
    # tencent-tokenhub) are absent from models.dev and SARTHI_OVERLAYS, so
...
def determine_api_mode(provider: str, base_url: str = "", model: str = "") -> str:
    ...
    *model* is optional but required for dual-wire providers (Nous) whose
    ...
    # chat_completions. The Sarthi overlay still advertises openai_chat
    ...
    # Direct provider checks for providers not in SARTHI_OVERLAYS
    if provider == "bedrock":
        return "bedrock_converse"
...
def resolve_provider_full(
    ...
    # 0.5 Exact Sarthi provider IDs must win over LOSSY alias collapsing.
    ...
    if canonical != raw:
        try:
            from sarthi_cli.auth import PROVIDER_REGISTRY as _AUTH_PROVIDER_REGISTRY
