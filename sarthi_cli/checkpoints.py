"""`sarthi checkpoints` CLI subcommand.

Gives users direct visibility and control over the filesystem checkpoint
store at ``~/.sarthi/checkpoints/``.  Actions:

    sarthi checkpoints               # same as `status`
    sarthi checkpoints status        # total size, project count, breakdown
    sarthi checkpoints list          # per-project checkpoint counts + workdir
    sarthi checkpoints prune [opts]  # force a sweep (ignores the 24h marker)
    sarthi checkpoints clear [-f]    # nuke the entire base (asks first)
    sarthi checkpoints clear-legacy  # delete just the legacy-* archives

Examples::

    sarthi checkpoints
    sarthi checkpoints prune --retention-days 3 --max-size-mb 200
    sarthi checkpoints clear -f
...
from sarthi_cli.sizefmt import format_bytes as _fmt_bytes
...
def cmd_status(args: argparse.Namespace) -> int:
    ...
    if legacy:
        ...
        print()
        print("Clear with: sarthi checkpoints clear-legacy")
    return 0
...
def register_cli(parser: argparse.ArgumentParser) -> None:
    """Wire subcommands onto the ``sarthi checkpoints`` parser."""
    parser.set_defaults(func=cmd_status)  # bare `sarthi checkpoints` → status
