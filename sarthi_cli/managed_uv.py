"""Sarthi-managed uv and Python runtime repair.

Sarthi owns its own uv binary at ``$SARTHI_HOME/bin/uv`` (or ``uv.exe`` on
Windows).  Every code path that needs uv resolves it from that single location.
If the binary is missing, ``ensure_uv()`` bootstraps it via the official
standalone installer with ``UV_UNMANAGED_INSTALL`` / ``UV_INSTALL_DIR`` pointed
at ``$SARTHI_HOME/bin`` so the installer writes directly there — no PATH
probing, no conda guards, no multi-location resolution chains.

The Python backing the install is different: it is shared by every Sarthi
profile because the checkout's ``venv`` is shared.  Runtime repair therefore
uses an install-scoped store under ``<checkout>/.sarthi-runtime/python``. A
...
from sarthi_cli.sqlite_runtime import SQLiteRuntimeInfo, probe_sqlite_runtime

logger = logging.getLogger(__name__)

_PROJECT_ROOT = Path(__file__).resolve().parents[1]
_RUNTIME_DIR_NAME = ".sarthi-runtime"
_VENV_NAME = "venv"
_ALT_VENV_NAME = ".venv"
...
def managed_uv_path() -> Path:
    """Return the path where Sarthi keeps *its* uv binary.

    ``$SARTHI_HOME/bin/uv`` on POSIX, ``$SARTHI_HOME\\bin\\uv.exe`` on
...
def managed_python_env(
    ...
    *,
    ...
    """Return a sanitized environment for Sarthi-private uv Python commands."""
    target = (
        Path(install_dir)
...
def _report_runtime_repair_failure(repair: RuntimeRepairResult) -> None:
    if repair.backup_venv is None:
        ...
        print(
            "    Sessions stay protected meanwhile: Sarthi keeps databases "
            "out of WAL mode on this SQLite build. The next `sarthi update` "
            "will retry."
        )
        return
...
class _UvResult(str):
    ...
    ``(path, fresh_bootstrap)`` tuple across releases. ``sarthi update`` runs
    the call site from the *old*, already-imported ``sarthi_cli.main`` against
...
def _uv_self_update_is_fresh(now: float | None = None) -> bool:
    ...
    uv releases roughly weekly while many users run ``sarthi update`` daily;
    re-running a blocking network self-update on every invocation is waste
    and, offline, an unbounded hang risk. A stamp file under SARTHI_HOME
    caches the last successful self-update time.
    """
    try:
        from sarthi_constants import get_sarthi_home

        stamp = get_sarthi_home() / "cache" / ".uv_self_update_stamp"
...
def update_managed_uv(
    *,
    ...
    Call this during ``sarthi update`` so the managed copy stays current.
...
def _reload_sarthi_constants():
    """Re-execute ``sarthi_constants`` from disk and return the fresh module.

    ``sarthi update`` imports ``sarthi_constants`` from the OLD checkout,
    ...
    return importlib.reload(sarthi_constants)
...
def _attempt_install_generation(
    ...
    *,
    ...
    try:
        ...
        logger.warning("uv resolved Python outside the Sarthi generation: %s", python)
        _remove_tree(generation, boundary=python_root)
        return None
...
def _install_safe_python_generation(
    ...
    *,
    ...
    # user isn't stuck on every `sarthi update` with no path to a fixed
...
def _windows_runtime_holders() -> tuple[bool, str]:
    if platform.system() != "Windows":
        return False, ""
    main_module = sys.modules.get("sarthi_cli.main")
    ...
    if holders:
        ...
        return True, f"other Sarthi processes still hold the venv (PID {pids})"
    return False, ""
...
def _refresh_managed_uv_catalog(uv_bin: str) -> bool:
    ...
    unmanaged installs.  Only the Sarthi-managed binary is ever refreshed;
...
def _default_live_venv(root: Path) -> Path:
    ...
    ``not-applicable`` on every ``sarthi update`` and stayed on
    journal_mode=DELETE forever — even though the WAL fallback warning
    promises that ``sarthi update`` repairs the runtime (issue class:
...
def _install_uv(target: Path) -> None:
    ...
    ``$SARTHI_HOME/bin/`` instead of ``~/.local/bin/``.
    """
