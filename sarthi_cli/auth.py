...
SPOTIFY_DOCS_URL = "https://sarthi-agent.vercel.app/docs/user-guide/features/spotify"
SPOTIFY_DASHBOARD_URL = "https://developer.spotify.com/dashboard"
SPOTIFY_ACCESS_TOKEN_REFRESH_SKEW_SECONDS = 120

OAUTH_OVER_SSH_DOCS_URL = "https://sarthi-agent.vercel.app/docs/guides/oauth-over-ssh"
...
def _load_global_auth_store() -> Dict[str, Any]:
    ...
    ``~/.sarthi/auth.json`` even when SARTHI_HOME is set to a profile
    path. The hermetic conftest does not redirect ``HOME``, so
    ``get_default_sarthi_root()`` for a profile-shaped SARTHI_HOME can
...
def _prompt_model_selection(
    ...
    """
    from sarthi_cli.models import _format_price_per_mtok
