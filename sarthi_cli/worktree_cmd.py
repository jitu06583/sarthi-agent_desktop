"""``sarthi worktree`` — audit and reclaim accumulated git worktrees/branches.

Attended counterpart of the silent startup pruner (see
``sarthi_cli/worktree_gc.py`` for the policy and shared invariants). Usage:

    sarthi worktree list             # audit: verdict + reason per tree
    sarthi worktree prune            # reap safe trees + merged branches
    sarthi worktree prune --dry-run  # show the plan, change nothing
    sarthi worktree prune --trees-only / --branches-only
"""
...
def cmd_worktree(args) -> int:
    ...
    if action == "list":
        ...
        print(
            ...
            f"{_fmt_size(reapable_mb)} reclaimable now via `sarthi worktree prune`."
        )
