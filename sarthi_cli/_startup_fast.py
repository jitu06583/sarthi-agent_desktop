...
This module is imported by ``sarthi_cli/main.py`` BEFORE its heavy import
wall (config, argparse tree, logging, providers). Everything here must stay
**stdlib-only and cheap** (os/sys file probes; no yaml, no sarthi_cli.config,
...
``sarthi_cli/config.py``'s ``get_container_exec_info()`` reads the same
...
def active_profile_may_override_home(sarthi_root: str) -> bool:
    """Cheap probe: does an active non-default profile redirect SARTHI_HOME?"""
    active_profile = os.path.join(sarthi_root, "active_profile")
    try:
...
def _resolved_home() -> str:
    sarthi_home = os.environ.get("SARTHI_HOME", "").strip()
    if sarthi_home:
        return sarthi_home
    return os.path.join(os.path.expanduser("~"), ".sarthi")
...
def container_mode_may_be_active() -> bool:
    ...
    sarthi_home = os.environ.get("SARTHI_HOME", "").strip()
    if sarthi_home:
        if os.path.exists(os.path.join(sarthi_home, ".container-mode")):
            return True
        parent_name = os.path.basename(os.path.dirname(os.path.normpath(sarthi_home)))
        return (
            parent_name != "profiles"
            and active_profile_may_override_home(sarthi_home)
        )

    default_home = os.path.join(os.path.expanduser("~"), ".sarthi")
...
def read_install_method() -> str | None:
    ...
    non-default profile might redirect SARTHI_HOME.
    """
...
def print_fast_version_info(*, check_updates: bool = True) -> None:
    """THE canonical ``sarthi --version`` output (also used by /version).
    ...
    ``sarthi version`` subcommand. Every lazy block degrades gracefully —
    ...
    try:
        from sarthi_cli.banner import UPDATE_AVAILABLE_NO_COUNT, check_for_updates
...
def try_fast_version(argv: list[str] | None = None) -> bool:
    """Handle ``sarthi --version`` before the heavy import wall.
    ...
    container. Termux keeps the SARTHI_TERMUX_DISABLE_FAST_CLI escape hatch.
    """
    if argv is None:
        argv = sys.argv[1:]
    is_termux = is_termux_env()
    if is_termux and os.environ.get("SARTHI_TERMUX_DISABLE_FAST_CLI") == "1":
        return False
    if is_termux:
