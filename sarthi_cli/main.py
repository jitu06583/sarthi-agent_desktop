#!/usr/bin/env python3
"""
Sarthi CLI - Main entry point.

Usage:
    sarthi                     # Interactive chat (default)
    sarthi chat                # Interactive chat
    sarthi gateway             # Run gateway in foreground
    sarthi gateway start       # Start gateway as service
    sarthi gateway stop        # Stop gateway service
    sarthi gateway status      # Show gateway status
    sarthi gateway install     # Install gateway service
    sarthi gateway uninstall   # Uninstall gateway service
    sarthi setup               # Interactive setup wizard
    sarthi logout              # Clear stored authentication
    sarthi status              # Show status of all components
    sarthi cron                # Manage cron jobs
    sarthi cron list           # List cron jobs
    sarthi cron status         # Check if cron scheduler is running
    sarthi doctor              # Check configuration and dependencies
    sarthi honcho setup                    # Configure Honcho AI memory integration
    sarthi honcho status                   # Show Honcho config and connection status
    sarthi honcho sessions                 # List directory → session name mappings
    sarthi honcho map <name>               # Map current directory to a session name
    sarthi honcho peer                     # Show peer names and dialectic settings
    sarthi honcho peer --user NAME         # Set user peer name
    sarthi honcho peer --ai NAME           # Set AI peer name
    sarthi honcho peer --reasoning LEVEL   # Set dialectic reasoning level
    sarthi honcho mode                     # Show current memory mode
    sarthi honcho mode [hybrid|honcho|local]  # Set memory mode
    sarthi honcho tokens                   # Show token budget settings
    sarthi honcho tokens --context N       # Set session.context() token cap
    sarthi honcho tokens --dialectic N     # Set dialectic result char cap
    sarthi honcho identity                 # Show AI peer identity representation
    sarthi honcho identity <file>          # Seed AI peer identity from a file (SOUL.md etc.)
    sarthi honcho migrate                  # Step-by-step migration guide: OpenClaw native → Sarthi + Honcho
    sarthi version             Show version
    sarthi update              Update to latest version
    sarthi uninstall           Uninstall Sarthi Agent
    sarthi acp                 Run as an ACP server for editor integration
    sarthi sessions browse     Interactive session picker with search

    sarthi claw migrate --dry-run  # Preview migration without changes
"""

# IMPORTANT: sarthi_bootstrap must be the very first import — it sets up
# UTF-8 stdio on Windows so print()/subprocess children don't hit
# UnicodeEncodeError with non-ASCII characters.  No-op on POSIX.
#
# Guarded against ModuleNotFoundError because ``sarthi_bootstrap`` is a
# top-level module registered via pyproject.toml's ``py-modules`` list.
# When the user upgrades code via ``git pull`` (or ``sarthi update``
# crashes between ``git reset --hard`` and ``uv pip install -e .``), the
# new code references ``sarthi_bootstrap`` but the editable install's
# ``.pth`` file still points at the old set of top-level modules.  Without
# this guard, sarthi crashes on import and the user can't run
# ``sarthi update`` to recover.  Missing the bootstrap means UTF-8 stdio
# setup is skipped on Windows — degraded, not broken.  POSIX is unaffected.
try:
    import sarthi_bootstrap  # noqa: F401
...
def _set_process_title() -> None:
    """Set the process title to 'sarthi' so tools like 'ps', 'top', and
    'htop' show the app name instead of 'python3.xx'.

    Purely cosmetic — non-fatal on any platform.

    Strategy (try in order):
      1. ``setproctitle`` (opt-in dep — installed via ``sarthi tools`` or
      ...
      4. No-op on Windows (the .exe name is already ``sarthi.exe``).
    """
    # Strategy 1: setproctitle (best — works on macOS, Linux, BSD)
    try:
        import setproctitle  # type: ignore[import-untyped]

        setproctitle.setproctitle("sarthi")
        return
    except ImportError:
        pass
    ...
    try:
        ...
        if system == "Linux":
            ...
            libc.prctl(15, b"sarthi", 0, 0, 0)  # PR_SET_NAME = 15
        elif system == "Darwin":
            libc = ctypes.CDLL("libc.dylib", use_errno=True)
            libc.pthread_setname_np(b"sarthi")
        # Windows: the .exe name is already ``sarthi.exe`` — nothing to do.
    except Exception:
        pass
...
# that run *before* sarthi_cli.config is importable. Mirrors the explicit
...
def _config_default_interface_early() -> str:
    ...
    value = "cli"
    try:
        home = os.environ.get("SARTHI_HOME")
        if home:
            cfg_path = os.path.join(home, "config.yaml")
        else:
            cfg_path = os.path.join(os.path.expanduser("~"), ".sarthi", "config.yaml")
...
def _wants_tui_early(argv: "list[str] | None" = None) -> bool:
    ...
    explicit ``--tui``/``SARTHI_TUI=1``, then a real-TTY gate (a
    ...
    cron jobs, pipes run ``sarthi … chat -q`` with stdio on a pipe. This
    ...
    if "--cli" in argv:
        return False
    if os.environ.get("SARTHI_TUI") == "1" or "--tui" in argv:
        return True
    try:
...
# `entry.tsx`; this is just the earlier cousin. ``SARTHI_TUI_NO_EARLY_DISABLE``
# escapes the behaviour for diagnostics.
def _suppress_mouse_residue_early() -> None:
    if os.environ.get("SARTHI_TUI_NO_EARLY_DISABLE") == "1":
        return
    if not _wants_tui_early():
        return
    try:
        # Skip when stdout is redirected (`sarthi --tui … >log`, CI capture):
...
def _print_fast_version_info() -> None:
    from sarthi_cli import __release_date__, __version__

    print(f"Sarthi Agent v{__version__} ({__release_date__})")
...
def _try_termux_ultrafast_version() -> bool:
    """Handle ``sarthi --version`` before config/logging imports on Termux."""
    if os.environ.get("SARTHI_TERMUX_DISABLE_FAST_CLI") == "1":
        return False
...
from sarthi_cli.subcommands._shared import add_accept_hooks_flag as _add_accept_hooks_flag
from sarthi_cli.subcommands.cron import build_cron_parser
from sarthi_cli.subcommands.gateway import build_gateway_parser
from sarthi_cli.subcommands.profile import build_profile_parser
from sarthi_cli.subcommands.model import build_model_parser
from sarthi_cli.subcommands.setup import build_setup_parser
from sarthi_cli.subcommands.postinstall import build_postinstall_parser
from sarthi_cli.subcommands.whatsapp import build_whatsapp_parser
from sarthi_cli.subcommands.slack import build_slack_parser
from sarthi_cli.subcommands.login import build_login_parser
from sarthi_cli.subcommands.logout import build_logout_parser
from sarthi_cli.subcommands.auth import build_auth_parser
from sarthi_cli.subcommands.status import build_status_parser
from sarthi_cli.subcommands.webhook import build_webhook_parser
from sarthi_cli.subcommands.hooks import build_hooks_parser
from sarthi_cli.subcommands.doctor import build_doctor_parser
from sarthi_cli.subcommands.security import build_security_parser
from sarthi_cli.subcommands.dump import build_dump_parser
from sarthi_cli.subcommands.debug import build_debug_parser
from sarthi_cli.subcommands.backup import build_backup_parser
from sarthi_cli.subcommands.import_cmd import build_import_cmd_parser
from sarthi_cli.subcommands.config import build_config_parser
from sarthi_cli.subcommands.console import build_console_parser
from sarthi_cli.subcommands.version import build_version_parser
from sarthi_cli.subcommands.update import build_update_parser
from sarthi_cli.subcommands.uninstall import build_uninstall_parser
from sarthi_cli.subcommands.dashboard import build_dashboard_parser
from sarthi_cli.subcommands.gui import build_gui_parser
from sarthi_cli.subcommands.logs import build_logs_parser
from sarthi_cli.subcommands.prompt_size import build_prompt_size_parser
from sarthi_cli.subcommands.memory import build_memory_parser
from sarthi_cli.subcommands.acp import build_acp_parser
from sarthi_cli.subcommands.tools import build_tools_parser
from sarthi_cli.subcommands.insights import build_insights_parser
from sarthi_cli.subcommands.skills import build_skills_parser
from sarthi_cli.subcommands.pairing import build_pairing_parser
from sarthi_cli.subcommands.plugins import build_plugins_parser
from sarthi_cli.subcommands.mcp import build_mcp_parser
from sarthi_cli.subcommands.claw import build_claw_parser


def _require_tty(command_name: str) -> None:
    """Exit with a clear error if stdin is not a terminal.

    Interactive TUI commands (sarthi tools, sarthi setup, sarthi model) use
    curses or input() prompts that spin at 100% CPU when stdin is a pipe.
    This guard prevents accidental non-interactive invocation.
    """
    if not sys.stdin.isatty():
        print(
            f"Error: 'sarthi {command_name}' requires an interactive terminal.\n"
...
# ---------------------------------------------------------------------------
# Profile override — MUST happen before any sarthi module import.
#
# Many modules cache SARTHI_HOME at import time (module-level constants).
# We intercept --profile/-p from sys.argv here and set the env var so that
# every subsequent ``os.getenv("SARTHI_HOME", ...)`` resolves correctly.
# The flag is stripped from sys.argv so argparse never sees it.
# Falls back to ~/.sarthi/active_profile for sticky default.
# ---------------------------------------------------------------------------
def _apply_profile_override() -> None:
    """Pre-parse --profile/-p and set SARTHI_HOME before imports."""
    argv = sys.argv[1:]
    profile_name = None
    consume = 0
    profile_index = None
    ...
    def _inside_mcp_add_args(index: int) -> bool:
        """True once argv reaches `sarthi mcp add ... --args <command argv>`.
        ...
        ``--profile``), not to Sarthi' own profile selector.
        """
        try:
    ...
    def _resolve_sudo_user_profile_env(name: str) -> str | None:
        """Resolve `sudo sarthi -p <name>` against the invoking user's home.
        ...
        candidate = home / ".sarthi" / "profiles" / name
        try:
            if candidate.is_dir():
    ...
    # after the subcommand (`sarthi chat -p coder`), so keep scanning broadly.
    ...
    # Mirrors sarthi_cli.profiles._PROFILE_ID_RE so we never call
    ...
    # 1.5 If SARTHI_HOME is already set and no explicit flag was given, trust it
    # only when it already points to a specific profile directory.  The
    # distinguishing heuristic: a profile path has "profiles" as its immediate
    # parent directory name (e.g. ~/.sarthi/profiles/coder or
    # /opt/data/profiles/coder).  If SARTHI_HOME points to the sarthi root
    # instead (e.g. systemd hardcodes SARTHI_HOME=/root/.sarthi), we must
    # still read active_profile — the user may have switched profiles via
    # `sarthi profile use` and the gateway should honour that choice.
    # See issue #22502.
    sarthi_home_env = os.environ.get("SARTHI_HOME", "")
    if profile_name is None and sarthi_home_env:
        if Path(sarthi_home_env).parent.name == "profiles":
            return

    # 2. If no flag, check active_profile in the sarthi root.
    #
    # EXCEPTION: a supervised s6 gateway child (exported by the container
    # run-script as SARTHI_S6_SUPERVISED_CHILD=1) must NOT follow the sticky
    ...
    # reserved ``gateway-default`` slot runs bare ``sarthi gateway run`` to mean
    # "the root SARTHI_HOME profile". If the reserved default child read
    ...
    if profile_name is None and not os.environ.get("SARTHI_S6_SUPERVISED_CHILD"):
        try:
            from sarthi_constants import get_default_sarthi_root

            active_path = get_default_sarthi_root() / "active_profile"
    ...
    # 3. If we found a profile, resolve and set SARTHI_HOME
    if profile_name is not None:
        try:
            from sarthi_cli.profiles import resolve_profile_env

            sarthi_home = resolve_profile_env(profile_name)
        except FileNotFoundError as exc:
            sarthi_home = _resolve_sudo_user_profile_env(profile_name)
            if not sarthi_home:
            ...
            # A bug in profiles.py must NEVER prevent sarthi from starting
            print(
                f"Warning: profile override failed ({exc}), using default",
                file=sys.stderr,
            )
            return
        os.environ["SARTHI_HOME"] = sarthi_home
...
# Load .env from ~/.sarthi/.env first, then project root as dev fallback.
# User-managed env files should override stale shell exports on restart.
from sarthi_cli.config import get_sarthi_home
from sarthi_cli.env_loader import load_sarthi_dotenv

load_sarthi_dotenv(project_env=PROJECT_ROOT / ".env")

# Bridge security.redact_secrets from config.yaml → SARTHI_REDACT_SECRETS env
# var BEFORE sarthi_logging imports agent.redact (which snapshots the flag at
...
try:
    ...
    _cfg_path = get_sarthi_home() / "config.yaml"
    if _cfg_path.exists():
        ...
        try:
            from sarthi_cli import managed_scope
            _early_cfg_raw = managed_scope.apply_managed_overlay(_early_cfg_raw)
        except Exception:
            pass
        if "SARTHI_REDACT_SECRETS" not in os.environ:
            _early_sec_cfg = _early_cfg_raw.get("security", {})
            if isinstance(_early_sec_cfg, dict):
                _early_redact = _early_sec_cfg.get("redact_secrets")
                if _early_redact is not None:
                    os.environ["SARTHI_REDACT_SECRETS"] = str(_early_redact).lower()
...
# Initialize centralized file logging early — all `sarthi` subcommands
...
try:
    from sarthi_logging import setup_logging as _setup_logging
...
if _FORCE_IPV4_EARLY:
    try:
        from sarthi_constants import apply_ipv4_preference as _apply_ipv4

        _apply_ipv4(force=True)
    except Exception:
        pass  # best-effort — don't crash if sarthi_constants not importable yet
...
from sarthi_cli import __version__, __release_date__

# Provider model-selection wizard flows extracted to sarthi_cli/model_setup_flows.py
# (god-file decomposition Phase 2). Re-imported here so select_provider_and_model and
# existing test monkeypatches (sarthi_cli.main._model_flow_*) keep resolving unchanged.
from sarthi_cli.model_setup_flows import (
...
def _termux_bundled_skills_stamp_path() -> Path:
    return get_sarthi_home() / "skills" / ".termux_bundled_sync_stamp"


def _termux_bundled_skills_sync_needed() -> bool:
    if not _is_termux_startup_environment():
        return True
    if os.environ.get("SARTHI_TERMUX_FORCE_SKILLS_SYNC") == "1":
        return True
    try:
...
def _termux_should_prefetch_update_check() -> bool:
    if not _is_termux_startup_environment():
        return True
    return os.environ.get("SARTHI_TERMUX_PREFETCH_UPDATES") == "1"
...
def _has_any_provider_configured() -> bool:
    ...
    from sarthi_cli.config import get_env_path, get_sarthi_home, load_config
    from sarthi_cli.auth import get_auth_status

    # Determine whether Sarthi itself has been explicitly configured (model
    # in config that isn't the hardcoded default). Used below to gate external
    # tool credentials (Claude Code, Codex CLI) that shouldn't silently skip
    # the setup wizard on a fresh install.
    from sarthi_cli.config import DEFAULT_CONFIG
    ...
    if isinstance(model_cfg, dict):
        ...
        _model_name = ""
    _has_sarthi_config = _model_name and _model_name != _DEFAULT_MODEL

    # Check env vars (may be set by .env or shell).
    # OPENAI_BASE_URL alone counts — local models (vLLM, llama.cpp, etc.)
    # often don't require an API key.
    from sarthi_cli.auth import PROVIDER_REGISTRY
    ...
    auth_file = get_sarthi_home() / "auth.json"
    if auth_file.exists():
        try:
            import json
    ...
    # Only count these if Sarthi has been explicitly configured — Claude Code
    # being installed doesn't mean the user wants Sarthi to use their tokens.
    if _has_sarthi_config:
        try:
...
def _exec_in_container(container_info: dict, cli_args: list):
    ...
    Args:
        container_info: dict with backend, container_name, exec_user, sarthi_bin
        cli_args: the original CLI arguments (everything after 'sarthi')
    """

    backend = container_info["backend"]
    container_name = container_info["container_name"]
    exec_user = container_info["exec_user"]
    sarthi_bin = container_info["sarthi_bin"]
        ...
        if sudo_path:
            ...
            if probe2.returncode != 0:
                print(
                    ...
                    f"  }}];\n"
                    f"\n"
                    f"Or run: sudo sarthi {' '.join(cli_args)}",
                    file=sys.stderr,
                )
                sys.exit(1)
        else:
            print(
                f"Error: container '{container_name}' not found via {backend}.\n"
                f"The container may be running under root. Try: sudo sarthi {' '.join(cli_args)}",
                file=sys.stderr,
            )
            sys.exit(1)
...
def _print_tui_exit_summary(
    ...
    print(f"  sarthi --tui --resume {target}")
    if title:
        print(f'  sarthi --tui -c "{title}"')
    print()
...
def _tui_need_npm_install(root: Path) -> bool:
    """True when @sarthi/ink is missing or node_modules is behind package-lock.json.
    ...
    *root* (``ui-tui/dist/entry.js``).
    ...
    ink = ws_root / "node_modules" / "@sarthi" / "ink" / "package.json"
    if not ink.is_file():
        return True
    if not lock.is_file():
        return False
...
_TUI_BUILD_INPUT_FILES = (
    ...
    "scripts/build.mjs",
    "packages/sarthi-ink/package.json",
    "packages/sarthi-ink/index.js",
    "packages/sarthi-ink/text-input.js",
)
...
def _tui_need_rebuild(root: Path) -> bool:
    ...
    or local edits. Set ``SARTHI_TUI_FORCE_BUILD=1`` to force the old behaviour.
    """
    force = (os.environ.get("SARTHI_TUI_FORCE_BUILD") or "").strip().lower()
...
def _ensure_tui_node() -> None:
    ...
    ``SARTHI_SKIP_NODE_BOOTSTRAP=1`` to disable auto-install.
    """
    if shutil.which("node") and shutil.which("npm"):
        return
    if os.environ.get("SARTHI_SKIP_NODE_BOOTSTRAP"):
        return
    ...
    sarthi_home = os.environ.get("SARTHI_HOME") or str(Path.home() / ".sarthi")
    try:
        ...
        result = subprocess.run(
            ...
            env={**os.environ, "SARTHI_HOME": sarthi_home},
            capture_output=True,
            text=True,
            encoding="utf-8",
            errors="replace",
    ...
    extras.extend([Path(sarthi_home) / "node" / "bin", Path.home() / ".local" / "bin"])
...
def _find_bundled_tui(sarthi_cli_dir: Path | None = None) -> Path | None:
    """Find a pre-built TUI entry.js bundled in the wheel."""
    if sarthi_cli_dir is None:
        sarthi_cli_dir = Path(__file__).parent
    bundled = sarthi_cli_dir / "tui_dist" / "entry.js"
...
def _restore_tui_workspace(tui_dir: Path) -> bool:
    ...
    files deleted in the working tree after ``sarthi update`` (HEAD stays
...
def _ensure_tui_workspace(tui_dir: Path) -> None:
    ...
    if _restore_tui_workspace(tui_dir):
        if not os.environ.get("SARTHI_QUIET"):
            print(f"Restored missing TUI workspace: {tui_dir}")
        return

    print(
        "Error: the TUI workspace is missing from this Sarthi checkout.\n"
        f"Expected directory: {tui_dir}\n"
        "This usually means `sarthi update` left tracked ui-tui files deleted.\n"
        "Recovery:\n"
        "  1. From the Sarthi checkout, run `git restore -- ui-tui`\n"
        "  2. Run `npm install --silent --no-fund --no-audit --progress=false`\n"
        "  3. Retry `sarthi --tui`\n"
        "If the checkout is still inconsistent, run `sarthi update --force`.",
        file=sys.stderr,
    )
    sys.exit(1)
...
def _make_tui_argv(tui_dir: Path, tui_dev: bool) -> tuple[list[str], Path]:
    """TUI: --dev → tsx src; else node dist (SARTHI_TUI_DIR prebuilt or esbuild)."""
    _ensure_tui_node()

    def _node_bin(bin: str) -> str:
        if bin == "node":
            env_node = os.environ.get("SARTHI_NODE")
            if env_node and os.path.isfile(env_node) and os.access(env_node, os.X_OK):
                return env_node
        path = shutil.which(bin)
        if not path and bin == "node":
            try:
                from sarthi_cli.dep_ensure import ensure_dependency
    ...
    ext_dir = os.environ.get("SARTHI_TUI_DIR")
    if tui_dev and ext_dir:
        print(
            f"Error: --dev is incompatible with SARTHI_TUI_DIR={ext_dir}\n"
            f"The prebuilt TUI has no source code to hot-reload.\n"
            f"Unset SARTHI_TUI_DIR (e.g. `unset SARTHI_TUI_DIR`) to use --dev from a checkout.",
            file=sys.stderr,
        )
        sys.exit(1)
    ...
    if tui_dev:
        # Keep the local @sarthi/ink package exports in sync with source.
        # --dev runs src/entry.tsx directly, but @sarthi/ink resolves through
        # packages/sarthi-ink/dist/entry-exports.js. If that dist bundle is
        ...
        npm = _node_bin("npm")
        ink_dir = tui_dir / "packages" / "sarthi-ink"
        result = subprocess.run(
            [npm, "run", "build"],
            cwd=str(ink_dir),
            capture_output=True,
...
def _normalize_tui_toolsets(toolsets: object) -> list[str]:
    ...
    try:
        from sarthi_cli.oneshot import _normalize_toolsets
...
def _launch_tui(
    ...
    env = os.environ.copy()
    try:
        from sarthi_cli.config import apply_terminal_config_to_env
        apply_terminal_config_to_env(env=env)
    except Exception:
        logger.debug("Failed to apply terminal config bridge for TUI launch", exc_info=True)
    active_session_fd, active_session_file = tempfile.mkstemp(
        prefix="sarthi-tui-active-session-", suffix=".json"
    )
    os.close(active_session_fd)
    env["SARTHI_TUI_ACTIVE_SESSION_FILE"] = active_session_file
    env["SARTHI_PYTHON_SRC_ROOT"] = os.environ.get(
        "SARTHI_PYTHON_SRC_ROOT", str(PROJECT_ROOT)
    )
    env.setdefault("SARTHI_PYTHON", sys.executable)
    env.setdefault("SARTHI_CWD", os.getcwd())
    ...
    if worktree:
        ...
        if not wt_info:
            sys.exit(1)
        env["SARTHI_CWD"] = wt_info["path"]
        env["TERMINAL_CWD"] = wt_info["path"]

    if model:
        env["SARTHI_MODEL"] = model
        env["SARTHI_INFERENCE_MODEL"] = model
    if provider:
        env["SARTHI_TUI_PROVIDER"] = provider
        env["SARTHI_INFERENCE_PROVIDER"] = provider
    tui_toolsets = _normalize_tui_toolsets(toolsets)
    if tui_toolsets:
        env["SARTHI_TUI_TOOLSETS"] = ",".join(tui_toolsets)
    if skills:
        if isinstance(skills, (list, tuple)):
            ...
            if flattened:
                env["SARTHI_TUI_SKILLS"] = ",".join(flattened)
        else:
            value = str(skills).strip()
            if value:
                env["SARTHI_TUI_SKILLS"] = value
    if query:
        env["SARTHI_TUI_QUERY"] = query
    if image:
        env["SARTHI_TUI_IMAGE"] = image
    if checkpoints:
        env["SARTHI_TUI_CHECKPOINTS"] = "1"
    if pass_session_id:
        env["SARTHI_TUI_PASS_SESSION_ID"] = "1"
    if max_turns is not None:
        env["SARTHI_TUI_MAX_TURNS"] = str(max_turns)
    if verbose:
        env["SARTHI_TUI_TOOL_PROGRESS"] = "verbose"
    elif quiet:
        env["SARTHI_TUI_TOOL_PROGRESS"] = "off"
    if accept_hooks:
        env["SARTHI_ACCEPT_HOOKS"] = "1"
    ...
    # SARTHI_TUI_RESUME is an internal hand-off from the Python wrapper to the
    # Ink app.  Because we start from os.environ.copy(), an exported/stale value
    # in the user's shell would otherwise make a plain `sarthi --tui` try to
    ...
    # still set SARTHI_TUI_RESUME themselves.
    env.pop("SARTHI_TUI_RESUME", None)
    if resume_session_id:
        env["SARTHI_TUI_RESUME"] = resume_session_id
    ...
    # Exit code 42 = TUI requested an update. Relaunch as `sarthi update` so
    ...
    if code == 42:
        from sarthi_cli.relaunch import relaunch
...
def _pin_kanban_board_env() -> None:
    """Pin the active kanban board into ``SARTHI_KANBAN_BOARD`` for the chat session.

    Without this, in-process tools (``kanban_*``) and shelled-out CLI calls
    (``sarthi kanban …``) resolve the board on different paths: the env-pin if
    set, otherwise the global ``<root>/kanban/current`` file. A concurrent
    ``sarthi kanban boards switch`` from another session can flip the file
    ...
    """
    if os.environ.get("SARTHI_KANBAN_BOARD"):
        return
    try:
        from sarthi_cli.kanban_db import get_current_board

        os.environ["SARTHI_KANBAN_BOARD"] = get_current_board()
    except Exception:
        pass


def _sync_bundled_skills_quietly() -> None:
    """Seed ``~/.sarthi/skills/`` with the bundled skill library on first launch.

    Called from any CLI entrypoint that the user might use as their first
    interaction with Sarthi — chat, dashboard (the desktop GUI's backend),
    ...
    dependency. Sarthi still functions without them; the user just sees an
...
def _resolve_use_tui(args) -> bool:
    ...
    workers, cron jobs, and pipelines run ``sarthi … chat -q`` with stdout
    ...
    try:
        ...
        return False
    if os.environ.get("SARTHI_TUI") == "1":
        return True
    try:
        from sarthi_cli.config import load_config
...
def cmd_chat(args):
    ...
    if continue_val and not getattr(args, "resume", None):
        if isinstance(continue_val, str):
            ...
            if resolved:
                ...
                print("Use 'sarthi sessions list' to see available sessions.")
                sys.exit(1)
        else:
    ...
    try:
        from sarthi_cli.xai_retirement import (
            MIGRATION_GUIDE_URL,
            RETIREMENT_DATE,
            find_retired_xai_refs,
            format_issue,
        )
        from sarthi_cli.config import load_config as _load_config_for_xai_check
        ...
        if _retired_xai_refs:
            ...
            sys.stderr.write("  \033[2mRun 'sarthi doctor' for details.\033[0m\n\n")
    except Exception:
        pass
    ...
    if not _has_any_provider_configured():
        print()
        print(
            "It looks like Sarthi isn't configured yet -- no API keys or providers found."
        )
        print()
        print("  Run:  sarthi setup")
        print()

        from sarthi_cli.setup import (
        ...
        if reply in {"", "y", "yes"}:
            ...
            return
        print()
        print("You can run 'sarthi setup' at any time to configure.")
        sys.exit(1)
    ...
    if _termux_should_prefetch_update_check():
        try:
            from sarthi_cli.banner import prefetch_update_check
    ...
    if getattr(args, "yolo", False):
        os.environ["SARTHI_YOLO_MODE"] = "1"

    # --ignore-user-config: make load_cli_config() / load_config() skip the
    # user's ~/.sarthi/config.yaml and return built-in defaults. Set BEFORE
    # importing cli (which runs `CLI_CONFIG = load_cli_config()` at module
    # import time). Credentials in .env are still loaded — this flag only
    # ignores behavioral/config settings.
    if getattr(args, "ignore_user_config", False):
        os.environ["SARTHI_IGNORE_USER_CONFIG"] = "1"

    # --ignore-rules: skip auto-injection of AGENTS.md/SOUL.md/.cursorrules
    # (rules), memory entries, and any preloaded skills coming from user config.
    # Maps to AIAgent(skip_context_files=True, skip_memory=True).
    if getattr(args, "ignore_rules", False):
        os.environ["SARTHI_IGNORE_RULES"] = "1"

    # --source: tag session source for filtering (e.g. 'tool' for third-party integrations)
    if getattr(args, "source", None):
        os.environ["SARTHI_SESSION_SOURCE"] = args.source
...
def cmd_gateway(args):
    ...
    from sarthi_cli.gateway import gateway_command
...
def cmd_proxy(args):
    ...
    from sarthi_cli.proxy.cli import cmd_proxy as _cmd_proxy
...
def cmd_whatsapp(args):
    ...
    from sarthi_cli.config import get_env_value, save_env_value
    from sarthi_constants import find_node_executable, with_sarthi_node_path
    ...
    if not current_mode:
        print()
        print("How will you use WhatsApp with Sarthi?")
        print()
    ...
    # has no creds.json.  Every subsequent `sarthi gateway` then paid a 30s
    ...
    session_dir = get_sarthi_home() / "whatsapp" / "session"
    ...
    if (session_dir / "creds.json").exists():
        ...
        if response.lower() in {"y", "yes"}:
            ...
            print("  Start the gateway with: sarthi gateway")
            return
    ...
    if (session_dir / "creds.json").exists():
        ...
        # and `sarthi gateway` skips it cleanly instead of paying a 30s
        ...
        if wa_mode == "bot":
            print("  Next steps:")
            print("    1. Start the gateway:  sarthi gateway")
            print("    2. Send a message to the bot's WhatsApp number")
            print("    3. The agent will reply automatically")
            print()
            print("  Tip: Agent responses are prefixed with '⚕ Sarthi Agent'")
        else:
            print("  Next steps:")
            print("    1. Start the gateway:  sarthi gateway")
        ...
        print()
        print("  Or install as a service: sarthi gateway install")
    else:
        print("⚠ Pairing may not have completed. Run 'sarthi whatsapp' to try again.")
...
def cmd_whatsapp_cloud(args):
    ...
    Distinct from ``sarthi whatsapp`` (the Baileys bridge wizard) — the
    two adapters are complementary, not alternatives. See
    ``sarthi_cli/setup_whatsapp_cloud.py``.
    """
    _require_tty("whatsapp-cloud")
    from sarthi_cli.setup_whatsapp_cloud import run_whatsapp_cloud_setup
...
def cmd_setup(args):
    ...
    from sarthi_cli.setup import run_setup_wizard
...
def cmd_postinstall(args):
    ...
    from sarthi_cli.config import stamp_install_method
    from sarthi_cli.dep_ensure import ensure_dependency

    stamp_install_method("pip")

    print("⚕ Sarthi post-install bootstrap")
    print()
...
def cmd_model(args):
    ...
    if getattr(args, "refresh", False):
        try:
            from sarthi_cli.models import clear_provider_models_cache
...
def select_provider_and_model(args=None):
    ...
    Shared by ``cmd_model`` (``sarthi model``) and the setup wizard
    (``setup_model_provider`` in setup.py).  Handles the full flow:
    provider picker, credential prompting, model selection, and config
    persistence.
    """
    from sarthi_cli.auth import (
        resolve_provider,
        AuthError,
        format_auth_error,
    )
    from sarthi_cli.config import (
        get_compatible_custom_providers,
        load_config,
        get_env_value,
    )
    from sarthi_cli.providers import resolve_provider_full
    ...
    effective_provider = (
        config_provider or os.getenv("SARTHI_INFERENCE_PROVIDER") or "auto"
    )
    compatible_custom_providers = get_compatible_custom_providers(config)
    def _named_custom_provider_map(cfg) -> dict[str, dict[str, str]]:
        from sarthi_cli.config import read_raw_config
    ...
    if not active and effective_provider != "auto":
        ...
        if active_def is not None:
            ...
            warning = (
                f"Unknown provider '{effective_provider}'. Check 'sarthi model' for "
                "available providers, or run 'sarthi doctor' to diagnose config "
                "issues."
            )
    ...
    # PROVIDER_GROUPS in sarthi_cli/models.py). A multi-member group shows one
    ...
    # a leftover OPENAI_BASE_URL in ~/.sarthi/.env can poison auxiliary
...
def _clear_stale_openai_base_url():
    """Remove OPENAI_BASE_URL from ~/.sarthi/.env if the active provider is not 'custom'.
    ...
    """
    from sarthi_cli.config import get_env_value, save_env_value, load_config
...
#
# Sarthi uses lightweight "auxiliary" models for side tasks (vision analysis,
...
# `sarthi model` provider picker. It does NOT re-run credential setup — it
# only routes already-authenticated providers to specific aux tasks. Users
# configure new providers through the normal `sarthi model` flow first.
# ─────────────────────────────────────────────────────────────────────────────
...
def _all_aux_tasks() -> list[tuple[str, str, str]]:
    ...
    :meth:`sarthi_cli.plugins.PluginContext.register_auxiliary_task`) appear
    in the same surfaces as built-in ones without core knowing about them.
    """
    tasks = list(_AUX_TASKS)
    try:
        from sarthi_cli.plugins import get_plugin_auxiliary_tasks
...
def _save_aux_choice(
    ...
    *,
    ...
    """
    from sarthi_cli.config import load_config, save_config
...
def _aux_config_menu() -> None:
    ...
    while True:
        ...
        print("  Sarthi only falls back to a lightweight backend (OpenRouter,")
...
def _aux_select_for_task(task: str) -> None:
    ...
    ``sarthi model`` flow, then route aux tasks to them here.
    """
    from sarthi_cli.config import load_config
    from sarthi_cli.model_switch import list_authenticated_providers
...
def _aux_flow_provider_model(
    ...
    from sarthi_cli.auth import _prompt_model_selection
    from sarthi_cli.models import get_pricing_for_provider
...
def _aux_flow_custom_endpoint(task: str, task_cfg: dict) -> None:
    ...
    from sarthi_cli.secret_prompt import masked_secret_prompt
...
def _prompt_provider_choice(choices, *, default=0, title="Select provider:"):
    ...
    """
    try:
        from sarthi_cli.setup import _curses_prompt_choice
...
def _prompt_custom_api_mode_selection(base_url: str, current_api_mode: str = "") -> Optional[str]:
    ...
    """
    from sarthi_cli.runtime_provider import _detect_api_mode_for_url
    ...
    mode_options = [
        (
            "",
            "Auto-detect",
            "Use Sarthi URL heuristics; best for standard OpenAI-compatible endpoints.",
        ),
        (
            "chat_completions",
            "Chat Completions",
...
def _remove_custom_provider(config):
    ...
    try:
        from sarthi_cli.curses_ui import curses_radiolist
...
# downstream call sites read `sarthi_cli.main._PROVIDER_MODELS` directly,
# so the symbol needs to be reachable as a module attribute. But importing
# the catalog eagerly costs ~55ms on every `sarthi` invocation — including
# fast paths like `sarthi --version` and slash-command dispatch that never
...
def __getattr__(name):
    ...
    if name in _LAZY_MODEL_EXPORTS:
        from sarthi_cli.models import _PROVIDER_MODELS
...
def _prompt_api_key(pconfig, existing_key: str, provider_id: str = "") -> tuple:
    """Shared API-key entry point for ``sarthi setup`` / ``sarthi model``.
    ...
    recover from a malformed paste without editing ``~/.sarthi/.env`` by hand.

    Returns ``(resolved_key, abort)``.  ``abort=True`` means the caller should
    ``return`` immediately — the user cancelled entry, declined to replace, or
    cleared the key and is now unconfigured.
    """
    from sarthi_cli.auth import LMSTUDIO_NOAUTH_PLACEHOLDER
    from sarthi_cli.config import save_env_value
    from sarthi_cli.secret_prompt import masked_secret_prompt
    ...
    from sarthi_cli.env_loader import format_secret_source_suffix
    ...
    if choice.startswith("c"):
        ...
        print(
            f"  API key cleared.  Re-run `sarthi setup` to configure {pconfig.name} again."
        )
        return "", True
...
def _run_anthropic_oauth_flow(save_env_value):
    ...
    def _activate_claude_code_credentials_if_available() -> bool:
        ...
        if creds and (
            ...
            from sarthi_constants import display_sarthi_home as _dhh_fn

            print(
                f"    Sarthi will use Claude's credential store directly instead of copying a setup-token into {_dhh_fn()}/.env."
            )
            return True
        return False
...
def cmd_login(args):
    """Authenticate Sarthi CLI with a provider."""
    from sarthi_cli.auth import login_command
...
def cmd_logout(args):
    ...
    from sarthi_cli.auth import logout_command
...
def cmd_auth(args):
    ...
    from sarthi_cli.auth_commands import auth_command
...
def cmd_status(args):
    ...
    from sarthi_cli.status import show_status
...
def cmd_cron(args):
    """Cron job management."""
    from sarthi_cli.cron import cron_command
...
def cmd_webhook(args):
    ...
    from sarthi_cli.webhook import webhook_command
...
def cmd_slack(args):
    ...
    Dispatches ``sarthi slack <subcommand>``. Currently supports:
    ...
    if sub in {None, ""}:
        ...
        print(
            "usage: sarthi slack <subcommand>\n"
            "\n"
            "subcommands:\n"
            "  manifest   Generate a Slack app manifest with every gateway\n"
            "             command registered as a native slash\n"
            "\n"
            "Run `sarthi slack manifest -h` for details.",
            file=sys.stderr,
        )
        return 1

    if sub == "manifest":
        from sarthi_cli.slack_cli import slack_manifest_command
...
def cmd_kanban(args):
    ...
    from sarthi_cli.kanban import kanban_command
...
def cmd_project(args):
    ...
    from sarthi_cli.projects_cmd import projects_command
...
def cmd_hooks(args):
    ...
    from sarthi_cli.hooks import hooks_command
...
def cmd_doctor(args):
    ...
    from sarthi_cli.doctor import run_doctor

    run_doctor(args)


def cmd_security(args):
    """Dispatch `sarthi security <subcmd>`."""
    sub = getattr(args, "security_command", None)
    if sub in ("audit", None):
        from sarthi_cli.security_audit import cmd_security_audit
...
def cmd_dump(args):
    ...
    from sarthi_cli.dump import run_dump
...
def cmd_debug(args):
    ...
    from sarthi_cli.debug import run_debug
...
def cmd_config(args):
    ...
    from sarthi_cli.config import config_command

    config_command(args)


def cmd_backup(args):
    """Back up Sarthi home directory to a zip file."""
    if getattr(args, "quick", False):
        from sarthi_cli.backup import run_quick_backup

        run_quick_backup(args)
    else:
        from sarthi_cli.backup import run_backup

        run_backup(args)


def cmd_import(args):
    """Restore a Sarthi backup from a zip file."""
    from sarthi_cli.backup import run_import

    run_import(args)


def _print_version_info(*, check_updates: bool = True) -> None:
    from sarthi_cli.config import detect_install_method
    from sarthi_cli.banner import format_banner_version_label
    ...
    try:
        from sarthi_cli.banner import check_for_updates
...
def cmd_uninstall(args):
    """Uninstall Sarthi Agent (or just the Chat GUI with --gui)."""
    ...
    if getattr(args, "gui_summary", False):
        from sarthi_cli.gui_uninstall import gui_install_summary
    ...
    if getattr(args, "gui", False):
        ...
        from sarthi_cli.uninstall import run_gui_uninstall
    ...
    from sarthi_cli.uninstall import run_uninstall
...
# Critical files that Sarthi must be able to import immediately after an
...
_UPDATE_CRITICAL_FILES = (
    "sarthi_cli/main.py",
    "sarthi_cli/config.py",
    "sarthi_cli/__init__.py",
    "sarthi_cli/web_server.py",
    "cli.py",
    "run_agent.py",
    "model_tools.py",
    "toolsets.py",
...
def _validate_critical_files_syntax(root) -> tuple[bool, str | None, str | None]:
    ...
    These are the files imported on every ``sarthi`` startup; if any of them
    ...
    root = Path(root)
    with tempfile.TemporaryDirectory(prefix="sarthi-syntax-check-") as tmpdir:
...
def _gateway_prompt(prompt_text: str, default: str = "", timeout: float = 300.0) -> str:
    ...
    Used by ``sarthi update --gateway`` so interactive prompts (stash restore,
...
def _web_ui_build_needed(web_dir: Path) -> bool:
    ...
    still outputs to ``sarthi_cli/web_dist/`` (per vite.config.ts
    outDir: "../sarthi_cli/web_dist"), NOT to ``web/dist/``, so Python
    ...
    dist_dir = project_root / "sarthi_cli" / "web_dist"
...
def _run_with_idle_timeout(
    ...
    *,
    ...
    ``sarthi`` launcher present but ``sarthi_cli`` not importable.
...
def _nixos_build_env() -> dict[str, str] | None:
    ...
    Two-tier resolution:
    1. Fast path — the sarthi venv's python3 (present in managed installs)
    ...
    # Tier 1: fast path — sarthi venv python3, no nix-shell overhead
    ...
    # a sarthi venv (pip / non-managed / bare-git installs).  The resolved
...
def _run_npm_install_deterministic(
    ...
    *,
    ...
    the working tree dirty and causes the next ``sarthi update`` to stash the
    lockfile — repeatedly.
    """
...
def _build_web_ui(web_dir: Path, *, fatal: bool = False) -> bool:
    ...
    Args:
        ...
        fatal: If True, print error guidance and return False on failure
               instead of a soft warning (used by ``sarthi web``).
    ...
    # build path usable on a stock `py -m sarthi_cli.main web` invocation.
    ...
    from sarthi_constants import with_sarthi_node_path
    ...
    if not npm:
        ...
        return not fatal
    build_env = with_sarthi_node_path()
    ...
    if r1.returncode != 0:
        _say(
            ...
            + ("" if fatal else " (sarthi web will not be available)")
        )
        _relay(r1)
        if fatal:
...
#   - ``sarthi update`` can unconditionally call ``sarthi desktop --build-only``
#     and it will skip if nothing actually changed
#   - ``sarthi desktop`` (interactive launch) skips the build when the
#     stamp matches, making repeated launches fast
#
# Stamp file: $SARTHI_HOME/desktop-build-stamp.json
# Schema:
#   {
...
def _desktop_stamp_path() -> Path:
    """Return the path to the desktop build stamp file under $SARTHI_HOME."""
    from sarthi_constants import get_sarthi_home
    return get_sarthi_home() / "desktop-build-stamp.json"
...
def _desktop_build_needed(desktop_dir: Path, project_root: Path, *, source_mode: bool) -> bool:
    ...
    ``sarthi update`` that pulled new source but hasn't built yet).
    """
...
def _desktop_packaged_executable(desktop_dir: Path) -> Optional[Path]:
    ...
    if sys.platform == "darwin":
        candidates = list(release_dir.glob("mac*/Sarthi.app/Contents/MacOS/Sarthi"))
    elif sys.platform == "win32":
        candidates = [
            release_dir / "win-unpacked" / "Sarthi.exe",
            release_dir / "win-ia32-unpacked" / "Sarthi.exe",
            release_dir / "win-arm64-unpacked" / "Sarthi.exe",
        ]
    else:
        candidates = [
            release_dir / "linux-unpacked" / "sarthi",
            release_dir / "linux-unpacked" / "Sarthi",
            release_dir / "linux-arm64-unpacked" / "sarthi",
            release_dir / "linux-arm64-unpacked" / "Sarthi",
        ]
...
def _purge_electron_build_cache(desktop_dir: Path) -> list[Path]:
    ...
    '…/linux-unpacked/Sarthi'`` desktop build failure: a corrupt zip in the
...
def _redownload_electron_dist(
    ...
    *,
    ...
    dl_env = with_sarthi_node_path(env)
    if mirror:
...
def _stop_desktop_processes_locking_build(desktop_dir: Path) -> list[int]:
    ...
    On Windows a running ``Sarthi.exe`` keeps an exclusive lock on
    ``release/win-unpacked/Sarthi.exe``. electron-builder's pack then can't
    delete the stale binary and dies with ``remove …\\Sarthi.exe: Access is
    ...
    an unrelated "Sarthi" process is never touched. Best-effort: never raises.
...
def _desktop_macos_relaunchable_fixup(desktop_dir: Path) -> None:
    ...
    tampering and macOS reports "Sarthi is damaged and can't be opened." The
    ...
    if exe is None:
        return
    # exe = .../Sarthi.app/Contents/MacOS/Sarthi  ->  app bundle = .../Sarthi.app
    app = exe.parents[2]
...
def _force_adhoc_macos_signing(env: dict, *, source_mode: bool) -> bool:
    ...
    machine* (``sarthi desktop --build-only`` → electron-builder ``--dir``).
...
def _desktop_linux_sandbox_fixup(packaged_executable: Path) -> bool:
    ...
    if not sandbox.exists():
        print(f"✗ Sarthi Desktop is missing Electron's Linux sandbox helper: {sandbox}")
        return False
    ...
    if not sudo:
        print("✗ Sarthi Desktop requires sudo to configure Electron's Linux sandbox helper.")
        return False
...
def _desktop_launch_options() -> tuple[list[str], str]:
    ...
    (normalized for the SARTHI_DESKTOP_DISABLE_GPU env var the Electron app
...
def cmd_gui(args: argparse.Namespace):
    ...
    try:
        from sarthi_logging import setup_logging as _setup_logging_gui
    ...
    # with_sarthi_node_path() copies os.environ when called with no arg.
    env = with_sarthi_node_path()
    if getattr(args, "fake_boot", False):
        env["SARTHI_DESKTOP_BOOT_FAKE"] = "1"
    if getattr(args, "ignore_existing", False):
        env["SARTHI_DESKTOP_IGNORE_EXISTING"] = "1"
    if getattr(args, "sarthi_root", None):
        env["SARTHI_DESKTOP_SARTHI_ROOT"] = str(Path(args.sarthi_root).expanduser().resolve())
    if getattr(args, "cwd", None):
        env["SARTHI_DESKTOP_CWD"] = str(Path(args.cwd).expanduser().resolve())
    else:
        env["SARTHI_DESKTOP_CWD"] = os.getcwd()
    ...
    # `SARTHI_DESKTOP_DISABLE_GPU=... sarthi desktop` keeps working.
    config_electron_flags, config_disable_gpu = _desktop_launch_options()
    if config_disable_gpu != "auto" and "SARTHI_DESKTOP_DISABLE_GPU" not in os.environ:
        env["SARTHI_DESKTOP_DISABLE_GPU"] = config_disable_gpu
    ...
    if source_mode or not skip_build:
        ...
        if not npm:
            ...
            print("Install Node.js, then run:  sarthi gui")
            sys.exit(1)
    else:
        npm = None
    ...
    if skip_build:
        ...
        if not build_needed:
            ...
            if not source_mode:
                ...
                # holds Sarthi.exe locked on Windows, so the pack can't replace
            ...
            if (
                ...
                if restored:
                    ...
                    for p in purged:
                        print(f"    - {p}")
                    # The purge can't remove a win-unpacked tree whose Sarthi.exe
            ...
            if build_result.returncode != 0:
                ...
                if sys.platform == "win32":
                    print("  If this says \"Access is denied\" on Sarthi.exe, close any")
                    print("  running Sarthi desktop window and retry.")
                print("  If the log shows Electron download retries, rebuild via a mirror:")
                print("    ELECTRON_MIRROR=<mirror-base-url> sarthi desktop --force-build")
                sys.exit(build_result.returncode or 1)
            packaged_executable = _desktop_packaged_executable(desktop_dir)
            if not source_mode:
                # Locally-built apps are ad-hoc signed; make them relaunchable after
                # an in-place self-update (otherwise macOS reports "Sarthi is
    ...
    if source_mode:
        print("→ Launching Sarthi Desktop from source build...")
    ...
    print(f"→ Launching packaged Sarthi Desktop: {' '.join(launch_command)}")
...
def _find_stale_dashboard_pids(
    *,
    ...
    """Return PIDs of ``sarthi dashboard`` processes other than ourselves.

    ``sarthi dashboard`` is a long-lived server process commonly started and
    forgotten.  When ``sarthi update`` replaces files on disk, the running
    ...
    *exclude_pids* is an optional set of PIDs that must never be returned.
    This is used by the Sarthi Desktop Electron app to protect its own
    backend child process: when the desktop spawns ``sarthi serve`` as
    ...
    environment variable ``SARTHI_DESKTOP_CHILD_PID`` on the spawned
    ...
    patterns = [
        "sarthi dashboard",
        "sarthi_cli.main dashboard",
        "sarthi_cli/main.py dashboard",
        # The headless backend (`sarthi serve`) is the same long-lived server
        # under a different command name — the desktop app spawns it. Reap it
        # on update for the same frontend/backend-mismatch reason.
        "sarthi serve",
        "sarthi_cli.main serve",
        "sarthi_cli/main.py serve",
    ]
    self_pid = os.getpid()
    ...
    try:
        if sys.platform == "win32":
            ...
            from sarthi_cli._subprocess_compat import windows_hide_flags
            ...
            # (rather than `pgrep -f "sarthi.*dashboard"`) keeps us consistent
            # with `sarthi_cli.gateway._scan_gateway_pids` and avoids the
...
def _print_curator_first_run_notice() -> None:
    """Print a short heads-up about the skill curator after `sarthi update`.
    ...
    print("  Preview now:  sarthi curator run --dry-run")
    print("  Pause it:     sarthi curator pause")
    print(
        "  Docs:         https://sarthi-agent.vercel.app/docs/user-guide/features/curator"
    )
...
def _print_curator_recent_run_notice() -> None:
    ...
    rename. ``sarthi update`` is a high-attention surface — surface the
    most recent run's rename map here, once.

    Show-once: state stamps ``last_run_summary_shown_at`` after printing.
    Subsequent ``sarthi update`` invocations skip the block until a newer
    ...
    print(
        ...
        "View anytime: sarthi curator status)"
    )
...
def _kill_stale_dashboard_processes(
    ...
    """Kill running ``sarthi dashboard`` processes.

    Called at the end of ``sarthi update`` (default ``reason``) and also
    from ``sarthi dashboard --stop`` (which overrides ``reason``).  The
    ...
    """
    # When the Sarthi Desktop Electron app spawns this dashboard as a
    # backend child, it sets SARTHI_DESKTOP_CHILD_PID so that the update
    # path can skip killing the desktop-managed process.  (#37532)
    exclude: set[int] | None = None
    raw_pid = os.environ.get("SARTHI_DESKTOP_CHILD_PID")
    if raw_pid:
    ...
    if killed:
        ...
        print("    sarthi dashboard --port <port>")
...
def _atomic_replace_dir(src: str, dst: str) -> None:
    ...
    """
    staging = f"{dst}.sarthi-update-staging"
    backup = f"{dst}.sarthi-update-old"
...
def _update_via_zip(args):
    """Update Sarthi Agent by downloading a ZIP archive.
    ...
    if branch != "main":
        ...
        print(
            ...
            "or NTFS filter holding files open) and rerun `sarthi update "
            f"--branch {branch}`, or update against main with `sarthi update`."
        )
        sys.exit(1)
    zip_url = (
        f"https://github.com/jitendra-singh-thakur/sarthi-agent/archive/refs/heads/{branch}.zip"
    )

    print("→ Downloading latest version...")
    tmp_dir = tempfile.mkdtemp(prefix="sarthi-update-")
    try:
        zip_path = os.path.join(tmp_dir, f"sarthi-agent-{branch}.zip")
        ...
        with zipfile.ZipFile(zip_path, "r") as zf:
            ...
            # symlink members. A GitHub source ZIP for sarthi-agent itself
        ...
        # GitHub ZIPs extract to sarthi-agent-<branch>/
        extracted = os.path.join(tmp_dir, f"sarthi-agent-{branch}")
    ...
    from sarthi_cli.managed_uv import ensure_uv, update_managed_uv
    ...
    try:
        ...
        if result.get("user_modified"):
            ...
            print(
                "    → see them: sarthi skills list-modified  "
    ...
    try:
        from sarthi_cli.model_catalog import seed_cache_from_checkout
...
def _stash_local_changes_if_needed(git_cmd: list[str], cwd: Path) -> Optional[str]:
    ...
    stash_name = datetime.now(timezone.utc).strftime(
        "sarthi-update-autostash-%Y%m%d-%H%M%S"
    )
...
def _restore_stashed_changes(
    ...
    if prompt_user:
        ...
        print("  Review the result afterward if Sarthi behaves unexpectedly.")
    ...
    if restore.returncode != 0 or has_conflicts:
        ...
        # files makes sarthi completely unrunnable (SyntaxError on import).
    ...
    if stash_selector is None:
        print(
            "⚠ Local changes were restored, but Sarthi couldn't find the stash entry to drop."
        )
        print(
        ...
        if drop.returncode != 0:
            print(
                "⚠ Local changes were restored, but Sarthi couldn't drop the saved stash entry."
            )
    ...
    print("  Review `git diff` / `git status` if Sarthi behaves unexpectedly.")
    return True
...
def _discard_stashed_changes(
    ...
    if stash_selector is None:
        print(
            ...
            "but Sarthi couldn't find the stash entry to drop."
        )
        _print_stash_cleanup_guidance(stash_ref)
        return False
    ...
    if drop.returncode != 0:
        print(
            "⚠ Configured to discard local changes, but Sarthi couldn't drop "
...
# =========================================================================
# Fork detection and upstream management for `sarthi update`
# =========================================================================

OFFICIAL_REPO_URLS = {
    "https://github.com/jitendra-singh-thakur/sarthi-agent.git",
    "git@github.com:NousResearch/sarthi-agent.git",
    "https://github.com/jitendra-singh-thakur/sarthi-agent",
    "git@github.com:NousResearch/sarthi-agent",
}
OFFICIAL_REPO_URL = "https://github.com/jitendra-singh-thakur/sarthi-agent.git"
...
def _should_skip_upstream_prompt() -> bool:
    ...
    return (get_sarthi_home() / SKIP_UPSTREAM_PROMPT_FILE).exists()
...
def _mark_skip_upstream_prompt():
    ...
    try:
        ...
        (get_sarthi_home() / SKIP_UPSTREAM_PROMPT_FILE).touch()
    except Exception:
        pass
...
def _sync_with_upstream_if_needed(git_cmd: list[str], cwd: Path) -> None:
    ...
    if not has_upstream:
        ...
        print()
        print("ℹ Your fork is not tracking the official Sarthi repository.")
        print("  This means you may miss updates from NousResearch/sarthi-agent.")
        print()
        try:
            response = (
        ...
        if response in {"", "y", "yes"}:
            ...
            if _add_upstream_remote(git_cmd, cwd):
                print(
                    "  ✓ Added upstream: https://github.com/jitendra-singh-thakur/sarthi-agent.git"
                )
                has_upstream = True
            else:
            ...
            print(
                "  Skipped. Run 'git remote add upstream https://github.com/jitendra-singh-thakur/sarthi-agent.git' to add later."
            )
...
def _invalidate_update_cache():
    ...
    ``sarthi update``, every profile is now current.
    """
    homes = []
    # Default profile home (Docker-aware — uses /opt/data in Docker)
    from sarthi_constants import get_default_sarthi_root

    default_home = get_default_sarthi_root()
...
# Install-scoped breadcrumb dropped right before ``sarthi update`` mutates the
...
# survives and the next ``sarthi`` launch finishes the install instead of
# limping along on a half-built venv (e.g. pip wiped, a core dep like Pillow
# never landed).  Lives next to the venv (not under $SARTHI_HOME) because the
...
def _recover_from_interrupted_install() -> None:
    """Finish a dependency install that a prior ``sarthi update`` left half-done.
    ...
    protocol stream (``sarthi acp`` speaks JSON-RPC on stdout) must never get
    ...
    # Windows self-lock guard: if sarthi.exe is the launcher that spawned
    ...
    if _is_windows():
        ...
        if scripts_dir is not None:
            shims = _sarthi_exe_shims(scripts_dir)
            if shims:
                ...
                try:
                    ...
                    for _anc in [_me] + list(_me.parents()):
                        ...
                        if _anc_norm in _shim_set:
                            print(
                                "✗ Sarthi is running from the binary that "
                                "needs to be replaced — the auto-recovery "
                                "cannot overwrite a running executable."
                            )
                            print(
                                "  Restart Sarthi from a different terminal, "
    ...
    try:
        ...
        print(
            "⚠ A previous `sarthi update` was interrupted mid-install — "
            "finishing dependency installation now..."
        )

        try:
            from sarthi_cli.managed_uv import ensure_uv
...
def _run_install_with_heartbeat(
    ...
    *,
    ...
    know ``sarthi update`` is still progressing even if pip/uv itself is silent.
    """
...
def _sarthi_exe_shims(scripts_dir: Path) -> list[Path]:
...
def _detect_concurrent_sarthi_instances(
    ...
    The Sarthi Desktop Electron app spawns ``sarthi.EXE`` as a backend child,
    so during ``sarthi update`` the user-invoked process and the desktop's
    ...
    shims (``sarthi.exe`` / ``sarthi-gateway.exe``) and returns ``(pid,
    process_name)`` pairs. The caller's own PID and its entire ancestor
    chain are excluded so the running ``sarthi update`` invocation never
    reports itself — this matters on Windows where the setuptools .exe
    launcher (``sarthi.exe``) is a separate process from the Python
    ...
    for shim in _sarthi_exe_shims(scripts_dir):
        try:
    ...
    # setuptools-generated sarthi.exe launcher is a separate native process
    ...
    # file lock) is the parent. Without excluding it, every ``sarthi update``
    ...
    #      sarthi.exe sitting *under* a non-Sarthi parent (e.g. a Sarthi
...
def _format_concurrent_instances_message(
    ...
    shim = scripts_dir / "sarthi.exe"
    lines = ["✗ Another sarthi.exe is running:"]
    ...
    lines.append("")
    lines.append("  Close Sarthi Desktop, exit any open `sarthi` REPLs, and")
    lines.append("  stop the gateway (`sarthi gateway stop`) before retrying.")
    lines.append("")
    if matches:
        ...
        lines.append("")
    lines.append("  Override with `sarthi update --force` if you've already")
    lines.append("  confirmed those processes will not write to the venv.")
    return "\n".join(lines)


def _quarantine_running_sarthi_exe(
    scripts_dir: Path, *, max_attempts: int = 4
) -> list[tuple[Path, Path]]:
    """Pre-empt Windows file lock on the running ``sarthi.exe``.
    ...
    when ``sarthi update`` runs, ``sarthi.exe`` IS the live process, and uv
    fails with ``Access is denied. (os error 5)``.

    We rename live shims to ``sarthi.exe.old.<unix-ms>`` first. uv then writes
    fresh shims at the original paths. The ``.old`` files are cleaned up on
    the next sarthi invocation by ``_cleanup_quarantined_exes``.
    ...
    handles (recovers in <1s), or the Sarthi Desktop backend child process
    ...
    3. Print a clear warning naming the most likely culprit (running
       Sarthi Desktop / gateway / REPL) and pointing to ``--force``.
    ...
    for shim in _sarthi_exe_shims(scripts_dir):
        ...
        print(
            "    Close Sarthi Desktop, exit other `sarthi` REPLs, stop the "
            "gateway, or pause AV scanning, then re-run `sarthi update`."
        )
...
def _restore_quarantined_exes(moved: list[tuple[Path, Path]]) -> None:
    """Roll back ``_quarantine_running_sarthi_exe`` if uv didn't write replacements."""
...
def _run_quarantined_install(
    ...
    *,
    ...
    """Run an editable install, quarantining the running ``sarthi.exe`` first.

    Any ``pip install -e .`` (or ``--reinstall``) rewrites the entry-point
    shims, and on Windows the live ``sarthi.exe`` is the running process —
    pip can neither delete nor overwrite it, so without quarantine the shim
    is left missing and ``sarthi`` drops off PATH. This wraps
    ...
    moved: list[tuple[Path, Path]] = []
    if scripts_dir is not None:
        moved = _quarantine_running_sarthi_exe(scripts_dir)
    try:
...
def _cleanup_quarantined_exes(scripts_dir: Path | None = None) -> None:
    """Sweep ``sarthi.exe.old.*`` left by prior updates.

    Called early on every sarthi invocation. The .old files are unlocked once
...
def _refresh_active_lazy_features() -> None:
    ...
    first use. ``sarthi update`` runs ``uv pip install -e .[all]`` which
    ...
    if failed:
        ...
        print("  `sarthi update` once the upstream issue is resolved.")
...
def _install_python_dependencies_with_optional_fallback(
    ...
    *,
    ...
    On Windows, pre-renames live ``sarthi.exe`` / ``sarthi-gateway.exe`` shims
    in the venv Scripts dir before each install attempt so uv can write fresh
    copies (Windows blocks REPLACE on a running .exe but allows RENAME). See
    ``_quarantine_running_sarthi_exe`` for the rationale.
    """
    ...
    # hours later inside ``sarthi update``'s desktop-rebuild or skill-sync
...
def _verify_console_scripts_installed(
    ...
    *,
    ...
    On Windows, ``uv pip install -e .`` can register ``sarthi.exe`` in the
    wheel RECORD while the file never lands on disk — typically when the live
    ``sarthi.exe`` shim is locked during ``sarthi update``, or when uv/distlib
    skips a launcher write. The symptom is ``sarthi-agent.exe`` and
    ``sarthi-acp.exe`` present but ``sarthi.exe`` missing, so ``sarthi`` drops
    ...
    try:
        ...
        print(
            "  ⚠ Entry point repair failed; try `sarthi update --force` after "
            "closing other sarthi processes."
        )
        return
    ...
    if still_missing:
        print(
            ...
            "Workaround: python -m sarthi_cli.main <command>"
        )
    else:
...
def _verify_core_dependencies_installed(
    ...
    *,
    ...
    # outer Python that drove ``sarthi update``, not the venv we just wrote
    ...
    #
    # Quarantine the running ``sarthi.exe`` first: ``--reinstall -e .``
    # rewrites the entry-point shims, and on Windows pip can't overwrite the
    # live launcher, which would leave ``sarthi`` off PATH.
    scripts_dir = _venv_scripts_dir() if _is_windows() else None
    ...
    try:
        ...
        print("  ⚠ Repair install failed; check `sarthi update` output above.")
        return
    ...
    try:
        ...
        print(
            ...
            "Run `sarthi update --force` after closing other sarthi processes."
        )
        return
...
def _install_psutil_android_compat(
    ...
    *,
    ...
    from sarthi_cli.psutil_android import PSUTIL_URL, prepare_patched_psutil_sdist
...
def _ensure_uv_for_termux(pip_cmd: list[str]) -> str | None:
    ...
    standalone uv into ``$SARTHI_HOME/bin/uv``, but on Termux the official
    ...
    """
    from sarthi_cli.managed_uv import resolve_uv
...
def _npm_manifest_paths() -> tuple[Path, ...]:
    ...
    lockfile is then unchanged but `sarthi update` is exactly the step
...
def _npm_lockfile_changed(sarthi_root: Path) -> bool:
    ...
    try:
        ...
        cache_file = sarthi_root / f".npm_lock_hash_{cache_key}"
...
def _record_npm_lockfile_hash(sarthi_root: Path) -> None:
...
def _resolve_node_runtime_npm() -> str | None:
    ...
    """
    from sarthi_constants import find_node_executable
...
def _update_node_dependencies() -> list[str]:
    ...
    if not npm:
        ...
        from sarthi_constants import is_wsl
        ...
        if is_wsl() and path_npm and _is_windows_npm_path(path_npm):
            ...
            print("    package manager), then re-run `sarthi update`.")
            failed = ["repo root"]
            if any(
    ...
    # Sarthi profile using this checkout. Keep one per-checkout cache under the
    # shared Sarthi root rather than rerunning npm once per named profile.
    shared_sarthi_root = get_default_sarthi_root()
    if not _npm_lockfile_changed(shared_sarthi_root):
    ...
    # need desktop during `sarthi update`, so we install root-only first
    ...
    def _partial_update_failure(*labels: str) -> list[str]:
        ...
        print("    deps). Fix npm and re-run `sarthi update`.")
        return list(labels)
    ...
    nixos_env = with_sarthi_node_path(_nixos_build_env())
    ...
    # hung. The chatty npm-deprecation noise during `sarthi update` comes from
    ...
    if ws_result.returncode == 0:
        _record_npm_lockfile_hash(shared_sarthi_root)
...
class _UpdateOutputStream:
    """Stream wrapper used during ``sarthi update`` to survive terminal loss.

    Wraps the process's original stdout/stderr so that:

    * Every write is also mirrored to an append-only log file
      (``~/.sarthi/logs/update.log``) that users can inspect after the
    ...
    * Writes to the original stream that fail with ``BrokenPipeError`` /
    ...
    ``_install_hangup_protection``, this makes ``sarthi update`` safe to
...
def _install_hangup_protection(gateway_mode: bool = False):
    ...
    Users commonly run ``sarthi update`` in an SSH session or a terminal
    ...
    2. ``sys.stdout`` / ``sys.stderr`` are wrapped to mirror output to
       ``~/.sarthi/logs/update.log`` and to silently absorb
    ...
    **intentionally left alone** — those are legitimate cancellation
    ...
    In gateway mode (``sarthi update --gateway``) the update is already
    ...
    try:
        ...
        # sarthi_cli.config.get_sarthi_home to simulate setup failure.
        from sarthi_cli.config import get_sarthi_home as _get_sarthi_home

        logs_dir = _get_sarthi_home() / "logs"
...
def _log_only_write(text: str) -> None:
    """Write ``text`` to ``~/.sarthi/logs/update.log`` only, never the terminal.

    During ``sarthi update`` ``sys.stdout`` is an ``_UpdateOutputStream`` that
...
def _cmd_update_check(branch: str = "main", *, branch_explicit: bool = False):
    """Implement ``sarthi update --check``: fetch and report without installing.
    ...
    if method == "docker":
        ...
        # same long-form ``docker pull`` guidance ``sarthi update`` (apply
        # path) uses — telling the user to "reinstall via curl" or that
        # ".git is missing" would point them at the wrong remediation.
        from sarthi_cli.config import format_docker_update_message
        print(format_docker_update_message())
        sys.exit(1)
    if method == "pip":
        from sarthi_cli.config import recommended_update_command
        from sarthi_cli.banner import check_via_pypi
...
def _ensure_fhs_path_guard() -> None:
    ...
    repaired on ``sarthi update`` without requiring a reinstall.  The
    ...
    ``sarthi`` prints ``command not found`` even though the symlink lives
    at /usr/local/bin/sarthi.

    Silent no-op on: non-Linux, non-root, non-FHS installs, and any system
    where ``bash -i -c 'command -v sarthi'`` already resolves.  Idempotent.
    """
    if sys.platform != "linux":
        return
    try:
    ...
    # /usr/local/bin/sarthi, code at /usr/local/lib/sarthi-agent).
    fhs_link = Path("/usr/local/bin/sarthi")
    ...
    path_comment = (
        "# Sarthi Agent — ensure /usr/local/bin is on PATH " "(RHEL non-login shells)"
    )
    wrote_any = False
...
def _run_pre_update_backup(args) -> None:
    """Create a full zip backup of SARTHI_HOME before running the update.
    ...
    SARTHI_HOME directories.  The ``--backup`` flag on ``sarthi update``
    ...
    # ``sarthi_cli/config.py``). Fall back to false if the key is missing
    # so the default behaviour matches the shipped config: zipping a large
    # SARTHI_HOME can add minutes to every update. Users who want the
    ...
    try:
        from sarthi_cli.backup import create_pre_update_backup
    except Exception as exc:
        print(
    ...
    # Render path using display_sarthi_home so the user sees ~/.sarthi/...
    try:
        from sarthi_constants import get_sarthi_home, display_sarthi_home

        home = get_sarthi_home()
        try:
            display_path = f"{display_sarthi_home()}/{out_path.relative_to(home)}"
        except ValueError:
    ...
    print(f"  Restore:  sarthi import {out_path}")
...
def _venv_core_imports_healthy() -> tuple[bool, str]:
    ...
    ``sarthi update`` may be driven by a different Python). Catches the
    ...
    Without this probe, ``sarthi update`` on a current checkout prints
    ...
    if not venv_python.exists():
        ...
        # dev may run sarthi from any interpreter), so report healthy to
        # avoid forcing reinstalls. But on a MANAGED install (the Windows
        # installer / desktop bootstrap stamps `.sarthi-bootstrap-complete`,
        ...
        managed_markers = (
            PROJECT_ROOT / ".sarthi-bootstrap-complete",
            _update_marker_path(),
        )
...
def _detect_venv_python_processes(
    *, exclude_pids: set[int] | None = None
    ...
    The sarthi.exe shim guard misses the biggest lock-holder class on
    Windows: the Desktop app's backend (``python.exe -m sarthi_cli.main
    ...
    calling process and its ancestors are always excluded (a CLI ``sarthi
    ...
    for proc in proc_iter:
        ...
        # this venv's path, or a `-m sarthi_cli.main ...` invocation tied to
        # this install (install root in the cmdline or as the working dir).
        if not is_holder and venv_prefix in cmdline_low:
            is_holder = True
        if not is_holder and "sarthi_cli.main" in cmdline_low:
...
def _format_venv_python_holders_message(matches: list[tuple[int, str, str]]) -> str:
    ...
    lines = [
        "✗ Other Sarthi processes are running from this install's venv:",
    ]
    for pid, name, cmdline in matches[:6]:
        hint = ""
        low = cmdline.lower()
        if "serve" in low or "dashboard" in low:
            hint = "  ← Sarthi Desktop backend (close the desktop app)"
        elif "gateway" in low:
            hint = "  ← gateway"
    ...
    lines.append(
        "  Close the Sarthi desktop app / other Sarthi terminals, then re-run:"
    )
    lines.append("    sarthi update")
    lines.append("  (or use `sarthi update --force-venv` to proceed anyway at your own risk)")
    return "\n".join(lines)
...
def _pause_windows_gateways_for_update() -> dict | None:
    ...
    sarthi.exe concurrent-instance guard does not see them. They still import
    ...
    try:
        ...
        from sarthi_cli.gateway import (
    ...
    if not running_pids:
        ...
        try:
            from sarthi_cli import gateway_windows
    ...
    print("→ Stopping Windows gateway process(es) before updating Sarthi...")
    try:
    ...
    # — e.g. a Windows Scheduled Task running ``pythonw.exe -m sarthi_cli.main
    ...
    if unmapped_pids:
        ...
        if respawnable < len(unmapped_pids):
            ...
            print("    Restart manually after update: sarthi gateway run")
...
def _cold_start_windows_gateway_after_update() -> None:
    ...
    ``sarthi gateway start`` uses (``gateway_windows._spawn_detached``).
    ...
    try:
        from sarthi_cli import gateway_windows
        from sarthi_cli.gateway import find_gateway_pids
    except Exception as exc:
...
def _discard_lockfile_churn(git_cmd, repo_root):
    ...
    ``sarthi update`` sees a clean tree instead of autostashing every run.
...
def cmd_update(args):
    """Update Sarthi Agent to the latest version.
    ...
    if is_managed():
        managed_error("update Sarthi Agent")
        return
    ...
    if getattr(args, "check", False):
        ...
        # what a subsequent `sarthi update --branch=<x>` would actually pull.
        branch = _resolve_update_branch(args)
        _cmd_update_check(
            branch=branch,
...
def _cmd_update_pip(args):
    """Update Sarthi via pip (for PyPI installs)."""
    from sarthi_cli import __version__
    from sarthi_cli.config import is_uv_tool_install
    ...
    if is_uv_tool_install():
        if not uv:
            ...
            sys.exit(1)
        cmd = [uv, "tool", "upgrade", "sarthi-agent"]
        ...
        cmd = [pipx, "upgrade", "sarthi-agent"]
    elif uv:
        cmd = [uv, "pip", "install", "--upgrade", "sarthi-agent"]
        if in_venv:
        ...
        cmd = [sys.executable, "-m", "pip", "install", "--upgrade", "sarthi-agent"]
    ...
    print("✓ Update complete! Restart sarthi to use the new version.")
...
def _cmd_update_impl(args, gateway_mode: bool):
    ...
    print("⚕ Updating Sarthi Agent...")
    print()

    # On Windows, abort early if another sarthi.exe is holding the venv shim
    ...
    if _is_windows() and not getattr(args, "force", False):
        scripts_dir = _venv_scripts_dir()
        if scripts_dir is not None:
            concurrent = _detect_concurrent_sarthi_instances(scripts_dir)
            if concurrent:
    ...
    # (most commonly the Desktop app's `sarthi serve` backend) will keep .pyd
    ...
    # the sarthi.exe shim guard above, but its lock probe only checks the shim
    ...
    try:
        ...
        if commit_count == 0:
            ...
            if not healthy:
                ...
                if healthy_after:
                    ...
                    print("  Close all Sarthi windows/gateways and re-run: sarthi update")
            else:
                print("✓ Already up to date!")
            _resume_windows_gateways_after_update(_windows_gateway_resume)
            return
        ...
        # though `git pull` can't touch $SARTHI_HOME, this is cheap
        ...
        try:
            from sarthi_cli.backup import create_quick_snapshot
        ...
        # orphan merge-conflict markers in sarthi_cli/config.py bricked
        # every user who ran ``sarthi update`` for the 7 minutes between
        ...
        try:
            ...
            # so the CLI stays bootable. The user can then retry ``sarthi
            ...
            if not syntax_ok:
                ...
                if pre_pull_sha:
                    ...
                    if rollback_result.returncode == 0:
                        ...
                        print("  Try ``sarthi update`` again later once a fix lands.")
                    else:
        ...
        # the old bytecode (e.g. get_sarthi_home added to sarthi_constants).
        removed = _clear_bytecode_cache(PROJECT_ROOT)
        if removed:
            print(
                f"  ✓ Cleared {removed} stale __pycache__ director{'y' if removed == 1 else 'ies'}"
        ...
        # the marker survives and the next ``sarthi`` launch finishes the
        ...
        # build.  ``sarthi desktop --build-only`` uses the content-hash stamp
        ...
        # never run ``sarthi desktop`` shouldn't be forced into a full
        # Electron build by ``sarthi update``.
        desktop_dir = PROJECT_ROOT / "apps" / "desktop"
        has_desktop_app = _desktop_packaged_executable(desktop_dir) is not None or _desktop_dist_exists(desktop_dir)
        if (desktop_dir / "package.json").exists() and _resolve_node_runtime_npm() and has_desktop_app:
            print("→ Checking if desktop app needs rebuilding...")
            _desktop_build_cmd = [sys.executable, "-m", "sarthi_cli.main", "desktop", "--build-only"]
            ...
            if build_result.returncode != 0:
                build_result = _run_logged_subprocess(_desktop_build_cmd, cwd=PROJECT_ROOT)
            if build_result.returncode != 0:
                print("  ⚠ Desktop build failed (non-fatal; run `sarthi desktop` to retry)")
                tail = "\n".join((build_result.stdout or "").strip().splitlines()[-15:])
                if tail:
                    print(tail)
                from sarthi_constants import display_sarthi_home as _dhh
        ...
        # current — so copy it straight over ~/.sarthi/cache/model_catalog.json
        ...
        # modules in this process.  Reload sarthi_constants so that any lazy
        # import executed below (skills sync, gateway restart) sees new
        # attributes like display_sarthi_home() added since the last release.
        try:
            import importlib
            import sarthi_constants as _hc
        ...
        # seed_profile_skills() uses subprocess with an explicit SARTHI_HOME so
        # it is not affected by sync_skills()'s module-level SARTHI_HOME cache,
        # which means the active profile is reliably synced regardless of whether
        # the caller's SARTHI_HOME env var points at the default or a named profile.
        try:
            from sarthi_cli.profiles import (
                list_profiles,
                seed_profile_skills,
            )
        ...
        try:
            from sarthi_cli.profiles import backfill_profile_envs
        ...
        if version_bump_only:
            ...
            try:
                ...
                print("     Run 'sarthi config migrate' to retry.")
            ...
            if response in {"", "y", "yes", "auto"}:
                ...
                if (gateway_mode or assume_yes or response == "auto") and missing_env:
                    print("  ℹ API keys require manual entry: sarthi config migrate")
            else:
                print()
                print("Skipped. Run 'sarthi config migrate' later to configure.")
        else:
        ...
        # it and warn loudly.
        try:
            from sarthi_cli.backup import restore_cron_jobs_if_emptied
        ...
        # check `sarthi curator status`. Self-stamps after printing so it
        ...
        # don't have it. Tying the refresh to ``sarthi update`` gives users a
        ...
        try:
            ...
            if (
                ...
                from sarthi_cli.tools_config import install_cua_driver
        ...
        # When running as ``sarthi update --gateway`` (spawned by the gateway's
        ...
        if gateway_mode:
            _exit_code_path = get_sarthi_home() / ".update_exit_code"
            try:
        ...
        try:
            ...
            def _resolve_manage_cmd(scope_: str, scope_cmd_: list, svc_name_: str):
                ...
                ``systemctl ... sarthi-gateway*`` also qualifies
                ...
                if (
                    ...
                    try:
                        ...
                        if not sudo_ok:
                            ...
                            # (NOPASSWD for systemctl ... sarthi-gateway*)
            ...
            # Discover all sarthi-gateway* units (default + profiles)
                ...
                for scope, scope_cmd in [
                    ...
                    try:
                        ...
                        for line in result.stdout.strip().splitlines():
                            ...
                            unit = parts[
                                0
                            ]  # e.g. sarthi-gateway.service or sarthi-gateway-coder.service
                            ...
                            if _manage_cmd is None:
                                print(
                                    ...
                                    f"    To let `sarthi update` restart it automatically, allow\n"
                            ...
                            # path in `sarthi gateway restart`
                        ...
                        print(
                            ...
                            f"Check the gateway with: sarthi gateway status"
                        )
            ...
            if restarted_services or killed_pids:
                ...
                if unmapped_count:
                    ...
                    print("    Restart manually: sarthi gateway run")
                    if unmapped_count > 1:
                        print(
                            "    (or: sarthi -p <profile> gateway run  for each profile)"
                        )
        ...
        # Warn if legacy Sarthi gateway unit files are still installed.
        # When both sarthi.service (from a pre-rename install) and the
        # current sarthi-gateway.service are enabled, they SIGTERM-fight
        # for the same bot token (see PR #11909). Flagging here means
        # every `sarthi update` surfaces the issue until the user migrates.
        try:
            from sarthi_cli.gateway import (
                has_legacy_sarthi_units,
                _find_legacy_sarthi_units,
                supports_systemd_services,
            )

            if supports_systemd_services() and has_legacy_sarthi_units():
                print()
                print("⚠ Legacy Sarthi gateway unit(s) detected:")
                for name, path, is_sys in _find_legacy_sarthi_units():
                    scope = "system" if is_sys else "user"
                    print(f"    {path}  ({scope} scope)")
                print()
                print("  These pre-rename units (sarthi.service) fight the current")
                print("  sarthi-gateway.service for the bot token and cause SIGTERM")
                print("  flap loops. Remove them with:")
                print()
                print("    sarthi gateway migrate-legacy")
                print()
        ...
        print("  sarthi model              # Select provider and model")
...
def _coalesce_session_name_args(argv: list) -> list:
    ...
    When a user types ``sarthi -c Pokemon Agent Dev`` without quoting the
...
def cmd_profile(args):
    ...
    from sarthi_constants import display_sarthi_home

    action = getattr(args, "profile_action", None)

    if action is None:
        # Bare `sarthi profile` — show current profile status
        profile_name = get_active_profile_name()
        dhh = display_sarthi_home()
        ...
        for p in profiles:
            if p.name == profile_name or (profile_name == "default" and p.is_default):
                ...
                if p.alias_path:
                    ...
                    print(f"Alias:          {alias_display} → sarthi -p {p.name}")
                break
        print()
        return
    ...
    if action == "list":
        ...
        try:
            ...
            if name == "default":
                print("Switched to: default (~/.sarthi)")
            else:
        ...
        try:
            ...
            if not no_alias:
                ...
                if collision:
                    ...
                    print(
                        f"  Choose a custom alias:  sarthi profile alias {name} --name <custom>"
                    )
                    print(f"  Or access via flag:     sarthi -p {name} chat")
                else:
        ...
        from sarthi_cli import profiles as _profiles_mod
        ...
        if name and not text_value and not auto_flag:
            try:
                if _profiles_mod.normalize_profile_name(name) == "default":
                    from sarthi_constants import get_sarthi_home as _hh
        ...
        from sarthi_cli import profile_describer as _pd
        ...
        if dist_name:
            ...
            print(f"  (run `sarthi profile info {name}` for full manifest)")
        if alias_name:
            is_windows = sys.platform == "win32"
            wrapper = _get_wrapper_dir() / (f"{alias_name}.bat" if is_windows else alias_name)
            print(f"Alias:   {alias_name} → sarthi -p {name}  ({wrapper})")
        print()
        ...
        from sarthi_cli.profiles import profile_exists, validate_alias_name
        ...
        from sarthi_cli.profiles import rename_profile
        ...
        from sarthi_cli.profiles import export_profile
        ...
        from sarthi_cli.profiles import import_profile
        ...
        import tempfile
        from sarthi_cli.profile_distribution import (
            plan_install,
        ...
        try:
            ...
            with tempfile.TemporaryDirectory(prefix="sarthi_dist_preview_") as tmp:
                plan = plan_install(
                    args.source,
                    Path(tmp),
            ...
            if plan.has_cron:
                print(
                    ...
                    f"  Review them with:  sarthi -p {plan.manifest.name} cron list"
                )
            print(f"\n  Use with:      sarthi -p {plan.manifest.name} chat")
        ...
        from sarthi_cli.profiles import get_profile_dir, normalize_profile_name
        ...
        try:
            ...
            if current is None:
                print(
                    ...
                    "Only profiles installed via `sarthi profile install` can be updated."
                )
                sys.exit(1)
            ...
            if plan.has_cron:
                print(
                    ...
                    f"sarthi -p {plan.manifest.name} cron list"
                )
        except (DistributionError, ValueError) as e:
            print(f"Error: {e}")
            sys.exit(1)
        ...
        from sarthi_cli.profile_distribution import describe_distribution, DistributionError
        ...
        if data.get("sarthi_requires"):
            print(f"Requires:     Sarthi {data['sarthi_requires']}")
        if data.get("source"):
...
def _render_distribution_plan(plan) -> None:
    ...
    from sarthi_cli.profile_distribution import MANIFEST_FILENAME
    mf = plan.manifest
    ...
    if mf.sarthi_requires:
        print(f"  Requires: Sarthi {mf.sarthi_requires}")
...
def _report_dashboard_status() -> int:
    """Print ``sarthi dashboard`` PIDs and return the count.

    Uses the same detection logic as ``_find_stale_dashboard_pids`` (the
    current process is excluded, but since ``sarthi dashboard --status``
    runs in a short-lived CLI process that never matches the pattern,
    the exclusion is irrelevant here).
    """
    pids = _find_stale_dashboard_pids()
    if not pids:
        print("No sarthi dashboard processes running.")
        return 0

    print(f"{len(pids)} sarthi dashboard process(es) running:")
    for pid in pids:
...
def _maybe_setup_dashboard_auth_interactively(args) -> None:
    ...
    ``sarthi dashboard register`` for OAuth.
    ...
    try:
        from sarthi_cli.web_server import should_require_auth
    ...
    try:
        from sarthi_cli.dashboard_auth import list_providers
        if list_providers():
    ...
    print("    [2] OAuth via Nous Portal (run `sarthi dashboard register`)")
    print("    [3] Cancel")
    print()
    ...
    if choice == "2":
        ...
        print(
            ...
            "the dashboard again:\n"
            "    sarthi dashboard register\n"
            "  It provisions a Nous Portal OAuth client and writes "
            "SARTHI_DASHBOARD_OAUTH_CLIENT_ID into ~/.sarthi/.env for you.\n"
            "  Docs: https://sarthi-agent.vercel.app/docs/"
    ...
    try:
        from sarthi_cli.plugins import discover_plugins
...
def cmd_dashboard(args):
    ...
    if getattr(args, "stop", False):
        ...
        if not pids:
            print("No sarthi dashboard processes running.")
            sys.exit(0)
        # Reuse the same SIGTERM-grace-SIGKILL path used after `sarthi update`.
    ...
    # launches the dashboard (`worker dashboard` → SARTHI_HOME points into
    ...
    try:
        from sarthi_cli.profiles import get_active_profile_name
    ...
    if (
        ...
        and os.environ.get("SARTHI_DESKTOP") != "1"
    ):
        ...
        reexec_argv = [
            sys.executable, "-m", "sarthi_cli.main",
            "-p", "default",
        ...
        # SARTHI_HOME.  We must resolve the root explicitly instead of just
        # dropping SARTHI_HOME: in the Docker layout the machine root is
        # /opt/data (set via `ENV SARTHI_HOME=/opt/data`), so an unset
        # SARTHI_HOME falls back to $HOME/.sarthi = /opt/data/.sarthi — an
        ...
        # update-button guard also misfires).  get_default_sarthi_root()
        # returns the root for both layouts: ~/.sarthi for a standard install
        ...
        try:
            from sarthi_constants import get_default_sarthi_root
            env["SARTHI_HOME"] = str(get_default_sarthi_root())
        except Exception:
            # Best-effort: if root resolution fails, fall back to the prior
            # behaviour (drop SARTHI_HOME) rather than block the reroute.
    ...
    # the same logs directory as every other Sarthi surface.
    try:
        from sarthi_logging import setup_logging as _setup_logging_gui
        _setup_logging_gui(mode="gui")
    except Exception:
    ...
    if _headless_backend:
        ...
        os.environ["SARTHI_SERVE_HEADLESS"] = "1"
    elif "SARTHI_WEB_DIST" not in os.environ and not getattr(args, "skip_build", False):
        ...
        _dist_root = (
            Path(os.environ["SARTHI_WEB_DIST"])
            if "SARTHI_WEB_DIST" in os.environ
            else PROJECT_ROOT / "sarthi_cli" / "web_dist"
        )
        ...
        # SARTHI_WEB_DIST is set without --skip-build: the build is skipped
        ...
        _dist_root = Path(os.environ["SARTHI_WEB_DIST"]).expanduser()
        if not (_dist_root / "index.html").exists():
            print(f"✗ SARTHI_WEB_DIST is set but no web dist found at: {_dist_root}")
            print("  Pre-build first:  npm install --workspace web && npm run build -w web")
            print("  Or unset SARTHI_WEB_DIST to build and use the default web UI dist.")
            sys.exit(1)
        # Write the expanded path back: web_server reads SARTHI_WEB_DIST raw
        # at import (no expanduser), so a validated "~/dist" would otherwise
        # pass here and still 404 there.
        os.environ["SARTHI_WEB_DIST"] = str(_dist_root)
        print(f"→ Using web dist from SARTHI_WEB_DIST: {_dist_root}")
    ...
    try:
        from sarthi_cli.mcp_startup import start_background_mcp_discovery
    ...
    from sarthi_cli.web_server import start_server
...
def cmd_dashboard_register(args):
    ...
    from sarthi_cli.dashboard_register import cmd_dashboard_register as _impl
...
def cmd_gateway_enroll(args):
    ...
    from sarthi_cli.gateway_enroll import cmd_gateway_enroll as _impl
...
def cmd_completion(args, parser=None):
    ...
    from sarthi_cli.completion import generate_bash, generate_zsh, generate_fish
...
def cmd_prompt_size(args):
    ...
    from sarthi_cli.prompt_size import cmd_prompt_size as _impl

    _impl(args)


def cmd_logs(args):
    """View and filter Sarthi log files."""
    from sarthi_cli.logs import tail_log, list_logs
...
def cmd_console(args):
    """Open the safe Sarthi command console."""
    from sarthi_cli.console_engine import run_console_repl
...
def _build_provider_choices() -> list[str]:
    ...
    try:
        from sarthi_cli.models import CANONICAL_PROVIDERS as _cp
...
# so that in ``sarthi -m gpt5 chat``, ``gpt5`` is correctly skipped as a
# flag value rather than misclassified as a subcommand. Kept in sync with
# the top-level flags declared in ``sarthi_cli/_parser.py``.
#
...
def _first_positional_argv() -> str | None:
    ...
    ``sarthi -m gpt5 --provider openai chat "msg"`` by skipping the
...
def _plugin_cli_discovery_needed() -> bool:
    ...
    if first is None:
        # Bare ``sarthi`` or only flags → defaults to ``chat``.
        return False
...
def _is_tui_chat_launch(args) -> bool:
    return bool(getattr(args, "tui", False) or os.environ.get("SARTHI_TUI") == "1")
...
def _prepare_agent_startup(args) -> None:
    ...
    # --yolo: chokepoint guarantee that SARTHI_YOLO_MODE is set before ANY
...
def _try_termux_fast_cli_launch() -> bool:
    ...
    from sarthi_cli._parser import build_top_level_parser
    ...
    if getattr(args, "oneshot", None):
        ...
        from sarthi_cli.oneshot import run_oneshot
    ...
    if args.command in {None, "chat"}:
        ...
        if interactive_prompt:
            ...
            os.environ["SARTHI_DEFER_AGENT_STARTUP"] = "1"
            os.environ["SARTHI_FAST_STARTUP_BANNER"] = "1"
            if getattr(args, "accept_hooks", False):
                os.environ["SARTHI_ACCEPT_HOOKS"] = "1"
        else:
            _prepare_agent_startup(args)
        cmd_chat(args)
        return True
...
def _try_termux_fast_tui_launch() -> bool:
    ...
    `sarthi --tui` is the hot path on phones. The full parser setup imports
...
def cmd_memory(args):
    ...
    if sub == "off":
        ...
        mem_dir = get_sarthi_home() / "memories"
        ...
        if not existing:
            print(
                f"\n  Nothing to reset — no memory files found in {display_sarthi_home()}/memories/\n"
            )
            return
        ...
        print(f"  Files were in: {display_sarthi_home()}/memories/\n")
    else:
        from sarthi_cli.memory_setup import memory_command

        memory_command(args)


def cmd_acp(args):
    """Launch Sarthi Agent as an ACP server."""
    try:
...
def cmd_tools(args):
    ...
    if action in {"list", "disable", "enable"}:
        from sarthi_cli.tools_config import tools_disable_enable_command

        tools_disable_enable_command(args)
    elif action == "post-setup":
        from sarthi_cli.tools_config import run_post_setup_command

        sys.exit(run_post_setup_command(args))
    else:
        _require_tty("tools")
        from sarthi_cli.tools_config import tools_command
...
def cmd_skills(args):
    ...
    if getattr(args, "skills_action", None) == "config":
        ...
        from sarthi_cli.skills_config import skills_command as skills_config_command

        skills_config_command(args)
    else:
        from sarthi_cli.skills_hub import skills_command

        skills_command(args)


def cmd_pairing(args):
    from sarthi_cli.pairing import pairing_command

    pairing_command(args)


def cmd_plugins(args):
    from sarthi_cli.plugins_cmd import plugins_command

    plugins_command(args)


def cmd_mcp(args):
    from sarthi_cli.mcp_config import mcp_command

    mcp_command(args)


def cmd_claw(args):
    from sarthi_cli.claw import claw_command

    claw_command(args)


def main():
    """Main entry point for sarthi CLI."""
    # Cosmetic: make the process show up as 'sarthi' instead of 'python3.11'
    # in ps/top/htop.  Non-fatal — just a nicer UX.
    _set_process_title()

    # Force UTF-8 stdio on Windows before anything prints.  No-op elsewhere.
    try:
        from sarthi_cli.stdio import configure_windows_stdio
        configure_windows_stdio()
    except Exception:
        pass

    # Sweep stale ``sarthi.exe.old.*`` quarantine files left by previous
    # ``sarthi update`` runs on Windows. Silent no-op on non-Windows or when
    # there's nothing to clean. See ``_quarantine_running_sarthi_exe``.
    try:
        _cleanup_quarantined_exes()
    except Exception:
        pass

    # Self-heal a venv left half-built by an interrupted ``sarthi update``
    ...
    # ``sarthi skills install update``) merely defers recovery one launch;
    # under-matching (missing ``sarthi -p work update``) would race a recovery
    ...
    # =========================================================================
    # model command  (parser built in sarthi_cli/subcommands/model.py)
    # =========================================================================
    build_model_parser(subparsers, cmd_model=cmd_model)

    from sarthi_cli.moa_cmd import cmd_moa
    ...
    # =========================================================================
    from sarthi_cli.fallback_cmd import cmd_fallback
    ...
    fallback_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "https://sarthi-agent.vercel.app/docs/user-guide/features/fallback-providers"
        ),
    )
    ...
    fallback_subparsers.add_parser(
        "add",
        help="Pick a provider + model (same picker as `sarthi model`) and append to the chain",
    )
    fallback_subparsers.add_parser(
        "remove",
        aliases=["rm"],
    ...
    secrets_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "instead of storing them in ~/.sarthi/.env.  Supports Bitwarden "
            "Secrets Manager and 1Password.  See: "
            "https://sarthi-agent.vercel.app/docs/user-guide/secrets/"
        ),
    )
    ...
    from sarthi_cli import secrets_cli as _secrets_cli
    from sarthi_cli import onepassword_secrets_cli as _op_secrets_cli
    ...
    # migrate command
    # =========================================================================
    from sarthi_cli.migrate import cmd_migrate, cmd_migrate_xai
    ...
    # =========================================================================
    # gateway + proxy commands  (parsers built in sarthi_cli/subcommands/gateway.py)
    # =========================================================================
    ...
    # =========================================================================
    # setup command  (parser built in sarthi_cli/subcommands/setup.py)
    # =========================================================================
    build_setup_parser(subparsers, cmd_setup=cmd_setup)

    # =========================================================================
    # postinstall command  (parser built in sarthi_cli/subcommands/postinstall.py)
    # =========================================================================
    build_postinstall_parser(subparsers, cmd_postinstall=cmd_postinstall)

    # =========================================================================
    # whatsapp command  (parser built in sarthi_cli/subcommands/whatsapp.py)
    # =========================================================================
    ...
    whatsapp_cloud_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "required). Distinct from `sarthi whatsapp` which sets up "
    ...
    # =========================================================================
    # slack command  (parser built in sarthi_cli/subcommands/slack.py)
    # =========================================================================
    ...
    # =========================================================================
    from sarthi_cli.send_cmd import register_send_subparser
    register_send_subparser(subparsers)

    # =========================================================================
    # login command  (parser built in sarthi_cli/subcommands/login.py)
    # =========================================================================
    build_login_parser(subparsers, cmd_login=cmd_login)

    # =========================================================================
    # logout command  (parser built in sarthi_cli/subcommands/logout.py)
    # =========================================================================
    build_logout_parser(subparsers, cmd_logout=cmd_logout)

    # =========================================================================
    # auth command  (parser built in sarthi_cli/subcommands/auth.py)
    # =========================================================================
    build_auth_parser(subparsers, cmd_auth=cmd_auth)

    # =========================================================================
    # status command  (parser built in sarthi_cli/subcommands/status.py)
    # =========================================================================
    build_status_parser(subparsers, cmd_status=cmd_status)

    # =========================================================================
    # cron command  (parser built in sarthi_cli/subcommands/cron.py)
    # =========================================================================
    build_cron_parser(subparsers, cmd_cron=cmd_cron)

    # =========================================================================
    # webhook command  (parser built in sarthi_cli/subcommands/webhook.py)
    # =========================================================================
    ...
    # =========================================================================
    from sarthi_cli.portal_cli import add_parser as _add_portal_parser
    _add_portal_parser(subparsers)

    # =========================================================================
    # kanban command — multi-profile collaboration board
    # =========================================================================
    from sarthi_cli.kanban import build_parser as _build_kanban_parser
    ...
    # =========================================================================
    from sarthi_cli.projects_cmd import build_parser as _build_project_parser
    ...
    # =========================================================================
    # hooks command  (parser built in sarthi_cli/subcommands/hooks.py)
    # =========================================================================
    build_hooks_parser(subparsers, cmd_hooks=cmd_hooks)

    # =========================================================================
    # doctor command  (parser built in sarthi_cli/subcommands/doctor.py)
    # =========================================================================
    ...
    # =========================================================================
    # security command  (parser built in sarthi_cli/subcommands/security.py)
    # =========================================================================
    build_security_parser(subparsers, cmd_security=cmd_security)

    # =========================================================================
    # dump command  (parser built in sarthi_cli/subcommands/dump.py)
    # =========================================================================
    build_dump_parser(subparsers, cmd_dump=cmd_dump)

    # =========================================================================
    # debug command  (parser built in sarthi_cli/subcommands/debug.py)
    # =========================================================================
    build_debug_parser(subparsers, cmd_debug=cmd_debug)

    # =========================================================================
    # backup command  (parser built in sarthi_cli/subcommands/backup.py)
    # =========================================================================
    ...
    checkpoints_parser = subparsers.add_parser(
        "checkpoints",
        help="Inspect / prune / clear ~/.sarthi/checkpoints/",
        description="Manage the filesystem checkpoint store — the shadow git "
        "repo sarthi uses to snapshot working directories before "
        "write_file/patch/terminal calls. Lets you see how much "
        "space checkpoints occupy, force a prune, or wipe the base.",
    )
    from sarthi_cli.checkpoints import register_cli as _register_checkpoints_cli
    _register_checkpoints_cli(checkpoints_parser)

    # =========================================================================
    # import command  (parser built in sarthi_cli/subcommands/import_cmd.py)
    # =========================================================================
    build_import_cmd_parser(subparsers, cmd_import=cmd_import)

    # =========================================================================
    # config command  (parser built in sarthi_cli/subcommands/config.py)
    # =========================================================================
    build_config_parser(subparsers, cmd_config=cmd_config)

    # =========================================================================
    # console command  (parser built in sarthi_cli/subcommands/console.py)
    # =========================================================================
    build_console_parser(subparsers, cmd_console=cmd_console)

    # =========================================================================
    # pairing command  (parser built in sarthi_cli/subcommands/pairing.py)
    # =========================================================================
    build_pairing_parser(subparsers, cmd_pairing=cmd_pairing)

    # =========================================================================
    # skills command  (parser built in sarthi_cli/subcommands/skills.py)
    # =========================================================================
    ...
    from sarthi_cli.bundles import register_cli as _bundles_register, bundles_command
    _bundles_register(bundles_parser)
    bundles_parser.set_defaults(func=bundles_command)

    # =========================================================================
    # plugins command  (parser built in sarthi_cli/subcommands/plugins.py)
    # =========================================================================
    ...
    # subcommand — ``sarthi --help``, ``sarthi version``, ``sarthi logs``,
    ...
    if _plugin_cli_discovery_needed():
        try:
            ...
            from sarthi_cli.plugins import discover_plugins, get_plugin_manager
    ...
    try:
        from sarthi_cli.curator import register_cli as _register_curator_cli
    ...
    pets_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "and Sarthi shows it reacting to agent activity across the CLI, "
            "TUI, and desktop app."
        ),
    )
    try:
        from sarthi_cli.pets import register_cli as _register_pets_cli
    ...
    try:
        from sarthi_cli.journey import register_cli as _register_journey_cli
    ...
    # =========================================================================
    # memory command  (parser built in sarthi_cli/subcommands/memory.py)
    # =========================================================================
    build_memory_parser(subparsers, cmd_memory=cmd_memory)

    # =========================================================================
    # tools command  (parser built in sarthi_cli/subcommands/tools.py)
    # =========================================================================
    ...
    computer_use_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "Linux.\n\n"
            "Use `sarthi computer-use install` to fetch and run the\n"
            "upstream cua-driver installer. This is equivalent to the\n"
            "post-setup hook that `sarthi tools` runs when you first\n"
            "enable the Computer Use toolset, and is a stable target\n"
            "for re-running the install if it didn't fire (e.g. when\n"
            "toggling the toolset on a returning-user setup).\n\n"
            "Use `sarthi computer-use doctor` to run cua-driver's\n"
    ...
    computer_use_perms = computer_use_sub.add_parser(
        ...
        description=(
            ...
            "terminal or the Sarthi app. `status` reports the driver's grant\n"
    ...
    def cmd_computer_use(args):
        ...
        if action == "status":
            import shutil
            import subprocess
            from sarthi_cli.tools_config import _cua_driver_cmd
            # Honor SARTHI_CUA_DRIVER_CMD for local-build testing — same
            ...
            if path:
                version = ""
                try:
                    from sarthi_cli.tools_config import _cua_driver_env
                ...
                try:
                    ...
                    if st and st.get("update_available"):
                        ...
                        print("    Run: sarthi computer-use install --upgrade")
                    elif st:
                        print("  ✓ Up to date.")
                    else:
                        # Older driver (no check-update verb) or offline.
                        print("  Refresh to latest: sarthi computer-use install --upgrade")
                except Exception:
                    print("  Refresh to latest: sarthi computer-use install --upgrade")
                return
            print("cua-driver: not installed")
            print("  Run: sarthi computer-use install")
            return
        if action == "doctor":
        ...
        if action == "permissions":
            ...
            if perms_action == "status":
                ...
                if not st["platform_supported"]:
                    ...
                    sys.exit(1)
                if not st["installed"]:
                    print("cua-driver: not installed. Run: sarthi computer-use install")
                    sys.exit(1)
                ...
                if st["can_grant"]:  # macOS TCC permissions
                    ...
                    if not st["ready"]:
                        print("  Grant: sarthi computer-use permissions grant")
    ...
    # =========================================================================
    # mcp command  (parser built in sarthi_cli/subcommands/mcp.py)
    # =========================================================================
    ...
    sessions_export.add_argument(
        ...
        help=(
            ...
            "md/qmd: output directory (default: <sarthi home>/session-exports)"
        ),
    )
    sessions_export.add_argument(
        "--format",
    ...
    def cmd_sessions(args):
        ...
        if action == "list":
            ...
            from sarthi_cli.session_filters import (
                build_prune_filters,
                describe_filters,
            )
            ...
            def _redact(data):
                if not args.redact or data is None:
                    return data
                from sarthi_cli.session_export_md import redact_session_data
            ...
            if args.format == "html":
                if not args.output or args.output == "-":
                    ...
                    return
                from sarthi_cli.session_export_html import (
                ...
                try:
                    if len(ids) == 1:
                        ...
                        out_dir = (
                            ...
                            else get_sarthi_home() / "session-exports"
                        )
            ...
            # Markdown / QMD export
            from sarthi_cli.session_export_md import (
            ...
            if args.output == "-":
                ...
                db.close()
                return
            output_dir = Path(args.output).expanduser() if args.output else get_sarthi_home() / "session-exports"
            ...
            if args.session_id:
                ...
                if args.delete_after_verified:
                    ...
                    if not ok:
                        ...
                        db.close()
                        return
                    sessions_dir = get_sarthi_home() / "sessions"
            ...
            # `sarthi sessions prune`: no time window and no filters at all
            ...
            # Launch sarthi --resume <id> by replacing the current process
    ...
    # =========================================================================
    # insights command  (parser built in sarthi_cli/subcommands/insights.py)
    # =========================================================================
    build_insights_parser(subparsers, cmd_insights=cmd_insights)

    # =========================================================================
    # claw command  (parser built in sarthi_cli/subcommands/claw.py)
    # =========================================================================
    build_claw_parser(subparsers, cmd_claw=cmd_claw)

    # =========================================================================
    # version command  (parser built in sarthi_cli/subcommands/version.py)
    # =========================================================================
    build_version_parser(subparsers, cmd_version=cmd_version)

    # =========================================================================
    # update command  (parser built in sarthi_cli/subcommands/update.py)
    # =========================================================================
    build_update_parser(subparsers, cmd_update=cmd_update)

    # =========================================================================
    # uninstall command  (parser built in sarthi_cli/subcommands/uninstall.py)
    # =========================================================================
    build_uninstall_parser(subparsers, cmd_uninstall=cmd_uninstall)

    # =========================================================================
    # acp command  (parser built in sarthi_cli/subcommands/acp.py)
    # =========================================================================
    build_acp_parser(subparsers, cmd_acp=cmd_acp)

    # =========================================================================
    # profile command  (parser built in sarthi_cli/subcommands/profile.py)
    # =========================================================================
    ...
    # =========================================================================
    # dashboard command  (parser built in sarthi_cli/subcommands/dashboard.py)
    # =========================================================================
    ...
    # for one release. The Sarthi-Setup.exe success screen tells users to
    # run `sarthi desktop` from a terminal, so the canonical name needs
    # to be the one that appears in --help (argparse promotes the primary
    # name; aliases stay hidden).
    # =========================================================================
    # gui command  (parser built in sarthi_cli/subcommands/gui.py)
    # =========================================================================
    build_gui_parser(subparsers, cmd_gui=cmd_gui)

    # =========================================================================
    # logs command  (parser built in sarthi_cli/subcommands/logs.py)
    # =========================================================================
    build_logs_parser(subparsers, cmd_logs=cmd_logs)

    # =========================================================================
    # prompt-size command  (parser built in sarthi_cli/subcommands/prompt_size.py)
    # =========================================================================
    ...
    # e.g. ``sarthi -c Pokemon Agent Dev`` → ``sarthi -c 'Pokemon Agent Dev'``
    ...
    from sarthi_cli.config import get_container_exec_info
    ...
    # fails (e.g. 'sarthi -c model' where 'model' is consumed as the
    ...
    # --yolo: set SARTHI_YOLO_MODE *before* plugin discovery.  The call to
    ...
    # so introspection/management commands (sarthi hooks list, cron
