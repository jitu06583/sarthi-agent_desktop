...
Usage:
    ...
    sarthi --version           Show version and update status
...
from sarthi_cli._subprocess_compat import suppress_platform_ver_console
...
# Two lines of inline path math so ``python sarthi_cli/main.py`` (script
# mode — sys.path[0] is sarthi_cli/, not the repo root) can import the
# canonical helpers; everything else lives in sarthi_cli._startup_fast.
_bootstrap_root = os.path.realpath(os.path.join(os.path.dirname(__file__), os.pardir))
if _bootstrap_root not in sys.path:
    sys.path.insert(0, _bootstrap_root)
from sarthi_cli import _startup_fast  # noqa: E402

# Early venv self-heal — MUST run before any third-party import below.  When
# a prior ``sarthi update`` left a recovery marker and a core package's import
# files were wiped (#57828 — failed lazy backend refresh), the module-level
# ``from sarthi_cli.env_loader import ...`` / ``from sarthi_cli.config import
...
# package directory, so if IT can't import, nothing else in sarthi_cli can
# either. It is also the canonical home of the probe/repair tables reused by
# the full recovery path below.
from sarthi_cli import _early_recovery as _early_recovery_mod
...
def _active_profile_may_override_home_fast(sarthi_root: str) -> bool:
    return _startup_fast.active_profile_may_override_home(sarthi_root)
...
def _try_ultrafast_version() -> bool:
    """Handle ``sarthi --version`` before config/logging imports."""
...
from sarthi_cli.subcommands.sync import build_sync_parser
...
from sarthi_cli.subcommands.pause import build_pause_parser
...
from sarthi_cli.subcommands.verify import build_verify_parser
from sarthi_cli.subcommands.security import build_security_parser
from sarthi_cli.subcommands.approvals import build_approvals_parser
...
from sarthi_cli.subcommands.import_agent import build_import_agent_parser
from sarthi_cli.subcommands.config import build_config_parser
from sarthi_cli.subcommands.skin import build_skin_parser
...
from sarthi_cli.subcommands.monitoring import build_monitoring_parser
...
try:
    ...
    # yaml.load — the SAME parse then serves sarthi_logging's
    # _read_logging_config and any later raw reads in this process, collapsing
    # 3-4 config.yaml parses per invocation into one.
    from sarthi_cli.config import read_raw_config as _read_raw_early
...
def _relative_time(ts) -> str:
    ...
    in :mod:`sarthi_cli.timefmt` so lightweight consumers don't have to
    import the whole CLI surface.
    """
    from sarthi_cli.timefmt import relative_time
...
def _has_any_provider_configured() -> bool:
    ...
    if isinstance(model_cfg, dict):
        ...
        if isinstance(_default, dict):
            from sarthi_cli.config import split_model_config_default
...
def _confirm_startup_expensive_model_override(args) -> None:
    ...
    try:
        ...
        from sarthi_cli.model_selection_guards import (
            combined_message,
            selection_warnings,
        )
    except Exception as exc:
...
def _resolve_last_session(source: str = "cli") -> Optional[str]:
    ...
    ``sarthi -c`` from repo A continues repo A's last session rather than the
    ...
    if continue_val and not getattr(args, "resume", None):
        if isinstance(continue_val, str):
            ...
            if resolved:
                ...
                print(
                    "Use 'sarthi sessions list' to see available sessions, or "
            ...
            try:
                from sarthi_cli.terminal_breadcrumbs import resolve_breadcrumb_session
...
def _ensure_tui_node() -> None:
    ...
    sarthi_home = str(get_sarthi_home())
    try:
...
def _make_tui_argv(tui_dir: Path, tui_dev: bool) -> tuple[list[str], Path]:
    ...
    def _node_bin(bin: str) -> str:
        if bin == "node":
            ...
            if env_node and os.path.isfile(env_node) and os.access(env_node, os.X_OK):
                return env_node
        # find_node_executable() prefers the managed $SARTHI_HOME/node tree,
        # which is not on PATH — a bare which() would declare "node not found"
        # and exit on an install whose only Node is the one Sarthi installed,
    ...
    # sarthi_cli/tui_dist/entry.js but never ships ui-tui/ at all (that
    ...
    if (
        ...
        def _run_tui_install() -> subprocess.CompletedProcess:
            ...
            return subprocess.run(
                ...
                encoding="utf-8",
                errors="replace",
                env=_npm_lifecycle_env(with_sarthi_node_path()),
            )

        result = _run_tui_install()
        if result.returncode != 0:
            # An npm outside the root package.json's `engines.npm` range fails
            # here before doing any work; repair once (upgrade a Sarthi-managed
            ...
            from sarthi_cli.npm_engine import maybe_repair_npm_engine
...
def _apply_tui_python_env(env: dict) -> None:
    ...
    src_root = str(env.get("SARTHI_PYTHON_SRC_ROOT") or "").strip()
    if not src_root or not Path(src_root).is_dir():
        env["SARTHI_PYTHON_SRC_ROOT"] = str(PROJECT_ROOT)

    cwd = str(env.get("SARTHI_CWD") or "").strip()
    if not cwd or not Path(cwd).is_dir():
        env["SARTHI_CWD"] = _safe_tui_cwd(env)

    python = str(env.get("SARTHI_PYTHON") or "").strip()
    if os.path.dirname(python):
        python_path = Path(python)
        if not python_path.is_absolute():
            python_path = Path(env["SARTHI_CWD"]) / python_path
    ...
    if not python_is_executable:
        env["SARTHI_PYTHON"] = sys.executable
...
def _launch_tui(
    ...
    # TUI child is a sarthi process: propagate the profile-home contract via
...
def cmd_chat(args):
    ...
    if isinstance(_resume_foreign, str) and _resume_foreign.strip().lower() in (
        ...
        print(f"  (later: sarthi --resume {_imported_id})")
    ...
    if _termux_should_prefetch_update_check():
        try:
            from sarthi_cli.banner import prefetch_banner_data, prefetch_update_check
    ...
    #
    # FIRST RUN is the exception: with an empty ~/.sarthi/skills the banner
    ...
    def _skills_dir_is_unseeded() -> bool:
        try:
            from sarthi_cli.config import get_sarthi_home
            skills_dir = Path(get_sarthi_home()) / "skills"
    ...
    if _skills_dir_is_unseeded():
        ...
        try:
            import sarthi_cli.banner as _banner_mod
...
def select_provider_and_model(args=None):
    ...
    from sarthi_cli.providers import (
    ...
    # Honor ``model_catalog.excluded_providers`` so the CLI ``sarthi model``
    ...
    # matching against sarthi_id / alias / canonical slug.
    _cli_excluded = {
        str(p).strip().lower()
...
def _aux_select_for_task(task: str) -> None:
    ...
    up new ones through the normal ``sarthi model`` flow, then route aux tasks
    to them here.
    """
    from sarthi_cli.config import load_config
    from sarthi_cli.inventory import build_aux_picker_rows, format_aux_picker_entries
...
# surface so argparse wiring and test monkeypatches on sarthi_cli.main.<name>
# keep resolving unchanged. Importing those modules eagerly costs ~50ms on
# every `sarthi` invocation, including fast paths like `sarthi --version`
...
_LAZY_COMMAND_EXPORTS = {
    ...
    "sarthi_cli.dashboard_procs": (
        "_detect_concurrent_sarthi_instances",
...
def _self():
    ...
    keeps monkeypatches on sarthi_cli.main.<name> working, exactly like a
...
def cmd_sync(args):
    ...
    if sub in {"enable", "disable"}:
        ...
        if not is_curation_eligible(skill):
            print(
                ...
                f"skills under ~/.sarthi/skills/ can sync.",
                file=sys.stderr,
            )
            return 1
    ...
    if sub == "status":
        ...
        if status.get("org_available"):
            ...
            if modified:
                print(
                    ...
                    f"{', '.join(modified)}\n"
                    f"  Share them back with `sarthi sync propose <skill>`. "
        ...
        if not status.get("logged_in"):
            ...
            print(
                "\nSync feature is off for this instance (set SARTHI_SYNC_ENABLED=1 "
            ...
            print(
                ...
                "SARTHI_SYNC_BASE_URL). Sync is inert.",
                file=sys.stderr,
            )
        return 0
    ...
    try:
        if sub == "pull":
            ...
            result = ssc.push_skills(identity=identity, message="sarthi sync push")
        elif sub == "now":
            pull_res = ssc.pull_skills(identity=identity)
            push_res = ssc.push_skills(identity=identity, message="sarthi sync now")
...
def cmd_verify(args):
    ...
    from sarthi_cli.verify_cmd import run_verify_command
...
def cmd_approvals(args):
    """Dispatch `sarthi approvals <subcmd>`."""
    from sarthi_cli.approvals_suggest import approvals_command
...
def cmd_skin(args):
    ...
    from sarthi_cli.skin_cmd import skin_command
...
def _print_version_info(*, check_updates: bool = True) -> None:
    ...
    # `sarthi --version` pre-import fast path (the `version` subcommand
...
# Update pipeline lives in sarthi_cli/update_cmd.py (main.py decomposition,
...
# monkeypatches on sarthi_cli.main.<name> keep resolving unchanged without
...
# validated against. Lives next to the checkout (NOT in SARTHI_HOME) because
...
def _sweep_stale_bytecode_if_checkout_changed() -> None:
    ...
    ``sarthi update``, a manual ``git pull``, a ZIP update, a file-sync
    ...
    Update-time clears alone can never close this class: ``sarthi update``
    ...
    ``sarthi`` entry point compares the checkout fingerprint (cheap file
...
def _web_ui_build_needed(web_dir: Path) -> bool:
    ...
    mtime comparison. ``git checkout`` / ``git pull`` / ``sarthi update``
    ...
    ``sarthi_cli/web_dist/`` (per vite.config.ts outDir), NOT ``web/dist/``,
...
def _web_ui_stamp_path() -> Path:
    """Return the path to the web UI build stamp file under $SARTHI_HOME."""
    from sarthi_constants import get_sarthi_home
    return get_sarthi_home() / "web-ui-build-stamp.json"
...
def _run_npm_install_deterministic(
    ...
    *,
    ...
    # of a Sarthi-managed install, or a freshly provisioned managed npm when
    ...
    run_env["PATH"] = with_sarthi_node_path(run_env)["PATH"]
...
def _build_web_ui(web_dir: Path, *, fatal: bool = False) -> bool:
    ...
    project_root = web_dir.parent.parent if web_dir.parent.name == "apps" else web_dir.parent
    dist_index = project_root / "sarthi_cli" / "web_dist" / "index.html"
    try:
...
def _do_build_web_ui(web_dir: Path, *, fatal: bool = False) -> bool:
    ...
    if not npm:
        ...
        return not fatal
    build_env = _npm_lifecycle_env(with_sarthi_node_path())
    ...
    # as `sarthi update`'s _update_node_dependencies() (ui-tui + web +
...
def _renderer_bundle_dir(desktop_dir: Path, *, source_mode: bool) -> Optional[Path]:
    ...
    # macOS: …/Sarthi.app/Contents/MacOS/Sarthi → …/Contents/Resources
    resources = (
...
def _renderer_bundle_torn(dist_dir: Path) -> bool:
    ...
    the intact SOURCE tree, ``sarthi desktop`` skips the rebuild that would fix
...
def _desktop_packaged_executable(desktop_dir: Path) -> Optional[Path]:
    ...
    if sys.platform == "win32" and len(existing) > 1:
        ...
        # Picking purely by mtime can then hand a wrong-architecture Sarthi.exe
...
#
# The desktop self-update chain (Desktop → sarthi-setup --update →
# `sarthi update` → `sarthi desktop --build-only` → relaunch) rebuilds
# Sarthi.exe on the end user's machine and used to verify only that the file
...
# wrong-architecture Sarthi.exe is caught BEFORE the updater replaces the
...
def _windows_native_machine_from_iswow64() -> Optional[str]:
    ...
    process arch) and rejected a correctly-built ARM64 ``Sarthi.exe``.
...
def _windows_native_machine() -> str:
    ...
    emulation: the desktop update chain runs an x64 sarthi-setup.exe (and thus
...
def _ensure_desktop_exe_launchable(
    ...
    print(f"✗ The built Sarthi.exe failed its integrity check: {error}")
    ...
    if restored is not None:
        print("  ↩ Update aborted — restored the previous working Sarthi.exe from backup.")
        print("    Your existing version was kept and still works. Run `sarthi desktop`")
        print("    (or the in-app update) again to retry with a fresh Electron download.")
        return restored, True

    print("  ✗ No usable backup was found to restore.")
    print("    Run `sarthi desktop --force-build` to rebuild, or re-run the Sarthi")
...
def _desktop_macos_local_codesign(
    ...
    #    contains the outer Sarthi.app component, so an absolute-parts check
...
def _desktop_macos_relaunchable_fixup(
    ...
    *,
    ...
    "Sarthi is damaged and can't be opened" — and macOS TCC forgets every
...
def _detect_linux_password_store() -> str | None:
    ...
    fails under `sarthi desktop` because the launcher environment doesn't look
...
def _desktop_launch_options() -> tuple[list[str], str, str]:
    ...
    is one of "auto"/"1"/"0" (normalized for the SARTHI_DESKTOP_DISABLE_GPU
...
def _register_linux_desktop_entry() -> None:
    """Install the XDG desktop entry for Sarthi Desktop (Linux only, best-effort).
    ...
    shell. ``sarthi uninstall --gui`` removes it.
    """
    try:
        from sarthi_cli.linux_desktop_entry import install_desktop_entry, is_supported
...
def cmd_gui(args: argparse.Namespace):
    ...
    # `SARTHI_DESKTOP_PASSWORD_STORE=... sarthi desktop` keeps working.
    if sys.platform == "linux" and "SARTHI_DESKTOP_PASSWORD_STORE" not in os.environ:
        password_store = (
        ...
        if password_store:
            env["SARTHI_DESKTOP_PASSWORD_STORE"] = password_store
    ...
    if skip_build:
        ...
        if not build_needed:
            ...
            # Put the Sarthi-managed Node on PATH so npm's child scripts (which
            # shell out to bare `node`, e.g. electron-winstaller's
            # select-7z-arch.js) resolve it even when the parent PATH is
            # stripped — the desktop updater chain (Desktop → sarthi-setup →
            # sarthi update) loses shell PATH customizations. Wrapping the
            # NixOS build env keeps its PYTHON hint while restoring managed Node
            # ahead of a bare PATH (same idiom as the `sarthi update` path).
            ...
            if not source_mode:
                ...
                # success on a Sarthi.exe Windows cannot load (truncated PE from
    ...
    # Linux: register the app in the desktop launcher, so Sarthi shows up
...
# Dashboard process-hygiene helpers live in sarthi_cli/dashboard_procs.py
...
# sarthi_cli.main.<name> keep resolving unchanged.
...
_DASHBOARD_SYSTEMD_UNIT = "sarthi-dashboard.service"
...
def _restart_managed_dashboard_service(
    ...
    # Probe the user manager first: Sarthi installs Linux services in the
...
def _get_systemd_service_for_pid(pid: int) -> str | None:
    ...
    ``sarthi-serve.service``).  Returns ``None`` when the PID is not
    ...
    try:
        ...
        for line in text.splitlines():
            line = line.strip()
            # Format: 0::/system.slice/sarthi-serve.service
...
def _dashboard_cmdline_for_pid(pid: int) -> list[str] | None:
    ...
    is reconstructed, but sarthi launch commands don't embed exotic args).
...
def _respawn_dashboard_processes(commands: list[list[str]]) -> list[list[str]]:
    """Best-effort respawn of manually-started dashboards after ``sarthi update``.
    ...
    log_path = get_sarthi_home() / "logs" / "dashboard-restart.log"
    try:
...
# Install-scoped breadcrumbs live next to the venv (not under $SARTHI_HOME)
...
def _recover_from_interrupted_install() -> None:
    """Finish update work left half-done by a prior ``sarthi update``.
...
def _recover_core_update_marker_locked() -> None:
    ...
    # Windows: a normal ``sarthi.exe`` launch always has the launcher as an
    ...
    self_locked = _windows_running_sarthi_launcher_locked()
    if self_locked:
        install_prefix, install_env = _default_venv_install_target()
        print(
            "  → Running from sarthi.exe; applying package-only first aid, "
    ...
    try:
        from sarthi_cli import _install_repair as _ir
        ...
        if self_locked:
            print(
                "  Sarthi is still running from the launcher that needs "
                "replacing. Close other Sarthi windows, restart from a "
...
def _windows_shim_in_process_chain() -> Path | None:
    ...
    ``venv\\Scripts\\sarthi.exe`` is a launcher that runs the interpreter with
    ...
    ``sarthi ...`` command holds its own shim, and an editable install run
    ...
    ``sarthi.exe`` belonging to some other install never matches.
    """
    if not _is_windows():
        return None
    ...
    if scripts_dir is None:
        return None
    shims = {_norm_exe_path(shim): shim for shim in _sarthi_exe_shims(scripts_dir)}
    if not shims:
        return None
...
def _windows_running_sarthi_launcher_locked() -> bool:
    """True when a venv ``sarthi*.exe`` shim is this process or an ancestor.
...
_UPDATE_REEXEC_ENV = "SARTHI_UPDATE_REEXEC"
...
def _reexec_dependency_sync_off_windows_shim() -> bool:
    ...
    The child re-runs ``sarthi update``, so the whole remaining flow — the
    ...
    from sarthi_constants import venv_python_path

    python_exe = venv_python_path(shim.parent.parent, windows=True)
    cmd = [str(python_exe), "-m", "sarthi_cli.main", *sys.argv[1:]]
...
def _default_venv_install_target() -> tuple[list[str], dict[str, str] | None]:
    ...
    try:
        ...
        uv_bin = None
    if uv_bin:
        from sarthi_constants import project_venv_dir
...
def _venv_scripts_dir() -> Path | None:
    ...
    from sarthi_constants import project_venv_dir, venv_bin_dir
...
def _quarantine_running_sarthi_exe(
    ...
    2. If all retries fail, print a clear warning naming the most likely
       culprit (running Sarthi Desktop / gateway / REPL).

    The updater's own launcher is no longer one of those culprits: an update
    started from ``sarthi.exe`` re-runs itself under the venv Python before
...
def _cleanup_pending_shim_renames(scripts_dir: Path) -> int:
    """Drop reboot renames older Sarthi versions queued for our shims.

    Sarthi used to fall back to ``MoveFileExW(MOVEFILE_DELAY_UNTIL_REBOOT)``
    ...
    try:
        ...
        with winreg.OpenKey(
            ...
            kept, removed = _filter_pending_shim_renames(
                entries, _sarthi_exe_shims(scripts_dir)
            )
            if not removed:
                return 0
            if kept:
...
class ShimQuarantineError(RuntimeError):
    """A live ``sarthi*.exe`` shim could not be renamed aside (#87331).
...
def _run_quarantined_install(
    ...
    *,
    ...
    if scripts_dir is not None:
        moved = _quarantine_running_sarthi_exe(scripts_dir, failed_out=failed)
    ...
    try:
        ...
        # quarantined aside and `sarthi` missing from PATH after a green
...
def _cleanup_quarantined_exes(scripts_dir: Path | None = None) -> None:
    """Sweep ``sarthi.exe.old.*`` and stale reboot renames left by prior updates.
...
def _run_package_only_install(
    ...
    *,
    ...
    rewrite ``sarthi.exe``. The editable-install quarantine path would rename
...
def _repair_venv_via_import_probes(
    ...
    *,
    ...
    detected (#57828). Package-only reinstall — never rewrites ``sarthi.exe``.
    ...
    print("  ⚠ Venv repair incomplete. Run manually, then `sarthi update`:")
    print(
...
def cmd_update(args):
    ...
    if getattr(args, "plan", False):
        ...
        # Sarthi runtime across profiles, its supervisor, and its running
        # code version — without mutating anything. Safe on a live fleet.
        from sarthi_cli.update_inventory import (
    ...
    from sarthi_cli.update_lock import (
    ...
    try:
        ...
        try:
            from sarthi_cli.update_receipt import finalize_pending_update_receipt
...
def cmd_profile(args):
    ...
    if action is None:
        ...
        from sarthi_cli.profiles import format_profile_label
    ...
    if action == "list":
        ...
        from sarthi_cli.profiles import normalize_profile_name, rename_profile
...
def _report_dashboard_status() -> int:
    ...
    print(f"{len(live)} sarthi dashboard process(es) running:")
...
def _maybe_setup_dashboard_auth_interactively(args) -> None:
    ...
    try:
        from sarthi_cli.config import load_config, save_config
        from sarthi_cli.plugins_cmd import ensure_basic_auth_plugin_enabled_in_config
...
def _read_ssh_session_token_file(path: str) -> str:
    ...
    if sys.platform == "win32":
        from sarthi_cli.windows_ssh_runtime import read_token
    ...
    # The Desktop client writes the token under $HOME/.sarthi/desktop-ssh: a
    # literal "~/.sarthi/desktop-ssh" in apps/desktop/electron/remote-lifecycle.ts
    # expanded against the account's $HOME, independent of SARTHI_HOME and the
    # active profile. Anchor validation to that same OS-home path, NOT to
    # get_sarthi_home(): a non-default sticky profile (or any SARTHI_HOME pointing
    # elsewhere, e.g. a Docker /opt/data root) re-homes get_sarthi_home() and
    # would otherwise reject every token the client legitimately wrote (#69551).
    token_root = _Path.home() / ".sarthi" / "desktop-ssh"
    try:
...
def _is_electron_packaged_web_dist(path: str) -> bool:
    ...
    Packaged Desktop sets ``SARTHI_WEB_DIST`` to ``.../app.asar/dist`` or
    ``.../app.asar.unpacked/dist``. A standalone ``sarthi dashboard`` that
...
def cmd_dashboard(args):
    ...
    if _token_file and not _headless_backend:
        raise SystemExit("--ssh-session-token-file is only valid with sarthi serve")

    # ── Sanitize Desktop-inherited env that hijacks a standalone launch ─
    # Desktop Electron spawns its backend with SARTHI_DESKTOP=1 plus
    # SARTHI_WEB_DIST=<packaged app.asar[/unpacked]/dist> (and often
    # SARTHI_SERVE_HEADLESS=1 on the serve path). A shell that inherits
    # those vars then runs `sarthi dashboard` would otherwise:
    #   - serve the desktop renderer → "Desktop IPC bridge is unavailable"
    #     (issue #52945), or
    #   - disable the SPA via inherited SARTHI_SERVE_HEADLESS.
    # Only strip Electron-packaged WEB_DIST contamination — caller-managed
    # SARTHI_WEB_DIST overrides (dev / custom builds) must still work.
    # The desktop-spawned backend itself (SARTHI_DESKTOP=1) keeps its dist.
    # Intentionally headless `serve` re-sets SARTHI_SERVE_HEADLESS below.
    if os.environ.get("SARTHI_DESKTOP") != "1":
        _inherited_web_dist = os.environ.get("SARTHI_WEB_DIST", "")
        if _is_electron_packaged_web_dist(_inherited_web_dist):
            os.environ.pop("SARTHI_WEB_DIST", None)
    if not _headless_backend:
        os.environ.pop("SARTHI_SERVE_HEADLESS", None)
    ...
    if (
        ...
        from tools.environments.local import build_subprocess_env
        # Exact env preservation: SARTHI_HOME is explicitly pinned to the
    ...
    from sarthi_cli.resource_limits import apply_nofile_soft_limit
    ...
    if _headless_backend:
        ...
        if not (_dist_root / "index.html").exists():
            ...
            # default dist location is recoverable: a custom SARTHI_WEB_DIST
            # points at a caller-managed directory the build cannot populate.
            _recoverable = "SARTHI_WEB_DIST" not in os.environ
            if _recoverable:
...
def _resolve_deferred_platform_cli_command(command_name: str | None) -> None:
    ...
    registers a top-level ``sarthi <name>`` command (e.g. Photon ->
    ...
    import it, so the CLI registration never happens and ``sarthi photon``
...
def _prepare_agent_startup(args) -> None:
    ...
    if not _is_tui_chat_launch(args):
        ...
        try:
            from sarthi_cli.plugins import start_background_plugin_discovery
...
def _try_fast_chat_launch() -> bool:
    ...
    ``sarthi`` / ``sarthi -w -s foo --yolo`` / ``sarthi chat`` don't need the
    ...
    """
    if os.environ.get("SARTHI_DISABLE_FAST_CHAT_LAUNCH") == "1":
        return False
    argv = sys.argv[1:]
...
def cmd_monitoring(args):
    ...
    if action == "status":
        ...
        print(f"  OTel SDK:       {'installed' if otlp_exporter.is_available() else 'not installed'} "
              f"(optional extra: sarthi-agent[otlp])")
...
def _cmd_skills_trust(args):
    """``sarthi skills trust [path]`` / ``sarthi skills untrust [path]``.
...
def _advertise_agent_env() -> None:
    ...
    (``sarthi-agent`` in huggingface.js ``agent-harnesses.ts``): standard-var
    matching is exact, so any other value is counted as "unknown".
    ``SARTHI_AGENT`` is the Sarthi-specific marker. setdefault: never
    clobber an outer harness (e.g. Sarthi running inside another agent's
    terminal).
    """
    os.environ.setdefault("AI_AGENT", "sarthi-agent")
    os.environ.setdefault("SARTHI_AGENT", "true")
...
def main():
    ...
    # If the checkout changed since the last launch (sarthi update, manual
    ...
    worktree_parser = subparsers.add_parser(
        ...
        description=(
            "Attended reclaim for the .worktrees/ directory sarthi -w sessions "
            "accumulate. Never deletes uncommitted tracked changes, unique "
            "unpushed commits, or in-use trees; untracked-only scratch is "
            "archived to ~/.sarthi/archive/worktree-prune/ before removal. See: "
    ...
    def _dispatch_worktree(_args):
        from sarthi_cli.worktree_cmd import cmd_worktree
    ...
    # `sarthi proxy` (defined elsewhere in this file) is a separate INBOUND
    ...
    from sarthi_cli import proxy_cli as _proxy_cli
    ...
    def _dispatch_egress(args):  # noqa: ANN001
        ...
        # from the inbound OAuth ``sarthi proxy`` subparser (dest='proxy_command').
    ...
    # =========================================================================
    # pause / resume commands  (parser built in sarthi_cli/subcommands/pause.py)
    # =========================================================================
    ...
    # =========================================================================
    # peer command — bot-to-bot DMs across machines (peer Sarthi gateways)
    # =========================================================================
    from sarthi_cli.subcommands.peer import build_peer_parser
    ...
    # =========================================================================
    # verify command  (parser built in sarthi_cli/subcommands/verify.py)
    # =========================================================================
    ...
    # =========================================================================
    # approvals command  (parser built in sarthi_cli/subcommands/approvals.py)
    # =========================================================================
    ...
    # =========================================================================
    # import-agent command  (parser: sarthi_cli/subcommands/import_agent.py)
    # =========================================================================
    def cmd_import_agent(args):
        from sarthi_cli.agent_import import import_agent_command
    ...
    # =========================================================================
    # skin command  (parser built in sarthi_cli/subcommands/skin.py)
    # =========================================================================
    ...
    # subcommand — ``sarthi --help``, ``sarthi logs``,
    ...
    def cmd_computer_use(args):
        ...
        if action == "install":
            from sarthi_cli.tools_config import (
        ...
        if action == "status":
            import os as _os
            import subprocess
            from sarthi_cli.tools_config import _cua_driver_contract_status
            ...
            override = _os.environ.get("SARTHI_CUA_DRIVER_CMD", "").strip()
            if path:
                version = ""
                try:
                ...
                if not contract.get("ready"):
                    ...
                    if override:
                        print(
                            "    Update the binary selected by SARTHI_CUA_DRIVER_CMD, or unset "
                            "the override and run: sarthi computer-use install --upgrade"
                        )
                    else:
    ...
    sessions_import = sessions_subparsers.add_parser(
        "import",
        help="Import a Claude Code or Codex CLI session into Sarthi",
        description=(
            "Pull a conversation started in Claude Code (~/.claude/projects) "
            "or Codex CLI (~/.codex/sessions) into the Sarthi session store "
            "so it can be resumed with 'sarthi --resume <id>'. The foreign "
    ...
    # cmd_sessions lives in sarthi_cli/sessions_cmd.py (main.py decomposition).
    ...
    # and lets monkeypatches on sarthi_cli.main.cmd_sessions keep working.
    ...
    # NOTE: the `sarthi version` subcommand was removed — `sarthi --version`
    ...
    # ``sarthi egress start`` refusing when credential_source=bitwarden
