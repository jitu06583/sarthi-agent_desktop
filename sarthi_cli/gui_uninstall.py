"""
Sarthi Desktop (Chat GUI) uninstaller.
...
the Python agent or the user's config/data:
  ...
  1. Source-built GUI (``sarthi desktop`` / ``sarthi gui``)
     Built inside the agent checkout under ``$SARTHI_HOME/sarthi-agent/``:
       ...
       - ``$SARTHI_HOME/desktop-build-stamp.json`` (the build freshness stamp)
  ...
  2. Packaged distributable (DMG / NSIS / AppImage / deb / rpm)
     ...
     own bundled Electron + a per-user Electron ``userData`` directory:
       - macOS:   ``/Applications/Sarthi.app`` or ``~/Applications/Sarthi.app``
       - Windows: ``%LOCALAPPDATA%\\Programs\\Sarthi`` (NSIS per-user)
...
the app name ("Sarthi"), separate from ``$SARTHI_HOME``:
  - macOS:   ``~/Library/Application Support/Sarthi``
  - Windows: ``%APPDATA%\\Sarthi``
  - Linux:   ``$XDG_CONFIG_HOME/Sarthi`` (default ``~/.config/Sarthi``)
...
``sarthi uninstall --gui`` (and friends) without paying for the full CLI.
"""
...
def _agent_root(sarthi_home: Path) -> Path:
    """The agent checkout root — same layout install.sh / install.ps1 use."""
    return sarthi_home / "sarthi-agent"


def desktop_userdata_dir() -> Path:
    """Return the Electron ``userData`` directory for the desktop app.

    Mirrors Electron's ``app.getPath('userData')`` for an app named "Sarthi"
    ...
    if sys.platform == "darwin":
        return home / "Library" / "Application Support" / "Sarthi"
...
def source_built_gui_artifacts(sarthi_home: Path) -> "list[Path]":
    """GUI build artifacts produced by ``sarthi desktop`` inside the checkout.

    These are removable on a GUI uninstall without harming the agent: the
    Python agent runs from ``sarthi-agent/`` source + ``venv/`` and never
    needs the Electron build output or node_modules.
    """
    agent_root = _agent_root(sarthi_home)
    ...
    return [
        ...
        sarthi_home / "desktop-build-stamp.json",
    ]
...
def packaged_gui_app_paths() -> "list[Path]":
    ...
    electron-builder output locations for the "Sarthi" product.
    """
    home = Path.home()
    paths: list[Path] = []
    if sys.platform == "darwin":
        paths += [
            Path("/Applications/Sarthi.app"),
            home / "Applications" / "Sarthi.app",
        ]
    elif sys.platform == "win32":
        local = os.environ.get("LOCALAPPDATA")
        local_base = Path(local) if local else (home / "AppData" / "Local")
        paths += [
            # NSIS per-user install (perMachine=false → Programs\Sarthi).
            local_base / "Programs" / "Sarthi",
            # Older / alternate layout some builds used.
            local_base / "sarthi-desktop",
        ]
        program_files = os.environ.get("ProgramFiles")
        if program_files:
            # NSIS per-machine fallback (needs admin to remove).
            paths.append(Path(program_files) / "Sarthi")
    else:
        ...
        # ``uninstall_gui``.
        from sarthi_cli.linux_desktop_entry import desktop_entry_path

        data = os.environ.get("XDG_DATA_HOME")
        data_base = Path(data) if data else (home / ".local" / "share")
        paths += [
            # The launcher entry `sarthi desktop` installs. Its icon lives
            # in the checkout, not in the installed app.
            desktop_entry_path(),
            # Some packaged builds emit this casing.
            data_base / "applications" / "Sarthi.desktop",
        ]
    return paths


def agent_is_installed(sarthi_home: Path) -> bool:
    """Return True when a usable Python agent install exists under SARTHI_HOME.
    ...
    if (agent_root / "sarthi_cli").is_dir():
        return True
...
def gui_is_installed(sarthi_home: Path) -> bool:
    """Return True when any desktop GUI artifact exists (built or packaged)."""
    for p in source_built_gui_artifacts(sarthi_home):
        if p.exists():
            return True
...
def gui_install_summary(sarthi_home: "Path | None" = None) -> dict:
    ...
    """
    home: Path = sarthi_home if sarthi_home is not None else get_sarthi_home()
...
def uninstall_gui(sarthi_home: "Path | None" = None, *, remove_userdata: bool = True) -> "list[Path]":
    ...
    Never touches ``sarthi-agent/sarthi_cli`` (agent source), ``venv/``, or any
    config / sessions / .env under ``$SARTHI_HOME``.
    ...
    if sys.platform.startswith("linux"):
        ...
        # Reindex so Sarthi disappears from the launcher.
        try:
            from sarthi_cli.linux_desktop_entry import (
                desktop_entry_path,
        ...
        log_info(
            ...
            "with your package manager (e.g. 'sudo apt remove sarthi' or "
            "'sudo dnf remove sarthi'). AppImage builds are a single file you "
