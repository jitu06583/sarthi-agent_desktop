...
- ``sarthi_cli._early_recovery.recover_if_needed`` — stdlib-only, runs BEFORE
  ``sarthi_cli.main``'s third-party imports, so it can complete a pending
  update while no native extension is mapped yet (#83569).
- ``sarthi_cli.main._recover_core_update_marker_locked`` — the historical
...
the corrupted-venv state it exists to repair. ``sarthi_cli.main`` imports
``managed_uv``, ``sarthi_constants``, and friends only in its late path; the
...
:func:`sarthi_cli._early_recovery._find_uv_binary` lookup and falls back to
...
from sarthi_cli import _early_recovery as _er
...
def _is_termux_env(env: dict | None = None) -> bool:
    """Stdlib Termux probe (sarthi_cli.main's version lives behind imports)."""
...
def _stdout_to_stderr():
    ...
    ``sarthi acp`` speaks JSON-RPC on stdout; an inherited-fd install child
...
def _venv_scripts_dir(root: Path) -> Path | None:
    ...
    # sarthi_constants is stdlib-only, so the canonical layout helpers are safe
...
def _sync_windows_cli_launchers(root: Path) -> list[Path]:
    """Copy the venv's Sarthi launchers into the dedicated PATH directory.
    ...
    ``sarthi.exe`` is required; ``sarthi-acp.exe`` is copied when available.
    Existing files are left alone because ``bin\\sarthi.exe`` may be the
    ...
    required_source = scripts_dir / "sarthi.exe"
    if not required_source.is_file():
        raise FileNotFoundError(
            f"required Sarthi launcher not found: {required_source}"
        )
    ...
    copied: list[Path] = []
    for name in ("sarthi.exe", "sarthi-acp.exe"):
    ...
    required_destination = bin_dir / "sarthi.exe"
    if not required_destination.is_file():
        raise FileNotFoundError(
            f"Sarthi launcher was not installed: {required_destination}"
        )
    return copied
...
def _quarantine_running_sarthi_exe(
    ...
    """Rename live sarthi*.exe shims aside so the installer can rewrite them.
...
def _run_install_cmd(cmd: list[str], *, env: dict | None, root: Path) -> None:
    ...
    failed: list[str] = []
    moved = (
        _quarantine_running_sarthi_exe(scripts_dir, failed_out=failed)
        if scripts_dir
        else []
    )
    if failed:
    ...
    try:
        ...
        # the quarantined shims renamed aside and `sarthi` gone from PATH
...
def run_core_install(root: Path) -> None:
    ...
    - quarantine live ``sarthi*.exe`` shims on Windows so they can be replaced
