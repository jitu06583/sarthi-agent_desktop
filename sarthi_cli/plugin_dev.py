"""Runtime-backed validation behind ``sarthi plugins doctor``.
...
def _doctor_runtime(plugin_path: Path):
    ...
    test framework. Registration code executes under a temporary SARTHI_HOME
    with outbound socket connects blocked.
    """
    temporary_home = tempfile.TemporaryDirectory(prefix="sarthi-plugin-doctor-")
    stack = ExitStack()
    ...
    stack.enter_context(
        patch.dict(
            os.environ,
            {
                ...
                "SARTHI_BUNDLED_PLUGINS": str(bundled),
                "SARTHI_ENABLE_PROJECT_PLUGINS": "0",
            },
            clear=False,
        )
    )
    ...
    modules_before = {
        ...
        if name == "sarthi_plugins" or name.startswith("sarthi_plugins.")
    }
    ...
    try:
        ...
        if not manifests:
            raise _DoctorLoadError(
                f"Sarthi discovery found no valid plugin manifest under {copied}"
            )
        if len(manifests) != 1:
            raise _DoctorLoadError(
        ...
        for name in list(sys.modules):
            if (
                ...
                and (name == "sarthi_plugins" or name.startswith("sarthi_plugins."))
            ):
...
def resolve_plugin_path(target: str | os.PathLike[str] | None = None) -> Path:
    ...
    user_root = get_sarthi_home() / "plugins"
    ...
    try:
        ...
        pass
    candidates.append(Path.cwd() / ".sarthi" / "plugins" / raw)
...
def _check_manifest_v2(report: "DoctorReport", manifest: Any) -> None:
    ...
    from sarthi_cli.plugins import SUPPORTED_MANIFEST_VERSION

    mv = getattr(manifest, "manifest_version", 1)
    if mv > SUPPORTED_MANIFEST_VERSION:
        report.warning(
            f"manifest_version {mv} is newer than this Sarthi supports "
    ...
    if missing:
        report.warning(
            ...
            + ", ".join(missing)
            + " — Sarthi never auto-installs plugin dependencies; "
    ...
    if schema:
        from sarthi_cli.plugins import _CONFIG_SCHEMA_TYPES
...
def doctor_plugin(target: str | os.PathLike[str] | None = None) -> DoctorReport:
    """Validate one plugin through Sarthi' real scanner and registration path."""
    try:
