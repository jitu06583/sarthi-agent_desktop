...
from sarthi_cli.memory_providers import (
...
def _warm_gateway_module() -> None:
    try:
        import sarthi_cli.gateway  # noqa: F401
    except Exception:
        pass
...
async def _lifespan(app: "FastAPI"):
    ...
    # Fire sarthi_cli.gateway import into a background thread so the event
    # loop is not blocked and SARTHI_DASHBOARD_READY fires without delay.
...
# ---------------------------------------------------------------------------
_SESSION_TOKEN = os.environ.get("SARTHI_DASHBOARD_SESSION_TOKEN") or secrets.token_urlsafe(32)
...
class ModelAssignment(BaseModel):
    ...
    # ``sarthi model`` custom flow collects. Honored only on the main slot for
...
def _count_status_active_sessions() -> int:
    ...
    while another Sarthi process is writing to it.
    """
    from sarthi_state import DEFAULT_DB_PATH, SessionDB
...
async def get_status(profile: Optional[str] = None):
    ...
    try:
        ...
        try:
            from sarthi_cli.dashboard_auth import list_providers as _list_providers
...
def _spawn_sarthi_action(subcommand: List[str], name: str) -> subprocess.Popen:
    ...
    popen_kwargs: Dict[str, Any] = {
        ...
        "env": {**os.environ, "SARTHI_NONINTERACTIVE": "1"},
    }
    if sys.platform == "win32":
...
def _spawn_gateway_restart(profile: Optional[str] = None) -> Tuple[subprocess.Popen, bool]:
    ...
    return _spawn_sarthi_action(subcommand, "gateway-restart"), False
...
async def update_sarthi():
    ...
    try:
        proc = _spawn_sarthi_action(["update"], "sarthi-update")
    except Exception as exc:
...
# ---------------------------------------------------------------------------
# Declared surface — curated desktop schema from sarthi_cli.memory_providers.
...
async def set_model_assignment(body: ModelAssignment, profile: Optional[str] = None):
    ...
    try:
        ...
        if model and not body.confirm_expensive_model:
            try:
                from sarthi_cli.model_cost_guard import expensive_model_warning
...
_PLATFORM_OVERRIDES: dict[str, dict[str, Any]] = {
    ...
    "email": {
        ...
        "docs_url": "https://sarthi-agent.vercel.app/docs/user-guide/messaging/",
        "env_vars": (
            "EMAIL_ADDRESS",
            "EMAIL_PASSWORD",
            "EMAIL_IMAP_HOST",
    ...
    "google_chat": {
        ...
        "docs_url": "https://sarthi-agent.vercel.app/docs/user-guide/messaging/google_chat",
    },
    "wecom": {
    ...
    "weixin": {
        ...
        "docs_url": "https://sarthi-agent.vercel.app/docs/user-guide/messaging/weixin/",
    ...
    "teams": {
        "docs_url": "https://sarthi-agent.vercel.app/docs/user-guide/messaging/teams",
    },
    "yuanbao": {
        "name": "Yuanbao (元宝)",
    ...
    "webhook": {
        ...
        "docs_url": "https://sarthi-agent.vercel.app/docs/user-guide/messaging/webhooks/",
...
_TELEGRAM_ONBOARDING_DEFAULT_URL = "https://setup.sarthi-agent.vercel.app"
...
async def get_messaging_platforms(profile: Optional[str] = None):
    ...
    # all resolve against the requested profile's SARTHI_HOME.
...
_OAUTH_PROVIDER_CATALOG: tuple[Dict[str, Any], ...] = (
    ...
    },
    {
        ...
        "docs_url": "https://sarthi-agent.vercel.app/docs/guides/xai-grok-oauth",
...
async def import_sessions_endpoint(request: Request):
    ...
    restores a whole Sarthi backup archive, while this endpoint is scoped to
...
async def get_session_stats(profile: Optional[str] = None):
    """Session-store statistics for the Sessions page (mirrors `sarthi sessions stats`).
...
def _fire_cron_job_for_profile(profile: str, job_id: str) -> bool:
    ...
    Scope both cron storage and the runtime Sarthi home so the job's store,
...
async def list_mcp_servers(profile: Optional[str] = None):
    from sarthi_cli.mcp_config import _get_mcp_servers
...
async def replace_mcp_servers(body: MCPServersReplace, profile: Optional[str] = None):
    ...
    """
    from sarthi_cli.mcp_config import _replace_mcp_servers
...
async def remove_mcp_server(name: str, profile: Optional[str] = None):
    from sarthi_cli.mcp_config import _remove_mcp_server
...
async def test_mcp_server(name: str, profile: Optional[str] = None):
    ...
    def _probe_scoped():
        ...
        # The probe touches no skills globals; it only needs the SARTHI_HOME
...
async def auth_mcp_server(name: str, profile: Optional[str] = None):
    ...
    Mirrors ``sarthi mcp login``: wipe cached OAuth state so the probe forces
    ...
    def _run():
        ...
        # resolve SARTHI_HOME via the contextvar override, which is all they need.
        with _config_profile_scope(profile), force_interactive_oauth():
            storage = SarthiTokenStorage(name)
...
async def install_mcp_catalog_entry(body: MCPCatalogInstall, profile: Optional[str] = None):
    ...
    # The -p subprocess rebinds SARTHI_HOME-derived paths in the child.
...
class ImportRequest(BaseModel):
    archive: str
    # Pass --force to `sarthi import`. The spawned action runs with
...
async def preview_skill_hub(identifier: str = "", profile: Optional[str] = None):
    ...
    def _run():
        from sarthi_cli.skills_hub import _resolve_source_meta_and_bundle
        from tools.skills_hub import create_source_router
...
class ProfileCreate(BaseModel):
    ...
    # via a subprocess scoped to the profile (`sarthi -p <name> skills install`)
    # because skills_hub.SKILLS_DIR is import-time-bound and the SARTHI_HOME
...
async def create_profile_endpoint(body: ProfileCreate):
    ...
    # profile's SARTHI_HOME at import). Returns PIDs for the UI to poll.
...
async def get_active_profile_endpoint():
    ...
    ``active`` is the sticky default written by ``sarthi profile use`` —
    the profile new CLI invocations pick up. ``current`` is the profile
    the running dashboard/gateway is scoped to (derived from SARTHI_HOME).
    """
    from sarthi_cli import profiles as profiles_mod
    try:
...
async def set_active_profile_endpoint(body: ProfileActiveUpdate):
    """Set the sticky active profile (mirrors ``sarthi profile use``).
...
async def update_profile_model_endpoint(name: str, body: ProfileModelUpdate):
    ...
    to the named profile via the SARTHI_HOME override.
    """
    profile_dir = _resolve_profile_dir(name)
    provider = (body.provider or "").strip()
    model = (body.model or "").strip()
...
async def describe_profile_auto_endpoint(name: str, body: ProfileDescribeAuto):
    ...
    (``auxiliary.profile_describer``). Mirrors ``sarthi profile describe
    <name> --auto``.
    ...
    _resolve_profile_dir(name)
    try:
        from sarthi_cli import profile_describer
...
async def get_skills(profile: Optional[str] = None):
    ...
    from sarthi_cli.skills_config import get_disabled_skills
...
async def get_toolsets(profile: Optional[str] = None):
    ...
    from sarthi_cli.platforms import platform_label
...
async def get_toolset_config(name: str, profile: Optional[str] = None):
    ...
    Surfaces the same provider rows the CLI ``sarthi tools`` picker shows
...
async def get_toolset_models(
    ...
    The GUI counterpart of the model picker `sarthi tools` runs after a
...
async def select_toolset_provider(
    ...
    core extracted from the CLI configurator — so the GUI and ``sarthi tools``
...
async def save_toolset_env(name: str, body: ToolsetEnvUpdate, profile: Optional[str] = None):
    ...
    Writes each ``key: value`` to ``~/.sarthi/.env`` via ``save_env_value`` —
    the same store ``sarthi tools`` writes when it prompts for keys. Keys are
...
async def run_toolset_post_setup(
    ...
    ``sarthi tools post-setup <key>`` and the frontend tails the log via
    ...
    ``profile`` spawns the hook as ``sarthi -p <profile> tools post-setup``.
    ...
    write per-profile state must see the same SARTHI_HOME the rest of the
...
async def grant_computer_use_permissions(profile: Optional[str] = None):
    """Spawn ``sarthi computer-use permissions grant`` as a background action.
...
def _resolve_chat_argv(
    ...
    """
    from sarthi_cli.main import PROJECT_ROOT, _make_tui_argv
    ...
    # bronze border #2563EB snaps to palette 173 (#D7875F, salmon-red) and
...
def _dashboard_console_context() -> str:
    ...
    return "hosted" if _default_sarthi_root_is_opt_data() else "local"
...
def _discover_user_themes() -> list:
    ...
    """
    themes_dir = get_sarthi_home() / "dashboard-themes"
    ...
    async def _serve():
        ...
        with server.capture_signals():
            ...
            if headless:
                ...
                print(f"  Sarthi backend listening on {host}:{actual_port}")
            else:
