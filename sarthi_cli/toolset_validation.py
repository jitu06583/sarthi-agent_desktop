...
the tool registry or launching Sarthi (mirrors the decoupled-helper pattern used
elsewhere in the CLI).

Motivated by #38798: a config migration silently rewrote the valid toolset name
``sarthi-cli`` to the non-existent ``sarthi``. ``resolve_toolset('sarthi')``
...
def validate_platform_toolsets(
    ...
    1. A toolset name that ``is_valid_toolset`` rejects — usually a corrupted or
       renamed entry. When ``sarthi-<platform>`` would have been valid (the exact
       #38798 shape, where ``cli`` held ``sarthi`` instead of ``sarthi-cli``),
    ...
    if valid_count == 0:
        warnings.append(
            ...
            "have no tools. Run `sarthi tools` to reconfigure."
        )
    return warnings
