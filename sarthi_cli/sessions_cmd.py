"""``sarthi sessions`` command — extracted from ``sarthi_cli/main.py``.
...
Helpers that stay in ``sarthi_cli.main`` (``get_sarthi_home``,
...
``sarthi_cli.main.<name>`` keep reaching this code path, and so imports stay
...
#: Default age floor for `sarthi sessions prune --never-active`.  Deliberately
...
def _prune_never_active_keyed(db, args):
    """`sarthi sessions prune --never-active` — drop leaked/dead keyed rows.
    ...
    """
    from sarthi_cli.session_filters import format_epoch, parse_duration_seconds
...
def cmd_sessions(args, sessions_parser=None):
    ...
    if action == "repair":
        ...
        if report.get("repaired"):
            ...
            print(f"    sarthi sessions recover --source {source_hint} \\")
            print("        --inspect-only")
    ...
    if action == "recover":
        ...
        from sarthi_cli.session_recovery import (
    ...
    if action == "import":
        from sarthi_cli.foreign_sessions import run_sessions_import
    ...
    if action == "list":
        ...
        if not _include_pinned and callable(_count_matches):
            ...
            if _pinned_skipped:
                ...
                _optin = (
                    ...
                    "first with `sarthi sessions unpin <id>`."
                    if action == "prune"
                    else "Unpin first with `sarthi sessions unpin <id>` to include them."
                )
                print(
        ...
        if skipped_open:
            ...
            print(
                ...
                "sessions. Use `sarthi sessions delete <id>` "
        ...
        if not pinned_rows:
            print(
                "No pinned sessions. Pin one with: sarthi sessions pin <session_id>"
            )
            return
        ...
        if result.get("vacuumed") is False:
            print("  (VACUUM was skipped or failed — run "
                  "`sarthi sessions optimize` later to reclaim freed space.)")
