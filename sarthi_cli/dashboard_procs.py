"""Dashboard process-hygiene helpers — extracted from ``sarthi_cli/main.py``.
...
``_detect_concurrent_sarthi_instances``) are lifted verbatim. References to
helpers that STAY in ``sarthi_cli.main`` (``_find_stale_dashboard_pids``,
``_respawn_dashboard_processes``, ``_is_windows``, ...) are routed through a
lazy ``_m()`` main reference so existing test monkeypatches on
``sarthi_cli.main.<name>`` keep reaching this code path, and imports stay
...
patches on ``sarthi_cli.main`` resolve unchanged.
"""
...
def _m():
    """Lazy ``sarthi_cli.main`` reference (call-time; keeps patches working)."""
...
def _scan_dashboard_processes(
    *,
    ...
    ``sarthi-dashboard.service`` systemd unit.  Managed units are restarted
...
def _sarthi_home_for_pid(pid: int) -> str | None:
    """Best-effort ``SARTHI_HOME`` from *pid*'s environment."""
    try:
        import psutil

        home = psutil.Process(pid).environ().get("SARTHI_HOME")
        if home:
            return home
    except Exception:
        pass
    ...
    for part in raw.split(b"\x00"):
        if part.startswith(b"SARTHI_HOME="):
...
def _is_ephemeral_port_zero_backend(argv: list[str]) -> bool:
    ...
    Ephemeral-port backends are owned by Sarthi Desktop (or become PPID-1
    orphans after a prior update respawn).  Replaying them after
    ``sarthi update`` multiplies listening backends because ``--port 0``
...
def _profile_key_for_respawn(
    argv: list[str], sarthi_home: str | None = None
) -> str:
    """Stable owner key: ``SARTHI_HOME`` when known, else ``--profile`` / ``-p``.

    ``SARTHI_HOME`` ending in ``profiles/<name>`` is normalized to
    ``profile:<name>`` so it shares a cap with an explicit ``--profile``
    flag for the same profile (#78821).  Non-profile homes (including
    distinct ``…/.sarthi`` roots) keep a resolved ``home:`` key so
    ...
    if sarthi_home:
        try:
            home_path = Path(sarthi_home).resolve()
        except (OSError, RuntimeError, ValueError):
            home_path = Path(sarthi_home)
        ...
        try:
            ...
            return f"home:{os.path.normcase(sarthi_home)}"
...
def _filter_dashboard_respawn_candidates(
    ...
    """Select which killed manual backends to respawn after ``sarthi update``.

    Each candidate is ``(pid, argv, sarthi_home)``.

    Rules (#78821):
    1. Never resurrect Desktop ephemeral ``serve|dashboard --port 0``
       backends — Desktop (``SARTHI_DESKTOP_CHILD_PID``) owns their
    ...
    3. Cap at most one managed backend per profile / ``SARTHI_HOME``.

    Intentionally does **not** blanket-skip every PPID-1 process: a prior
    ``sarthi update`` respawn detaches with ``start_new_session=True``, so
    ...
    for _pid, argv, sarthi_home in candidates:
        if not argv:
            continue
        ...
        if norm in seen_cmdlines:
            continue
        profile_key = _profile_key_for_respawn(argv, sarthi_home)
...
def _kill_stale_dashboard_processes(
    ...
    *,
    ...
    """Kill running ``sarthi dashboard`` / ``sarthi serve`` processes.
    ...
    When ``restart_managed`` is true (the ``sarthi update`` path), a detected
    ``sarthi-dashboard.service`` is restarted through systemd; any OTHER
    killed PID that was supervised by a systemd unit (custom unit names —
    e.g. a remote backend's ``sarthi-serve.service``) has its owning unit
    ...
    *already_restarted_units* names units (no ``.service`` suffix) the
    caller already restarted directly — e.g. ``sarthi update``'s systemd
    fleet-restart loop, which restarts ``sarthi-serve*`` units before this
    ...
    # caller asked for restarts (the `sarthi update` path) — `--stop` must
    ...
    if restart_managed and sys.platform != "win32":
        for pid in pids:
            ...
            if not pid_service[pid]:
                ...
                # Snapshot SARTHI_HOME before the kill so per-profile caps
                # still work after the process is gone (#78821).
                cmdline = _m()._dashboard_cmdline_for_pid(pid)
                if cmdline:
                    pid_cmdline[pid] = cmdline
                    pid_home[pid] = _sarthi_home_for_pid(pid)

        if already_restarted_units:
            # Already handled directly by the caller (e.g. sarthi update's
    ...
    #    remote backend (sarthi serve) under Restart=on-failure never comes
...
def _detect_concurrent_sarthi_instances(
    ...
    for shim in _m()._sarthi_exe_shims(scripts_dir):
        try:
...
def _is_desktop_local_serve_cmdline(command: str) -> bool:
    ...
    Desktop primary/pool backends launch as::
        ...
        sarthi serve --host 127.0.0.1 --port 0
        sarthi serve --isolated --host 127.0.0.1 --port 0 ...
    ...
    if "serve" not in cmd:
        return False
    if "sarthi" not in cmd and "sarthi_cli" not in cmd:
        return False
...
def _exclude_pids_from_env() -> set[int]:
    """PIDs Desktop marks as live backends (SARTHI_DESKTOP_CHILD_PID)."""
    raw = os.environ.get("SARTHI_DESKTOP_CHILD_PID", "")
    out: set[int] = set()
...
# on the *remote* host for every ``sarthi serve`` backend it spawns over SSH
# (see apps/desktop/electron/remote-lifecycle.ts). A backend started from
# another client/machine — e.g. a MacBook driving a ``sarthi serve`` on a Mac
...
def _sarthi_home_dir() -> Path:
    """Resolved Sarthi home (SARTHI_HOME override or ~/.sarthi)."""
    override = os.environ.get("SARTHI_HOME", "").strip()
    if override:
...
def _valid_lockfile_payload(parsed: object, ownership_id: str) -> bool:
    ...
    *adds* lock-owned PIDs to its spare-set.
    ...
    for field in ("profile", "sarthiPath", "sarthiHome", "logPath", "startedAt"):
        value = parsed.get(field)
        if not isinstance(value, str) or len(value) > 1024:
            return False
    # logPath is written as ``{lock_root}/{ownershipId}/{spawnNonce}.log``. We
    # only check the suffix so a relocated SARTHI_HOME (different leading path)
...
def _lock_owned_serve_pids(base_dir: Path | None = None) -> set[int]:
    ...
    Scans ``{sarthi_home}/desktop-ssh/<ownershipId>/backend.lock.json`` (the
    ...
    root = base_dir if base_dir is not None else (
        _sarthi_home_dir() / _REMOTE_LOCK_SUBDIR
    )
    owned: set[int] = set()
    if not root.is_dir():
        return owned
...
def _reap_orphaned_desktop_local_serves(
    *,
    reason: str = "orphaned desktop-local sarthi serve",
    signal_term=None,
    signal_kill=None,
    sleep_fn=None,
    lock_owned_pids_fn=None,
) -> dict[str, list]:
    """Kill leftover Desktop-local ``sarthi serve`` backends with no parent.
    ...
    running under SARTHI_PARENT_PID; this helper clears *already* orphaned
    ...
    - never self / never SARTHI_DESKTOP_CHILD_PID entries
