...
1. **Update receipt** — a machine-readable JSON record of what one
   ``sarthi update`` run discovered, did, skipped (and why), written to
   ``<SARTHI_HOME>/logs/update_receipts/``. Silent-failure classes this
...
``sarthi_cli.build_info.get_code_identity()``: an image build reports
...
# Module-level current receipt. ``sarthi update`` is a single-threaded CLI
...
class UpdateReceipt:
    """Collects the observable facts of one ``sarthi update`` run."""
...
def _receipt_dir() -> Path:
    ...
    return get_sarthi_home() / "logs" / _RECEIPT_DIR_NAME
...
def finalize_pending_update_receipt(
    ...
    ``sarthi update`` has many early-termination paths (Windows
...
def print_fleet_version_matrix(fleet: list[dict[str, Any]]) -> bool:
    ...
    if any_stale:
        ...
        print("      sarthi gateway restart                # active profile")
        print("      sarthi -p <profile> gateway restart   # named profile")
    return any_stale
