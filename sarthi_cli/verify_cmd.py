"""``sarthi verify`` — detect a project's run recipe and smoke-test it.
...
``.sarthi/environment.json``), then runs bootstrap/build/test phases and an
...
(:mod:`agent.verification_evidence`), so a passing ``sarthi verify`` satisfies
