"""MCP picker — interactive `sarthi mcp picker` (also the default `sarthi mcp`).

Lists every catalog entry plus any custom MCP servers the user has added via
``sarthi mcp add``, lets them pick one, and routes to install / enable /
disable / uninstall / configure-tools flows.

Mirrors the `sarthi plugin` picker UX: arrow keys to navigate, ENTER on a row
...
from sarthi_cli.cli_output import prompt_yes_no
from sarthi_cli.curses_ui import curses_single_select
from sarthi_cli.mcp_catalog import (
    CatalogEntry,
    CatalogError,
    catalog_diagnostics,
    install_entry,
...
def _enable_disable(name: str, *, enable: bool) -> None:
    ...
    print(color(
        ...
        "Start a new Sarthi session for changes to take effect.",
        Colors.GREEN,
    ))
...
def _configure_tools(name: str) -> None:
    ...
    """
    import argparse
    from sarthi_cli.mcp_config import cmd_mcp_configure
...
def _print_rows_text(rows: List[_Row]) -> None:
    ...
    as the default output of `sarthi mcp catalog`."""
    if not rows:
        print()
    ...
    print()
    print(color(
        "  Install: sarthi mcp install <name>    Picker: sarthi mcp",
        Colors.DIM,
    ))

    # Surface manifest-version warnings so users know when their Sarthi is
    ...
    if future:
        ...
        for name, _, msg in future:
            print(color(
                f"  ⚠ '{name}' requires a newer Sarthi — run `sarthi update` "
...
def show_catalog() -> None:
    """`sarthi mcp catalog` — print the curated list + custom servers, no interaction."""
    _print_rows_text(_build_rows())


def run_picker() -> None:
    """`sarthi mcp picker` (and default `sarthi mcp`) — interactive selector.
...
def install_by_name(identifier: str) -> int:
    """`sarthi mcp install <name>` — non-interactive entry-point.
    ...
    if entry is None:
        print(color(
            ...
            "Run `sarthi mcp catalog` to see available entries.",
            Colors.RED,
        ))
        return 1
    try:
