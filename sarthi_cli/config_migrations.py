...
``sarthi_cli.config.migrate_config``. Each step is a function
...
``sarthi_cli.config`` imports :func:`run_migrations` lazily (inside
``migrate_config``), and every step function here resolves its helpers
(``read_raw_config``, ``_persist_migration``, ``get_env_value``, …) lazily
through the live ``sarthi_cli.config`` module object at call time via
:func:`_cfg`. There is deliberately NO module-level import of
``sarthi_cli.config`` here, so no circular import can form — and, just as
importantly, tests that monkeypatch helpers on ``sarthi_cli.config`` (e.g.
``patch("sarthi_cli.config.read_raw_config", ...)``) keep working, because
...
#: SARTHI_TOOL_PROGRESS*) forever is not worth it. Below-floor configs are
...
def support_floor_message() -> str:
    ...
    return (
        ...
        f"{display_sarthi_home()}/config.yaml and run `sarthi setup` to "
...
def _cfg():
    """Return the live ``sarthi_cli.config`` module (lazy, cycle-free)."""
...
def _migrate_to_21(results: Dict[str, Any], quiet: bool) -> None:
    ...
    read_raw_config = _c.read_raw_config
    _persist_migration = _c._persist_migration
    get_sarthi_home = _c.get_sarthi_home
...
def _migrate_to_38(results: Dict[str, Any], quiet: bool) -> None:
    ...
    from sarthi_cli.relay_plugin_cutover import legacy_relay_plugin_keys
    ...
    message = (
        ...
        "SARTHI_NEMO_RELAY_PLUGINS_TOML."
    )
