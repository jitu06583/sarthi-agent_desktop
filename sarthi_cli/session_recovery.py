"""Offline, non-destructive recovery for a damaged Sarthi session database.
...
def _copy_source_bundle(source: Path, snapshot_dir: Path) -> tuple[Path, list[str]]:
    ...
    locks -- the failure class ``sarthi_cli.sqlite_safe_read`` exists to
    ...
    keeps this path consistent with ``sarthi_state._backup_db_file``.
    """
    from sarthi_cli.sqlite_safe_read import LiveConnectionError, offline_file_access
...
def _snapshot_and_inspect(
    ...
    temp_dir = tempfile.TemporaryDirectory(
        prefix="sarthi-session-recovery-",
        dir=str(work_root),
    )
    ...
    try:
        ...
        if before != after:
            raise SessionRecoverySafetyError(
                ...
                "Stop every Sarthi process using this profile and retry. "
                "This includes the interactive `sarthi` CLI session this "
                ...
                "Sarthi process even after the gateway is stopped. Run the "
                "recovery from a fresh shell with no `sarthi` session open, "
...
def _recover_via_lost_and_found(
    *,
    ...
    from sarthi_cli.session_lost_and_found import (
