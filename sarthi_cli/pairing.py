...
Usage:
    sarthi pairing list              # Show all pending + approved users
    sarthi pairing approve <platform> <request-id|code>  # Approve a pairing request
    sarthi pairing revoke <platform> <user_id> # Revoke user access
    sarthi pairing clear-pending     # Clear all expired/pending codes
"""

def pairing_command(args):
    """Handle sarthi pairing subcommands."""
    ...
    if action == "list":
        ...
        print("Usage: sarthi pairing {list|approve|revoke|clear-pending}")
        print("Run 'sarthi pairing --help' for details.")
...
def _cmd_list(store):
    ...
    if pending:
        ...
        print("\n  Approve with: sarthi pairing approve <platform> <request-id>")
...
def _cmd_approve(store, platform: str, code: str):
    ...
    if result:
        ...
        print(
            ...
            "~/.sarthi/platforms/pairing/_rate_limits.json\n".format(platform)
        )
    else:
        print(f"\n  Pairing request or code '{code}' not found or expired for platform '{platform}'.")
        print("  Run 'sarthi pairing list' to see pending requests.\n")
