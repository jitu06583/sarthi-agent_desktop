...
from sarthi_cli.urllib_security import open_credentialed_url
...
def _xai_curated_models() -> list[str]:
    ...
    Reads $SARTHI_HOME/models_dev_cache.json directly (no network) so this
    runs at import time without blocking. Falls back to ``_XAI_STATIC_FALLBACK``
    when the cache is empty or unreadable. Sarthi refreshes the cache from
...
def provider_model_ids(provider: Optional[str], *, force_refresh: bool = False) -> list[str]:
    ...
    if normalized in ("openai", "openai-api"):
        ...
        if api_key:
            ...
            # agentic catalog so ``/model`` matches what ``sarthi model`` shows.
