"""
sarthi fallback — manage the fallback provider chain.
...
Subcommands:
  sarthi fallback [list]   Show the current fallback chain (default when no subcommand)
  sarthi fallback add      Pick provider + model via the same picker as `sarthi model`,
                           then append the selection to the chain
  sarthi fallback remove   Pick an entry to delete from the chain
  sarthi fallback clear    Remove all fallback entries

Storage: ``fallback_providers`` in ``~/.sarthi/config.yaml`` (top-level, list of
...
def _restore_auth_active_provider(value: Any) -> None:
    ...
    try:
        from sarthi_cli.auth import _auth_store_lock, _load_auth_store, _save_auth_store
        ...
        # `sarthi model` to fix it.  Don't fail the fallback add.
        pass
...
def cmd_fallback_list(args) -> None:  # noqa: ARG001
    ...
    if not chain:
        ...
        print()
        print("  Add one with:  sarthi fallback add")
        print()
        return
    ...
    print("  Docs: https://sarthi-agent.vercel.app/docs/user-guide/features/fallback-providers")
    print()
...
def cmd_fallback_add(args) -> None:
    """Launch the same picker as `sarthi model`, then append the selection to the chain."""
    from sarthi_cli.main import _require_tty, select_provider_and_model
    ...
    print("  `sarthi model` — select the provider + model you want as a fallback.")
    print()
    ...
    print()
    print("  Run `sarthi fallback list` to view, or `sarthi fallback remove` to delete.")
...
def cmd_fallback(args) -> None:
    """Top-level dispatcher for ``sarthi fallback [subcommand]``."""
