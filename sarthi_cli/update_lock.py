"""Cross-process mutual exclusion for in-flight Sarthi updates.

Three different surfaces can start an update of the same install tree:

* ``sarthi update`` from a terminal,
* the dashboard's Update button (``POST /api/sarthi/update`` →
  ``_spawn_sarthi_action(["update"])``, detached),
* the desktop's Update button, which hands off to the Tauri
  ``sarthi-setup --update`` and, on its failure screen, to install-mode
...
once — so a dashboard-spawned ``sarthi update`` and an installer-driven
...
unchanged and remain byte-compatible with the Rust and Electron readers:
    ...
    <SARTHI_HOME>/.sarthi-update-in-progress   body: "<pid>\\n<started_at_unix>"
...
then spawns ``sarthi update`` as a child stage. Without a handoff the child
sees its own parent's live marker and refuses — the GUI update deadlocks
against itself on every attempt ("Sarthi is still running", retry forever).
...
* A live holder that is a *process ancestor* of ours is likewise our own
  ...
  ``sarthi-setup`` binary under ``~/.sarthi`` is only refreshed by a full
  installer run (``copy_self_to_sarthi_home`` deliberately no-ops during
  ...
  ancestry check those users get exit 2 ("Sarthi is still running") on every
  GUI update forever, with no Sarthi process actually running.
"""
...
MARKER_NAME = ".sarthi-update-in-progress"

# Set by an orchestrating updater (the Tauri `sarthi-setup --update` flow) to
# its own pid before spawning `sarthi update` as a child stage. The parent
...
HANDOFF_PID_ENV = "SARTHI_UPDATE_HANDOFF_PID"
...
# to show "Sarthi is still running" instead of a generic failure. Naming it
...
def update_marker_path() -> Path:
    ...
    Uses the *process* Sarthi home (never the context-local profile override):
    the Rust updater resolves ``$SARTHI_HOME`` or the platform default, and the
    ...
    return get_process_sarthi_home() / MARKER_NAME
...
def _is_ancestor_pid(pid: int) -> bool:
    ...
    The orchestrating updater spawns ``sarthi update`` as a (grand)child, so a
    ...
    parent chain. This heals the fleet of staged ``sarthi-setup`` binaries
...
def describe_holder(holder: UpdateHolder) -> str:
    ...
    return (
        f"✗ Another Sarthi update is already running (PID {holder.pid}, "
...
class UpdateLock:
    ...
    def acquire(self) -> bool:
        ...
        updater spawning `sarthi update` as a stage): we run under ITS claim
