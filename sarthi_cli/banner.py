...
Pure display functions with no SarthiCLI state dependency.
"""
import json
import logging
import os
...
_GOLD = "\033[1;38;2;255;215;0m"  # True-color #2DD4BF bold
_BOLD = "\033[1m"
_DIM = "\033[2m"
_RST = "\033[0m"
...
from sarthi_cli import __version__ as VERSION, __release_date__ as RELEASE_DATE

SARTHI_AGENT_LOGO = """[bold #3B82F6]███████╗ █████╗ ██████╗ ████████╗██╗  ██╗██╗      █████╗  ██████╗ ███████╗███╗   ██╗████████╗[/]
[bold #2563EB]██╔════╝██╔══██╗██╔══██╗╚══██╔══╝██║  ██║██║     ██╔══██╗██╔════╝ ██╔════╝████╗  ██║╚══██╔══╝[/]
[#0D9488]███████╗███████║██████╔╝   ██║   ███████║██║ █ ███████║██║  ███╗█████╗  ██╔██╗ ██║   ██║[/]
[#0EA5A5]╚════██║██╔══██║██╔══██╗   ██║   ██╔══██║██║ █ ██╔══██║██║   ██║██╔══╝  ██║╚██╗██║   ██║[/]
[#10B981]███████║██║  ██║██║  ██║   ██║   ██║  ██║██║   ██║  ██║╚██████╔╝███████╗██║ ╚████║   ██║[/]
[#059669]╚══════╝╚═╝  ╚═╝╚═╝  ╚═╝   ╚═╝   ╚═╝  ╚═╝╚═╝   ╚═╝  ╚═╝ ╚═════╝ ╚══════╝╚═╝  ╚═══╝   ╚═╝[/]
[dim]Knows You. Grow With You.  ·  Design & Developed by Jitendra Singh Thakur[/]"""

SARTHI_CADUCEUS = """[#10B981]⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀❁[/]
[#10B981]⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀╱[/]
[#2563EB]⠀⠀⠀⠀⠀·⠀∙⠀•⠀●⠀●⠀●⠀●⠀◉[/]
[#2563EB]⠀⠀⠀·⠀●⠀●⠀●⠀●⠀●⠀●⠀●[/]
[#3B82F6]⠀⠀∙⠀●⠀●⠀●⠀●[/]
...
# (e.g. nix-built sarthi — no local git history to count against).
UPDATE_AVAILABLE_NO_COUNT = -1

_UPSTREAM_REPO_URL = "https://github.com/jitendra-singh-thakur/sarthi-agent.git"
_OFFICIAL_REPO_CANONICAL = "github.com/jitendra-singh-thakur/sarthi-agent"
...
def _fetch_pypi_latest(package: str = "sarthi-agent") -> Optional[str]:
...
def check_for_updates() -> Optional[int]:
    """Check whether a Sarthi update is available.

    Two paths: if ``SARTHI_REVISION`` is set (nix builds embed it), compare
    ...
    cache_file = sarthi_home / ".update_check"
    embedded_rev = os.environ.get("SARTHI_REVISION") or None

    # Docker images have no working tree to count commits against — the
    # published image excludes `.git` (see .dockerignore) and sets no
    # SARTHI_REVISION (that's nix-only). Without this guard the checks below
    ...
    # and `sarthi update` correctly refuses to run in-place inside the
    # container anyway. The dashboard's REST `/api/sarthi/update/check`
    ...
    try:
        from sarthi_cli.config import detect_install_method, get_project_root
    ...
    if embedded_rev:
        ...
        # $SARTHI_HOME/sarthi-agent/ may be a stale copy from --clone-all;
        # Path(__file__) always resolves to the actual installed checkout.
        repo_dir = Path(__file__).parent.parent.resolve()
        if not (repo_dir / ".git").exists():
            repo_dir = sarthi_home / "sarthi-agent"
        if not (repo_dir / ".git").exists():
...
def _resolve_repo_dir() -> Optional[Path]:
    """Return the active Sarthi git checkout, or None if this isn't a git install.

    Prefers the running code's location over the profile-scoped path
    because ``$SARTHI_HOME/sarthi-agent/`` may be a stale copy carried
    over by ``--clone-all``.
    """
    repo_dir = Path(__file__).parent.parent.resolve()
    if not (repo_dir / ".git").exists():
...
def get_git_banner_state(repo_dir: Optional[Path] = None) -> Optional[dict]:
    ...
    ``sarthi_cli/build_info.py``) and return it as a frozen
    ...
    if repo_dir is None:
        ...
        try:
            from sarthi_cli.build_info import get_build_sha
...
_RELEASE_URL_BASE = "https://github.com/jitendra-singh-thakur/sarthi-agent/releases/tag"
...
def get_latest_release_tag(repo_dir: Optional[Path] = None) -> Optional[tuple]:
    ...
    Sarthi checkout. Cached per-process. Release URL always points at the
    canonical NousResearch/sarthi-agent repo (forks don't get a link).
    """
...
def format_banner_version_label() -> str:
    ...
    base = f"Sarthi Agent v{VERSION} ({RELEASE_DATE})"
...
def build_welcome_banner(console: "Console", model: str, cwd: str,
    ...
    accent = _skin_color("banner_accent", "#10B981")
    dim = _skin_color("banner_dim", "#059669")
    ...
    try:
        ...
        _hero = _bskin.banner_hero if hasattr(_bskin, 'banner_hero') and _bskin.banner_hero else SARTHI_CADUCEUS
    except Exception:
        _bskin = None
        _hero = SARTHI_CADUCEUS
    left_lines = ["", _hero, ""]
    ...
    try:
        from sarthi_cli.codex_runtime_switch import get_current_runtime
    ...
    try:
        ...
        if behind is not None and behind != 0:
            from sarthi_cli.config import get_managed_update_command, recommended_update_command
            if behind > 0:
                ...
                # UPDATE_AVAILABLE_NO_COUNT: nix-built sarthi; we know an update
    ...
    title_color = _skin_color("banner_title", "#2DD4BF")
    border_color = _skin_color("banner_border", "#2563EB")
    ...
    if term_width >= 95:
        _logo = _bskin.banner_logo if _bskin and hasattr(_bskin, 'banner_logo') and _bskin.banner_logo else SARTHI_AGENT_LOGO
        console.print(_logo)
        console.print()
