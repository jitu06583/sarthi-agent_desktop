...
def cprint(text: str):
    ...
    try:
        ...
        # absent stdout (pythonw.exe, CI, `sarthi ... > file`) raises
    ...
    try:
        ...
        req = urllib.request.Request(
            ...
            headers={
                ...
                "User-Agent": "sarthi-cli-update-check",
            },
        )
...
def _check_via_local_git(repo_dir: Path) -> Optional[int]:
    ...
    if _is_official_ssh_remote(origin_url):
        ...
        # ahead checkout as behind nudges the user into `sarthi update`,
...
def check_for_updates() -> Optional[int]:
    ...
    # SARTHI_REVISION (that's nix-only). Returning None makes both the Rich
    # banner (build_welcome_banner) and the Ink badge (branding.tsx, guarded
    # on `typeof === 'number' && > 0`) show nothing. The dashboard's REST
    # `/api/sarthi/update/check` endpoint short-circuits docker the same way
...
def _banner_snapshot_path() -> Path:
    return get_sarthi_home() / "cache" / "banner_snapshot.json"
...
def banner_snapshot_fingerprint() -> Optional[str]:
    ...
    try:
        ...
        for p in (get_config_path(), get_sarthi_home() / ".env"):
            try:
                st = p.stat()
...
def build_welcome_banner(console: "Console", model: str, cwd: str,
    ...
    if (provider or "").strip().lower() == "moa":
        ...
        if not (model or "").strip() or (model or "").strip().lower() == "unknown":
            ...
            left_lines.append(
                ...
                f"[dim {dim}]— run /model or sarthi setup[/]"
            )
        else:
    ...
    if not _has_native_mcp:
        try:
            from sarthi_cli.plugins import get_portable_mcp_server_names_nowait
