...
on Japanese locales, etc.).  Sarthi's banners, tool output feed, and slash
...
Entry points (``cli.py`` ``main``, ``sarthi_cli/main.py`` CLI dispatch,
...
def _flip_console_code_page_to_utf8() -> None:
    ...
    is silent — if there's no attached console (e.g. Sarthi is running
...
def configure_windows_stdio() -> bool:
    ...
    Set ``SARTHI_DISABLE_WINDOWS_UTF8=1`` in the environment to opt out
    ...
    if os.environ.get("SARTHI_DISABLE_WINDOWS_UTF8") in {"1", "true", "True", "yes"}:
        _CONFIGURED = True
        return False
    ...
    # Augment PATH with the Sarthi-managed Git install directories so
    ...
    # already-running shells don't see the change — which means sarthi
    ...
    # install without Sarthi-managed PortableGit).
    ...
    # stdin is re-configured for completeness; Sarthi's interactive
...
def _default_windows_editor() -> str:
    ...
    The prompt_toolkit buffer's ``open_in_editor`` and Sarthi's
    ``sarthi config edit`` both honour ``$EDITOR``.  Users who prefer a
    ...
    Set this before launching Sarthi (User env var in Windows Settings, or
    export in a PowerShell profile) and Sarthi picks it up automatically.
    """
    import shutil
...
def _augment_path_with_known_tools() -> None:
    """Prepend well-known Sarthi-managed tool directories to os.environ['PATH'].
    ...
    ``%LOCALAPPDATA%\\sarthi\\git\\bin`` to the User PATH via
    ...
    *spawned* processes only — already-running shells (including the one the
    user invokes ``sarthi`` from right after install) retain their old PATH.

    Any subprocess Sarthi spawns — bash, ``rg``, ``grep``, ``npm`` — inherits
    ...
    Patch-up strategy: add the known Sarthi-managed tool directories to our
    ...
    # see on next launch.
    candidate_dirs = [
        os.path.join(local_appdata, "sarthi", "git", "cmd"),
        os.path.join(local_appdata, "sarthi", "git", "bin"),
        os.path.join(local_appdata, "sarthi", "git", "usr", "bin"),
        # Sarthi venv Scripts directory — host of the sarthi.exe shim itself,
        # also where any pip-installed console scripts land.  Usually already
        # on PATH when the user invokes sarthi, but harmless to include.
        os.path.join(local_appdata, "sarthi", "sarthi-agent", "venv", "Scripts"),
