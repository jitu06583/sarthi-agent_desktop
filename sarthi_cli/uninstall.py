"""
Sarthi Agent Uninstaller.

Provides options for:
- Full uninstall: Remove everything including configs and data
- Keep data: Remove code but keep ~/.sarthi/ (configs, sessions, logs)
"""
...
def remove_path_from_shell_configs():
    """Remove Sarthi PATH entries from shell configuration files."""
    ...
    for config_path in configs:
        try:
            ...
            for line in content.split('\n'):
                # Skip the "# Sarthi Agent" comment and following line
                if '# Sarthi Agent' in line or '# sarthi-agent' in line:
                    skip_next = True
                    continue
                if skip_next and ('sarthi' in line.lower() and 'PATH' in line):
                    skip_next = False
                    continue
                skip_next = False
                
                # Remove any PATH line containing sarthi
                if 'sarthi' in line.lower() and ('PATH=' in line or 'path=' in line.lower()):
                    continue
...
def remove_node_symlinks(sarthi_home: Path) -> list:
    ...
    symlinks node/npm/npx into the same directory as the ``sarthi`` command:
    ...
    the FHS fix).  Only symlinks that resolve into this Sarthi home's ``node``
    directory are removed — links the user has repointed elsewhere (nvm, fnm,
    etc.) are left untouched.
    """
    node_dir = (sarthi_home / "node").resolve()
    removed = []
...
def uninstall_gateway_service():
    ...
    - All platforms: standalone ``sarthi gateway run`` processes
    ...
    try:
        from sarthi_cli.gateway import kill_gateway_processes, find_gateway_pids
    ...
    # 3. macOS: uninstall launchd plist
        ...
        try:
            from sarthi_cli.gateway import get_launchd_plist_path
...
#
#   1. Sets User-scope env vars ``SARTHI_HOME`` and ``SARTHI_GIT_BASH_PATH``
...
#      like ``%LOCALAPPDATA%\sarthi\git\cmd``, ``%LOCALAPPDATA%\sarthi\git\bin``,
#      ``%LOCALAPPDATA%\sarthi\git\usr\bin``, ``%LOCALAPPDATA%\sarthi\node``.
#      Again not in any rc file — only accessible via the registry or the
#      .NET [Environment] API.
#   3. Downloads PortableGit to ``%LOCALAPPDATA%\sarthi\git\`` and Node to
#      ``%LOCALAPPDATA%\sarthi\node\`` as user-scoped, isolated copies.
#      These are ~200MB combined and serve no purpose after uninstall.
#   4. On the ``sarthi dashboard`` + gateway paths, drops files into
#      ``%LOCALAPPDATA%\sarthi\gateway-service\`` and sometimes
...
def _sarthi_path_markers(sarthi_home: Path) -> list[str]:
    """Path-entry substrings that identify Sarthi-owned User-PATH entries."""
    root = str(sarthi_home).rstrip("\\/")
    # Match on prefix so sub-entries (git\cmd, git\bin, git\usr\bin, node, etc.)
    # all get swept.  Also match the bare sarthi-agent install dir.
    markers = [root + "\\sarthi-agent", root + "\\git", root + "\\node", root + "\\venv"]
    # Also match if SARTHI_HOME was customised to somewhere else — find-and-nuke
    # any entry whose path component contains "sarthi".  We don't want to catch
    # unrelated entries like "csarthi-foo" or "ephermeral", so we look for
    # backslash-sarthi as a word-ish boundary.
    return markers


def remove_path_from_windows_registry(sarthi_home: Path) -> list[str]:
    """Strip Sarthi-owned entries from User-scope PATH in the registry.
    ...
    try:
        with winreg.OpenKey(winreg.HKEY_CURRENT_USER, key_path, 0,
            ...
            markers = _sarthi_path_markers(sarthi_home)
            kept: list[str] = []
...
def remove_sarthi_env_vars_windows() -> list[str]:
    """Delete SARTHI_HOME and SARTHI_GIT_BASH_PATH from User-scope env vars."""
    try:
        import winreg
    except ImportError:
        return []
    ...
    try:
        with winreg.OpenKey(winreg.HKEY_CURRENT_USER, "Environment", 0,
                            winreg.KEY_READ | winreg.KEY_WRITE) as key:
            for name in ("SARTHI_HOME", "SARTHI_GIT_BASH_PATH"):
                try:
...
def _is_default_sarthi_home(sarthi_home: Path) -> bool:
    """Return True when ``sarthi_home`` points at the default (non-profile) root."""
    try:
        from sarthi_constants import get_default_sarthi_root
        return sarthi_home.resolve() == get_default_sarthi_root().resolve()
    except Exception:
        return False
...
def _uninstall_profile(profile) -> None:
    ...
    remove its alias wrapper, and wipe its SARTHI_HOME directory.

    We shell out to ``sarthi -p <name> gateway stop|uninstall`` because
    service names, unit paths, and plist paths are all derived from the
    current SARTHI_HOME and can't be easily switched in-process.
    """
    import sys as _sys
    name = profile.name
    ...
    #    Use `python -m sarthi_cli.main` so we don't depend on a `sarthi`
    #    wrapper that may be half-removed mid-uninstall.
    sarthi_invocation = [_sys.executable, "-m", "sarthi_cli.main", "--profile", name]
    ...
    # 3. Wipe the profile's SARTHI_HOME directory.
    try:
...
def run_gui_uninstall(args):
    ...
    Mirrors ``sarthi uninstall --gui``. Removes the desktop app's built
    artifacts, the packaged app bundle (best-effort), and the Electron
    userData dir — nothing under ``$SARTHI_HOME`` config/sessions/.env, and
    never the Python agent or its venv.
    """
    from sarthi_cli.gui_uninstall import (
        agent_is_installed,
        gui_install_summary,
        uninstall_gui,
    )
    ...
    summary = gui_install_summary(sarthi_home)
    skip_confirm = bool(getattr(args, "yes", False))

    print()
    print(color("┌─────────────────────────────────────────────────────────┐", Colors.MAGENTA, Colors.BOLD))
    print(color("│         ⚕ Sarthi Chat GUI Uninstaller                  │", Colors.MAGENTA, Colors.BOLD))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.MAGENTA, Colors.BOLD))
    print()

    if not summary["gui_installed"]:
        print("No Sarthi Chat GUI installation was found.")
        print(f"  Checked: {sarthi_home}, and the standard app locations for this OS.")
        return

    print(color("This removes the Chat GUI only. The Sarthi agent stays installed.", Colors.CYAN))
    print()
    ...
    print()
    if agent_is_installed(sarthi_home):
        print(color("Kept intact:", Colors.GREEN, Colors.BOLD))
        print(f"  • The Sarthi agent at {sarthi_home / 'sarthi-agent'}")
        print(f"  • Your config, sessions, and secrets under {sarthi_home}")
        print()
    ...
    print(color("└─────────────────────────────────────────────────────────┘", Colors.GREEN, Colors.BOLD))
    print()
    print("The Sarthi agent is still installed. Run 'sarthi' to use the CLI,")
    print("or 'sarthi uninstall' to remove the agent too.")
    print()
...
def run_uninstall(args):
    ...
    Options:
    - Full uninstall: removes code + ~/.sarthi/ (configs, data, logs)
    - Keep data: removes code but keeps ~/.sarthi/ for future reinstall
    """
    ...
    # offer to clean them up too instead of leaving zombie SARTHI_HOMEs
    # and systemd units behind.
    is_default_profile = _is_default_sarthi_home(sarthi_home)
    named_profiles = _discover_named_profiles() if is_default_profile else []

    # Non-interactive fast path (``--yes``): no prompts. ``--full`` selects a
    # full wipe (code + ~/.sarthi data); otherwise keep-data. Named profiles
    ...
    print()
    print(color("┌─────────────────────────────────────────────────────────┐", Colors.MAGENTA, Colors.BOLD))
    print(color("│            ⚕ Sarthi Agent Uninstaller                  │", Colors.MAGENTA, Colors.BOLD))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.MAGENTA, Colors.BOLD))
    print()
    ...
    print(f"  Config:  {sarthi_home / 'config.yaml'}")
    print(f"  Secrets: {sarthi_home / '.env'}")
    print(f"  Data:    {sarthi_home / 'cron/'}, {sarthi_home / 'sessions/'}, {sarthi_home / 'logs/'}")
    print()
    ...
    # their alias wrappers, and wiping their SARTHI_HOME dirs. Otherwise
    ...
    print()
    if full_uninstall:
        print(color("⚠️  WARNING: This will permanently delete ALL Sarthi data!", Colors.RED, Colors.BOLD))
        ...
        print("This will remove the Sarthi code but keep your configuration and data.")
...
def _print_uninstall_dry_run(*, project_root: Path, sarthi_home: Path, full_uninstall: bool) -> None:
    ...
    print("  • Sarthi PATH entries from shell configs / Windows User PATH")
    print("  • Sarthi wrapper scripts and Sarthi-managed node/npm/npx symlinks")
    print("  • Desktop Chat GUI artifacts")
    print(f"  • Code checkout: {project_root}")
    if full_uninstall:
        print(f"  • Sarthi config/data: {sarthi_home}")
        if _is_default_sarthi_home(sarthi_home):
        ...
        print(f"  • Keep Sarthi config/data: {sarthi_home}")
    print()
...
def _perform_uninstall(
    *,
    ...
    ``sarthi`` wrapper + node symlinks → remove the desktop Chat GUI artifacts →
    delete the code checkout → (Windows) remove PortableGit/Node → optionally
    wipe ``$SARTHI_HOME`` data and named profiles on full uninstall.
    """
    print()
    ...
    if _is_windows():
        ...
        # Expand %LOCALAPPDATA% etc. in sarthi_home so the marker matching is
        # against fully resolved paths — installer writes literal strings
        # like C:\Users\<u>\AppData\Local\sarthi\git\cmd, not %LOCALAPPDATA%.
        removed_path_entries = remove_path_from_windows_registry(Path(os.path.expandvars(str(sarthi_home))))
        if removed_path_entries:
            for entry in removed_path_entries:
                log_success(f"Removed from User PATH: {entry}")
        else:
            log_info("No Sarthi-owned PATH entries in User environment")

        log_info("Removing SARTHI_HOME / SARTHI_GIT_BASH_PATH User env vars...")
        removed_env = remove_sarthi_env_vars_windows()
        if removed_env:
            for name in removed_env:
                log_success(f"Removed User env var: {name}")
        else:
            log_info("No Sarthi-set User env vars to remove")
    
    # 3. Remove wrapper script
    log_info("Removing sarthi command...")
    ...
    #     (only when they still point into this Sarthi home's node dir, so we
    #     never clobber an existing nvm / user-managed Node).
    log_info("Removing Sarthi-managed node/npm/npx symlinks...")
    removed_node_links = remove_node_symlinks(sarthi_home)
    if removed_node_links:
        for link in removed_node_links:
            log_success(f"Removed {link}")
    else:
        log_info("No Sarthi-managed node/npm/npx symlinks found")
    ...
    #     step-5 rmtree(sarthi_home) would sweep the in-tree artifacts anyway,
    #     but the packaged app + Electron userData live OUTSIDE SARTHI_HOME and
    #     must be cleaned explicitly here.
    log_info("Removing desktop Chat GUI artifacts...")
    try:
        from sarthi_cli.gui_uninstall import uninstall_gui
        gui_removed = uninstall_gui(sarthi_home)
        if not gui_removed:
    ...
    try:
        if project_root.exists():
            # If the install is inside ~/.sarthi/, just remove the sarthi-agent subdir
            if sarthi_home in project_root.parents or project_root.parent == sarthi_home:
    ...
    #     under SARTHI_HOME but they're install tooling, not config — safe to
    #     remove even in "keep data" mode.  If we're doing a full uninstall
    #     the step-5 rmtree(sarthi_home) would sweep them anyway; calling
    ...
    if _is_windows():
        ...
        removed_artifacts = remove_portable_tooling_windows(sarthi_home)
    ...
    # 5. Optionally remove ~/.sarthi/ data directory (and named profiles)
    if full_uninstall:
        # 5a. Stop and remove each named profile's gateway service and
        #     alias wrapper. The profile SARTHI_HOME dirs live under
        ...
        try:
            if sarthi_home.exists():
                shutil.rmtree(sarthi_home)
                log_success(f"Removed {sarthi_home}")
        except Exception as e:
            log_warn(f"Could not fully remove {sarthi_home}: {e}")
            log_info("You may need to manually remove it")
    else:
        log_info(f"Keeping configuration and data in {sarthi_home}")
    ...
    print()
    print("Thank you for using Sarthi Agent! ⚕")
    print()
...
def main(argv=None) -> int:
    """Module entrypoint: ``python -m sarthi_cli.uninstall --mode <gui|lite|full>``.
    ...
    so ``import sarthi_cli`` resolves from source while the venv is torn down.

    This module imports only stdlib + ``sarthi_constants`` + ``sarthi_cli.colors``
    (and lazily ``sarthi_cli.gui_uninstall``), so it runs fine under a bare
    system Python with no site-packages from the venv.
    """
    import argparse

    parser = argparse.ArgumentParser(prog="python -m sarthi_cli.uninstall")
    parser.add_argument(
        "--mode",
