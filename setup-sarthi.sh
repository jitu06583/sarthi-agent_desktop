...
# 5. Symlinks the 'sarthi' CLI command into a user-facing bin dir
...
echo ""
echo -e "${CYAN}⚕ Sarthi Agent Setup${NC}"
echo ""
...
if is_termux; then
    ...
    if [ -n "$UV_CMD" ]; then
        ...
        _uv_log="$(mktemp 2>/dev/null || echo "/tmp/sarthi-uv-install.$$.log")"
...
# ============================================================================
# PATH setup — symlink sarthi into a user-facing bin dir
# ============================================================================

echo -e "${CYAN}→${NC} Setting up sarthi command..."

SARTHI_BIN="$SCRIPT_DIR/venv/bin/sarthi"
COMMAND_LINK_DIR="$(get_command_link_dir)"
COMMAND_LINK_DISPLAY_DIR="$(get_command_link_display_dir)"
mkdir -p "$COMMAND_LINK_DIR"
ln -sf "$SARTHI_BIN" "$COMMAND_LINK_DIR/sarthi"
echo -e "${GREEN}✓${NC} Symlinked sarthi → $COMMAND_LINK_DISPLAY_DIR/sarthi"
...
# ============================================================================
# Seed bundled skills into ~/.sarthi/skills/
# ============================================================================

SARTHI_SKILLS_DIR="${SARTHI_HOME:-$HOME/.sarthi}/skills"
mkdir -p "$SARTHI_SKILLS_DIR"

echo ""
echo "Syncing bundled skills to ~/.sarthi/skills/ ..."
if "$SCRIPT_DIR/venv/bin/python" "$SCRIPT_DIR/tools/skills_sync.py" 2>/dev/null; then
    echo -e "${GREEN}✓${NC} Skills synced"
else
    # Fallback: copy if sync script fails (missing deps, etc.)
    if [ -d "$SCRIPT_DIR/skills" ]; then
        cp -rn "$SCRIPT_DIR/skills/"* "$SARTHI_SKILLS_DIR/" 2>/dev/null || true
...
echo "Other commands:"
echo "  sarthi status        # Check configuration"
if is_termux; then
    echo "  sarthi gateway       # Run gateway in foreground"
else
    echo "  sarthi gateway install # Install gateway service (messaging + cron)"
fi
echo "  sarthi cron list     # View scheduled jobs"
echo "  sarthi doctor        # Diagnose issues"
echo ""
...
if [[ $REPLY =~ ^[Yy]$ ]] || [[ -z $REPLY ]]; then
    ...
    "$SCRIPT_DIR/venv/bin/python" -m sarthi_cli.main setup
fi
