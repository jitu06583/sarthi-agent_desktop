...
_SARTHI_CORE_TOOLS = [
    ...
    # read-only terminal tab (both gated on SARTHI_DESKTOP via check_fn —
    ...
    # spawned as a kanban worker (SARTHI_KANBAN_TASK env set) or the current
...
TOOLSETS = {
    ...
    "x_search": {
        "description": (
            ...
            "enable in `sarthi tools` → X (Twitter) Search."
        ),
        "tools": ["x_search"],
        "includes": []
    },
    ...
    "kanban": {
        "description": (
            ...
            "is spawned by the kanban dispatcher (SARTHI_KANBAN_TASK env "
    ...
    # Coding posture (base Sarthi — CLI/TUI/desktop/ACP). Auto-selected in a
    ...
    "coding": {
        ...
        # non-configurable-toolset recovery loop in sarthi_cli/tools_config.py).
        "posture": True,
    },
    
    # ==========================================================================
    # Full Sarthi toolsets (CLI + messaging platforms)
    #
    ...
    # the `sarthi send` CLI), not by the model deciding to send on its own.
    # ==========================================================================
    ...
    "sarthi-cli": {
        ...
        "tools": _SARTHI_CORE_TOOLS,
        "includes": []
    },

    "sarthi-cron": {
        # Mirrors sarthi-cli so cron's "default" toolset is the same set of
        # core tools users see interactively — then `sarthi tools` filters
        # them down per the platform config. _DEFAULT_OFF_TOOLSETS (moa,
        # homeassistant) are excluded by _get_platform_tools() unless
        # the user explicitly enables them.
        "description": "Default cron toolset - same core tools as sarthi-cli; gated by `sarthi tools`",
        "tools": _SARTHI_CORE_TOOLS,
        "includes": []
    },
    ...
    "sarthi-email": {
        "description": "Email bot toolset - interact with Sarthi via email (IMAP/SMTP)",
        "tools": _SARTHI_CORE_TOOLS,
        "includes": []
    },
    ...
    "sarthi-sms": {
        "description": "SMS bot toolset - interact with Sarthi via SMS (Twilio)",
        "tools": _SARTHI_CORE_TOOLS,
        "includes": []
    },
    ...
    "sarthi-webhook": {
        ...
        "tools": _SARTHI_WEBHOOK_SAFE_TOOLS,
        "includes": []
    },
    ...
    "sarthi-gateway": {
        ...
        "tools": [],
        "includes": ["sarthi-telegram", "sarthi-discord", "sarthi-whatsapp", "sarthi-slack", "sarthi-signal", "sarthi-bluebubbles", "sarthi-homeassistant", "sarthi-email", "sarthi-sms", "sarthi-mattermost", "sarthi-matrix", "sarthi-dingtalk", "sarthi-feishu", "sarthi-wecom", "sarthi-wecom-callback", "sarthi-weixin", "sarthi-qqbot", "sarthi-webhook", "sarthi-yuanbao"]
    }
}
...
def bundle_non_core_tools(toolset_name: str) -> Set[str]:
    """Return a ``sarthi-*`` bundle's platform-specific tools, excluding core.

    Platform bundles are defined as ``_SARTHI_CORE_TOOLS + [platform extras]``.
    ...
    Bundle nesting is one level deep in practice (only ``sarthi-gateway``
...
def resolve_toolset(name: str, visited: Set[str] = None, *, include_registry: bool = True) -> List[str]:
    ...
    if not toolset:
        # Auto-generate a toolset for plugin platforms (sarthi-<name>).
        # Gives them _SARTHI_CORE_TOOLS plus any tools the plugin registered
        ...
        if include_registry and name.startswith("sarthi-"):
            platform_name = name[len("sarthi-"):]
            try:
                from gateway.platform_registry import platform_registry
                if platform_registry.is_registered(platform_name):
                    plugin_tools = set(_SARTHI_CORE_TOOLS)
                    try:
                        from tools.registry import registry
                        plugin_tools.update(
