"""Run the read-tool eval through the REAL Sarthi AIAgent.

For each task: fresh temp SARTHI_HOME, fresh fixture workspace, real
...
def run_task(task, model: str, provider: str, timeout_mult: float,
    ...
    sarthi_home = Path(tempfile.mkdtemp(prefix="readtool-home-")) / ".sarthi"
    ...
    try:
        ...
        shutil.rmtree(sarthi_home.parent, ignore_errors=True)
    return result
...
def main() -> int:
    ...
    if not os.environ.get("OPENROUTER_API_KEY"):
        raise SystemExit(
            ...
            "source ~/.sarthi/.env; set +a  — then relaunch."
        )
