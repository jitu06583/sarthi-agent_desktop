...
interface SarthiConsoleModalProps {
  open: boolean;
  onClose: () => void;
}
...
export function SarthiConsoleModal({ open, onClose }: SarthiConsoleModalProps) {
  const modalRef = useModalBehavior({ open, onClose });
  const hostRef = useRef<HTMLDivElement | null>(null);
  const termRef = useRef<XtermTerminal | null>(null);
  const wsRef = useRef<WebSocket | null>(null);
  const lineRef = useRef("");
  const promptRef = useRef("sarthi> ");
  const inputPromptRef = useRef("sarthi> ");
  ...
  const handleFrame = useCallback(
    (frame: ConsoleFrame) => {
      ...
      if (frame.type === "ready") {
        const nextPrompt = frame.prompt || "sarthi> ";
  ...
  useEffect(() => {
    ...
    writeLine(term, "\x1b[2mConnecting to Sarthi Console...\x1b[0m");
  ...
  return createPortal(
    <div
      ...
      role="dialog"
      aria-modal="true"
      aria-labelledby="sarthi-console-title"
    >
      <div
        className={cn(
          themedBody,
