...
import { api, SARTHI_BASE_PATH } from "@/lib/api";
...
export const MANIFEST_CACHE_KEY = "sarthi:plugin-manifests";
...
export function usePlugins() {
  ...
  useEffect(() => {
    ...
    for (const manifest of manifests) {
      ...
      if (manifest.css) {
        const cssUrl = `${SARTHI_BASE_PATH}/dashboard-plugins/${manifest.name}/${manifest.css}`;
      ...
      const baseUrl = `${SARTHI_BASE_PATH}/dashboard-plugins/${manifest.name}/${manifest.entry}`;
      const scriptSrc = import.meta.env.DEV
        ? `${baseUrl}?sarthi_dv=${Date.now()}`
        : baseUrl;
      ...
      script.setAttribute("data-sarthi-plugin", manifest.name);
