...
const SIDEBAR_COLLAPSED_KEY = "sarthi-sidebar-collapsed";
          ...
          >
            ...
            >
              ...
              {sidebarNav.pluginItems.length > 0 && (
                <div
                  aria-labelledby="sarthi-sidebar-plugin-nav-heading"
                ...
                >
                  <span
                    ...
                    id="sarthi-sidebar-plugin-nav-heading"
                  >
...
function SidebarSystemActions({
  ...
  const { activeAction, isBusy, isRunning, pendingAction, runAction } =
    useSystemActions();
  const canUpdateSarthi = status?.can_update_sarthi === true;
  ...
  const updateConfirmDescription = useMemo(() => {
    if (updateConfirmInfo?.behind && updateConfirmInfo.behind > 0) {
      ...
      return `This will run 'sarthi update' (${cmd}) and pull ${n} new commit${n === 1 ? "" : "s"}. The gateway restarts when the update finishes; the current session keeps its prompt cache until then.`;
    }
    const cmd = updateConfirmInfo?.update_command ?? "sarthi update";
    return (
      t.status.updateSarthiConfirmMessage ??
      `This will run 'sarthi update' (${cmd}) and restart the gateway when it finishes.`
    );
  }, [t.status.updateSarthiConfirmMessage, updateConfirmInfo]);
  ...
  if (canUpdateSarthi) {
    items.push({
      action: "update",
      icon: Download,
      label: t.status.updateSarthi,
      runningLabel: t.status.updatingSarthi,
      spin: false,
    });
  }
  ...
  return (
    ...
    <ConfirmDialog
      ...
      description={
        ...
        "This restarts the Sarthi gateway process. Connected channels and active sessions will reconnect afterward."
      }
    ...
    <ConfirmDialog
      cancelLabel={t.common.cancel}
      confirmLabel={t.status.updateSarthiConfirmNow ?? "Update now"}
      description={
      ...
      title={t.status.updateSarthiConfirmTitle ?? `${t.status.updateSarthi}?`}
    />
    </>
  );
}
