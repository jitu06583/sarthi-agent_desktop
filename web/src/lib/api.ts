import { buildSarthiWebSocketUrl } from "@sarthi/shared";

// The dashboard can be served either at the root of its host (e.g.
// https://kanban.tilos.com/) or under a URL prefix when reverse-proxied
// (e.g. https://mission-control.tilos.com/sarthi/). The Python backend
// injects ``window.__SARTHI_BASE_PATH__`` into index.html based on the
// incoming ``X-Forwarded-Prefix`` header so the SPA can address its own
// ``/api/...`` and ``/dashboard-plugins/...`` URLs correctly without a
// rebuild. Empty string means "served at root".
function readBasePath(): string {
  if (typeof window === "undefined") return "";
  const raw = window.__SARTHI_BASE_PATH__ ?? "";
  if (!raw) return "";
...
export const SARTHI_BASE_PATH = readBasePath();
const BASE = SARTHI_BASE_PATH;
...
declare global {
  interface Window {
    __SARTHI_SESSION_TOKEN__?: string;
    __SARTHI_BASE_PATH__?: string;
    ...
    __SARTHI_AUTH_REQUIRED__?: boolean;
  }
}
const SESSION_HEADER = "X-Sarthi-Session-Token";
...
export async function fetchJSON<T>(
  ...
  const token = window.__SARTHI_SESSION_TOKEN__;
  if (token) {
  ...
  if (res.status === 401) {
    ...
    // (``sarthi update``, ``sarthi gateway restart``, etc.). A tab kept
    // open across the restart holds the OLD token in
    // ``window.__SARTHI_SESSION_TOKEN__`` from the previous HTML render,
    ...
    if (!window.__SARTHI_AUTH_REQUIRED__ && !options?.allowUnauthorized) {
      let alreadyReloaded = false;
      try {
        alreadyReloaded =
          sessionStorage.getItem("sarthi.tokenReloadAttempted") === "1";
      } catch {
        /* SSR / privacy mode — fall through to throw */
      }
      if (!alreadyReloaded) {
        try {
          sessionStorage.setItem("sarthi.tokenReloadAttempted", "1");
        } catch {
  ...
  if (res.ok) {
    ...
    // current ``window.__SARTHI_SESSION_TOKEN__`` is valid, so the next
    // 401 — if any — should be allowed to trigger its own reload cycle.
    try {
      sessionStorage.removeItem("sarthi.tokenReloadAttempted");
    } catch {
      /* SSR / privacy mode — ignore */
    }
  }
 ...
 *
 * The dashboard's gated-mode WS auth (``sarthi_cli.web_server._ws_auth_ok``)
...
export async function buildWsAuthParam(): Promise<[string, string]> {
  if (window.__SARTHI_AUTH_REQUIRED__) {
 ...
 *  - loopback / ``--insecure``: attach the ``X-Sarthi-Session-Token`` header.
 *  - gated OAuth: no token header (it's absent by design); the
 *    ``sarthi_session_at`` cookie rides along via ``credentials: 'include'``.
 *
 ...
 * ``window.__SARTHI_SESSION_TOKEN__`` directly, so the gated-mode ticket
...
export async function buildWsUrl(
  ...
  return buildSarthiWebSocketUrl({
...
export const api = {
  ...
  updateSarthi: () =>
    fetchJSON<ActionResponse>("/api/sarthi/update", { method: "POST" }),
  checkSarthiUpdate: (force = false) =>
    fetchJSON<UpdateCheckResponse>(
      `/api/sarthi/update/check${force ? "?force=true" : ""}`,
    ),
...
export interface SkillHubSource {
  ...
  /** sarthi-index only: whether the centralized index loaded. */
  available?: boolean;
}
...
export interface StatusResponse {
   ...
   * updates are handled by the outer launcher instead of ``sarthi update``. */
  can_update_sarthi?: boolean;
  config_path: string;
