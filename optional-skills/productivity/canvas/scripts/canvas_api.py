#!/usr/bin/env python3
"""Canvas LMS API CLI for Sarthi Agent.
...
def _check_config():
    ...
    if missing:
        sarthi_env = os.path.join(
            os.environ.get("SARTHI_HOME", os.path.expanduser("~/.sarthi")), ".env"
        )
        print(
            f"Missing required environment variables: {', '.join(missing)}\n"
            f"Set them in {sarthi_env} or export them in your shell.\n"
...
def main():
    parser = argparse.ArgumentParser(
        description="Canvas LMS API CLI for Sarthi Agent"
    )
