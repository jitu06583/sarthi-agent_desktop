...
### 1. Start order: editor first, Sarthi second

Sarthi probes MCP servers at session start. If the editor (and its server)
...
`LogModelContextProtocol` with the address), then open a NEW Sarthi session.
...
restart the editor, restart the Sarthi session.
...
server fails to bind (Output Log) or Sarthi' probe times out. Fix: change
Server Port Number in Editor Preferences > Model Context Protocol AND the
`url` in `~/.sarthi/config.yaml` (`mcp_servers.unreal-engine`), then restart
...
### 5. GenerateClientConfig is not for Sarthi

`ModelContextProtocol.GenerateClientConfig` writes config files for Claude
Code/Cursor/VSCode/Gemini/Codex into the project root. Sarthi' connection
lives in `~/.sarthi/config.yaml` via `sarthi mcp install unreal-engine`.
Running GenerateClientConfig neither helps nor harms Sarthi — just don't
mistake it for the Sarthi setup step.
...
explicitly warns against overlapping calls. Sarthi executes same-turn tool
...
### 9. Timeouts: Sarthi gives up before Unreal does

Sarthi' default per-call timeout is 120 s. Asset imports, first-shader
...
`<Project>/Saved/Screenshots/...`). If Sarthi runs elsewhere (SSH backend,
