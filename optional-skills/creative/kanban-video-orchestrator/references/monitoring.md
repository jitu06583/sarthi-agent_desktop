...
sarthi kanban watch --tenant <project-slug>

# Snapshot of the board
sarthi kanban list --tenant <project-slug>
sarthi kanban list --tenant <project-slug> --json     # machine-readable

# Per-status counts + oldest-ready age
sarthi kanban stats --tenant <project-slug>

# Visual dashboard (browser)
sarthi dashboard

# Inspect a specific task (includes comments + events)
sarthi kanban show <task-id>

# Follow a single task's event stream
sarthi kanban tail <task-id>
```

Verify available subcommands with `sarthi kanban --help` — the kanban CLI
...
|---------|--------------|--------|
| Task RUNNING but no heartbeat in 2+ min | Worker stuck, infinite loop, blocked on input | `sarthi kanban show <id>` — read the worker's last events. The dispatcher SIGTERMs tasks that exceed their `max-runtime`; if you need to stop one earlier, `sarthi kanban block <id>` then `sarthi kanban archive <id>`, and create a re-run task. |
| Same task retried 2+ times | Reproducible failure (missing key, bad spec, broken tool) | `sarthi kanban show <id>` to read failure events. Fix root cause before re-running. |
| RUNNING longer than max_runtime | Task is slow but progressing OR genuinely stuck | Check heartbeats with `sarthi kanban tail <id>`. If progressing, the dispatcher will SIGTERM eventually anyway — raise `max-runtime` on a re-created task. |
...
| Tasks created but never picked up | Profile not running, or tenant mismatch, or dispatcher not running | Check `sarthi profile list` (profile exists?), `sarthi status` (gateway/dispatcher up?), and verify tenant. |
...
sarthi kanban comment <renderer-task-id> "Scene 3 looks too sparse \
— increase visual density. Tighten color palette to brand spec."

# 2. Create a re-render task with the original as parent
sarthi kanban create "Scene 3 — re-render with feedback" \
...
NEW_TASK_ID=$(sarthi kanban create "Generate SRT captions from voiceover" \
    --assignee captioner \
    --workspace dir:"$HOME/projects/video-pipeline/<slug>" \
    --tenant <slug> \
...
sarthi kanban link "$NEW_TASK_ID" <editor-task-id>
```
...
sarthi kanban block <task-id>
sarthi kanban archive <task-id>

# Diagnose what happened
sarthi kanban show <task-id>      # task body, comments, recent events
sarthi kanban tail <task-id>      # follow the live event stream
sarthi kanban log <task-id>       # worker process log
```
...
3. Re-fire the initial `sarthi kanban create` for the director
...
while true; do
    clear
    sarthi kanban list --tenant <slug>
    echo "---"
    sarthi kanban stats --tenant <slug>
    sleep 30
done
```

For a live event feed, run `sarthi kanban watch --tenant <slug>` in a
...
- **Profile process not running.** Tasks queue indefinitely in READY if no
  worker for that profile is online. Check `sarthi profile list` and start
