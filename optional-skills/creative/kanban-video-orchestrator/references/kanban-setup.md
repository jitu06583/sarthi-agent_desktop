...
the actual `setup.sh` that creates the project workspace, configures Sarthi
...
2. **Create profiles** — `sarthi profile create <name> --clone`
3. **Configure profiles** — patch each profile's
   `~/.sarthi/profiles/<name>/config.yaml` to set toolsets, always_load skills,
   and `cwd`
4. **Write SOUL.md per profile** — the personality + role definition
5. **Copy any provided assets + write `brief.md`, `TEAM.md`, and `taste/`**
6. **Fire the initial kanban task** — `sarthi kanban create` assigned to the director
...
```bash
sarthi profile create director --clone 2>/dev/null || true
```
...
Each profile has a YAML config at `~/.sarthi/profiles/<name>/config.yaml`. The
...
p = os.path.expanduser(f"~/.sarthi/profiles/{profile}/config.yaml")
with open(p) as f:
...
PyYAML must be installed in the user's Python (it ships with most Sarthi
...
Each profile gets a `SOUL.md` at `~/.sarthi/profiles/<name>/SOUL.md` that
...
```bash
sarthi kanban create "Direct production of <video title>" \
    --assignee director \
...
the Sarthi `.env` (`${SARTHI_HOME:-$HOME/.sarthi}/.env`) and macOS Keychain
(if on macOS):
...
check_key() {
    ...
    local kc_account="$2"
    local kc_service="$3"
    local _sarthi_env="${SARTHI_HOME:-$HOME/.sarthi}/.env"
    if grep -q "^${var}=" "$_sarthi_env" 2>/dev/null && \
       [ -n "$(grep "^${var}=" "$_sarthi_env" | cut -d= -f2-)" ]; then
        return 0
    fi
    ...
    echo "ERROR: ${var} not set in ${_sarthi_env} or Keychain (${kc_account}/${kc_service})"
    return 1
}

check_key ELEVENLABS_API_KEY sarthi ELEVENLABS_API_KEY || exit 1
check_key OPENROUTER_API_KEY sarthi OPENROUTER_API_KEY || exit 1
# ...
```
