...
Maps each role archetype to the Sarthi skills it should `always_load` and the
toolsets it needs. Only references skills that ship in the public sarthi-agent
...
## Sarthi skills relevant to video production

### Visual / rendering skills (`sarthi-agent/skills/creative/`)
...
### Design / pre-production skills (`sarthi-agent/skills/creative/`)
...
### Audio / media skills (`sarthi-agent/skills/creative/` + `skills/media/`)
...
These are **not** Sarthi skills but external CLIs / APIs that profiles invoke.
...
## Built-in Sarthi tools for media review

These are native Sarthi tools — not invoked via terminal but through their own
...
key is present in `${SARTHI_HOME:-~/.sarthi}/.env` (or macOS Keychain) before firing the kanban.
...
| Anthropic | `ANTHROPIC_API_KEY` | every Sarthi profile (Claude) |

If a key is missing, prompt the user to add it. Storage methods, in order of
preference: macOS Keychain → `${SARTHI_HOME:-~/.sarthi}/.env` → environment variable.
...
When a new Sarthi-public video skill ships:
