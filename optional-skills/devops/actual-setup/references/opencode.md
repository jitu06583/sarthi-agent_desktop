...
is typically only in `~/.sarthi/.env`, not a shell profile, so the env form
...
```
OpenCode DOES use the `provider/model` slash form on the CLI (unlike Sarthi,
...
`sarthi-custom-providers` skill, pitfall 2). Sarthi hits this because it forwards
