...
Usage (via cron with --no-agent):
    ...
    sarthi cron create api-events \\
      --schedule "*/1 * * * *" --no-agent \\
      --script "$SARTHI_HOME/skills/devops/watchers/scripts/watch_http_json.py" \\
...
def main() -> int:
    ...
    req = urllib.request.Request(args.url, headers={"User-Agent": "Sarthi-Watcher/1.0"})
