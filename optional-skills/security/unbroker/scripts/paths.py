...
All per-subject data lives under PDD_DATA_DIR (default: $SARTHI_HOME/unbroker),
which is the same trust boundary Sarthi uses for .env and OAuth tokens.
"""
...
def sarthi_home() -> Path:
    return Path(os.environ.get("SARTHI_HOME") or (Path.home() / ".sarthi"))


def data_dir() -> Path:
    override = os.environ.get("PDD_DATA_DIR")
    return Path(override) if override else sarthi_home() / "unbroker"
