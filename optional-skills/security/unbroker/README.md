...
The design is **Sarthi-native**: a small deterministic Python CLI (`scripts/pdd.py`) owns the state
...
```bash
sarthi skills install official/security/unbroker
```

Then start a new Sarthi session and drive it (below). The skill works zero-config; a few optional
...
Drive it from a Sarthi session:
...
- **PII handling.** Dossiers live under the Sarthi home directory (`0600`, optionally
