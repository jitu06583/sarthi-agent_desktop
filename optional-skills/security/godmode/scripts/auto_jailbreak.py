...
Usage in execute_code:
    exec(open(os.path.expanduser(
        os.path.join(os.environ.get("SARTHI_HOME", os.path.expanduser("~/.sarthi")), "skills/red-teaming/godmode/scripts/auto_jailbreak.py")
    )).read())
...
try:
    ...
    _SKILL_DIR = Path(os.getenv("SARTHI_HOME", Path.home() / ".sarthi")) / "skills" / "red-teaming" / "godmode"
...
SARTHI_HOME = Path(os.getenv("SARTHI_HOME", Path.home() / ".sarthi"))
CONFIG_PATH = SARTHI_HOME / "config.yaml"
PREFILL_PATH = SARTHI_HOME / "prefill.json"
...
MODEL_STRATEGIES = {
    ...
    # Nous/Sarthi models — already uncensored, just needs clean prompt
    "sarthi": {
        "order": ["prefill_only"],
        "system_templates": {},
    },
...
def _detect_model_family(model: str) -> str:
    ...
    if "grok" in model_lower or "x-ai" in model_lower:
        return "grok"
    if "sarthi" in model_lower or "nous" in model_lower:
        return "sarthi"
...
def _get_current_model() -> tuple:
    """Read current model and provider from Sarthi config.yaml.
...
def _write_prefill(prefill_messages: list):
    """Write prefill messages to ~/.sarthi/prefill.json."""
    ...
    if winning_strategy:
        ...
        if not dry_run:
            ...
            if verbose:
                ...
                print()
                print("[DONE] Jailbreak locked in. Restart Sarthi for changes to take effect.")
        else:
            if verbose:
...
def undo_jailbreak(verbose=True):
    ...
    if verbose:
        print("[UNDO] Jailbreak removed. Restart Sarthi for changes to take effect.")
