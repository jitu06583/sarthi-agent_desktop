...
Usage:
  ...
  <server_name> matches the files in $SARTHI_HOME/mcp-tokens/<server>.json etc.
...
def _sarthi_home():
    # Prefer Sarthi' own resolver (profile-safe); fall back to env then ~/.sarthi.
    try:
        from sarthi_constants import get_sarthi_home
        return str(get_sarthi_home())
    except Exception:
        return os.environ.get("SARTHI_HOME") or os.path.expanduser("~/.sarthi")


def _tokens_dir():
    return os.path.join(_sarthi_home(), "mcp-tokens")
    ...
    status, hdrs, body = _post(
        ...
        data={
            ...
            "params": {"protocolVersion": "2025-06-18", "capabilities": {},
                       "clientInfo": {"name": "sarthi-diag", "version": "1.0"}},
        },
        headers={
