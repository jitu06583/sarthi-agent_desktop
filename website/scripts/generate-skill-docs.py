...
def rewrite_relative_links(body: str, meta: dict[str, Any]) -> str:
    ...
    base = f"https://github.com/jitendra-singh-thakur/sarthi-agent/blob/main/{source_dir}/{meta['rel_path']}"
...
def render_skill_page(
    ...
    sarthi_meta = (fm.get("metadata") or {}).get("sarthi") or {}
    tags = sarthi_meta.get("tags") or []
    related = sarthi_meta.get("related_skills") or []
    ...
    if meta["source_kind"] == "bundled":
        ...
        info_rows.append(
            (
                "Source",
                "Optional — install with `sarthi skills install official/"
                + meta["category"]
                + "/"
                + meta["slug"]
                + "`",
...
def build_catalog_md_bundled(entries: list[tuple[dict[str, Any], dict[str, Any]]]) -> str:
    ...
    lines = [
        ...
        'description: "Catalog of bundled skills that ship with Sarthi Agent"',
        "---",
        "",
        "# Bundled Skills Catalog",
        "",
        "Sarthi ships with a large built-in skill library copied into `~/.sarthi/skills/` on install. Each skill below links to a dedicated page with its full definition, setup, and usage.",
        "",
        "Sarthi also syncs bundled skills on `sarthi update`, but the sync manifest respects local deletions and user edits. If a skill listed here is missing from your profile's `~/.sarthi/skills/` tree, it is still shipped with Sarthi; restore it with `sarthi skills reset <name> --restore`.",
        "",
