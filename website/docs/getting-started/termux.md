...
description: "Run Sarthi Agent directly on an Android phone with Termux"
---

# Sarthi on Android with Termux
...
Sarthi Agent can run directly on an Android phone through [Termux](https://termux.dev/).
...
That does not stop Sarthi from working well as a phone-native CLI agent — it just means the recommended mobile install is intentionally narrower than the desktop/server install.
...
APT-managed Sarthi installs are marked with install method `apt`. Sarthi therefore does not run its Git self-updater against package-owned files; use the package manager instead:

```bash
pkg update
pkg upgrade sarthi-agent
```

Packaging/repository/signing problems for this option should be reported to the community packaging repositories above. Sarthi runtime bugs can still be reported here, keeping in mind that Android/Termux support is best-effort.

---

## Option 1: One-line installer

Sarthi now ships a Termux-aware installer path:
...
- links `sarthi` into `$PREFIX/bin` so it stays on your Termux PATH
...
### 5. Put `sarthi` on your Termux PATH

```bash
ln -sf "$PWD/venv/bin/sarthi" "$PREFIX/bin/sarthi"
```

`$PREFIX/bin` is already on PATH in Termux, so this makes the `sarthi` command persist across new shells without re-activating the venv every time.
...
Or set keys directly in `~/.sarthi/.env`.
...
### `sarthi doctor` says ripgrep or Node is missing
