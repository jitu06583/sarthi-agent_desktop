...
title: "Git Worktrees"
description: "Run multiple Sarthi agents safely on the same repository using git worktrees and isolated checkouts"
---

# Git Worktrees

Sarthi Agent is often used on large, long‑lived repositories. When you want to:
...
This page shows how to combine worktrees with Sarthi so each session has a clean, isolated working directory.

## Why Use Worktrees with Sarthi?

Sarthi treats the **current working directory** as the project root:

- CLI: the directory where you run `sarthi` or `sarthi chat`
- Messaging gateways: the directory set by `terminal.cwd` in `~/.sarthi/config.yaml`
...
Sarthi creates `.worktrees/my-experiment/` inside the repo (branch
`sarthi/my-experiment`, based on the freshly-fetched remote tip unless
`worktree_sync: false`), and retargets the session's terminal and file tools
into it — no restart needed. Omit the name to get a random `sarthi-<id>`
...
git worktree add ../repo-feature feature/sarthi-experiment
```

This creates:

- A new directory: `../repo-feature`
- A new branch: `feature/sarthi-experiment` checked out in that directory

Now you can `cd` into the new worktree and run Sarthi there:

```bash
cd ../repo-feature

# Start Sarthi in the worktree
sarthi
```
...
git worktree add ../repo-experiment-a feature/sarthi-a
git worktree add ../repo-experiment-b feature/sarthi-b
```
...
- Works on its own branch (`feature/sarthi-a` vs `feature/sarthi-b`).
...
- Sarthi checkpoint data under `~/.sarthi/checkpoints/` is not automatically pruned when you remove a worktree, but it is usually very small.

## Best Practices

- **One worktree per Sarthi experiment**
...
- **Name branches after the experiment**
  - e.g. `feature/sarthi-checkpoints-docs`, `feature/sarthi-refactor-tests`.
- **Commit frequently**
...
- **Avoid running Sarthi from the bare repo root when using worktrees**
  - Prefer the worktree directories instead, so each agent has a clear scope.

## Using `sarthi -w` (Automatic Worktree Mode)

Sarthi has a built‑in `-w` flag that **automatically creates a disposable git worktree** with its own branch. You don't need to set up worktrees manually — just `cd` into your repo and run:
...
- Check out an isolated branch (e.g. `sarthi/sarthi-<hash>`).
...
```bash
sarthi -w -z "Fix issue #123"
```

For parallel agents, open multiple terminals and run `sarthi -w` in each — every invocation gets its own worktree and branch automatically.

## Putting It All Together

- Use **git worktrees** to give each Sarthi session its own clean checkout.
