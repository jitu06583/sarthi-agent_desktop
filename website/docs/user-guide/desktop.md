...
The [manual update process](https://sarthi-agent.vercel.app/docs/getting-started/updating) also works with the GUI.
...
You can also set the backend URL without the UI via the `SARTHI_DESKTOP_REMOTE_URL` environment variable before launching the app (it overrides the in-app setting); you still sign in from the Gateway settings panel.
