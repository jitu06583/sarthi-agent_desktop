...
If you only run one Sarthi agent, you don't need this page — see
...
[Connecting Desktop to Many Sarthi Instances](./multi-connection-desktop.md).

## When to use this

You want this setup when you have two or more Sarthi agents that should all
...
(`ai.sarthi.gateway-<name>.plist`) or systemd user service
(`sarthi-gateway-<name>.service`). This guide adds the patterns for managing
them collectively.
...
sarthi profile create coder
sarthi profile create personal-bot
sarthi profile create research
...
```bash
sarthi config set gateway.multiplex_profiles true
sarthi gateway restart
```

Equivalently, in the default profile's `~/.sarthi/config.yaml`:
...
You do **not** run `sarthi gateway start` for the secondary profiles — the
...
With a multiplexer running, a named-profile `sarthi gateway start` / `run` is a
...
- `/p/coder/...` API-server requests must use `API_SERVER_KEY` from
  `~/.sarthi/profiles/coder/.env`; the default listener key is rejected.
...
home). `sarthi status` reports the multiplexer and the profiles it serves;
`sarthi status -p <name>` slices to one profile. Each profile still writes its
...
`~/.local/bin/sarthi-gateways` and `chmod +x` it:
...
usage() {
  echo "Usage: sarthi-gateways {start|stop|restart|status|list}"
}
...
run_for_profile() {
  ...
  if [ "$profile" = "default" ]; then
    sarthi gateway "$action"
  else
    sarthi -p "$profile" gateway "$action"
  fi
}
...
```bash
sarthi-gateways start      # start every configured profile
sarthi-gateways stop       # stop every configured profile
sarthi-gateways restart    # restart all
sarthi-gateways status     # status across all
sarthi-gateways list       # delegates to `sarthi gateway list`
```

:::tip
The `default` profile is targeted with `sarthi gateway <action>` (no `-p`),
not `sarthi -p default gateway <action>`. The wrapper above handles both forms.
:::
...
These are equivalent to `sarthi -p coder gateway <action>` — useful if a
...
| Platform | Path                                                              |
| -------- | ----------------------------------------------------------------- |
| macOS    | `~/Library/LaunchAgents/ai.sarthi.gateway-<profile>.plist`        |
| Linux    | `~/.config/systemd/user/sarthi-gateway-<profile>.service`         |

The default profile keeps the historical names: `ai.sarthi.gateway.plist` /
`sarthi-gateway.service`.
...
```bash
# Default profile
tail -f ~/.sarthi/logs/gateway.log
tail -f ~/.sarthi/logs/gateway.error.log

# Named profile
tail -f ~/.sarthi/profiles/<name>/logs/gateway.log
tail -f ~/.sarthi/profiles/<name>/logs/gateway.error.log
```

Stream every profile's log simultaneously:

```bash
tail -f ~/.sarthi/logs/gateway.log ~/.sarthi/profiles/*/logs/gateway.log
```

The CLI also has a structured log viewer:

```bash
sarthi logs -f                  # follow default profile
sarthi -p coder logs -f         # follow one profile
sarthi logs --help              # filters, levels, JSON output
```

## Identify what's actually running

```bash
sarthi profile list             # profiles + model + gateway state
sarthi-gateways status          # full status across every profile
launchctl list | grep sarthi    # macOS — PIDs and labels
systemctl --user list-units 'sarthi-gateway-*'   # Linux — units
```
...
```
~/.sarthi/profiles/<name>/
├── .env              # API keys, bot tokens (chmod 600)
├── config.yaml       # model, provider, toolsets, gateway settings
└── SOUL.md           # personality / system prompt
```

The default profile uses `~/.sarthi/` directly with the same three files.

Edit them with any editor or via the CLI:

```bash
sarthi config set model.model anthropic/claude-sonnet-4    # default profile
...
caffeinate -i -w $(cat ~/.sarthi/gateway.pid) &   # awake while default gateway runs
...
systemd-inhibit --what=idle:sleep --who=sarthi --why="gateways running" \
  sleep infinity &
...
`sarthi-gateway-<profile>.service`) continue running across SSH disconnects
and reboots.
...
grep -H 'TELEGRAM_BOT_TOKEN\|DISCORD_BOT_TOKEN' \
     ~/.sarthi/.env ~/.sarthi/profiles/*/.env
```

## Updating the code

`sarthi update` pulls the latest code once and syncs new bundled skills into
every profile:
...
You ran `sarthi gateway start` after a previous `sarthi gateway stop`. The
...
```bash
ps -ef | grep "sarthi_cli.*-p <profile>"
cat ~/.sarthi/profiles/<profile>/gateway.pid
...
```bash
# macOS
launchctl unload ~/Library/LaunchAgents/ai.sarthi.gateway-<profile>.plist
launchctl load   ~/Library/LaunchAgents/ai.sarthi.gateway-<profile>.plist

# Linux
systemctl --user restart sarthi-gateway-<profile>.service
```

### Health check

```bash
sarthi doctor                  # default profile
sarthi -p <profile> doctor     # one profile
```
