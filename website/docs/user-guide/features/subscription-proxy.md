...
chat completions — use your Sarthi-managed provider subscription as their
...
| Use case | "Use Sarthi as a chat backend" | "Use my Portal sub from another app" |
...
This opens your browser for the Nous Portal OAuth flow. Sarthi stores
the refresh token in `~/.sarthi/auth.json` — the same place all Sarthi
...
Model:      Sarthi-4-70B    # or Sarthi-4.3-36B, Hermes-4-405B
```
...
interface in `sarthi_cli/proxy/adapters/`.
...
If you see `not logged in`, run `sarthi portal`. If you see
...
`sarthi_cli/proxy/adapters/<provider>.py` and registering it in
