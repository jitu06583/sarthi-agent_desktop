...
[Honcho](https://github.com/plastic-labs/honcho) is an AI-native memory backend that adds dialectic reasoning and deep user modeling on top of Sarthi's built-in memory system. Instead of simple key-value storage, Honcho maintains a running model of who the user is — their preferences, communication style, goals, and patterns — by reasoning about conversations after they happen.
...
**Multi-agent profiles**: When multiple Sarthi instances talk to the same user (e.g., a coding assistant and a personal assistant), Honcho maintains separate "peer" profiles. Each peer sees only its own observations and conclusions, preventing cross-contamination of context.

## Setup

```bash
sarthi memory setup    # select "honcho" from the provider list
```
...
- `per-session` — each `sarthi` run gets a fresh session. Clean starts, memory via tools. Recommended for new users.
...
Server-side toggles set via the [Honcho dashboard](https://app.honcho.dev) win over local defaults — Sarthi syncs them back at session init.
...
The `sarthi honcho` subcommand is **only registered when Honcho is the active memory provider** (`memory.provider: honcho` in `config.yaml`). On a fresh install, configure Honcho directly with `sarthi memory setup honcho` (or run `sarthi memory setup` and pick it from the list); the `sarthi honcho` subcommand then appears on the next invocation.

```bash
sarthi memory setup honcho    # Configure Honcho directly (works before activation)
sarthi honcho status          # Connection status, config, and key settings
sarthi honcho setup           # Redirects to `sarthi memory setup` (post-activation alias)
sarthi honcho strategy        # Show or set session strategy (per-session/per-directory/per-repo/global)
sarthi honcho peer            # Show or update peer names + dialectic reasoning level
sarthi honcho mode            # Show or set recall mode (hybrid/context/tools)
sarthi honcho tokens          # Show or set token budget for context and dialectic
sarthi honcho identity        # Seed or show the AI peer's Honcho identity
sarthi honcho sync            # Sync Honcho config to all existing profiles
sarthi honcho peers           # Show peer identities across all profiles
sarthi honcho sessions        # List known Honcho session mappings
sarthi honcho map             # Map current directory to a Honcho session name
sarthi honcho enable          # Enable Honcho for the active profile
sarthi honcho disable         # Disable Honcho for the active profile
sarthi honcho migrate         # Step-by-step migration guide from openclaw-honcho
```

## Migrating from `sarthi honcho`

If you previously used the standalone `sarthi honcho setup`:
...
No re-login or re-setup needed. Run `sarthi memory setup` and select "honcho" — the wizard detects your existing config.
