...
When Sarthi Agent runs inside a messaging gateway (Slack, Discord, Telegram,
...
persona in `~/.sarthi/SOUL.md`, or as a named preset under
`agent.personalities` in `~/.sarthi/config.yaml` (switchable per session
via `/personality`).
...
If you use Sarthi' kanban multi-agent workflow, workers can attach
...
Install MCP servers via `~/.sarthi/config.yaml` under the `mcp_servers`
...
back into the thread as a native attachment. Sarthi Agent's deliverable
