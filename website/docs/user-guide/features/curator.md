...
On a brand-new install (or the first time a pre-curator install ticks after `sarthi update`), the curator **does not run immediately**. The first observation seeds `last_run_at` to "now" and defers the first real pass by one full `interval_hours`. This gives you a full interval to review your skill library, pin anything important, or opt out entirely before the curator ever touches it.

If you want to see what the curator *would* do before it runs for real, run `sarthi curator run --dry-run` — it produces the same review report without mutating the library.
:::

A run has two phases:

1. **Automatic transitions** (deterministic, no LLM). Skills unused for `stale_after_days` (30) become `stale`; skills unused for `archive_after_days` (90) are moved to `~/.sarthi/skills/.archive/`. This is the always-on pruning behavior — it runs whenever the curator is enabled, with no aux-model cost.
...
**Easiest — `sarthi model`:**

```bash
sarthi model                   # → "Auxiliary models — side-task routing"
...
:::note Legacy config
Earlier releases used a one-off `curator.auxiliary.{provider,model}` block. That path still works but emits a deprecation log line — please migrate to `auxiliary.curator` above so the curator shares the same plumbing (`sarthi model`, dashboard Models tab, `base_url`, `api_key`, `timeout`, `extra_body`) as every other aux task.
:::

## CLI

```bash
sarthi curator status         # last run, counts, pinned list, LRU top 5
sarthi curator run            # trigger a run now (blocks until done). Prune-only unless curator.consolidate: true
sarthi curator run --consolidate # force the LLM consolidation pass on for this run, overriding the config default
sarthi curator run --background  # fire-and-forget: start the run in a background thread
sarthi curator run --dry-run  # preview only — report without any mutations
sarthi curator backup         # take a manual snapshot of ~/.sarthi/skills/
sarthi curator rollback       # restore from the newest snapshot
sarthi curator rollback --list     # list available snapshots
sarthi curator rollback --id <ts>  # restore a specific snapshot
sarthi curator rollback -y         # skip the confirmation prompt
sarthi curator pause          # stop runs until resumed
sarthi curator resume
sarthi curator pin <skill>    # never auto-transition this skill
sarthi curator unpin <skill>
sarthi curator adopt <skill>    # hand an unmanaged skill to the curator
sarthi curator adopt --all-unmanaged   # hand over every unmanaged skill
sarthi curator list-unmanaged   # itemize skills with no provenance marker
sarthi curator restore <skill>  # move an archived skill back to active
sarthi curator list-archived    # list skills currently in ~/.sarthi/skills/.archive/
sarthi curator archive <skill>  # manually archive a single skill now
sarthi curator prune [--days N] # bulk-archive agent-created skills idle >= N days (default 90)
sarthi curator ledger           # list the per-mutation audit ledger (all actors)
sarthi curator ledger --skill <name> --limit 50  # filter/paginate ledger entries
sarthi curator rollback <entry-id>  # undo a single mutation from the ledger
sarthi curator purge [--days N] [--dry-run]  # delete archived skills older than the TTL (explicit only)
```

## Backups and rollback

Before every real curator pass, Sarthi takes a tar.gz snapshot of `~/.sarthi/skills/` at `~/.sarthi/skills/.curator_backups/<utc-iso>/skills.tar.gz`. If a pass archives or consolidates something you didn't want touched, you can undo the whole run with one command:

```bash
sarthi curator rollback        # restore newest snapshot (with confirmation)
sarthi curator rollback -y     # skip the prompt
sarthi curator rollback --list # see all snapshots with reason + size
```

The rollback itself is reversible: before replacing the skills tree, Sarthi takes another snapshot tagged `pre-rollback to <target-id>`, so a mistaken rollback can be undone by rolling forward to that one with `--id`.

You can also take manual snapshots at any time with `sarthi curator backup --reason "before-refactor"`. The `--reason` string lands in the snapshot's `manifest.json` and is shown in `--list`.
...
Set `curator.backup.enabled: false` to disable automatic snapshotting. The manual `sarthi curator backup` command still works when backups are disabled only if you set `enabled: true` first — the flag gates both paths symmetrically so there's no way to accidentally skip the pre-run snapshot on mutating runs.

`sarthi curator status` also lists the five least-recently-used skills — a quick way to see what's likely to become stale next.
...
Whole-run snapshots answer "undo everything the last curator pass did" — but sometimes you want to know *who changed what* and undo exactly one mutation. Every skill mutation — curator auto-transitions, agent `skill_manage` calls, and your own CLI archive/restore/purge — appends one entry to the append-only JSONL ledger at `~/.sarthi/skills/.curator_ledger.jsonl`:
...
Archived skills are kept forever by default. If you want `~/.sarthi/skills/.archive/` bounded, set a TTL and purge explicitly — purging never runs automatically, and every purged skill is captured into the ledger (with blobs) first, so even a purge leaves an auditable, recoverable trail:
...
```bash
sarthi curator purge --dry-run   # preview what would be deleted
sarthi curator purge             # delete archives older than the TTL (with confirmation)
sarthi curator purge --days 90   # one-off TTL override
```

## What "agent-created" means

The curator only manages skills explicitly marked as **agent-created** in
`~/.sarthi/skills/.usage.json`. A skill qualifies when ALL of the following
are true:

1. Its name is **not** in `~/.sarthi/skills/.bundled_manifest` (bundled skills shipped with the repo).
2. Its name is **not** in `~/.sarthi/skills/.hub/lock.json` (hub-installed skills).
...
If you manually created a `SKILL.md` or pointed Sarthi at an external skill
...
**To see which skills the curator actually manages**, run `sarthi curator status`.
...
`sarthi curator status` reports an **unmanaged** count alongside the managed
one:
...
unmanaged (no provenance marker): 112 total
  ...
  never auto-staled or archived — `sarthi curator adopt <name>` hands one over
```
...
```bash
sarthi curator list-unmanaged                    # itemize them, with reasons
sarthi curator adopt <name> [<name> ...]         # hand specific skills over
sarthi curator adopt --all-unmanaged --dry-run   # preview the full list
sarthi curator adopt --all-unmanaged             # hand over everything (prompts)
sarthi curator adopt --all-unmanaged --yes       # skip the prompt
```
...
`.usage.json`; read it as policy. `sarthi curator adopt` changes the policy, and
...
**wrote** it — Sarthi edits user-authored skills on your behalf constantly. An
...
- Archives are recoverable via `sarthi curator restore <name>`

If you want to protect a specific skill from ever being touched — for example a
hand-authored skill you rely on — use `sarthi curator pin <name>`. See the next
section.
...
- The **agent's `skill_manage` tool** refuses `delete` on it, pointing the user at `sarthi curator unpin <name>`. Patches and edits still go through, so the agent can improve a pinned skill's content as pitfalls come up without a pin/unpin/re-pin dance.

Pin and unpin with:

```bash
sarthi curator pin <skill>
sarthi curator unpin <skill>
```

The flag is stored as `"pinned": true` on the skill's entry in `~/.sarthi/skills/.usage.json`, so it survives across sessions.
...
If you want a stronger guarantee than "no deletion" — for instance, freezing a skill's content entirely while the agent still reads it — edit `~/.sarthi/skills/<name>/SKILL.md` directly with your editor. The pin guards tool-driven deletion, not your own filesystem access.

## Usage telemetry

The curator maintains a sidecar at `~/.sarthi/skills/.usage.json` with one entry per skill:
...
Every curator run writes a timestamped directory under `~/.sarthi/logs/curator/`:
...
```bash
sarthi curator restore <skill-name>
```

This moves the skill back from `~/.sarthi/skills/.archive/` to the active tree and resets its state to `active`. The restore refuses if a bundled or hub-installed skill has since been installed under the same name (would shadow upstream).

## Disabling per environment

The curator is on by default. To turn it off:

- **For one profile only:** edit `~/.sarthi/config.yaml` (or the active profile's config) and set `curator.enabled: false`.
- **For just one run:** `sarthi curator pause` — the pause persists across sessions; use `resume` to re-enable.
