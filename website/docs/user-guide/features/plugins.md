...
title: "Plugins"
description: "Extend Sarthi with custom tools, hooks, and integrations via the plugin system"
---

# Plugins

Sarthi has a plugin system for adding custom tools, hooks, and integrations without modifying core code.
...
[Adding Tools](/developer-guide/adding-tools) page is for built-in Sarthi
core tools that live in `tools/` and `toolsets.py`.

**→ [Build a Sarthi Plugin](/developer-guide/plugins)** — step-by-step guide with a complete working example.

## Quick overview

Drop a directory into `~/.sarthi/plugins/` with a `plugin.yaml` and Python code:

```
~/.sarthi/plugins/my-plugin/
...
Start Sarthi — your tools appear alongside built-in tools. The model can call them immediately.

### Minimal working example

Here is a complete plugin that adds a `hello_world` tool and logs every tool call via a hook.

**`~/.sarthi/plugins/hello-world/plugin.yaml`**
...
**`~/.sarthi/plugins/hello-world/__init__.py`**

```python
"""Minimal Sarthi plugin — registers a tool and a hook."""
...
Drop both files into `~/.sarthi/plugins/hello-world/`, restart Sarthi, and the model can immediately call `hello_world`. The hook prints a log line after every tool invocation.

The model-facing tool description belongs in `schema["description"]`. The optional `ctx.register_tool(description=...)` value is separate `ToolEntry` registry metadata: when omitted, it defaults to the schema description, but Sarthi does not copy it back into a schema that lacks `description`. Prefer defining the text once in the schema. If you provide both values, keep them synchronized; the model sees the schema value.

Project-local plugins under `./.sarthi/plugins/` are disabled by default. Enable them only for trusted repositories by setting `SARTHI_ENABLE_PROJECT_PLUGINS=true` before starting Sarthi.
...
| Add CLI commands | `ctx.register_cli_command(name, help, setup_fn, handler_fn)` — adds `sarthi <plugin> <subcommand>` |
...
| Gate on env vars | `requires_env: [API_KEY]` in plugin.yaml — prompted during `sarthi plugins install` |
| Distribute via pip | `[project.entry-points."sarthi_agent.plugins"]` |
...
|--------|------|----------|
| Bundled | `<repo>/plugins/` | Ships with Sarthi — see [Built-in Plugins](/user-guide/features/built-in-plugins) |
| User | `~/.sarthi/plugins/` | Personal plugins |
| Project | `.sarthi/plugins/` | Project-specific plugins (requires `SARTHI_ENABLE_PROJECT_PLUGINS=true`) |
| pip | `sarthi_agent.plugins` entry_points | Distributed packages |
| Nix | `services.sarthi-agent.extraPlugins` / `extraPythonPackages` | NixOS declarative installs — see [Nix Setup](/getting-started/nix-setup#plugins) |

Later sources override earlier ones on name collision, so a user plugin with the same name as a bundled plugin replaces it.

### Plugin sub-categories

Within each source, Sarthi also recognizes sub-category directories that route plugins to specialized discovery systems:
...
User plugins at `~/.sarthi/plugins/model-providers/<name>/` and `~/.sarthi/plugins/memory/<name>/` override bundled plugins of the same name — last-writer-wins in `register_provider()` / `register_memory_provider()`. Drop a directory in, and it replaces the built-in without any repo edits.

## Plugins are opt-in (with a few exceptions)

**General plugins and user-installed backends are disabled by default** — discovery finds them (so they show up in `sarthi plugins` and `/plugins`), but nothing with hooks or tools loads until you add the plugin's name to `plugins.enabled` in `~/.sarthi/config.yaml`. This stops third-party code from running without your explicit consent.
...
```bash
sarthi plugins                    # interactive toggle (space to check/uncheck)
sarthi plugins enable <name>      # add to allow-list
sarthi plugins disable <name>     # remove from allow-list + add to disabled
```

After `sarthi plugins install owner/repo`, you're asked `Enable 'name' now? [y/N]` — defaults to no. Skip the prompt for scripted installs with `--enable` or `--no-enable`.

For a reproducible install, pin a full immutable commit (tags, branches, and
abbreviated SHAs are not accepted):

```bash
sarthi plugins install owner/repo --ref 0123456789abcdef0123456789abcdef01234567
```

Sarthi checks out the commit detached, verifies that `HEAD` exactly matches the
requested SHA, and records the canonical source, installed revision, and pin
status in the current profile. `sarthi plugins update` refuses to move a pinned
plugin; choose a new exact commit explicitly with
`sarthi plugins install <source> --force --ref <new-commit>`. The
...
Several categories of plugin bypass `plugins.enabled` — they're part of Sarthi' built-in surface and would break basic functionality if gated off by default:
...
| **User-installed platforms** (under `~/.sarthi/plugins/platforms/`) | Opt-in via `plugins.enabled` — third-party gateway adapters need explicit consent. |

In short: **bundled "always-works" infrastructure loads automatically; third-party general plugins are opt-in.** The `plugins.enabled` allow-list is the gate specifically for arbitrary code a user drops into `~/.sarthi/plugins/`.

### Approval transports

An approval transport changes **where a human sees and answers** an existing
Sarthi tool-approval request. It does not decide whether a command needs
...
`present` may be synchronous or async. Sarthi runs it on a bounded worker and
...
`transport_fallback: builtin`. Without that exact opt-in, Sarthi never
materializes the prompt on another surface.

Sarthi still owns hardline blocks, sudo-stdin protection, user deny rules,
...
When you upgrade to a version of Sarthi that has opt-in plugins (config schema v21+), any user plugins already installed under `~/.sarthi/plugins/` that weren't already in `plugins.disabled` are **automatically grandfathered** into `plugins.enabled`. Your existing setup keeps working. Bundled standalone plugins are NOT grandfathered — even existing users have to opt in explicitly. (Bundled platform/backend plugins never needed grandfathering because they were never gated.)

## Available hooks

Plugins can register the 26 lifecycle events currently accepted by `sarthi_cli.plugins.VALID_HOOKS`. The **[Event Hooks catalog](/user-guide/features/hooks#shipped-plugin-hook-catalog)** is canonical for exact timing, return handling, payload fields, and privacy notes.
...
Sarthi has four kinds of plugins:

| Type | What it does | Selection | Location |
|------|-------------|-----------|----------|
| **General plugins** | Add tools, hooks, slash commands, CLI commands | Multi-select (enable/disable) | `~/.sarthi/plugins/` |
...
The table above shows the four plugin categories, but within "General plugins" the `PluginContext` exposes several distinct extension points — and Sarthi also accepts extensions outside the Python plugin system (config-driven backends, shell-hooked commands, external servers, etc.). Use this table to find the right doc for what you want to build:

| Want to add… | How | Authoring guide |
|---|---|---|
| A **tool** the LLM can call | Python plugin — `ctx.register_tool()` | [Build a Sarthi Plugin](/developer-guide/plugins) · [Adding Tools](/developer-guide/adding-tools) |
| A **lifecycle hook** (pre/post LLM, session start/end, tool filter) | Python plugin — `ctx.register_hook()` | [Hooks reference](/user-guide/features/hooks) · [Build a Sarthi Plugin](/developer-guide/plugins) |
| A **slash command** for the CLI / gateway | Python plugin — `ctx.register_command()` | [Build a Sarthi Plugin](/developer-guide/plugins) · [Extending the CLI](/developer-guide/extending-the-cli) |
| A **subcommand** for `sarthi <thing>` | Python plugin — `ctx.register_cli_command()` | [Extending the CLI](/developer-guide/extending-the-cli) |
...
| An **STT backend** (any CLI — whisper.cpp, custom whisper binary, local ASR CLI) | Config-driven (recommended) — declare under `stt.providers.<name>` with `type: command` in `config.yaml`, or set `SARTHI_LOCAL_STT_COMMAND` for the legacy single-command escape hatch. OR Python backend plugin — `ctx.register_transcription_provider()` for Python-SDK engines (OpenRouter, SenseAudio, Gemini-STT, etc.). | [STT Setup](/user-guide/features/tts#stt-custom-command-providers) · [Python plugin guide](/user-guide/features/tts#python-plugin-providers-stt) |
| **External tools via MCP** (filesystem, GitHub, Linear, Notion, any MCP server) | Config-driven — declare `mcp_servers.<name>` with `command:` / `url:` in `config.yaml`. Sarthi auto-discovers the server's tools and registers them alongside built-ins. | [MCP](/user-guide/features/mcp) |
| **Additional skill sources** (custom GitHub repos, private skill indexes) | CLI — `sarthi skills tap add <repo>` | [Skills Hub](/user-guide/features/skills#skills-hub) · [Publishing a custom tap](/user-guide/features/skills#publishing-a-custom-skill-tap) |
| **Gateway event hooks** (fire on `gateway:startup`, `session:start`, `agent:end`, `command:*`) | Drop `HOOK.yaml` + `handler.py` into `~/.sarthi/hooks/<name>/` | [Event Hooks](/user-guide/features/hooks#gateway-event-hooks) |
...
On NixOS, plugins can be installed declaratively via the module options — no `sarthi plugins install` needed. See the **[Nix Setup guide](/getting-started/nix-setup#plugins)** for full details.
...
```bash
sarthi plugins                               # unified interactive UI
sarthi plugins list                          # table: enabled / disabled / not enabled
sarthi plugins search <term>                 # search the community plugin index
sarthi plugins install <name>                # install by index name (resolved to repo @ pinned ref)
sarthi plugins install user/repo             # install from Git, then prompt Enable? [y/N]
sarthi plugins install user/repo --enable    # install AND enable (no prompt)
sarthi plugins install user/repo --no-enable # install but leave disabled (no prompt)
sarthi plugins update my-plugin              # pull latest (local edits are autostashed and re-applied)
sarthi plugins remove my-plugin              # uninstall
sarthi plugins enable my-plugin              # add to allow-list
sarthi plugins disable my-plugin             # remove from allow-list + add to disabled
sarthi plugins capabilities [my-plugin]      # declared vs granted capabilities
```

### One-click install links (Desktop)

Sarthi Desktop registers the `sarthi://` URL scheme, so a website, README, or
chat message can link straight to a plugin install:

```
sarthi://plugin/install?repo=owner/repo            # main install link
sarthi://plugin/install?repo=owner/repo&enable=1   # enable the agent plugin after install
sarthi://plugin/install?repo=owner/repo&force=1    # replace an existing install
```

Clicking one opens Sarthi and shows a **confirmation dialog** — the repo id,
...
Install from Git**. Legacy `sarthi://plugin-agent/…` and
`sarthi://plugin-desktop/…` URLs route into the same dialog. In dev builds
(`npm run dev`) the scheme is `sarthi-dev://`.

Websites need no SDK — a normal anchor works:

```html
<a href="sarthi://plugin/install?repo=owner/repo&enable=1">Install in Sarthi</a>
```

MCP servers have the equivalent link form — see
[Add to Sarthi link](/reference/mcp-config-reference#add-to-sarthi-link).
...
When a plugin declares capabilities, `sarthi plugins install` (and
`sarthi plugins enable`) shows the list with one-line risk descriptions and
...
granted, `sarthi plugins update` surfaces the additions and asks again. New
...
`sarthi plugins enable <id>` interactively to grant them later.

Inspect the state at any time:

```bash
sarthi plugins capabilities             # all plugins with declared/granted capabilities
sarthi plugins capabilities my-plugin   # one plugin, declared vs granted
```
...
not a code audit, and Sarthi has not reviewed the plugin's code. Only install
...
`sarthi plugins search <term>` searches the **community plugin index** — a
static, machine-readable JSON catalog of community plugins. Matching is fuzzy
across name, description, and tags:

```bash
sarthi plugins search telegram               # fuzzy search
sarthi plugins search                        # browse the whole index
sarthi plugins search --capability platform  # filter by declared capability
sarthi plugins search media --json           # machine-readable output
sarthi plugins search --refresh              # bypass the 24h local cache
```
...
```bash
sarthi plugins install sarthi-media-studio
```
...
(`https://raw.githubusercontent.com/NousResearch/sarthi-plugin-index/main/index.json`,
overridable via `sarthi config set plugins.index_url <url>`). Fetches are
cached under `~/.sarthi/cache/plugin_index.json` for 24 hours; when the
remote is unreachable the stale cache is used, and when there is no cache at
all a bundled seed copy ships with Sarthi — so search works fully offline.

**Index entry format.** Each entry is a JSON object:

```json
{
  "name": "sarthi-media-studio",
  "description": "Generative media workspace plugin.",
  "author": "NousResearch",
  "tags": ["media", "image-gen"],
  "repo": "NousResearch/sarthi-media-studio",
  "ref": "<40-char commit SHA>",
  "subdir": null,
  "homepage": "https://github.com/NousResearch/sarthi-media-studio",
...
(`sarthi_cli/data/plugin_index.json` in the repo) is the format reference.

**Submitting a plugin.** The index is maintained as a plain JSON file —
submit a pull request to the
[sarthi-plugin-index](https://github.com/NousResearch/sarthi-plugin-index)
...
A **plugin pack** is a declarative, shareable YAML file (`sarthi-pack.yaml`)
...
version: 1.0.0
plugins:
  - name: sarthi-media-studio            # bare community-index name…
...
```bash
sarthi plugins pack show ./sarthi-pack.yaml     # dry-run review
sarthi plugins pack install ./sarthi-pack.yaml  # review → confirm → install
sarthi plugins pack export > sarthi-pack.yaml   # snapshot the current install
sarthi plugins pack export --enabled-only       # only plugins.enabled
```
...
same pinned install path as `sarthi plugins install --ref <sha>` and record
...
capability-consent prompt — identical to a single `sarthi plugins install`.
...
provenance (installed via `sarthi plugins install`). Local-only plugins are
...
auto-installed — install those manually for now (`sarthi skills`). Wiring
skill-hub ids into pack install is a documented follow-up seam.

### Install-time security scanning

Every `sarthi plugins install` and `sarthi plugins update` runs a static
...
On `sarthi plugins update`, a dangerous verdict on the updated tree
...
Running `sarthi plugins` with no arguments opens a composite interactive screen:
...
The default for a newly-installed or bundled plugin is `not enabled`. `sarthi plugins list` shows all three distinct states so you can tell what's been explicitly turned off vs. what's just waiting to be enabled.
...
- Sarthi reuses that session's stored platform, chat, thread, profile, and conversation history. Plugins cannot supply a new chat route through this API.
- Sarthi rechecks the stored route against the gateway's current authorisation rules before dispatch.
- Routes that relied only on an adapter-time or upstream authorisation decision are rejected unless Sarthi can revalidate them from current core allowlists, pairing, or explicit allow-all configuration.
...
- The route and conversation are pinned while dispatch is pending. Sarthi drops the request if topic recovery changes the route or the session rotates before handling starts.
...
:::warning
Only grant gateway injection to plugins you trust. Sarthi checks this host API permission and restricts it to existing session routes, but Python plugins run in-process and this setting is not a sandbox.
:::
...
`ctx.call_mcp()` lets a plugin call a tool on one of the user's configured MCP servers — synchronously, from any hook or tool handler — routing through Sarthi' existing native MCP client (same connections, trust-tier gates, circuit breaker, and reconnect logic as model-invoked MCP tools; never a parallel client).
