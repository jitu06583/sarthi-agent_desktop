...
Sarthi can show an animated **pet** — a small mascot sprite that reacts to what
...
- Pets are installed into your profile's `pets/` directory
  (`<SARTHI_HOME>/pets/<slug>/`), so each [profile](../profiles.md) keeps its
  own set.
...
In the terminal (CLI/TUI), Sarthi renders the sprite at full fidelity when your
...
sarthi pets list
sarthi pets list cat

# Install a pet and make it active in one step
sarthi pets install boba --select
...
## `sarthi pets` commands

| Goal | Command |
| --- | --- |
| Browse the gallery | `sarthi pets list [query] [--limit N]` |
| List installed pets | `sarthi pets list --installed` |
| Install a pet | `sarthi pets install <slug> [--select] [--force]` |
| Set the active pet | `sarthi pets select [slug]` (omit slug for a picker) |
| Resize the pet everywhere | `sarthi pets scale <factor>` (e.g. `0.5`, clamped 0.1–3.0) |
| Preview/animate | `sarthi pets show [slug] [--state <s>] [--cycle] [--once] [--mode <m>] [--scale <f>]` |
| Disable the pet | `sarthi pets off` |
| Remove an installed pet | `sarthi pets remove <slug>` |
| Diagnose setup | `sarthi pets doctor` |

`sarthi pets show` flags:
...
Beyond installing pre-made pets from the gallery, Sarthi can **generate a brand-new pet** from a text description — its own AI sprite-generation pipeline.

- CLI/TUI: `/hatch <description>` (alias `/generate-pet`), or `sarthi pets` → the generate flow.
...
- If no reference-capable backend is configured, generation surfaces an actionable error pointing you to `sarthi tools` → Image Generation. (Installing/adopting existing gallery pets needs no image backend.)
- Override the backend with the `SARTHI_PET_IMAGE_PROVIDER` env var (e.g. `SARTHI_PET_IMAGE_PROVIDER=openrouter`).
...
always-on-top desktop window. Out there it stays visible while Sarthi is
...
Run `sarthi pets doctor` — it reports:
...
- The petdex npm CLI installs to `~/.codex/pets`; Sarthi uses its own
  profile-scoped `<SARTHI_HOME>/pets/` instead — install through `sarthi pets`.

## See also

- The [`sarthi-agent` skill](../skills/bundled/autonomous-ai-agents/autonomous-ai-agents-sarthi-agent.md)
