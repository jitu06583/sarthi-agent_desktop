...
description: "Customize Sarthi Agent's personality with a global SOUL.md, built-in personalities, and custom persona definitions"
---

# Personality & SOUL.md

Sarthi Agent's personality is fully customizable. `SOUL.md` is the **primary identity** — it's the first thing in the system prompt and defines who the agent is.

- `SOUL.md` — a durable persona file that lives in `SARTHI_HOME` and serves as the agent's identity (slot #1 in the system prompt)
- built-in or custom `/personality` presets — session-level system-prompt overlays

If you want to change who Sarthi is — or replace it with an entirely different agent persona — edit `SOUL.md`.

## How SOUL.md works now

Sarthi now seeds a default `SOUL.md` automatically in:

```text
~/.sarthi/SOUL.md
```

More precisely, it uses the current instance's `SARTHI_HOME`, so if you run Sarthi with a custom home directory, it will use:
...
- Sarthi creates a starter `SOUL.md` automatically if one does not exist yet
- Existing user `SOUL.md` files are never overwritten
- Sarthi loads `SOUL.md` only from `SARTHI_HOME`
- Sarthi does not look in the current working directory for `SOUL.md`
- If `SOUL.md` exists but is empty, or cannot be loaded, Sarthi falls back to a built-in default identity
...
If Sarthi loaded `SOUL.md` from whatever directory you happened to launch it in, your personality could change unexpectedly between projects. By loading only from `SARTHI_HOME`, the personality belongs to the Sarthi instance itself.

That also makes it easier to teach users:
- "Edit `~/.sarthi/SOUL.md` to change Sarthi' default personality."
...
- how Sarthi should handle uncertainty, disagreement, or ambiguity
...
## What Sarthi injects into the prompt
...
If the file is empty, whitespace-only, or cannot be read, Sarthi falls back to a built-in default identity ("You are Sarthi Agent, an intelligent AI assistant created by Nous Research..."). This fallback also applies when `skip_context_files` is set (e.g., in subagent/delegation contexts).
...
Sarthi ships with built-in personalities you can switch to with `/personality`.
...
| **pirate** | Captain Sarthi, tech-savvy buccaneer |
...
These are convenient overlays, but your global `SOUL.md` still gives Sarthi its persistent default personality unless the overlay meaningfully changes it.

## Custom personalities in config

Built-in personalities are always available on every surface (CLI, messaging platforms, TUI, and the desktop app). You can add your own — or override a built-in by reusing its name — in `~/.sarthi/config.yaml` under `agent.personalities`.
...
1. Keep a thoughtful global `SOUL.md` in `~/.sarthi/SOUL.md`
...
- [SOUL.md Guide](/guides/use-soul-with-sarthi)

## CLI appearance vs conversational personality

Conversational personality and CLI appearance are separate:

- `SOUL.md`, `agent.system_prompt`, and `/personality` affect how Sarthi speaks
- `display.skin` and `/skin` affect how Sarthi looks in the terminal
