...
```yaml
web:
  use_gateway: false   # Sarthi now uses FIRECRAWL_API_KEY from .env
```

`sarthi tools` automatically clears the flag when you pick a non-gateway provider, so this usually happens for you.
...
No — keep them in `.env`. When `use_gateway: true`, Sarthi skips direct keys and uses the gateway. Flip the flag back to `false` and your keys become the source again. The gateway isn't a lock-in.
