...
1. Run `sarthi tools`, pick `🖱️  Computer Use (macOS/Windows/Linux)`.
...
| Platform | Prereqs |
|---|---|
| **macOS** | System Settings → Privacy & Security → **Accessibility** + **Screen Recording** → allow your terminal (or Sarthi app). `sarthi computer-use doctor` will tell you which permission is missing. |
...
agent is acting here." Each Sarthi run declares its own cua-driver
...
Sarthi intentionally keeps its skill (`skills/computer-use/SKILL.md`)
focused on the Sarthi-side `computer_use` action vocabulary — the
...
- **Sarthi spawns its own `cua-driver mcp` child over stdio** — it does
...
- **Sarthi-side skill** — `skills/computer-use/SKILL.md` — teaches the
...
- **cua-driver skill pack** — for platform-specific deep dives
  ...
  install` autodetects Sarthi (planned follow-up), this happens
