...
Sarthi Agent can drive your desktop — clicking, typing, scrolling,
...
The built-in `computer_use` toolset is the recommended Sarthi integration. It
...
**Fresh installs already have the driver.** The Sarthi installer
...
- **`sarthi tools`** → pick `🖱️  Computer Use` — installs the driver
...
```
sarthi computer-use install
```

This fetches and runs the upstream cua-driver installer — `install.sh`
on macOS/Linux, `install.ps1` on Windows. Use `sarthi computer-use
status` to verify the install.

Already have cua-driver? Sarthi reuses it when it supports the 0.20 runtime
contract. During setup, toolset enablement, `sarthi update`, and the first
`computer_use` call of a session, Sarthi checks the local version and
...
selected with `SARTHI_CUA_DRIVER_CMD` stays
under your control, so Sarthi reports the incompatibility and leaves it
unchanged.

If you install Cua Driver first, `cua-driver skills install` installs Cua's
skill pack under `~/.cua-driver/skills/cua-driver`. Sarthi autodetection is a
planned cua-driver follow-up, so currently point Sarthi at that directory or
...
interface. The built-in toolset provides Sarthi actions, configuration,
...
| Platform | Prereqs |
|---|---|
| **macOS** | System Settings → Privacy & Security → **Accessibility** + **Screen Recording**. Grant the identity named by `sarthi computer-use doctor`. Standard mode uses CuaDriver.app; bounded and unrestricted modes use the Sarthi host identity. |
...
```
sarthi -t computer_use chat
```

or add `computer_use` to your enabled toolsets in `~/.sarthi/config.yaml`.

## Permission modes and logged-in browser profiles

Sarthi maps its existing approval UX onto cua-driver's immutable runtime
...
| Sarthi session | cua-driver mode | Human intervention | `existing_profile` |
|---|---|---|---|
| Manual or smart approvals (default) | `standard` | Normal Sarthi approvals; Cua stops at its protected boundary | Refuses unless `computer_use.grant_existing_profile: true` (one-time config opt-in) |
...
| `--yolo`, `/yolo`, or `approvals.mode: off` | private `unrestricted` daemon | One explicit Sarthi risk acceptance; no runtime Cua prompts | Refuses unless `computer_use.grant_existing_profile: true`; YOLO does not substitute for this grant |
...
Sarthi then launches the cua-driver runtime with the trusted-launcher grant
...
computer_use:
  ...
  capability_manifest: ~/.sarthi/cua-manifest.yaml
```
...
for the format). Sarthi launches a private runtime with
...
resetting or closing the Sarthi session, cancellation cleanup, or process exit
closes that transport session. Sarthi also stops private runtimes that it
launched for bounded, unrestricted, or existing-profile access. One Sarthi
...
embedded service under the Sarthi host identity.
...
## `sarthi computer-use doctor` — your first triage stop

`sarthi computer-use doctor` runs cua-driver's structured
`health_report` MCP tool and prints a per-check matrix. It's the single
fastest way to find out *why* an action isn't working.

```
$ sarthi computer-use doctor
...
Sarthi run declares a public cua-driver **session name** (something like
`sarthi-3a7b9c14d2e8`). The name labels cursor identity and related state, so
...
Sarthi keeps its wrapper skill (`skills/autonomous-ai-agents/computer-use/SKILL.md`)
focused on the Sarthi-side `computer_use` workflow and action vocabulary. For
...
The command installs the pack under `~/.cua-driver/skills/cua-driver`. Sarthi
autodetection is a planned cua-driver follow-up, so currently point Sarthi at
...
These are **platform deep dives, not duplicates of the Sarthi skill** —
...
Code, Codex, OpenCode, OpenClaw, and Antigravity; **Sarthi
...
image capture also saves a bounded, shareable copy under Sarthi' image cache
...
Sarthi applies multi-layer guardrails:
...
Pair with `approvals.mode: manual` in `~/.sarthi/config.yaml` if you
want every action confirmed.

## Token efficiency

Screenshots are expensive. Sarthi applies four layers of optimisation:
...
- **Windows: elevated (admin) windows can't be driven from a normal
  ...
  Sarthi agent) cannot enumerate the UIA tree of, or inject mouse input
  ...
  run the Sarthi agent itself at High integrity (launch from an
...
- **Platform-specific deployment gotchas:**
  - **macOS** uses private SkyLight SPIs. Apple can change them in any
    OS update. Sarthi warns when the installed cua-driver is older than
    the version it was tested against.
  - **Windows** SSH sessions run in **Session 0**, which has no
    interactive desktop. Drive Sarthi from inside the RDP / console
...
```
SARTHI_CUA_DRIVER_CMD=/path/to/your/cua-driver
```

Swap the backend entirely (for testing):

```
SARTHI_COMPUTER_USE_BACKEND=noop   # records calls, no side effects
```

### Telemetry

cua-driver ships with anonymous usage telemetry (PostHog) enabled by default
upstream. **Sarthi disables it for you** — on every cua-driver invocation
(the MCP backend, `status`, `doctor`, and install) Sarthi sets
...
When it's on, `sarthi computer-use doctor` reports `telemetry: enabled`;
...
unreleased fix — point Sarthi at a binary you built from source instead
of the published release. Sarthi resolves the driver with
`shutil.which("cua-driver")` and **does not enforce
`SARTHI_CUA_DRIVER_VERSION`**, so a local build (reported as
...
- `-NoAutoStart` skips registering the `cua-driver-serve` logon daemon
  — you don't need it for Sarthi testing (see notes).
...
### Option B — point Sarthi straight at the built binary (fastest loop)

Skip the install ceremony entirely: `cargo build` and set
`SARTHI_CUA_DRIVER_CMD` to the resulting binary. Best for rapid
edit/build/test.
...
```
# Windows (.env)
SARTHI_CUA_DRIVER_CMD=C:\path\to\cua\libs\cua-driver\rust\target\debug\cua-driver.exe
# macOS / Linux (.env)
SARTHI_CUA_DRIVER_CMD=/path/to/cua/libs/cua-driver/rust/target/debug/cua-driver
```

### Confirm Sarthi is using your build

- `sarthi computer-use status` prints the resolved binary path and
  version.
- `sarthi computer-use doctor` confirms the binary is reachable and
...
- **Sarthi spawns a `cua-driver mcp` stdio proxy.** In a normal session the
  proxy connects to (and may start) the standard machine daemon. In explicit
  Sarthi YOLO, Sarthi instead owns a private `cua-driver serve --embedded`
...
- **Rebuild loop.** After editing cua-driver source, re-run
  ...
  for Option A, or just re-`cargo build` for Option B — no Sarthi
  change needed either way.
- **Local builds skip the version check.** Sarthi warns when the
...
**First action when anything's off: run `sarthi computer-use doctor`.**
...
Run `sarthi computer-use install` to fetch the cua-driver binary, or
run `sarthi tools` and enable the Computer Use toolset.
...
Wayland without an XWayland bridge. `sarthi computer-use doctor` will
...
- **Sarthi-side skill** — `skills/autonomous-ai-agents/computer-use/SKILL.md` — teaches the
  Sarthi `computer_use` action vocabulary; this is what the agent loads.
- **cua-driver skill pack** — for platform-specific deep dives
  ...
  `LINUX.md` / `RECORDING.md` / `WEB_APPS.md`. Sarthi autodetection is a
  planned follow-up; currently point Sarthi at the installed pack directory
...
- **cua.ai/docs** — the cua-driver project's documentation:
  ...
  - [Connect your agent](https://cua.ai/docs/how-to-guides/driver/connect-your-agent) — register cua-driver with various harnesses (Sarthi among them)
