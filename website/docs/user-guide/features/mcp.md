...
Manifests live at
[`optional-mcps/<name>/manifest.yaml`](https://github.com/jitendra-singh-thakur/sarthi-agent/tree/main/optional-mcps)
...
[`optional-mcps/`](https://github.com/jitendra-singh-thakur/sarthi-agent/tree/main/optional-mcps).
...
Most hosted MCP servers (Linear, Sentry, Atlassian, Asana, Figma, Stripe, …) require OAuth 2.1 instead of a static bearer token. Set `auth: oauth` and Sarthi handles discovery, dynamic client registration, PKCE, token exchange, refresh, and step-up auth via the MCP Python SDK.
...
The MCP server reads conversation data directly from Sarthi's session store (`~/.sarthi/sessions/sessions.json` and the SQLite database). A background thread polls the database for new messages and maintains an in-memory event queue. For sending messages, it uses the same internal send engine (`tools/send_message_tool.py`) that powers cron delivery and the `sarthi send` CLI.
