...
description: "Plugins shipped with Sarthi Agent that run automatically via lifecycle hooks — disk-cleanup and friends"
---

# Built-in Plugins

Sarthi ships a small set of plugins bundled with the repository. They live under `<repo>/plugins/<name>/` and load automatically alongside user-installed plugins in `~/.sarthi/plugins/`. They use the same plugin surface as third-party plugins — hooks, tools, slash commands — just maintained in-tree.

See the [Plugins](/user-guide/features/plugins) page for the general plugin system, and [Build a Sarthi Plugin](/developer-guide/plugins) to write your own.
...
2. **User** — `~/.sarthi/plugins/<name>/`
3. **Project** — `./.sarthi/plugins/<name>/` (requires `SARTHI_ENABLE_PROJECT_PLUGINS=1`)
4. **Pip entry points** — `sarthi_agent.plugins`

On name collision, later sources win — a user plugin named `disk-cleanup` would replace the bundled one.

`plugins/memory/` and `plugins/context_engine/` are deliberately excluded from bundled scanning. Those directories use their own discovery paths because memory providers and context engines are single-select providers configured through `sarthi memory setup` / `context.engine` in config.

## Bundled plugins are opt-in

Bundled plugins ship disabled. Discovery finds them (they appear in `sarthi plugins list` and the interactive `sarthi plugins` UI), but none load until you explicitly enable them:

```bash
sarthi plugins enable disk-cleanup
```

Or via `~/.sarthi/config.yaml`:
...
This is the same mechanism user-installed plugins use. Bundled plugins are never auto-enabled — not on fresh install, not for existing users upgrading to a newer Sarthi. You always opt in explicitly.

To turn a bundled plugin off again:

```bash
sarthi plugins disable disk-cleanup
# or: remove it from plugins.enabled in config.yaml
```

## Currently shipped

The repo ships these bundled plugins under `plugins/`. All are opt-in — enable them via `sarthi plugins enable <name>`.
...
| `sarthi-achievements` | dashboard tab | Steam-style collectible badges generated from your real Sarthi session history |
...
Memory providers (`plugins/memory/*`) and context engines (`plugins/context_engine/*`) are listed separately on [Memory Providers](./memory-providers.md) — they're managed through `sarthi memory` and `sarthi plugins` respectively. The full per-plugin detail for the two long-running hooks-based plugins follows.
...
| Hook | Behaviour |
|---|---|
| `post_tool_call` | When `write_file` / `terminal` / `patch` creates a file matching `test_*`, `tmp_*`, or `*.test.*` inside `SARTHI_HOME` or `/tmp/sarthi-*`, track it silently as `test` / `temp` / `cron-output`. |
...
| empty dirs under SARTHI_HOME | always | Never |
...
**State** — everything lives at `$SARTHI_HOME/disk-cleanup/`:
...
**Safety** — cleanup only ever touches paths under `SARTHI_HOME` or `/tmp/sarthi-*`. Windows mounts (`/mnt/c/...`) are rejected. Well-known top-level state dirs (`logs/`, `memories/`, `sessions/`, `cron/`, `cache/`, `skills/`, `plugins/`, `disk-cleanup/` itself) are never removed even when empty — a fresh install does not get gutted on first session end.

**Enabling:** `sarthi plugins enable disk-cleanup` (or check the box in `sarthi plugins`).

**Disabling again:** `sarthi plugins disable disk-cleanup`.
...
**Enabling:** `sarthi plugins enable security-guidance` (or check the box in `sarthi plugins`).

**Disabling again:** `sarthi plugins disable security-guidance`.
...
Traces Sarthi turns, LLM calls, and tool invocations to [Langfuse](https://langfuse.com) — an open-source LLM observability platform. One span per turn, one generation per API call, one tool observation per tool call. Usage totals, per-type token counts, and cost estimates come out of Sarthi' canonical `agent.usage_pricing` numbers, so the Langfuse dashboard sees the same breakdown (input / output / `cache_read_input_tokens` / `cache_creation_input_tokens` / `reasoning_tokens`) that appears in `sarthi logs`.
...
```bash
sarthi tools          # → Langfuse Observability → Cloud or Self-Hosted
```

The wizard collects your keys, `pip install`s the `langfuse` SDK, and adds `observability/langfuse` to `plugins.enabled` for you. Restart Sarthi and the next turn ships a trace.
...
Then put the credentials in `~/.sarthi/.env`:
...
| Hook | Behaviour |
|---|---|
| `pre_api_request` / `pre_llm_call` | Open (or reuse) a per-turn root span "Sarthi turn". Start a `generation` child observation for this API call with serialized recent messages as input. |
...
| `post_tool_call` | Close the tool observation with sanitized `result`. `read_file` payloads get summarized (head + tail + omitted-line count) so a huge file read stays under `SARTHI_LANGFUSE_MAX_CHARS`. |

Session grouping keys off the Sarthi session ID (or task ID for sub-agents) via `langfuse.propagate_attributes`, so everything in a single `sarthi chat` session lives under one Langfuse session.

**Verify:**

```bash
sarthi plugins list                 # observability/langfuse should show "enabled"
sarthi chat -q "hello"              # check the Langfuse UI for a "Sarthi turn" trace
```
...
| `SARTHI_LANGFUSE_RELEASE` | — | Release/version tag |
| `SARTHI_LANGFUSE_SAMPLE_RATE` | `1.0` | Sampling rate passed to the SDK (0.0–1.0) |
| `SARTHI_LANGFUSE_MAX_CHARS` | `12000` | Per-field truncation for message content / tool args / tool results |
| `SARTHI_LANGFUSE_DEBUG` | `false` | Verbose plugin logging to `agent.log` |

Sarthi-prefixed and standard SDK env vars (`LANGFUSE_PUBLIC_KEY`, `LANGFUSE_SECRET_KEY`, `LANGFUSE_BASE_URL`) are both accepted — Sarthi-prefixed wins when both are set.
...
**Disabling:** `sarthi plugins disable observability/langfuse`. The plugin module is still discovered, but no module code runs until you re-enable.

### NeMo Relay native integration (migration note)

NeMo Relay is no longer a bundled Sarthi plugin. Do not run `sarthi plugins enable observability/nemo_relay`; Sarthi core now owns the Relay session, turn, LLM, and tool lifecycles.

To opt into Relay middleware or exporters, create a standard Relay `plugins.toml`, then set `SARTHI_NEMO_RELAY_PLUGINS_TOML` to that file before starting Sarthi. The policy is process-wide for every profile hosted by that Sarthi process. See the [NeMo Relay observability configuration](https://docs.nvidia.com/nemo/relay/configure-plugins/observability/about) for ATOF, ATIF, and OpenTelemetry options.

The old `SARTHI_NEMO_RELAY_ATOF_*` and `SARTHI_NEMO_RELAY_ATIF_*` settings no longer activate exporters. `sarthi doctor` reports these stale settings when no replacement `plugins.toml` is selected.
...
Both defaults preserve one session scope for the full session. Rotated spans retain the same `session_id` and add `sarthi.session.segment` plus `sarthi.session.segment_reason` (`compaction` or `max_turns`).
...
- Post-meeting artifacts (transcript, status) saved under `~/.sarthi/workspace/meetings/<meeting_id>/`

**Setup:**

```bash
sarthi plugins enable google_meet
sarthi meet setup   # preflight: playwright, chromium, auth file
sarthi meet auth    # opens a browser to sign into Google and saves session state —
...
**Disabling:** `sarthi plugins disable google_meet`. Any saved transcripts stay in `~/.sarthi/workspace/meetings/` until you remove them.

### sarthi-achievements

Adds a **Steam-style achievements tab to the dashboard** — 60+ collectible, tiered badges generated from your real Sarthi session history. Tool-chain feats, debugging patterns, vibe-coding streaks, skill/memory usage, model/provider variety, lifestyle quirks (weekend and night sessions). Originally authored by [@PCinkusz](https://github.com/PCinkusz) as an external plugin; brought in-tree so it stays in lockstep with Sarthi feature changes.

**How it works:**

- Scans your entire `~/.sarthi/state.db` session history on the dashboard backend
...
- Unlock state is persisted to `$SARTHI_HOME/plugins/sarthi-achievements/state.json`
...
| Secret | Hidden until Sarthi detects the first related signal in your history |

**API** — routes mount under `/api/plugins/sarthi-achievements/`:
...
**State files** — live under `$SARTHI_HOME/plugins/sarthi-achievements/`:

| File | Contents |
|---|---|
| `state.json` | Unlock history: which badges you've earned and when. Stable across Sarthi updates. |
...
**Enabling:** Nothing to enable — `sarthi-achievements` is a dashboard-only plugin (no lifecycle hooks, no model-visible tools). It auto-registers as a tab in `sarthi dashboard` on first launch. The `plugins.enabled` config only gates lifecycle/tool plugins; dashboard plugins are discovered purely via their `dashboard/manifest.json`.

**Opting out:** Delete or rename `plugins/sarthi-achievements/dashboard/manifest.json`, or override it with a user plugin of the same name in `~/.sarthi/plugins/sarthi-achievements/` that ships no dashboard. The plugin's state files under `$SARTHI_HOME/plugins/sarthi-achievements/` survive — reinstalling preserves your unlock history.

## Adding a bundled plugin

Bundled plugins are written exactly like any other Sarthi plugin — see [Build a Sarthi Plugin](/developer-guide/plugins). The only differences are:

- Directory lives at `<repo>/plugins/<name>/` instead of `~/.sarthi/plugins/<name>/`
- Manifest source is reported as `bundled` in `sarthi plugins list`
