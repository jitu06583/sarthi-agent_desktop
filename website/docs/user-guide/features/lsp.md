...
Sarthi runs full language servers — pyright, gopls, rust-analyzer,
...
This is the same architecture top-tier coding agents use. Sarthi
...
1. Sarthi captures a baseline of current diagnostics for the file.
...
…). Sarthi auto-detects the binary on PATH or in
`<SARTHI_HOME>/lsp/bin/`.
...
2. Download the latest release zip from
   ...
   and extract it.
3. Point Sarthi at the extracted bundle — the directory that contains
   `PowerShellEditorServices/Start-EditorServices.ps1`. Either:
   - set `lsp.servers.powershell.command: ["/path/to/bundle"]` in
     `config.yaml`, or
   - extract it to `<SARTHI_HOME>/lsp/PowerShellEditorServices`, or
   - export `PSES_BUNDLE_PATH=/path/to/bundle`.

`sarthi lsp status` reports `installed` once `pwsh` is found; if the
...
`node_modules` tree — Sarthi installs both packages together when you
run `sarthi lsp install typescript` or auto-install fires on first
use.

## CLI

```
sarthi lsp status          # service state + per-server install status
sarthi lsp list            # registry, optionally --installed-only
sarthi lsp install <id>    # eagerly install one server
sarthi lsp install-all     # try every server with a known recipe
sarthi lsp restart         # tear down running clients
sarthi lsp which <id>      # print resolved binary path
```

`sarthi lsp status` is the best starting point — it shows which
...
lsp:
  ...
  #   auto    — install via npm/pip/go install into <SARTHI_HOME>/lsp/bin
...
When `install_strategy: auto`, Sarthi installs binaries into
`<SARTHI_HOME>/lsp/bin/`. NPM packages land in
`<SARTHI_HOME>/lsp/node_modules/` with bin symlinks one level up.
Go binaries come from `go install` with `GOBIN` pointed at the
staging dir.

Nothing is ever installed to `/usr/local/`, `~/.local/`, or any other
shared location — the staging dir is fully Sarthi-owned and is
...
Servers are kept alive for the life of the Sarthi process. There's
...
**`sarthi lsp status` shows a server as "missing"**

The binary isn't on PATH and isn't in `<SARTHI_HOME>/lsp/bin/`. Run
`sarthi lsp install <server_id>` to attempt an auto-install, or
install the binary manually through the language's normal toolchain.

**`Backend warnings` section in `sarthi lsp status`**
...
When `sarthi lsp status` shows a `Backend warnings` section, install
...
`~/.sarthi/logs/agent.log`.

**Server starts but never returns diagnostics**

Check `~/.sarthi/logs/agent.log` for `[agent.lsp.client]` entries —
...
the rest of the session. Run `sarthi lsp restart` to clear the set;
