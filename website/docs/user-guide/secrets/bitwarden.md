...
Pull API keys from [Bitwarden Secrets Manager](https://bitwarden.com/products/secrets-manager/) at process startup instead of storing them in plaintext inside `~/.sarthi/.env`. One bootstrap secret (a machine-account access token) replaces N per-provider keys, and rotating a credential becomes a single change in the Bitwarden web app.
...
2. Sarthi stores that single token in `~/.sarthi/.env` as `BWS_ACCESS_TOKEN`.
3. Every time `sarthi` (or the gateway, or a cron job) starts, after `~/.sarthi/.env` has loaded, Sarthi calls `bws secret list <project_id>` and sets the returned keys into `os.environ`.
4. By default Sarthi **overrides** values already in your environment, so Bitwarden is the source of truth — rotate a key once in the web app and every Sarthi process picks it up on next start. Flip `override_existing: false` in config if you want `.env` to win instead.

The `bws` binary is auto-downloaded into `~/.sarthi/bin/` on first use — no `apt`, no `brew`, no `sudo`.
...
2. Create or pick a **Project** (e.g. "Sarthi keys").
...
4. **Machine accounts → New machine account → My Sarthi machine** → **Projects** tab → grant Read access to your project.
...
1. Download and verify `bws v2.0.0` into `~/.sarthi/bin/bws`.
2. Prompt you for the access token (input is hidden). Stored in `~/.sarthi/.env` as `BWS_ACCESS_TOKEN`.
...
```bash
sarthi secrets bitwarden status
```

From now on, every `sarthi` invocation pulls fresh secrets at startup. You'll see a one-line summary in stderr the first time secrets are applied in a process.

## CLI

| Command | What it does |
|---|---|
| `sarthi secrets bitwarden setup` | Interactive wizard (install binary, prompt for token, pick project, test fetch) |
| `sarthi secrets bitwarden status` | Show config + binary version + token presence/validation |
| `sarthi secrets bitwarden token` | Rotate the access token: validate the new token against Bitwarden, then store it in `.env` |
| `sarthi secrets bitwarden sync` | Dry-run: pull secrets now and show what would be applied |
| `sarthi secrets bitwarden sync --apply` | Pull and export into the current shell's environment |
| `sarthi secrets bitwarden install` | Just download the pinned `bws` binary (no auth required) |
| `sarthi secrets bitwarden disable` | Flip `enabled: false`; leaves token + project id in place |
...
Bitwarden Secrets Manager: → Run `sarthi secrets bitwarden token` to paste a fresh access token ...
```

Fix it without re-running the whole wizard:

```bash
sarthi secrets bitwarden token                     # masked prompt
sarthi secrets bitwarden token --access-token 0.…  # non-interactive
```
...
| `encrypted_cache.enabled` | `false` | Store the last successful fetch in an AES-GCM encrypted cache at `~/.sarthi/cache/bws_cache.enc.json`. |
...
| `auto_install` | `true` | When true, `bws` is auto-downloaded into `~/.sarthi/bin/` on first use. |

## Failure modes

Bitwarden never blocks Sarthi startup. If anything goes wrong, you'll see a one-line warning in stderr and Sarthi continues with whatever credentials `.env` already had:

| Symptom | Cause | Fix |
|---|---|---|
| `BWS_ACCESS_TOKEN is not set` | Enabled in config but token cleared from `.env` | Re-run `sarthi secrets bitwarden setup` |
| `Bitwarden rejected the machine-account access token … invalid_client` | Token revoked, expired, machine account deleted — or the token belongs to another region (e.g. EU token hitting the US identity endpoint) | Run `sarthi secrets bitwarden token` to paste a fresh token; for region mismatches re-run setup and pick EU/self-hosted (or set `secrets.bitwarden.server_url`) |
| `bws exited 1: invalid access token` | Token revoked or wrong | Run `sarthi secrets bitwarden token` with a new token |
...
- Sarthi will refuse to let Bitwarden overwrite the bootstrap token itself, even with `override_existing: true`. If you store `BWS_ACCESS_TOKEN` as a secret inside the project, it's silently skipped during apply.
...
- The pinned version (`bws v2.0.0` at time of writing) is updated through PRs to this repo — Sarthi does not auto-upgrade `bws` to "latest" because upstream release shapes can change.

## When NOT to use this

- **Single-machine personal setups** where `~/.sarthi/.env` is fine. You're trading one credential for another and adding a network dependency at startup.
...
The good case for this is multi-machine fleets, shared dev boxes, gateway VPSes, or any setup where you want centralized rotation and revocation across multiple Sarthi installations.
