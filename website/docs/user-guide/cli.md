...
sidebar_position: 1
title: "CLI Interface"
description: "Master the Sarthi Agent terminal interface — commands, keybindings, personalities, and more"
---

# CLI Interface

Sarthi Agent's CLI is a full terminal user interface (TUI) — not a web UI. It features multiline editing, slash-command autocomplete, conversation history, interrupt-and-redirect, and streaming tool output. Built for people who live in the terminal.

:::tip First-time setup
One command — `sarthi setup --portal` — and you're ready to `sarthi chat`. See [Nous Portal](/integrations/nous-portal).
:::

:::tip
Sarthi also ships a modern TUI with modal overlays, mouse selection, and non-blocking input. Launch it with `sarthi --tui` — see the [TUI](tui.md) guide.
:::
...
sarthi chat --query-file prompt.txt
sarthi chat --query-file - < prompt.txt

# With a specific model
sarthi chat --model "anthropic/claude-sonnet-4"

# With a specific provider
sarthi chat --provider nous        # Use Nous Portal
sarthi chat --provider openrouter  # Force OpenRouter

# With specific toolsets
sarthi chat --toolsets "web,terminal,skills"

# Start with one or more skills preloaded
sarthi -s sarthi-agent-dev,github-auth
sarthi chat -s github-pr-workflow -q "open a draft PR"

# Resume previous sessions
sarthi --continue             # Resume the most recent CLI session (-c)
sarthi --resume <session_id>  # Resume a specific session by ID (-r)
sarthi --resume latest        # Resume the most recent session (same as -c)
sarthi --resume latest --in ./dir  # Resume ./dir's latest session, staying in ./dir

# Verbose mode (debug output)
sarthi chat --verbose

# Isolated git worktree (for running multiple agents in parallel)
sarthi -w                         # Interactive mode in worktree
sarthi -w -z "Fix issue #123"     # Single query in worktree
```

### Worktree cleanup

`sarthi -w` sessions create disposable worktrees under `<repo>/.worktrees/`.
...
```bash
sarthi worktree list              # audit: age, size, verdict, reason per tree
sarthi worktree prune             # remove safe trees + delete merged branches
sarthi worktree prune --dry-run   # show the plan without changing anything
sarthi worktree prune --trees-only     # leave local branches alone
sarthi worktree prune --branches-only  # leave worktrees alone
```
...
- Trees **in use by a running sarthi session** are never touched.
- **Untracked-only scratch** (PR body drafts, notes) is archived to
  `~/.sarthi/archive/worktree-prune/` before its tree is removed — never
  destroyed.
...
The `sarthi plugins` commands manage native Sarthi plugins and portable Agent
...
sarthi plugins disable <plugin-name>
sarthi plugins update <plugin-name>
sarthi plugins remove <plugin-name>
```

Portable packages remain disabled until explicitly enabled. Sarthi currently
...
<img className="docs-terminal-figure" src="/docs/img/docs/cli-layout.svg" alt="Stylized preview of the Sarthi CLI layout showing the banner, conversation area, and fixed input prompt." />
<p className="docs-figure-caption">The Sarthi CLI banner, conversation stream, and fixed input prompt rendered as a stable docs figure instead of fragile text art.</p>
...
| ⚠ YOLO | **YOLO mode warning** — shown whenever `SARTHI_YOLO_MODE` is on (either `sarthi --yolo` at launch or `/yolo` toggled mid-session). Mirrors the banner-line warning so you can't forget you're in auto-approve mode. |
...
| `Ctrl+D` | Exit |
| `Ctrl+Z` | Suspend Sarthi to background (Unix only). Run `fg` in the shell to resume. |
...
- **Approvals still apply.** A dangerous command (`rm -rf`, writes to `~/.sarthi/config.yaml`, etc.) goes through the same approval prompt the agent's `terminal` tool uses. `!` is a cost/latency shortcut, not a security bypass.
...
Type `/` to see the autocomplete dropdown. Sarthi supports a large set of CLI slash commands, dynamic skill commands, and user-defined quick commands.
...
| `/voice tts` | Toggle spoken playback for Sarthi replies |
...
```bash
sarthi -s sarthi-agent-dev,github-auth
sarthi chat -s github-pr-workflow -s github-auth
```

Sarthi loads each named skill into the session prompt before the first turn. The same flag works in interactive mode and single-query mode.

## Skill Slash Commands

Every installed skill in `~/.sarthi/skills/` is automatically registered as a slash command. The skill name becomes the command:
...
You can also define custom personalities in `~/.sarthi/config.yaml`:

```yaml
personalities:
  helpful: "You are a helpful, friendly AI assistant."
  kawaii: "You are a kawaii assistant! Use cute expressions..."
  pirate: "Arrr! Ye be talkin' to Captain Sarthi..."
  # Add your own!
```
...
`Ctrl+J` and backslash continuation are enabled by default, matching Claude Code / Codex / OpenCode multiline shortcuts. On supported terminals such as iTerm2, Sarthi also requests extended key reporting so `Shift+Enter` arrives as a distinct newline key. If your terminal sends LF for plain `Enter` and you need the legacy `Ctrl+J`-as-submit fallback, opt out:
...
Most terminals send the same byte sequence for `Enter` and `Shift+Enter` by default, so applications cannot distinguish them. Sarthi recognises `Shift+Enter` only when the terminal sends a distinct sequence via the [Kitty keyboard protocol](https://sw.kovidgoyal.net/kitty/keyboard-protocol/) or xterm's `modifyOtherKeys` mode.
...
Where the terminal cannot distinguish them, `Alt+Enter` and `Ctrl+J` continue to work by default. **On Windows Terminal specifically, `Alt+Enter` is captured by the terminal (toggles fullscreen) and never reaches Sarthi — use `Ctrl+Enter` (delivered as `Ctrl+J`) or `Ctrl+J` directly for a newline.**
...
The first time you press Enter while Sarthi is working, Sarthi prints a one-line reminder explaining the `/busy` knob. It only fires once per install; `onboarding.seen.busy_input_prompt` in `config.yaml` records that it was shown. Delete that key to see the tip again.
:::

### Suspending to Background

On Unix systems, press **`Ctrl+Z`** to suspend Sarthi to the background — just like any terminal process. The shell prints a confirmation:

```
Sarthi Agent has been suspended. Run `fg` to bring Sarthi Agent back.
```
...
Resume this session with:
  sarthi --resume 20260225_143052_a1b2c3
...
```bash
sarthi --continue                          # Resume the most recent CLI session
sarthi -c                                  # Short form
sarthi -c "my project"                     # Resume a named session (latest in lineage)
sarthi --resume 20260225_143052_a1b2c3     # Resume a specific session by ID
sarthi --resume "refactoring auth"         # Resume by title
sarthi --resume latest                     # Resume the most recent session (same as -c)
sarthi --resume latest --in ./my-project   # Latest session for ./my-project's workspace
sarthi -r 20260225_143052_a1b2c3           # Short form
```

Resuming restores the full conversation history from SQLite. The agent sees all previous messages, tool calls, and responses — just as if you never left.

Use `/title My Session Name` inside a chat to name the current session, or `sarthi sessions rename <id> <title>` from the command line. Use `sarthi sessions list` to browse past sessions.

### Session Storage

CLI sessions are stored in Sarthi's SQLite state database under `~/.sarthi/state.db`. The database keeps:
...
Sarthi immediately confirms the task and gives you back the prompt:
