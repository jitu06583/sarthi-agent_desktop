...
| List/filter by metadata (age, source, cost, tokens, workspace) | `sarthi sessions list` / `stats` via terminal |
| Rename | `sarthi sessions rename <session_id> <title...>` |
| Bulk soft-hide (reversible) | `sarthi sessions archive <filters>` |
| Delete (destructive) | `sarthi sessions delete` / `sarthi sessions prune <filters>` |
| Export before deleting anything valuable | `sarthi sessions export --session-id <id> --format md` |
...
`sarthi sessions list --source telegram --limit 50` instead.
...
- Before deleting anything with meaningful content, offer
  `sarthi sessions export --format md` as a backup.
...
After a cleanup pass, re-run the discovery query and `sarthi sessions list`
