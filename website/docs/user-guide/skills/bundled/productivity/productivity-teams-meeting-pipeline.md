...
Operate the Teams meeting summary pipeline via Sarthi CLI — summarize meetings, inspect pipeline status, replay jobs, manage Microsoft Graph subscriptions.
...
Everything operator-facing is a `sarthi teams-pipeline` subcommand run via the terminal tool. There are no new model tools for this pipeline — the CLI is the surface.
...
Before using the pipeline, verify these are set in `${SARTHI_HOME:-~/.sarthi}/.env`:
...
```bash
sarthi teams-pipeline validate              # config snapshot — run first after any change
sarthi teams-pipeline token-health          # Graph token status
sarthi teams-pipeline token-health --force-refresh   # force a fresh token acquisition
sarthi teams-pipeline list                  # recent meeting jobs
sarthi teams-pipeline list --status failed  # only failed jobs
sarthi teams-pipeline show <job-id>         # full detail of one job
sarthi teams-pipeline subscriptions         # current Graph webhook subscriptions
```
...
```bash
sarthi teams-pipeline subscribe \
  --resource communications/onlineMeetings/getAllTranscripts \
  --notification-url https://<your-public-host>/msgraph/webhook \
  --client-state "$MSGRAPH_WEBHOOK_CLIENT_STATE"

sarthi teams-pipeline renew-subscription <sub-id> --expiration <iso-8601>
sarthi teams-pipeline delete-subscription <sub-id>
sarthi teams-pipeline maintain-subscriptions            # renew near-expiry ones
sarthi teams-pipeline maintain-subscriptions --dry-run  # show what would be renewed
```
...
1. Run `sarthi teams-pipeline subscriptions` — if it's empty or all entries show `expirationDateTime` in the past, that's the cause.
2. Recreate with `subscribe` as shown above.
3. **Set up automated renewal immediately** via `sarthi cron add`, a systemd timer, or plain crontab. The operator runbook at `/docs/guides/operate-teams-meeting-pipeline#automating-subscription-renewal-required-for-production` has all three options. 12-hour interval is safe (6x headroom against the 72h limit).
