...
description: "Use when a Sarthi Kanban worker wants to run Codex CLI as an isolated implementation lane while Sarthi keeps ownership of task lifecycle, reconciliation, tes..."
---
...
Use when a Sarthi Kanban worker wants to run Codex CLI as an isolated implementation lane while Sarthi keeps ownership of task lifecycle, reconciliation, testing, and handoff.
...
| Related skills | [`codex`](/docs/user-guide/skills/bundled/autonomous-ai-agents/autonomous-ai-agents-codex), [`sarthi-agent`](/docs/user-guide/skills/bundled/autonomous-ai-agents/autonomous-ai-agents-sarthi-agent) |
...
This skill defines the lightweight Sarthi+Codex dual-lane convention for Kanban workers. Sarthi is always the task owner: it calls `kanban_show`, decides whether Codex is appropriate, creates or selects an isolated workspace, starts and monitors Codex, reconciles any diff, runs verification, and writes the final `kanban_complete` or `kanban_block` handoff. Codex is an input lane only. Codex output is not a task completion signal, not a trusted reviewer, and not allowed to write durable Kanban state directly.

The convention exists so a Sarthi worker can use Codex for bounded implementation help without changing the dispatcher. The dispatcher must still spawn Sarthi workers. A worker may optionally spawn Codex inside its own run, then accept, partially accept, or reject the lane after independent review and tests.
...
- A bounded diff can be evaluated by Sarthi in one run.
- The repo can be copied or checked out in an isolated git worktree/branch.
- Sarthi can run the relevant tests itself after Codex exits.
...
1. Sarthi owns the Kanban lifecycle. Codex must never call `kanban_complete`, `kanban_block`, `kanban_create`, gateway messaging, or any Sarthi board CLI as a substitute for the worker.
2. Sarthi owns final acceptance. Treat Codex commits/diffs as untrusted patches until reviewed and verified.
3. Sarthi owns test execution. Codex may run tests, but those runs are advisory; repeat required verification from Sarthi with the repo's canonical wrapper.
4. Sarthi owns safety. If Codex changes safety boundaries, risk gates, live trading behavior, or secrets handling, reject the lane even if tests pass.
5. Sarthi owns cleanup. Kill stuck Codex processes and remove temporary worktrees when they are no longer needed.
...
```bash
TASK_ID="${SARTHI_KANBAN_TASK:-t_manual}"
REPO="/path/to/repo"
...
If the current Kanban workspace is already an isolated git worktree created for this task, you may create a sibling Codex branch inside it only if `git status --short` is clean except for intentional Sarthi edits. Otherwise create a separate temporary worktree and cherry-pick or copy accepted commits back after reconciliation.
...
Run these before spawning Codex. Missing Codex is a normal reason to skip the lane, not a task blocker if Sarthi can do the task directly.
...
Sarthi owns the Kanban lifecycle; do not call Sarthi kanban tools or messaging.
...
Do not use `--yolo` for prediction-market-bot or safety-sensitive repos. Prefer `--full-auto` inside the isolated worktree, then rely on Sarthi reconciliation.
...
- Explicit statement: Sarthi owns Kanban lifecycle; Codex is an input lane only.
...
- Verification commands Codex may run and commands Sarthi will run afterward.
...
Sarthi must perform this checklist before accepting any Codex lane result:

- [ ] `git -C <WORKTREE> status --short --branch` shows only expected files.
- [ ] `git -C <WORKTREE> diff --stat` and `git diff` were reviewed by Sarthi.
...
- [ ] Sarthi ran the canonical tests itself, using `scripts/run_tests.sh` for Sarthi Agent or the repo's documented wrapper for other repos.
- [ ] Any Codex-run tests are listed separately from Sarthi-run tests.
- [ ] Accepted commits/diffs were applied to the Sarthi-owned workspace/branch.
...
```json
{
  "codex_lane": {
    ...
    "tests_run": [
      {"command": "scripts/run_tests.sh tests/tools/test_x.py", "exit_code": 0, "owner": "sarthi"},
...
```json
{
  "codex_lane": {
    ...
    "result": "rejected",
    "accepted_commits": [],
    "rejected_reason": "Direct Sarthi edit was smaller and safer than spawning Codex.",
    "tests_run": [],
    "artifacts": []
  }
}
...
1. Treating Codex self-report as verification. Always inspect the diff and rerun tests from Sarthi.
2. Running Codex in the user's dirty main checkout. Always isolate in a worktree/branch.
3. Letting Codex own Kanban. Codex may summarize progress, but Sarthi writes board state.
...
- [ ] Sarthi reviewed `git diff` and safety-sensitive files.
- [ ] Sarthi ran canonical tests independently.
