...
| Version | `2.0.0` |
| Author | Francesco Bonacci (f-trycua), Sarthi Agent |
| License | MIT |
...
Sarthi drives [cua-driver](https://github.com/trycua/cua) under the hood.
This wrapper skill teaches the Sarthi `computer_use` workflow and action
...
skill installed by `cua-driver skills install`. Sarthi autodetection is a
planned cua-driver follow-up, so currently point Sarthi at the resulting
...
1. **Config grant (standard and unrestricted modes).** When
   ...
   Sarthi applies the same host-side floor in unrestricted mode. If it is not
...
Explicit Sarthi YOLO (`--yolo`, `/yolo`, or `approvals.mode: off`) launches an
...
|---|---|
| `cua-driver not installed` | Run `sarthi computer-use install`, or `sarthi tools` and enable Computer Use |
| Captures consistently return empty / "no on-screen window" | On Linux: DISPLAY may not be set (X11) or you're on pure Wayland — ask the user to run `sarthi computer-use doctor`. On Windows: you may be in Session 0 (SSH session) instead of the interactive desktop — see the cua-driver `WINDOWS.md` deep-dive |
...
| Type text disappears into a terminal emulator | cua-driver detects terminals (Ghostty, iTerm2, Terminal.app, Windows Terminal, mintty, etc.) and routes through key-event synthesis — should "just work" on a recent cua-driver. If it doesn't, ask the user to run `sarthi computer-use doctor` |
...
| Anything else weird | **First action: ask the user to run `sarthi computer-use doctor`.** It runs the cua-driver `health_report` MCP tool and prints a structured per-check matrix. Their output tells you (and them) exactly what's wrong |
...
Sarthi intentionally keeps THIS skill focused on the Sarthi-side
...
Sarthi autodetection is a planned follow-up in trycua/cua. For now, the command
installs the pack under `~/.cua-driver/skills/cua-driver`; point Sarthi at that
