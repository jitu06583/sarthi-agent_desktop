...
| | |
|---|---|
| Source | Optional — install with `sarthi skills install official/security/unbroker` |
...
- **Optional upgrades** (the skill works zero-config without these; `setup --auto` turns on every
  ...
  - Email automation, two credential-free-or-not options:
    - **Browser mode (no password): `setup --email-mode browser`.** The agent sends opt-out/CCPA
      ...
      `browser_*` tools. Nothing is stored. This requires Sarthi to be pointed at the operator's own
      ...
      `chrome --remote-debugging-port=9222 --user-data-dir="$HOME/.sarthi/chrome-debug"` (a dedicated
...
- **Dossiers are plaintext by default** (JSON, `0600` under `SARTHI_HOME`). For at-rest encryption run
  ...
  a full-`SARTHI_HOME` read; set `PDD_AGE_IDENTITY` to a separate volume for real key separation.
