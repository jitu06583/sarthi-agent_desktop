...
| | |
|---|---|
| Source | Optional — install with `sarthi skills install official/security/1password` |
| Path | `optional-skills/security/1password` |
| Version | `1.0.0` |
| Author | arceus77-7, enhanced by Sarthi Agent |
| License | MIT |
...
- `tmux` available for stable authenticated sessions during Sarthi terminal calls (desktop app flow only)
...
### Service Account (recommended for Sarthi)

Set `OP_SERVICE_ACCOUNT_TOKEN` in `${SARTHI_HOME:-~/.sarthi}/.env` (the skill will prompt for this on first load).
...
## Sarthi Execution Pattern (desktop app flow)

Sarthi terminal commands are non-interactive by default and can lose auth context between calls.
...
```bash
SOCKET_DIR="${TMPDIR:-/tmp}/sarthi-tmux-sockets"
mkdir -p "$SOCKET_DIR"
SOCKET="$SOCKET_DIR/sarthi-op.sock"
