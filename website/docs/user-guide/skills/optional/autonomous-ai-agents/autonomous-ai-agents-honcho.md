...
description: "Configure and use Honcho memory with Sarthi -- cross-session user modeling, multi-profile peer isolation, observation config, dialectic reasoning, session su..."
---
...
Configure and use Honcho memory with Sarthi -- cross-session user modeling, multi-profile peer isolation, observation config, dialectic reasoning, session summaries, and context budget enforcement. Use when setting up Honcho, troubleshooting memory, managing profiles with Honcho peers, or tuning observation, recall, and dialectic settings.

## Skill metadata

| | |
|---|---|
| Source | Optional — install with `sarthi skills install official/autonomous-ai-agents/honcho` |
...
# Honcho Memory for Sarthi

Honcho provides AI-native cross-session user modeling. It learns who the user is across conversations and gives every Sarthi profile its own peer identity while sharing a unified view of the user.
...
See: https://docs.honcho.dev/v3/guides/integrations/sarthi#running-honcho-locally-with-sarthi

### Verify

```bash
sarthi honcho status    # shows resolved config, connection test, peer info
```
...
3. **AI peer card** -- the identity card for this Sarthi profile's AI peer
...
Honcho models conversations as interactions between **peers**. Sarthi creates two peers per session:

- **User peer** (`peerName`): represents the human. Honcho builds a user representation from observed messages.
- **AI peer** (`aiPeer`): represents this Sarthi instance. Each profile gets its own AI peer so agents develop independent views.
...
| `per-session` | New Honcho session each Sarthi run |
| `global` | Single session across all directories |

Manual override: `sarthi honcho map my-project-name`
...
Each Sarthi profile gets its own Honcho AI peer while sharing the same workspace (user context). This means:
...
# creates host block sarthi.coder, AI peer "coder", inherits config from default
```

What `--clone` does for Honcho:
1. Creates a `sarthi.coder` host block in `honcho.json`
...
```bash
sarthi honcho sync    # creates host blocks for all profiles that don't have one yet
```
...
Guidelines for Sarthi when Honcho memory is active.
...
Config file: `$SARTHI_HOME/honcho.json` (profile-local) or `~/.honcho/config.json` (global).
...
Run `sarthi honcho setup`. Ensure `memory.provider: honcho` is in `~/.sarthi/config.yaml`.

### Memory not persisting across sessions
Check `sarthi honcho status` -- verify `saveMessages: true` and `writeFrequency` isn't `session` (which only writes on exit).

### Profile not getting its own peer
Use `--clone` when creating: `sarthi profile create <name> --clone`. For existing profiles: `sarthi honcho sync`.
...
| Command | Description |
|---------|-------------|
| `sarthi honcho setup` | Interactive setup wizard (cloud/local, identity, observation, recall, sessions) |
| `sarthi honcho status` | Show resolved config, connection test, peer info for active profile |
| `sarthi honcho enable` | Enable Honcho for the active profile (creates host block if needed) |
| `sarthi honcho disable` | Disable Honcho for the active profile |
| `sarthi honcho peer` | Show or update peer names (`--user <name>`, `--ai <name>`, `--reasoning <level>`) |
| `sarthi honcho peers` | Show peer identities across all profiles |
| `sarthi honcho mode` | Show or set recall mode (`hybrid`, `context`, `tools`) |
| `sarthi honcho tokens` | Show or set token budgets (`--context <N>`, `--dialectic <N>`) |
| `sarthi honcho sessions` | List known directory-to-session-name mappings |
| `sarthi honcho map <name>` | Map current working directory to a Honcho session name |
| `sarthi honcho identity` | Seed AI peer identity or show both peer representations |
| `sarthi honcho sync` | Create host blocks for all Sarthi profiles that don't have one yet |
| `sarthi honcho migrate` | Step-by-step migration guide from OpenClaw native memory to Sarthi + Honcho |
| `sarthi memory setup` | Generic memory provider picker (selecting "honcho" runs the same wizard) |
| `sarthi memory status` | Show active memory provider and config |
| `sarthi memory off` | Disable external memory provider |
