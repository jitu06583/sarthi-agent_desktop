...
| | |
|---|---|
| Source | Optional — install with `sarthi skills install official/creative/unreal-mcp` |
...
Companion skill for the `unreal-engine` entry in the Sarthi MCP catalog. The
...
Two halves, in this order: the editor side must be up before Sarthi connects.
...
### One-time, Sarthi side

    sarthi mcp install unreal-engine
...
`~/.sarthi/config.yaml` under `mcp_servers.unreal-engine` to match.

Do NOT use `ModelContextProtocol.GenerateClientConfig` for Sarthi — that
writes `.mcp.json`-style files for Claude Code/Cursor/etc. Sarthi connects
...
2. Start the Sarthi session. Tools register as `mcp_unreal_engine_*`. If
...
three meta-tools, and every real tool is reached through them. Through Sarthi
they appear as:
...
then happens at `sarthi mcp install`/`configure` time instead. Tool-search
...
3. **NEVER issue overlapping calls.** Do not batch multiple
   `mcp_unreal_engine_*` calls in one turn — Sarthi runs batched calls
...
- **Start order matters.** Editor + server up first, then the Sarthi
...
- **Timeouts on long operations.** Sarthi' per-call default is 120 s; asset
  imports, big level saves, and renders can exceed it. Raise
  `mcp_servers.unreal-engine.timeout` in `~/.sarthi/config.yaml` for
