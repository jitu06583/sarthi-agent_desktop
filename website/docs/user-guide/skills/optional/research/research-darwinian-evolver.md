...
| | |
|---|---|
| Source | Optional — install with `sarthi skills install official/research/darwinian-evolver` |
| Path | `optional-skills/research/darwinian-evolver` |
| Version | `0.1.0` |
| Author | Bihruze (Asahi0x), Sarthi Agent |
| License | MIT |
...
import upstream classes into Sarthi itself.
...
```bash
mkdir -p ~/.sarthi/cache/darwinian-evolver && cd ~/.sarthi/cache/darwinian-evolver
...
```bash
cd ~/.sarthi/cache/darwinian-evolver/darwinian_evolver \
...
Open `~/.sarthi/cache/darwinian-evolver/darwinian_evolver/darwinian_evolver/lineage_visualizer.html`
...
SKILL_DIR=~/.sarthi/skills/research/darwinian-evolver
DE_DIR=~/.sarthi/cache/darwinian-evolver/darwinian_evolver
...
7. **AGPL.** Never `from darwinian_evolver import ...` inside Sarthi core.
   Custom driver scripts under `~/.sarthi/skills/...` are user-side and fine.
