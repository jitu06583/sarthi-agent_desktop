...
| | |
|---|---|
| Source | Optional — install with `sarthi skills install official/mcp/fastmcp` |
...
| Related skills | [`sarthi-agent`](/docs/user-guide/skills/bundled/autonomous-ai-agents/autonomous-ai-agents-sarthi-agent), [`mcporter`](/docs/user-guide/skills/optional/mcp/mcp-mcporter) |
...
- smoke-test a server with the FastMCP CLI before wiring it into Sarthi or another client
...
Use `native-mcp` when the server already exists and only needs to be connected to Sarthi. Use `mcporter` when the goal is ad-hoc CLI access to an existing MCP server instead of building one.
...
```bash
python ~/.sarthi/skills/mcp/fastmcp/scripts/scaffold_fastmcp.py \
...
```bash
python ~/.sarthi/skills/mcp/fastmcp/scripts/scaffold_fastmcp.py --list
```
...
When the goal is Sarthi integration, either:

- configure the server in `~/.sarthi/config.yaml` using the `native-mcp` skill, or
...
### Sarthi cannot see the deployed server

The server-building part may be correct while the Sarthi config is not. Load the `native-mcp` skill and configure the server in `~/.sarthi/config.yaml`, then restart Sarthi.
