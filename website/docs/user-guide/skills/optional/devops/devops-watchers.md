...
| | |
|---|---|
| Source | Optional — install with `sarthi skills install official/devops/watchers` |
...
All three live in `$SARTHI_HOME/skills/devops/watchers/scripts/` once the skill is installed. Each reads `WATCHER_STATE_DIR` (defaults to `$SARTHI_HOME/watcher-state/`) for its state file, keyed by the `--name` argument.
...
```bash
python $SARTHI_HOME/skills/devops/watchers/scripts/watch_rss.py \
  --name hn --url https://news.ycombinator.com/rss --max 5
```

Watch a GitHub repo (set `GITHUB_TOKEN` in `${SARTHI_HOME:-~/.sarthi}/.env` to avoid the 60 req/hr anonymous rate limit):
...
```bash
python $SARTHI_HOME/skills/devops/watchers/scripts/watch_http_json.py \
...
Every watcher writes `$SARTHI_HOME/watcher-state/<name>.json`. Inspect:

```bash
cat $SARTHI_HOME/watcher-state/hn.json
```

Force a replay (next run treated as first poll):

```bash
rm $SARTHI_HOME/watcher-state/hn.json
```
...
4. **Putting the state dir where the agent's sandbox can't write.** `$SARTHI_HOME/watcher-state/` is always writable. Docker/Modal backends may not see arbitrary host paths.
