...
| | |
|---|---|
| Source | Optional — install with `sarthi skills install official/productivity/canvas` |
...
4. Name the token (e.g., "Sarthi Agent"), set an optional expiry, and click **Generate Token**
5. Copy the token and add to `${SARTHI_HOME:-~/.sarthi}/.env`:
...
```bash
CANVAS="python $SARTHI_HOME/skills/productivity/canvas/scripts/canvas_api.py"
