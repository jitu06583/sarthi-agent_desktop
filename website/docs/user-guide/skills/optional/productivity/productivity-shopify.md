...
| | |
|---|---|
| Source | Optional — install with `sarthi skills install official/productivity/shopify` |
...
4. Save to `${SARTHI_HOME:-~/.sarthi}/.env`:
   ```
...
}' '{"input":{"title":"Test Hoodie","status":"DRAFT","vendor":"Sarthi","productType":"Apparel","tags":["test"]}}'
```
