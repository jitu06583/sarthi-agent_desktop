...
| | |
|---|---|
| Source | Optional — install with `sarthi skills install official/productivity/here-now` |
...
- Skill helper paths:
  - `${SARTHI_SKILL_DIR}/scripts/publish.sh` for publishing sites
  - `${SARTHI_SKILL_DIR}/scripts/drive.sh` for private Drive storage

## Create a site

```bash
PUBLISH="${SARTHI_SKILL_DIR}/scripts/publish.sh"
bash "$PUBLISH" {file-or-dir} --client sarthi
```
...
```bash
PUBLISH="${SARTHI_SKILL_DIR}/scripts/publish.sh"
bash "$PUBLISH" {file-or-dir} --slug {slug} --client sarthi
```
...
```bash
DRIVE="${SARTHI_SKILL_DIR}/scripts/drive.sh"
bash "$DRIVE" default
...
| `--client {name}`      | Agent name for attribution (e.g. `sarthi`)    |
