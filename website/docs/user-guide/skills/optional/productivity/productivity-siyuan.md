...
| | |
|---|---|
| Source | Optional — install with `sarthi skills install official/productivity/siyuan` |
...
3. Store it in `${SARTHI_HOME:-~/.sarthi}/.env`:
   ```
...
```yaml
# In ~/.sarthi/config.yaml under mcp_servers:
mcp_servers:
  siyuan:
    command: npx
