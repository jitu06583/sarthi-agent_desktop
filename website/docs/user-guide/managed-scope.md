...
`~/.sarthi/config.yaml`, `~/.sarthi/.env`, and even the shell environment — for
...
Managed scope is read from a system-level directory, default `/etc/sarthi`:

```text
/etc/sarthi/
├── config.yaml     # managed config layer (wins over ~/.sarthi/config.yaml)
└── .env            # managed env layer (wins over ~/.sarthi/.env + shell)
```
...
The location can be relocated with the `SARTHI_MANAGED_DIR` environment variable
(for containers or non-`/etc` deployments). This is a deployment/bootstrap path
knob — like `SARTHI_HOME` — set by the same administrator who owns the managed
files. It is **never persisted** to any `.env` by Sarthi.

```bash
# Point managed scope at a custom directory (set by IT / the deployment, not the user)
export SARTHI_MANAGED_DIR=/opt/org/sarthi-policy
```

:::warning
A user who can set `SARTHI_MANAGED_DIR` can repoint managed scope at a directory
...
left user-settable. `sarthi doctor` reports the *resolved* managed directory so
...
|---|---|---|
| 1 | `/etc/sarthi/config.yaml` (managed) | `/etc/sarthi/.env` (managed) |
| 2 | `~/.sarthi/config.yaml` (user) | `~/.sarthi/.env` (user) |
...
```bash
sarthi config        # shows a header naming the managed source + the pinned keys
sarthi doctor        # reports the resolved managed dir + pinned key counts
```

If you try to change a managed value, Sarthi refuses and names the source:

```bash
$ sarthi config set model.default my/model
Cannot set 'model.default': it is managed by your administrator
(/etc/sarthi/config.yaml) and cannot be changed.
```

The same applies to managed secrets — `sarthi config set` / setup will not write
a user value for an env key pinned by the managed `.env`.

## Setting up a managed scope (administrators)

```bash
sudo mkdir -p /etc/sarthi

# Pin some config values for every user on this machine
sudo tee /etc/sarthi/config.yaml >/dev/null <<'YAML'
model:
  provider: nous
security:
  redact_secrets: true
...
sudo tee /etc/sarthi/.env >/dev/null <<'ENV'
OPENAI_API_BASE=https://inference.example.com/v1
ENV

sudo chmod 0755 /etc/sarthi
sudo chmod 0644 /etc/sarthi/config.yaml /etc/sarthi/.env
```

Changes take effect on the next Sarthi start (a malformed managed file is logged
loudly and ignored — it never blocks startup, but the admin should check
`sarthi doctor` to confirm the policy is being applied).

## Security model and limitations (v1)

- **Enforcement is filesystem permissions only.** If a user has write access to
  the managed directory (or runs Sarthi as `root`), managed scope is advisory.
