...
Connect Sarthi to **iMessage** through [Photon][photon], a managed
...
your first iMessage from Sarthi — just a phone number we can bind to
your account.
:::
...
both directions. Because the SDK is TypeScript-only, Sarthi runs it in a
...
sarthi photon setup --phone +15551234567
```
...
2. **Finds or creates** the `Sarthi Agent` project on your account.
...
6. **Runs `npm install`** inside the plugin's sidecar directory. On
   ...
   `~/.sarthi/photon/sidecar`; set `PHOTON_SIDECAR_DIR` to pin an
   explicit location.

Runtime credentials are written to `~/.sarthi/.env`
...
(device token, dashboard project id) lives in `~/.sarthi/auth.json` under
`credential_pool.photon` / `credential_pool.photon_project`.

## Authorizing users

Photon uses the same authorization model as every other Sarthi
channel. Choose one approach:

**DM pairing (default).** When an unknown number messages your Photon
line, Sarthi replies with a pairing code. Approve it with:

```bash
sarthi pairing approve photon <CODE>
```

Use `sarthi pairing list` to see pending codes and approved users.

**Pre-authorize specific numbers** (in `~/.sarthi/.env`):

```bash
PHOTON_ALLOWED_USERS=+15551234567,+15559876543
```

**Open access** (dev only, in `~/.sarthi/.env`):
...
By default Sarthi responds to every authorized DM and group message.
...
they match a wake-word pattern. The defaults match `Sarthi` and
`@Sarthi agent` variants. For a custom agent name, set regex patterns:
...
Send an iMessage to your assigned number and Sarthi will reply.

## Status & troubleshooting

```bash
sarthi photon status
```

Prints saved credentials, sidecar health, your registered number, and the
assigned iMessage line Sarthi uses. When a Photon token and dashboard project
...
- **`sidecar deps : ✗ run sarthi photon install-sidecar`** — Node is
  installed but `spectrum-ts` isn't. Run the suggested command.
- **`device token : ✗ missing`** — run `sarthi photon setup` to log in.
- **`No iMessage line assigned yet`** — Spectrum is enabled but no line
  has been provisioned; re-run `sarthi photon setup` or check the
  [dashboard][app].
- **Sidecar won't start** — confirm `node --version` is 18.17+ and that
  `sarthi photon install-sidecar` completed without errors.
...
- **Outbound attachments are supported.** Sarthi sends images, voice
  ...
  media.
- **Native polls are supported.** Sarthi sends poll content through
  spectrum-ts' `poll()` builder via the sidecar's `/send-poll` endpoint.
- **Message effects are supported.** Sarthi sends text with native iMessage
...
- **Cron and standalone sends need the gateway running.** Out-of-process
  senders (cron jobs, `sarthi send`, the dashboard) reuse the sidecar the
  gateway spawned — they read its port/token from
  `<sarthi-home>/runtime/photon-sidecar.json`, written once the sidecar
...
- **Shared/free-tier lines can't initiate conversations with new
  ...
  brand-new recipient will be rejected by Photon even when Sarthi is set
...
| `PHOTON_MENTION_PATTERNS` | Sarthi wake words  | JSON list / comma / newline regex patterns for group mentions |
