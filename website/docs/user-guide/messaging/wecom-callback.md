...
Connect Sarthi to WeCom (Enterprise WeChat) as a self-built enterprise application using the callback/webhook model.

:::info WeCom Bot vs WeCom Callback
Sarthi supports two WeCom integration modes:
...
> Run `sarthi gateway setup` and pick **WeCom Callback** for a guided walk-through.
...
3. Sarthi decrypts the message, queues it for the agent
...
(Use `sarthi gateway start` only after `sarthi gateway install` has registered the systemd/launchd service.)
...
console. A mismatch between the token configured in Sarthi and the token the
...
in `~/.sarthi/.env` values around `=` will also break signature checks. After
fixing, restart `sarthi gateway run`.
...
Check `sarthi gateway run` logs for the bound host/port. If the adapter bound to
