...
sidebar_position: 16
title: "Yuanbao"
description: "Connect Sarthi Agent to the Yuanbao enterprise messaging platform via WebSocket gateway"
---

# Yuanbao

Connect Sarthi to [Yuanbao](https://yuanbao.tencent.com/), Tencent's enterprise messaging platform. The adapter uses a WebSocket gateway for real-time message delivery and supports both direct (C2C) and group conversations.
...
After initial setup, verify these variables in `~/.sarthi/.env`:
...
All standard Sarthi commands work on Yuanbao:
...
4. Review gateway logs: `tail -f ~/.sarthi/logs/gateway.log`
...
4. Consider enabling verbose logging: `SARTHI_LOG_LEVEL=debug`
...
Yuanbao has a maximum message size. Sarthi automatically chunks large responses with Markdown-aware splitting (respects code fences, tables, and paragraph boundaries).
...
```bash
SARTHI_LOG_LEVEL=debug sarthi gateway
```
...
```bash
sarthi chat -q "Send 'Hello from CLI' to yuanbao:group:group_code"
```
