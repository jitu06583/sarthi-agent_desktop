...
[A2A](https://a2a-protocol.org) is the open Agent2Agent protocol (v1.0, stewarded by the Linux Foundation) for communication between independent AI agents. The Sarthi A2A plugin works in **both directions**: your agent can call other A2A agents as tools, and other agents can send tasks to your Sarthi over HTTP.

It interoperates with any A2A-compliant peer — another Sarthi, LangChain, CrewAI, Google ADK agents, or anything built on the official `a2a-sdk`.

## When to use A2A

- **Sarthi ↔ Sarthi across machines** — let your desktop agent hand tasks to a Sarthi on a server, or vice versa, each with its own memory, tools, and credentials.
...
- **Being a callable service** — expose your Sarthi so other frameworks' agents can send it tasks.
...
```bash
sarthi tools enable a2a --platform cli        # CLI/TUI sessions
sarthi tools enable a2a --platform telegram   # or any messaging platform
sarthi tools enable a2a --platform a2a        # let inbound A2A tasks call peers (agent chaining)
```
...
With the platform enabled, Sarthi serves:
...
- **Audit log** — every exchange appends to `~/.sarthi/a2a_audit.jsonl`.
