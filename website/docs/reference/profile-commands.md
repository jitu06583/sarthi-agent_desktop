...
This page covers all commands related to [Sarthi profiles](../user-guide/profiles.md). For general CLI commands, see [CLI Commands Reference](./cli-commands.md).
...
Top-level command for managing profiles. Running `sarthi profile` without a subcommand shows help.
...
```bash
sarthi profile use <name>
```

Sets `<name>` as the active profile. All subsequent `sarthi` commands (without `-p`) will use this profile.
...
sarthi profile use default
```

## `sarthi profile create`

```bash
sarthi profile create <name> [options]
```
...
| `--description "<text>"` | One- or two-sentence description of what this profile is good at. Used by the kanban orchestrator to route tasks based on role instead of profile name alone. Skip and add later via `sarthi profile describe`. Persisted in `<profile_dir>/profile.yaml`. |
| `--no-skills` | Create an **empty** profile with zero bundled skills enabled. Writes a `.no-bundled-skills` marker into the profile so future `sarthi update` runs won't re-seed the bundled set, and refuses to combine with `--clone`, `--clone-from`, or `--clone-all` (which would copy skills in anyway). Useful for narrow orchestrator profiles or sandbox profiles that should not inherit the full skill catalog. To toggle this on an already-created profile (including the default `~/.sarthi`), use `sarthi skills opt-out` / `sarthi skills opt-in`. |
...
sarthi profile create work2 --clone-from work

# Clone everything from a specific profile
sarthi profile create work2-backup --clone-from work --clone-all
```

## `sarthi profile describe`

```bash
sarthi profile describe [<name>] [options]
```
...
sarthi profile describe researcher

# Set it explicitly
sarthi profile describe researcher --text "Reads source code and writes findings."

# Let the LLM generate one
sarthi profile describe researcher --auto

# Fill in descriptions for every profile that doesn't have one
sarthi profile describe --all --auto
```

## `sarthi profile delete`

```bash
sarthi profile delete <name> [options]
```
...
```bash
sarthi profile delete mybot
sarthi profile delete mybot --yes
```

:::warning
This permanently deletes the profile's entire directory including all config, memories, sessions, and skills. The `default` profile (`~/.sarthi`) cannot be deleted — use `sarthi uninstall` to remove everything.
:::

## `sarthi profile show`

```bash
sarthi profile show <name>
```

Displays details about a profile including its home directory, configured model, gateway status, skills count, and configuration file status.

This shows the profile's Sarthi home directory, not the terminal working directory. Terminal commands start from `terminal.cwd` (or the launch directory on the local backend when `cwd: "."`).
...
```bash
$ sarthi profile show work
Profile: work
Path:    ~/.sarthi/profiles/work
...
## `sarthi profile alias`

```bash
sarthi profile alias <name> [options]
```

Regenerates the shell alias script at `~/.local/bin/<name>`. Useful if the alias was accidentally deleted or if you need to update it after moving your Sarthi installation.
...
```bash
sarthi profile alias work
# Creates/updates ~/.local/bin/work

sarthi profile alias work --name mywork
# Creates ~/.local/bin/mywork

sarthi profile alias work --remove
# Removes the wrapper script
```

## `sarthi profile rename`

```bash
sarthi profile rename <old-name> <new-name>
```
...
```bash
sarthi profile rename mybot assistant
# ~/.sarthi/profiles/mybot → ~/.sarthi/profiles/assistant
# ~/.local/bin/mybot → ~/.local/bin/assistant
```

## `sarthi profile export`

```bash
sarthi profile export <name> [options]
```
...
```bash
sarthi profile export work
# Creates work.tar.gz in the current directory

sarthi profile export work -o ./work-2026-03-29.tar.gz
```
...
## `sarthi profile import`

```bash
sarthi profile import <archive> [options]
```
...
```bash
sarthi profile import ./work-2026-03-29.tar.gz
# Infers profile name from the archive

sarthi profile import ./work-2026-03-29.tar.gz --name work-restored
```
...
| `--alias` | Also create a shell wrapper (e.g. `telemetry` → `sarthi -p telemetry`). |
...
sarthi profile install github.com/kyle/telemetry-distribution --alias

# Install from a full HTTPS git URL
sarthi profile install https://github.com/kyle/telemetry-distribution.git

# Install from SSH
sarthi profile install git@github.com:kyle/telemetry-distribution.git

# Install from a local directory during development
sarthi profile install ./telemetry/
```

### `sarthi profile update`

```bash
sarthi profile update <name> [--force-config] [--yes]
```
...
```bash
sarthi profile info <name>
```

Prints the profile's distribution manifest — name, version, required
Sarthi version, author, env var requirements, the source URL/path, and
...
`sarthi profile list` also shows the distribution name and version in a
`Distribution` column, and `sarthi profile show <name>` / `delete <name>`
...
sarthi profile install git@github.com:your-org/internal-assistant.git

# Uses your git credential helper
sarthi profile install https://github.com/your-org/internal-assistant.git
```
...
`sarthi_requires` supports `>=`, `<=`, `==`, `!=`, `>`, `<`, or a bare
version (treated as `>=`). Install fails with a clear error if the current
Sarthi version doesn't satisfy the spec.
...
2. Initialize a git repo (or use an existing one) and push to GitHub /
   GitLab / any host Sarthi can clone from.
3. Tell recipients to run `sarthi profile install <your-repo-url>`.
...
## `sarthi -p` / `sarthi --profile`

```bash
sarthi -p <name> <command> [options]
sarthi --profile <name> <command> [options]
```

Global flag to run any Sarthi command under a specific profile without changing the sticky default. This overrides the active profile for the duration of the command.
...
```bash
sarthi -p work chat -q "Check the server status"
sarthi --profile dev gateway start
sarthi -p personal skills list
sarthi -p work config edit
```

## `sarthi completion`

```bash
sarthi completion <shell>
```
...
- `sarthi profile <TAB>` — subcommands (list, use, create, etc.)
- `sarthi profile use <TAB>` — profile names
- `sarthi -p <TAB>` — profile names
