...
description: "Official optional skills shipped with sarthi-agent — install via sarthi skills install official/<category>/<skill>"
---

# Optional Skills Catalog

Optional skills ship with sarthi-agent under `optional-skills/` but are **not active by default**. Install them explicitly:

```bash
sarthi skills install official/<category>/<skill>
```

For example:

```bash
sarthi skills install official/blockchain/solana
sarthi skills install official/mlops/flash-attention
```
...
```bash
sarthi skills uninstall <skill-name>
```
...
| [**honcho**](/docs/user-guide/skills/optional/autonomous-ai-agents/autonomous-ai-agents-honcho) | Configure and use Honcho memory with Sarthi -- cross-session user modeling, multi-profile peer isolation, observation config, dialectic reasoning, session summaries, and context budget enforcement. Use when setting up Honcho, troubleshoo... |
...
| [**kanban-video-orchestrator**](/docs/user-guide/skills/optional/creative/creative-kanban-video-orchestrator) | Plan, set up, and monitor a multi-agent video production pipeline backed by Sarthi Kanban. Use when the user wants to make ANY video — narrative film, product/marketing, music video, explainer, ASCII/terminal art, abstract/generative loo... |
...
| [**sarthi-s6-container-supervision**](/docs/user-guide/skills/optional/devops/devops-sarthi-s6-container-supervision) | Modify, debug, or extend the s6-overlay supervision tree inside the Sarthi Agent Docker image — adding new services, debugging profile gateways, understanding the Architecture B main-program pattern. |
...
| Skill | Description |
|-------|-------------|
| [**agentmail**](/docs/user-guide/skills/optional/email/email-agentmail) | Give the agent its own dedicated email inbox via AgentMail. Send, receive, and manage email autonomously using agent-owned email addresses (e.g. sarthi-agent@agentmail.to). |
...
| Skill | Description |
|-------|-------------|
| [**openclaw-migration**](/docs/user-guide/skills/optional/migration/migration-openclaw-migration) | Migrate a user's OpenClaw customization footprint into Sarthi Agent. Imports Sarthi-compatible memories, SOUL.md, command allowlists, user skills, and selected workspace assets from ~/.openclaw, then reports exactly what could not be mig... |
...
| [**telephony**](/docs/user-guide/skills/optional/productivity/productivity-telephony) | Give Sarthi phone capabilities without core tool changes. Provision and persist a Twilio number, send and receive SMS/MMS, make direct calls, and place AI-driven outbound calls through Bland.ai or Vapi. |
