...
You need **tenant admin rights** (or an admin to grant consent on your behalf) to finish this. Bookmark the values you collect — they go into `~/.sarthi/.env` at the end.
...
4. Fill in:
   - **Name:** `Sarthi Teams Meeting Pipeline` (or any name you'll recognize).
...
3. **Description:** `sarthi-graph-secret`. **Expires:** pick a value that matches your rotation policy (6-24 months is typical).
4. Click **Add**.
...
Put the three values you collected into `~/.sarthi/.env`:
...
Sarthi ships a Graph auth smoke-test. From your Sarthi install:
...
2. Update `MSGRAPH_CLIENT_SECRET` in `~/.sarthi/.env` with the new value.
3. Restart the gateway so the new secret is picked up: `sarthi gateway restart`.
