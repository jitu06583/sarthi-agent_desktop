...
```bash
sarthi teams-pipeline validate
```
...
```bash
sarthi teams-pipeline token-health
sarthi teams-pipeline token-health --force-refresh
```
...
```bash
sarthi teams-pipeline subscriptions
```

### Renew near-expiry subscriptions

```bash
sarthi teams-pipeline maintain-subscriptions
sarthi teams-pipeline maintain-subscriptions --dry-run
```
...
#### Option 1: Sarthi cron (recommended if you already run the Sarthi gateway)

Sarthi ships a built-in cron scheduler. The `--no-agent` mode runs a script as the job (rather than using an LLM), and `--script` must point at a file under `~/.sarthi/scripts/`. First create the script:

```bash
mkdir -p ~/.sarthi/scripts
cat > ~/.sarthi/scripts/maintain-teams-subscriptions.sh <<'EOF'
#!/usr/bin/env bash
exec sarthi teams-pipeline maintain-subscriptions
EOF
chmod +x ~/.sarthi/scripts/maintain-teams-subscriptions.sh
```

Then register a script-only cron job that runs every 12 hours (gives 6x headroom against the 72h expiry window):

```bash
sarthi cron create "0 */12 * * *" \
...
Create `/etc/systemd/system/sarthi-teams-pipeline-maintain.service`:

```ini
[Unit]
Description=Sarthi Teams pipeline subscription maintenance
After=network-online.target

[Service]
Type=oneshot
User=sarthi
EnvironmentFile=/etc/sarthi/env
ExecStart=/usr/local/bin/sarthi teams-pipeline maintain-subscriptions
```

And `/etc/systemd/system/sarthi-teams-pipeline-maintain.timer`:

```ini
[Unit]
Description=Run Sarthi Teams pipeline subscription maintenance every 12 hours
...
sudo systemctl enable --now sarthi-teams-pipeline-maintain.timer
systemctl list-timers sarthi-teams-pipeline-maintain.timer
```

#### Option 3: Plain crontab

```cron
0 */12 * * * /usr/local/bin/sarthi teams-pipeline maintain-subscriptions >> /var/log/sarthi/teams-pipeline-maintain.log 2>&1
```

Make sure the cron environment has the `MSGRAPH_*` credentials. Simplest fix: source `~/.sarthi/.env` at the top of a wrapper script that crontab calls.
...
```bash
sarthi teams-pipeline subscriptions   # should show expirationDateTime advanced
sarthi teams-pipeline maintain-subscriptions --dry-run   # should show "0 expiring soon" most of the time
```
...
```bash
sarthi teams-pipeline list
sarthi teams-pipeline list --status failed
sarthi teams-pipeline show <job-id>
```

### Replay a stored job

```bash
sarthi teams-pipeline run <job-id>
```
...
- run `sarthi teams-pipeline maintain-subscriptions --dry-run`
- inspect `sarthi teams-pipeline list --status failed`
...
- run `sarthi teams-pipeline validate`
...
Check:
- whether you manually replayed a job with `sarthi teams-pipeline run`
...
- [ ] `sarthi teams-pipeline validate` returns an OK snapshot
- [ ] `sarthi teams-pipeline token-health --force-refresh` succeeds
- [ ] **`maintain-subscriptions` is scheduled** (Sarthi cron, systemd timer, or crontab — see [Automating subscription renewal](#automating-subscription-renewal-required-for-production)). Without this, Graph subscriptions silently expire within 72 hours.
