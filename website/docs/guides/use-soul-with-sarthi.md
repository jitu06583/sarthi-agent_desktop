---
sidebar_position: 7
title: "Use SOUL.md with Sarthi"
description: "How to use SOUL.md to shape Sarthi Agent's default voice, what belongs there, and how it differs from AGENTS.md and /personality"
---

# Use SOUL.md with Sarthi

`SOUL.md` is the **primary identity** for your Sarthi instance. It's the first thing in the system prompt — it defines who the agent is, how it speaks, and what it avoids.

If you want Sarthi to feel like the same assistant every time you talk to it — or if you want to replace the Sarthi persona entirely with your own — this is the file to use.
...
- personality
- communication style
- how direct or warm Sarthi should be
- what Sarthi should avoid stylistically
- how Sarthi should relate to uncertainty, disagreement, and ambiguity

In short:
- `SOUL.md` is about who Sarthi is and how Sarthi speaks
...
Sarthi now uses only the global SOUL file for the current instance:

```text
~/.sarthi/SOUL.md
```

If you run Sarthi with a custom home directory, it becomes:
...
Sarthi automatically seeds a starter `SOUL.md` for you if one does not already exist.

That means most users now begin with a real file they can read and edit immediately.

Important:
- if you already have a `SOUL.md`, Sarthi does not overwrite it
- if the file exists but is empty, Sarthi adds nothing from it to the prompt

## How Sarthi uses it

When Sarthi starts a session, it reads `SOUL.md` from `SARTHI_HOME`, scans it for prompt-injection patterns, truncates it if needed, and uses it as the **agent identity** — slot #1 in the system prompt. This means SOUL.md completely replaces the built-in default identity text.
...
That alone can noticeably change how Sarthi feels.
...
Sarthi already tries to be helpful and clear. `SOUL.md` should add real personality and style, not restate obvious defaults.
...
# Style
How Sarthi should sound.

# Avoid
What Sarthi should not do.

# Defaults
How Sarthi should behave when ambiguity appears.
```
...
Then restart Sarthi or start a new session.
...
### I edited SOUL.md but Sarthi still sounds the same

Check:
- you edited `~/.sarthi/SOUL.md` or `$SARTHI_HOME/SOUL.md`
...
### Sarthi is ignoring parts of my SOUL.md
