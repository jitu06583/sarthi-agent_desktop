...
description: "Send text from any shell script, cron job, CI hook, or monitoring daemon to Telegram, Discord, Slack, Signal, and other platforms using `sarthi send`."
---

# Pipe Script Output to Messaging Platforms

`sarthi send` is a small, scriptable CLI that pushes a message to any
messaging platform Sarthi is already configured for. Think of it as a
...
- Piping any tool's output anywhere (`make | sarthi send --to slack:#builds`)

The command reuses the same credentials and platform adapters that `sarthi
...
sarthi send --to slack:#eng --subject "[CI] build.log" --file build.log

# Thread target (Telegram topic, Discord thread)
sarthi send --to telegram:-1001234567890:17585 "threaded reply"

# List every configured target
sarthi send --list

# Filter by platform
sarthi send --list telegram
```
...
Any platform Sarthi ships adapters for works as a target:
...
`sarthi send` resolves the message body in this order:

1. **Positional argument** — `sarthi send --to telegram "hi"`
2. **`--file PATH`** — `sarthi send --to telegram --file msg.txt`
3. **Piped stdin** — `echo hi | sarthi send --to telegram`

When stdin is a TTY (no pipe), Sarthi does **not** wait for input — you'll
...
if [ "$ram_pct" -ge 85 ]; then
  sarthi send --to telegram --subject "⚠ MEMORY WARNING" \
    "RAM ${ram_pct}% on $(hostname)"
fi
```

Because `sarthi send` reuses your Sarthi config, the same script works on
any host where Sarthi is installed — no need to export bot tokens into
...
`sarthi send`. If the Python interpreter can't load because the box is
...
if ./scripts/deploy.sh; then
  sarthi send --to slack:#deploys "✅ ${CI_COMMIT_SHA:0:7} deployed"
else
  tail -n 100 deploy.log | sarthi send \
...
0 9 * * * /usr/local/bin/generate-metrics.sh \
  | /home/me/.sarthi/bin/sarthi send \
...
```bash
./train.py --epochs 200 && \
  sarthi send --to telegram "training done" || \
  sarthi send --to telegram "training failed (exit $?)"
```
...
sarthi send --to telegram --quiet "keepalive" || {
  echo "Telegram delivery failed" >&2
  exit 1
}

# Capture the message ID for later editing / threading
msg_id=$(sarthi send --to discord:#ops --json "build started" \
  | jq -r .message_id)
```

---

## Does `sarthi send` Need the Gateway Running?

**Usually no.** For any bot-token platform — Telegram, Discord, Slack,
Signal, SMS, WhatsApp Cloud API, and most others — `sarthi send` calls
the platform's REST endpoint directly using credentials from
`~/.sarthi/.env` and `~/.sarthi/config.yaml`. It's a standalone subprocess
...
pointing at the gateway; start it with `sarthi gateway start` and retry.
...
# Machine-readable
sarthi send --list --json
```

The listing is built from `~/.sarthi/channel_directory.json`, which the
gateway refreshes every few minutes while it's running. If you see
"no channels discovered yet", start the gateway once (`sarthi gateway
...
| Approach | Multi-platform | Reuses Sarthi creds | Needs gateway | Best for |
|----------|----------------|---------------------|---------------|----------|
| `sarthi send` | ✅ | ✅ | No (bot-token) | Everything below |
...
`sarthi send` is intentionally the simplest possible surface. If you need
...
If you just need to pipe a raw string, reach for `sarthi send`.
...
- [Gateway Internals](/developer-guide/gateway-internals) —
  the delivery router that `sarthi send` shares with cron delivery.
