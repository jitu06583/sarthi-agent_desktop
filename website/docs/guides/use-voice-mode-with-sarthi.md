---
sidebar_position: 8
title: "Use Voice Mode with Sarthi"
description: "A practical guide to setting up and using Sarthi voice mode across CLI, Telegram, Discord, and Discord voice channels"
---

# Use Voice Mode with Sarthi
...
- you want Sarthi sitting in a Discord voice channel for live conversation
...
There are really three different voice experiences in Sarthi.
...
## Step 1: make sure normal Sarthi works first
...
Sarthi supports both local and cloud speech stacks.
...
### If you use `sarthi setup`

If you choose NeuTTS in the setup wizard, Sarthi checks whether `neutts` is already installed. If it is missing, the wizard tells you NeuTTS needs the Python package `neutts` and the system package `espeak-ng`, offers to install them for you, installs `espeak-ng` with your platform package manager, and then runs:
...
4. Sarthi transcribes and responds
...
- asking Sarthi to structure your thoughts in real time

#### Accessibility / low-typing sessions

If typing is inconvenient, voice mode is one of the fastest ways to stay in the full Sarthi loop.

## Tuning CLI behavior

### Silence threshold

If Sarthi starts/stops too aggressively, tune:
...
Sarthi stays a normal chat bot, but can speak replies.
...
- you want Sarthi to function like a portable research or ops assistant
...
Sarthi joins a Discord VC, listens to user speech, transcribes it, runs the normal agent pipeline, and speaks replies back into the channel.
...
- Sarthi detects speech boundaries
- transcripts are posted in the associated text channel
- Sarthi responds in text and audio
...
1. get text Sarthi working
2. run `sarthi setup tts` to enable voice support
