...
description: "Complete guide to migrating your OpenClaw / Clawdbot setup to Sarthi Agent — what gets migrated, how config maps, and what to check after."
---
...
sarthi claw migrate --preset full --migrate-secrets --yes
```
...
| `--no-backup` | Skip the pre-migration zip snapshot of `~/.sarthi/` (by default a single restore-point archive is written before apply, under `~/.sarthi/backups/pre-migration-*.zip`; restorable with `sarthi import`). |
...
| What | OpenClaw source | Sarthi destination | Notes |
|------|----------------|-------------------|-------|
| Persona | `workspace/SOUL.md` | `~/.sarthi/SOUL.md` | Direct copy |
| Workspace instructions | `workspace/AGENTS.md` | `AGENTS.md` in `--workspace-target` | Requires `--workspace-target` flag |
| Long-term memory | `workspace/MEMORY.md` | `~/.sarthi/memories/MEMORY.md` | Parsed into entries, merged with existing, deduped. Uses `§` delimiter. |
| User profile | `workspace/USER.md` | `~/.sarthi/memories/USER.md` | Same entry-merge logic as memory. |
| Daily memory files | `workspace/memory/*.md` | `~/.sarthi/memories/MEMORY.md` | All daily files merged into main memory. |
...
| Source | OpenClaw location | Sarthi destination |
|--------|------------------|-------------------|
| Workspace skills | `workspace/skills/` | `~/.sarthi/skills/openclaw-imports/` |
| Managed/shared skills | `~/.openclaw/skills/` | `~/.sarthi/skills/openclaw-imports/` |
| Personal cross-project | `~/.agents/skills/` | `~/.sarthi/skills/openclaw-imports/` |
| Project-level shared | `workspace/.agents/skills/` | `~/.sarthi/skills/openclaw-imports/` |

Skill conflicts are handled by `--skill-conflict`: `skip` leaves the existing Sarthi skill, `overwrite` replaces it, `rename` creates a `-imported` copy.
...
| What | OpenClaw config path | Sarthi config path | Mapping |
|------|---------------------|-------------------|---------|
...
| OpenClaw config path | Sarthi config path | Notes |
|---------------------|-------------------|-------|
...
| OpenClaw field | Sarthi field | Notes |
|----------------|-------------|-------|
...
| TTS assets | `~/.sarthi/tts/` (file copy) |

### Messaging platforms

| Platform | OpenClaw config path | Sarthi `.env` variable | Notes |
|----------|---------------------|----------------------|-------|
...
| What | OpenClaw path | Sarthi path | Notes |
|------|-------------|-------------|-------|
...
| Gateway auth token | `gateway.auth.token` | `.env` → `SARTHI_GATEWAY_TOKEN` | Requires `--migrate-secrets` |
...
### Archived (no direct Sarthi equivalent)

These are saved to `~/.sarthi/migration/openclaw/<timestamp>/archive/` for manual review:

| What | Archive file | How to recreate in Sarthi |
|------|-------------|--------------------------|
| `IDENTITY.md` | `archive/workspace/IDENTITY.md` | Merge into `SOUL.md` |
| `TOOLS.md` | `archive/workspace/TOOLS.md` | Sarthi has built-in tool instructions |
...
| Cron jobs | `archive/cron-config.json` | Recreate with `sarthi cron create` |
| Plugins | `archive/plugins-config.json` | See [plugins guide](/user-guide/features/hooks) |
| Hooks/webhooks | `archive/hooks-config.json` | Use `sarthi webhook` or gateway hooks |
| Memory backend | `archive/memory-backend-config.json` | Configure via `sarthi honcho` |
| Skills registry | `archive/skills-registry-config.json` | Use `sarthi skills config` |
...
| Multi-agent list | `archive/agents-list.json` | Use Sarthi profiles |
...
The migration resolves all three formats. For env templates and SecretRef objects with `source: "env"`, it looks up the value in `~/.openclaw/.env` and the `openclaw.json` env sub-object. SecretRef objects with `source: "file"` or `source: "exec"` can't be resolved automatically — the migration warns about these, and those values must be added to Sarthi manually via `sarthi config set`.

## After migration

1. **Check the migration report** — printed on completion with counts of migrated, skipped, and conflicting items.

2. **Review archived files** — anything in `~/.sarthi/migration/openclaw/<timestamp>/archive/` needs manual attention.

3. **Start a new session** — imported skills and memory entries take effect in new sessions, not the current one.

4. **Verify API keys** — run `sarthi status` to check provider authentication.

5. **Test messaging** — if you migrated platform tokens, restart the gateway: `systemctl --user restart sarthi-gateway`

6. **Check session policies** — run `sarthi config show` and verify the `session_reset` value matches your expectations.

7. **Re-pair WhatsApp** — WhatsApp uses QR code pairing (Baileys), not token migration. Run `sarthi whatsapp` to pair.

8. **Archive cleanup** — after confirming everything works, run `sarthi claw cleanup` to rename leftover OpenClaw directories to `.pre-migration/` (prevents state confusion).
...
Keys might be stored in several places depending on your OpenClaw version: inline in `openclaw.json` under `models.providers.*.apiKey`, in `~/.openclaw/.env`, in the `openclaw.json` `"env"` sub-object, or in `agents/main/agent/auth-profiles.json`. The migration checks all four. If keys use `source: "file"` or `source: "exec"` SecretRefs, they can't be resolved automatically — add them via `sarthi config set`.

### Skills not appearing after migration

Imported skills land in `~/.sarthi/skills/openclaw-imports/`. Start a new session for them to take effect, or run `/skills` to verify they're loaded.

### TTS voice not migrated

OpenClaw stores TTS settings in two places: `messages.tts.providers.*` and the top-level `talk` config. The migration checks both. If your voice ID was set via the OpenClaw UI (stored in a different path), you may need to set it manually: `sarthi config set tts.elevenlabs.voice_id YOUR_VOICE_ID`.
