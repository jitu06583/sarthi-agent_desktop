...
3. Sets `model.provider: nous` in `~/.sarthi/config.yaml`
...
OAuth needs a browser, but the loopback callback runs on the machine where Sarthi is running. Two options:

```bash
# Option A: SSH port forwarding (preferred)
ssh -N -L 8642:127.0.0.1:8642 user@remote-host    # in a local terminal
sarthi setup --portal                              # on the remote, open the printed URL in your local browser

# Option B: device-code login (works from Cloud Shell, Codespaces, EC2 Instance Connect)
sarthi auth add nous --type oauth
# Then re-run `sarthi setup --portal` to wire the provider + gateway
```
...
```
Hey, search the web for "Sarthi Agent release notes" and summarize the top 3 hits.
```

You should see Sarthi call `web_search` (Firecrawl-backed, through the gateway) and respond with a summary. If the search runs and the response makes sense, you're done — the Portal is wired up end to end.

## 5. Pick the model you actually want

`sarthi setup --portal` lets you pick a model during setup, but the whole point of the subscription is access to the full catalog — switch any time with `/model` mid-session:
...
sarthi config set model.default anthropic/claude-sonnet-4.6
```

### Don't pick Sarthi-4 for agent work

Sarthi-4-70B and Hermes-4-405B are available on the Portal at deep discounts, but they're **chat/reasoning models**, not tool-call-tuned. They will struggle with multi-step agent loops. Use them for conversation/research work through the [subscription proxy](/user-guide/features/subscription-proxy) from non-agent tools. For Sarthi Agent itself, stick to the frontier agentic models above.

The Portal's own [info page](https://portal.nousresearch.com/info) carries this warning too — it's the official Nous guidance, not just a Sarthi-side opinion.
...
These rows appear in `sarthi tools` even before you've logged into Nous Portal — if you pick "Nous Subscription" without an active session, Sarthi runs the Portal login inline (without changing your inference provider or your other tools).
...
Then in any messaging-platform session (Telegram, Discord, Signal, etc.), send a voice message and Sarthi will transcribe it, respond, and reply with synthesized voice — all on your Portal subscription.
...
If you use [Sarthi profiles](/user-guide/profiles) (e.g. a separate config per project), the Portal refresh token is automatically shared across all profiles via a shared token store. Sign in once on any profile, and the rest pick it up automatically.

For team setups where multiple humans share a machine, each human has their own Portal account → each home directory holds its own `~/.sarthi/auth.json` → no token sharing across users. This is the right boundary.

## Troubleshooting

### `sarthi portal info` shows "not logged in" after `sarthi setup --portal`
...
```bash
sarthi config set model.provider nous
```
...
Re-verify with `sarthi portal info`.
...
Your Portal refresh token was invalidated (password change, manual revoke, session expiry). The token is now quarantined locally so Sarthi doesn't replay it endlessly. Just log in again:

```bash
sarthi auth add nous
```
...
`sarthi portal info` will tell you whether you're actually routing through the Portal or some other provider. Common causes:
...
- Multiple Sarthi profiles where you're using the wrong one (check `sarthi profile list`)

### Want to revoke and start clean

```bash
sarthi auth logout nous       # wipes the local refresh token
...
- **[Subscription proxy](/user-guide/features/subscription-proxy)** — Use your Portal subscription from non-Sarthi tools
...
- **[Profiles](/user-guide/profiles)** — Share one Portal login across multiple Sarthi configurations
