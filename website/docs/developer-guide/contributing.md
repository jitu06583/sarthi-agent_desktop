...
sidebar_position: 4
title: "Contributing"
description: "How to contribute to Sarthi Agent — dev setup, code style, PR process"
---

# Contributing

Thank you for contributing to Sarthi Agent! This guide covers setting up your dev environment, understanding the codebase, and getting your PR merged.
...
- Building a custom/local tool without modifying Sarthi core? Start with [Build a Sarthi Plugin](../developer-guide/plugins/index.md)
- Building a new built-in core tool for Sarthi itself? Start with [Adding Tools](./adding-tools.md)
...
You can also run a fully isolated Sarthi instance (throwaway SARTHI_HOME, separate Electron
userData, distinct Electron app name to avoid the single-instance lock):

```bash
scripts/dev-sandbox.sh python -m sarthi_cli.main
scripts/dev-sandbox.sh --persistent python -m sarthi_cli.main desktop  # state survives restarts, but lives in the worktree :)
```
...
echo 'OPENROUTER_API_KEY=sk-or-v1-your-key' >> ~/.sarthi/.env
```
