...
description: "How the iron-proxy egress firewall integrates with Sarthi — module layout, lifecycle, security invariants, and extension points"
---

# Egress proxy internals

This page covers the architecture of the egress credential-injection firewall (`sarthi egress` / iron-proxy) from a contributor / plugin author's perspective. End-user setup + usage docs live at [Egress proxy](../user-guide/egress/iron-proxy.md).
...
sarthi_cli/proxy_cli.py               Wizard + slash command handlers.
                                       `sarthi egress {install,setup,start,stop,
                                       status,disable,config}`.  Wires the
                                       core module into argparse.

sarthi_cli/main.py:_dispatch_egress   Top-level subparser dispatcher.
                                       dest='egress_command' (intentionally
                                       disjoint from the inbound OAuth
                                       `sarthi proxy` subparser, which uses
                                       dest='proxy_command').

sarthi_cli/config.py: proxy schema    The `proxy:` block in DEFAULT_CONFIG.
...
tools/environments/docker.py
  ...
  DockerEnvironment.__init__          Docker-side merge logic: collision
                                       ...
                                       _SARTHI_EGRESS_NODE_OPTIONS_APPEND
...
tests/test_iron_proxy_e2e.py          Live E2E (gated on SARTHI_RUN_E2E=1).
...
sarthi egress install
  -> agent.proxy_sources.iron_proxy.install_iron_proxy(force=...)
       ...
       Stage into ~/.sarthi/bin/.iron-proxy_XXXX, chmod 755, os.replace
         to ~/.sarthi/bin/iron-proxy (atomic).
       _VERSION_CACHE.pop(target) so a forced reinstall re-probes
         --version on next call.

sarthi egress setup [--from-bitwarden | --no-bitwarden] [--rotate-tokens]
  -> proxy_cli.cmd_setup
...
| Path | Mode | Test |
|---|---|---|
| `~/.sarthi/proxy/` (dir) | `0o700` | `test_proxy_state_dir_is_0o700` |
...
`merge_mappings(existing, discovered, rotate=False)` MUST return prior tokens for providers that overlap.  Re-running `sarthi egress setup` cannot silently 401 running sandboxes.  `--rotate-tokens` is the explicit opt-in.
...
`cmd_setup` MUST NOT downgrade `credential_source: bitwarden` to `env` on re-run without an explicit `--no-bitwarden` flag.  Running `sarthi egress setup` (no flag) preserves whatever was previously configured.
...
If the provider uses SigV4 / SDK-minted OAuth / request signatures, a static header swap cannot cover it.  Add the env var to `_NON_BEARER_PROVIDERS` so the wizard and `sarthi egress status` warn about it:
...
4. Sets the seven env vars (HTTPS_PROXY, NO_PROXY, REQUESTS_CA_BUNDLE, SSL_CERT_FILE, CURL_CA_BUNDLE, NODE_EXTRA_CA_CERTS, SARTHI_EGRESS_PROXY) and the per-mapping `SARTHI_PROXY_TOKEN_<NAME>` vars.
5. Distributes the CA cert into the sandbox at a path the runtime will trust (typically `/etc/ssl/certs/sarthi-egress-ca.crt`).
...
SARTHI_RUN_E2E=1 scripts/run_tests.sh tests/test_iron_proxy_e2e.py

# Live PTY smoke against `sarthi egress`
SARTHI_HOME=/tmp/sarthi-egress-test python3 -m sarthi_cli.main egress --help
SARTHI_HOME=/tmp/sarthi-egress-test python3 -m sarthi_cli.main egress setup --help
```
...
- Bitwarden Secrets Manager integration: [`sarthi secrets bitwarden`](https://hermes-agent.nousresearch.com/docs/user-guide/secrets/bitwarden)
- CLI command reference: [`sarthi egress`](https://hermes-agent.nousresearch.com/docs/reference/cli-commands#sarthi-egress)
