...
| Variable | Meaning |
|----------|---------|
| `SARTHI_MAIN_CHECKOUT` | The deps checkout — where `node_modules` really lives, and whose `.venv/bin/python` runs the backend. |
| `SARTHI_GUI_DEPS_CHECKOUT` | Where the desktop deps (`apps/desktop/node_modules`) live. Defaults to `SARTHI_MAIN_CHECKOUT`; override only if you keep desktop deps elsewhere. |

Neither is read by Sarthi itself — they're private to these helpers. The variables Sarthi *does* read are covered in [Environment Variables](../reference/environment-variables.md).

## `htui` — TUI from the worktree

The Ink TUI has a dev path already: `sarthi --tui --dev` runs the TypeScript sources via `tsx` instead of the prebuilt bundle. `htui` is a one-liner over it that also points the run at the current worktree's `ui-tui/`:

```bash
htui() {
  local root
  root="$(_sarthi_root)" || { echo "htui: not in a Sarthi checkout" >&2; return 1; }
  ( cd "$root" && PYTHONPATH="$root" \
      "$SARTHI_MAIN_CHECKOUT/.venv/bin/python" -m sarthi_cli.main --tui --dev "$@" )
}
```

`--dev` compiles from source, so it links `ui-tui/node_modules` from `SARTHI_MAIN_CHECKOUT` when the root lockfile matches and installs locally otherwise (see [`_sarthi_root` / linking helpers](#shared-helpers)).

:::warning `--dev` and `SARTHI_TUI_DIR` are mutually exclusive
`SARTHI_TUI_DIR` points Sarthi at a *prebuilt* bundle (Nix, system packages), which has no source to hot-reload. If it's set in your shell, `sarthi --tui --dev` exits with an error. Run `unset SARTHI_TUI_DIR` before `htui`.
:::
...
hgui() {
  ...
  root="$(_sarthi_root)" || { echo "hgui: not in a Sarthi checkout" >&2; return 1; }
  deps="${SARTHI_GUI_DEPS_CHECKOUT:-$SARTHI_MAIN_CHECKOUT}"
  ...
  if cmp -s "$root/package-lock.json" "$deps/package-lock.json"; then
    _sarthi_link_deps "$desktop" "$deps/apps/desktop"
    _sarthi_link_deps "$root" "$deps"
  else
  ...
  trap '_sarthi_gui_cleanup "$root"' INT TERM EXIT

  ( cd "$desktop"
    export PATH="$root/node_modules/.bin:$PATH"
    SARTHI_DESKTOP_SARTHI_ROOT="$root" \
    SARTHI_DESKTOP_PYTHON="$SARTHI_MAIN_CHECKOUT/.venv/bin/python" \
    SARTHI_DESKTOP_IGNORE_EXISTING=1 \
    SARTHI_DESKTOP_CWD="$root" \
    npm run dev )
}
```
...
|----------|----------------|
| `SARTHI_DESKTOP_SARTHI_ROOT` | Runs the backend from **this worktree**, not the packaged/PATH `sarthi`. |
| `SARTHI_DESKTOP_PYTHON` | Reuses the deps checkout's venv instead of re-resolving a Python. |
| `SARTHI_DESKTOP_IGNORE_EXISTING` | Ignores any `sarthi` on `PATH` so it can't shadow the worktree. |
| `SARTHI_DESKTOP_CWD` | Opens the desktop chat rooted at the worktree. |
...
```bash
# The enclosing worktree, verified as a real Sarthi checkout.
_sarthi_root() {
  local root
  root="$(git rev-parse --show-toplevel 2>/dev/null)" || return 1
  [[ -f "$root/sarthi_cli/main.py" && -d "$root/ui-tui" ]] && print -r "$root"
}
...
_sarthi_gui_cleanup() {
  ...
  lsof -t -i:5174 >/dev/null 2>&1 && killport 5174
  pgrep -f 'sarthi_cli\.main.*dashboard.*--port 0' 2>/dev/null | xargs -r kill -TERM 2>/dev/null
}
```
...
- [TUI](../user-guide/tui.md) — `sarthi --tui --dev` and the `SARTHI_TUI_DIR` prebuild path
...
- [Environment Variables](../reference/environment-variables.md) — every `SARTHI_*` variable Sarthi reads
