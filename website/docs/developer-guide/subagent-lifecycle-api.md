...
Plugins can launch and supervise fresh Sarthi child sessions without importing
...
active agent turn fails closed with `No active Sarthi parent session`.
...
overrides, and per-launch timeouts are explicitly rejected until Sarthi can
support them without weakening isolation. Use `allowed_toolsets` to narrow a
child; Sarthi's existing unsafe-tool block remains enforced.
