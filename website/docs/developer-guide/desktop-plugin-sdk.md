...
title: "Desktop Plugin SDK (@sarthi/plugin-sdk)"
description: "Extend the native Sarthi Desktop app — panes, pages, sidebar nav, status bar, palette commands, keybinds, themes, and a scoped backend namespace, with one import and no build step."
---

# Desktop Plugin SDK

The native [Sarthi Desktop](/user-guide/desktop) app is contribution-driven: every
...
A **desktop plugin** is a single ESM file that default-exports a `SarthiPlugin`.
It imports one module — `@sarthi/plugin-sdk` — and gets everything: the app's
...
`$SARTHI_HOME/desktop-plugins/<id>/plugin.js` and the app loads it within seconds
and hot-reloads every save.

:::warning This is not the web-dashboard plugin SDK
"Plugin" means several unrelated things across Sarthi. This page is the **native
desktop app** (`sarthi desktop`) SDK — the `@sarthi/plugin-sdk` module and
`$SARTHI_HOME/desktop-plugins/`. The **web dashboard** (`sarthi dashboard`) has
its own, unrelated plugin system on `window.__SARTHI_PLUGIN_SDK__` with a
`manifest.json` — documented at
[Extending the Dashboard](/user-guide/features/extending-the-dashboard). Python
CLI/gateway plugins are documented at [Build a Sarthi Plugin](/developer-guide/plugins).
...
|------|-------|-----|------------|
| **Disk** (recommended) | `$SARTHI_HOME/desktop-plugins/<id>/plugin.js` | users, agents | none — plain ESM, loaded uncompiled |
| **Unified package** | `$SARTHI_HOME/plugins/<id>/desktop/plugin.js` | plugins that also ship agent-side code | none — same disk pipeline |
| **Bundled** | `apps/desktop/src/plugins/<id>/plugin.tsx` | in-tree, shipped with the app | the app's own Vite build |

All three take the same `SarthiPlugin` contract, appear in **Settings → Plugins**,
...
Create `$SARTHI_HOME/desktop-plugins/hello/plugin.js` (that's `~/.sarthi/...`
by default, or `~/.sarthi/profiles/<name>/...` under a named profile). The folder
name must equal the plugin `id`.

```javascript
// ~/.sarthi/desktop-plugins/hello/plugin.js
import { host, haptic, useValue } from '@sarthi/plugin-sdk'
...
function HelloPane() {
  ...
  return jsxs('div', {
    ...
    children: [
      jsx('div', { className: 'font-medium', children: 'Hello, Sarthi' }),
      jsx('div', {
...
The only importable specifiers are `@sarthi/plugin-sdk`, `react`, and
`react/jsx-runtime` — everything else fails to resolve, on purpose.
:::

## The plugin contract

A plugin default-exports a `SarthiPlugin`:

```ts
interface SarthiPlugin {
...
interface PluginContext {
  ...
  os: PluginOs
  /** Plugin-scoped JSON persistence (keys live under `sarthi.plugin.<id>.`). */
...
```javascript
import { ROUTES_AREA, SIDEBAR_NAV_AREA } from '@sarthi/plugin-sdk'
...
```javascript
import { STATUSBAR_AREAS, TITLEBAR_AREAS } from '@sarthi/plugin-sdk'
...
```javascript
import { PALETTE_AREA, KEYBINDS_AREA } from '@sarthi/plugin-sdk'
...
```javascript
import { THEMES_AREA } from '@sarthi/plugin-sdk'
...
```javascript
import { Button, useTheme } from '@sarthi/plugin-sdk'
...
```javascript
import { host, requestTheme } from '@sarthi/plugin-sdk'
...
```javascript
import { TRANSCRIPT_DIRECTIVE_AREA } from '@sarthi/plugin-sdk'
...
`window.sarthi.send('get-price eth')` (or a declarative
`<button data-sarthi-send="get-price eth">` — no script needed) hands that
...
```javascript
import { Contribute, TITLEBAR_AREAS } from '@sarthi/plugin-sdk'
...
for requests; `targetProfile` is the backend Sarthi profile served by that route.
...
approval/turn alerts use. It fires only while the user is away from Sarthi
...
ctx.os.notify({
  ...
  // Body click → focus Sarthi + navigate. Same vocabulary as OS deep links:
  activate: 'sarthi://index-network/intent/1',
  ...
  actions: [
    { id: 'open', label: 'Open', activate: 'sarthi://index-network/intent/1' },
    { id: 'dismiss', label: 'Dismiss', onAction: () => dismiss('1') },
  ],
})
```

`activate` is deeplink-compatible: `sarthi://index-network/intent/1` and the
hash path `/index-network/intent/1` resolve to the same in-app route (and the
same `sarthi://…` URL works as an OS deep link). Action buttons only render on
...
```javascript
import { useQuery, useMutation, useQueryClient, atom, computed, useValue } from '@sarthi/plugin-sdk'
...
```javascript
import { queryClient } from '@sarthi/plugin-sdk'
...
a package into `~/.sarthi/plugins` is inert on every surface until the user
...
backend, the remote box's `~/.sarthi/plugins` is not reachable as a filesystem —
...
the `sarthi://` scheme — a plain anchor on your website or README:
...
`sarthi-dev://`. Full link reference:
...
`dashboard/` subfolder of a regular Sarthi plugin and declare it in a
`manifest.json`:
...
sarthi-agent codebase directly (`sarthi_state`, `sarthi_cli.config`, …). See
...
`config.yaml` (and not in `plugins.disabled`). Project plugins (`./.sarthi/`)
...
(`sarthi.plugin.<id>.*`) so plugins can't read or clobber each other:
...
export a `SarthiPlugin`). It's discovered by `discoverBundledPlugins()` at boot —
...
1. It goes through the app's Vite build, so you can write **real JSX** and import
   the SDK by its `@sarthi/plugin-sdk` alias.
2. It's still lint-fenced to `@sarthi/plugin-sdk` + `react` only — no `@/…` app
   internals.
...
- **Only three specifiers resolve:** `@sarthi/plugin-sdk`, `react`,
...
| Plugin contract | `SarthiPlugin`, `PluginContext`, `PluginContribution`, `PluginStorage`, `PluginOs`, `PluginRestOptions`, `PluginNativeNotificationInput`, `PluginNotificationAction`, `SarthiOpenTarget`, `Contribution` |
...
### Agents: the `sarthi-desktop-plugins` skill

When an agent writes a desktop plugin, it should load the bundled
**`sarthi-desktop-plugins`** skill — it carries the same contract as this page in
...
`$SARTHI_HOME/desktop-plugins/<id>/plugin.js` and the folder name matches the
export `id`. Run ⌘K → **Reload desktop plugins**. Check the app for an error
toast naming the failure, and tail `sarthi logs gui -f`.

**"unsupported import" on load.** A disk plugin may only import
`@sarthi/plugin-sdk`, `react`, and `react/jsx-runtime`. Remove any other import.
...
`~/.sarthi/plugins/<id>/dashboard/manifest.json` has `"api": "plugin_api.py"`,
that the plugin is in `plugins.enabled` in `config.yaml`, and restart the gateway
(backend routes mount at startup). Tail `~/.sarthi/logs/errors.log` for
