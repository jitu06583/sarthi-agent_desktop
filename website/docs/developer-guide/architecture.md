...
sidebar_position: 1
title: "Architecture"
description: "Sarthi Agent internals — major subsystems, execution paths, data flow, and where to read next"
---

# Architecture

This page is the top-level map of Sarthi Agent internals. Use it to orient yourself in the codebase, then dive into subsystem-specific docs for implementation details.
...
│ sarthi_state.py   │              │ Browser (5 backends)  │
...
├── cli.py                    # SarthiCLI — interactive terminal UI (large file)
├── model_tools.py            # Tool discovery, schema collection, dispatch
├── toolsets.py               # Tool groupings and platform presets
├── sarthi_state.py           # SQLite session/state database with FTS5
├── sarthi_constants.py       # SARTHI_HOME, profile-aware paths
...
│
├── sarthi_cli/               # CLI subcommands and setup
│   ├── main.py               # Entry point — all `sarthi` subcommands (large file)
...
│   ├── skills_config.py      # sarthi skills — enable/disable per platform
│   ├── skills_hub.py         # /skills slash command
│   ├── tools_config.py       # sarthi tools — enable/disable per platform
│   ├── plugins.py            # PluginManager — discovery, loading, hooks
│   ├── callbacks.py          # Terminal callbacks (clarify, sudo, approval)
│   └── gateway.py            # sarthi gateway start/stop
│
...
```text
User input → SarthiCLI.process_input()
...
Three discovery sources: `~/.sarthi/plugins/` (user), `.sarthi/plugins/` (project), and pip entry points. Plugins register tools, hooks, and CLI commands through a context API. Two specialized plugin types exist: memory providers (`plugins/memory/`) and context engines (`plugins/context_engine/`). Both are single-select — only one of each can be active at a time, configured via `sarthi plugins` or `config.yaml`.
...
Exposes Sarthi as an editor-native agent over stdio/JSON-RPC for VS Code, Zed, and JetBrains.
...
| **Profile isolation** | Each profile (`sarthi -p <name>`) gets its own SARTHI_HOME, config, memory, sessions, and gateway PID. Multiple profiles run concurrently. |
