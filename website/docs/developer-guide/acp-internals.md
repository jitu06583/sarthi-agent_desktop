...
The ACP adapter wraps Sarthi' synchronous `AIAgent` in an async JSON-RPC stdio server.
...
- reject options -> Sarthi `deny`

Timeouts and bridge failures deny by default.

### Tool rendering helpers

`acp_adapter/tools.py` maps Sarthi tools to ACP tool kinds and builds editor-facing content.
...
Instead it reuses Sarthi' runtime resolver:
...
- ACP sessions are persisted to the shared `~/.sarthi/state.db` (SessionDB) and transparently restored across process restarts; they appear in `session_search`
...
- `toolsets.py` — `sarthi-acp` toolset definition
- `sarthi_cli/main.py` — `sarthi acp` CLI subcommand
- `pyproject.toml` — `[acp]` optional dependency + `sarthi-acp` script
