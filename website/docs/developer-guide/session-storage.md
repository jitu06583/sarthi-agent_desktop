...
Sarthi Agent uses a SQLite database (`~/.sarthi/state.db`) to persist session
metadata, full message history, and model configuration across CLI and gateway
sessions. This replaces the earlier per-session JSONL file approach.

Source file: `sarthi_state.py`


## Architecture Overview

```
~/.sarthi/state.db (SQLite, WAL mode)
...
Abridged — see `SCHEMA_SQL` in `sarthi_state.py` for the full current column list
...
indexed columns — see `SCHEMA_SQL` in `sarthi_state.py` for the exact SQL.
...
Multiple sarthi processes (gateway + CLI sessions + worktree agents) share one
...
db = SessionDB()                           # Default: ~/.sarthi/state.db
...
Default path: `~/.sarthi/state.db`

This is derived from `sarthi_constants.get_sarthi_home()` which resolves to
`~/.sarthi/` by default, or the value of `SARTHI_HOME` environment variable.
