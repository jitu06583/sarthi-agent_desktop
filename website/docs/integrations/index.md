...
Sarthi Agent connects to external systems for AI inference, tool servers, IDE workflows, programmatic access, and more. These integrations extend what Sarthi can do and where it can run.
...
Sarthi supports multiple AI inference providers out of the box. Use `sarthi model` to configure interactively, or set them in `config.yaml`.

- **[AI Providers](/integrations/providers)** — OpenRouter, Anthropic, OpenAI, Google, and any OpenAI-compatible endpoint. Sarthi auto-detects capabilities like vision, streaming, and tool use per provider.
...
- **[MCP Servers](/user-guide/features/mcp)** — Connect Sarthi to external tool servers via Model Context Protocol. Access tools from GitHub, databases, file systems, browser stacks, internal APIs, and more without writing native Sarthi tools. Supports both stdio and SSE transports, per-server tool filtering, and capability-aware resource/prompt registration.

## Web Search Backends

The `web_search` and `web_extract` tools support eight backend providers, configured via `config.yaml` or `sarthi tools`:
...
If `web.backend` is not set, the backend is auto-detected from whichever API key is available. Self-hosted Firecrawl is also supported via `FIRECRAWL_API_URL`. Selecting Tavily in `sarthi tools` works without a key.

## Browser Automation

Sarthi includes full browser automation with multiple backend options for navigating websites, filling forms, and extracting information:
...
- **[IDE Integration (ACP)](/user-guide/features/acp)** — Use Sarthi Agent inside ACP-compatible editors such as VS Code, Zed, and JetBrains. Sarthi runs as an ACP server, rendering chat messages, tool activity, file diffs, and terminal commands inside your editor.

## Programmatic Access

- **[API Server](/user-guide/features/api-server)** — Expose Sarthi as an OpenAI-compatible HTTP endpoint. Any frontend that speaks the OpenAI format — Open WebUI, LobeChat, LibreChat, NextChat, ChatBox — can connect and use Sarthi as a backend with its full toolset.
...
Sarthi runs as a gateway bot on 27+ messaging platforms, all configured through the same `gateway` subsystem:
...
| **Slack** | [api.slack.com/apps?new_app=1](https://api.slack.com/apps?new_app=1) | The **Create New App** dialog — pick *From an app manifest* and paste the manifest `sarthi slack manifest --agent-view` generates |
...
- **[Buzz](/integrations/buzz)** — Block's Nostr-based human+agent workspace. Three integration paths: Buzz Desktop spawns Sarthi as a managed ACP runtime, the `buzz-acp` relay bridge hosts a Sarthi identity server-side, or the native gateway platform joins Buzz channels with full Sarthi memory/skills/approvals/cron. The overview page compares all three.
...
- **[Plugin System](/user-guide/features/plugins)** — Extend Sarthi with custom tools, lifecycle hooks, and CLI commands without modifying core code. Plugins are discovered from `~/.sarthi/plugins/`, project-local `.sarthi/plugins/`, and pip-installed entry points.
- **[Build a Plugin](/developer-guide/plugins)** — Step-by-step guide for creating Sarthi plugins with tools, hooks, and CLI commands.
