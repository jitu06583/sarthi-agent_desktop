...
| **OpenAI Codex** | `sarthi model` (ChatGPT OAuth, uses Codex models) |
...
If a token refresh fails with a terminal error (HTTP 4xx, `invalid_grant`, revoked grant, etc.), Sarthi marks the refresh token as dead and stops replaying it so you don't see a flood of identical auth failures. The next request surfaces a typed re-auth message instead. Run `sarthi auth add openai-codex` (or `sarthi model` → OpenAI Codex) to start a fresh device-code login; the quarantine clears on the next successful exchange.
:::
...
`VERTEX_PROJECT_ID` / `VERTEX_REGION` env vars override the `config.yaml` values. Install with `pip install 'sarthi-agent[vertex]'` (or let Sarthi lazy-install `google-auth` on first use). See the [Google Vertex AI guide](/guides/google-vertex) for the full walkthrough, and the [Google Gemini guide](/guides/google-gemini) for the static-API-key AI Studio path instead.
...
By default, Sarthi explicitly asks LM Studio to load the selected model with 64K context length before the first request.
...
custom_providers:
  ...
  - name: work
    ...
    api_mode: chat_completions   # set explicitly by `sarthi model` → Custom Endpoint wizard; auto-detection still happens as a fallback
...
The `sarthi model` → Custom Endpoint wizard now prompts for `api_mode` explicitly and persists your answer to `config.yaml`. URL-based auto-detection (e.g. `/anthropic` paths → `anthropic_messages`) still happens as a fallback when the field is left blank.
...
The cloud providers listed in [Other Compatible Providers](#other-compatible-providers) all speak OpenAI's REST dialect, so they wire up the same way under `custom_providers:`. Three worked recipes follow. Each drops into `~/.sarthi/config.yaml` and the matching API key goes in `~/.sarthi/.env`.
