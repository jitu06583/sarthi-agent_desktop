...
| 变量 | 描述 |
|----------|-------------|
| `SARTHI_MAX_ITERATIONS` | 每次对话的最大工具调用迭代次数（默认：90） |
...
| `SARTHI_TOOL_PROGRESS` | 工具进度显示的已弃用兼容变量。优先使用 `config.yaml` 中的 `display.tool_progress`。 |
| `SARTHI_TOOL_PROGRESS_MODE` | 工具进度模式的已弃用兼容变量。优先使用 `config.yaml` 中的 `display.tool_progress`。 |
...
| `SARTHI_BACKGROUND_NOTIFICATIONS` | gateway 中后台进程通知模式：`all`（默认）、`result`、`error`、`off` |
