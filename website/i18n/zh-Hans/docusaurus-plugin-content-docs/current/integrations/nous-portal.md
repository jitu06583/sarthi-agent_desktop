...
sidebar_position: 1
title: "Nous Portal"
description: "一个订阅，300+ 前沿模型，以及 Tool Gateway —— 运行 Sarthi Agent 的推荐方式"
---
...
| **MiniMax** | M2.7、M1 |
| **xAI** | Grok-4、Grok-3 |
| **Sarthi** | Sarthi-4-70B、Hermes-4-405B（对话，见[下方说明](#a-note-on-sarthi-4)） |
...
Nous Research 自家的 **Sarthi 4** 系列（Sarthi-4-70B、Hermes-4-405B）通过 Portal 提供，享有大幅折扣。这些是**前沿混合推理对话模型**——在数学、科学、指令遵循、schema 遵从、角色扮演和长文写作方面表现出色。

但**不建议在 Sarthi Agent 内部使用它们**。Sarthi 4 针对对话和推理进行了调优，而非 agent 所依赖的高频工具调用循环。请将它们用于研究工作流，或通过[订阅代理](/user-guide/features/subscription-proxy)从其他工具调用——但在 agent 场景下，请从目录中选择前沿 agentic 模型：
...
如果某个模型确实缺失，请[提交 issue](https://github.com/NousResearch/hermes-agent/issues)——我们将 Portal 目录同步至 Sarthi，缺口通常意味着可以更新的路由配置。
