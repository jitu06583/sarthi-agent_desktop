...
if command -v gh &>/dev/null && gh auth status &>/dev/null; then
  ...
  if [ -z "$GITHUB_TOKEN" ]; then
    if [ -f ~/.sarthi/.env ] && grep -q "^GITHUB_TOKEN=" ~/.sarthi/.env; then
      GITHUB_TOKEN=$(grep "^GITHUB_TOKEN=" ~/.sarthi/.env | head -1 | cut -d= -f2 | tr -d '\n\r')
...
curl -s -X POST \
  ...
  -d "{
    \"commit_id\": \"$HEAD_SHA\",
    \"event\": \"COMMENT\",
    \"body\": \"Code review from Sarthi Agent\",
    \"comments\": [
...
```bash
source "${SARTHI_HOME:-$HOME/.sarthi}/skills/github/github-auth/scripts/gh-env.sh"
...
```bash
# 若无问题——批准
gh pr review $PR_NUMBER --approve --body "Reviewed by Sarthi Agent. Code looks clean — good test coverage, no security concerns."
