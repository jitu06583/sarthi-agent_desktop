---
title: "Sarthi Agent — 配置、扩展或贡献 Sarthi Agent"
sidebar_label: "Sarthi Agent"
description: "配置、扩展或贡献 Sarthi Agent"
---
...
|---|---|
| 来源 | 内置（默认安装） |
| 路径 | `skills/autonomous-ai-agents/sarthi-agent` |
| 版本 | `2.1.0` |
| 作者 | Sarthi Agent + Teknium |
| 许可证 | MIT |
| 平台 | linux, macos, windows |
| 标签 | `sarthi`, `setup`, `configuration`, `multi-agent`, `spawning`, `cli`, `gateway`, `development` |
...
:::info
以下是 Sarthi 在触发此 skill 时加载的完整 skill 定义。这是 agent 在 skill 激活时看到的指令内容。
:::

# Sarthi Agent

Sarthi Agent 是 Nous Research 开发的开源 AI agent 框架，可在终端、消息平台和 IDE 中运行。它与 Claude Code（Anthropic）、Codex（OpenAI）和 OpenClaw 同属一类——使用工具调用（tool calling）与系统交互的自主编码和任务执行 agent。Sarthi 支持任意 LLM 提供商（OpenRouter、Anthropic、OpenAI、DeepSeek、本地模型及 15+ 其他提供商），可在 Linux、macOS 和 WSL 上运行。

Sarthi 的差异化特性：

- **通过 skill 自我提升** — Sarthi 通过将可复用流程保存为 skill 来从经验中学习。当它解决复杂问题、发现工作流或被纠正时，可以将该知识持久化为 skill 文档，加载到未来的会话中。skill 随时间积累，使 agent 在你的特定任务和环境中表现越来越好。
...
- **Profiles（配置文件）** — 运行多个独立的 Sarthi 实例，各自拥有隔离的配置、会话、skill 和记忆。
- **可扩展** — 插件、MCP 服务器、自定义工具、webhook 触发器、cron 调度以及完整的 Python 生态系统。

人们将 Sarthi 用于软件开发、研究、系统管理、数据分析、内容创作、家庭自动化，以及任何受益于具有持久上下文和完整系统访问权限的 AI agent 的场景。
...
sarthi auth                 交互式凭据管理器
sarthi auth add PROVIDER    添加 OAuth 或 API key 凭据（例如 nous、openai-codex、qwen-oauth）
sarthi auth list            列出已存储的凭据
sarthi auth remove PROVIDER 移除已存储的凭据
...
~/.sarthi/sessions/         Session transcripts
...
Profiles 使用 `~/.sarthi/profiles/<name>/`，布局相同。

### 配置节

使用 `sarthi config edit` 或 `sarthi config set section.key value` 编辑。
...
支持 20+ 个提供商。通过 `sarthi model` 或 `sarthi setup` 设置。
...
通过 `sarthi tools`（交互式）或 `sarthi tools enable/disable NAME` 启用/禁用。
...
完整枚举位于 `toolsets.py` 的 `TOOLSETS` 字典中；`_SARTHI_CORE_TOOLS` 是大多数平台继承的默认工具包。
...
常见的"为什么 Sarthi 对我的输出/工具调用/命令做了 X？"开关——以及更改它们的确切命令。其中大多数需要新会话（聊天中的 `/reset`，或启动新的 `sarthi` 调用），因为它们在启动时只读取一次。

### 工具输出中的密钥脱敏

密钥脱敏**默认关闭** — 工具输出（终端 stdout、`read_file`、网页内容、子 agent 摘要等）不经修改直接传递。如果用户希望 Sarthi 在 API key、token 和密钥进入对话上下文和日志之前自动屏蔽它们：

```bash
sarthi config set security.redact_secrets true       # 全局启用
```

**需要重启。** `security.redact_secrets` 在导入时快照 — 在会话中途切换（例如通过工具调用执行 `export SARTHI_REDACT_SECRETS=true`）对正在运行的进程**不会**生效。告知用户在终端运行 `sarthi config set security.redact_secrets true`，然后启动新会话。这是有意为之——防止 LLM 在任务中途自行切换该开关。
...
```bash
sarthi config set privacy.redact_pii true    # 启用
sarthi config set privacy.redact_pii false   # 禁用（默认）
```

### 命令审批提示

默认情况下（`approvals.mode: smart`），Sarthi 会让辅助 LLM 评估被标记为破坏性的 shell 命令（`rm -rf`、`git reset --hard` 等）。模式如下：
...
```bash
sarthi config set approvals.mode smart       # 推荐的折中方案
sarthi config set approvals.mode off         # 绕过一切（不推荐）
```
...
某些 shell hook 集成在触发前需要明确加入允许列表。通过 `~/.sarthi/shell-hooks-allowlist.json` 管理——在 hook 首次尝试运行时以交互方式提示。

### 禁用 web/browser/image-gen 工具

要完全阻止模型访问网络或媒体工具，打开 `sarthi tools` 并按平台切换。在下次会话（`/reset`）后生效。参见上方的工具与 Skill 部分。
...
将额外的 Sarthi 进程作为完全独立的子进程运行——拥有独立的会话、工具和环境。

### 何时使用此方式 vs delegate_task

| | `delegate_task` | 生成 `sarthi` 进程 |
|-|-----------------|--------------------------|
...
Sarthi 使用 prompt_toolkit，需要真实终端。使用 tmux 进行交互式生成：
...
- **fire-and-forget 使用 `sarthi chat -q`** — 无需 PTY
...
持久化调度器——`cron/jobs.py` + `cron/scheduler.py`。通过 `cronjob` 工具、`sarthi cron` CLI（`list`、`add`、`edit`、`pause`、`resume`、`run`、`remove`）或 `/cron` 斜杠命令驱动。
...
- **CLI：** `sarthi curator <verb>` — `status`、`run`、`pause`、`resume`、`pin`、`unpin`、`archive`、`restore`、`prune`、`backup`、`rollback`。
...
用于多 profile/多 worker 协作的持久化 SQLite 看板（kanban）。用户通过 `sarthi kanban <verb>` 驱动；调度器生成的 worker 看到由 `SARTHI_KANBAN_TASK` 控制的专注 `kanban_*` toolset，orchestrator profile 可以选择加入更广泛的 `kanban` toolset。普通会话除非配置，否则没有任何 `kanban_*` schema 占用。
...
Sarthi 在 Windows 上原生运行（PowerShell、cmd、Windows Terminal、git-bash mintty、VS Code 集成终端）。大多数功能开箱即用，但 Win32 和 POSIX 之间有一些差异曾给我们带来麻烦——遇到新问题时请在此记录，以免下一个人（或下一个会话）重新踩坑。
...
**正斜杠几乎在所有地方都有效。** `C:/Users/...` 被每个 Sarthi 工具和大多数 Windows API 接受。在代码和日志中优先使用正斜杠——避免在 bash 中转义反斜杠。
...
### 工具不可用
1. `sarthi tools` — 检查 toolset 是否为你的平台启用
2. 某些工具需要环境变量（检查 `.env`）
3. 启用工具后执行 `/reset`

### 模型/提供商问题
1. `sarthi doctor` — 检查配置和依赖
2. `sarthi auth` — 重新认证 OAuth 提供商（或 `sarthi auth add <provider>`）
3. 检查 `.env` 中是否有正确的 API key
4. **Copilot 403**：`gh auth login` 的 token **不适用于** Copilot API。必须通过 `sarthi model` → GitHub Copilot 使用 Copilot 专用 OAuth 设备码流程。
...
### Skill 未显示
1. `sarthi skills list` — 验证已安装
2. `sarthi skills config` — 检查平台启用状态
3. 显式加载：`/skill name` 或 `sarthi -s name`
...
- **Gateway 崩溃循环**：重置失败状态：`systemctl --user reset-failed sarthi-gateway`
...
配置：`~/.sarthi/config.yaml`（设置）、`~/.sarthi/.env`（API key）。
...
**2. 添加到 `toolsets.py`** → `_SARTHI_CORE_TOOLS` 列表。

自动发现：任何包含顶层 `registry.register()` 调用的 `tools/*.py` 文件都会自动导入——无需手动列出。

所有处理器必须返回 JSON 字符串。路径使用 `get_sarthi_home()`，永远不要硬编码 `~/.sarthi`。

### 添加斜杠命令

1. 在 `sarthi_cli/commands.py` 的 `COMMAND_REGISTRY` 中添加 `CommandDef`
...
- 测试自动将 `SARTHI_HOME` 重定向到临时目录——永远不会触及真实的 `~/.sarthi/`
- 推送任何变更前运行完整套件
...
- 所有路径使用 `sarthi_constants` 中的 `get_sarthi_home()`（profile 安全）
