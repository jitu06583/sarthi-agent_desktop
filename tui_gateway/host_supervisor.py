...
def _default_registry_path() -> Path:
    return get_sarthi_home() / "state" / _REGISTRY_NAME
...
class HostSupervisor:
    ...
    def __init__(
        ...
        *,
        ...
        expected_sarthi_home: str | None = None,
        autostart: bool = True,
    ) -> None:
        ...
        self.expected_sarthi_home = expected_sarthi_home if expected_sarthi_home is not None else str(get_sarthi_home())
    ...
    def _spawn_locked(self, *, reason: str) -> None:
        ...
        if self.env:
            env.update(self.env)
        env["SARTHI_COMPUTE_HOST_HEARTBEAT_SECS"] = str(self.heartbeat_secs)
    ...
    def _validate_hello(self) -> None:
        ...
        got_home = str(hello.get("sarthi_home") or "")
        if got_home and got_home != self.expected_sarthi_home:
            raise RuntimeError(f"compute host SARTHI_HOME mismatch: {got_home} != {self.expected_sarthi_home}")
