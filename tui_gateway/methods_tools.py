...
def _(rid, params: dict) -> dict:
    ...
    try:
        ...
        # Re-bind SARTHI_HOME to the session's profile so get_skill_commands()
        ...
        _home_token = (
            set_sarthi_home_override(_profile_home) if _profile_home else None
        )
        try:
...
def _(rid, params: dict) -> dict:
    try:
        ...
        api_key = get_secret("SARTHI_API_KEY", "") or cfg.get("api_key", "")
...
def _(rid, params: dict) -> dict:
    ...
    # Optional profile scoping: cronjob() keys off SARTHI_HOME, so scoping the
    ...
    if profile:
        try:
            ...
            from sarthi_constants import set_sarthi_home_override
    ...
    try:
        ...
        if token is not None:
            try:
                from sarthi_constants import reset_sarthi_home_override
...
def _(rid, params: dict) -> dict:
    ...
    keys], transport}]}`` — the same catalog `sarthi mcp` offers, so
...
# (sarthi_cli/web_routers/mcp.py) so a desktop plugin can manage MCP servers for
# ANY profile, not just the launch profile. Each accepts an optional ``profile``
# param that scopes SARTHI_HOME via set_sarthi_home_override (omitted/None = the
# launch profile) in a try/finally, exactly like ``skills.manage`` / ``mcp.catalog``.
# All persistence reuses sarthi_cli/mcp_config.py helpers — no logic is duplicated.
...
def _(rid, params: dict) -> dict:
    ...
    if err:
        return err
    try:
        from sarthi_cli.config import load_config, save_config, save_env_value
        from sarthi_cli.mcp_config import (
...
def _(rid, params: dict) -> dict:
    ...
    (``window.sarthiDesktop.openExternal``) and then polls
    ...
    the SAME interactive MCP OAuth machinery ``sarthi mcp login`` uses
    ...
    try:
        ...
        sarthi_home = str(get_sarthi_home().expanduser().resolve(strict=False))
        result = mcp_oauth_sessions.start_flow(sarthi_home, name, cfg)
        return _ok(
            rid,
            {
                "ok": True,
...
def _(rid, params: dict) -> dict:
    ...
    Actions:
      ...
      - ``install`` → git-clone into ``~/.sarthi/plugins/`` (non-interactive).
    ...
    plugins live under each profile's SARTHI_HOME, so a client can list or
                ...
                out.append(
                    {
                        ...
                        # skills/MCP format) vs a native Sarthi plugin.
