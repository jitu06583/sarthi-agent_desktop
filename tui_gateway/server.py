...
class _SlashWorker:
    ...
    def __init__(self, session_key: str, model: str, profile_home: str | None = None):
        ...
        env = sarthi_subprocess_env(inherit_credentials=True)
        if profile_home:
            ...
            # gateway's launch SARTHI_HOME (#40677).
            env["SARTHI_HOME"] = str(profile_home)
...
def _finalize_session(session: dict | None, end_reason: str = "tui_close") -> None:
    ...
    if agent is not None:
        try:
            from sarthi_cli.plugins import invoke_hook
...
def _profile_scoped(handler):
    """Bind ``params['profile']``'s SARTHI_HOME around a pet RPC handler.

    Pets are per-profile: ``display.pet.*`` lives in the profile's config.yaml and
    sprites install under its ``pets/`` dir (both resolve via ``get_sarthi_home``).
...
def _save_cfg(cfg: dict):
    ...
    from sarthi_cli.config import atomic_config_write
...
def _load_approval_mode() -> str:
    from sarthi_cli.config import DEFAULT_CONFIG, _deep_merge
...
def _load_enabled_toolsets() -> list[str] | None:
    ...
    try:
        ...
        if not enabled:
            return None
        # The desktop Project tools are off _SARTHI_CORE_TOOLS (every other
...
def _available_personalities(cfg: dict | None = None) -> dict:
    try:
        ...
        try:
            from sarthi_cli.config import load_config as _load_full_cfg
...
def _cfg_max_turns(cfg: dict, default: int) -> int:
    try:
        env_max = int(os.environ.get("SARTHI_TUI_MAX_TURNS", "") or 0)
        if env_max > 0:
            return env_max
    except (TypeError, ValueError):
        pass
...
def _(rid, params: dict) -> dict:
    ...
    # and each turn re-bind SARTHI_HOME. None/own profile → launch (unchanged).
...
def _(rid, params: dict) -> dict:
    ...
    try:
        ...
        # custom `SARTHI_SESSION_SOURCE` values, and older installs with
...
def _(rid, params: dict) -> dict:
    ...
    _enable_gateway_prompts()
    home_token = (
        set_sarthi_home_override(str(profile_home)) if profile_home is not None else None
    )
    try:
        db.reopen_session(target)
        raw_history = db.get_messages_as_conversation(target)
    ...
    with _session_resume_lock:
        ...
        try:
            init_home_token = (
                set_sarthi_home_override(str(profile_home))
                if profile_home is not None
                else None
            )
            try:
                ...
                if init_home_token is not None:
                    reset_sarthi_home_override(init_home_token)
            if sid in _sessions:
                ...
                # Remember the profile home so each turn re-binds SARTHI_HOME (the
...
def _(rid, params: dict) -> dict:
    ...
    # ``sarthi --tui`` session legitimately rides the real stdio transport and
    # must stay visible.
...
def _(rid, params: dict) -> dict:
    ...
    transfer is performed by the separate ``sarthi gateway`` process, whose
...
def _(rid, params: dict) -> dict:
    ...
    try:
        ...
        from sarthi_cli.pets import _set_active
...
def _(rid, params: dict) -> dict:
    ...
    try:
        from agent.pet import store
        from sarthi_cli.pets import _clear_active_if
...
def _(rid, params: dict) -> dict:
    ...
    try:
        ...
        if new_slug != slug:
            try:
                from sarthi_cli.pets import _rename_active_if
...
def _(rid, params: dict) -> dict:
    ...
    try:
        from sarthi_cli.pets import _set_enabled
...
def _(rid, params: dict) -> dict:
    ...
    """
    try:
        from sarthi_cli.pets import set_pet_scale
...
# (agent/billing_view.py + sarthi_cli/nous_billing.py) — same as /credits.
...
def _(rid, params: dict) -> dict:
    ...
    """
    from sarthi_cli.nous_billing import BillingError, post_charge
...
def _(rid, params: dict) -> dict:
    ...
    """
    from sarthi_cli.nous_billing import BillingError, get_charge_status
...
def _(rid, params: dict) -> dict:
    ...
    """
    from sarthi_cli.nous_billing import BillingError, patch_auto_top_up
...
def _(rid, params: dict) -> dict:
    ...
    agent = session["agent"]
    # Mirror the classic CLI /save: snapshot under the Sarthi profile home
    # (~/.sarthi/sessions/saved/) rather than the project/workspace CWD, and
...
def _(rid, params: dict) -> dict:
    from sarthi_cli.input_sanitize import sanitize_user_prompt_text
...
def _(rid, params: dict) -> dict:
    ...
    if err:
        return err
    try:
        from sarthi_cli.clipboard import has_clipboard_image, save_clipboard_image
    except Exception as e:
        return _err(rid, 5027, f"clipboard unavailable: {e}")

    session["image_counter"] = session.get("image_counter", 0) + 1
    img_dir = _sarthi_home / "images"
...
def _desktop_attachment_dir(session: dict) -> Path:
    root = Path(_session_cwd(session)).resolve() / ".sarthi" / "desktop-attachments"
    root.mkdir(parents=True, exist_ok=True)
    return root
    ...
    *,
    ...
    Three cases:
      ...
      2. The path resolves to a gateway-visible file OUTSIDE the workspace — copy
         it into ``.sarthi/desktop-attachments/`` so the ``@file:`` ref resolves.
      3. The path doesn't exist on the gateway (the common remote case: it's a
         ...
         write them into ``.sarthi/desktop-attachments/``.
...
def _(rid, params: dict) -> dict:
    ...
    prompt = "\n".join(
        ...
        for line in [
            ...
            "Restart exactly the app intended for the Preview URL, not Sarthi Desktop itself.",
            ...
            "First inspect what process, if any, owns the Preview URL port. If a stale server exists, inspect its cwd and prefer that cwd over the Sarthi/Desktop process cwd.",
            ...
            "Before declaring success, verify the Preview URL responds with the intended app, not Sarthi Desktop. If it serves Sarthi/Desktop UI or another unrelated app, stop that process and report failure.",
    ...
    if key == "model":
        try:
            ...
            if session:
                ...
                from sarthi_cli.model_switch import parse_model_flags
...
def _is_repo_junk(root: str) -> bool:
    ...
    anything under SARTHI_HOME (~/.sarthi by default) — config/sessions/skills,
    ...
    return real == home or real == sarthi_home or real.startswith(sarthi_home + os.sep)
...
def _is_session_cwd_junk(cwd: str) -> bool:
    ...
    return real == home or real == sarthi_home
...
def _(rid, params: dict) -> dict:
    ...
    if key == "provider":
        try:
            from sarthi_cli.models import list_available_providers, normalize_provider
    ...
    if key == "profile":
        ...
        return _ok(rid, {"home": str(_sarthi_home), "display": display_sarthi_home()})
    if key == "project":
    ...
    if key == "mtime":
        cfg_path = _sarthi_home / "config.yaml"
        try:
            return _ok(
...
def _(rid, params: dict) -> dict:
    try:
        from sarthi_cli.main import _has_any_provider_configured
...
def _(rid, params: dict) -> dict:
    ...
    try:
        from sarthi_cli.runtime_provider import resolve_runtime_provider
        from sarthi_cli.auth import has_usable_secret
        from sarthi_cli.main import _has_any_provider_configured
        ...
        if not provider_configured and provider == "bedrock" and source in {
            ...
            return _ok(
                rid,
                {
                    ...
                    "source": source,
                    "error": "No Sarthi provider is configured.",
                },
            )
...
@method("reload.env")
def _(rid, params: dict) -> dict:
    """Re-read ``~/.sarthi/.env`` into the gateway process via
    ``sarthi_cli.config.reload_env``, matching classic CLI's ``/reload``
...
def _(rid, params: dict) -> dict:
    ...
    try:
        from sarthi_cli.commands import (
            COMMAND_REGISTRY,
            SUBCOMMANDS,
            _build_description,
        )
...
@method("cli.exec")
def _(rid, params: dict) -> dict:
    """Run `python -m sarthi_cli.main` with argv; capture stdout/stderr (non-interactive only)."""
    ...
    try:
        r = subprocess.run(
            [sys.executable, "-m", "sarthi_cli.main", *argv],
            capture_output=True,
            text=True,
            timeout=min(int(params.get("timeout", 240)), 600),
            cwd=os.getcwd(),
            # cli.exec runs `python -m sarthi_cli.main` (can drive the agent) →
...
def _(rid, params: dict) -> dict:
    ...
    if name == "moa":
        ...
        try:
            from sarthi_cli.moa_config import moa_usage, normalize_moa_config
...
def _(rid, params: dict) -> dict:
    ...
    try:
        from sarthi_cli.commands import SlashCommandCompleter
...
def _(rid, params: dict) -> dict:
    ...
    try:
        ...
        from sarthi_cli.config import is_managed, save_env_value
        from sarthi_cli.inventory import build_models_payload, load_picker_context
        ...
        if pconfig.auth_type != "api_key":
            return _err(
                ...
                f"run `sarthi model` to configure",
            )
        if not pconfig.api_key_env_vars:
            return _err(rid, 4004, f"no env var defined for {pconfig.name}")

        # Save the key to ~/.sarthi/.env
...
def _(rid, params: dict) -> dict:
    ...
    """
    try:
        from sarthi_cli.auth import PROVIDER_REGISTRY, clear_provider_auth
        from sarthi_cli.config import remove_env_value
    ...
    try:
        ...
        if parsed.scheme in {"ws", "wss"} and parsed.path.startswith(
            ...
            if not ok and _is_default_local_cdp(parsed):
                from sarthi_cli.browser_connect import launch_chrome_debug
...
def _(rid, params: dict) -> dict:
    try:
        cfg = _load_cfg()
        model = _resolve_model()
        api_key = os.environ.get("SARTHI_API_KEY", "") or cfg.get("api_key", "")
        masked = f"****{api_key[-4:]}" if len(api_key) > 4 else "(not set)"
        base_url = os.environ.get("SARTHI_BASE_URL", "") or cfg.get("base_url", "")
        ...
        sections = [
            ...
            },
            {
                ...
                "rows": [
                    ...
                    ["Config File", str(_sarthi_home / "config.yaml")],
                ],
            },
        ]
...
def _(rid, params: dict) -> dict:
    ...
    gateway. Shares its renderer with the ``sarthi journey`` CLI.
    """
    try:
...
def _(rid, params: dict) -> dict:
    ...
    try:
        ...
        if action == "install":
            from sarthi_cli.skills_hub import do_install
        ...
        if action == "browse":
            from sarthi_cli.skills_hub import browse_skills
        ...
        if action == "inspect":
            from sarthi_cli.skills_hub import inspect_skill
...
def _(rid, params: dict) -> dict:
    ...
    primitives as ``sarthi plugins`` / the dashboard, so the three surfaces
