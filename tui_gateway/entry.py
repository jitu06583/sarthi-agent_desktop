...
# from shadowing Sarthi's own top-level modules.  ``sarthi_bootstrap`` lives at
...
import sarthi_bootstrap

sarthi_bootstrap.harden_import_path()
...
# shared owner in sarthi_cli.mcp_startup — the wait/in-flight/join helpers
...
# sarthi_cli.mcp_startup.start_background_mcp_discovery can actually fire —
...
def _install_sidecar_publisher() -> None:
    ...
    Activated by `SARTHI_TUI_SIDECAR_URL`, set by the dashboard's
    ...
    """
    url = os.environ.get("SARTHI_TUI_SIDECAR_URL")
...
# tested; override via ``SARTHI_TUI_GATEWAY_SHUTDOWN_GRACE_S`` if a
...
def _shutdown_grace_seconds() -> float:
    raw = (os.environ.get("SARTHI_TUI_GATEWAY_SHUTDOWN_GRACE_S") or "").strip()
    if not raw:
...
def _log_signal(signum: int, frame) -> None:
    ...
    (``SARTHI_TUI_GATEWAY_SHUTDOWN_GRACE_S``, default
...
# ``sarthi --tui``) imports cleanly there.  SIGBREAK (Windows' Ctrl+Break)
...
def wait_for_mcp_discovery(timeout: "float | None" = None) -> None:
    ...
    CLI path via ``sarthi_cli.mcp_startup``); ``timeout`` overrides it.
    """
    thread = _mcp_discovery_thread
    if thread is not None and thread.is_alive():
        try:
            from sarthi_cli.mcp_startup import _resolve_discovery_timeout
    ...
    # → sarthi_cli.mcp_startup); wait on it so the first agent build still
    ...
    # session profile's SARTHI_HOME first), so a launch profile with no
    ...
    try:
        from sarthi_cli.mcp_startup import (
...
def mcp_discovery_in_flight() -> bool:
    ...
    ``sarthi --tui`` path spawns ITS thread here (``_mcp_discovery_thread``),
    while the desktop app + dashboard WebSocket sidecar (``tui_gateway/ws.py``)
    and ``sarthi dashboard`` spawn theirs via
    ``sarthi_cli.mcp_startup.start_background_mcp_discovery``. The late-refresh
...
def join_mcp_discovery(timeout: float | None = None) -> bool:
    ...
    entry thread first, then the ``sarthi_cli.mcp_startup`` thread used by the
    ...
    try:
        from sarthi_cli.mcp_startup import join_mcp_discovery as _startup_join
...
def _has_configured_mcp_servers() -> bool:
    ...
    from sarthi_cli.mcp_startup import _has_configured_mcp_servers as configured
...
def ensure_mcp_discovery_started() -> None:
    ...
    binding the session profile's SARTHI_HOME override — the shared owner in
    ``sarthi_cli.mcp_startup`` captures the caller's context-local override
...
def main():
    ...
    # Live-apply skins Sarthi activates mid-conversation.
