...
handlers delegate to the same `sarthi_cli.profiles` primitives the REST
endpoints use.
...
def _(rid, params: dict) -> dict:
    """List Sarthi profiles (name, path, model, description, skill count).
    ...
    def _canonical_session_row(profile_path):
        ...
        describe the same session (sarthi-agent#88200) with no client-side
        pointer involved.
    ...
    def _latest_profile_session_rows(profile_path):
        ...
        sessions never surface in conversation lists (sarthi-agent#90268).
    ...
    try:
        ...
        # append the protocol to SOUL.md (the desktop's sarthi-bots plugin)
...
def _(rid, params: dict) -> dict:
    ...
    ``sarthi setup`` in that flow to recover. A profile spawned as an
    ...
    # instead (sarthi_cli.auth: profile reads fall back to the global
    ...
    def _mirror_voice_sections() -> bool:
        ...
        profile via the context-local SARTHI_HOME override — the same
        ...
        try:
            ...
            token = set_sarthi_home_override(str(path))
            try:
    ...
    if model and provider:
        try:
            from sarthi_cli.web_routers.profiles import _write_profile_model
        ...
        try:
            from sarthi_cli.config import load_config_readonly, read_user_config_raw
            from sarthi_cli.web_routers.profiles import _write_profile_model
            from sarthi_constants import (
                reset_sarthi_home_override,
                set_sarthi_home_override,
...
def _(rid, params: dict) -> dict:
    ...
    All reads are scoped to the profile via the SARTHI_HOME override.
    """
    name = str(params.get("name") or "").strip()
    if not name:
    ...
    try:
        ...
        try:
            ...
            # Toolsets: the same filtered universe the `sarthi tools`
            ...
            # composites (sarthi-discord, feishu_drive, ...) and reports
            ...
            try:
                ...
                platform_enabled = set()
            try:
                from sarthi_cli.tools_config import _DEFAULT_OFF_TOOLSETS
            except Exception:
            ...
            for ts_name, ts_label, ts_desc in _get_effective_configurable_toolsets():
                ...
                # a per-profile editor — `sarthi tools` / Settings is where
            ...
            description = ""
            try:
                from sarthi_cli.profiles import read_profile_meta
...
def _(rid, params: dict) -> dict:
    ...
    try:
        ...
        if isinstance(params.get("description"), str):
            try:
                from sarthi_cli.profiles import write_profile_meta
        ...
        if needs_cfg:
            ...
            try:
                ...
                if isinstance(params.get("disabled_skills"), list):
                    try:
                        from sarthi_cli.skills_config import save_disabled_skills
