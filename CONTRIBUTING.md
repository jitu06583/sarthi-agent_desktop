# Contributing to Sarthi Agent

Thank you for contributing to Sarthi Agent! This guide covers everything you need: setting up your dev environment, understanding the architecture, deciding what to build, and getting your PR merged.
...
2. **Cross-platform compatibility** — macOS, different Linux distros, and WSL2 on Windows. We want Sarthi to work everywhere.
...
Related: #38284 covers the agent-side analog — Sarthi itself checking existing issues and PRs before deep self-troubleshooting. This section is the human-contributor complement.
...
Bundled skills (in `skills/`) ship with every Sarthi install. They should be **broadly useful to most users**:

- Document handling, web research, common dev workflows, system administration
- Used regularly by a wide range of people

If your skill is official and useful but not universally needed (e.g., a paid service integration, a heavyweight dependency), put it in **`optional-skills/`** — it ships with the repo but isn't activated by default. Users can discover it via `sarthi skills browse` (labeled "official") and install it with `sarthi skills install` (no third-party warning, built-in trust).

If your skill is specialized, community-contributed, or niche, it's better suited for a **Skills Hub** — upload it to a skills registry and share it in the [Nous Research Discord](https://discord.gg/NousResearch). Users can install it with `sarthi skills install`.

---

## Memory Providers: Ship as a Standalone Plugin

**We are no longer accepting new memory providers into this repo.** The set of built-in providers under `plugins/memory/` (honcho, mem0, supermemory, byterover, hindsight, holographic, openviking, retaindb) is closed. If you want to add a new memory backend, publish it as a **standalone plugin repo** that users install into `~/.sarthi/plugins/` (or via a pip entry point).

Standalone memory plugins:

- Implement the same `MemoryProvider` ABC (`agent/memory_provider.py`) — `sync_turn`, `prefetch`, `shutdown`, and optionally `post_setup(sarthi_home, config)` for setup-wizard integration
...
- Integrate with `sarthi memory setup` via `post_setup()` — no need to touch core code
...
The installer creates the Sarthi venv, wires the `sarthi` command, stamps the
install method for `sarthi update`, and clones the full git project into
`$SARTHI_HOME/sarthi-agent` (usually `~/.sarthi/sarthi-agent`). That keeps your
...
Use this only if you intentionally do not want Sarthi' managed install layout
(for example, a throwaway clone inside a container or CI job). If you install
this way, make sure you run the `sarthi` entrypoint from this venv; running the
system `python3 -m sarthi_cli.main` can pick up unrelated system Python
packages.
...
```bash
mkdir -p ~/.sarthi/{cron,sessions,logs,memories,skills}
cp cli-config.yaml.example ~/.sarthi/config.yaml
touch ~/.sarthi/.env

# Add at minimum an LLM provider key:
echo "OPENROUTER_API_KEY=***" >> ~/.sarthi/.env
```

### Run

```bash
# The standard installer already put `sarthi` on PATH.
sarthi doctor
sarthi chat -q "Hello"
```

If you used the manual clone fallback, run `./sarthi` from the checkout or
symlink this clone's venv explicitly:

```bash
mkdir -p ~/.local/bin
ln -sf "$(pwd)/venv/bin/sarthi" ~/.local/bin/sarthi
```
...
├── cli.py                    # SarthiCLI class — interactive TUI, prompt_toolkit integration
├── model_tools.py            # Tool orchestration (thin layer over tools/registry.py)
├── toolsets.py               # Tool groupings and presets (sarthi-cli, sarthi-telegram, etc.)
├── sarthi_state.py           # SQLite session database with FTS5 full-text search, session titles
...
│
├── sarthi_cli/               # CLI command implementations
...
### User configuration (stored in `~/.sarthi/`)

| Path | Purpose |
|------|---------|
| `~/.sarthi/config.yaml` | Settings (model, terminal, toolsets, compression, etc.) |
| `~/.sarthi/.env` | API keys and secrets |
| `~/.sarthi/auth.json` | OAuth credentials (Nous Portal) |
| `~/.sarthi/skills/` | All active skills (bundled + hub-installed + agent-created) |
| `~/.sarthi/memories/` | Persistent memory (MEMORY.md, USER.md) |
| `~/.sarthi/state.db` | SQLite session database |
| `~/.sarthi/sessions/` | Gateway routing index (`sessions.json`), request-dump breadcrumbs, gateway `*.jsonl` transcripts, and (optionally) per-session JSON snapshots when `sessions.write_json_snapshots: true` is set. The per-session snapshots are off by default; state.db is canonical. |
| `~/.sarthi/cron/` | Scheduled job data |
| `~/.sarthi/whatsapp/session/` | WhatsApp bridge credentials |
...
- **Session persistence**: All conversations are stored in SQLite (`sarthi_state.py`) with full-text search and unique session titles. Per-session JSON snapshots in `~/.sarthi/sessions/` were superseded by the SQLite store and are off by default; opt back in with `sessions.write_json_snapshots: true` if you have external tooling that consumes the JSON files directly.
...
(for example `_SARTHI_CORE_TOOLS` or a dedicated toolset); otherwise the tool
...
Four fields are supported under `metadata.sarthi`:
...
The user may skip setup and keep loading the skill. Sarthi only exposes metadata (`stored_as`, `skipped`, `validated`) to the model — never the secret value.
...
Gateway and messaging sessions never collect secrets in-band; they instruct the user to run `sarthi setup` or update `~/.sarthi/.env` locally.
...
2. **Tools referenced in SKILL.md prose must be native Sarthi tools or MCP servers the skill explicitly expects.** When the skill needs a capability, point at the proper tool by name in backticks: `` `terminal` ``, `` `web_extract` ``, `` `web_search` ``, `` `read_file` ``, `` `write_file` ``, `` `patch` ``, `` `search_files` ``, `` `vision_analyze` ``, `` `browser_navigate` ``, `` `delegate_task` ``, `` `image_generate` ``, `` `text_to_speech` ``, `` `cronjob` ``, `` `memory` ``, `` `skill_view` ``, `` `todo` ``, `` `execute_code` ``.
...
4. **`author` credits the human contributor first.** For external contributions, the contributor's real name + GitHub handle goes first (`Jane Doe (jane-doe)`); "Sarthi Agent" is the secondary collaborator. If the contributor's commit shows "Sarthi Agent" as author because they used Sarthi to draft the skill, replace it with their actual name — credit the human, not the tool.
...
- **No external dependencies unless absolutely necessary.** Prefer stdlib Python, curl, and existing Sarthi tools (`web_extract`, `terminal`, `read_file`).
...
- **Test it.** Run `sarthi --toolsets skills -q "Use the X skill to do Y"` and verify the agent follows the instructions correctly.

---

## Adding a Skin / Theme

Sarthi uses a data-driven skin system — no code changes needed to add a new skin.

**Option A: User skin (YAML file)**

Create `~/.sarthi/skins/<name>.yaml`:
...
Add to `_BUILTIN_SKINS` dict in `sarthi_cli/skin_engine.py`. Use the same schema as above but as a Python dict. Built-in skins ship with the package and are always available.

**Activating:**
- CLI: `/skin mytheme` or set `display.skin: mytheme` in config.yaml
- Config: `display: { skin: mytheme }`

See `sarthi_cli/skin_engine.py` for the full schema and existing skins as examples.

---

## Cross-Platform Compatibility

Sarthi runs on Linux, macOS, and native Windows (plus WSL2). When writing code
...
1. **Never call `os.kill(pid, 0)` for liveness checks.** `os.kill(pid, 0)`
   ...
   **Preferred:** use `psutil` (a core dependency — always available):
   ...
   If you specifically need the sarthi wrapper (it has a stdlib fallback
...
15. **Two different quoting schemes in one command line.** `subprocess.run
    ...
    helpers and never cross them. See `sarthi_cli/gateway_windows.py::
...
Sarthi has terminal access. Security matters.
...
2. **Test manually**: Run `sarthi` and exercise the code path you changed
