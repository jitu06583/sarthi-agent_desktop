...
Existing call sites in `sarthi_cli/main.py` remain for legacy fallback only;
the preferred UI is curses (stdlib) because `simple_term_menu` has
ghost-duplication rendering bugs in tmux/iTerm2 with arrow keys. New
interactive menus must use `sarthi_cli/curses_ui.py` — see
`sarthi_cli/tools_config.py` for the canonical pattern.
