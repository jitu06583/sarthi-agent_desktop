# OpenAI-Compatible API Server for Sarthi Agent
...
SSE streaming. By exposing this endpoint, sarthi-agent becomes instantly
...
│  AnythingLLM     │  │    {"messages": [...]}             │  sarthi-agent   │
│  NextChat        │  │                                    │  gateway        │
...
A user would:
1. Set `API_SERVER_ENABLED=true` in `~/.sarthi/.env`
2. Run `sarthi gateway` (API server starts alongside Telegram/Discord/etc.)
3. Point Open WebUI (or any frontend) at `http://localhost:8642/v1`
4. Chat with sarthi-agent through any OpenAI-compatible UI
...
| GET | `/v1/models` | List available "models" (returns sarthi-agent as a model) |
...
Authorization: Bearer sarthi-api-key-here
...
But sarthi-agent sessions have persistent state (memory, skills, tool context).
...
Frontends send `"model": "sarthi-agent"` (or whatever). The actual LLM model
used is configured server-side in config.yaml. The API server maps any
requested model name to the configured sarthi-agent model.
...
1. `gateway/platforms/api_server.py` — new adapter
   ...
   - Frontend system prompt layered on top of sarthi-agent's core prompt
...
Once implemented, sarthi-agent works as a drop-in backend for:
