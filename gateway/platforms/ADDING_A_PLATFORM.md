...
There are two ways to add a platform to the Sarthi gateway:

## Plugin Path (Recommended for Community/Third-Party)

Create a plugin directory in `~/.sarthi/plugins/` (or under `plugins/platforms/`
...
**zero changes to core Sarthi code**.
...
- `env_enablement_fn: () -> Optional[dict]` — seeds `PlatformConfig.extra`
  ...
  `sarthi gateway status` or `get_connected_platforms()` until the SDK
  instantiates.
...
- `plugin.yaml` `requires_env` / `optional_env` rich-dict entries —
  auto-populate `OPTIONAL_ENV_VARS` in `sarthi_cli/config.py` so the setup
...
Checklist for integrating a platform directly into the Sarthi core.
...
And add it to the `sarthi-gateway` composite:

```python
"sarthi-gateway": {
    "includes": [..., "sarthi-your-platform"]
}
```
...
## 12. Status Display (`sarthi_cli/status.py`)
...
## 13. Gateway Setup Wizard (`sarthi_cli/gateway.py`)
...
grep -r "telegram\|discord\|whatsapp\|slack" gateway/ tools/ agent/ cron/ sarthi_cli/ toolsets.py \
  --include="*.py" -l | sort -u
