...
# Audio file extensions Sarthi recognizes for native audio delivery.
# Keep Telegram's narrower attachment/voice sets below separate: formats such
# as MPEG-2 Layer II are audio to Sarthi but unsupported by sendAudio/sendVoice.
_AUDIO_MIME_TYPES = {
    ".ogg": "audio/ogg",
    ".opus": "audio/opus",
    ".mp3": "audio/mpeg",
...
def _thread_metadata_for_source(source, reply_to_message_id: str | None = None) -> dict | None:
    ...
    value. Telegram private-chat topics created through Sarthi' DM-topic helper
...
def _reply_anchor_for_event(event) -> str | None:
    ...
    replying to the triggering message. Sarthi-created Telegram private-chat
    ...
    if (
        ...
        and raw_message.get("_sarthi_no_thread_response")
    ):
...
def build_auto_tts_output_path(platform) -> str:
    ...
    TTS tool's ``SARTHI_SESSION_PLATFORM`` contextvar — that contextvar is
...
from sarthi_constants import get_default_sarthi_root, get_sarthi_dir, get_sarthi_home
...
GATEWAY_SECRET_CAPTURE_UNSUPPORTED_MESSAGE = (
    ...
    "Load this skill in the local CLI to be prompted, or add the key to ~/.sarthi/.env manually."
)
...
IMAGE_CACHE_DIR = get_sarthi_dir("cache/images", "image_cache")


def _resolve_cache_dir(constant_name: str, new_subpath: str, old_name: str) -> Path:
    """Resolve fresh via get_sarthi_dir (active profile), unless a test has
    monkeypatched the constant away from its import-time default."""
    fresh = get_sarthi_dir(new_subpath, old_name)
...
def get_inbound_media_max_bytes() -> int:
    ...
    """
    try:
        from sarthi_cli.config import load_config_readonly as _load_config
        ...
        for attempt in range(retries + 1):
            try:
                async with client.stream(
                    ...
                    url,
                    headers={
                        "User-Agent": "Mozilla/5.0 (compatible; SarthiAgent/1.0)",
                        "Accept": "image/*,*/*;q=0.8",
                    },
                ) as response:
...
AUDIO_CACHE_DIR = get_sarthi_dir("cache/audio", "audio_cache")
...
VIDEO_CACHE_DIR = get_sarthi_dir("cache/videos", "video_cache")
...
DOCUMENT_CACHE_DIR = get_sarthi_dir("cache/documents", "document_cache")
SCREENSHOT_CACHE_DIR = get_sarthi_dir("cache/screenshots", "browser_screenshots")
...
_SARTHI_ROOT = get_default_sarthi_root()
MEDIA_DELIVERY_ALLOW_DIRS_ENV = "SARTHI_MEDIA_ALLOW_DIRS"
MEDIA_DELIVERY_TRUST_RECENT_ENV = "SARTHI_MEDIA_TRUST_RECENT_FILES"
MEDIA_DELIVERY_TRUST_RECENT_SECONDS_ENV = "SARTHI_MEDIA_TRUST_RECENT_SECONDS"
...
MEDIA_DELIVERY_STRICT_ENV = "SARTHI_MEDIA_DELIVERY_STRICT"
MEDIA_DELIVERY_SAFE_ROOTS = (
    ...
    _SARTHI_HOME / "image_cache",
    _SARTHI_HOME / "audio_cache",
    _SARTHI_HOME / "video_cache",
    _SARTHI_HOME / "document_cache",
    _SARTHI_HOME / "browser_screenshots",
    ...
    _SARTHI_HOME / "cache" / "images",
    _SARTHI_HOME / "cache" / "audio",
    _SARTHI_HOME / "cache" / "videos",
    _SARTHI_HOME / "cache" / "documents",
    _SARTHI_HOME / "cache" / "screenshots",
)
...
def _profile_cache_roots() -> List[Path]:
    """Return per-profile canonical cache roots under the shared Sarthi root.

    Profile gateways write generated artifacts to
    ``<root>/profiles/<name>/cache/{images,audio,...}``. The static safe-roots
    list only covers the *active* SARTHI_HOME's cache, so a gateway running at
    the root (e.g. ``SARTHI_HOME=/opt/data``) while the model emits a
    ...
    is consulted (which otherwise wins when SARTHI_HOME is symlinked under a
    denied prefix and $HOME is not that prefix). See issue #31733.
    """
    roots: List[Path] = []
    profiles_dir = _SARTHI_ROOT / "profiles"
    try:
...
def _kanban_attachment_roots() -> List[Path]:
    ...
    home_override = os.environ.get("SARTHI_KANBAN_HOME", "").strip()
    root = Path(home_override).expanduser() if home_override else _SARTHI_ROOT
...
def _media_delivery_denied_paths() -> List[Path]:
    ...
    # The active Sarthi profile and shared Sarthi root both contain control
    ...
    # SARTHI_HOME root. The set mirrors the canonical read guard in
    ...
    # tree, so skills/, logs/, and ad-hoc agent-written files under ~/.sarthi
    ...
    for sarthi_root in (_SARTHI_HOME, _SARTHI_ROOT):
        for rel in _ROOT_CREDENTIAL_FILES:
            denied.append(sarthi_root / rel)
...
def _path_under_denied_prefix(resolved: Path) -> bool:
    ...
    sub-directories inside home (``~/.ssh``, ``~/.aws``, ...) and Sarthi
    secrets (``~/.sarthi/.env``, ``auth.json``) are *separate, more-specific*
...
def _cache_dir_container_mounts() -> List[Tuple[Path, Path]]:
    """(host, container) pairs for the auto-mounted Sarthi cache dirs.

    The agent legitimately sees generated artifacts at ``/root/.sarthi/...``
...
def _translate_docker_container_media_path(candidate: Path) -> Optional[Path]:
    ...
    auto-mounted Sarthi cache dirs (``/root/.sarthi/...``), the default
    ...
    # In-process gateways (Desktop backend, `sarthi serve`) may not have
    ...
    # are longer prefixes, so /root/.sarthi/... still translates to the host
    ...
    if default_home is not None and not any(c.as_posix() == "/root" for _, c in mounts):
        # /root/.sarthi/* that did NOT match a cache mount is the container's
        ...
        if not candidate.as_posix().startswith("/root/.sarthi"):
...
def validate_media_delivery_path(path: str) -> Optional[str]:
    ...
    ``SARTHI_MEDIA_DELIVERY_STRICT=1``): the file MUST live under a
    Sarthi-managed cache, under an operator-allowlisted root
    (``SARTHI_MEDIA_ALLOW_DIRS``), or be freshly produced inside the
    ...
    # credential/secret stores under the Sarthi root (~/.sarthi/.env,
    ...
    # ``MEDIA:~/.sarthi/google_token.json``) remain rejected.
...
_PLAINTEXT_GATEWAY_RESTART_PATTERNS: tuple[re.Pattern[str], ...] = (
    ...
    re.compile(r"^(?:please\s+)?restart\s+(?:the\s+)?sarthi\s+gateway[.!?\s]*$", re.IGNORECASE),
    re.compile(r"^(?:please\s+)?restart\s+sarthi[.!?\s]*$", re.IGNORECASE),
)
...
class BasePlatformAdapter(ABC):
    ...
    # into the ``SARTHI_SESSION_ASYNC_DELIVERY`` contextvar at session-bind
    ...
    # Sarthi commands.  Default "/" (most platforms deliver "/approve" etc.
    ...
    def __init__(self, config: PlatformConfig, platform: Platform):
        ...
        # Cross-SARTHI_HOME token takeover is armed by GatewayRunner only for
        ...
        self._busy_text_mode: str = (
            os.environ.get("SARTHI_GATEWAY_BUSY_TEXT_MODE", "interrupt").strip().lower()
            or "interrupt"
        )
        self._busy_text_debounce_seconds: float = _float_env(
            "SARTHI_GATEWAY_BUSY_TEXT_DEBOUNCE_SECONDS", 0.35
        )
        self._busy_text_hard_cap_seconds: float = _float_env(
            "SARTHI_GATEWAY_BUSY_TEXT_HARD_CAP_SECONDS", 1.0
        )
    ...
    def prefers_fresh_final_streaming(
        ...
        implementation supports. Telegram is the motivating case: Sarthi sends
    ...
    def _acquire_platform_lock(self, scope: str, identity: str, resource_desc: str) -> bool:
        ...
        A live cross-SARTHI_HOME holder may be replaced only when the runner
        ...
        if owner_profile:
            ...
            remedy = (
                ...
                f"(sarthi --profile {owner_profile} gateway stop)."
            )
        else:
            holder = f" (PID {owner_pid})" if owner_pid else ""
    ...
    async def send_voice(
        ...
        **kwargs,
        ...
        host filesystem path that would leak the Sarthi home layout.
        """
    ...
    async def send_video(
        ...
        **kwargs,
        ...
        would leak the Sarthi home layout.
        """
    ...
    def _mask_json_string_media(content: str) -> str:
        ...
        Serialized tool results frequently embed a previous reply's text, e.g.::
            ...
            {"result": "MEDIA:/Users/x/.sarthi/media/generated/stale.png"}
    ...
    def _get_human_delay() -> float:
        ...
        Reads from env vars:
          SARTHI_HUMAN_DELAY_MODE: "off" (default) | "natural" | "custom"
          SARTHI_HUMAN_DELAY_MIN_MS: minimum delay in ms (default 800, custom mode)
          SARTHI_HUMAN_DELAY_MAX_MS: maximum delay in ms (default 2500, custom mode)
        """
        mode = os.getenv("SARTHI_HUMAN_DELAY_MODE", "off").lower()
        if mode == "off":
            return 0.0
        if mode == "natural":
        ...
        try:
            min_ms = int(os.getenv("SARTHI_HUMAN_DELAY_MIN_MS", "800"))
        except (TypeError, ValueError):
            min_ms = 800
        try:
            max_ms = int(os.getenv("SARTHI_HUMAN_DELAY_MAX_MS", "2500"))
        except (TypeError, ValueError):
            max_ms = 2500
        return random.uniform(min_ms / 1000.0, max_ms / 1000.0)
    ...
    async def _process_message_background(self, event: MessageEvent, session_key: str) -> None:
        ...
        try:
            ...
            if response:
                ...
                if (self._should_auto_tts_for_chat(event.source.chat_id)
                        ...
                        and not self._streaming_tts_turn_completed(
                            session_key,
                            getattr(interrupt_event, "_sarthi_run_generation", None),
                            event=event,
                        )):
                    try:
                        ...
                        if check_tts_requirements():
                            ...
                            # SARTHI_SESSION_PLATFORM contextvar the tool would
            ...
            # not at the top of this task.  _sarthi_run_generation is set on
    ...
    def build_source(
        ...
        per-profile SARTHI_HOME isolation.
        """
