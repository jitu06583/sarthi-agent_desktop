...
def _resolve_profile_path(path_value: Any) -> Optional[Path]:
    """Resolve a user path, mapping ~/.sarthi to the active profile home."""
    ...
    if raw == "~/.sarthi":
        return sarthi_home
    if raw.startswith("~/.sarthi/"):
        return sarthi_home / raw.removeprefix("~/.sarthi/")
    path = Path(raw).expanduser()
    if path.is_absolute():
        return path
    return sarthi_home / path


def _resolve_script_path(script_value: Any) -> tuple[Optional[Path], Optional[str]]:
    """Resolve a route script under SARTHI_HOME/scripts."""
    if not isinstance(script_value, str) or not script_value.strip():
        return None, "script path is empty"
    from sarthi_constants import get_sarthi_home

    scripts_root = (get_sarthi_home() / "scripts").resolve()
    raw_text = os.path.expandvars(script_value.strip())
    if raw_text == "~/.sarthi" or raw_text.startswith("~/.sarthi/"):
...
class WebhookRouteProcessor:
    ...
    def run_route_script(self, script_value: Any, payload: dict) -> tuple[bool, Optional[dict]]:
        ...
        if (
            ...
            or transformed.get("__sarthi_ignore__") is True
        ):
            return False, None
