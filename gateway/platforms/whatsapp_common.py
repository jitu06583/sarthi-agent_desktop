...
class WhatsAppBehaviorMixin:
    ...
    DEFAULT_REPLY_PREFIX: str = "⚕ *Sarthi Agent*\n────────────\n"
...
def resolve_whatsapp_bridge_dir() -> Path:
    """Resolve the WhatsApp bridge directory, mirroring to SARTHI_HOME if needed.

    When the install tree is read-only (e.g., Docker /opt/sarthi), this function
    mirrors the bridge source to a writable SARTHI_HOME location and returns that
    ...
    # Try SARTHI_HOME location first
    sarthi_home = get_sarthi_home()
    sarthi_home_bridge = sarthi_home / "scripts" / "whatsapp-bridge"
    ...
    # Install dir is read-only, mirror to SARTHI_HOME if needed
    if sarthi_home_bridge.exists():
        return sarthi_home_bridge

    # Mirror the bridge source to SARTHI_HOME
    try:
        sarthi_home_bridge.parent.mkdir(parents=True, exist_ok=True)
        shutil.copytree(
            install_bridge,
            sarthi_home_bridge,
            dirs_exist_ok=False,
