...
The PID file lives at ``{SARTHI_HOME}/gateway.pid``.  SARTHI_HOME defaults to
``~/.sarthi`` but can be overridden via the environment variable.  This means
separate SARTHI_HOME directories naturally get separate PID files — a property
...
from sarthi_constants import get_sarthi_home, _get_platform_default_sarthi_home
...
def _get_starts_log_path() -> Path:
    ...
    collision."""
    return get_sarthi_home() / "gateway-starts.log"
...
def _get_process_sarthi_home() -> Path:
    """Return the process-level SARTHI_HOME, skipping context-local overrides.
    ...
    ``get_sarthi_home()`` honors ``_SARTHI_HOME_OVERRIDE`` contextvar used for
...
def _canonical_sarthi_home(path: Path | str) -> Path:
    """Return a stable absolute SARTHI_HOME path for persisted identity data."""
    return Path(path).expanduser().resolve(strict=False)


def _same_sarthi_home(left: Path | str, right: Path | str) -> bool:
    """Compare SARTHI_HOME paths with the host platform's case semantics."""
    return os.path.normcase(str(_canonical_sarthi_home(left))) == os.path.normcase(
        str(_canonical_sarthi_home(right))
    )


# Mirrors sarthi_cli.profiles._PROFILE_ID_RE — duplicated here because gateway
# identity code must stay import-light (sarthi_constants + stdlib only).
_PROFILE_LABEL_RE = re.compile(r"^[a-z0-9][a-z0-9_-]{0,63}$")


def _profile_label_for_home(home: Path | str) -> Optional[str]:
    """Best-effort profile label for a SARTHI_HOME path.

    Returns the profile name for ``<root>/profiles/<name>`` layouts (both
    ``~/.sarthi/profiles/coder`` and Docker ``/opt/data/profiles/coder``),
    ``"default"`` for the deployment's root home, and ``None`` when no label
    can be inferred.  Never raises — this feeds diagnostics only.
    """
    try:
        canonical = _canonical_sarthi_home(home)
    except Exception:
        return None
    ...
    try:
        ...
        if _same_sarthi_home(canonical, get_default_sarthi_root()):
            return "default"
    except Exception:
        pass
    try:
        if _same_sarthi_home(canonical, _get_platform_default_sarthi_home()):
            return "default"
    except Exception:
        pass
    return None
...
def scoped_lock_owner_label(record: Optional[dict[str, Any]]) -> Optional[str]:
    ...
    SARTHI_HOME profile than the caller.  Prefers the explicit ``profile``
    field stamped by :func:`acquire_scoped_lock`; falls back to inferring the
    label from the persisted ``sarthi_home`` for locks written before the
...
def _get_pid_path() -> Path:
    """Return the path to the gateway PID file, respecting SARTHI_HOME."""
    home = _get_process_sarthi_home()
...
def _get_lock_dir() -> Path:
    ...
    override = os.getenv("SARTHI_GATEWAY_LOCK_DIR")
    if override:
        return Path(override)
    state_home = Path(os.getenv("XDG_STATE_HOME", Path.home() / ".local" / "state"))
    return state_home / "sarthi" / _LOCKS_DIRNAME
...
# Reject epoch values before 2000-01-01T00:00:00Z: nothing in Sarthi' lifetime
...
def _gateway_command_subcommand(command: str | None) -> str | None:
    """Return the Sarthi gateway lifecycle subcommand from a command line.
    ...
    test also matched ``sarthi_cli.main gateway status`` and even unrelated
    ...
    (``"C:\\Program Files\\...\\sarthi-gateway.exe"``) survive, and strips
    ``--profile``/``-p`` selectors from anywhere in argv -- Sarthi's
    ...
    for token in tokens:
        ...
        if basename in ("sarthi-gateway", "sarthi-gateway.exe"):
            return "run"

    joined = " ".join(tokens)
    has_gateway_entry = (
        "sarthi_cli.main" in joined
        or "sarthi_cli/main.py" in joined
        or any(t.rsplit("/", 1)[-1] in ("sarthi", "sarthi.exe") for t in tokens)
    )
    ...
    for i, token in enumerate(filtered):
        if token != "gateway":
            continue
        if i + 1 >= len(filtered):
            return "run"  # bare `sarthi gateway` defaults to `run`
        return filtered[i + 1]
    return None
...
def looks_like_gateway_runtime_command_line(command: str | None) -> bool:
    ...
    use this broader matcher only when validating Sarthi-owned runtime records
...
def _looks_like_gateway_process(pid: int) -> bool:
    """Return True when the live PID still looks like the Sarthi gateway."""
...
def _profile_name_for_home(profile_home: Path) -> Optional[str]:
    """Return the profile id a SARTHI_HOME directory represents, or None.

    A named profile's home is ``<root>/profiles/<name>`` (immediate parent is
    ``profiles``).  The root/default home (``~/.sarthi`` or ``$SARTHI_HOME``)
...
def _command_line_belongs_to_profile(command: str, profile_home: Path) -> bool:
    ...
    Mirrors ``sarthi_cli.gateway._matches_current_profile`` so the dashboard's
    ...
    explicit ``SARTHI_HOME=<path>``) on its argv; the default/root gateway runs
    bare with no profile flag.
    """
    # Normalize separators before the substring match: on Windows,
    # str(Path) renders backslashes while a SARTHI_HOME= value on the argv
    ...
    if profile_name is not None and profile_name != "default":
        ...
        return (
            ...
            or f"sarthi_home={home_lc}" in command_lc
        )
    ...
    # a non-matching explicit SARTHI_HOME= on the argv. SARTHI_HOME is usually
    ...
    if "--profile " in command_lc or " -p " in command_lc:
        return False
    if "sarthi_home=" in command_lc and f"sarthi_home={home_lc}" not in command_lc:
        return False
    return True
...
def _build_pid_record() -> dict:
    return {
        ...
        # SARTHI_HOME-local.  Persist the owning gateway's process home so an
        # explicit cross-profile --replace can place its planned-takeover
        # marker where the target process will actually read it.
        "sarthi_home": str(_canonical_sarthi_home(_get_process_sarthi_home())),
    }
...
def _get_code_identity_fields() -> dict[str, Any]:
    ...
    ``sarthi_cli``; the helper itself is cached per process. A gateway
    keeps serving the module versions it imported at startup, so stamping
    the identity into ``gateway_state.json`` lets `sarthi update` (and the
    ...
    """
    try:
        from sarthi_cli.build_info import get_code_identity
...
def write_runtime_status(
    *,
    ...
    if served_profiles is not _UNSET:
        ...
        # for a single-profile gateway. Lets `sarthi status` show per-profile
...
def read_runtime_status(path: Optional[Path] = None) -> Optional[dict[str, Any]]:
    ...
    can do so without mutating ``SARTHI_HOME`` in-process.  Defaults to
...
def resolve_gateway_liveness(
    *,
    ...
    passes its ``sarthi_cli.web_server`` bindings so the long-standing
...
def get_runtime_status_running_pid(
    ...
    *,
    ...
    SARTHI_HOME.  Pass it when validating *another* profile's state file (the
...
def acquire_scoped_lock(scope: str, identity: str, metadata: Optional[dict[str, Any]] = None) -> tuple[bool, Optional[dict[str, Any]]]:
    ...
    at once (e.g. the same Telegram bot token across different SARTHI_HOME dirs).
    """
    ...
    # is inferable; readers fall back to deriving it from sarthi_home.
    profile = _profile_label_for_home(_get_process_sarthi_home())
    if profile:
...
# enabled in the user's systemd (e.g. ``sarthi.service`` + ``sarthi-
# gateway.service``).
#
...
def _get_takeover_marker_path(sarthi_home: Optional[Path] = None) -> Path:
    """Return the path to the --replace takeover marker file.

    ``sarthi_home`` is supplied only for a verified cross-home handoff.  The
    target process always consumes the marker from its own process-level home.
    """
    home = sarthi_home or _get_process_sarthi_home()
    return _canonical_sarthi_home(home) / _TAKEOVER_MARKER_FILENAME
...
def _consume_pid_marker_for_self(
    ...
    *,
    ...
    # TARGET home.  That permits a deliberate cross-SARTHI_HOME --replace while
    ...
    our_home = _get_process_sarthi_home()
    target_home = record.get("target_sarthi_home")
    if target_home is not None:
        if not isinstance(target_home, str) or not _same_sarthi_home(
            target_home, our_home
        ):
            return False
    else:
        replacer_home = record.get("replacer_sarthi_home")
        if replacer_home is not None and not _same_sarthi_home(
    ...
    # ``sarthi gateway stop`` on Windows would be misclassified as an
...
def write_takeover_marker(
    ...
    *,
    ...
    gateway's SARTHI_HOME rather than the replacer's.  Same-home callers omit
    ...
    """
    try:
        marker_home = _canonical_sarthi_home(
            target_home or _get_process_sarthi_home()
        )
        ...
        record = {
            ...
            "target_sarthi_home": str(marker_home),
            "replacer_pid": os.getpid(),
            "replacer_sarthi_home": str(
                _canonical_sarthi_home(_get_process_sarthi_home())
            ),
...
def _validated_scoped_lock_gateway_owner(
    ...
    the lock record, the target SARTHI_HOME's gateway PID record, and the live
    ...
    pid_record_home = pid_record.get("sarthi_home")
    if not isinstance(pid_record_home, str) or not _same_sarthi_home(
