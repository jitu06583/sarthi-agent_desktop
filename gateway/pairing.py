...
PAIRING_DIR = get_sarthi_dir("platforms/pairing", "pairing")
...
def _read_allowlist_env(env_var: str) -> str:
    ...
    ``sarthi_cli.config.save_env_value`` / ``remove_env_value``, which target
...
def _sync_allowlist_remove(platform: str, user_id: str) -> None:
    ...
    if len(remaining) == len(ids):
        return  # Not present.
    try:
        from sarthi_cli.config import save_env_value, remove_env_value
...
def _merge_pairing_dir(active_dir: Path, alternate_dir: Path) -> None:
    ...
    Older installs use ``{SARTHI_HOME}/pairing`` while newer code/docs may
    write ``{SARTHI_HOME}/platforms/pairing``. If both directories exist, the
...
class PairingStore:
    ...
    profile's own SARTHI_HOME using the same legacy/consolidated layout rules
    as ``sarthi -p <name> pairing ...``. This keeps multiplex gateways and
    profile-scoped CLI approvals on one whitelist. Without a profile, storage
    is the global pairing directory for the current SARTHI_HOME.
    """

    def __init__(self, profile: Optional[str] = None):
        # Resolve storage directory lazily — tests use a temp SARTHI_HOME
        ...
        if profile:
            ...
            # ``sarthi -p <profile> pairing ...`` process does. Merge the
    ...
    def _load_json(self, path: Path) -> dict:
        if path.exists():
            try:
                ...
                # the file, then the gateway process — running as `sarthi` after
                ...
                logger.warning(
                    ...
                    "If you ran `docker exec <container> sarthi pairing approve ...` as root, "
                    "re-run with `docker exec -u sarthi <container> ...` and "
                    "chown the existing file to the sarthi user, or restart the "
    ...
    def approve_request(self, platform: str, request_id: str) -> Optional[dict]:
        ...
        This is the grant path for authenticated admin surfaces (``sarthi
