...
from sarthi_constants import get_sarthi_dir, get_sarthi_home
...
class PairingStore:
    ...
    ``<SARTHI_HOME>/profiles/<name>/pairing/`` (per-profile, used by
    multiplexing gateways so each profile has its own whitelist).
    Without a profile, storage is the global ``<SARTHI_HOME>/pairing/``
    directory (backward-compat for the ``sarthi pairing`` CLI).
    """
    ...
    def __init__(self, profile: Optional[str] = None):
        ...
        if profile:
            ...
            self._dir = get_sarthi_home() / "profiles" / profile / "pairing"
        else:
