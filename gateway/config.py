...
def _normalize_multiplex_profile_allowlist(value: Any) -> Optional[List[str]]:
    ...
    from sarthi_cli.profiles import normalize_profile_name, validate_profile_name
...
# dashboard's pre-write mutation validation (sarthi_cli/web_server.py) so
...
class PlatformConfig:
    ...
    # "Sarthi is thinking…"). Platforms with textless indicators (Discord,
...
class GatewayConfig:
    ...
    # Storage paths
    sessions_dir: Path = field(default_factory=lambda: get_sarthi_home() / "sessions")
    ...
    # gateway behaves exactly as before — single SARTHI_HOME, no profile stamping.
...
def load_gateway_config() -> GatewayConfig:
    ...
    2. ~/.sarthi/config.yaml (primary user-facing config)
    3. ~/.sarthi/gateway.json (legacy — provides defaults under config.yaml)
    4. Built-in defaults
    """
    _home = get_sarthi_home()
    gw_data: dict = {}
    ...
    try:
        ...
        if config_yaml_path.exists():
            ...
            # through sarthi_cli.config.load_config, so without this a managed
            ...
            # (e.g. via `sarthi config set gateway.<key> ...`, which naturally
            ...
            if isinstance(gateway_section, dict):
                if "multiplex_profiles" in gateway_section and "multiplex_profiles" not in gw_data:
                    # gateway.multiplex_profiles written by `sarthi config set gateway.multiplex_profiles true`
            ...
            if not isinstance(streaming_cfg, dict) and isinstance(gateway_section, dict):
                ...
                # ``sarthi config set gateway.streaming.*``
