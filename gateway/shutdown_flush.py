...
1. ``flush_pending_to_file()`` — called BEFORE ``_pending_messages.clear()``
   ...
   under ``<sarthi_home>/pending_messages/``.
...
def _get_flush_dir():
    """Return the pending-messages flush directory under the active SARTHI_HOME."""
    from sarthi_constants import get_sarthi_home

    flush_dir = get_sarthi_home() / "pending_messages"
...
def spool_dropped_transcript_message(
    ...
    ``<sarthi_home>/pending_messages/``), so a runtime cap rotation no
