...
def snapshot_shutdown_context(received_signal: Any = None) -> Dict[str, Any]:
    ...
    * Wall-clock and monotonic timestamps for cross-correlating later phases
    ...
    try:
        ...
        if sarthi_home_str:
            takeover_path = Path(sarthi_home_str) / ".gateway-takeover.json"
            if takeover_path.exists():
                try:
                    ...
                    pass
            planned_stop_path = Path(sarthi_home_str) / ".gateway-planned-stop.json"
...
def check_systemd_timing_alignment(drain_timeout: float) -> Optional[Dict[str, Any]]:
    ...
    upgraded sarthi-agent but never re-ran ``sarthi setup`` to regenerate
    ...
    try:
        # /proc/self/cgroup gives us "0::/user.slice/.../sarthi-gateway.service"
    ...
    # that's the common case for sarthi.
    ...
    # constant in sarthi_cli/gateway.py.
    headroom = 30.0
...
def parse_systemd_duration_to_us(raw: str) -> Optional[int]:
    ...
    Public: also consumed by sarthi_cli.gateway's restart-wait sizing.
    """
    if not raw:
        return None
    units = {
