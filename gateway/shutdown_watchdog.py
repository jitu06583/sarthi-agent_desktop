...
2. An event-loop heartbeat file at ``<SARTHI_HOME>/state/gateway.heartbeat`` so
...
def _process_sarthi_home() -> Path:
    """SARTHI_HOME for process-level identity files (ignore profile overrides)."""
...
def get_loop_heartbeat_path(home: Optional[Path] = None) -> Path:
    """Return ``<SARTHI_HOME>/state/gateway.heartbeat``."""
    base = home if home is not None else _process_sarthi_home()
    ...
    *,
    ...
    def _watchdog() -> None:
        ...
        try:
            ...
            pass
        try:
            from sarthi_logging import drain_log_queue
