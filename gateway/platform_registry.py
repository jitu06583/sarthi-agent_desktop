...
class PlatformEntry:
    ...
    # status displays (``sarthi setup``, ``sarthi status``, the dashboard
    ...
    # Env vars this platform needs (for ``sarthi setup`` display).
    ...
    # built-ins).  Used by ``sarthi gateway setup`` to auto-enable the
...
class PlatformRegistry:
    ...
    def __init__(self) -> None:
        ...
        # Plugin adapters are isolated per resolved SARTHI_HOME and overlay the
        ...
        # plugin-discovery time added several seconds to *every* `sarthi`
        # invocation -- including plain `sarthi chat`, which never touches any
        ...
        # `sarthi setup`/`gateway status`, send_message).
    ...
    def _resolve_all(self) -> None:
        ...
        gateway startup, ``sarthi setup``/``gateway status``, channel
