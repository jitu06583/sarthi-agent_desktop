...
Defenses 1 and 2 (the ``_SARTHI_GATEWAY`` guard on ``sarthi gateway
...
raw ``terminal("launchctl kickstart -k gui/<uid>/ai.sarthi.gateway")``,
...
State lives in ``<SARTHI_HOME>/gateway/restart_loop.json`` so it is
...
def _state_path():
    return get_sarthi_home() / "gateway" / "restart_loop.json"
