...
def build_subscription_state(*, timeout: float = 15.0) -> SubscriptionState:
    ...
    Dev override: when ``SARTHI_DEV_SUBSCRIPTION_FIXTURE`` names a fixture state,
...
def dev_fixture_subscription_state() -> Optional[SubscriptionState]:
    """Return a fixture :class:`SubscriptionState` for ``SARTHI_DEV_SUBSCRIPTION_FIXTURE``.
    ...
    Throwaway scaffolding — mirrors ``SARTHI_DEV_CREDITS_FIXTURE``.
    """
    name = (os.getenv("SARTHI_DEV_SUBSCRIPTION_FIXTURE") or "").strip().lower()
    if not name:
        return None
    ...
    return SubscriptionState(logged_in=False, error=f"unknown SARTHI_DEV_SUBSCRIPTION_FIXTURE: {name}")
