...
def openclaw_residue_hint_cli() -> str:
    """Banner shown the first time Sarthi starts and finds ``~/.openclaw/``.

    Points users at ``sarthi claw migrate`` (non-destructive port of config,
    memory, and skills) first. ``sarthi claw cleanup`` is mentioned as the
    ...
    return (
        ...
        "To port your config, memory, and skills over to Sarthi, run "
        "`sarthi claw migrate`.\n"
        "If you've already migrated and want to archive the old directory, "
        "run `sarthi claw cleanup` (renames it to ~/.openclaw.pre-migration — "
