...
2. **Command-type providers** declared under ``tts.providers.<name>:
   ...
   CLI into Sarthi with shell-template placeholders. **Wins over a
...
shipped today) or ``~/.sarthi/plugins/tts/<name>/`` (user-installed).
...
``{success: False, error: …}`` JSON envelope the rest of Sarthi
expects.
"""
...
class TTSProvider(abc.ABC):
    ...
    def display_name(self) -> str:
        """Human-readable label shown in ``sarthi tools``.
    ...
    def is_available(self) -> bool:
        ...
        Must NOT raise — used by the picker and ``sarthi setup`` for
