...
Data resolution order:
  ...
  2. Disk cache (~/.sarthi/models_dev_cache.json — any age; stale data is
...
# ---------------------------------------------------------------------------
# Provider ID mapping: Sarthi ↔ models.dev
# ---------------------------------------------------------------------------

# Sarthi provider names → models.dev provider IDs
PROVIDER_TO_MODELS_DEV: Dict[str, str] = {
    ...
    # under the "meta" provider id; Sarthi' provider is "meta-ai" (and the
...
# Reverse mapping: models.dev id → Sarthi ids (built lazily; many-to-one,
# e.g. both "meta" and "meta-ai" may map to the same models.dev id).
_MODELS_DEV_TO_PROVIDER: Optional[Dict[str, List[str]]] = None


def _models_dev_to_sarthi_ids(mdev_id: str) -> List[str]:
    """Return the Sarthi provider ids that map to *mdev_id* (may be [])."""
    global _MODELS_DEV_TO_PROVIDER
    if _MODELS_DEV_TO_PROVIDER is None:
        reverse: Dict[str, List[str]] = {}
        for sarthi_id, mapped in PROVIDER_TO_MODELS_DEV.items():
            reverse.setdefault(mapped, []).append(sarthi_id)
...
def _get_cache_path() -> Path:
    ...
    return get_sarthi_home() / "models_dev_cache.json"


def _get_etag_path() -> Path:
    """Return path to the ETag sidecar file for conditional GET."""
    from sarthi_constants import get_sarthi_home
    return get_sarthi_home() / "models_dev_cache.etag"
...
def fetch_models_dev(
    ...
    When ``force_refresh=True`` (used by ``sarthi config refresh``, the
...
def _provider_override_section(provider: str) -> Optional[Dict[str, Any]]:
    ...
    Accepts either the Sarthi provider id or the models.dev provider id as
    ...
    # Reverse: caller passed a models.dev id, config keyed by Sarthi id.
    for sarthi_id in _models_dev_to_sarthi_ids(provider_key):
        if sarthi_id != provider_key:
            candidates.append(sarthi_id)
...
def _get_provider_models(
    ...
    """Resolve a Sarthi provider ID to its models dict from models.dev.
...
def list_provider_models(
    ...
    picker (``sarthi model``), which is an interactive user-facing flow
    where a fresh catalog is worth a short network wait.
    """
    from sarthi_cli.models import normalize_provider
...
# Gemma models whose TPM quotas are too small for normal Sarthi agent traffic.
...
def get_provider_info(
    ...
    Accepts either a Sarthi provider ID (e.g. "kilocode") or a models.dev
    ...
    ``allow_network=False``.
    """
    # Resolve Sarthi ID → models.dev ID
...
def get_model_info(
    ...
    Accepts Sarthi or models.dev provider ID.  Tries exact match then
