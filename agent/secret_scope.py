...
- ``set_secret_scope(mapping)`` installs the active profile's secrets for the
  ...
  via ``copy_context()`` exactly like the SARTHI_HOME override).
...
_GLOBAL_ENV_EXACT = frozenset({
    # Sarthi runtime / deployment
    "SARTHI_HOME", "SARTHI_PROFILE", "SARTHI_GATEWAY_LOCK_DIR",
    "SARTHI_MAX_ITERATIONS", "SARTHI_MAX_TOKENS", "SARTHI_API_TIMEOUT",
    "SARTHI_REDACT_SECRETS", "SARTHI_NOUS_TIMEOUT_SECONDS",
    "_SARTHI_GATEWAY",
    # OS / interpreter
    "PATH", "HOME", "USER", "LANG", "LC_ALL", "TZ", "PWD", "SHELL", "TMPDIR",
    "VIRTUAL_ENV", "PYTHONPATH", "SSL_CERT_FILE",
    # Kanban paths (per-board, not per-profile-secret)
    "SARTHI_KANBAN_DB", "SARTHI_KANBAN_WORKSPACES_ROOT", "SARTHI_KANBAN_BOARD",
...
_GLOBAL_ENV_PREFIXES = (
    "SARTHI_KANBAN_",
    "SARTHI_TELEGRAM_",   # tuning knobs (batch delays, fallback toggles) — NOT the token
...
def load_env_file(env_path: Path) -> Dict[str, str]:
    ...
    ``set_secret_scope``. Parses the small KEY=VALUE subset Sarthi writes
    ...
    ``sarthi_cli.config._parse_env_value``) but never mutates the process
    ...
    # Parse values with the canonical Sarthi parser: save_env_value
    ...
    from sarthi_cli.config import _parse_env_value
...
def build_profile_secret_scope(sarthi_home: Path) -> Dict[str, str]:
    ...
    """
    home = Path(sarthi_home)
    secrets = load_env_file(home / ".env")

    try:
        from sarthi_cli.env_loader import get_secret_source_values
