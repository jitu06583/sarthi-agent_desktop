...
row taxonomy is inferred from the atlas shape so Sarthi can render both legacy
...
# rows (1536x1872), while older Sarthi/petdex sheets used 9 columns x 8 rows
...
class PetState(str, Enum):
    ...
    These are Sarthi' activity state names. They are not always identical to the
...
# Legacy Sarthi/petdex row order (top -> bottom) used by the older 8-row,
...
# Canonical Sarthi activity names -> accepted row-name aliases in descending
