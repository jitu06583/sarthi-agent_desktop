...
``sarthi pause`` writes a sentinel file at ``$SARTHI_HOME/ESTOP``;
``sarthi resume`` removes it. While the sentinel exists:
...
* new gateway turns get a brief "Sarthi is paused" reply instead of an
...
hold even if the file was created by ``touch ~/.sarthi/ESTOP``.
...
def _sarthi_home() -> Path:
    """Resolve the active SARTHI_HOME (profile-aware) at call time."""
    try:
...
def sentinel_path() -> Path:
    """Path of the ESTOP sentinel under the active SARTHI_HOME."""
    return _sarthi_home() / SENTINEL_NAME
...
def is_engaged() -> bool:
    ...
    exists (permission error, transient I/O failure on SARTHI_HOME), report
...
def paused_reply() -> Optional[str]:
    ...
    if reason:
        return (
            f"⏸️ Sarthi is paused ({reason}). New work is on hold; "
            "run `sarthi resume` to pick things back up."
        )
    return (
        "⏸️ Sarthi is paused. New work is on hold; "
        "run `sarthi resume` to pick things back up."
    )
