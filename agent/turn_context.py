...
def build_turn_context(
    ...
    *,
    ...
    # Tag log records on this thread with the session ID for ``sarthi logs``.
    ...
    if agent.compression_enabled and _should_run_preflight_estimate(
        ...
        # Sarthi only initiates compaction in "sarthi" mode (#36801).
        _codex_native_auto = (
        ...
        if _preflight_deferred:
            ...
            logger.info(
                "Skipping Sarthi preflight compression for codex app-server "
                "(mode=%s); Sarthi will not start thread compaction here.",
    ...
    if agent._memory_manager:
        ...
        # to surface it. Rendered by Sarthi (via _emit_status), so it always
