...
``auth.json``.  They intentionally have no dependency on ``sarthi_cli.auth`` so
...
# Sources Sarthi owns and can intentionally persist in auth.json.  Everything
...
def sanitize_borrowed_credential_payload(
    ...
    Owned sources (manual entries and Sarthi-owned OAuth/device-code state)
