...
class MemoryManager:
    ...
    def add_provider(self, provider: MemoryProvider) -> None:
        ...
        _core_tool_names = set(_SARTHI_CORE_TOOLS)
    ...
    def _strip_skill_scaffolding(text: str) -> Optional[str]:
        ...
        When a user invokes a /skill or /bundle, Sarthi expands the turn into
    ...
    def _prefetch_provider(
        ...
        # Propagate the caller's contextvars (profile SARTHI_HOME override)
    ...
    def _submit_background(self, fn, *, kind: str = "write") -> None:
        ...
        dashboard, cron) is a ContextVar-scoped SARTHI_HOME override, and
    ...
    def initialize_all(self, session_id: str, **kwargs) -> None:
        ...
        Automatically injects ``sarthi_home`` into *kwargs* so that every
        provider can resolve profile-scoped storage paths without importing
        ``get_sarthi_home()`` themselves.
        """
        if "sarthi_home" not in kwargs:
            from sarthi_constants import get_sarthi_home
            kwargs["sarthi_home"] = str(get_sarthi_home())
        for provider in self._providers:
            try:
