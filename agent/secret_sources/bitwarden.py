...
Sarthi pulls API keys from Bitwarden Secrets Manager at process startup
so they don't have to live in plaintext in ``~/.sarthi/.env``.

Design summary
--------------

* The ``bws`` binary is auto-installed into ``<sarthi_home>/bin/bws`` on
  first use.  Sarthi pins one version (``_BWS_VERSION``) and downloads
...
* The access token is stored in ``~/.sarthi/.env`` as
...
* Pulling secrets is a single ``bws secret list <project_id>
  ...
  ``cache_ttl_seconds`` so back-to-back ``sarthi`` invocations don't
  hammer the API.
* Failures NEVER block Sarthi startup.  Missing binary, no network,
...
# In-process cache so repeated load_sarthi_dotenv() calls (CLI startup,
# gateway hot-reload, test suites) don't re-fetch from BSM.
_CacheKey = Tuple[str, str, str]  # (access_token_fingerprint, project_id, server_url)
_CACHE: Dict[_CacheKey, _CachedFetch] = {}

# Disk-persisted cache so back-to-back CLI invocations (e.g. `sarthi chat -q ...`
...
# <sarthi_home>/cache/bws_cache.json. The file holds only the secret VALUES,
# never the access token. It's plaintext-equivalent to ~/.sarthi/.env (which
...
_ENCRYPTED_CACHE_INFO = b"sarthi-bws-encrypted-cache-v1"
...
def _disk_cache_path(home_path: Optional[Path] = None) -> Path:
    """Return the disk cache path under sarthi_home/cache/.

    Thin wrapper over the shared DiskCache, kept for tests and any direct
    callers; falls back to `$SARTHI_HOME` / `~/.sarthi` when home is None.
    """
    return _DISK_CACHE.path(home_path)


def _encrypted_disk_cache_path(home_path: Optional[Path] = None) -> Path:
    """Return the encrypted disk cache path under sarthi_home/cache/."""
...
def _sarthi_bin_dir() -> Path:
    """Where Sarthi stores its managed binaries.  Profile-aware."""
...
def find_bws(*, install_if_missing: bool = False) -> Optional[Path]:
    ...
    Resolution order:
      1. ``<sarthi_home>/bin/bws``  (our managed copy — preferred)
...
def install_bws(*, force: bool = False) -> Path:
    ...
    path catch these; the user-facing ``sarthi secrets bitwarden setup``
    ...
    with tempfile.TemporaryDirectory(prefix="sarthi-bws-") as tmpdir:
        tmp = Path(tmpdir)
...
def _summarize_bws_stderr(raw: str) -> str:
    ...
    Everything from ``Location:`` on is diagnostic noise for a Sarthi
...
# ---------------------------------------------------------------------------
# Public entry point — called from sarthi_cli.env_loader
# ---------------------------------------------------------------------------
...
def apply_bitwarden_secrets(
    *,
    ...
    This is the function ``load_sarthi_dotenv()`` calls after the .env
    ...
    if not access_token:
        result.error = (
            ...
            "not set.  Run `sarthi secrets bitwarden setup`."
        )
        return result
    ...
    if not project_id:
        result.error = (
            ...
            "Run `sarthi secrets bitwarden setup`."
        )
        return result
    ...
    if binary is None:
        result.error = (
            ...
            "Run `sarthi secrets bitwarden setup` to install."
        )
        return result
    ...
    *fetches* — precedence, override semantics, conflict warnings, and
    ...
    def remediation(self, kind, cfg: dict) -> str:
        if kind in (ErrorKind.AUTH_FAILED, ErrorKind.AUTH_EXPIRED):
            return (
                "Run `sarthi secrets bitwarden token` to paste a fresh access "
                "token (create one in the Bitwarden web app: Secrets Manager → "
                "Machine accounts → Access tokens).  Wrong region?  Re-run "
                "`sarthi secrets bitwarden setup` and pick EU/self-hosted."
            )
...
def clear_caches(home_path: Optional[Path] = None) -> None:
    ...
    Used after a token rotation (`sarthi secrets bitwarden token`) so the
