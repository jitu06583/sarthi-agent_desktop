...
cache problem must never block Sarthi startup.
"""
...
def resolve_cache_home(home_path: Optional[Path] = None) -> Path:
    """Resolve the Sarthi home used for cache paths.

    ``home_path`` is whatever ``load_sarthi_dotenv()`` already resolved;
    falling back to ``$SARTHI_HOME`` / ``~/.sarthi`` keeps direct callers
...
class DiskCache(Generic[K]):
    ...
    One JSON object per backend lives at ``<sarthi_home>/cache/<basename>``::
    ...
    def write(
        ...
        try:
            ...
            # metadata isn't exposed if SARTHI_HOME is ever made traversable.
            try:
