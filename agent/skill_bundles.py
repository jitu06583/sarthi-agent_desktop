...
Storage
-------
Bundles live in ``~/.sarthi/skill-bundles/*.yaml`` (and the equivalent
profile-aware directory under ``SARTHI_HOME``). Each file looks like::
...
- :func:`list_bundles` — return rich info for display (``sarthi bundles``)
...
def _bundles_dir() -> Path:
    """Return the canonical bundles directory under SARTHI_HOME.

    Honors ``SARTHI_BUNDLES_DIR`` for tests; falls back to
    ``<SARTHI_HOME>/skill-bundles``.
    """
    override = os.environ.get("SARTHI_BUNDLES_DIR")
    if override:
        return Path(override).expanduser()
    return get_sarthi_home() / "skill-bundles"
...
# ---------------------------------------------------------------------------
# File-level CRUD helpers — used by `sarthi bundles` CLI subcommand.
# ---------------------------------------------------------------------------
