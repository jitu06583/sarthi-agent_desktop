...
Owns one Codex thread per Sarthi session. Drives `turn/start`, consumes
...
# Sarthi' tools.terminal.security_mode → Codex's permissions profile id.
# Defaults if config is missing → workspace-write (matches Codex's own default).
_SARTHI_TO_CODEX_PERMISSION_PROFILE = {
...
def _coerce_turn_input_text(user_input: Any) -> str:
    """Collapse Sarthi/OpenAI rich content into app-server text input.
...
class CodexAppServerSession:
    """One Codex thread per Sarthi session, lifetime owned by AIAgent.
    ...
    def __init__(
        ...
        *,
        ...
        self._permission_profile = (
            permission_profile or _SARTHI_TO_CODEX_PERMISSION_PROFILE.get(
                os.environ.get("SARTHI_TERMINAL_SECURITY_MODE", "auto"),
                "workspace-write",
            )
        )
    ...
    def ensure_started(self) -> str:
        ...
        self._client.initialize(
            client_name="sarthi",
            client_title="Sarthi Agent",
            client_version=_get_sarthi_version(),
        )
    ...
    def run_turn(
        ...
        *,
        ...
        # supports rich content but Sarthi' text path is the common case).
        try:
    ...
    def _handle_server_request(self, req: dict) -> None:
        """Translate a codex server request (approval) into Sarthi' approval
        ...
        if method == "item/commandExecution/requestApproval":
            ...
            # OAuth, form data). For our own sarthi-tools callback we
            # auto-accept — the user already approved Sarthi' tools
    ...
    def _decide_exec_approval(self, params: dict) -> str:
        ...
        This is protocol-level routing only — it carries NO Sarthi
        approval-mode/timeout logic. The Sarthi-side resolution happens
    ...
    def _decide_apply_patch_approval(self, params: dict) -> str:
        ...
        Protocol-level routing only; Sarthi approval-mode/timeout
...
def _approval_choice_to_codex_decision(choice: str) -> str:
    """Map Sarthi approval choices onto codex's CommandExecutionApprovalDecision
    / FileChangeApprovalDecision wire values.

    Sarthi returns 'once', 'session', 'always', or 'deny'.
    ...
    NOT in tools/approval.py: the Sarthi approval mode/timeout resolution
    ...
    # "prompt expired"; the Sarthi-side messaging already distinguishes them.
    return "decline"
...
def _get_sarthi_version() -> str:
    """Best-effort Sarthi version string for codex's userAgent line."""
    try:
