...
projection into Sarthi' display, approval bridging, transcript projection into
...
"codex_app_server"`. Sarthi' default tool dispatch is unchanged when this
...
class CodexAppServerClient:
    ...
    def __init__(
        ...
        # `os.environ.copy()` also handed it every Tier-1 Sarthi secret — gateway
        ...
        spawn_env = sarthi_subprocess_env(inherit_credentials=True)
        if env:
        ...
        if spawn_env.get("SARTHI_KANBAN_TASK"):
            kanban_db = spawn_env.get("SARTHI_KANBAN_DB")
            kanban_root = (
                ...
                if kanban_db
                    ...
                    "SARTHI_KANBAN_ROOT",
                    os.path.join(
                        spawn_env.get("SARTHI_HOME", os.path.expanduser("~/.sarthi")),
                        "kanban",
                    ),
                )
            )
    ...
    def initialize(
        self,
        client_name: str = "sarthi",
        client_title: str = "Sarthi Agent",
