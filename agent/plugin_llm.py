...
Plugins built on Sarthi Agent often need to make their own LLM calls
...
Today the only stable plugin surfaces extend an existing Sarthi
...
:class:`~sarthi_cli.plugins.PluginContext`:
...
embedded slugs, no shorthands. This mirrors Sarthi' main config
...
handles every provider, fallback chain, and per-task override Sarthi
supports.
"""
...
class PluginLlm:
    ...
    Instances are constructed by :class:`sarthi_cli.plugins.PluginContext`
