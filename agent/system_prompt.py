...
upstream prefix cache warm.  See ``sarthi-agent-dev``'s
...
from sarthi_constants import get_default_sarthi_root, get_sarthi_home
...
_PLUGIN_SECTION_FRAME_RE = re.compile(
    ...
    r"<!-- sarthi-plugin-section-chars:(?P<chars>[0-9]{1,4}) -->\n\n",
    re.MULTILINE,
)
...
def _tui_embedded_pane_clarifier(hint: str) -> str:
    ...
    Triggered by ``SARTHI_DESKTOP_TERMINAL=1`` (set by ``main.cjs`` only on the
    ...
    if _TUI_EMBEDDED_PANE_CLARIFIER in hint:
        return hint
    if not is_truthy_value(os.getenv("SARTHI_DESKTOP_TERMINAL")):
        return hint
...
def _plugin_session_info(agent: Any) -> Dict[str, str]:
    ...
    try:
        ...
        # the SARTHI_HOME ContextVar (#86313 class; plugin half per @helix4u).
...
def _frozen_plugin_prompt_sections(agent: Any) -> tuple:
    ...
    if isinstance(stored_prompt, str) and stored_prompt:
        ...
        return rendered
    try:
        from sarthi_cli.plugins import render_system_prompt_sections
...
def _plugin_section_blocks(sections: tuple, position: str) -> List[str]:
    from sarthi_cli.plugins import format_system_prompt_sections
...
def _agent_home(agent: Any) -> Optional[Path]:
    ...
    1. A bound SARTHI_HOME ContextVar override wins. Surfaces that multiplex
...
def _profile_name_for_home(home: Path) -> str:
    ...
    Uses :func:`get_default_sarthi_root` (NOT ``get_sarthi_home()``): on a
    correctly bound profile session the ambient home IS the profile dir, so
    ``get_sarthi_home()/profiles`` would never contain ``home`` and every
...
def build_system_prompt_parts(agent: Any, system_message: Optional[str] = None) -> Dict[str, str]:
    ...
    AIAgent.  Sarthi never re-renders parts of this string mid-
    ...
    # Some execution modes (cron) still want SARTHI_HOME persona while keeping
    # cwd project instructions disabled.
    _soul_loaded = False
    if agent.load_soul_identity or not agent.skip_context_files:
        # Scope the SOUL.md read to the agent's OWN home (see _agent_home) —
        # ambient resolution on a thread that lost the SARTHI_HOME ContextVar
    ...
    # Pointer to the sarthi-agent skill + docs for user questions about Sarthi itself.
    stable_parts.append(SARTHI_AGENT_HELP_GUIDANCE)
    ...
    # SARTHI_KANBAN_TASK env var). Normal chat sessions never see
    ...
    # Coding posture (base Sarthi, any interactive coding surface in a code
    ...
    # `sarthi -p <bot> chat --in ~ -c "Bot Chat"` and the desktop pins), on
    # installs where Bot Mode manages profiles (ui_meta['sarthi-bots']).
    ...
    # Active-profile hint — names the Sarthi profile the agent is running
    # under so it doesn't conflate ~/.sarthi/skills/ (default profile) with
    # ~/.sarthi/profiles/<active>/skills/ (this profile's). Deterministic
    ...
    # ambient SARTHI_HOME: on a build thread that lost the ContextVar this
    ...
    # from get_default_sarthi_root(): get_sarthi_home() on a bound profile
    ...
    # to the legacy behavior (and patchable via this module's get_sarthi_home).
    if _agent_home_path is not None:
        _home_str = str(_agent_home_path)
        _root_str = str(get_default_sarthi_root())
    else:
        _home_str = _root_str = str(get_sarthi_home())
    if active_profile == "default":
        post_workspace_parts.append(
            "Active Sarthi profile: default. Other profiles (if any) live "
        ...
        # data sits at the ROOT (get_default_sarthi_root()), which in ambient
        # profile mode is NOT get_sarthi_home().
        profile_home = _home_str
        default_root = get_default_sarthi_root()
        post_workspace_parts.append(
            f"Active Sarthi profile: {active_profile}. This session reads "
    ...
    if not agent.skip_context_files:
        ...
        # (developing Sarthi). Every other surface (desktop chat panel,
    ...
    from sarthi_time import get_timezone as _sarthi_tz, now as _sarthi_now
    now = _sarthi_now()
