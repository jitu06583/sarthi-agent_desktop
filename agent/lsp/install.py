...
appropriate.  All installs go to a Sarthi-owned bin staging dir,
``<SARTHI_HOME>/lsp/bin/``, so we don't pollute the user's global
toolchain.
...
- ``manual`` — never install; if a binary is missing, the server is
  silently skipped and the user is told about it via ``sarthi lsp
  status``.
...
# ``<SARTHI_HOME>/lsp/bin/`` first, then on PATH.
#
# Optional fields:
...
INSTALL_RECIPES: Dict[str, Dict[str, Any]] = {
    ...
    # `sarthi lsp status` reports the host's presence.
...
def sarthi_lsp_bin_dir() -> Path:
    """Return the Sarthi-owned bin staging dir for LSP servers."""
    from sarthi_constants import get_sarthi_home

    p = get_sarthi_home() / "lsp" / "bin"
...
def _existing_binary(name: str) -> Optional[str]:
    ...
    for staged in _native_binary_candidates(sarthi_lsp_bin_dir() / name):
...
def _install_npm(
    ...
    """
    # Managed npm first: $SARTHI_HOME/node is not on an arbitrary process's
    # PATH, so a bare which() misses the Node that Sarthi installed and
    ...
    if npm is None:
        ...
        return None
    staging = sarthi_lsp_bin_dir().parent  # <SARTHI_HOME>/lsp/
    ...
    for c in _native_binary_candidates(nm_bin):
        if c.exists():
            ...
            link = sarthi_lsp_bin_dir() / c.name
            if not link.exists():
                try:
                    link.symlink_to(c)
...
def _install_pip(pkg: str, bin_name: str) -> Optional[str]:
    """Install a Python package into a sarthi-owned target dir.
    ...
    console script.
    """
    pip_target = sarthi_lsp_bin_dir().parent / "python-packages"
    ...
    for script_dir in script_dirs:
        for bin_path in _native_binary_candidates(script_dir / bin_name):
            if bin_path.exists():
                link = sarthi_lsp_bin_dir() / bin_path.name
                if not link.exists():
                    try:
...
def detect_status(pkg: str) -> str:
    ...
    Used by the ``sarthi lsp status`` CLI to give users a quick
