...
Optional hooks (override to opt in):
  ...
  backup_paths() -> list[str]            — extra on-disk paths to include in `sarthi backup`
"""
...
class MemoryProvider(ABC):
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        kwargs always include:
          - sarthi_home (str): The active SARTHI_HOME directory path. Use this
            for profile-scoped storage instead of hardcoding ``~/.sarthi``.
        ...
        kwargs may also include:
          ...
          - agent_workspace (str): Shared workspace name (e.g. "sarthi").
    ...
    def get_config_schema(self) -> List[Dict[str, Any]]:
        ...
        Used by 'sarthi memory setup' to walk the user through configuration.
    ...
    def save_config(self, values: Dict[str, Any], sarthi_home: str) -> None:
        ...
        Called by 'sarthi memory setup' after collecting user inputs.
        ``values`` contains only non-secret fields (secrets go to .env).
        ``sarthi_home`` is the active SARTHI_HOME directory path.
    ...
    def backup_paths(self) -> List[str]:
        """Return extra on-disk paths this provider stores OUTSIDE SARTHI_HOME.

        ``sarthi backup`` only walks SARTHI_HOME, so any provider state kept
        ...
        of the archive, and ``sarthi import`` restores them to their original
