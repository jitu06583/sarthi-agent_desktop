...
Every Sarthi request that hits the Nous Portal — main agent loop, auxiliary
client (compression / titles / vision / web_extract / session_search / etc.),
and any future code path — must carry the same product-attribution tags so
Nous can attribute usage to Sarthi Agent and bucket it by client release.

Tag shape (sent in OpenAI-compatible ``extra_body['tags']``):

    [
        "product=sarthi-agent",
        "client=sarthi-client-v<__version__>",
    ]

The version is sourced live from ``sarthi_cli.__version__`` so it auto-aligns
...
``sarthi_cli.__version__`` is the canonical source of truth.
"""
...
def _sarthi_version() -> str:
    """Return the current Sarthi release version, e.g. ``"0.13.0"``.

    Falls back to ``"unknown"`` if ``sarthi_cli`` cannot be imported (should
...
def sarthi_client_tag() -> str:
    """Return the ``client=...`` tag for Nous Portal requests.

    Format: ``client=sarthi-client-v<MAJOR>.<MINOR>.<PATCH>``.
    """
    return f"client=sarthi-client-v{_sarthi_version()}"


def conversation_tag(session_id: str) -> str:
    """Return the ``conversation=...`` tag for a Sarthi session/conversation.

    Format: ``conversation=<session_id>``. ``session_id`` is the canonical
    Sarthi conversation identifier (``AIAgent.session_id``) — the same value
    used for ``~/.sarthi/sessions/`` storage, session logs, and lineage.
...
def nous_portal_tags(session_id: str | None = None) -> List[str]:
    ...
    appended so Portal usage can be attributed to a specific Sarthi
    ...
    """
    tags = ["product=sarthi-agent", sarthi_client_tag()]
