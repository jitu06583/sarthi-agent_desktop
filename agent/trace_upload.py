"""Upload a Sarthi session transcript to Hugging Face as an agent trace.

Sarthi stores sessions in its own SQLite store (``sarthi_state.SessionDB``),
...
* **Zero LLM turn.** This is a deterministic export — it never spends a
  model call. The ``sarthi trace upload`` subcommand calls
...
* **Private by default.** Traces can contain prompts, tool output, local
  ...
  is passed through Sarthi' secret redactor (``force=True``) unless the
...
DEFAULT_DATASET_NAME = "sarthi-traces"
_SARTHI_VERSION = "sarthi-agent"
...
# ---------------------------------------------------------------------------
# Conversion: Sarthi OpenAI-format messages -> Claude Code JSONL
# ---------------------------------------------------------------------------
...
def _redact(text: Any, enabled: bool) -> Any:
    ...
    Non-strings pass through untouched. Uses Sarthi' shared redactor with
...
def build_trace_jsonl(
    ...
    *,
    ...
    """Render Sarthi conversation messages as Claude Code JSONL text.
...
_NO_TOKEN_MESSAGE = (
    ...
    "2. Add it to your environment as HF_TOKEN (e.g. in ~/.sarthi/.env):\n"
    "     HF_TOKEN=hf_xxxxxxxxxxxxxxxxxxxx\n"
    "3. Run /upload-trace again (or `sarthi trace upload`)."
)
...
def upload_session_trace(
    ...
    *,
    ...
    the user's private ``{user}/sarthi-traces`` dataset. Returns a
