...
# Sarthi WebUI, NAS "update" flows that preserve an old entrypoint, etc.)
...
#
# Test hook: SARTHI_TINI_SHIM_TARGET overrides the `/init` path (and
# SARTHI_TINI_SHIM_WRAPPER the main-wrapper path) so unit tests can
# record argv without a real s6 tree.

set -e

INIT_TARGET="${SARTHI_TINI_SHIM_TARGET:-/init}"
WRAPPER="${SARTHI_TINI_SHIM_WRAPPER:-/opt/sarthi/docker/main-wrapper.sh}"
