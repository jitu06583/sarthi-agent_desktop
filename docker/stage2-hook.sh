...
#
# Wired into the image as /etc/cont-init.d/01-sarthi-setup by the
...
SARTHI_HOME="${SARTHI_HOME:-/opt/data}"
INSTALL_DIR="/opt/sarthi"

# Drop to sarthi via s6-setuidgid, but skip it when already non-root.
as_sarthi() { [ "$(id -u)" = 0 ] || { "$@"; return; }; s6-setuidgid sarthi "$@"; }
...
# starts non-root. The baked install tree under /opt/sarthi is intentionally
# root-owned and non-writable; mutable runtime state must live under
# $SARTHI_HOME. An arbitrary `--user` UID therefore cannot repair or populate
...
# default) and pass SARTHI_UID/SARTHI_GID — or the PUID/PGID aliases — which the
...
# `id -u sarthi` is the unremapped build UID (10000) because no root-only remap
# could run. root starts (id -u = 0) and the normal supervised drop to the
# sarthi UID are both unaffected.
cur_uid="$(id -u)"
if [ "$cur_uid" != 0 ] && [ "$cur_uid" != "$(id -u sarthi)" ]; then
    cat >&2 <<EOF
[stage2] ERROR: container started with --user $cur_uid (an arbitrary, non-sarthi UID).
...
and the baked /opt/sarthi install tree is intentionally root-owned and
...
Start the container as root (the default) and pass your host UID/GID instead:
    ...
    docker run -e SARTHI_UID=\$(id -u) -e SARTHI_GID=\$(id -g) ...

NAS users (Synology / unRAID / UGOS) can use the PUID/PGID aliases:

    docker run -e PUID=\$(id -u) -e PGID=\$(id -g) ...

The image remaps the sarthi user to that UID/GID at boot and chowns the data
...
# --- Bootstrap SARTHI_HOME as root ---
# Create the directory (and any missing parents) while we still have root
# privileges so the chown checks below see real metadata and the later
# `s6-setuidgid sarthi mkdir -p` block doesn't EACCES on root-owned
# ancestors. Without this, custom SARTHI_HOME paths whose parents only
# root can create (e.g. `SARTHI_HOME=/home/sarthi/.sarthi` in a Compose
...
# --- UID/GID remap ---
# Accept PUID/PGID as aliases for SARTHI_UID/SARTHI_GID.  NAS users (UGOS,
...
# UID 10000 leaves the runtime unable to read the volume.  SARTHI_UID/
# SARTHI_GID still win when both are set.  See #15290, salvages #25872.
SARTHI_UID="${SARTHI_UID:-${PUID:-}}"
SARTHI_GID="${SARTHI_GID:-${PGID:-}}"

if [ -n "${SARTHI_UID:-}" ] && validate_uid_gid "$SARTHI_UID" && [ "$SARTHI_UID" != "$(id -u sarthi)" ]; then
    echo "[stage2] Changing sarthi UID to $SARTHI_UID"
    usermod -u "$SARTHI_UID" sarthi
fi
if [ -n "${SARTHI_GID:-}" ] && validate_uid_gid "$SARTHI_GID" && [ "$SARTHI_GID" != "$(id -g sarthi)" ]; then
    echo "[stage2] Changing sarthi GID to $SARTHI_GID"
    # -o allows non-unique GID (e.g. macOS GID 20 "staff" may already
    # exist as "dialout" in the Debian-based container image).
    groupmod -o -g "$SARTHI_GID" sarthi 2>/dev/null || true
fi
...
# host's `docker` group (or root). The supervised sarthi user (UID 10000)
...
# sarthi process with `Groups: 10000` (998 gone); after this hook adds
...
# matching entry that includes sarthi. Idempotent across container
...
#   - sarthi is already a member of the right group (idempotent restart)
...
for sock in /var/run/docker.sock /run/docker.sock; do
    ...
    if id -G sarthi 2>/dev/null | tr ' ' '\n' | grep -qx "$sock_gid"; then
        echo "[stage2] sarthi already in group $sock_gid for $sock"
        break
    fi
    ...
    if usermod -aG "$sock_group" sarthi 2>/dev/null; then
        echo "[stage2] Added sarthi to group $sock_group (GID $sock_gid) for $sock"
    else
        echo "[stage2] Warning: usermod -aG $sock_group sarthi failed; docker backend may fail with EACCES"
    fi
    break
done

# --- Fix ownership of data volume ---
# When SARTHI_UID is remapped or the top-level $SARTHI_HOME isn't owned by
# the runtime sarthi UID, restore ownership to sarthi — but ONLY for the
# directories sarthi actually writes to. The full $SARTHI_HOME may be a
# host-mounted bind containing unrelated user files; `chown -R` would
# silently destroy host ownership of those (see issue #19788).
#
# The canonical list of sarthi-owned subdirs is the same one the s6-setuidgid
# mkdir -p block below seeds. Keep them in sync if the seed list changes.
actual_sarthi_uid=$(id -u sarthi)
...
chown_sarthi_tree() {
    ...
    chown -R sarthi:sarthi "$target" 2>/dev/null || \
        echo "[stage2] Warning: chown $target failed (rootless container?) — continuing"
}

tree_has_non_sarthi_owner() {
    target="$1"
    find "$target" \( ! -user sarthi -o ! -group sarthi \) -print -quit 2>/dev/null | grep -q .
}

needs_chown=false
if [ "$(stat -c %u "$SARTHI_HOME" 2>/dev/null)" != "$actual_sarthi_uid" ]; then
    needs_chown=true
fi
if [ "$needs_chown" = true ]; then
    echo "[stage2] Fixing ownership of $SARTHI_HOME (targeted) to sarthi ($actual_sarthi_uid)"
    # In rootless Podman the container's "root" is mapped to an
    # unprivileged host UID — chown will fail. That's fine: the volume
    # is already owned by the mapped user on the host side.
    #
    # Top-level $SARTHI_HOME: chown the directory itself (not its contents)
    # so sarthi can mkdir new subdirs but bind-mounted host files keep
    # their existing ownership.
    if refuse_symlinked_path "chown" "$SARTHI_HOME"; then
        :
    else
        chown sarthi:sarthi "$SARTHI_HOME" 2>/dev/null || \
            echo "[stage2] Warning: chown $SARTHI_HOME failed (rootless container?) — continuing"
    fi
    # Sarthi-owned subdirs: recursive chown is safe here because these are
    # created and managed exclusively by sarthi (see the s6-setuidgid mkdir
    ...
    for sub in cron sessions logs hooks memories skills skins plans workspace home profiles pairing platforms/pairing lazy-packages; do
        if [ -e "$SARTHI_HOME/$sub" ] && tree_has_non_sarthi_owner "$SARTHI_HOME/$sub"; then
            chown_sarthi_tree "$SARTHI_HOME/$sub"
        fi
    done
fi
...
# sarthi user. Hosted/container instances keep mutable state under
# $SARTHI_HOME (/opt/data) and run with PYTHONDONTWRITEBYTECODE plus
# SARTHI_DISABLE_LAZY_INSTALLS=1. Keeping /opt/sarthi root-owned and
...
# $SARTHI_HOME/lazy-packages dir on the data volume (Dockerfile sets
# SARTHI_LAZY_INSTALL_TARGET). That dir is appended to the END of sys.path,
...
# even though installs are re-enabled. The dir is seeded + chowned to sarthi
...
# Always reset ownership of $SARTHI_HOME/profiles to sarthi on every
# boot. Profile dirs and files can land owned by root when commands
# are invoked via `docker exec <container> sarthi …` (which defaults
# to root unless `-u` is passed), and that breaks the cont-init
# reconciler (02-reconcile-profiles) which runs as sarthi and walks
...
if [ -d "$SARTHI_HOME/profiles" ] && tree_has_non_sarthi_owner "$SARTHI_HOME/profiles"; then
    chown_sarthi_tree "$SARTHI_HOME/profiles"
fi

# Always reset ownership of $SARTHI_HOME/cron on every boot for the same
# docker-exec/root-write reason as profiles/. The cron scheduler state
# (jobs.json) must stay readable by the unprivileged sarthi runtime even
...
# gate as profiles/).
if [ -d "$SARTHI_HOME/cron" ] && tree_has_non_sarthi_owner "$SARTHI_HOME/cron"; then
    chown_sarthi_tree "$SARTHI_HOME/cron"
fi

# Always ensure logs/gateways is sarthi-owned (#45258). Formerly healed by
...
# top-level $SARTHI_HOME is mis-owned, so a warm volume with sarthi-owned
# SARTHI_HOME but root-owned logs/gateways would otherwise leave
# s6-setuidgid sarthi mkdir failing with Permission denied. Non-recursive:
# profile leaf dirs are each created/owned by their own log/run as sarthi.
if [ -d "$SARTHI_HOME/logs/gateways" ]; then
    if refuse_symlinked_path "chown" "$SARTHI_HOME/logs/gateways"; then
        :
    else
        chown sarthi:sarthi "$SARTHI_HOME/logs/gateways" 2>/dev/null || true
    fi
fi
...
# sarthi pairing approve …` defaults to uid=0 and writes 0600 root-owned
# approval files that the unprivileged sarthi gateway cannot read,
# silently leaving the approved user unauthorized (#10270). The targeted
# data-volume chown above only runs when the top-level $SARTHI_HOME is
...
# and cron/.
if [ -d "$SARTHI_HOME/platforms/pairing" ] && tree_has_non_sarthi_owner "$SARTHI_HOME/platforms/pairing"; then
    chown_sarthi_tree "$SARTHI_HOME/platforms/pairing"
fi
# Legacy location (pre-consolidated layout).
if [ -d "$SARTHI_HOME/pairing" ] && tree_has_non_sarthi_owner "$SARTHI_HOME/pairing"; then
    chown_sarthi_tree "$SARTHI_HOME/pairing"
fi

# Reset ownership of sarthi-owned top-level state files on every boot.
# The targeted data-volume chown above only covers sarthi-owned
# *subdirectories*; loose state files living directly under $SARTHI_HOME
# are missed. When those files are created or rewritten by
# `docker exec <container> sarthi …` (root unless `-u` is passed) they
# land root-owned, and the unprivileged sarthi runtime then hits
...
# sweep so host-owned files in a bind-mounted $SARTHI_HOME are never
...
# entries of sarthi_cli.profile_distribution.USER_OWNED_EXCLUDE plus the
...
for f in \
    ...
    active_profile; do
    if [ -e "$SARTHI_HOME/$f" ]; then
        if refuse_symlinked_path "chown" "$SARTHI_HOME/$f"; then
            :
        else
            chown sarthi:sarthi "$SARTHI_HOME/$f" 2>/dev/null || true
        fi
    fi
done

# --- config.yaml permissions ---
# Ensure config.yaml is readable by the sarthi runtime user even if it
# was edited on the host after initial ownership setup.
if [ -f "$SARTHI_HOME/config.yaml" ]; then
    if refuse_symlinked_path "chown/chmod" "$SARTHI_HOME/config.yaml"; then
        :
    else
        chown sarthi:sarthi "$SARTHI_HOME/config.yaml" 2>/dev/null || true
        chmod 640 "$SARTHI_HOME/config.yaml" 2>/dev/null || true
    fi
fi

# --- Seed directory structure as sarthi user ---
# Run as sarthi via s6-setuidgid so dirs end up owned correctly (matters
# under rootless Podman where chown back to root would fail).
#
# Use direct `mkdir -p` invocation (no `sh -c "..."` wrapper) so the
# shell isn't a second interpreter — defends against $SARTHI_HOME values
# containing shell metacharacters. PR #30136 review item O2.
as_sarthi mkdir -p \
    "$SARTHI_HOME/backups" \
    "$SARTHI_HOME/cron" \
    "$SARTHI_HOME/sessions" \
    "$SARTHI_HOME/logs" \
    "$SARTHI_HOME/logs/gateways" \
    "$SARTHI_HOME/hooks" \
    "$SARTHI_HOME/memories" \
    "$SARTHI_HOME/skills" \
    "$SARTHI_HOME/skins" \
    "$SARTHI_HOME/plans" \
    "$SARTHI_HOME/workspace" \
    "$SARTHI_HOME/home" \
    "$SARTHI_HOME/pairing" \
    "$SARTHI_HOME/platforms/pairing" \
    "$SARTHI_HOME/lazy-packages"

# --- Install-method stamp ---
# The 'docker' stamp is baked into the immutable install tree at
# /opt/sarthi/.install_method (see Dockerfile), NOT written here into
# $SARTHI_HOME. detect_install_method() reads the code-scoped stamp first.
#
# Why we no longer stamp $SARTHI_HOME: it is a shared DATA volume, commonly
# bind-mounted from the host (~/.sarthi:/opt/data) and sometimes shared with a
...
# 'sarthi update'. To heal homes already poisoned by older images, remove a
# stale 'docker' stamp from $SARTHI_HOME if one is present (the host install's
# own installer re-creates its code-scoped stamp; a genuine container relies on
# the baked /opt/sarthi stamp, so deleting the data-dir copy is safe).
if [ -f "$SARTHI_HOME/.install_method" ]; then
    stamped="$(tr -d '[:space:]' < "$SARTHI_HOME/.install_method" 2>/dev/null || true)"
    if [ "$stamped" = "docker" ]; then
        rm -f "$SARTHI_HOME/.install_method" 2>/dev/null || true
    fi
fi
...
seed_one() {
    dest=$1
    src=$2
    if [ ! -f "$SARTHI_HOME/$dest" ] && [ -f "$INSTALL_DIR/$src" ]; then
        if refuse_symlinked_path "seed" "$SARTHI_HOME/$dest"; then
            :
        else
            as_sarthi cp "$INSTALL_DIR/$src" "$SARTHI_HOME/$dest"
        fi
    fi
}
...
# the first-boot seed above depends on /opt/sarthi/.env.example being present
...
# not generate one. Sarthi loads $SARTHI_HOME/.env with override=True, so
...
if [ -n "${API_SERVER_KEY:-}" ]; then
    if [ -f "$SARTHI_HOME/.env" ] && grep -q '^API_SERVER_KEY=..*' "$SARTHI_HOME/.env" 2>/dev/null; then
        echo "[stage2] Warning: API_SERVER_KEY is set in both the container environment and $SARTHI_HOME/.env — the .env value wins at runtime (loaded with override=True)"
    else
        ...
        if [ -f "$SARTHI_HOME/.env" ] && ! refuse_symlinked_path "clean" "$SARTHI_HOME/.env"; then
            sed -i '/^API_SERVER_KEY=$/d' "$SARTHI_HOME/.env" 2>/dev/null || true
        fi
        echo "[stage2] API_SERVER_KEY provided via container environment — skipping generation"
    fi
elif ! grep -q '^API_SERVER_KEY=..*' "$SARTHI_HOME/.env" 2>/dev/null; then
    if refuse_symlinked_path "append" "$SARTHI_HOME/.env"; then
        :
    else
        if [ ! -f "$SARTHI_HOME/.env" ]; then
            ...
            (umask 077 && as_sarthi touch "$SARTHI_HOME/.env") 2>/dev/null || true
        fi
        if [ -f "$SARTHI_HOME/.env" ]; then
            ...
            if [ -n "$_gen_key" ]; then
                ...
                sed -i '/^API_SERVER_KEY=$/d' "$SARTHI_HOME/.env" 2>/dev/null || true
                if printf 'API_SERVER_KEY=%s\n' "$_gen_key" >> "$SARTHI_HOME/.env" 2>/dev/null; then
                    echo "[stage2] Generated API_SERVER_KEY for the loopback gateway api_server"
                else
                    echo "[stage2] Warning: could not write API_SERVER_KEY to $SARTHI_HOME/.env (read-only volume?) — gateway api_server (cron fires) will be unavailable"
                fi
            fi
            unset _gen_key
        else
            echo "[stage2] Warning: could not create $SARTHI_HOME/.env — gateway api_server (cron fires) will be unavailable"
        fi
    fi
fi
...
if [ -f "$SARTHI_HOME/.env" ]; then
    if refuse_symlinked_path "chown/chmod" "$SARTHI_HOME/.env"; then
        :
    else
        chown sarthi:sarthi "$SARTHI_HOME/.env" 2>/dev/null || true
        chmod 600 "$SARTHI_HOME/.env" 2>/dev/null || true
    fi
fi
...
# as the unprivileged `sarthi` user — without this it gets EACCES on every
...
# sarthi group (any group member could e.g. stop/suspend this machine), not
...
if [ -S /.fly/api ]; then
    if refuse_symlinked_path "chgrp/chmod" /.fly/api; then
        :
    elif chgrp sarthi /.fly/api 2>/dev/null && chmod g+w /.fly/api 2>/dev/null; then
        echo "[stage2] Granted sarthi group access to the Fly Machines API socket"
    else
...
# $SARTHI_HOME on the mounted volume. Run the same safe, non-interactive
# config-schema migrations that `sarthi update` runs for non-Docker installs,
# after first-boot seeding and before supervised gateway services start.
# Set SARTHI_SKIP_CONFIG_MIGRATION=1 for controlled/manual migrations.
if [ -f "$SARTHI_HOME/config.yaml" ]; then
    s6-setuidgid sarthi "$INSTALL_DIR/.venv/bin/python" "$INSTALL_DIR/scripts/docker_config_migrate.py" \
...
if [ ! -f "$SARTHI_HOME/auth.json" ] && [ -n "${SARTHI_AUTH_JSON_BOOTSTRAP:-}" ]; then
    if refuse_symlinked_path "seed" "$SARTHI_HOME/auth.json"; then
        :
    else
        printf '%s' "$SARTHI_AUTH_JSON_BOOTSTRAP" > "$SARTHI_HOME/auth.json"
        chown sarthi:sarthi "$SARTHI_HOME/auth.json" 2>/dev/null || true
        chmod 600 "$SARTHI_HOME/auth.json"
    fi
fi
...
# supply a freshly-issued session via SARTHI_AUTH_JSON_REBOOTSTRAP (distinct
...
if [ -f "$SARTHI_HOME/auth.json" ] && [ -n "${SARTHI_AUTH_JSON_REBOOTSTRAP:-}" ]; then
    if refuse_symlinked_path "reseed" "$SARTHI_HOME/auth.json"; then
        :
    else
        s6-setuidgid sarthi "$INSTALL_DIR/.venv/bin/python" \
            "$INSTALL_DIR/scripts/docker_rebootstrap_nous_session.py" \
            "$SARTHI_HOME/auth.json" \
...
# can set SARTHI_GATEWAY_BOOTSTRAP_STATE=running; we seed the state file
...
# SARTHI_AUTH_JSON_BOOTSTRAP seeds auth.json. The [ ! -f ] guard is the
...
if [ ! -f "$SARTHI_HOME/gateway_state.json" ] && \
        [ "${SARTHI_GATEWAY_BOOTSTRAP_STATE:-}" = "running" ]; then
    if refuse_symlinked_path "seed" "$SARTHI_HOME/gateway_state.json"; then
        :
    else
        printf '{"gateway_state":"running"}\n' > "$SARTHI_HOME/gateway_state.json"
        chown sarthi:sarthi "$SARTHI_HOME/gateway_state.json" 2>/dev/null || true
        chmod 644 "$SARTHI_HOME/gateway_state.json"
    fi
fi
...
if [ -d "$INSTALL_DIR/skills" ]; then
    as_sarthi "$INSTALL_DIR/.venv/bin/python" "$INSTALL_DIR/tools/skills_sync.py" \
        || echo "[stage2] Warning: skills_sync.py failed; continuing"
fi

# --- Discover agent-browser's Chromium binary ---
# The image's Dockerfile runs `npx playwright install chromium`, which
# populates ``$PLAYWRIGHT_BROWSERS_PATH`` (=/opt/sarthi/.playwright) with
# a ``chromium_headless_shell-<build>/chrome-headless-shell-linux64/``
# directory. agent-browser (the runtime CLI Sarthi spawns for the
...
# main-wrapper.sh propagates it into the supervised ``sarthi`` process
...
if [ -z "${AGENT_BROWSER_EXECUTABLE_PATH:-}" ] && \
    ...
    if [ -n "$browser_bin" ]; then
        ...
        # up for all supervised services (main-sarthi, dashboard, etc.).
