...
#   first arg is anything else    → exec `sarthi <args>` (subcommand passthrough)
#
...
# an arbitrary non-root, non-sarthi UID, the bootstrap was skipped and the
# baked image dirs (owned by the sarthi build UID) are unwritable, so fail
...
if [ "$cur_uid" != 0 ] && [ "$cur_uid" != "$(id -u sarthi)" ]; then
    cat >&2 <<EOF
[sarthi] ERROR: container started with --user $cur_uid (an arbitrary, non-sarthi UID) — not supported.
...
# to the sarthi user's home before dropping privileges so libraries that
...
_sarthi_orig_cwd="${SARTHI_ORIG_CWD:-$PWD}"
...
# the user's command so `sarthi chat` starts in the Docker -w
...
# Sarthi subcommand pass-through.
drop sarthi "$@"
