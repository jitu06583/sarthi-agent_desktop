...
class SessionDB:
    ...
    def _sqlite_supports_fts5(self, cursor: sqlite3.Cursor) -> bool:
        try:
            cursor.execute("CREATE VIRTUAL TABLE temp._sarthi_fts5_probe USING fts5(x)")
            cursor.execute("DROP TABLE temp._sarthi_fts5_probe")
            return True
        except sqlite3.OperationalError as exc:
    ...
    def search_sessions_by_id(
        ...
        a session id from logs, CLI output, or another Sarthi surface and jump
    ...
    def optimize_fts(self) -> int:
        ...
        disabled via ``SARTHI_DISABLE_FTS_TRIGRAM`` or not yet created), so
