...
unprivileged `sarthi` user, giving the layout most people have —
$SARTHI_HOME/sarthi-agent plus a ~/.local/bin launcher. Pass --root for the FHS
...
Environment:
  SARTHI_DEV_SANDBOX_DIR    Sandbox directory name, relative to the repo root
                            (default: .sarthi-sandbox).

Examples:
  # create a sandbox, install this branch as `main`, and then drop to a shell,
  # skipping `sarthi setup` & the browser tools for speed.
...
GIT_ROOT="${SARTHI_SANDBOX_SOURCE_ROOT:-$(git rev-parse --show-toplevel)}"
...
if [ -n "$INSTALL_REF" ]; then
  ...
  UPSTREAM_REPO="$(mktemp -d -t sarthi-sandbox-upstream.XXXXXX)"
...
if [ "$INSTALL_SHORTCUT" = true ]; then
  ...
  set -- bash -c '
    ...
    if [ "$install_status" -eq 0 ] && [ -f /work/promote-main ]; then
      ...
      if git --git-dir=/work/repos/sarthi-agent.git update-ref refs/heads/main "$next_main"; then
...
# A user-level install writes the `sarthi` launcher to ~/.local/bin and the
# checkout to $SARTHI_HOME; both live under the sandbox HOME, which is bound
...
SOURCE_REF="$COMMIT"
SNAPSHOT_REPO=""
FAKE_REPO="$SANDBOX_ROOT/root/repos/sarthi-agent.git"
git -C "$SANDBOX_ROOT/root/repos" init --bare -q sarthi-agent.git
if [ -n "$INSTALL_REF" ]; then
...
if [ -n "$(git -C "$GIT_ROOT" status --porcelain)" ]; then
  ...
  SNAPSHOT_REPO="$(mktemp -d -t sarthi-sandbox-snapshot.XXXXXX)"
  git -C "$SNAPSHOT_REPO" init -q
  git -C "$SNAPSHOT_REPO" fetch -q "$GIT_ROOT" "$COMMIT"
  git -C "$SNAPSHOT_REPO" config user.name 'Sarthi sandbox'
...
if [ -n "$INSTALL_REF" ]; then
  git --git-dir="$FAKE_REPO" fetch -q --force "$SOURCE_REPO" \
    "$SOURCE_REF:refs/sarthi-sandbox/next"
