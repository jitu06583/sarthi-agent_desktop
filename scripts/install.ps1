...
#
# Usage:
#   iex (irm https://sarthi-agent.vercel.app/install.ps1)
#
...
function Test-Node {
    ...
    try {
        ...
        if ($zipName) {
            ...
            if ($extractedDir) {
                if (Test-Path "$SarthiHome\node") { Remove-Item -Recurse -Force "$SarthiHome\node" }
                Move-Item $extractedDir.FullName "$SarthiHome\node"
...
function Set-PathVariable {
    ...
    if ($NoVenv) {
        ...
        $sarthiBin = "$InstallDir\venv\Scripts"
    }
    
    # Add the venv Scripts dir to user PATH so sarthi is globally available
    # On Windows, the sarthi.exe in venv\Scripts\ has the venv Python baked in
...
function Install-Desktop {
    ...
    # enforces the build floor (^20.19 || >=22.12) and prepends the Sarthi-managed
...
try {
    ...
    Write-Host "  Invoke-WebRequest -Uri 'https://sarthi-agent.vercel.app/install.ps1' -OutFile install.ps1" -ForegroundColor Yellow
