# ============================================================================
# Sarthi Agent Installer for Windows
# ============================================================================
...
param(
    ...
    [switch]$ForceCommit,
    [string]$Tag = "",
    [string]$SarthiHome = $(if ($env:SARTHI_HOME) { $env:SARTHI_HOME } else { "$env:LOCALAPPDATA\sarthi" }),
    [string]$InstallDir = $(if ($env:SARTHI_HOME) { "$env:SARTHI_HOME\sarthi-agent" } else { "$env:LOCALAPPDATA\sarthi\sarthi-agent" }),
    ...
    # builds apps/desktop into a launchable Sarthi.exe.
    #
    # Why opt-in:
    #   * Sarthi-Setup.exe (the signed Tauri bootstrap installer) passes
    ...
    #     from inside an already-launched Sarthi.exe; if THAT recursively
    #     built apps/desktop it would try to overwrite the live Sarthi.exe
    ...
    #     `sarthi desktop` already builds on demand.
...
# SARTHI_HOME and InstallDir -- in that short form:
...
function Write-PathDiag {
    ...
    [Console]::Error.WriteLine("[sarthi] $Message")
}
...
function ConvertTo-LongPath {
    ...
    try {
        if (-not ([System.Management.Automation.PSTypeName]'SarthiInstall.LongPath').Type) {
            Add-Type -Namespace 'SarthiInstall' -Name 'LongPath' -MemberDefinition @'
...
'@
        ...
        $length = [SarthiInstall.LongPath]::GetLongPathNameW($Path, $buffer, $buffer.Capacity)
...
# long. An explicitly passed -SarthiHome / -InstallDir is normalized in place
...
if ($PSBoundParameters.ContainsKey('SarthiHome')) {
    $SarthiHome = ConvertTo-LongPath $SarthiHome
} else {
    $SarthiHome = ConvertTo-LongPath $(
        if ($env:SARTHI_HOME) { $env:SARTHI_HOME } else { "$env:LOCALAPPDATA\sarthi" }
    )
}
if ($PSBoundParameters.ContainsKey('InstallDir')) {
    ...
    $InstallDir = ConvertTo-LongPath $(
        if ($env:SARTHI_HOME) { "$env:SARTHI_HOME\sarthi-agent" } else { "$env:LOCALAPPDATA\sarthi\sarthi-agent" }
    )
}
if ($script:NormalizedProfilePaths) {
    ...
    Write-PathDiag "resolved install paths: SarthiHome=$SarthiHome InstallDir=$InstallDir"
}
...
function Write-Banner {
    ...
    Write-Host "|             * Sarthi Agent Installer                    |" -ForegroundColor Magenta
    Write-Host "+---------------------------------------------------------+" -ForegroundColor Magenta
...
function Write-BrowserEnv {
    ...
    if (-not (Test-Path $SarthiHome)) {
        New-Item -ItemType Directory -Force -Path $SarthiHome | Out-Null
    }
    $envFile = Join-Path $SarthiHome ".env"
...
function Install-AgentBrowser {
    ...
    # which every consumer (tools/browser_tool.py, `sarthi update`'s npx
    ...
    $prefixDir = Join-Path $SarthiHome "node"
...
function Install-Uv {
    # Sarthi owns its own uv at $SarthiHome\bin\uv.exe.  Always install there --
    # no PATH probing, no conda guards, no multi-location resolution chains.
    # The runtime update path (sarthi_cli/managed_uv.py) looks in the same
    # place, so install.ps1 and `sarthi update` stay in sync.
    $managedUv = Join-Path $SarthiHome "bin\uv.exe"
    ...
    Write-Info "Installing managed uv into $SarthiHome\bin ..."
    New-Item -ItemType Directory -Path (Join-Path $SarthiHome "bin") -Force | Out-Null

    # UV_INSTALL_DIR tells the astral installer to place the binary
    # directly into $SarthiHome\bin instead of ~/.local/bin.
    $prevEAP = $ErrorActionPreference
    try {
        $ErrorActionPreference = "Continue"
        $env:UV_INSTALL_DIR = Join-Path $SarthiHome "bin"
        ...
        # (sarthi_cli/managed_uv.py looks only at $SarthiHome\bin\uv.exe).
...
# Put the Sarthi-managed Node dir at the FRONT of the persisted User PATH.
#
...
# environment (a standalone sarthi-setup.exe run, a user typing `npm`) silently
...
# Upgrade the Sarthi-managed Node tree's bundled npm into $NpmRange.
#
...
function Update-ManagedNpm {
    ...
    if (Test-ManagedNodeInUse $NodeDir) {
        Write-Warn "Sarthi-managed Node.js is in use by a running app; skipping the bundled npm upgrade (applies on a later update with the app closed)."
        return $false
    }

    Write-Info "Upgrading bundled npm to satisfy $range ..."

    $tmpCwd = Join-Path $env:TEMP ("sarthi-npm-upgrade-" + [Guid]::NewGuid().ToString("N"))
...
function Resolve-UvCmd {
    ...
    # sibling process and SARTHI_HOME wasn't propagated).
...
function Resolve-AvailablePythonVersion {
    ...
    # ``$script:PythonVersion = $fallbackVer`` mutation.  Under Sarthi-Setup.exe
...
function New-GitBashAslrFailureReason {
    ...
    return @(
        ...
        "Then rerun Sarthi setup. If the override is blocked or later re-applied, ask your Windows administrator to allow this per-program exception."
...
function Install-Git {
    ...
    which Sarthi uses to run shell commands.
    ...
    package manager):
      ...
      2. Download **PortableGit** from the official git-for-windows GitHub
         ...
         ``%LOCALAPPDATA%\sarthi\git`` -- never touches system Git, never
    ...
    **Why PortableGit, not MinGit:**  MinGit is the minimal-automation
    ...
    Sarthi needs ``bash.exe`` to run shell commands.  PortableGit is the
    ...
    blocked).  Owning the Sarthi copy of Git ourselves is predictable and
    recoverable: if it ever breaks, ``Remove-Item %LOCALAPPDATA%\sarthi\git``
    and re-running this installer fully recovers.

    After install we locate ``bash.exe`` and persist the path in
    ``SARTHI_GIT_BASH_PATH`` (User scope) so Sarthi can find it in a fresh
    ...
    if (Get-Command git -ErrorAction SilentlyContinue) {
        ...
        Write-Info "Trying a Sarthi-managed PortableGit install instead..."
    }

    # Download PortableGit into $SarthiHome\git.  Always works as long as
    # we can reach github.com -- no admin, no winget, no reliance on the
    # user's possibly-broken system Git install.
    Write-Info "Git not found -- downloading PortableGit to $SarthiHome\git\ ..."
    ...
    try {
        ...
        if ($arch -eq "32-bit-mingit") {
            Write-Warn "32-bit Windows detected -- PortableGit is 64-bit only.  Installing MinGit 32-bit as a last resort; bash-dependent Sarthi features (terminal tool, agent-browser) will not work on this machine."
        ...
        Write-Info "then re-run this installer.  Sarthi needs Git Bash on Windows to run"
...
function Set-GitBashEnvVar {
    ...
    ``SARTHI_GIT_BASH_PATH`` (User env scope) so Sarthi can find it even before
    ...
    #
    # Layouts:
    #   PortableGit (our default): $SarthiHome\git\bin\bash.exe
    #   MinGit (32-bit fallback):  $SarthiHome\git\usr\bin\bash.exe
    $candidates += "$SarthiHome\git\bin\bash.exe"       # PortableGit layout (primary)
    $candidates += "$SarthiHome\git\usr\bin\bash.exe"   # MinGit / PortableGit usr\bin fallback
    ...
    foreach ($candidate in $candidates) {
        if ($candidate -and (Test-Path $candidate)) {
            [Environment]::SetEnvironmentVariable("SARTHI_GIT_BASH_PATH", $candidate, "User")
            $env:SARTHI_GIT_BASH_PATH = $candidate
            $script:GitBashPath = $candidate
            Write-Info "Set SARTHI_GIT_BASH_PATH=$candidate"
            return
        }
    }

    Write-Warn "Could not locate bash.exe -- Sarthi may not find Git Bash."
    Write-Info "If needed, set SARTHI_GIT_BASH_PATH manually to your bash.exe path."
}
...
function Test-Node {
    ...
    if (Get-Command node -ErrorAction SilentlyContinue) {
        ...
        Write-Warn "Node.js $version is too old (Sarthi requires Node >=26)"
    }

    # Prefer a Sarthi-managed Node from a previous run over a too-old system one.
    $managedNode = "$SarthiHome\node\node.exe"
    if ((Test-Path $managedNode) -and (Test-NodeVersionOk (& $managedNode --version))) {
        $version = & $managedNode --version
        $env:Path = "$SarthiHome\node;$env:Path"
        Set-ManagedNodeFirstOnUserPath "$SarthiHome\node"
        Write-Success "Node.js $version found (Sarthi-managed)"
        ...
        Update-ManagedNpm "$SarthiHome\node" | Out-Null
        $script:HasNode = $true
        return $true
    }

    Write-Info "Installing Sarthi-managed Node.js $NodeVersion LTS..."
    ...
    # The portable zip path drops node.exe + npm into $SarthiHome\node\
    ...
    Write-Info "Downloading portable Node.js $NodeVersion to $SarthiHome\node\ ..."
    ...
    try {
        ...
        if ($zipName) {
            ...
            $tmpDir = "$env:TEMP\sarthi-node-extract"
            ...
            if ($extractedDir) {
                ...
                Get-ChildItem "$SarthiHome" -Directory -Filter "node.old-*" -ErrorAction SilentlyContinue |
                    Where-Object { $_.LastWriteTime -lt (Get-Date).AddMinutes(-10) } |
                    Remove-Item -Recurse -Force -ErrorAction SilentlyContinue
                Get-ChildItem "$SarthiHome" -Directory -Filter "node.new-*" -ErrorAction SilentlyContinue |
                    Where-Object { $_.LastWriteTime -lt (Get-Date).AddMinutes(-10) } |
                    Remove-Item -Recurse -Force -ErrorAction SilentlyContinue
                ...
                if (Test-Path "$SarthiHome\node") {
                    try {
                        Rename-Item "$SarthiHome\node" $backup -ErrorAction Stop
                    } catch {
                        Write-Warn "Sarthi-managed Node.js is in use by a running app; deferring its upgrade. Close the app and re-run the update."
                ...
                $version = & "$SarthiHome\node\node.exe" --version
                Write-Success "Node.js $version installed to $SarthiHome\node\ (portable, user-scoped)"
...
function Install-SystemPackages {
    ...
    if ($hasWinget) {
        ...
        foreach ($pkg in $wingetPkgs) {
            $log = "$env:TEMP\sarthi-winget-$($pkg -replace '[^A-Za-z0-9]','_')-$(Get-Random).log"
            $pkgLogs[$pkg] = $log
...
function Install-Repository {
        ...
        if ($repoValid) {
            ...
            try {
                ...
                if (-not [string]::IsNullOrWhiteSpace(($statusOut -join "`n"))) {
                    ...
                    # the index conflict state is dropped. Mirrors the `sarthi
                    # update` path (#4735).
                    ...
                    $stashName = "sarthi-install-autostash-" + (Get-Date -Format "yyyyMMdd-HHmmss")
                ...
                if ($Commit) {
                    ...
                    # BACKWARDS. sarthi-setup.exe bakes its build-time commit
                    ...
                    # ff-only pull cannot succeed -- mirror ``sarthi update`` and
                ...
                if ($autostashRef) {
                    ...
                    if ($restoreNow) {
                        ...
                        if (($restoreExit -eq 0) -and ($conflictedFiles.Count -eq 0)) {
                            ...
                            Write-Warn "Review git diff / git status if Sarthi behaves unexpectedly."
                        } else {
            ...
            try {
                ...
                Write-Info "terminals, running sarthi processes) and try again."
                throw
            }
        }
    }
    ...
    if (-not $didUpdate) {
        ...
        if (-not $cloneSuccess) {
            ...
            try {
                ...
                $extractPath = "$env:TEMP\sarthi-agent-extract"
                ...
                if ($extractedDir) {
                    ...
                    # `sarthi update` (see the notes at the shared clone-path
    ...
    # next `sarthi update` checkout aborts on a "dirty" tree the user never
...
function Install-Venv {
    ...
    # Re-resolve the interpreter before creating the venv.  Under Sarthi-Setup.exe
    ...
    if (Test-Path -LiteralPath "venv") {
        ...
        # speedups.pyd) are loaded as DLLs by any running sarthi process.
        ...
        if ($env:OS -eq "Windows_NT") {
            $myPid = $PID
            Write-Info "Stopping any running sarthi processes before recreating venv..."
            ...
            try {
                schtasks /Query /FO CSV 2>$null | ConvertFrom-Csv | Where-Object { $_.TaskName -like '*Sarthi_Gateway*' } | ForEach-Object {
                    $tn = $_.TaskName
            ...
            # The launcher CLI (sarthi.exe) plus its child tree.
            & taskkill /F /T /IM sarthi.exe /FI "PID ne $myPid" 2>$null | Out-Null
            # taskkill /IM sarthi.exe is NOT enough: the gateway/agent that a
            # scheduled task or watchdog autostarts runs as
            # `pythonw.exe -m sarthi_cli.main gateway run` straight out of
            # venv\Scripts\, so its image name is python/pythonw, not sarthi.exe.
            ...
            # Sarthi children re-exec through .sarthi-runtime, so killing only
            ...
            # user (see sarthi_cli/gateway_windows.py), so the installer always
        ...
        try {
            ...
            throw (
                ...
                "A process still has the install directory open (often a non-Sarthi " +
    ...
    $env:UV_PYTHON = $venvPythonExe
        ...
        # the task fires normally on next logon and `sarthi update` / the
...
function Restore-VenvBackup {
    ...
    # venv so Sarthi (and the venv-blocker probe) stay usable (#83149).
    $backupName = Get-PendingVenvBackup
    if (-not $backupName) { return }
    try {
...
function Install-Dependencies {
    ...
    if (Test-Path "uv.lock") {
        ...
        # empty and producing the broken state where `sarthi.exe` exists
...
try:
    ...
    out = []
    for s in specs:
        m = re.search(r'sarthi-agent\[([\w-]+)\]', s)
...
"@ 2>$null
    ...
    if (-not $installed) {
        throw "Failed to install sarthi-agent package even with no extras. Inspect the uv pip install output above."
    }
    ...
    # isn't set, leaving venv\ empty and sarthi.exe broken with
    ...
    # here, not 30 seconds later when the user runs `sarthi`.
    if (-not $NoVenv) {
        $venvPython = "$InstallDir\venv\Scripts\python.exe"
        if (-not (Test-Path $venvPython)) {
            throw "Install reported success but $venvPython does not exist. The dependency sync likely landed in a sibling .venv\ directory. Re-run the installer; if it persists, close Sarthi processes and preserve existing venv directories before retrying. Do not delete venv in place."
        }
        ...
        if ($importExitCode -ne 0) {
            ...
            $hint = if (Test-Path $sibling) {
                "Detected sibling .venv\ at $sibling -- uv synced there instead of venv\. Close Sarthi processes, preserve the existing venv, and rerun the installer so the transactional recovery path can move directories safely."
            } else {
    ...
    Complete-VenvTransaction
        ...
        # a failed update leaves Sarthi and its blocker probe usable.
        Restore-VenvBackup
        Pop-Location
        throw
    }
    ...
    if (-not $NoVenv) {
        # uv on Windows can register sarthi.exe in dist-info/RECORD but fail to
        ...
        # `sarthi` command while sarthi-agent.exe / sarthi-acp.exe exist
...
"@ 2>$null
            if ($LASTEXITCODE -eq 0 -and $scriptNames) {
                ...
                if ($missing.Count -gt 0) {
                    ...
                    if ($stillMissing.Count -gt 0) {
                        ...
                        Write-Info "Workaround: `"$pythonExe`" -m sarthi_cli.main <command>"
                    } else {
    ...
    # users hit and lazy-import errors from `sarthi dashboard` are confusing.
    ...
    if (Test-Path $pythonExe) {
        ...
        try {
            & $pythonExe -m py_compile "$InstallDir\sarthi_cli\web_server.py" 2>&1 | Out-Null
            if ($LASTEXITCODE -eq 0) { $webServerSyntaxOk = $true }
        } catch { }
        $ErrorActionPreference = $prevEAP
        if (-not $webOk) {
            Write-Warn "fastapi/uvicorn not importable -- `sarthi dashboard` will not work."
            Write-Info "Attempting targeted install of [web] extra as last resort..."
            & $UvCmd pip install -e ".[web]"
            if ($LASTEXITCODE -eq 0) {
                Write-Success "[web] extra installed; `sarthi dashboard` should now work."
            } else {
        ...
        if (-not $webServerSyntaxOk) {
            throw "dashboard backend source failed syntax check: sarthi_cli/web_server.py"
        }
    }
...
function Install-SarthiCommandLaunchers {
    param(
        [Parameter(Mandatory=$true)] [string]$Root
    )

    # Expose ONLY the Sarthi launchers on PATH -- never the whole
    # venv\Scripts directory. Requiring sarthi.exe before creating bin keeps
    # the PATH stage from reporting success with an unusable command.
    $scriptsDir = Join-Path $Root "venv\Scripts"
    $requiredSource = Join-Path $scriptsDir "sarthi.exe"
    if (-not (Test-Path -LiteralPath $requiredSource -PathType Leaf)) {
        throw "Cannot set up the sarthi command: required launcher not found: $requiredSource"
    }

    $sarthiBin = Join-Path $Root "bin"
    New-Item -ItemType Directory -Force -Path $sarthiBin | Out-Null
    foreach ($launcher in @("sarthi.exe", "sarthi-acp.exe")) {
        $src = Join-Path $scriptsDir $launcher
        if (Test-Path -LiteralPath $src -PathType Leaf) {
            Copy-Item -Force -LiteralPath $src -Destination (Join-Path $sarthiBin $launcher)
        }
    }

    $requiredDestination = Join-Path $sarthiBin "sarthi.exe"
    if (-not (Test-Path -LiteralPath $requiredDestination -PathType Leaf)) {
        throw "Cannot set up the sarthi command: launcher was not installed: $requiredDestination"
    }
    return $sarthiBin
}

function Set-PathVariable {
    Write-Info "Setting up sarthi command..."
    
    if ($NoVenv) {
        $sarthiBin = "$InstallDir"
    } else {
        $sarthiBin = "$InstallDir\bin"
        Install-SarthiCommandLaunchers -Root $InstallDir | Out-Null
    }
    ...
    if ((-not $NoVenv) -and $currentPath -like "*$legacyBin*") {
        ...
        $currentPath = $cleaned
        Write-Info "Removed legacy venv\Scripts from user PATH (kept sarthi via $sarthiBin)"
    }
    
    if ($currentPath -notlike "*$sarthiBin*") {
        [Environment]::SetEnvironmentVariable(
            "Path",
            "$sarthiBin;$currentPath",
            "User"
        )
        Write-Success "Added to user PATH: $sarthiBin"
    } else {
        Write-Info "PATH already configured"
    }
    
    # Set SARTHI_HOME so the Python code finds config/data in the right place.
    # Only needed on Windows where we install to %LOCALAPPDATA%\sarthi instead
    # of the Unix default ~/.sarthi
    $currentSarthiHome = [Environment]::GetEnvironmentVariable("SARTHI_HOME", "User")
    if (-not $currentSarthiHome -or $currentSarthiHome -ne $SarthiHome) {
        [Environment]::SetEnvironmentVariable("SARTHI_HOME", $SarthiHome, "User")
        Write-Success "Set SARTHI_HOME=$SarthiHome"
    }
    $env:SARTHI_HOME = $SarthiHome
    
    # Update current session
    $env:Path = "$sarthiBin;$env:Path"
    
    Write-Success "sarthi command ready"
}

function Write-BootstrapMarker {
    # Writes $InstallDir\.sarthi-bootstrap-complete which tells the Sarthi
    ...
    # Sarthi-Setup.exe) or fall back to whatever git resolves in the
    # checkout. The desktop validates schemaVersion + pinnedCommit
    # length but doesn't enforce that HEAD matches the pin (users
    # update via `sarthi update` which moves HEAD legitimately).
    ...
    $markerPath = Join-Path $InstallDir ".sarthi-bootstrap-complete"
    $marker = [ordered]@{
        schemaVersion = 1
...
function Copy-ConfigTemplates {
    ...
    # Create the SARTHI_HOME directory structure ($SarthiHome, default %LOCALAPPDATA%\sarthi)
    New-Item -ItemType Directory -Force -Path "$SarthiHome\cron" | Out-Null
    New-Item -ItemType Directory -Force -Path "$SarthiHome\sessions" | Out-Null
    New-Item -ItemType Directory -Force -Path "$SarthiHome\logs" | Out-Null
    New-Item -ItemType Directory -Force -Path "$SarthiHome\pairing" | Out-Null
    New-Item -ItemType Directory -Force -Path "$SarthiHome\hooks" | Out-Null
    New-Item -ItemType Directory -Force -Path "$SarthiHome\image_cache" | Out-Null
    New-Item -ItemType Directory -Force -Path "$SarthiHome\audio_cache" | Out-Null
    New-Item -ItemType Directory -Force -Path "$SarthiHome\memories" | Out-Null
    New-Item -ItemType Directory -Force -Path "$SarthiHome\skills" | Out-Null

    
    # Create .env
    $envPath = "$SarthiHome\.env"
    ...
    # Create config.yaml
    $configPath = "$SarthiHome\config.yaml"
    ...
    # (the default PS5 behaviour), and Sarthi's prompt-injection scanner
    ...
    # PowerShell version.
    $soulPath = "$SarthiHome\SOUL.md"
    if (-not (Test-Path $soulPath)) {
        # MUST match DEFAULT_SOUL_MD in sarthi_cli/default_soul.py. The runtime
...
"@
    ...
    Write-Success "Configuration directory ready: $SarthiHome"
    
    # Seed bundled skills into $SarthiHome\skills (manifest-based, one-time per skill)
    Write-Info "Syncing bundled skills to $SarthiHome\skills ..."
    ...
    if (Test-Path $pythonExe) {
        try {
            ...
            Write-Success "Skills synced to $SarthiHome\skills"
        } catch {
            # Fallback: simple directory copy
            $bundledSkills = "$InstallDir\skills"
            $userSkills = "$SarthiHome\skills"
            if ((Test-Path $bundledSkills) -and -not (Get-ChildItem $userSkills -Exclude '.bundled_manifest' -ErrorAction SilentlyContinue)) {
                Copy-Item -Path "$bundledSkills\*" -Destination $userSkills -Recurse -Force -ErrorAction SilentlyContinue
                Write-Success "Skills copied to $SarthiHome\skills"
            }
        }
    }
}
...
function Install-NodeDeps {
    if (-not $HasNode) {
        # Cross-process driver mode (Sarthi-Setup.exe runs each -Stage NAME
    ...
    if (-not $npmCmd) {
        ...
        Write-Info "Open a new PowerShell window and re-run 'sarthi setup tools' later."
        return
    }
    $npmExe = $npmCmd.Source
    if ($npmExe -like "*.ps1") {
    ...
    if (Test-Path "$InstallDir\package.json") {
        ...
        $browserLog = "$env:TEMP\sarthi-npm-browser-$(Get-Random).log"
        ...
        if ($browserNpmOk) {
            ...
            if (-not $npxExe) {
                ...
                $pwLog = "$env:TEMP\sarthi-playwright-install-$(Get-Random).log"
                Push-Location $InstallDir
    ...
    if (Test-Path "$tuiDir\package.json") {
        ...
        $tuiLog = "$env:TEMP\sarthi-npm-tui-$(Get-Random).log"
...
# failure is non-fatal (browser_exec can still run via uvx, and `sarthi tools`
...
function Install-BrowserUseCli {
    ...
    $managedBin = Join-Path $SarthiHome "bin"
    $managedBu = Join-Path $managedBin "browser-use.exe"
    # MANAGED-FIRST: only Sarthi' managed copy short-circuits. A browser-use
    ...
    $ErrorActionPreference = "Continue"
    try {
        # UV_TOOL_BIN_DIR keeps the binary inside Sarthi' managed bin dir,
        ...
        if ($LASTEXITCODE -eq 0) {
            ...
            Write-Info "Install later with: uv tool install browser-use  (or via 'sarthi tools')"
        }
    } catch {
...
# Provision it at install time so enabling the tool later -- via `sarthi
...
# paths still lazy-install via install_cua_driver() (sarthi_cli/tools_config)
...
function Install-CuaDriver {
    ...
    $ErrorActionPreference = "Continue"
    try {
        # Same upstream installer `sarthi computer-use install` runs. Bounded
        ...
        # Sarthi' _CUA_INSTALLER_TIMEOUT (660s).
        ...
        if (Wait-Job $job -Timeout 660) {
            ...
            if ($installedCuaDriver -and (Test-CuaDriverRuntimeContract -DriverPath $installedCuaDriver.Source)) {
                Write-Success "Computer Use driver installed (enable via 'sarthi tools' -> Computer Use)"
            } else {
                Write-Warn "Computer Use driver install did not produce a compatible runtime -- repair it before enabling the tool."
                Write-Info "Install later with: sarthi computer-use install"
            }
        } else {
...
# the final `electron` -> `Sarthi` rename dies with ENOENT and every re-run
...
function Install-Desktop {
    # Build apps/desktop into a launchable Sarthi.exe. Only called from
    ...
    #
    # The Tauri bootstrap installer's launch_sarthi_desktop command
    # resolves apps/desktop/release/win-unpacked/Sarthi.exe directly,
    ...
    # enforces the build floor (Node >=26) and prepends the Sarthi-managed
    ...
    # The --dir mode produces an unpacked Sarthi.exe in
    ...
    # are NOT trying to work around). The Sarthi icon + product name are
    # stamped onto Sarthi.exe by our own rcedit step (Set-DesktopExeIdentity)
    ...
    $buildLog = "$env:TEMP\sarthi-desktop-build-$(Get-Random).log"
    ...
    $exeCandidates = @(
        "$desktopDir\release\win-unpacked\Sarthi.exe",
        "$desktopDir\release\win-arm64-unpacked\Sarthi.exe"
    )
    $found = $false
    $desktopExe = $null
    ...
    if (-not $found) {
        throw "Desktop build completed but no Sarthi.exe was found under $desktopDir\release\*-unpacked\"
    }

    # 3b. The Sarthi icon + identity are stamped onto Sarthi.exe by the
    ...
    #     SIDs under %LOCALAPPDATA% (electron/electron#51761, sarthi-agent#38216).
    ...
    #    Sarthi.exe. We deliberately do NOT point them at `sarthi desktop`: that
...
function New-DesktopShortcuts {
    ...
    try {
        ...
        $targets = @(
            (Join-Path ([Environment]::GetFolderPath('Programs')) 'Sarthi.lnk'),
            (Join-Path ([Environment]::GetFolderPath('Desktop')) 'Sarthi.lnk')
        )
        ...
        # with the Sarthi icon, but without this the shortcut can keep drawing
...
function Install-PlatformSdks {
    ...
    # ~/.sarthi/.env are importable.  Two problems this solves:
    #
...
function Invoke-SetupWizard {
    ...
    if ($NonInteractive) {
        ...
        Write-Info "Skipping setup wizard (non-interactive). Configure via the GUI or 'sarthi setup'."
        return
    }
    ...
    # Run sarthi setup using the venv Python directly (no activation needed)
    if (-not $NoVenv) {
        & ".\venv\Scripts\python.exe" -m sarthi_cli.main setup
    } else {
        python -m sarthi_cli.main setup
    }
...
function Start-GatewayIfConfigured {
    ...
    $sarthiCmd = "$InstallDir\venv\Scripts\sarthi.exe"
    if (-not (Test-Path $sarthiCmd)) {
        $sarthiCmd = "sarthi"
    }

    # If WhatsApp is enabled but not yet paired, run foreground for QR scan
    $whatsappEnabled = $content | Where-Object { $_ -match "^WHATSAPP_ENABLED=true" }
    $whatsappSession = "$SarthiHome\whatsapp\session\creds.json"
    if ($whatsappEnabled -and -not (Test-Path $whatsappSession)) {
        Write-Host ""
        Write-Info "WhatsApp is enabled but not yet paired."
        Write-Info "Running 'sarthi whatsapp' to pair via QR code..."
        Write-Host ""
    ...
    if ($NonInteractive) {
        ...
        Write-Info "Start the gateway later with: sarthi gateway"
        return
    }
    ...
    if ($response -eq "" -or $response -match "^[Yy]") {
        ...
        try {
            $logFile = "$SarthiHome\logs\gateway.log"
            Start-Process -FilePath $sarthiCmd -ArgumentList "gateway" `
                -RedirectStandardOutput $logFile `
                -RedirectStandardError "$SarthiHome\logs\gateway-error.log" `
                -WindowStyle Hidden
            Write-Success "Gateway started! Your bot is now online."
            Write-Info "Logs: $logFile"
            Write-Info "To stop: close the gateway process from Task Manager"
        } catch {
            Write-Warn "Failed to start gateway. Run manually: sarthi gateway"
        }
    } else {
        Write-Info "Skipped. Start the gateway later with: sarthi gateway"
    }
}
...
function Write-Completion {
    ...
    Write-Host "$SarthiHome\config.yaml"
    Write-Host "   API Keys:  " -NoNewline -ForegroundColor Yellow
    Write-Host "$SarthiHome\.env"
    Write-Host "   Data:      " -NoNewline -ForegroundColor Yellow
    Write-Host "$SarthiHome\cron\, sessions\, logs\"
    Write-Host "   Code:      " -NoNewline -ForegroundColor Yellow
    Write-Host "$SarthiHome\sarthi-agent\"
    Write-Host ""
    ...
    Write-Host ""
    Write-Host "   sarthi              " -NoNewline -ForegroundColor Green
    Write-Host "Start chatting"
    Write-Host "   sarthi setup        " -NoNewline -ForegroundColor Green
    Write-Host "Configure API keys & settings"
    Write-Host "   sarthi config       " -NoNewline -ForegroundColor Green
    Write-Host "View/edit configuration"
    Write-Host "   sarthi config edit  " -NoNewline -ForegroundColor Green
    Write-Host "Open config in editor"
    Write-Host "   sarthi gateway      " -NoNewline -ForegroundColor Green
    Write-Host "Start messaging gateway (Telegram, Discord, etc.)"
    Write-Host "   sarthi update       " -NoNewline -ForegroundColor Green
...
$InstallStages = @(
    ...
    @{ Name = "repository";       Title = "Cloning Sarthi repository";            Category = "install";      NeedsUserInput = $false; Worker = "Stage-Repository" }
...
if ($IncludeDesktop) {
    ...
    # (Sarthi-Setup.exe), never via the irm|iex CLI one-liner.
    $InstallStages += @{ Name = "desktop"; Title = "Building desktop app"; Category = "install"; NeedsUserInput = $false; Worker = "Stage-Desktop" }
}
$InstallStages += @(
    @{ Name = "path";             Title = "Adding Sarthi to PATH";                Category = "finalize";     NeedsUserInput = $false; Worker = "Stage-Path" }
