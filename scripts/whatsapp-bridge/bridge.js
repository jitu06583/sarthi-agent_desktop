 ...
 *
 * Usage:
 *   node bridge.js --port 3000 --session ~/.sarthi/whatsapp/session
 */
...
const SESSION_DIR = getArg('session', path.join(process.env.HOME || '~', '.sarthi', 'whatsapp', 'session'));
// Cache directories: the Python gateway passes the profile-aware paths via
// env (SARTHI_HOME-aware, new cache/ layout).  Fall back to the legacy
// hardcoded locations for bridges launched outside the gateway.
const IMAGE_CACHE_DIR = process.env.SARTHI_IMAGE_CACHE_DIR
  || path.join(process.env.HOME || '~', '.sarthi', 'image_cache');
const DOCUMENT_CACHE_DIR = process.env.SARTHI_DOCUMENT_CACHE_DIR
  || path.join(process.env.HOME || '~', '.sarthi', 'document_cache');
const AUDIO_CACHE_DIR = process.env.SARTHI_AUDIO_CACHE_DIR
  || path.join(process.env.HOME || '~', '.sarthi', 'audio_cache');
...
// `sarthi update` updates bridge.js on disk but a long-lived bridge process
...
const DEFAULT_REPLY_PREFIX = '⚕ *Sarthi Agent*\n────────────\n';
...
function enqueuePollUpdateEvent({ key, update, selectedOptions, aggregation }) {
  ...
  const pollId = key?.id
    ...
    || '';
  // Only surface votes on polls Sarthi itself created (tracked when
...
async function startSocket() {
  ...
  sock = makeWASocket({
    ...
    browser: ['Sarthi Agent', 'Chrome', '120.0'],
    ...
    for (const msg of messages) {
      ...
      if (msg.key.fromMe) {
        ...
        if (WHATSAPP_MODE === 'bot') {
          ...
          // the owner replied to would leak into Sarthi and trigger
      ...
      // Ignore Sarthi' own reply messages in self-chat mode to avoid loops.
    ...
    switch (type) {
      case 'image':
        if (ext === 'gif') {
          ...
          let tmpGifMp4 = null;
          try {
            tmpGifMp4 = path.join(tmpdir(), `sarthi_gif_${randomBytes(6).toString('hex')}.mp4`);
            execFileSync(
              'ffmpeg',
      ...
      case 'audio': {
        ...
        let tmpPath = null;
        if (needsConversion) {
          tmpPath = path.join(tmpdir(), `sarthi_voice_${randomBytes(6).toString('hex')}.ogg`);
          try {
            execFileSync(
              'ffmpeg',
