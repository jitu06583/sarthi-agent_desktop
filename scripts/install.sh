...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        -h|--help)
            ...
            echo "                   Installs optional deps + runs sarthi setup"
            echo "                   Does NOT clone repo or create venv"
            exit 0
            ;;
        *)
...
detect_os() {
    case "$(uname -s)" in
        ...
        CYGWIN*|MINGW*|MSYS*)
            ...
            log_info "  iex (irm https://sarthi-agent.vercel.app/install.ps1)"
            exit 1
            ;;
        *)
            OS="unknown"
...
# string clears that floor; anything below it is replaced with the Sarthi-
...
check_node() {
    ...
    if [ -x "$SARTHI_HOME/node/bin/node" ] && node_satisfies_build "$("$SARTHI_HOME/node/bin/node" --version)"; then
    ...
    if command -v node &> /dev/null; then
        log_warn "Node.js $(node --version) is too old for the desktop build (need ^20.19 or >=22.12) — installing Sarthi-managed Node $NODE_VERSION LTS..."
...
setup_path() {
    ...
    if [ "$USE_VENV" = true ]; then
        SARTHI_BIN="$INSTALL_DIR/venv/bin/sarthi"
    else
    ...
    if [ ! -x "$SARTHI_BIN" ]; then
        log_warn "sarthi entry point not found at $SARTHI_BIN"
...
install_node_deps() {
    ...
    if [ -f "$INSTALL_DIR/ui-tui/package.json" ]; then
        ...
        run_with_timeout "$NODE_DEPS_TIMEOUT" npm install --silent || {
            log_warn "TUI npm install failed or timed out (sarthi --tui may not work)"
        }
...
ensure_browser() {
    ...
    local ab_bin="$SARTHI_HOME/node/bin/agent-browser"
    if [ -x "$ab_bin" ]; then
...
postinstall_mode() {
    ...
    log_info "Post-install mode: setting up Sarthi for pip install"
    ...
    SARTHI_CMD="$(command -v sarthi 2>/dev/null || echo "")"
    if [ -n "$SARTHI_CMD" ]; then
        log_info "Running sarthi setup..."
        "$SARTHI_CMD" setup
    else
        log_warn "sarthi command not found on PATH"
        log_info "Try: python -m sarthi_cli.main setup"
    fi
}
...
install_desktop() {
    ...
    # floor (^20.19 || >=22.12) and prepends the Sarthi-managed Node to PATH, so
    ...
    # trips Gatekeeper's tamper check ("Sarthi is damaged and can't be opened").
