...
When Sarthi runs in TUI mode, the gateway child process communicates with
...
# ``get_sarthi_home() / "plugins"`` (user) and ``./.sarthi/plugins/`` (project,
# gated behind ``SARTHI_ENABLE_PROJECT_PLUGINS``) — see
# ``sarthi_cli/plugins.py:10-12``.  The guard only checked the bundled
...
# this script runs as a standalone subprocess — ``sarthi_constants`` isn't
...
def main() -> int:
    ...
    # Add repo root to sys.path so we can import sarthi_constants (this script
    ...
    # plugins in get_sarthi_home()/plugins/ and project plugins in
    # ./.sarthi/plugins/, where code like ori/hooks.py can spawn
    # subprocesses with inherited stdin — #67639).
    plugin_roots: list[Path] = [get_sarthi_home() / "plugins"]
    if os.environ.get("SARTHI_ENABLE_PROJECT_PLUGINS"):
        plugin_roots.append(Path.cwd() / ".sarthi" / "plugins")
