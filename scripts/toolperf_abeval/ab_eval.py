...
measured in the production session DB) through `sarthi chat` twice — once per
...
Environment:
  ...
  ABEVAL_HOME    SARTHI_HOME for runs   (default: $ABEVAL_ROOT/home)
                 Must be a configured Sarthi home with credentials for the
...
""".strip(),
            ...
            env.update({
                ...
                "SARTHI_NEMO_RELAY_PLUGINS_TOML": str(relay_config),
            })
            q = TASKS[name].replace("{WORK}", str(work))
            t0 = time.time()
            try:
                p = subprocess.run(
                    [sys.executable, "-m", "sarthi_cli.main", "chat", "--query", q,
