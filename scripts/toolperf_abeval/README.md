...
- **Two arms, one variable.** `baseline` and `fixes` runs differ ONLY by
  ...
  Sarthi home, same model, same tasks, same reps.
...
1. Create a dedicated Sarthi home with credentials for the models under test:
   ...
   integration at it; no Sarthi observability plugin needs to be enabled.
