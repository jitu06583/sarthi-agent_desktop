...
VERSION_FILE = REPO_ROOT / "sarthi_cli" / "__init__.py"
...
AUTHOR_MAP = {
    ...
    "kavi@local.sarthi": "kavioavio",  # Issue #46544 / PR #47705 evolution (smart DENY exact-operation owner override)
    ...
    "sunsky.lau@gmail.com": "liuhao1024",  # PR #56993 salvage (gateway: process-level SARTHI_HOME for pid/lock/status identity files; #56986)
    ...
    "wyuebei@gmail.com": "wyuebei-cloud",  # PR #56640 salvage (sarthi journey: replace GNU-only %-d strftime with dt.day for Windows)
    ...
    "sarthi.wanderer@yahoo.com": "trismegistus-wanderer",  # PR #31856 salvage (gateway: defer idle-TTL agent-cache eviction until the session store says the session actually expired, so the expiry watcher can still fire MemoryProvider.on_session_end with the live transcript; #11205)
    ...
    "me@keslerm.com": "keslerm",  # PR #3459 salvage (gateway: 'log' tool_progress mode — silent in chat, tool calls appended to ~/.sarthi/logs/tool_calls.log via rotating handler; duplicate of #3458 by @dlkakbs who submitted 4 min earlier — both credited)
    ...
    "121278003+Cossackx@users.noreply.github.com": "Cossackx",  # PR #52528 salvage (Windows sarthi-shim resolution + prefer --update on recovery; #52378)
    ...
    "sarthi@marian.local": "Schrotti77",
    ...
    "274096618+sarthi-agent-dhabibi@users.noreply.github.com": "dhabibi",
    ...
    "sarthi@nousresearch.com": "NousResearch",
    ...
    "sberan@gmail.com": "sberan",  # PR #54494 salvage (--connect-timeout flag on sarthi mcp add)
    ...
    "sarthi@example.com": "shellybotmoyer",  # PR #18915 (bot-committed)
    ...
    "github@nadyasarthi.anonaddy.com": "ruangraung",  # PR #42308
    ...
    "sylw3st3rr@gmail.com": "Sylw3ster",  # salvage of #23252 (SARTHI_KANBAN_BOARD restore)
    ...
    "58116817+TurgutKural@users.noreply.github.com": "TurgutKural",  # salvage of #23356 (SARTHI_HOME inject)
    ...
    "sauravsejal40@gmail.com": "Saurav0989",  # PR #27071 (docs: sarthi-eval community link)
    ...
    "sarthiagent26@gmail.com": "sarthiagent26",  # PR #26438 (kimi model-name reasoning pad)
    "276067471+sarthiagent26@users.noreply.github.com": "sarthiagent26",
    ...
    "vigo@sarthi": "hawknewton",  # PR #26294 (bedrock boto3 lazy_deps)
    ...
    "hex.sarthi@agentmail.to": "oseftg",
    ...
    "kdkcfp@gmail.com": "slowtokki0409",  # PR #29025 (ignore local Sarthi runtime files)
    ...
    "2663402852@qq.com": "x1am1",  # PR #35098 (chown root-owned top-level SARTHI_HOME state files)
    ...
    "zhchl@sarthi-agent.local": "8294",  # PR #50572 salvage (honor config context_length on banner)
...
def update_version_files(semver: str, calver_date: str):
    ...
    # Python package version. The desktop About panel reads the live Sarthi
...
def _update_acp_registry_versions(semver: str) -> None:
    ...
    if ACP_REGISTRY_MANIFEST.exists():
        ...
        if "package" in uvx:
            uvx["package"] = f"sarthi-agent[acp]=={semver}"
...
def parse_coauthors(body: str) -> list:
    ...
    _ignored_emails = {"noreply@anthropic.com", "noreply@github.com",
                       "cursoragent@cursor.com", "sarthi@nousresearch.com"}
...
def generate_changelog(commits, tag_name, semver, repo_url="https://github.com/jitendra-singh-thakur/sarthi-agent",
    ...
    lines.append(f"# Sarthi Agent v{semver} ({tag_name})")
    lines.append("")
    ...
    if first_release:
        ...
        lines.append("> for Sarthi Agent. See below for everything included in this initial release.")
        lines.append("")
...
def main():
    parser = argparse.ArgumentParser(description="Sarthi Agent Release Tool")
    ...
    print(f"{'='*60}")
    print("  Sarthi Agent Release Preview")
    print(f"{'='*60}")
    ...
    if args.publish:
        ...
        tag_result = git_result(
            "tag", "-a", tag_name, "-m",
            f"Sarthi Agent v{new_version} ({calver_date})\n\nWeekly release"
        )
        ...
        gh_cmd = [
            ...
            "--title", f"Sarthi Agent v{new_version} ({calver_date})",
        ...
        if result and result.returncode == 0:
            ...
            print(
                f"    gh release create {tag_name} --title 'Sarthi Agent v{new_version} ({calver_date})' "
