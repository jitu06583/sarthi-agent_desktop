...
function Invoke-Normalization {
    ...
    try {
        ...
        # its diagnostics are all `[sarthi] `-prefixed, so the two separate
        ...
        $stderr = ($raw | Where-Object { $_ -like '`[sarthi`]*' }) -join "`n"
        $stdout = ($raw | Where-Object { $_ -notlike '`[sarthi`]*' }) -join "`n"
    } finally {
    ...
    return @{
        ...
        SarthiHome = $(if ($paths) { $paths.sarthi_home } else { $null })
...
$expectedInstallDir = "$($longRoot)${sep}AppData${sep}Local" + '\sarthi\sarthi-agent'
...
$result = Invoke-Normalization -Environment @{ TEMP = $shortTemp; TMP = $shortTemp } `
    -ExtraArgs @('-InstallDir', (Join-Path $shortProfile 'custom-sarthi'))
Assert-Equal -Expected (Join-Path $longRoot 'custom-sarthi') -Actual $result.InstallDir -Label "explicit -InstallDir keeps the caller's directory, on the long root"
