"""Run a real Sarthi CLI turn and validate the Relay shared-metrics output."""
...
def _resolve_sarthi_executable(sarthi_repo: Path) -> Path:
    for relative_path in (
        Path(".venv") / "bin" / "sarthi",
        Path(".venv") / "Scripts" / "sarthi.exe",
    ):
        candidate = sarthi_repo / relative_path
        if candidate.is_file():
            return candidate
    discovered = shutil.which("sarthi")
    if discovered:
        return Path(discovered)
    raise SystemExit(
        "Sarthi executable not found in the repository virtual environment "
        "or on PATH"
    )
...
def _arguments() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=Path,
        default=Path.cwd(),
        help="Sarthi source checkout containing .venv/bin/sarthi",
    )
    parser.add_argument(
        "--relay-python",
        type=Path,
    ...
    parser.add_argument(
        ...
        type=Path,
        default=None,
        help="Directory for the isolated SARTHI_HOME and captured output",
    )
...
def _validate_store(database_path: Path) -> list[dict[str, Any]]:
    ...
    if by_name["sarthi.client.active"] != [
        {
        ...
        raise AssertionError(
            f"Unexpected client-active counter: {by_name['sarthi.client.active']}"
        )
    [model] = by_name["sarthi.model_route.count"]
    expected_model = {
        "name": "sarthi.model_route.count",
        "dimensions": {
            "model": MODEL_CANARY,
            "provider": "custom",
        },
    ...
    if model != expected_model:
        raise AssertionError(
            f"Unexpected model counter: {by_name['sarthi.model_route.count']}"
        )
    expected_start = {
    ...
    if by_name["sarthi.task_run.started"] != [expected_start]:
        raise AssertionError(
            f"Unexpected task start: {by_name['sarthi.task_run.started']}"
        )
    [terminal] = by_name["sarthi.task_run.finished"]
    ...
    [tool] = by_name["sarthi.tool_call.count"]
    ...
    lifecycle = by_name["sarthi.skill.lifecycle.count"]
    expected_actions = {
        "archived",
        "created",
        "edited",
    ...
    loads = by_name["sarthi.skill.load.count"]
...
def _validate_packages(
    ...
    try:
        ...
        raise RuntimeError(
            "The Sarthi development environment requires jsonschema"
        ) from exc
    ...
    if metrics["sarthi.client.active"] != [
        {
            "name": "sarthi.client.active",
            "type": "counter",
            "dimensions": {},
        ...
        raise AssertionError(
            f"Unexpected client-active metric: {metrics['sarthi.client.active']}"
        )
    [model] = metrics["sarthi.model_route.count"]
    if model["dimensions"] != {
        "model": MODEL_CANARY,
        "provider": "custom",
    } or model["value"] != 2:
        raise AssertionError(
            f"Unexpected model metric: {metrics['sarthi.model_route.count']}"
        )
    [terminal] = metrics["sarthi.task_run.finished"]
    ...
    [tool] = metrics["sarthi.tool_call.count"]
    if (
        tool["dimensions"]
        != {
            "approval_outcome": "not_required",
    ...
    lifecycle = metrics["sarthi.skill.lifecycle.count"]
    ...
    loads = metrics["sarthi.skill.load.count"]
    if {
        (
...
def main() -> int:
    args = _arguments()
    sarthi_repo = args.sarthi_repo.resolve()
    relay_python = args.relay_python.resolve() if args.relay_python else None
    sarthi = _resolve_sarthi_executable(sarthi_repo)
    ...
    if args.output_dir:
        ...
        root = Path(tempfile.mkdtemp(prefix="sarthi-relay-shared-metrics-"))
    home = root / "sarthi-home"
    ...
    (root / "sarthi.stdout.txt").write_text(result.stdout, encoding="utf-8")
    (root / "sarthi.stderr.txt").write_text(result.stderr, encoding="utf-8")
    if result.returncode != 0:
        raise AssertionError(
            f"Sarthi exited with {result.returncode}\n"
            f"stdout:\n{result.stdout}\nstderr:\n{result.stderr}"
        )
    ...
    if PROMPT_CANARY not in json.dumps(request.get("messages", [])):
        raise AssertionError("Sarthi model request did not contain the prompt canary")
    ...
    if TOOL_CALL_CANARY not in follow_up or TOOL_RESULT_CANARY not in follow_up:
        raise AssertionError("Sarthi did not return the tool result to the model")
    if RESPONSE_CANARY not in result.stdout:
        raise AssertionError("Sarthi did not print the mock model response")
    ...
    skill_result = subprocess.run(
        [
            ...
            "-c",
            "\n".join([
                "from sarthi_cli.observability import relay_shared_metrics",
    ...
    package_paths, packages = _validate_packages(
        ...
        / "observability"
        / "schemas"
        / "sarthi.shared_metrics.v2.schema.json",
    )

    print("Sarthi -> NeMo Relay shared-metrics smoke test passed")
