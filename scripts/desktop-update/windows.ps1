...
# (%SARTHI_HOME%\sarthi-setup.exe). That binary has no self-update path --
# copy_self_to_sarthi_home deliberately no-ops during --update -- so every
...
#
# This script lives in the repo checkout, so EVERY `sarthi update` refreshes
...
#     -InstallRoot <path>   repo checkout (SARTHI_HOME\sarthi-agent)
#     -Branch <ref>         branch to update against
#     -DesktopPid <pid>     the Electron main process to wait out
#     [-RelaunchExe <path>] Sarthi.exe to start when done (omit = no relaunch)
#     [-NoUi]               headless (tests); default shows a progress window
#     [-NoMarkerCleanup]    leave .sarthi-update-in-progress in place (tests)
#
...
# .sarthi-update-result.json for the relaunched Desktop to surface, and
# relaunches the Desktop so the user is never left stranded.
#
# Marker: we claim SARTHI_HOME\.sarthi-update-in-progress with OUR pid as
# step 0 (the wrapper cmd.exe pid the Desktop saw is useless -- it exits
# immediately), retaining SARTHI_UPDATE_STARTED_AT from the Desktop hand-off.
# sarthi_cli/update_lock.py's ancestry rule lets our
# `sarthi update` child adopt the claim; electron/update-marker.ts parks a
...
# pass our foreground right on to the new Sarthi.exe pid.
try {
    Add-Type -Namespace SarthiHandoff -Name Win32 -MemberDefinition @'
...
$SarthiHome = if ($InstallRoot) { Split-Path -Parent $InstallRoot } else { $TempDir }
$MarkerPath = Join-Path $SarthiHome ".sarthi-update-in-progress"
$LogDir = Join-Path $SarthiHome "logs"
$LogPath = Join-Path $LogDir "desktop-update-handoff.log"
$ResultPath = Join-Path $SarthiHome ".sarthi-update-result.json"
$script:Ui = $null
$script:UiStage = "Sarthi will open once done."   # until the first gate; matches ui.html
...
function Publish-UiProgress([string]$Message) {
    ...
    # stderr remain asynchronously drained in Invoke-SarthiStep and are never
...
function Show-ProgressWindow {
    ...
    if ($htmlPath -and $edge) {
        ...
        if ($server) {
            try {
                ...
                $edgeProfile = Join-Path $TempDir ("sarthi-update-ui-{0}" -f $PID)
                $edgeArgs = @(
    ...
    try {
        ...
        try {
            $form.Activate()
            if ($script:Win32) { [SarthiHandoff.Win32]::SetForegroundWindow($form.Handle) | Out-Null }
        } catch {}
        [System.Windows.Forms.Application]::DoEvents()
...
function Show-ErrorFinale([string]$Message) {
    ...
    # log tail -- `sarthi debug share` uploads the real evidence and the
    ...
    try {
        ...
        $ui.Sub.Text = "Run `"sarthi debug share`" in a terminal to send a report."
        ...
        try {
            $ui.Form.Activate()
            if ($script:Win32) { [SarthiHandoff.Win32]::SetForegroundWindow($ui.Form.Handle) | Out-Null }
        } catch {}
...
function Start-DesktopRelaunch {
    ...
    # update it can remove the old Sarthi.exe before writing the replacement,
    ...
    # DO NOT spawn Sarthi.exe as our child: Electron/Chromium calls
    ...
    try {
        ...
        if ($r -and $r.ReturnValue -eq 0) {
            ...
            try {
                if ($script:Win32) {
                    [SarthiHandoff.Win32]::AllowSetForegroundWindow([int]$r.ProcessId) | Out-Null
                    ...
                    while ((Get-Date) -lt $deadline) {
                        ...
                        if ($hwnd -ne [System.IntPtr]::Zero) {
                            [SarthiHandoff.Win32]::ShowWindow($hwnd, 9) | Out-Null  # SW_RESTORE
                            [SarthiHandoff.Win32]::SetForegroundWindow($hwnd) | Out-Null
    ...
    if (-not $spawned) {
        ...
        # verify by watching for a fresh Sarthi process.
        try {
            ...
            while ((Get-Date) -lt $explorerDeadline) {
                ...
                if ($fresh.Count -gt 0) {
                    ...
                    try {
                        if ($script:Win32) {
                            [SarthiHandoff.Win32]::AllowSetForegroundWindow([int]$fresh[0].Id) | Out-Null
...
# closes it. `sarthi update` deliberately runs its build steps with stdout
# inherited (sarthi_cli/main.py, the tee-stderr runner), so the tree under a
...
if ($env:SARTHI_UPDATE_PIPE_DRAIN_SECONDS) {
    $parsedGrace = 0
    if ([int]::TryParse($env:SARTHI_UPDATE_PIPE_DRAIN_SECONDS, [ref]$parsedGrace) -and $parsedGrace -ge 0) {
...
function Invoke-SarthiStep([string]$Exe, [string[]]$SarthiArgs, [string]$Tag) {
    ...
    # afterwards, where `sarthi debug share` picks it up.
    #
    ...
    # .sarthi-update-result.json, clearing .sarthi-update-in-progress,
    ...
    # Sarthi" until the user killed something by hand. Losing the tail of a
    ...
    $psi.Arguments = ($SarthiArgs | ForEach-Object { '"{0}"' -f ($_ -replace '"', '\"') }) -join ' '
    $psi.UseShellExecute = $false
    $psi.RedirectStandardOutput = $true
    $psi.RedirectStandardError = $true
    # sarthi update prints UTF-8 (checkmarks, arrows, box glyphs). PS 5.1
...
# the page can be QA'd in any browser; SARTHI_SELFTEST_FAIL=1 exercises the
# error state, SARTHI_SELFTEST_HOLD_SECONDS delays the terminal event.
if ($SelfTestUi) {
    ...
    $hold = 6
    if ($env:SARTHI_SELFTEST_HOLD_SECONDS) { $hold = [int]$env:SARTHI_SELFTEST_HOLD_SECONDS }
    Publish-UiProgress "Testing quiet update"
    Start-Sleep -Seconds $hold
    if ($env:SARTHI_SELFTEST_FAIL) {
...
# -SelfTestPipeDrain: prove Invoke-SarthiStep survives a leaked pipe ------
# The #90455 deadlock needs no update, no checkout and no Sarthi install to
...
if ($SelfTestPipeDrain) {
    ...
    if ($env:SARTHI_SELFTEST_HOLD_SECONDS) { $hold = [int]$env:SARTHI_SELFTEST_HOLD_SECONDS }
    $floodKb = 8192
    if ($env:SARTHI_SELFTEST_FLOOD_KB) { $floodKb = [int]$env:SARTHI_SELFTEST_FLOOD_KB }
    # $PSHOME is this interpreter's own directory -- no hardcoded system path.
    $powershell = Join-Path $PSHOME "powershell.exe"
    $stamp = [Guid]::NewGuid().ToString("N")
    $childPs1 = Join-Path $TempDir "sarthi-pipe-drain-$stamp.ps1"
    $floodPs1 = Join-Path $TempDir "sarthi-pipe-flood-$stamp.ps1"
    $pidFile = Join-Path $TempDir "sarthi-pipe-drain-$stamp.pid"
...
'@
    ...
    # merely loud. `sarthi update` is this shape -- the Electron/vite build
...
'@
    ...
    $res = Invoke-SarthiStep $powershell @(
    ...
    $flood = Invoke-SarthiStep $powershell @(
...
try {
    ...
    try {
        ...
        $startedAt = 0L
        $hasStartedAt = [int64]::TryParse($env:SARTHI_UPDATE_STARTED_AT, [ref]$startedAt)
    ...
    Publish-UiProgress "Waiting for Sarthi to close"
    if ($DesktopPid -gt 0) {
        ...
        if (Get-Process -Id $DesktopPid -ErrorAction SilentlyContinue) {
            ...
            $finalCode = 4
            $finalMsg = "Update aborted: the Sarthi window (pid $DesktopPid) did not exit within 30s. Nothing was changed. Close Sarthi fully and try again."
    ...
    Publish-UiProgress "Preparing Sarthi files"
    $shim = Join-Path $InstallRoot "venv\Scripts\sarthi.exe"
    if (Test-Path -LiteralPath $shim) {
        ...
        if (-not $unlocked) {
            ...
            $finalCode = 5
            $finalMsg = "Update aborted: another process is still holding the Sarthi install open (venv\Scripts\sarthi.exe locked after 20s). Nothing was changed. Close other Sarthi windows/terminals and try again."
            Write-HandoffLog $finalMsg
            exit $finalCode
        }
    ...
    # --force skips only the sarthi.exe shim guard, which step 2 just PROVED
    ...
    #
    # DRIVE THE UPDATE THROUGH venv\Scripts\python.exe, NOT venv\Scripts\sarthi.exe.
    # `uv pip install -e .` has to replace the console-script shims, so
    # _quarantine_running_sarthi_exe must first rename the running sarthi.exe
    # out of the way. On Windows that rename fails whenever ANY child process
    # spawned from that sarthi.exe is still alive: a child inherits a handle on
    # the parent image, and the resulting sharing violation is indistinguishable
    # from a user leaving a second Sarthi window open. It is the inherited
    ...
    # install whose Start Menu shortcut points at a Sarthi.exe that no longer
    ...
    #
    # Running the same code as `python.exe -m sarthi_cli.main update` puts the
    ...
    if (-not (Test-Path -LiteralPath $pythonExe)) {
        $finalCode = 3
        $finalMsg = "Update aborted: $pythonExe is missing. The install needs repair (run the Sarthi installer or `sarthi doctor`)."
        Write-HandoffLog $finalMsg
        exit $finalCode
    }
    $updateArgs = @("-m", "sarthi_cli.main", "update", "--yes", "--gateway", "--force", "--branch", $Branch)
    ...
    # collides with the "close all Sarthi windows" sentinel.
    try {
        $updateHelp = & $pythonExe -m sarthi_cli.main update --help 2>$null | Out-String
        if ($updateHelp -match "--keep-stash") {
            $updateArgs += "--keep-stash"
        } else {
            Write-HandoffLog "installed sarthi predates --keep-stash; running without it"
        }
    } catch {
    ...
    $res = Invoke-SarthiStep $pythonExe $updateArgs "update"
    Write-HandoffLog "sarthi update exit code: $($res.Code)"

    if ($res.Code -ne 0 -and $res.Code -ne 2) {
        # One retry for the update-boundary class (fresh code on disk, stale
        # code in memory). Exit 2 ("close all Sarthi windows") is not retryable.
    ...
    # `sarthi update` treats a Desktop GUI build failure as NON-fatal (prints
    ...
    if ($res.Code -eq 0 -and $res.Output -match "Desktop build failed") {
        Write-HandoffLog "sarthi update reported a desktop build failure (non-fatal there, fatal here); retrying build"
        Publish-UiProgress "Rebuilding Desktop"
        $rebuild = Invoke-SarthiStep $pythonExe @("-m", "sarthi_cli.main", "desktop", "--force-build", "--build-only") "rebuild"
    ...
    if ($res.Code -eq 0 -and -not $desktopBuildFailed) {
        ...
        $finalCode = 6
        $finalMsg = "Code and dependencies updated, but the Desktop app REBUILD FAILED - you are running the previous build. Run `sarthi desktop --force-build` from a terminal to retry."
    } else {
        $finalCode = $res.Code
        $finalMsg = "Update failed (exit $($res.Code)). Run `sarthi debug share` in a terminal to send a report."
    }
    exit $finalCode
} finally {
    ...
    #   3. only then the terminal UI state — done means "Sarthi is back",
    ...
    if ($finalCode -ne 0) {
        ...
        Publish-UiProgress "Opening Sarthi"
        ...
        if (-not $cameBack -and $RelaunchExe) {
            ...
            $finalMsg = "Update complete. Reopen Sarthi to finish (it could not restart itself)."
