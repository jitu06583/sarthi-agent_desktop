...
# tree generates llms.txt — the index every LLM (Sarthi included, via the
# sarthi-agent skill) reads to learn what Sarthi can do — and
...
_MCP_CATALOG_FILES = {"sarthi_cli/mcp_catalog.py"}
...
# Rust crates — currently just the Tauri bootstrap installer (Sarthi-Setup).
...
def _py_test_only(p: str) -> bool:
    ...
    Product jobs (Desktop E2E's ``sarthi serve`` backend, the Docker image)
