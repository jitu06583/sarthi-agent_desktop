#!/usr/bin/env python3
"""Live test harness for Sarthi Agent's Tool Search feature.
...
# Force-isolate the test environment BEFORE any sarthi imports.
ORIGINAL_HOME = os.environ.get("SARTHI_HOME")
ORIGINAL_AUTH = Path.home() / ".sarthi" / "auth.json"
...
def setup_isolated_home(enabled: bool, listing: str = "off",
    ...
    """Create a fresh ~/.sarthi/ for one test, copying minimal credentials.

    Also reads OPENROUTER_API_KEY from the user's real ``~/.sarthi/.env`` so
    the agent can authenticate against OpenRouter inside the isolated home.
    """
    home_dir = Path(tempfile.mkdtemp(prefix="sarthi_ts_live_"))
    sarthi_home = home_dir / ".sarthi"
    sarthi_home.mkdir(parents=True)

    if ORIGINAL_AUTH.exists():
        shutil.copy(ORIGINAL_AUTH, sarthi_home / "auth.json")

    # Copy .env so OPENROUTER_API_KEY (or others) are visible to the agent
    # running inside the isolated home.
    real_env_file = Path.home() / ".sarthi" / ".env"
    if real_env_file.exists():
        shutil.copy(real_env_file, sarthi_home / ".env")
        ...
        load_sarthi_dotenv(sarthi_home=str(Path.home() / ".sarthi"))
    ...
    (sarthi_home / "config.yaml").write_text(_yaml_dump(cfg), encoding="utf-8")
    return sarthi_home
...
def reset_module_state():
    """Drop cached modules so the new SARTHI_HOME takes effect."""
    keys = [k for k in sys.modules.keys()
            if k.startswith(("tools.", "model_tools", "toolsets",
                             "sarthi_cli", "agent.", "run_agent"))]
    for k in keys:
        del sys.modules[k]
...
def run_one_scenario(scenario: Dict[str, Any], enabled: bool, out_dir: Path) -> Dict[str, Any]:
    ...
    os.environ["SARTHI_HOME"] = str(home)
...
def main():
    ...
    # Restore original SARTHI_HOME
    if ORIGINAL_HOME is not None:
        os.environ["SARTHI_HOME"] = ORIGINAL_HOME
    else:
