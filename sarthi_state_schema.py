...
``sarthi_state.SessionDB``. It defines no ``__init__`` and no state of its
...
``SessionDB.__init__``. It must never import sarthi_state (cycle) — shared
module-level constants live in sarthi_state_common.
"""
...
from sarthi_state_common import (
    DEFERRED_INDEX_SQL,
    FTS_CJK_STALE_KEY,
    FTS_STALE_KEY,
    FTS_SQL,
...
# Moved methods logged under the "sarthi_state" logger before the split;
# keep that logger identity so log filtering/capture behavior is unchanged.
logger = logging.getLogger("sarthi_state")
...
def schema_read_probe_statements() -> tuple:
    ...
    ``sarthi_cli/web_server.py``) run these probes right after a read-only
    ...
    served an empty session list after `sarthi update` until the user's
...
class SessionSchemaMixin:
    ...
    def _parse_schema_columns(schema_sql: str) -> Dict[str, Dict[str, str]]:
        ...
        try:
            from sarthi_constants import get_sarthi_home
            cache_path = get_sarthi_home() / "cache" / "schema_columns.json"
    ...
    def _init_schema(self):
        ...
        if row is None:
            ...
            if current_version < 23:
                ...
                # optimization is available. `sarthi sessions optimize-storage`
        ...
        if fts5_available:
            ...
            # not yet opted into `sarthi db optimize`) must keep its EXISTING
