"""Windows UTF-8 bootstrap for Sarthi entry points.
...
should be imported at the very top of every Sarthi entry point
(``sarthi``, ``sarthi-agent``, ``sarthi-acp``, ``python -m gateway.run``,
...
def suppress_platform_ver_console() -> None:
    ...
    Mirrors ``sarthi_cli._subprocess_compat.suppress_platform_ver_console``
    ...
    ``sarthi_bootstrap``, never ``sarthi_cli.main``.
    """
    if not _IS_WINDOWS:
        return
    try:
...
def harden_import_path(src_root: str | None = None) -> None:
    """Stop a package in the current directory from shadowing Sarthi modules.

    Sarthi ships top-level modules with common names (``utils``, ``proxy``,
    ...
    We drop the relative forms outright, then force the real Sarthi source root
    ...
    """
    root = src_root or os.environ.get("SARTHI_PYTHON_SRC_ROOT") or os.path.dirname(
...
def activate_durable_lazy_target() -> None:
    ...
    (``SARTHI_LAZY_INSTALL_TARGET``, e.g. ``/opt/data/lazy-packages``).
    ...
    """
    if not os.environ.get("SARTHI_LAZY_INSTALL_TARGET", "").strip():
        return
    try:
...
# Apply on import — entry points just need ``import sarthi_bootstrap``
# (or ``from sarthi_bootstrap import apply_windows_utf8_bootstrap``) at
