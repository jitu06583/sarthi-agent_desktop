...
let
    ...
    (callPackage pyproject-nix.build.packages {
      ...
      (
        lib.composeManyExtensions [
          ...
          # Sarthi derivation. This is deliberately a derivation environment
          # variable, not a devShell variable: ``nix develop -c uv build``
          # must remain blocked.
          (final: prev: {
            sarthi-agent = prev.sarthi-agent.overrideAttrs (_old: {
              SARTHI_NIX_BUILD = "1";
            });
          })
        ]
  ...
  # $SARTHI_PYTHON_SRC_ROOT at runtime.
  workspaceRoot = ./..;
  ...
  editableOverlay = editableWorkspace.mkEditablePyprojectOverlay {
    root = "$SARTHI_PYTHON_SRC_ROOT"; # resolved at shellHook time
  };
  ...
  editableSet = pythonSet.overrideScope (
    lib.composeManyExtensions [
      editableOverlay
      (final: prev: {
        sarthi-agent = prev.sarthi-agent.overrideAttrs (old: {
...
in
{
  venv = pythonSet.mkVirtualEnv "sarthi-agent-env" {
    sarthi-agent = dependency-groups;
  };
  editableVenv = editableSet.mkVirtualEnv "sarthi-agent-editable-env" {
    sarthi-agent = dependency-groups;
  };
}
