# nix/packages.nix — Sarthi Agent package built with uv2nix
{ inputs, ... }:
{
  perSystem =
    {
    ...
    let
      minimal = pkgs.callPackage ./sarthi-agent.nix {
    ...
    in
    {
      packages = {
        ...
        tui = full.sarthiTui;
        web = full.sarthiWeb;
        desktop = full.sarthiDesktop;

        update-npm-lockfile = full.sarthiNpmLib.updateNpmLockfile;
      };
    };
}
