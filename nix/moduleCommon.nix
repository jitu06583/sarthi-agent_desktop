...
#
# `services.sarthi-agent` is the same option set on both modules. Both modules
...
#   homeManagerModules.nix  sarthiHome, programs.sarthi-agent (the CLI and
...
let
  ...
  deepConfigType = types.mkOptionType {
    name = "sarthi-config-attrs";
    description = "Sarthi YAML config (attrset), merged deeply via lib.recursiveUpdate.";
    ...
    options = {
      ...
      auth = mkOption {
        ...
        description = ''
          ...
          (remote MCP servers). Tokens are stored in $SARTHI_HOME/mcp-tokens/.
        '';
      };
  ...
  sharedOptions =
    ...
    {
      enable = lib.mkEnableOption "Sarthi Agent";
      ...
      package = mkOption {
        ...
        description = "The sarthi-agent package to use.";
      };
      ...
      settings = mkOption {
        ...
        default = { };
        description = ''
          The Sarthi configuration, as an attribute set. The module joins the
          ...
          which includes the keys that `sarthi config set` and the settings
      ...
      environmentFiles = mkOption {
        ...
        description = ''
          ...
          $SARTHI_HOME/.env. Sarthi reads that file at each start, with
          load_sarthi_dotenv().

          Each activation writes .env again from the start. Thus a secret
          file cannot go into .env two times.
        '';
        example = literalExpression ''[ config.sops.secrets."sarthi/env".path ]'';
      };
      ...
      authFile = mkOption {
        ...
        description = ''
          ...
          does not exist. Thus a token that Sarthi refreshes at runtime stays
          after an activation.
        '';
      };
      ...
      documents = mkOption {
        ...
        description = ''
          ...
          Sarthi reads SOUL.md and memories/ from SARTHI_HOME, so put those
          files in `sarthiHomeFiles`.
      ...
      sarthiHomeFiles = mkOption {
        type = documentsType;
        default = { };
        description = ''
          Files that the module installs into SARTHI_HOME. Each key is a path
          relative to that directory, and the module makes the necessary
          subdirectories. Each value is a string or a path.

          Sarthi reads SOUL.md and the memory files from SARTHI_HOME and not
          from the working directory. Declare those files here, or Sarthi
          does not load them.
        '';
        example = literalExpression ''
          {
      ...
      extraPlugins = mkOption {
        ...
        default = [ ];
        description = ''
          Directory-based plugin packages to symlink into the sarthi plugins
          directory. Each package must contain a plugin.yaml and __init__.py
          at its root. Sarthi discovers these automatically on startup.
        '';
        example = literalExpression ''
          [
      ...
      extraPythonPackages = mkOption {
        ...
        description = ''
          ...
          sarthi_agent.plugins entry-point group. Each package must be built
          with the same Python interpreter as sarthi (python312).
        '';
        example = literalExpression ''
          [
      ...
      extraDependencyGroups = mkOption {
        ...
        description = ''
          ...
          Use this for optional extras already declared in sarthi-agent's
      ...
      extraArgs = mkOption {
        type = types.listOf types.str;
        default = [ ];
        description = "Extra command-line arguments for `sarthi gateway`.";
      };
      ...
      # `sarthi serve` and `sarthi dashboard` are the same entry point,
      # sarthi_cli.main:cmd_dashboard, with one flag of difference. serve runs
      # without a user interface. dashboard also serves the web application.
      # Both give the /api/ws and /api/pty sockets that Sarthi Desktop
      ...
      # controls an external gateway, with `sarthi gateway restart`. It does
      ...
      backend = {
        mode = mkOption {
          ...
          description = ''
            ...
            - "serve"     — the backend without a user interface. It gives
                            the /api/ws and /api/pty sockets that Sarthi
                            Desktop connects to.
        ...
        sessionTokenFile = mkOption {
          ...
          description = ''
            ...
            SARTHI_DASHBOARD_SESSION_TOKEN. That token authorizes the /api
            routes and the /api/ws socket. Sarthi Desktop presents the same
          ...
          '';
          example = literalExpression ''config.sops.secrets."sarthi/desktop-token".path'';
        };
      };
    };
  ...
  # removal leaves them with no `sarthi` on the PATH and no message. The
  ...
  installPackageRemovedMessage =
    value:
    ''
      services.sarthi-agent.installPackage was removed. Sarthi now
      separates the installation from the services, which is the
      Home Manager convention:

        programs.sarthi-agent.enable = ${lib.boolToString (value != false)};  # the sarthi CLI, and SARTHI_HOME for your shells
        programs.sarthi-agent.desktop.enable = true;  # the desktop application

      `services.sarthi-agent` keeps the state, the configuration and
  ...
  mkConfigFiles =
    ...
    let
      generated = pkgs.writeText "sarthi-config.yaml" (
  ...
  mkDocumentTree =
    { pkgs, documents }:
    pkgs.runCommand "sarthi-documents" { } (
      ''
        mkdir -p $out
      ''
  ...
  mkEnvScript =
    { pkgs, environment }:
    let
      base = pkgs.writeText "sarthi-env-base" (
        lib.concatStringsSep "\n" (lib.mapAttrsToList (k: v: "${k}=${v}") environment)
        + lib.optionalString (environment != { }) "\n"
      );
    in
    pkgs.writeShellScript "sarthi-env-merge" ''
      set -eu
      ...
      for file in "$@"; do
        if [ -r "$file" ]; then
          ...
          echo "sarthi-agent: WARNING cannot read environmentFile $file" >&2
        fi
      done
    '';
  ...
  mkStateScript =
    {
      ...
      # SARTHI_MANAGED variable of the service. The value tells the shell
    ...
    in
    ''
      # Directories. The service units and Sarthi make most of these
      ...
      ${run}mkdir -p ${
        lib.escapeShellArgs (
          ...
          ++ map (d: "${sarthiHome}/${d}") stateDirs
        )
      }

      # config.yaml: merge the Nix settings into the file on disk. Sarthi
      ...
      ${
        if cfg.configFile != null then
          "${inst} -m ${modes.config} -D ${configFiles.effective} ${sarthiHome}/config.yaml"
        else
          ''
            ${run}${configFiles.mergeScript} ${configFiles.generated} ${sarthiHome}/config.yaml
            ${run}chmod ${modes.config} ${sarthiHome}/config.yaml
          ''
      }

      # The managed-mode marker. It makes an interactive shell also refuse to
      # change the configuration that Nix owns.
      ${inst} -m ${modes.managed} ${pkgs.writeText "sarthi-managed" managedSystem} ${sarthiHome}/.managed

      ${lib.optionalString (cfg.environment != { } || cfg.environmentFiles != [ ]) ''
        ${run}${envScript} ${sarthiHome}/.env ${modes.env} ${lib.escapeShellArgs cfg.environmentFiles}
        ${lib.optionalString (owner != null) "${run}chown ${owner} ${sarthiHome}/.env"}
      ''}

      ${lib.optionalString (cfg.authFile != null) (
        if cfg.authFileForceOverwrite then
          "${inst} -m ${modes.auth} ${cfg.authFile} ${sarthiHome}/auth.json"
        else
          ''
            if [ ! -e ${sarthiHome}/auth.json ]; then
              ${inst} -m ${modes.auth} ${cfg.authFile} ${sarthiHome}/auth.json
            fi
          ''
      )}
      ...
      ${installDocuments homeDocumentTree sarthiHome cfg.sarthiHomeFiles}
      ...
      ${run}find ${sarthiHome}/plugins -maxdepth 1 -type l -name 'nix-managed-*' -delete 2>/dev/null || true
      ${lib.concatMapStringsSep "\n" (plugin: ''
        if [ ! -f ${plugin}/plugin.yaml ]; then
          echo "sarthi-agent: ERROR extraPlugins entry '${plugin}' has no plugin.yaml" >&2
          exit 1
        fi
        ${run}ln -sfn ${plugin} ${sarthiHome}/plugins/nix-managed-${lib.getName plugin}
      '') cfg.extraPlugins}
    '';
  ...
  gatewayArgv =
    cfg:
    [
      "${effectivePackage cfg}/bin/sarthi"
      "gateway"
    ]
    ++ cfg.extraArgs;
  ...
  #
  # `exec` on the last line keeps sarthi as the MainPID of the unit. No shell
  ...
  backendLauncher =
    ...
    pkgs.writeShellScript "sarthi-backend-launch" (
      builtins.replaceStrings [ "@HOST@" ] [ ''"$_target"'' ] ''
        ...
        ${lib.optionalString (cfg.backend.sessionTokenFile != null) ''
          ...
          if [ ! -r "$_token_file" ]; then
            echo "sarthi-backend: cannot read the session token file '$_token_file'. The unit stops." >&2
            echo "sarthi-backend: backend.sessionTokenFile must name a runtime path that this user can read." >&2
            exit 1
          fi

          SARTHI_DASHBOARD_SESSION_TOKEN="$(${pkgs.coreutils}/bin/tr -d '\r\n' < "$_token_file")"
          export SARTHI_DASHBOARD_SESSION_TOKEN

          if [ -z "$SARTHI_DASHBOARD_SESSION_TOKEN" ]; then
            echo "sarthi-backend: the session token file '$_token_file' is empty. The unit stops." >&2
            exit 1
          fi
        ''}
        ${
          if cfg.backend.waitFor == null then
            ...
            ''
              ...
              while :; do
                ...
                if [ "$_waited" -ge "$_timeout" ]; then
                  echo "sarthi-backend: '$_target' did not resolve after ''${_timeout}s. The unit stops." >&2
                  exit 1
                fi

                if [ "$_waited" = 0 ]; then
                  echo "sarthi-backend: waits for '$_target' to resolve..." >&2
                fi
            ...
            ''
              ...
              while :; do
                ...
                if [ "$_waited" -ge "$_timeout" ]; then
                  echo "sarthi-backend: interface '$_iface' had no IPv4 address after ''${_timeout}s. The unit stops." >&2
                  echo "sarthi-backend: a fallback address can expose the backend more widely than you intend." >&2
                  exit 1
                fi

                if [ "$_waited" = 0 ]; then
                  echo "sarthi-backend: waits for an IPv4 address on '$_iface'..." >&2
                fi
                ${pkgs.coreutils}/bin/sleep 2
                _waited=$(( _waited + 2 ))
              done
        ...
        echo "sarthi-backend: binds to $_target:${toString cfg.backend.port} (from $_how)" >&2
  ...
  backendDescription =
    ...
    if cfg.backend.mode == "dashboard" then
      "Sarthi Agent web dashboard and desktop backend"
    else
      "Sarthi Agent backend for Sarthi Desktop";

  # The environment that each Sarthi process needs, from either module.
  #
  # managedSystem gives the value of SARTHI_MANAGED. The CLI reads that
    ...
    [
      {
        ...
        message = ''
          ...
          To give Sarthi an identity and a memory, use
          ${optionPath}.sarthiHomeFiles instead. Those files go to
          SARTHI_HOME. Sarthi reads SOUL.md and memories/ only from there.
        '';
      }
    ];
  ...
  # The subdirectories of SARTHI_HOME that both modules make.
  stateSubdirs = [
    "cron"
    "sessions"
    "logs"
