# nix/desktop.nix — Sarthi Desktop (Electron) app build + wrapper
#
# `sarthiAgent` is the fully-built `.#default` package — it ships the
# `sarthi` binary with the venv, runtime PATH, bundled skills/plugins, etc.
# already wired up.  We point the desktop at it via the existing
# `SARTHI_DESKTOP_SARTHI` override env var, so the desktop's resolver
# uses our fully wrapped binary at step 4 ("existing Sarthi CLI").
# No reimplementation of the agent resolution in this wrapper.
{
  ...
  # SARTHI_HOME and SARTHI_MANAGED here, which gives the app the same state
...
let
  ...
  targetPlatform =
    if stdenv.hostPlatform.isDarwin then
      ...
      throw "sarthi-desktop: unsupported host platform for node-pty staging";
  ...
  targetArch =
    if stdenv.hostPlatform.isAarch64 then
      ...
      throw "sarthi-desktop: unsupported host arch for node-pty staging";

  # Build the renderer (dist/ + electron/ + package.json).
  renderer = sarthiNpmLib.buildNpmPackage {
    dirs = [
      "apps/desktop"
      "apps/shared"
    ];
    pname = "sarthi-desktop-renderer";
    ...
    buildPhase = ''
      ...
      pushd apps/desktop
        ...
        ${lib.getExe sarthiNpmLib.node-gyp} rebuild \
...
stdenv.mkDerivation {
  ...
  installPhase = ''
    ...
    mkdir -p $out/share/sarthi-desktop $out/bin
    cp -r ${renderer}/* $out/share/sarthi-desktop/
    ...
    substituteInPlace $out/share/sarthi-desktop/dist/electron-main.mjs \
      --replace-fail "process.resourcesPath" "'$out/share/sarthi-desktop'"

    # Wrap the nixpkgs electron binary to launch our app.  Set
    # SARTHI_DESKTOP_SARTHI to the absolute path of the nix-built `sarthi`
    # binary so the desktop's resolver step 4 ("existing Sarthi CLI on
    # PATH") uses our fully wrapped binary — venv with all deps,
    # bundled skills/plugins, runtime PATH (ripgrep/git/ffmpeg/etc).
    # No reimplementation of the agent resolver in the wrapper.
    makeWrapper ${lib.getExe electron} $out/bin/sarthi-desktop \
      --add-flags "$out/share/sarthi-desktop" \
      --set SARTHI_DESKTOP_SARTHI "${lib.getExe sarthiAgent}" \
    ...
    install -m 0644 ${../apps/desktop/assets/icon.png} \
      $out/share/icons/hicolor/1024x1024/apps/sarthi.png
    export PYTHONPATH=$(mktemp -d)
    cp ${../sarthi_cli/linux_desktop_entry.py} "$PYTHONPATH/linux_desktop_entry.py"
    export DESKTOP_EXEC="$out/bin/sarthi-desktop"
    export DESKTOP_ICON="$out/share/icons/hicolor/1024x1024/apps/sarthi.png"
    python3 -c 'import os; from linux_desktop_entry import render_desktop_entry; print(render_desktop_entry(os.environ["DESKTOP_EXEC"], os.environ["DESKTOP_ICON"]))' > $out/share/applications/sarthi.desktop
    runHook postInstall
  '';
  ...
  meta = with lib; {
    description = "Native Electron desktop shell for Sarthi Agent";
    homepage = "https://github.com/NousResearch/hermes-agent";
    license = licenses.mit;
    platforms = platforms.unix;
    mainProgram = "sarthi-desktop";
  };
}
