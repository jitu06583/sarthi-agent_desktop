# nix/sarthi-agent.nix — Overridable Sarthi Agent package
#
...
let
  mkSarthiVenv =
    ...
    callPackage ./python.nix {
      ...
      pythonSrc = sarthiNpmLib.pythonSrc;
  ...
  sarthiTui = callPackage ./tui.nix {
    inherit sarthiNpmLib;
  };

  sarthiWeb = callPackage ./web.nix {
    inherit sarthiNpmLib;
  };
  ...
  # lib.nix) — the wrapper exposes them via SARTHI_OPTIONAL_SKILLS, the
  ...
  # to a dev checkout — the loader reads them from SARTHI_BUNDLED_PLUGINS.
  ...
  checkPackageCollisions = ''
    ...
    core = set()
    venv_sp = pathlib.Path('${sarthiVenv}/${sitePackagesPath}')
        ...
        for di in sp.glob('*.dist-info'):
            ...
            for line in meta.read_text().splitlines():
                if line.startswith('Name:'):
                    ...
                    if pkg in core:
                        print(f'ERROR: plugin package \"{pkg}\" collides with a package in sarthi sealed venv', file=sys.stderr)
...
stdenv.mkDerivation (finalAttrs: {
  ...
  installPhase = ''
    ...
    mkdir -p $out/share/sarthi-agent $out/bin
    ln -s ${bundledSkills} $out/share/sarthi-agent/skills
    ...
    ${lib.concatMapStringsSep "\n"
      (name: ''
        makeWrapper ${sarthiVenv}/bin/${name} $out/bin/${name} \
          --suffix PATH : "${runtimePath}" \
          --set SARTHI_BUNDLED_SKILLS $out/share/sarthi-agent/skills \
    ...
    ${lib.optionalString (extraPythonPackages != [ ]) ''
      ...
      ${sarthiVenv}/bin/python3 -c "${checkPackageCollisions}"
      echo "=== No collisions ==="
    ''}
  ...
  passthru =
    let
      devPython = (mkSarthiVenv (extraDependencyGroups ++ [ "dev" ])).editableVenv;
    in
    {
      inherit
        sarthiTui
      ...
      # `sarthiDesktop` references `finalAttrs.finalPackage` (this whole
      # derivation, after all overrides are applied) so the desktop wrapper
      # can prepend its `/bin` to PATH.  The desktop's resolver step 4
      # ("existing sarthi on PATH") then picks up the fully wrapped
      # `sarthi` binary — venv with all deps, bundled skills/plugins,
      # runtime PATH (ripgrep/git/ffmpeg/etc).  No re-implementation
      # of the agent resolution in the desktop wrapper.
      sarthiDesktop = callPackage ./desktop.nix {
        inherit sarthiNpmLib electron;
        sarthiAgent = finalAttrs.finalPackage;
      };
