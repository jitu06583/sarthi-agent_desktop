    ...
    in {
      ...
      checks = {
        ...
        bundled-locales = pkgs.runCommand "sarthi-bundled-locales" { } ''
          ...
          echo "=== Rendering via the wrapper override (SARTHI_BUNDLED_LOCALES) ==="
          export HOME=$(mktemp -d)
          ...
          BARE_DIR=$(cd "$HOME" && ${sarthiVenv}/bin/python3 -c "from agent import i18n; print(i18n._locales_dir())")
          BARE=$(cd "$HOME" && ${sarthiVenv}/bin/python3 -c "from agent import i18n; print(i18n.t('gateway.reset.header_default', lang='en'))")
          echo "resolved dir (no env var): $BARE_DIR"
          echo "rendered: $BARE"
          test "$BARE" != "gateway.reset.header_default" || \
            (echo "FAIL: sealed venv could not resolve locales without SARTHI_BUNDLED_LOCALES — data-files materialization regressed"; exit 1)
        ...
        # Verify SARTHI_NODE is set in wrapper and points to Node 20+
