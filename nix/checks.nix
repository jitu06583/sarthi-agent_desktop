...
{ inputs, ... }: {
  perSystem = { pkgs, lib, self', ... }:
    let
      sarthi-agent = self'.packages.default;
      sarthiVenv = sarthi-agent.sarthiVenv;
      ...
      evalHomeModule =
        ...
        inputs.home-manager.lib.homeManagerConfiguration {
          ...
          modules = [
            ...
            {
              home = {
                username = "sarthi-check";
                homeDirectory = "/home/sarthi-check";
                stateVersion = "24.11";
              };
            }
            { services.sarthi-agent = settings; }
      ...
      evalHomeSplit =
        ...
        inputs.home-manager.lib.homeManagerConfiguration {
          ...
          modules = [
            ...
            {
              programs.sarthi-agent = programs;
              services.sarthi-agent = services;
            }
          ];
        };

      # The option names that each module defines under
      # services.sarthi-agent. The internal names that the module system adds
      # are not in the list.
      moduleOptionNames =
        eval: lib.attrNames (lib.filterAttrs (n: _: !lib.hasPrefix "_" n) eval.options.services.sarthi-agent);
      ...
      configKeys = pkgs.runCommand "sarthi-config-keys" {} ''
        set -euo pipefail
        export HOME=$TMPDIR
        ${sarthiVenv}/bin/python3 -c '
import json, sys
...
' > $out
    ...
    in {
      ...
      checks = {
        ...
        in pkgs.runCommand "sarthi-cross-eval" { } (
          if failures != [] then
        ...
        build-package = pkgs.runCommand "sarthi-build-package" { } ''
          echo "PASS: package built at ${sarthi-agent}"
          mkdir -p $out
          echo "ok" > $out/result
        '';

        # Verify the devShell builds successfully (cross-platform).
        build-devshell = pkgs.runCommand "sarthi-build-devshell" { } ''
          ...
          let
            enabled = evalHomeSplit {
              ...
              services = {
                ...
                environment.SARTHI_TEST = "1";
                environmentFiles = [ "/run/secrets/sarthi-env" ];
                sarthiHomeFiles."SOUL.md" = "test soul";
            ...
            processes =
              if pkgs.stdenv.hostPlatform.isDarwin then
                ...
                }) (lib.filterAttrs (n: _: lib.hasPrefix "sarthi" n) cfg.launchd.agents)
              else
                lib.mapAttrs (_: unit: {
                  argv = [ unit.Service.ExecStart ];
                  env = unit.Service.Environment;
                }) (lib.filterAttrs (n: _: lib.hasPrefix "sarthi" n) cfg.systemd.user.services);
            ...
            activation = cfg.home.activation.sarthiAgentSetup.data;

            failures =
              lib.optional (names != [
                "sarthi-agent"
                "sarthi-backend"
              ]) "expected sarthi-agent + sarthi-backend processes, got: ${toString names}"
              ++ lib.optional (
                !lib.hasInfix "bin/sarthi gateway" (argvOf "sarthi-agent")
              ) "gateway process does not run `sarthi gateway`: ${argvOf "sarthi-agent"}"
              ++ lib.optional (
                !lib.hasInfix "bin/sarthi serve" (argvOf "sarthi-backend")
              ) "backend process does not run `sarthi serve`: ${argvOf "sarthi-backend"}"
              ++ lib.optional (
                !lib.hasInfix "--no-open" (argvOf "sarthi-backend")
              ) "backend must pass --no-open so a service never opens a browser"
              ++ lib.optional (
                lib.any (n: !lib.hasInfix "/home/sarthi-check/.sarthi" (envOf n)) names
              ) "gateway and backend must share one SARTHI_HOME"
              ++ lib.optional (
                cfg.home.sessionVariables.SARTHI_HOME or null != "/home/sarthi-check/.sarthi"
              ) "programs.sarthi-agent.enable must export SARTHI_HOME for interactive shells"
              ++ lib.optional (
                !lib.hasInfix "sarthi-config-merge" activation
              ) "activation must deep-merge config.yaml, not overwrite it"
              ++ lib.optional (
                !lib.hasInfix "/home/sarthi-check/.sarthi/SOUL.md" activation
              ) "sarthiHomeFiles must install into SARTHI_HOME"
              ++ lib.optional (
                !lib.hasInfix "/home/test-user/workspace/AGENTS.md" activation
              ) "documents must install into workingDirectory"
              # The CLI reads SARTHI_MANAGED to name the rebuild command when
              # it refuses to write the configuration. A Home Manager install
              # has no nixos-rebuild command. Thus it must not report NixOS.
              ++ lib.optional (
                !lib.any (n: lib.hasInfix "SARTHI_MANAGED=home-manager" (envOf n)) names
              ) "processes must report SARTHI_MANAGED=home-manager"
              ++ lib.optional (
                !lib.hasInfix "sarthi-managed" activation
              ) "activation must write a .managed marker naming the managing system";
          in
          pkgs.runCommand "sarthi-home-manager-module" { } (
            if failures != [ ] then
        ...
        workspace-files-need-a-directory =
          let
            ...
            # the priority.
            sameAsDefault = "/home/sarthi-check";
            ...
            cases = [
              ...
              {
                name = "sarthiHomeFiles needs no directory";
                ok = accepts { sarthiHomeFiles."SOUL.md" = "x"; };
              }
              {
          ...
          in
          pkgs.runCommand "sarthi-workspace-files-need-a-directory" { } (
            if failed != [ ] then
        ...
        # ── The desktop application shares one SARTHI_HOME ───────────────
        # `programs.enable` exports SARTHI_HOME with home.sessionVariables,
        ...
        # no shell profile. Thus the desktop application would open ~/.sarthi
        # while the services use the SARTHI_HOME of the module, and the user
        ...
        home-manager-desktop =
          let
            tokenFile = "/run/secrets/sarthi-desktop-token";
            ...
            enabled = evalHomeSplit {
              ...
              services = {
                enable = true;
                sarthiHome = "/home/sarthi-check/.sarthi-work";
            ...
            desktopPackages = builtins.filter (p: (p.pname or "") == "sarthi-desktop") cfg.home.packages;
            ...
            # second Sarthi runtime beside the services is the fault that
            # `programs.enable` plus a plain desktop package would give.
            agentPackages = builtins.filter (p: (p.pname or "") == "sarthi-agent") cfg.home.packages;
            ...
            backendScript =
              let
                argv =
                  if pkgs.stdenv.hostPlatform.isDarwin then
                    cfg.launchd.agents.sarthi-backend.config.ProgramArguments
                  else
                    [ cfg.systemd.user.services.sarthi-backend.Service.ExecStart ];
            ...
            failures =
              ...
              ) "programs.desktop.enable must install exactly one sarthi-desktop package, got ${toString (lib.length desktopPackages)}"
              ++ lib.optional (
                setValue "SARTHI_HOME" != "/home/sarthi-check/.sarthi-work"
              ) "the launcher must carry SARTHI_HOME: a GUI launcher reads no shell profile, so home.sessionVariables never reaches it (got: ${toString (setValue "SARTHI_HOME")})"
              ++ lib.optional (
                setValue "SARTHI_MANAGED" != "home-manager"
              ) "the launcher must report SARTHI_MANAGED=home-manager while the services own the configuration (got: ${toString (setValue "SARTHI_MANAGED")})"
              ++ lib.optional (
                lib.length agentPackages == 1
                && setValue "SARTHI_DESKTOP_SARTHI" != "${lib.head agentPackages}/bin/sarthi"
              ) "the launcher must pin the agent package that programs.enable installs, and not a second runtime: ${toString (setValue "SARTHI_DESKTOP_SARTHI")}"

              # ── The application reaches the backend of the service ──────
              ++ lib.optional (
                setValue "SARTHI_DESKTOP_REMOTE_URL" != "http://127.0.0.1:9231"
              ) "the launcher must name the backend of the service, or the application starts a second one (got: ${toString (setValue "SARTHI_DESKTOP_REMOTE_URL")})"
              ++ lib.optional (
                !lib.hasInfix "SARTHI_DESKTOP_REMOTE_TOKEN" wrapper
              ) "the launcher must give a token with the URL: the desktop resolver throws when the URL is set alone"
              ++ lib.optional (
                !lib.hasInfix "SARTHI_DASHBOARD_SESSION_TOKEN" backendScript
              ...
              ++ lib.optional (
                setValue "SARTHI_DESKTOP_REMOTE_TOKEN" != null
              ) "the token must never be a --set value: makeWrapper writes it into the world-readable Nix store";
          in
          pkgs.runCommand "sarthi-home-manager-desktop" { } (
            if failures != [ ] then
              throw "Home Manager desktop check failed:\n${lib.concatMapStringsSep "\n" (f: "  - ${f}") failures}"
            else
              ''
                echo "PASS: the desktop launcher shares SARTHI_HOME, the runtime and the backend of the service"
                mkdir -p $out
                echo "ok" > $out/result
              ''
          );
        ...
        home-manager-desktop-standalone =
          let
            ...
            failures =
              ...
              ++ lib.optional (
                !lib.hasInfix "--set SARTHI_HOME" wrapper
              ) "the launcher must carry SARTHI_HOME even with no services"
              ++ lib.optional (
                lib.hasInfix "SARTHI_MANAGED" wrapper
              ) "the launcher must not claim a managed install when no activation writes one"
              ++ lib.optional (
                lib.hasInfix "SARTHI_DESKTOP_REMOTE_URL" wrapper
              ) "the launcher must not name a backend when the services run none"
              ++ lib.optional (
                cfg.systemd.user.services ? sarthi-backend || cfg.launchd.agents ? sarthi-backend
              ) "programs.enable alone must start no service";
          in
          pkgs.runCommand "sarthi-home-manager-desktop-standalone" { } (
            if failures != [ ] then
        ...
        # command line. A silent removal thus leaves them with no `sarthi`
        ...
        home-manager-install-package-removed =
          let
            ...
            cases = [
              {
                value = true;
                expect = "programs.sarthi-agent.enable = true;";
              }
              {
                value = false;
                expect = "programs.sarthi-agent.enable = false;";
              }
            ];
          ...
          in
          pkgs.runCommand "sarthi-home-manager-install-package-removed" { } (
            if failures != [ ] then
        ...
        module-option-parity =
          ...
          in
          pkgs.runCommand "sarthi-module-option-parity" { } (
            if failures != [ ] then
        ...
        nixos-module =
          let
            ...
            units = lib.filterAttrs (n: _: lib.hasPrefix "sarthi" n) cfg.systemd.services;
            names = lib.attrNames units;
            execOf = name: units.${name}.serviceConfig.ExecStart;
            activation = cfg.system.activationScripts."sarthi-agent-setup".text;

            failures =
              lib.optional (names != [
                "sarthi-agent"
                "sarthi-backend"
              ]) "expected sarthi-agent + sarthi-backend units, got: ${toString names}"
              ++ lib.optional (
                !lib.hasInfix "bin/sarthi gateway" (execOf "sarthi-agent")
              ) "gateway unit does not run `sarthi gateway`: ${execOf "sarthi-agent"}"
              ++ lib.optional (
                !lib.hasInfix "bin/sarthi dashboard" (execOf "sarthi-backend")
              ) "backend unit does not run `sarthi dashboard`: ${execOf "sarthi-backend"}"
              ++ lib.optional (
                units.sarthi-agent.environment.SARTHI_HOME != units.sarthi-backend.environment.SARTHI_HOME
              ) "gateway and backend must share one SARTHI_HOME"
              ++ lib.optional (
                !lib.hasInfix "/var/lib/sarthi/.sarthi/SOUL.md" activation
              ) "sarthiHomeFiles must install into SARTHI_HOME";
          ...
          in
          pkgs.runCommand "sarthi-nixos-module" { } (
            if failures != [ ] then
        ...
        # polls and then execs sarthi, and the assertions reject a
        # configuration that cannot work.
        backend-bind-wait =
          let
            execOf =
              settings:
              (evalNixosModule ({ enable = true; } // settings)).config.systemd.services.sarthi-backend.serviceConfig.ExecStart;
            ...
            failures =
              ...
              lib.optional (!lib.hasInfix "bin/sarthi serve --host 127.0.0.1" direct)
                "without waitFor the backend must exec sarthi directly, got: ${direct}"
              ++ lib.optional (lib.hasInfix "sarthi-backend-launch" direct)
                "without waitFor the backend must not use the launcher"

              # The hostname mode polls the resolver, then binds the name.
              ++ lib.optional (!lib.hasInfix "sarthi-backend-launch" hostnameWait)
              ...
              ++ lib.optional (!lib.hasInfix "exec " hostnameScript)
                "the launcher must exec sarthi, so that it keeps the MainPID"
              ...
              ++ lib.optional (!lib.hasInfix "bin/sarthi dashboard" interfaceScript)
          ...
          in
          pkgs.runCommand "sarthi-backend-bind-wait" { } (
            if failures != [ ] then
        ...
        # $SARTHI_HOME/.env. The important property is that a second run
        ...
        env-file-assembly =
          let
            envScript = (import ./moduleCommon.nix { inherit lib; }).mkEnvScript {
              inherit pkgs;
              environment = {
                SARTHI_PUBLIC = "visible";
              };
            };
          in
          pkgs.runCommand "sarthi-env-file-assembly" { } ''
            set -e
            workdir=$(mktemp -d)
            ...
            grep -qx 'SARTHI_PUBLIC=visible' "$workdir/.env" || \
        ...
        service-argv =
          let
            ...
            sentinel = "--sarthi-nix-argv-probe";
            probe = argv: lib.escapeShellArgs (argv ++ [ sentinel ]);
          in
          pkgs.runCommand "sarthi-service-argv" { } ''
            set -e
        ...
        package-contents = pkgs.runCommand "sarthi-package-contents" { } ''
          set -e
          echo "=== Checking binaries ==="
          test -x ${sarthi-agent}/bin/sarthi || (echo "FAIL: sarthi binary missing"; exit 1)
          test -x ${sarthi-agent}/bin/sarthi-agent || (echo "FAIL: sarthi-agent binary missing"; exit 1)
          echo "PASS: All binaries present"

          echo "=== Checking version ==="
          ${sarthi-agent}/bin/sarthi --version 2>&1 | grep -qi "sarthi" || (echo "FAIL: version check"; exit 1)
        ...
        entry-points-sync = pkgs.runCommand "sarthi-entry-points-sync" { } ''
          set -e
          echo "=== Checking entry points match pyproject.toml [project.scripts] ==="
          for bin in sarthi sarthi-agent sarthi-acp; do
            test -x ${sarthi-agent}/bin/$bin || (echo "FAIL: $bin binary missing from Nix package"; exit 1)
        ...
        cli-commands = pkgs.runCommand "sarthi-cli-commands" { } ''
          set -e
          export HOME=$(mktemp -d)

          echo "=== Checking sarthi --help ==="
          ${sarthi-agent}/bin/sarthi --help 2>&1 | grep -q "gateway" || (echo "FAIL: gateway subcommand missing"; exit 1)
          ${sarthi-agent}/bin/sarthi --help 2>&1 | grep -q "config" || (echo "FAIL: config subcommand missing"; exit 1)
        ...
        bundled-skills = pkgs.runCommand "sarthi-bundled-skills" { } ''
          set -e
          echo "=== Checking bundled skills ==="
          test -d ${sarthi-agent}/share/sarthi-agent/skills || (echo "FAIL: skills directory missing"; exit 1)
          echo "PASS: skills directory exists"

          # -L: skills/ is a symlink to the filtered source store path
          SKILL_COUNT=$(find -L ${sarthi-agent}/share/sarthi-agent/skills -name "SKILL.md" | wc -l)
          test "$SKILL_COUNT" -gt 0 || (echo "FAIL: no SKILL.md files found in skills directory"; exit 1)
          echo "PASS: $SKILL_COUNT bundled skills found"

          grep -q "SARTHI_BUNDLED_SKILLS" ${sarthi-agent}/bin/sarthi || \
            (echo "FAIL: SARTHI_BUNDLED_SKILLS not in wrapper"; exit 1)
          echo "PASS: SARTHI_BUNDLED_SKILLS set in wrapper"

          # Optional skills ship via the wrapper too (pythonSrc excludes
          # them from the wheel, so the env var is the only path in nix).
          test -d ${sarthi-agent}/share/sarthi-agent/optional-skills || \
            (echo "FAIL: optional-skills directory missing"; exit 1)
          OPT_COUNT=$(find -L ${sarthi-agent}/share/sarthi-agent/optional-skills -name "SKILL.md" | wc -l)
          test "$OPT_COUNT" -gt 0 || (echo "FAIL: no SKILL.md files in optional-skills"; exit 1)
          grep -q "SARTHI_OPTIONAL_SKILLS" ${sarthi-agent}/bin/sarthi || \
            (echo "FAIL: SARTHI_OPTIONAL_SKILLS not in wrapper"; exit 1)
          echo "PASS: $OPT_COUNT optional skills found, SARTHI_OPTIONAL_SKILLS set in wrapper"
        ...
        bundled-plugins = pkgs.runCommand "sarthi-bundled-plugins" { } ''
          set -e
          echo "=== Checking bundled plugins ==="
          test -d ${sarthi-agent}/share/sarthi-agent/plugins || (echo "FAIL: plugins directory missing"; exit 1)
          echo "PASS: plugins directory exists"

          test -f ${sarthi-agent}/share/sarthi-agent/plugins/platforms/irc/plugin.yaml || \
            (echo "FAIL: irc plugin manifest missing"; exit 1)
          echo "PASS: irc plugin manifest present"

          grep -q "SARTHI_BUNDLED_PLUGINS" ${sarthi-agent}/bin/sarthi || \
            (echo "FAIL: SARTHI_BUNDLED_PLUGINS not in wrapper"; exit 1)
          echo "PASS: SARTHI_BUNDLED_PLUGINS set in wrapper"
        ...
        bundled-locales = pkgs.runCommand "sarthi-bundled-locales" { } ''
          set -e
          echo "=== Checking bundled locales ==="
          test -d ${sarthi-agent}/share/sarthi-agent/locales || (echo "FAIL: locales directory missing"; exit 1)
          echo "PASS: locales directory exists"

          # -L: locales/ is a symlink to the source store path
          LOC_COUNT=$(find -L ${sarthi-agent}/share/sarthi-agent/locales -name "*.yaml" | wc -l)
          test "$LOC_COUNT" -ge 16 || (echo "FAIL: expected >=16 catalogs, found $LOC_COUNT"; exit 1)
          echo "PASS: $LOC_COUNT locale catalogs found"

          test -f ${sarthi-agent}/share/sarthi-agent/locales/en.yaml || (echo "FAIL: en.yaml missing"; exit 1)
          echo "PASS: en.yaml present"

          grep -q "SARTHI_BUNDLED_LOCALES" ${sarthi-agent}/bin/sarthi || \
            (echo "FAIL: SARTHI_BUNDLED_LOCALES not in wrapper"; exit 1)
          echo "PASS: SARTHI_BUNDLED_LOCALES set in wrapper"

          # locales/ is a bare data dir (no __init__.py), shipped via a
          # symlink + SARTHI_BUNDLED_LOCALES (not via wheel data-files).
          # Verify the wrapper override resolves real strings.
          export HOME=$(mktemp -d)
          RENDERED=$(cd "$HOME" && SARTHI_BUNDLED_LOCALES=${sarthi-agent}/share/sarthi-agent/locales \
            ${sarthiVenv}/bin/python3 -c "from agent import i18n; print(i18n.t('gateway.reset.header_default', lang='en'))")
          echo "rendered: $RENDERED"
          test "$RENDERED" != "gateway.reset.header_default" || (echo "FAIL: i18n returned the raw key with SARTHI_BUNDLED_LOCALES set"; exit 1)
        ...
        # SARTHI_OPTIONAL_MCPS (not via wheel data-files).
        bundled-mcps = pkgs.runCommand "sarthi-bundled-mcps" { } ''
          set -e
          echo "=== Checking bundled optional-mcps ==="
          test -d ${sarthi-agent}/share/sarthi-agent/optional-mcps || (echo "FAIL: optional-mcps directory missing"; exit 1)
          echo "PASS: optional-mcps directory exists"

          MANIFEST_COUNT=$(find -L ${sarthi-agent}/share/sarthi-agent/optional-mcps -name "manifest.yaml" | wc -l)
          test "$MANIFEST_COUNT" -gt 0 || (echo "FAIL: no manifest.yaml files found"; exit 1)
          echo "PASS: $MANIFEST_COUNT catalog manifests found"

          grep -q "SARTHI_OPTIONAL_MCPS" ${sarthi-agent}/bin/sarthi || \
            (echo "FAIL: SARTHI_OPTIONAL_MCPS not in wrapper"; exit 1)
          echo "PASS: SARTHI_OPTIONAL_MCPS set in wrapper"

          export HOME=$(mktemp -d)
          CATALOG=$(cd "$HOME" && ${sarthi-agent}/bin/sarthi mcp catalog 2>/dev/null || true)
          echo "catalog output: $CATALOG"
          test -n "$CATALOG" || (echo "FAIL: sarthi mcp catalog returned empty"; exit 1)
        ...
        bundled-tui = pkgs.runCommand "sarthi-bundled-tui" { } ''
          set -e
          echo "=== Checking bundled TUI ==="
          test -d ${sarthi-agent}/ui-tui || (echo "FAIL: ui-tui directory missing"; exit 1)
          echo "PASS: ui-tui directory exists"

          test -f ${sarthi-agent}/ui-tui/dist/entry.js || (echo "FAIL: compiled entry.js missing"; exit 1)
          echo "PASS: compiled entry.js present"

          # self-contained bundle; no runtime node_modules expected

          grep -q "SARTHI_TUI_DIR" ${sarthi-agent}/bin/sarthi || \
            (echo "FAIL: SARTHI_TUI_DIR not in wrapper"; exit 1)
          echo "PASS: SARTHI_TUI_DIR set in wrapper"
        ...
        # Verify SARTHI_NODE is set in wrapper and points to Node 26+
        # (Sarthi pins its toolchain to Node 26 everywhere)
        sarthi-node = pkgs.runCommand "sarthi-node-version" { } ''
          set -e
          echo "=== Checking SARTHI_NODE in wrapper ==="
          grep -q "SARTHI_NODE" ${sarthi-agent}/bin/sarthi || \
            (echo "FAIL: SARTHI_NODE not set in wrapper"; exit 1)
          echo "PASS: SARTHI_NODE present in wrapper"

          SARTHI_NODE=$(sed -n "s/^export SARTHI_NODE='\(.*\)'/\1/p" ${sarthi-agent}/bin/sarthi)
          test -x "$SARTHI_NODE" || (echo "FAIL: SARTHI_NODE=$SARTHI_NODE not executable"; exit 1)
          echo "PASS: SARTHI_NODE executable at $SARTHI_NODE"

          NODE_MAJOR=$("$SARTHI_NODE" --version | sed 's/^v//' | cut -d. -f1)
          test "$NODE_MAJOR" -ge 26 || \
            (echo "FAIL: Node v$NODE_MAJOR < 26, Sarthi requires Node 26"; exit 1)
          echo "PASS: Node v$NODE_MAJOR >= 26"

          echo "=== All SARTHI_NODE checks passed ==="
          mkdir -p $out
          echo "ok" > $out/result
        '';

        # Verify SARTHI_MANAGED guard works on all mutation commands
        managed-guard = pkgs.runCommand "sarthi-managed-guard" { } ''
          set -e
          export HOME=$(mktemp -d)
          ...
          check_blocked() {
            local label="$1"
            shift
            OUTPUT=$(SARTHI_MANAGED=true "$@" 2>&1 || true)
          ...
          echo "=== Checking SARTHI_MANAGED guards ==="
          check_blocked "config set" ${sarthi-agent}/bin/sarthi config set model foo
          check_blocked "config edit" ${sarthi-agent}/bin/sarthi config edit
        ...
        extra-python-packages = let
          ...
          sarthiWithExtra = sarthi-agent.override {
            extraPythonPackages = [ testPkg ];
          };
        in pkgs.runCommand "sarthi-extra-python-packages" { } ''
          set -e
          echo "=== Checking extraPythonPackages PYTHONPATH injection ==="

          grep -q "PYTHONPATH" ${sarthiWithExtra}/bin/sarthi || \
            (echo "FAIL: PYTHONPATH not in wrapper"; exit 1)
          echo "PASS: PYTHONPATH present in wrapper"

          grep -q "${testPkg}" ${sarthiWithExtra}/bin/sarthi || \
          ...
          if grep -q "PYTHONPATH" ${sarthi-agent}/bin/sarthi; then
        ...
        extra-dependency-groups = let
          sarthiWithGroups = sarthi-agent.override {
            extraDependencyGroups = [ "honcho" ];
          };
        in pkgs.runCommand "sarthi-extra-dependency-groups" { } ''
          set -e
          ...
          echo "derivation: ${sarthiWithGroups}"
          echo "venv: ${sarthiWithGroups.sarthiVenv}"
        ...
        messaging-variant = pkgs.runCommand "sarthi-messaging-variant" { } ''
          set -e
          echo "=== Checking discord.py importable from messaging variant ==="
          ${self'.packages.messaging.sarthiVenv}/bin/python3 -c \
        ...
        in pkgs.runCommand "sarthi-config-roundtrip" {
          ...
          merge_and_load() {
            local sarthi_home="$1"
            export SARTHI_HOME="$sarthi_home"
            ${configMergeScript} ${nixSettings} "$sarthi_home/config.yaml"
            ${sarthiVenv}/bin/python3 -c '
import json, sys
