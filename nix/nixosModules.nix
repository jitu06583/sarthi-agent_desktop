# nix/nixosModules.nix — NixOS module for sarthi-agent
#
# Two modes:
...
{ inputs, ... }: {
  ...
  let
    ...
    # Deep-merge config type (from 0xrsydn/nix-sarthi-agent)
    ...
    # terminal.cwd replaces the deprecated MESSAGING_CWD env var — sarthi
    ...
    generatedConfigFile = pkgs.writeText "sarthi-config.yaml" configJson;
    ...
    documentDerivation = pkgs.runCommand "sarthi-documents" { } (
      ''
        mkdir -p $out
      '' + lib.concatStringsSep "\n" (
        lib.mapAttrsToList (name: value:
          ...
          else "cat > $out/${name} <<'SARTHI_DOC_EOF'\n${value}\nSARTHI_DOC_EOF"
        ) cfg.documents
      )
    );
  ...
  in {
    options.services.sarthi-agent = with lib; {
      enable = mkEnableOption "Sarthi Agent gateway service";

      # ── Package ──────────────────────────────────────────────────────────
      package = mkOption {
        type = types.package;
        default = sarthi-agent;
      ...
      settings = mkOption {
        ...
        default = { };
        description = ''
          Declarative Sarthi config (attrset). Deep-merged across module
      ...
      environmentFiles = mkOption {
        ...
        description = ''
          ...
          Contents are merged into $SARTHI_HOME/.env at activation time.
          Sarthi reads this file on every startup via load_sarthi_dotenv().
        '';
      };
      ...
      environment = mkOption {
        ...
        default = { };
        description = ''
          Non-secret environment variables. Merged into $SARTHI_HOME/.env
      ...
      extraPackages = mkOption {
        ...
        description = ''
          ...
          Implemented via the sarthi user's per-user profile
    ...
    config = lib.mkIf cfg.enable (lib.mkMerge [
      ...
      (lib.mkIf (cfg.mcpServers != { }) {
        services.sarthi-agent.settings.mcp_servers = lib.mapAttrs (_name: srv:
          # Stdio transport
      ...
      (lib.mkIf cfg.addToSystemPackages {
        ...
        environment.variables.SARTHI_HOME = "${cfg.stateDir}/.sarthi";
      })
      ...
      # ── Assertions ─────────────────────────────────────────────────────
      {
        ...
        in [{
          ...
          message = "services.sarthi-agent.extraPlugins: duplicate plugin names detected: ${toString names}. If using fetchFromGitHub, set name = \"plugin-name\" to disambiguate.";
        }];
      }
      ...
      # ── Directories ───────────────────────────────────────────────────
      {
        systemd.tmpfiles.rules = [
          ...
          "d ${cfg.stateDir}/.sarthi        2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.sarthi/cron   2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.sarthi/sessions 2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.sarthi/logs   2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.sarthi/memories 2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.sarthi/plugins 2770 ${cfg.user} ${cfg.group} - -"
      ...
      {
        system.activationScripts."sarthi-agent-setup" = lib.stringAfter ([ "users" ] ++ lib.optional (config.system.activationScripts ? setupSecrets) "setupSecrets") ''
          # Ensure directories exist (activation runs before tmpfiles)
          mkdir -p ${cfg.stateDir}/.sarthi
          mkdir -p ${cfg.stateDir}/home
          mkdir -p ${cfg.workingDirectory}
          chown ${cfg.user}:${cfg.group} ${cfg.stateDir} ${cfg.stateDir}/.sarthi ${cfg.stateDir}/home ${cfg.workingDirectory}
          chmod 2770 ${cfg.stateDir} ${cfg.stateDir}/.sarthi ${cfg.workingDirectory}
          ...
          find ${cfg.stateDir}/.sarthi -maxdepth 1 \
            \( -name "*.db" -o -name "*.db-wal" -o -name "*.db-shm" -o -name "SOUL.md" \) \
            -exec chmod g+rw {} + 2>/dev/null || true
          for _subdir in cron sessions logs memories plugins; do
            mkdir -p "${cfg.stateDir}/.sarthi/$_subdir"
            chown ${cfg.user}:${cfg.group} "${cfg.stateDir}/.sarthi/$_subdir"
            chmod 2770 "${cfg.stateDir}/.sarthi/$_subdir"
            find "${cfg.stateDir}/.sarthi/$_subdir" -type f \
              -exec chmod g+rw {} + 2>/dev/null || true
          done
          ...
          # sarthi-group users can save settings via the CLI/TUI, else 0640).
          ${if cfg.configFile != null then ''
            install -o ${cfg.user} -g ${cfg.group} -m ${configYamlMode} -D ${configFile} ${cfg.stateDir}/.sarthi/config.yaml
          '' else ''
            ${configMergeScript} ${generatedConfigFile} ${cfg.stateDir}/.sarthi/config.yaml
            chown ${cfg.user}:${cfg.group} ${cfg.stateDir}/.sarthi/config.yaml
            chmod ${configYamlMode} ${cfg.stateDir}/.sarthi/config.yaml
          ''}

          # Managed mode marker (so interactive shells also detect NixOS management)
          touch ${cfg.stateDir}/.sarthi/.managed
          chown ${cfg.user}:${cfg.group} ${cfg.stateDir}/.sarthi/.managed
          chmod 0644 ${cfg.stateDir}/.sarthi/.managed
          ...
          ${if cfg.container.enable then ''
            cat > ${cfg.stateDir}/.sarthi/.container-mode <<'SARTHI_CONTAINER_MODE_EOF'
    # Written by NixOS activation script. Do not edit manually.
    backend=${cfg.container.backend}
    container_name=${containerName}
    exec_user=${cfg.user}
    sarthi_bin=${containerDataDir}/current-package/bin/sarthi
    SARTHI_CONTAINER_MODE_EOF
            chown ${cfg.user}:${cfg.group} ${cfg.stateDir}/.sarthi/.container-mode
            chmod 0644 ${cfg.stateDir}/.sarthi/.container-mode
          '' else ''
            rm -f ${cfg.stateDir}/.sarthi/.container-mode
            ...
            ${lib.concatStringsSep "\n" (map (user:
              ...
              in ''
                if [ -L "${symlinkPath}" ] && [ "$(readlink "${symlinkPath}")" = "${cfg.stateDir}/.sarthi" ]; then
                  rm -f "${symlinkPath}"
          ...
          ${lib.optionalString (cfg.authFile != null) ''
            ${if cfg.authFileForceOverwrite then ''
              install -o ${cfg.user} -g ${cfg.group} -m 0600 ${cfg.authFile} ${cfg.stateDir}/.sarthi/auth.json
            '' else ''
              if [ ! -f ${cfg.stateDir}/.sarthi/auth.json ]; then
                install -o ${cfg.user} -g ${cfg.group} -m 0600 ${cfg.authFile} ${cfg.stateDir}/.sarthi/auth.json
              fi
            ''}
          ''}
          ...
          # Sarthi reads $SARTHI_HOME/.env at startup via load_sarthi_dotenv(),
          # so this is the single source of truth for both native and container mode.
          ${lib.optionalString (cfg.environment != {} || cfg.environmentFiles != []) ''
            ENV_FILE="${cfg.stateDir}/.sarthi/.env"
            install -o ${cfg.user} -g ${cfg.group} -m 0640 /dev/null "$ENV_FILE"
            cat > "$ENV_FILE" <<'SARTHI_NIX_ENV_EOF'
    ${envFileContent}
    SARTHI_NIX_ENV_EOF
            ${lib.concatStringsSep "\n" (map (f: ''
              if [ -f "${f}" ]; then
        ...
        find ${cfg.stateDir}/.sarthi/plugins -maxdepth 1 -type l -name 'nix-managed-*' -delete 2>/dev/null || true
        ...
        ${lib.concatStringsSep "\n" (map (plugin:
          ...
          in ''
            ...
            ln -sfn ${plugin} ${cfg.stateDir}/.sarthi/plugins/nix-managed-${name}
            chown -h ${cfg.user}:${cfg.group} ${cfg.stateDir}/.sarthi/plugins/nix-managed-${name}
          '') cfg.extraPlugins)}
        '';
      }
      ...
      (lib.mkIf (!cfg.container.enable) {
        systemd.services.sarthi-agent = {
          ...
          environment = {
            HOME = cfg.stateDir;
            SARTHI_HOME = "${cfg.stateDir}/.sarthi";
            SARTHI_MANAGED = "true";
