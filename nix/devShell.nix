...
{ ... }:
{
  perSystem =
    ...
    let
      ...
      sarthiNpmLib = self'.packages.default.passthru.sarthiNpmLib;
      ...
      sarthiAgentDevShellHook = self'.packages.default.passthru.devShellHook;
    in
    {
      devShells.default = pkgs.mkShell {
        packages = with pkgs; [
          (pkgs.runCommand "sarthi" { } ''
            mkdir -p $out/bin
            install -Dm755 ${../sarthi} $out/bin/sarthi
          '')
        ...
        shellHook = ''
          ${sarthiAgentDevShellHook}
          ${sarthiNpmLib.mkNpmDevShellHook npmPackageJsonPaths}
          ...
          export SARTHI_PYTHON_SRC_ROOT=$(git rev-parse --show-toplevel)
          ...
          echo "Sarthi Agent dev shell in $SARTHI_PYTHON_SRC_ROOT"
          echo "Ready. Run 'sarthi' or 'sandbox sarthi' to start."
        '';
      };
    };
}
