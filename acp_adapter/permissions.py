"""ACP permission bridging for Sarthi dangerous-command approvals."""
...
# Maps ACP permission option ids to Sarthi approval result strings.
...
def _build_permission_options(
    *, allow_permanent: bool, smart_denied: bool = False,
    ...
    """Return ACP options that match Sarthi approval semantics."""
    ...
    if not smart_denied:
        options.append(PermissionOption(
            ...
            # hint while keeping Sarthi semantics in the option id.
            kind="allow_always",
...
def _map_outcome_to_sarthi(outcome: object, *, allowed_option_ids: set[str]) -> str:
    """Map an ACP permission outcome into Sarthi approval strings."""
    ...
    if option_id not in allowed_option_ids:
        ...
        return "deny"
    return _OPTION_ID_TO_SARTHI.get(option_id, "deny")
...
def make_approval_callback(
    ...
    """
    Return a Sarthi-compatible approval callback that bridges to ACP.
