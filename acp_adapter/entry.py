"""CLI entry point for the sarthi-agent ACP adapter.

Loads environment variables from ``~/.sarthi/.env``, configures logging
...
try:
    ...
    # shadowing Sarthi's own modules — ``sarthi acp`` can be started from any
...
def _load_env() -> None:
    """Load .env from SARTHI_HOME (default ``~/.sarthi``)."""
    from sarthi_cli.env_loader import load_sarthi_dotenv

    sarthi_home = get_sarthi_home()
    loaded = load_sarthi_dotenv(sarthi_home=sarthi_home)
    if loaded:
        for env_file in loaded:
            logging.getLogger(__name__).info("Loaded env from %s", env_file)
    else:
        logging.getLogger(__name__).info(
            "No .env found at %s, using system env", sarthi_home / ".env"
        )
...
def _parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        prog="sarthi-acp",
        description="Run Sarthi Agent as an ACP stdio server.",
    )
    parser.add_argument("--version", action="store_true", help="Print Sarthi version and exit")
    parser.add_argument(
        "--check",
        action="store_true",
    ...
    parser.add_argument(
        "--setup",
        action="store_true",
        help="Run interactive Sarthi provider/model setup for ACP terminal auth",
    )
    parser.add_argument(
        "--setup-browser",
        action="store_true",
        help="Install agent-browser + Playwright Chromium into ~/.sarthi/node/ "
...
def _print_version() -> None:
    ...
    print(sarthi_version)


def _run_check() -> None:
    import acp  # noqa: F401
    from acp_adapter.server import SarthiACPAgent  # noqa: F401

    print("Sarthi ACP check OK")


def _run_setup() -> None:
    from sarthi_cli.main import main as sarthi_main

    old_argv = sys.argv[:]
    try:
        sys.argv = [old_argv[0] if old_argv else "sarthi", "model"]
        sarthi_main()
    finally:
        sys.argv = old_argv
...
def main(argv: list[str] | None = None) -> None:
    ...
    logger.info("Starting sarthi-agent ACP adapter")
    ...
    import acp
    from .server import SarthiACPAgent
    ...
    if os.environ.get("SARTHI_ACP_SKIP_CONFIGURED_MCP", "").strip() != "1":
        try:
