"""ACP auth helpers — detect and advertise Sarthi authentication methods."""
...
TERMINAL_SETUP_AUTH_METHOD_ID = "sarthi-setup"


def detect_provider() -> Optional[str]:
    """Resolve the active Sarthi runtime provider, or None if unavailable.
...
def has_provider() -> bool:
    """Return True if Sarthi can resolve any runtime provider credentials."""
    return detect_provider() is not None


def build_auth_methods() -> list[Any]:
    """Return registry-compatible ACP auth methods for Sarthi.
    ...
    not have Sarthi provider credentials configured yet, so Sarthi always
    ...
    if provider:
        methods.append(
            AuthMethodAgent(
                ...
                description=(
                    "Authenticate Sarthi using the currently configured "
