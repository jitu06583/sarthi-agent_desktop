/**
 * Sarthi Kanban — Dashboard Plugin
 *
 * Board view for the multi-agent collaboration board backed by
 * ~/.sarthi/kanban.db. Calls the plugin's backend at /api/plugins/kanban/
 * and tails task_events over a WebSocket for live updates.
 *
 * Plain IIFE, no build step. Uses window.__SARTHI_PLUGIN_SDK__ for React +
...
(function () {
  ...
  const SDK = window.__SARTHI_PLUGIN_SDK__;
  if (!SDK) return;
  ...
  const COLUMN_DOT = {
    triage: "sarthi-kanban-dot-triage",
    todo: "sarthi-kanban-dot-todo",
    ready: "sarthi-kanban-dot-ready",
    running: "sarthi-kanban-dot-running",
    blocked: "sarthi-kanban-dot-blocked",
    review: "sarthi-kanban-dot-review",
    done: "sarthi-kanban-dot-done",
    archived: "sarthi-kanban-dot-archived",
  };
  ...
  const MIME_TASK = "text/x-sarthi-task";
  ...
  const LS_BOARD_KEY = "sarthi.kanban.selectedBoard";
  ...
  function renderMarkdown(src) {
    ...
    html = html.replace(/\u0000CODE(\d+)\u0000/g, (_m, i) =>
      `<pre class="sarthi-kanban-md-code"><code>${escapeHtml(blocks[Number(i)])}</code></pre>`,
    );
    return html;
  }
  ...
  function sanitizeMarkdownAttrs(tag, attrs) {
    ...
    if (tag === "pre" && /\sclass=(["'])sarthi-kanban-md-code\1/i.test(attrs)) {
      return ' class="sarthi-kanban-md-code"';
    }
    return "";
  }
  ...
  function MarkdownBlock(props) {
    ...
    if (!enabled) {
      return h("pre", { className: "sarthi-kanban-pre" }, props.source || "");
    }
    return h("div", {
      className: "sarthi-kanban-md",
  ...
  // standard `drop` event and our `sarthi-kanban:drop` event.
  // -------------------------------------------------------------------------
  ...
  function attachTouchDrag(el, taskId) {
    ...
    function onDown(e) {
      ...
      proxy.classList.add("sarthi-kanban-touch-proxy");
      ...
      function move(ev) {
        ...
        if (target !== lastTarget) {
          if (lastTarget) lastTarget.classList.remove("sarthi-kanban-column--drop");
          if (target) target.classList.add("sarthi-kanban-column--drop");
          lastTarget = target;
        }
      }
      function up() {
        ...
        if (lastTarget) {
          lastTarget.classList.remove("sarthi-kanban-column--drop");
          const status = lastTarget.getAttribute("data-kanban-column");
          const isTrash = lastTarget.hasAttribute("data-kanban-trash");
          if (isTrash) {
            lastTarget.dispatchEvent(new CustomEvent("sarthi-kanban:delete", {
              detail: { taskId },
              bubbles: true,
            }));
          } else if (status) {
            lastTarget.dispatchEvent(new CustomEvent("sarthi-kanban:drop", {
  ...
  function KanbanPage() {
    ...
    useEffect(function () {
      ...
      function openWs() {
        ...
        // loopback. Reading window.__SARTHI_SESSION_TOKEN__ directly (the old
   ...
   const deleteTask = useCallback(function (taskId) {
    ...
    return h(ErrorBoundary, null,
      h("div", { className: "sarthi-kanban flex flex-col gap-4" },
        h(BoardSwitcher, {
          board: board,
          boardList: boardList,
  ...
  function AttentionStrip(props) {
    ...
    return h("div", {
      className: cn(
        "sarthi-kanban-attention",
        "sarthi-kanban-attention--" + topSev,
      ),
    },
      h("div", { className: "sarthi-kanban-attention-bar" },
        h("span", { className: "sarthi-kanban-attention-icon" },
          topSev === "critical" ? "!!!" : topSev === "error" ? "!!" : "⚠"),
        h("span", { className: "sarthi-kanban-attention-text" },
          diagTasks.length === 1
      ...
      expanded
        ? h("div", { className: "sarthi-kanban-attention-list" },
            diagTasks.map(function (task) {
              ...
              return h("div", {
                key: task.id,
                className: cn(
                  "sarthi-kanban-attention-row",
                  "sarthi-kanban-attention-row--" + sev,
                ),
              },
                h("span", { className: "sarthi-kanban-attention-row-sev" },
                  sev === "critical" ? "!!!" : sev === "error" ? "!!" : "⚠"),
                h("span", { className: "sarthi-kanban-attention-row-id" }, task.id),
                h("span", { className: "sarthi-kanban-attention-row-title" },
                  task.title || tx(t, "untitled", "(untitled)")),
                h("span", { className: "sarthi-kanban-attention-row-meta" },
                  task.assignee ? "@" + task.assignee : tx(t, "unassigned", "unassigned"),
                  " \u00b7 ",
                  kinds.length > 0 ? kinds.join(", ") : tx(t, "diagnostic", "diagnostic"),
                ),
                h("button", {
                  className: "sarthi-kanban-attention-row-btn",
  ...
  function DiagnosticActionButton(props) {
    ...
    const cls = cn(
      "sarthi-kanban-diag-action-btn",
      action.suggested ? "sarthi-kanban-diag-action-btn--suggested" : "",
    );
    ...
    return h("span", { className: cls + " sarthi-kanban-diag-action-btn--unknown" },
      label);
  }
  ...
  function DiagnosticCard(props) {
    ...
    const execAction = function (action) {
      ...
      if (action.kind === "comment") {
        ...
        const ta = document.querySelector(".sarthi-kanban-drawer-comment-row input, .sarthi-kanban-drawer-comment-row textarea");
        if (ta) {
    ...
    const sevClass = "sarthi-kanban-diag--" + (diag.severity || "warning");
    return h("div", { className: cn("sarthi-kanban-diag", sevClass) },
      h("div", { className: "sarthi-kanban-diag-header" },
        h("span", { className: "sarthi-kanban-diag-sev" },
          diag.severity === "critical" ? "!!!" :
          diag.severity === "error" ? "!!" : "\u26a0"),
        h("span", { className: "sarthi-kanban-diag-title" },
          diag.title),
      ),
      h("div", { className: "sarthi-kanban-diag-detail" },
        diag.detail),
      diag.data && Object.keys(diag.data).length > 0
        ? h("div", { className: "sarthi-kanban-diag-data" },
            Object.keys(diag.data).map(function (k) {
              const v = diag.data[k];
              if (Array.isArray(v) && v.length > 0 && typeof v[0] === "string" &&
                  v[0].indexOf("t_") === 0) {
                // Task-id list — render as chips.
                return h("div", { key: k, className: "sarthi-kanban-diag-data-row" },
                  h("span", { className: "sarthi-kanban-diag-data-key" }, k + ":"),
                  v.map(function (x) {
                    return h("code", {
                      key: x, className: "sarthi-kanban-event-phantom-chip",
                    }, x);
                  }),
                );
              }
              return h("div", { key: k, className: "sarthi-kanban-diag-data-row" },
                h("span", { className: "sarthi-kanban-diag-data-key" }, k + ":"),
                h("span", { className: "sarthi-kanban-diag-data-val" },
      ...
      reassignAction
        ? h("div", { className: "sarthi-kanban-diag-reassign-row" },
            h("span", { className: "sarthi-kanban-diag-reassign-label" },
              tx(t, "reassignTo", "Reassign to:")),
            h("select", {
              className: "sarthi-kanban-recovery-select",
        ...
        : null,
      h("div", { className: "sarthi-kanban-diag-actions" },
      ...
      msg
        ? h("div", {
            className: cn(
              "sarthi-kanban-diag-msg",
              msg.ok ? "sarthi-kanban-diag-msg--ok" : "sarthi-kanban-diag-msg--err",
            ),
          }, msg.text)
        : null,
    );
  ...
  function DiagnosticsSection(props) {
    ...
    return h("div", { className: "sarthi-kanban-section" },
      h("div", { className: "sarthi-kanban-section-head-row" },
        h("span", { className: "sarthi-kanban-section-head" },
          hasOpenDiags
            ? h("span", { className: "sarthi-kanban-section-head-warning" },
                `\u26a0 ${tx(t, "diagnostics", "Diagnostics")} (${diags.length})`)
            : tx(t, "diagnostics", "Diagnostics"),
        ),
        h("button", {
          className: "sarthi-kanban-section-toggle",
          onClick: function () { setOpen(function (x) { return !x; }); },
          type: "button",
        }, open ? tx(t, "hide", "Hide") : tx(t, "show", "Show")),
      ),
      open
        ? h("div", { className: "sarthi-kanban-diag-list" },
  ...
  function DocsLink() {
    return h("a", {
      ...
      className: "sarthi-kanban-docs-link",
      title: "Open Sarthi Kanban docs in a new tab",
      "aria-label": "Sarthi Kanban documentation",
    }, "?");
  }
  ...
  function OrchestrationPanel() {
    ...
    // each triage card (or runs `sarthi kanban decompose <id>`) and tasks
    ...
    return h(Card, { className: "p-3" },
      h(CardContent, { className: "p-2 flex flex-col gap-3" },
        ...
        msg ? h("div", {
          className: msg.ok ? "sarthi-kanban-msg-ok" : "sarthi-kanban-msg-err",
        }, msg.text) : null,
  ...
  function BoardSwitcher(props) {
    ...
    if (!shouldShow) {
      return h("div", {
        className: "sarthi-kanban-boardswitcher-compact",
    ...
    return h("div", { className: "sarthi-kanban-boardswitcher" },
      h("div", { className: "sarthi-kanban-boardswitcher-inner" },
  ...
  function NewBoardDialog(props) {
    ...
    return h("div", {
      className: "sarthi-kanban-dialog-backdrop",
      onClick: function (e) { if (e.target === e.currentTarget) props.onCancel(); },
    },
      h("form", {
        className: "sarthi-kanban-dialog",
        onSubmit: onSubmit,
      },
        h("div", { className: "sarthi-kanban-dialog-title" },
        ...
        h("div", { className: "sarthi-kanban-dialog-actions" },
          h(Button, {
            type: "button",
  ...
  function BoardToolbar(props) {
    ...
    return h("div", { className: "flex flex-wrap items-end gap-3" },
      ...
      h("div", { className: "flex flex-col gap-1",
                 title: "Filter by assigned Sarthi profile. Profiles are the named agent identities that claim and work on tasks." },
  ...
  function BulkActionBar(props) {
    ...
    return h("div", { className: "sarthi-kanban-bulk" },
      h("span", { className: "sarthi-kanban-bulk-count" },
      ...
      h("div", { className: "sarthi-kanban-bulk-priority",
      ...
      h("div", { className: "sarthi-kanban-bulk-reassign",
                 title: "Reassign selected tasks to a different Sarthi profile. Pick a profile (or unassign) and click Apply." },
        h(Select, Object.assign({
          value: assignee,
          className: "h-7 text-xs",
      ...
      h("label", { className: "sarthi-kanban-bulk-reclaim-first", title: "Reclaim any active claims before reassigning" },
        h(Checkbox, {
  ...
  function TrashDropZone(props) {
    ...
    useEffect(function () {
      ...
      el.addEventListener("sarthi-kanban:delete", onTouchDelete);
      return function () { el.removeEventListener("sarthi-kanban:delete", onTouchDelete); };
    }, [props.onDelete]);
    ...
    return h("div", {
      ...
      className: cn(
        "sarthi-kanban-trash",
        dragOver ? "sarthi-kanban-trash--drop" : "",
        props.draggingTaskId ? "sarthi-kanban-trash--active" : "",
      ),
      onDragOver: handleDragOver,
      onDragLeave: handleDragLeave,
      onDrop: handleDrop,
    },
      h("span", { className: "sarthi-kanban-trash-icon" }, "🗑️"),
      h("span", { className: "sarthi-kanban-trash-label" },
  ...
  function BoardColumns(props) {
    ...
    const isPanBlockedTarget = useCallback(function (target) {
      ...
      if (target.closest && target.closest(".sarthi-kanban-card")) return true;
      if (target.closest && target.closest(".sarthi-kanban-column-add")) return true;
      if (target.closest && target.closest(".sarthi-kanban-col-check")) return true;
    ...
    const stopPan = useCallback(function () {
      ...
      if (el) {
        ...
        el.classList.remove("sarthi-kanban-columns--panning");
        el.style.userSelect = "";
      }
    ...
    const handleMouseDown = useCallback(function (e) {
      ...
      setIsPanning(true);
      el.classList.add("sarthi-kanban-columns--panning");
    ...
    const handleDragStart = useCallback(function (e) {
      const card = e.target.closest && e.target.closest(".sarthi-kanban-card");
      if (!card) return;
    ...
    return h("div", {
      ref: columnsRef,
      className: cn(
        "sarthi-kanban-columns",
        isScrollable ? "sarthi-kanban-columns--scrollable" : "",
        isPanning ? "sarthi-kanban-columns--panning" : "",
      ),
      onDragStart: handleDragStart,
      onDragEnd: handleDragEnd,
  ...
  function Column(props) {
    ...
    useEffect(function () {
      ...
      el.addEventListener("sarthi-kanban:drop", onTouchDrop);
      return function () { el.removeEventListener("sarthi-kanban:drop", onTouchDrop); };
    ...
    return h("div", {
      ...
      className: cn(
        "sarthi-kanban-column",
        dragOver ? "sarthi-kanban-column--drop" : "",
      ),
      onDragOver: handleDragOver,
      onDragLeave: handleDragLeave,
      onDrop: handleDrop,
    },
      h("div", { className: "sarthi-kanban-column-header",
                 title: colHelp || "" },
        h(Checkbox, {
          className: "sarthi-kanban-col-check",
        ...
        h("span", { className: cn("sarthi-kanban-dot", COLUMN_DOT[props.column.name]) }),
        h("span", { className: "sarthi-kanban-column-label" },
          colLabel || props.column.name),
        h("span", { className: "sarthi-kanban-column-count",
                    title: `${props.column.tasks.length} task${props.column.tasks.length === 1 ? "" : "s"} in this column` },
          props.column.tasks.length),
        h("button", {
          type: "button",
          className: "sarthi-kanban-column-add",
          title: tx(t, "createTask", "Create task in this column"),
          onClick: function () { setShowCreate(function (v) { return !v; }); },
        }, showCreate ? "×" : "+"),
      ),
      h("div", { className: "sarthi-kanban-column-sub" },
        colHelp || ""),
      ...
      h("div", { className: "sarthi-kanban-column-body" },
        props.column.tasks.length === 0
          ? h("div", { className: "sarthi-kanban-empty" }, tx(t, "noTasks", "— no tasks —"))
          : lanes
            ? lanes.map(function (lane) {
                return h("div", { key: lane.assignee, className: "sarthi-kanban-lane" },
                  h("div", { className: "sarthi-kanban-lane-head" },
                    h("span", { className: "sarthi-kanban-lane-name" }, lane.assignee),
                    h("span", { className: "sarthi-kanban-lane-count" }, lane.tasks.length),
                  ),
  ...
  function stalenessClass(task) {
    ...
    if (age >= tier.red)   return "sarthi-kanban-card--stale-red";
    if (age >= tier.amber) return "sarthi-kanban-card--stale-amber";
    return "";
  }
  ...
  function TaskCard(props) {
    ...
    const handleDragStart = function (e) {
      ...
      const selectedCards = document.querySelectorAll(".sarthi-kanban-card--selected");
      if (selectedCards.length > 1 && props.selected) {
        const ghost = document.createElement("div");
        ghost.className = "sarthi-kanban-drag-ghost";
    ...
    return h("div", {
      ...
      className: cn(
        "sarthi-kanban-card",
        props.selected ? "sarthi-kanban-card--selected" : "",
        props.failed ? "sarthi-kanban-card--failed" : "",
        props.draggingSource ? "sarthi-kanban-card--dragging-source" : "",
        stalenessClass(t),
      ),
      draggable: true,
      tabIndex: 0,
      ...
      h(Card, null,
        h(CardContent, { className: "sarthi-kanban-card-content" },
          h("div", { className: "sarthi-kanban-card-row" },
            h("label", {
              className: "sarthi-kanban-card-check-wrap",
              title: tx(i18n, "selectForBulk", "Select for bulk actions"),
              onClick: function (e) { e.stopPropagation(); },
            },
              h(Checkbox, {
                className: "sarthi-kanban-card-check",
            ...
            h("span", { className: "sarthi-kanban-card-id",
                        title: `Task id: ${t.id}. Use this id with kanban_show, /kanban show, or sarthi kanban show.` }, t.id),
            t.warnings && t.warnings.count > 0
              ? h("span", {
                  className: cn(
                    "sarthi-kanban-warning-badge",
                    "sarthi-kanban-warning-badge--" + (t.warnings.highest_severity || "warning"),
                  ),
                  title: (
            ...
            needsAssignee
              ? h(Badge, {
                  variant: "outline",
                  className: "sarthi-kanban-needs-assignee",
          ...
          h("div", { className: "sarthi-kanban-card-title" },
            t.title || tx(i18n, "untitled", "(untitled)")),
          h("div", { className: "sarthi-kanban-card-row sarthi-kanban-card-meta" },
            t.assignee
              ? h("span", { className: "sarthi-kanban-assignee",
                            title: `Assigned to Sarthi profile @${t.assignee}` }, "@", t.assignee)
              : h("span", { className: "sarthi-kanban-unassigned",
                            title: needsAssignee
            ...
            t.comment_count > 0
              ? h("span", { className: "sarthi-kanban-count",
            ...
            t.link_counts && (t.link_counts.parents + t.link_counts.children) > 0
              ...
              : null,
            h("span", { className: "sarthi-kanban-ago",
  ...
  // centered modal reusing the sarthi-kanban-dialog chrome so the form is
  ...
  function InlineCreate(props) {
    ...
    return h("div", {
      ...
      h("form", {
        className: "sarthi-kanban-dialog sarthi-kanban-create-dialog",
            ...
            h("div", { className: "flex flex-col gap-1 flex-1" },
              ...
              h(Input, {
                ...
                title: props.columnName === "triage"
                  ? "Sarthi profile that will spec this task (default: the dispatcher's configured specifier). Leave blank to let the dispatcher pick."
                  : "Sarthi profile to assign. Leave blank and the dispatcher will pick from available profiles when the task is Ready.",
  ...
  function TaskDrawer(props) {
    ...
    return h("div", { className: "sarthi-kanban-drawer-shade", onClick: props.onClose },
      h("div", {
        className: "sarthi-kanban-drawer",
        onClick: function (e) { e.stopPropagation(); },
      },
        h("div", { className: "sarthi-kanban-drawer-head" },
          h("span", { className: "text-xs text-muted-foreground" }, props.taskId),
          h("button", {
            type: "button",
            onClick: props.onClose,
            className: "sarthi-kanban-drawer-close",
        ...
        data ? h("div", { className: "sarthi-kanban-drawer-comment-foot" },
          h("div", {
            className: "sarthi-kanban-comment-hint text-xs text-muted-foreground",
          ...
          h("div", { className: "sarthi-kanban-drawer-comment-row" },
            h(Input, {
              value: newComment,
  ...
  function AttachmentsSection(props) {
    ...
    return h("div", { className: "sarthi-kanban-section" },
      h("div", { className: "sarthi-kanban-section-head" },
      ...
      atts.length === 0
        ...
        : atts.map(function (a) {
            return h("div", {
              ...
              h("button", {
                type: "button",
                className: "sarthi-kanban-attachment-link truncate",
                title: a.filename,
  ...
  function TaskDetail(props) {
    ...
    return h("div", { className: "sarthi-kanban-drawer-body" },
      h("div", { className: "sarthi-kanban-drawer-title" },
        h("span", { className: cn("sarthi-kanban-dot", COLUMN_DOT[t.status]) }),
        props.editing
          ? h(TitleEditor, {
              initial: t.title || "",
          ...
          : h("span", {
              className: "sarthi-kanban-drawer-title-text",
              title: tx(i18n, "clickToEdit", "Click to edit"),
              onClick: function () { props.setEditing(true); },
            }, t.title || tx(i18n, "untitled", "(untitled)")),
      ),
      h("div", { className: "sarthi-kanban-drawer-meta" },
      ...
      (function () {
        ...
        if (finalResult) {
          ...
          return h("div", { className: "sarthi-kanban-section" },
            h("div", { className: "sarthi-kanban-section-head" }, label),
            h(MarkdownBlock, { source: finalResult, enabled: props.renderMarkdown }),
          );
        }
        if (isDone && isParent) {
          return h("div", { className: "sarthi-kanban-section" },
            h("div", { className: "sarthi-kanban-section-head" }, tx(i18n, "result", "Result")),
            h("div", { className: "sarthi-kanban-done-no-result sarthi-kanban-done-parent-note" },
        ...
        if (isDone) {
          return h("div", { className: "sarthi-kanban-section" },
            h("div", { className: "sarthi-kanban-section-head" }, tx(i18n, "result", "Result")),
            h("div", { className: "sarthi-kanban-done-no-result" },
      ...
      childResults.length > 0 ? h("div", { className: "sarthi-kanban-section" },
        h("div", { className: "sarthi-kanban-section-head" },
        ...
        childResults.map(function (child) {
          ...
          return h("div", { key: child.id, className: "sarthi-kanban-comment" },
            h("div", { className: "sarthi-kanban-comment-head" },
              h("span", { className: "sarthi-kanban-comment-author" },
                `${child.id} · ${child.title || tx(i18n, "untitled", "(untitled)")}`),
              h(Badge, { variant: "outline" }, child.status),
              h("button", {
                type: "button",
                className: "sarthi-kanban-diag-action-btn",
      ...
      h("div", { className: "sarthi-kanban-section" },
        h("div", { className: "sarthi-kanban-section-head" },
        ...
        comments.length === 0
          ...
          : comments.map(function (c) {
              return h("div", { key: c.id, className: "sarthi-kanban-comment" },
                h("div", { className: "sarthi-kanban-comment-head" },
                  h("span", { className: "sarthi-kanban-comment-author" }, c.author || "anon"),
                  h("span", { className: "sarthi-kanban-comment-ago" },
      ...
      h("div", { className: "sarthi-kanban-section" },
        ...
        events.slice().reverse().slice(0, 20).map(function (e) {
          ...
          return h("div", {
            ...
            className: cn(
              "sarthi-kanban-event",
              isDiag ? "sarthi-kanban-event--hallucination" : "",
            ),
          },
            isDiag
              ? h("div", { className: "sarthi-kanban-event-header" },
                  h("span", { className: "sarthi-kanban-event-warning-icon" }, "⚠"),
                  h("span", { className: "sarthi-kanban-event-warning-label" },
                    getDiagnosticEventLabel(i18n, e.kind) || e.kind),
                  h("span", { className: "sarthi-kanban-event-ago" },
                    timeAgo ? timeAgo(e.created_at) : ""),
                )
              : h("div", { className: "sarthi-kanban-event-header-plain" },
                  h("span", { className: "sarthi-kanban-event-kind" }, e.kind),
                  h("span", { className: "sarthi-kanban-event-ago" },
                    timeAgo ? timeAgo(e.created_at) : ""),
                ),
            isDiag && phantoms.length > 0
              ? h("div", { className: "sarthi-kanban-event-phantom-row" },
                  h("span", { className: "sarthi-kanban-event-phantom-label" },
                    tx(i18n, "phantomIds", "Phantom ids:")),
                  phantoms.map(function (pid) {
                    return h("code", {
                      key: pid,
                      className: "sarthi-kanban-event-phantom-chip",
                    }, pid);
                  }),
                )
              : null,
            e.payload && !isDiag
              ? h("code", { className: "sarthi-kanban-event-payload" },
    ...
    return h("div", { className: "sarthi-kanban-section" },
      h("div", { className: "sarthi-kanban-section-head-row" },
        ...
        !showAll
          ? h("button", {
              type: "button",
              onClick: function () { setExpanded(true); },
              className: "sarthi-kanban-edit-link",
      ...
      visible.map(function (r) {
        const outcomeClass = r.ended_at
          ? `sarthi-kanban-run--${r.outcome || r.status || "ended"}`
          : "sarthi-kanban-run--active";
        return h("div", { key: r.id, className: cn("sarthi-kanban-run", outcomeClass) },
          h("div", { className: "sarthi-kanban-run-head" },
            h("span", { className: "sarthi-kanban-run-outcome" },
              r.ended_at ? (r.outcome || r.status || tx(t, "ended", "ended")) : tx(t, "active", "active")),
            h("span", { className: "sarthi-kanban-run-profile" },
              r.profile ? `@${r.profile}` : tx(t, "noProfile", "(no profile)")),
            h("span", { className: "sarthi-kanban-run-elapsed" }, fmtElapsed(r)),
            h("span", { className: "sarthi-kanban-run-ago" },
              timeAgo ? timeAgo(r.started_at) : ""),
          ),
          r.summary
            ? h("div", { className: "sarthi-kanban-run-summary" }, r.summary)
            : null,
          r.error
            ? h("div", { className: "sarthi-kanban-run-error" }, r.error)
            : null,
          (r.metadata && Object.keys(r.metadata).length > 0)
            ? (function () {
                ...
                return h("details", {
                    className: "sarthi-kanban-run-meta-block",
                    open: !collapsed,
                  },
                  h("summary", { className: "sarthi-kanban-run-meta-label" }, "Metadata"),
                  h("code", { className: "sarthi-kanban-run-meta" }, json),
                );
              })()
            : null,
        );
  ...
  function WorkerLogSection(props) {
    ...
    if (state.loading) {
      ...
      body = h("pre", { className: "sarthi-kanban-pre sarthi-kanban-log" },
        data.content || "(empty)");
    }
  ...
  function MetaRow(props) {
    return h("div", { className: "sarthi-kanban-meta-row" },
      h("span", { className: "sarthi-kanban-meta-label" }, props.label),
      h("span", { className: "sarthi-kanban-meta-value" }, props.value),
    );
  }
  ...
  function TitleEditor(props) {
    ...
    return h("div", { className: "sarthi-kanban-edit-row" },
      h(Input, {
  ...
  function AssigneeEditor(props) {
    ...
    if (!editing) {
      return h("div", { className: "sarthi-kanban-meta-row" },
        h("span", { className: "sarthi-kanban-meta-label" }, tx(t, "assignee", "Assignee")),
        h("span", {
          className: "sarthi-kanban-meta-value sarthi-kanban-editable",
  ...
  function PriorityEditor(props) {
    ...
    if (!editing) {
      return h("div", { className: "sarthi-kanban-meta-row" },
        h("span", { className: "sarthi-kanban-meta-label" }, tx(t, "priority", "Priority")),
        h("span", {
          className: "sarthi-kanban-meta-value sarthi-kanban-editable",
          onClick: function () { setEditing(true); },
  ...
  function ModelEditor(props) {
    ...
    if (!editing) {
      return h("div", { className: "sarthi-kanban-meta-row" },
        h("span", { className: "sarthi-kanban-meta-label" }, tx(t, "model", "Model")),
        h("span", {
          className: cn(
            "sarthi-kanban-meta-value sarthi-kanban-editable",
    ...
    return h("div", { className: "sarthi-kanban-meta-row" },
      h("span", { className: "sarthi-kanban-meta-label" }, tx(t, "model", "Model")),
      loading
        ? h("span", { className: "sarthi-kanban-meta-value text-muted-foreground" },
  ...
  function BodyEditor(props) {
    ...
    return h("div", { className: "sarthi-kanban-section" },
      h("div", { className: "sarthi-kanban-section-head-row" },
        h("span", { className: "sarthi-kanban-section-head" }, tx(t, "description", "Description")),
        editing
      ...
      editing
        ? h("textarea", {
            className: "sarthi-kanban-textarea",
            value: v,
            rows: 8,
            onChange: function (e) { setV(e.target.value); },
          })
  ...
  function DependencyEditor(props) {
    ...
    return h("div", { className: "sarthi-kanban-section" },
      h("div", { className: "sarthi-kanban-section-head" }, tx(t, "dependencies", "Dependencies")),
      h("div", { className: "sarthi-kanban-deps-row" },
        h("span", { className: "sarthi-kanban-deps-label" }, tx(t, "parents", "Parents:")),
        h("div", { className: "sarthi-kanban-deps-chips" },
          (links.parents || []).length === 0
            ? h("span", { className: "sarthi-kanban-deps-empty" }, tx(t, "none", "none"))
            : (links.parents || []).map(function (id) {
                return h("span", { key: id, className: "sarthi-kanban-dep-chip" },
                  id,
                  h("button", {
                    type: "button",
                    className: "sarthi-kanban-dep-chip-x",
      ...
      h("div", { className: "sarthi-kanban-deps-row" },
        h("span", { className: "sarthi-kanban-deps-label" }, tx(t, "children", "Children:")),
        h("div", { className: "sarthi-kanban-deps-chips" },
  ...
  function StatusActions(props) {
    ...
    return h("div", null,
      h("div", { className: "sarthi-kanban-actions" },
        specifyButton,
        decomposeButton,
      ...
      specifyMsg ? h("div", {
        className: specifyMsg.ok
          ? "sarthi-kanban-msg-ok"
          : "sarthi-kanban-msg-err",
  ...
  function HomeSubsSection(props) {
    ...
    return h("div", { className: "sarthi-kanban-section" },
      ...
      h("div", { className: "sarthi-kanban-home-subs" },
        channels.map(function (hc) {
          ...
          return h(Button, {
            ...
            className: hc.subscribed
              ? "sarthi-kanban-home-sub sarthi-kanban-home-sub--on"
              : "sarthi-kanban-home-sub",
          }, label);
        })
      )
    );
  ...
  if (window.__SARTHI_PLUGINS__ && typeof window.__SARTHI_PLUGINS__.register === "function") {
    window.__SARTHI_PLUGINS__.register("kanban", KanbanPage);
  }
})();
