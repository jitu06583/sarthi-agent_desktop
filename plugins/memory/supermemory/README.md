...
- Hosted: API key from [app.supermemory.ai/integrations?connect=sarthi](http://app.supermemory.ai/integrations?connect=sarthi)
...
Before running `sarthi memory setup`, add the local endpoint to
...
Then run `sarthi memory setup` and enter the local server's API key. Configuring
the endpoint first ensures the setup connection probe also stays local.

## Config

Config file: `$SARTHI_HOME/supermemory.json`
...
| `container_tag` | `sarthi` | Container tag used for search and writes. Supports `{identity}` template for profile-scoped tags (e.g. `sarthi-{identity}` → `sarthi-coder`). |
...
`https://api.supermemory.ai`. Sarthi resolves it once and uses the same endpoint
...
All Supermemory API calls send `x-sm-source: sarthi`, and document writes stamp
`metadata.sm_source: sarthi`. This is a **functional routing key, not telemetry**:
it groups Sarthi-written memories into a dedicated "Sarthi" Space in the
...
Use `{identity}` in the `container_tag` to scope memories per Sarthi profile:

```json
{
  "container_tag": "sarthi-{identity}"
}
```

For a profile named `coder`, this resolves to `sarthi-coder`. The default profile resolves to `sarthi-default`. Without `{identity}`, all profiles share the same container.
