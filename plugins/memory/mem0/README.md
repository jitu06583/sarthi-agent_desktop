...
```bash
sarthi memory setup    # select "mem0"
```
...
Behavioral settings live in `$SARTHI_HOME/mem0.json` (set them via `sarthi memory setup`). Only the secret `MEM0_API_KEY` belongs in `~/.sarthi/.env`.
...
| `user_id` | `sarthi-user` | User identifier on Mem0 |
...
2. Point the plugin at it — via the setup wizard:
   ...
   ```
3. Start a fresh Sarthi session and call `mem0_search` — it connects to your server.
...
```bash
sarthi memory setup mem0 --mode oss \
...
```bash
sarthi memory setup mem0 --mode oss --oss-llm-key sk-...
```

Or edit `$SARTHI_HOME/mem0.json` directly:
```json
{
  "mode": "oss",
  "oss": {
    ...
    "vector_store": {"provider": "qdrant", "config": {"path": "~/.sarthi/mem0_qdrant"}}
  }
}
```
...
```bash
sarthi memory setup mem0 --mode platform --api-key sk-...
```
...
- Check `user_id` matches between sessions (`$SARTHI_HOME/mem0.json`).
