...
> **Honcho docs:** <https://docs.honcho.dev/v3/guides/integrations/sarthi>
...
```bash
sarthi memory setup honcho   # configure Honcho directly (works on a fresh install)
sarthi memory setup          # generic picker, choose Honcho from the list
```
...
Or manually:
```bash
sarthi config set memory.provider honcho
echo "HONCHO_API_KEY=***" >> ~/.sarthi/.env
```

> `sarthi honcho setup` also works, but only **after** Honcho is the active
> memory provider — the `honcho` subcommand is registered for the active
> provider only. On a fresh install, use `sarthi memory setup honcho`.
...
cycle (not per pass). Select a fast, inexpensive model under `sarthi model`
...
| Priority | Path | Scope |
|----------|------|-------|
| 1 | `$SARTHI_HOME/honcho.json` | Profile-local (isolated Sarthi instances) |
| 2 | `~/.sarthi/honcho.json` | Default profile (shared host blocks) |
| 3 | `~/.honcho/config.json` | Global (cross-app interop) |

Host key is derived from the active Sarthi profile: `sarthi` (default) or `sarthi_<profile>`.
...
> **Deprecated:** `pinPeerName` is a legacy alias for `pinUserPeer`, still read for back-compat (`pinUserPeer` wins where both are set). `sarthi honcho setup` migrates it onto `pinUserPeer` on touch and never writes it.
...
| 4 | `per-session` strategy | Sarthi session ID (`20260415_a3f2b1`) |
| 5 | `per-repo` strategy | Git root directory name (`sarthi-agent`) |
| 6 | `per-directory` strategy | Current directory basename (`src`) |
| 7 | `global` strategy | Workspace name (`sarthi`) |

Gateway platforms always resolve via priority 3 (per-chat isolation) regardless of `sessionStrategy`. The strategy setting only affects CLI sessions.

If `sessionPeerPrefix` is `true`, the peer name is prepended: `alice-sarthi-agent`.

#### What each strategy produces

- **`per-directory`** — basename of `$PWD`. Opening sarthi in `~/code/myapp` and `~/code/other` gives two separate sessions. Same directory = same session across runs.
...
- **`per-session`** — Sarthi session ID (timestamp + hex). Every `sarthi` invocation starts a fresh Honcho session. Falls back to `per-directory` if no session ID is available.
- **`global`** — workspace name. One session for everything. Memory accumulates across all directories and runs.

### Multi-Profile Pattern

Multiple Sarthi profiles can share one workspace while maintaining separate AI identities. Config resolution is **host block > root > env var > default** — host blocks inherit from root, so shared settings only need to be declared once:
...
Both profiles see the same user (`yourname`) in the same shared environment (`sarthi`), but each AI peer builds its own observations, conclusions, and behavior patterns. The coder's memory stays code-oriented; the main agent's stays broad.

Host key is derived from the active Sarthi profile: `sarthi` (default) or `sarthi_<profile>` (e.g. `sarthi -p coder` -> host key `sarthi_coder`). Older `sarthi.<profile>` host blocks are still read for compatibility and are migrated when the CLI writes profile-scoped Honcho config.
...
| `SARTHI_HONCHO_HOST` | Host key override |
...
| `HONCHO_OAUTH_CLIENT_ID` | OAuth client (default `sarthi-agent`) |
...
|---------|-------------|
| `sarthi memory setup honcho` | Configure Honcho directly — works on a fresh install |
| `sarthi honcho setup` | Interactive setup wizard (only registered once Honcho is the active provider; redirects to `sarthi memory setup`) |
| `sarthi honcho status` | Show resolved config for active profile |
| `sarthi honcho enable` / `disable` | Toggle Honcho for active profile |
| `sarthi honcho mode <mode>` | Change recall or observation mode |
| `sarthi honcho peer --user <name>` | Update user peer name |
| `sarthi honcho peer --ai <name>` | Update AI peer name |
| `sarthi honcho tokens --context <N>` | Set context token budget |
| `sarthi honcho tokens --dialectic <N>` | Set dialectic max chars |
| `sarthi honcho map <name>` | Map current directory to a session name |
| `sarthi honcho sync` | Create host blocks for all Sarthi profiles |
