...
``sarthi://`` handler. Endpoints are env-overridable with local-dev defaults
...
# The loopback redirect registered for the Sarthi OAuth client. IP-literal so
...
_DEFAULT_CLIENT_ID = "sarthi-agent"
...
def begin_authorization(
    ...
    *,
    ...
    (``sarthi-desktop`` / ``sarthi-cli``) so the consent side can attribute
...
_CALLBACK_HTML = (
    ...
    b"<div>Connected to Honcho. You can close this tab and return to Sarthi.</div>"
)
...
def start_loopback_flow_background(
    *,
    ...
    # the request, where a context-local SARTHI_HOME override can't reach.
