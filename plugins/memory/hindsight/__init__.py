...
Config via environment variables:
  ...
  HINDSIGHT_BANK_ID                — memory bank identifier (default: sarthi)
  ...
  HINDSIGHT_RETAIN_SOURCE          — metadata source value attached to retained memories (default: sarthi)
  HINDSIGHT_RETAIN_USER_PREFIX     — label used before user turns in retained transcripts
  HINDSIGHT_RETAIN_ASSISTANT_PREFIX — label used before assistant turns in retained transcripts

Or via $SARTHI_HOME/hindsight/config.json (profile-scoped), falling back to
...
# via the ``retain_source`` config key or HINDSIGHT_RETAIN_SOURCE (e.g. "sarthi").
...
def _check_local_runtime() -> tuple[bool, str | None]:
    ...
    so Sarthi can degrade gracefully instead of repeatedly trying to start
    ...
    healthy and ``sarthi memory status`` would stay green while the daemon
...
def _local_runtime_hint(reason: str | None) -> str:
    ...
    selected local_embedded without going through ``sarthi memory setup`` — a
    ...
    if "no module named" in text and ("hindsight'" in text or 'hindsight"' in text
        ...
        return (
            ...
            f"{sys.executable} hindsight-all — or run 'sarthi memory setup'. "
...
def _load_config() -> dict:
    ...
    Resolution order:
      1. $SARTHI_HOME/hindsight/config.json  (profile-scoped)
    ...
    profile_path = get_sarthi_home() / "hindsight" / "config.json"
    ...
    return {
        ...
        "banks": {
            "sarthi": {
                "bankId": os.environ.get("HINDSIGHT_BANK_ID", "sarthi"),
...
def _embedded_profile_name(config: dict[str, Any]) -> str:
    """Return the Hindsight embedded profile name for this Sarthi config."""
    profile = config.get("profile", "sarthi")
    return str(profile or "sarthi")
...
def _load_simple_env(path) -> dict[str, str]:
    ...
    values: dict[str, str] = {}
    # utf-8-sig, not plain utf-8: this is also used on the Sarthi .env during
...
def _resolve_bank_id_template(template: str, fallback: str, **placeholders: str) -> str:
    ...
    Supported placeholders (each is sanitized before substitution):
      {profile}   — active Sarthi profile name (from agent_identity)
      {workspace} — Sarthi workspace name (from agent_workspace)
      {platform}  — "cli", "telegram", "discord", etc.
      {user}      — platform user id (gateway sessions)
      {session}   — current session id

    Missing/empty placeholders are rendered as the empty string and then
    collapsed — e.g. ``sarthi-{user}`` with no user becomes ``sarthi``.
...
class HindsightMemoryProvider(MemoryProvider):
    ...
    def __init__(self):
        ...
        self._retain_context = "conversation between Sarthi Agent and the User"
    ...
    def save_config(self, values, sarthi_home):
        """Write config to $SARTHI_HOME/hindsight/config.json."""
        import json
        from pathlib import Path
        config_dir = Path(sarthi_home) / "hindsight"
    ...
    def post_setup(self, sarthi_home: str, config: dict) -> None:
        ...
        from sarthi_cli.memory_setup import _CANCELLED, _curses_select, _print_cancelled_setup
        ...
        provider_config.setdefault("bank_id", "sarthi")
        ...
        self.save_config(provider_config, sarthi_home)
        ...
        if mode == "local_embedded":
            ...
            config_path = Path(sarthi_home) / "hindsight" / "config.json"
            try:
            ...
            if not llm_api_key:
                llm_api_key = _load_simple_env(Path(sarthi_home) / ".env").get("HINDSIGHT_LLM_API_KEY", "")
            if not llm_api_key:
    ...
    def _offer_starter_template(self, mode: str, provider_config: dict, env_writes: dict) -> None:
        """Offer to seed the bank with a Sarthi starter template (best-effort)."""
        from sarthi_cli.memory_setup import _CANCELLED, _curses_select
        ...
        bank_id = provider_config.get("bank_id", "sarthi")
    ...
    def get_config_schema(self):
        return [
            ...
            {"key": "bank_id", "description": "Memory bank name (static fallback when bank_id_template is unset)", "default": "sarthi"},
            {"key": "bank_id_template", "description": "Optional template to derive bank_id dynamically. Placeholders: {profile}, {workspace}, {platform}, {user}, {session}. Example: sarthi-{profile}", "default": ""},
            ...
            {"key": "retain_context", "description": "Context label for retained memories", "default": "conversation between Sarthi Agent and the User"},
        ...
        if self._client is None:
            if self._mode == "local_embedded":
                ...
                logger.debug("Creating HindsightEmbedded client (profile=%s, provider=%s)",
                             self._config.get("profile", "sarthi"), llm_provider)
                kwargs = dict(
                    profile=self._config.get("profile", "sarthi"),
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        try:
            ...
            if Version(installed) < Version(_MIN_CLIENT_VERSION):
                ...
                # durable data-volume target instead of /opt/sarthi (NS-605).
                from tools.lazy_deps import install_specs
        ...
        banks = cfg_get(self._config, "banks", "sarthi", default={})
        static_bank_id = self._config.get("bank_id") or banks.get("bankId", "sarthi")
        ...
        self._retain_context = self._config.get("retain_context", "conversation between Sarthi Agent and the User")
        ...
        # Sarthi emits a "👁️ Hindsight — recalled N memories" status line so the
        ...
        if self._mode == "local_embedded":
            ...
            if hasattr(os, "geteuid") and os.geteuid() == 0:
                msg = (
                    ...
                    "memory daemon. Run Sarthi as a non-root user, or switch "
                    "to cloud / local_external mode via 'sarthi memory setup'."
                )
                logger.warning(msg)
                # Surface to the terminal too — a daemon that never starts
                # would otherwise fail silently and the user would only see
                # Sarthi get sluggish. (issue #13125)
                try:
    ...
    def on_session_switch(
        ...
        **kwargs,
        ...
        document. See sarthi-agent#6672.
        ...
        if self._client is not None:
            try:
                if self._mode == "local_embedded":
                    ...
                    # When Sarthi created/used that client on the shared async loop,
