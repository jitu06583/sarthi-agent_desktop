"""sarthi-memory-store — holographic memory plugin using MemoryProvider interface.
...
Config in $SARTHI_HOME/config.yaml (profile-scoped):
  plugins:
    sarthi-memory-store:
      db_path: $SARTHI_HOME/memory_store.db   # omit to use the default
      auto_extract: false
      default_trust: 0.5
...
def _load_plugin_config() -> dict:
    try:
        ...
        return cfg_get(all_config, "plugins", "sarthi-memory-store", default={}) or {}
    except Exception:
        return {}
...
class HolographicMemoryProvider(MemoryProvider):
    ...
    def save_config(self, values, sarthi_home):
        """Write config to config.yaml under plugins.sarthi-memory-store."""
        from pathlib import Path
        config_path = Path(sarthi_home) / "config.yaml"
        try:
            import yaml
            ...
            existing["plugins"]["sarthi-memory-store"] = values
    ...
    def get_config_schema(self):
        ...
        _default_db = f"{display_sarthi_home()}/memory_store.db"
        return [
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        _sarthi_home = str(get_sarthi_home())
        _default_db = _sarthi_home + "/memory_store.db"
        db_path = self._config.get("db_path", _default_db)
        # Expand $SARTHI_HOME in user-supplied paths so config values like
        # "$SARTHI_HOME/memory_store.db" or "~/.sarthi/memory_store.db" both
        # resolve to the active profile's directory.
        if isinstance(db_path, str):
            db_path = db_path.replace("$SARTHI_HOME", _sarthi_home)
            db_path = db_path.replace("${SARTHI_HOME}", _sarthi_home)
