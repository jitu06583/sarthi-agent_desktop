# A2A — Agent-to-Agent protocol for Sarthi
...
peer (another Sarthi, LangChain, CrewAI, Google ADK, OpenClaw, …). Stdlib only —
no `a2a-sdk` dependency.

## Enable

```bash
sarthi gateway setup      # pick A2A, or:
```
...
When the `a2a` platform is enabled, Sarthi serves a v1.0 Agent Card at
...
- Every exchange is logged to `~/.sarthi/a2a_audit.jsonl`.
- Conversations persist to `~/.sarthi/a2a_conversations/` — they survive context
