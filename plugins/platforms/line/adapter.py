"""
LINE Messaging API platform adapter for Sarthi Agent.
...
to Sarthi Agent. It deliberately ports the *strongest* idea from each into
...
class LineAdapter(BasePlatformAdapter):
    ...
    async def _handle_media(self, request) -> Any:
        ...
        ``/private/tmp`` on macOS), and ``SARTHI_HOME``. PR #8398.
        """
        from aiohttp import web
        ...
        try:
            ...
            sarthi_home = Path(get_sarthi_home()).resolve()
        except Exception:
            sarthi_home = Path.home().joinpath(".sarthi").resolve()
...
def is_connected(config) -> bool:
    """Surface in ``sarthi status`` even before the adapter is instantiated."""
...
def _env_enablement() -> Optional[Dict[str, Any]]:
    ...
    Lets ``sarthi status`` reflect a LINE configuration that lives entirely
...
def interactive_setup() -> None:
    """Minimal stdin wizard for ``sarthi setup line``.

    Mirrors the irc/teams style: prompts for the two required vars, plus
    one optional public URL. Writes to ``~/.sarthi/.env`` via ``sarthi_cli.config``.
    """
    print()
    ...
    try:
        from sarthi_cli.config import get_env_value as _get_env, save_env_value as _set_env
    except ImportError:
        print("sarthi_cli.config not available; set LINE_* vars manually in ~/.sarthi/.env")
        return
