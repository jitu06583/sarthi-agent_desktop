"""
Email platform adapter for the Sarthi gateway.

Allows users to interact with Sarthi by sending emails.
...
def _send_imap_id(imap: "imaplib.IMAP4") -> None:
    ...
    try:
        ...
        imap.xatom(
            "ID",
            f'("name" "sarthi-agent" "version" "{_sarthi_version}" '
            '"vendor" "NousResearch" '
...
class EmailAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        # send-helper, and `sarthi config show` already read. Without the
    ...
    async def connect(self, *, is_reconnect: bool = False) -> bool:
        ...
        if missing:
            message = (
                ...
                + ", ".join(missing)
                + ". Set it via `sarthi gateway setup` (env) or platforms.email "
                "in config.yaml."
            )
    ...
    def _send_email(
        ...
        subject = ctx.get("subject", "Sarthi Agent")
        ...
        msg_id = f"<sarthi-{uuid.uuid4().hex[:12]}@{self._message_id_domain()}>"
...
# sarthi_cli/gateway.py, and the _send_email dispatch in
