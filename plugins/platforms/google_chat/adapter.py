...
# ``model_tools`` import time, so ``sarthi status``, ``sarthi chat``, etc.
...
# ``sarthi_plugins.platforms__google_chat.adapter`` once the plugin
...
#     fresh "Sarthi is thinking…" card during the small window between
...
def _mime_for_message_type(mime: str) -> MessageType:
    """Map a MIME string to a sarthi MessageType.
...
def card_spec_to_cards_v2(card_spec: Dict[str, Any]) -> Dict[str, Any]:
    ...
    return {"cardId": str(card_spec.get("card_id") or "sarthi-card"), "card": card}
...
class GoogleChatAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        # Persisted to ${SARTHI_HOME}/google_chat_thread_counts.json so
        ...
        try:
            ...
            _sarthi_home = _get_sarthi_home()
        except (ModuleNotFoundError, ImportError):
            _sarthi_home = _Path.home() / ".sarthi"
        self._thread_count_store = _ThreadCountStore(
            _sarthi_home / "google_chat_thread_counts.json"
        )
        ...
        # they don't sit in the chat forever as "Sarthi is thinking…".
    ...
    def _bot_id_cache_path(self) -> _Path:
        ...
        base = os.getenv("SARTHI_HOME", str(_Path.home() / ".sarthi"))
    ...
    async def _dispatch_message(self, msg: Dict[str, Any], envelope: Dict[str, Any]) -> None:
        ...
        try:
            ...
            if text.startswith("/setup-files") and event.source is not None:
                ...
                # ${SARTHI_HOME}/google_chat_user_tokens/<sanitized>.json
    ...
    async def _build_message_event(
        ...
        """Parse a Chat API message into a sarthi MessageEvent."""
        space = envelope.get("space") or msg.get("space") or {}
    ...
    async def send_typing(self, chat_id: str, metadata: Any = None) -> None:
        """Post a visible 'Sarthi is thinking…' marker message.
        ...
        "Sarthi is thinking…" stuck in chat forever, plus one card that
        ...
        body: Dict[str, Any] = {"text": "Sarthi is thinking…"}
        if thread_id:
            body["thread"] = {"name": thread_id}
    ...
    async def stop_typing(self, chat_id: str) -> None:
        ...
        Google Chat has no separate typing API: the "Sarthi is thinking…"
    ...
    async def on_processing_complete(
        ...
        see "Sarthi is thinking…" stuck forever.
        """
    ...
    async def _consume_typing_card_with_text(
        ...
        creating a fresh "Sarthi is thinking…" card during any subsequent
...
def interactive_setup() -> None:
    """Walk the user through Google Chat configuration via ``sarthi setup``.

    The setup wizard at ``sarthi_cli/gateway.py`` calls this for plugin
    ...
    prompt for env vars, persist them to ``~/.sarthi/.env`` so the next
    ...
    print_info("  3. Create a Pub/Sub topic (e.g. sarthi-chat-events) and a Pull subscription.")
    ...
    print()
    print_success("Google Chat configuration saved to ~/.sarthi/.env")
    print_info("Restart the gateway: sarthi gateway restart")
...
async def _standalone_send(
    ...
    *,
    ...
    runner is not in this process (e.g. ``sarthi cron`` running as a
    separate process from ``sarthi gateway``).  Without this hook,
...
def register(ctx) -> None:
    """Plugin entry point — called by the Sarthi plugin system at startup.
    ...
    ctx.register_platform(
        ...
        install_hint="pip install 'sarthi-agent[google_chat]'",
        ...
        # the "Sarthi is thinking..." marker patches and edit overhead.
        ...
        platform_hint=(
            ...
            "are generating a response, a 'Sarthi is thinking…' marker message "
