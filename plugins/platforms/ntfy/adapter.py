"""ntfy platform adapter (Sarthi plugin).
...
``plugins/platforms/ntfy/``. The Sarthi plugin loader scans the
    ...
    platforms:
      ntfy:
        ...
        extra:
          ...
          topic: "sarthi-in"              # subscribe topic (incoming)
          publish_topic: "sarthi-out"     # optional — defaults to topic
...
_ECHO_TAG = "sarthi-agent"  # tag added to outgoing messages for echo-loop prevention
...
async def _standalone_send(
    ...
    *,
    ...
    ``sarthi cron`` running standalone). Without this hook,
...
def register(ctx) -> None:
    ...
    ctx.register_platform(
        ...
        install_hint="pip install httpx   # already a Sarthi dependency",
        # Env-driven auto-configuration: seeds PlatformConfig.extra so
        # env-only setups show up in `sarthi gateway status` without
