"""SimpleX Chat platform adapter (Sarthi plugin).
...
This adapter ships as a Sarthi platform plugin under
``plugins/platforms/simplex/``. The Sarthi plugin loader scans the
...
Optional environment variables:
    ...
    SARTHI_SIMPLEX_TEXT_BATCH_DELAY
...
discoverable and ``sarthi setup`` can describe it even when websockets is
...
# Imported at module top because they're stdlib-only inside Sarthi — no
...
class SimplexAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig, **kwargs):
        ...
        self._text_batch_delay = float(
            os.getenv("SARTHI_SIMPLEX_TEXT_BATCH_DELAY", "0.8")
        )
...
def interactive_setup() -> None:
    """Minimal stdin wizard for ``sarthi setup gateway`` → SimpleX.

    Prompts for the WebSocket URL and the optional allowlist / groups /
    auto-accept / home channel. Writes to ``~/.sarthi/.env`` via
    ``sarthi_cli.config``.
    """
    print()
    ...
    try:
        ...
        print(
            "sarthi_cli.config not available; set SIMPLEX_* vars manually in "
            "~/.sarthi/.env"
        )
        return
