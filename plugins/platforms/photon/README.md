...
This plugin connects Sarthi Agent to iMessage (and other Spectrum
...
TypeScript-only, Sarthi runs it inside a small supervised Node sidecar and
...
There is no separate `login` command; like every other Sarthi channel,
...
Run `sarthi photon status` to see what's configured.

## Credentials

Runtime SDK credentials live in `~/.sarthi/.env` (the same place every other
...
Management metadata lives in `~/.sarthi/auth.json` under `credential_pool`:
...
| `PHOTON_TELEMETRY`        | false                      | Spectrum SDK telemetry — toggle with `sarthi photon telemetry on\|off` (restart the gateway to apply) |
...
- **Inbound attachments and voice notes are downloaded.** The sidecar reads
  ...
  something arrived. If Spectrum emits a `richlink` content object, Sarthi
  ...
  `.pluginPayloadAttachment` images immediately after the URL; Sarthi coalesces
...
- **Native polls are supported.** Sarthi posts poll content through
...
- **Cron/standalone sends require a running gateway.** Processes outside
  the gateway (cron subprocesses, `sarthi send`) cannot spawn the sidecar;
  they authenticate to the gateway's live sidecar via the runtime record at
  `<sarthi-home>/runtime/photon-sidecar.json` (written after the sidecar's
...
6. Verify end-to-end: `sarthi photon status`, a DM and a group roundtrip,
