...
Small Node helper that bridges Sarthi Agent to Photon's Spectrum SDK
(`spectrum-ts`).  Sarthi is Python; Photon has no public HTTP
...
- exposes a loopback-only HTTP control channel for the Python adapter
  to push send/typing requests (auth via `X-Sarthi-Sidecar-Token`)
- drains the inbound message stream so `spectrum-ts` keeps its
  reconnect/heartbeat machinery alive and Sarthi can receive inbound messages
...
The Sarthi plugin's `sarthi photon setup` command runs `npm install`
here automatically.
...
`Space.send(...)` API. Sarthi is Python, so replies go through this sidecar
