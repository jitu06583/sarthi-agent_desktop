// Sarthi Agent — Photon Spectrum sidecar
//
...
//
// Inbound  (gRPC -> Sarthi): the SDK's `app.messages` async iterator is a
...
//   gateway connects.
// Outbound (Sarthi -> gRPC): `/send` drives `space.send(...)`; `/typing`
//   sends the documented `typing("start" | "stop")` content builder.
//
// Protocol (all requests require `X-Sarthi-Sidecar-Token: ${TOKEN}`):
...
//                          default off — toggle with `sarthi photon telemetry`)
  ...
  streamRestartTimer = setTimeout(() => {
    ...
    console.error(
      `photon-sidecar: upstream stream degraded for ${degradedForMs}ms; ` +
        "exiting so Sarthi can restart the Photon adapter"
    );
    process.exit(75);
...
const PROBE_MSG_PREFIX = "sarthi-liveness-probe-";
...
// instead of a cryptic module-resolution error during import. Apply Sarthi'
...
function inboundStreamErrorMessage(e) {
  ...
  // iMessage internal error. Local Sarthi allowlists cannot cause or fix this:
  ...
  if (
    ...
    out +=
      ...
      "this is upstream of Sarthi, so inbound iMessages may not be delivered " +
...
const server = http.createServer(async (req, res) => {
  if (!tokenOk(req.headers["x-sarthi-sidecar-token"])) {
  ...
  try {
    ...
    if (req.url === "/send-attachment") {
      ...
      // overrides only when Sarthi supplied them so a known-good
