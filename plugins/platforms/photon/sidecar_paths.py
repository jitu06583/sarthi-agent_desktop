...
under an immutable ``/opt/sarthi`` (read-only for the sarthi user), which
...
4. Source dir read-only and deps missing or stale → mirror the sidecar
   source files to ``$SARTHI_HOME/photon/sidecar`` (the durable data volume,
...
def resolve_sidecar_dir(source_dir: Optional[Path] = None) -> Path:
    ...
    mirror = get_sarthi_home() / "photon" / "sidecar"
    try:
