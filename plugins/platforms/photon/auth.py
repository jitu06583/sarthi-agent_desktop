...
Credential storage mirrors every other Sarthi channel:

    * runtime SDK creds  -> ``~/.sarthi/.env``  (``PHOTON_PROJECT_ID`` =
      project id, ``PHOTON_PROJECT_SECRET``) via ``save_env_value``
    * management metadata -> ``~/.sarthi/auth.json`` under
...
try:
    import httpx
except ImportError:  # pragma: no cover - httpx is a sarthi dependency
...
# registers Sarthi as its own client_id.
...
# Default name of the project Sarthi provisions for the operator.
DEFAULT_PROJECT_NAME = "Sarthi Agent"
...
# ---------------------------------------------------------------------------
# auth.json helpers — share the file with the rest of sarthi-agent.

def _auth_json_path() -> Path:
    """Resolve ``~/.sarthi/auth.json`` honouring the active Sarthi profile."""
    try:
        from sarthi_constants import get_sarthi_home
        return Path(get_sarthi_home()) / "auth.json"
    except Exception:
        return Path(os.path.expanduser("~/.sarthi")) / "auth.json"
...
def _save_auth(data: Dict[str, Any]) -> None:
    ...
    # sarthi_cli/auth.py:_save_auth_store (#19673, #21148).
    fd = os.open(
        str(tmp),
...
def store_photon_token(token: str) -> None:
    ...
    from sarthi_cli.auth import _auth_store_lock
...
def load_project_credentials() -> Tuple[Optional[str], Optional[str]]:
    ...
    Precedence: process env (``~/.sarthi/.env`` is loaded into the gateway's
...
def store_project_credentials(
    *,
    ...
    The runtime SDK creds land in ``~/.sarthi/.env`` via the same
...
def _persist_runtime_env(spectrum_project_id: str, project_secret: str) -> None:
    """Write the SDK creds to ``~/.sarthi/.env`` (canonical runtime store).
    ...
    try:
        ...
        logger.warning("photon: sarthi_cli.config unavailable — skipping .env write")
        return
    try:
...
def print_credential_summary(emit: Any = print) -> None:
    ...
    labels["device_token"] = (
        ...
        else "✗ missing (run `sarthi photon setup`)"
    )
    ...
    labels["phone_number"] = (
        phone if phone else "✗ missing (run `sarthi photon setup --phone ...`)"
    )
    labels["assigned_phone_number"] = (
        assigned if assigned else "✗ missing (run `sarthi photon setup`)"
    )
...
def credential_summary() -> Dict[str, str]:
    ...
    def _present_phone() -> str:
        ...
        return phone or "✗ missing (run `sarthi photon setup --phone ...`)"

    def _present_assigned_phone() -> str:
        _phone, assigned = load_user_numbers()
        return assigned or "✗ missing (run `sarthi photon setup`)"
