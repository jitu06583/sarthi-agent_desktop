    ...
    try:
        ...
        if status is not None and 400 <= status < 500:
            raise ValueError(
                ...
                f"  • Pick a different model via `sarthi tools` → Video Generation."
            ) from exc
        raise


def _check_fal_video_available() -> bool:
    """True if the FAL video backend selected via `sarthi tools` (or, on a
        ...
        if not _check_fal_video_available():
            ...
            return error_response(
                error=(
                    ...
                    "(run `sarthi tools` → Video Generation → FAL to configure) "
                    "or sign in to Nous (`sarthi setup`) for managed gateway access."
                ),
                error_type="auth_required",
                provider="fal",
                prompt=prompt,
            ...
            if not endpoint:
                return error_response(
                    error=(
                        ...
                        f"via `sarthi tools` → Video Generation."
                    ),
