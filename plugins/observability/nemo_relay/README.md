...
Optional Sarthi observability plugin that maps Sarthi observer hooks to
...
not replace Sarthi Agent's planner, tools, memory, model provider routing, or
CLI UX. Instead, this plugin lets Sarthi emit NeMo Relay lifecycle events for
the work Sarthi already owns: sessions, turns, provider/API calls, tool calls,
approval prompts, and delegated subagents.

With this plugin enabled, Sarthi Agent can:

- Preserve Sarthi execution as NeMo Relay scopes, LLM spans, tool spans, and
  mark events.
...
```bash
sarthi plugins enable observability/nemo_relay
```

The `SARTHI_NEMO_RELAY_*` environment variables below only configure an
already-enabled plugin. They do not enable plugin discovery by themselves.

For isolated test homes, enable the plugin in the same `SARTHI_HOME` that the
agent run will use:

```bash
env SARTHI_HOME=/tmp/sarthi-nemo-relay-test \
  sarthi plugins enable observability/nemo_relay
```

Runs started with `--ignore_user_config` skip the enabled-plugin state from
`SARTHI_HOME`, so local E2E tests should omit that flag unless the test harness
loads `observability/nemo_relay` explicitly another way.

`SARTHI_HOME` is the Sarthi profile/config home used by both
`sarthi plugins enable ...` and the later `sarthi chat ...` run. If unset,
Sarthi uses the user's default home, usually `~/.sarthi`. For isolated smoke
tests, choose any writable temporary directory and use the same value for every
command in that test:

```bash
export SARTHI_HOME=/tmp/sarthi-nemo-relay-test
sarthi plugins enable observability/nemo_relay
sarthi chat --query 'Reply exactly ok' --provider custom --model qwen3.6:35b
```

For source checkouts, make sure the `sarthi` command you run is built from the
...
uv run sarthi plugins enable observability/nemo_relay
uv run sarthi chat --query 'Reply exactly ok' --provider custom --model qwen3.6:35b
```
...
```bash
uv build --wheel
python -m pip install --force-reinstall dist/sarthi_agent-*.whl
...
The plugin can configure exporters directly from `SARTHI_NEMO_RELAY_*`
...
```bash
export SARTHI_NEMO_RELAY_ATOF_ENABLED=1
export SARTHI_NEMO_RELAY_ATOF_OUTPUT_DIRECTORY=.nemo-relay/atof
export SARTHI_NEMO_RELAY_ATIF_ENABLED=1
export SARTHI_NEMO_RELAY_ATIF_OUTPUT_DIRECTORY=.nemo-relay/atif
```

Optional overrides:

- `SARTHI_NEMO_RELAY_ATOF_FILENAME`
- `SARTHI_NEMO_RELAY_ATOF_MODE` (`append` or `overwrite`)
- `SARTHI_NEMO_RELAY_ATIF_FILENAME_TEMPLATE`
- `SARTHI_NEMO_RELAY_ATIF_AGENT_NAME`
- `SARTHI_NEMO_RELAY_ATIF_AGENT_VERSION`
- `SARTHI_NEMO_RELAY_ATIF_MODEL_NAME`
- `SARTHI_NEMO_RELAY_ATIF_SUBAGENT_EXPORT_MODE` (`embedded` by default; set `all` to also write standalone child files)

### NeMo Relay Component Config

To initialize NeMo Relay from a component config, create a `plugins.toml` file
and point Sarthi at it:

```bash
export SARTHI_NEMO_RELAY_PLUGINS_TOML=.nemo-relay/plugins.toml
```
...
agent_name = "Sarthi Agent"
agent_version = "local"
```

When `SARTHI_NEMO_RELAY_PLUGINS_TOML` is set and initializes successfully, NeMo
Relay owns exporter lifecycle through that config. The direct
`SARTHI_NEMO_RELAY_ATOF_*` fallback setup is skipped. If the same
`plugins.toml` observability config enables `atif`, the direct
`SARTHI_NEMO_RELAY_ATIF_*` fallback setup is also skipped so Sarthi does not
double-export trajectories on teardown. If `plugins.toml` initialization fails,
Sarthi keeps the direct env-var fallbacks active for that run.
...
exposes `llm.execute(...)` / `tools.execute(...)`, Sarthi routes LLM and tool
...
Sarthi feature-detects the dynamic-plugin activation API available in NeMo Relay
0.6 and later. Configure native or worker plugins with Sarthi-owned
...
with this Sarthi-owned section. The gateway combines those records with
separate lifecycle state for enablement, trust policy, and worker environments;
the Python binding does not yet expose that resolver. Sarthi rejects
...
Sarthi activates these plugins before registering its managed LLM and tool
...
activation API, Sarthi logs an actionable warning and continues with the
...
For the full generic Sarthi middleware contract, see
...
export SARTHI_HOME=/tmp/sarthi-nemo-relay-docs/sarthi-home
mkdir -p "$SARTHI_HOME"

cat > "$SARTHI_HOME/config.yaml" <<'YAML'
model:
  provider: custom
  default: qwen3.6:35b
...
This run starts a parent Sarthi session, delegates to a child subagent, has the
child call `terminal`, and writes both ATOF and ATIF.

```bash
export SARTHI_NEMO_RELAY_ATOF_ENABLED=1
export SARTHI_NEMO_RELAY_ATOF_OUTPUT_DIRECTORY=/tmp/sarthi-nemo-relay-docs/subagent/atof
export SARTHI_NEMO_RELAY_ATOF_FILENAME=nested-subagent-atof.jsonl
export SARTHI_NEMO_RELAY_ATOF_MODE=overwrite
export SARTHI_NEMO_RELAY_ATIF_ENABLED=1
export SARTHI_NEMO_RELAY_ATIF_OUTPUT_DIRECTORY=/tmp/sarthi-nemo-relay-docs/subagent/atif
export SARTHI_NEMO_RELAY_ATIF_FILENAME_TEMPLATE='nested-subagent-atif-{session_id}.json'
export SARTHI_NEMO_RELAY_ATIF_AGENT_NAME='Sarthi Agent E2E'
export SARTHI_NEMO_RELAY_ATIF_AGENT_VERSION=docs-example
export SARTHI_NEMO_RELAY_ATIF_SUBAGENT_EXPORT_MODE=all
...
{"kind":"mark","name":"sarthi.subagent.start","metadata":{"parent_session_id":"docs-parent-session","session_id":"docs-child-session","subagent_id":"sa-0-docs","child_role":"leaf"}}
...
```json
{
  ...
  "session_id": "docs-parent-session",
  "agent": {"name": "Sarthi Agent E2E", "version": "docs-example", "model_name": "qwen3.6:35b"},
  "steps": [
    {
      "source": "agent",
...
message. Sarthi dispatches the read-only tools as one batch, and NeMo Relay
records both tool invocations.

```bash
mkdir -p /tmp/sarthi-nemo-relay-docs/workdir
printf 'docs_parallel_alpha_function\n' > /tmp/sarthi-nemo-relay-docs/workdir/alpha.txt
printf 'docs_parallel_beta_function\n' > /tmp/sarthi-nemo-relay-docs/workdir/beta.txt
cd /tmp/sarthi-nemo-relay-docs/workdir

export SARTHI_NEMO_RELAY_ATOF_ENABLED=1
export SARTHI_NEMO_RELAY_ATOF_OUTPUT_DIRECTORY=/tmp/sarthi-nemo-relay-docs/parallel/atof
export SARTHI_NEMO_RELAY_ATOF_FILENAME=parallel-tools-atof.jsonl
export SARTHI_NEMO_RELAY_ATOF_MODE=overwrite
export SARTHI_NEMO_RELAY_ATIF_ENABLED=1
export SARTHI_NEMO_RELAY_ATIF_OUTPUT_DIRECTORY=/tmp/sarthi-nemo-relay-docs/parallel/atif
export SARTHI_NEMO_RELAY_ATIF_FILENAME_TEMPLATE='parallel-tools-atif-{session_id}.json'
export SARTHI_NEMO_RELAY_ATIF_AGENT_NAME='Sarthi Agent E2E'
export SARTHI_NEMO_RELAY_ATIF_AGENT_VERSION=docs-example
...
- Sarthi sessions map to `agent` scopes.
- Sarthi API request hooks map to `llm` scope start/end events.
- Sarthi tool hooks map to `tool` scope start/end events.
...
The `observability/nemo_relay` plugin uses Sarthi execution middleware to hand
...
Enable it for Sarthi:

```bash
export SARTHI_NEMO_RELAY_PLUGINS_TOML=/tmp/sarthi-middleware-test/plugins.toml
```

When the adaptive component is enabled and the installed NeMo Relay runtime
exposes `llm.execute(...)` and `tools.execute(...)`, Sarthi routes execution
through these boundaries:

```text
Sarthi provider call
  -> llm_execution middleware
    -> nemo_relay.llm.execute(...)
      -> Sarthi provider adapter next_call(...)

Sarthi tool call
  -> tool_execution middleware
    -> nemo_relay.tools.execute(...)
      -> Sarthi tool dispatcher next_call(...)
```
...
middleware for a local Sarthi run. This path requires a NeMo Relay runtime that
supports `[components.config.tool_parallelism]`, as provided by the supported
0.x release range beginning with 0.5.

```bash
export SARTHI_HOME=/tmp/sarthi-middleware-test/sarthi-home
mkdir -p "$SARTHI_HOME" /tmp/sarthi-middleware-test/nemo-relay
...
cat > /tmp/sarthi-middleware-test/nemo-relay/plugins.toml <<'TOML'
version = 1
...
[components.config.atof]
enabled = true
output_directory = "/tmp/sarthi-middleware-test/atof"
filename = "middleware-events.jsonl"
mode = "overwrite"

[components.config.atif]
enabled = true
output_directory = "/tmp/sarthi-middleware-test/atif"
filename_template = "middleware-trajectory-{session_id}.json"
agent_name = "Sarthi Middleware E2E"
agent_version = "local"
...
export SARTHI_NEMO_RELAY_PLUGINS_TOML=/tmp/sarthi-middleware-test/nemo-relay/plugins.toml
