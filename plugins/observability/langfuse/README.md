...
Generation observations include the Sarthi system prompt when the provider
uses a separate `system` param (Anthropic Messages API). Open an **LLM call**
child span to inspect `role: system` (truncated via `SARTHI_LANGFUSE_MAX_CHARS`).
...
SARTHI_LANGFUSE_CAPTURE=sanitized    # content capture mode (see below)
SARTHI_LANGFUSE_DEBUG=true           # verbose plugin logging
```

## Capture modes

`SARTHI_LANGFUSE_CAPTURE` controls how much *content* (prompts, responses,
