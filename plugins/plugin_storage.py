...
``<sarthi home>/plugins/<name>/``. That tree is the plugin *install* dir —
``sarthi plugins remove`` deletes it and ``sarthi plugins update`` git-pulls
...
``<sarthi home>/plugin-data/<name>/``, owned by the user, untouched by
...
through ``agent.secret_scope`` / ``.env`` like everywhere else in Sarthi.
...
# Mirrors the plugin-name shape `sarthi plugins install` accepts. Anything
...
def plugin_data_dir(name: str) -> Path:
    ...
    ``<sarthi home>/plugin-data/<name>/`` — survives plugin update and
    removal, and follows the active profile because it resolves through
    :func:`sarthi_constants.get_sarthi_home` on every call. Don't cache the
    result across profile switches.
    """
    from sarthi_constants import get_sarthi_home

    root = get_sarthi_home() / "plugin-data" / _validate_name(name)
