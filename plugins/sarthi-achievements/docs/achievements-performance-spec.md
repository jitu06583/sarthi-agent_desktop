# Sarthi Achievements Performance Spec (Post-Hackathon)

Status: Draft (no code changes yet)
Owner: sarthi-achievements plugin
...
### `GET /api/plugins/sarthi-achievements/achievements`
...
### `POST /api/plugins/sarthi-achievements/rescan` (optional)
Manual refresh trigger.
Prefer async trigger + immediate status response.

### `GET /api/plugins/sarthi-achievements/scan-status` (optional new)
Reports scan state for UX/ops.

### Removed
- `GET /api/plugins/sarthi-achievements/overview`
...
- Persist plugin data under:
  - `~/.sarthi/plugins/sarthi-achievements/scan_snapshot.json`

### Phase 2
- Incremental scan checkpoints with per-session fingerprints.
- Persist checkpoint data under:
  - `~/.sarthi/plugins/sarthi-achievements/scan_checkpoint.json`
...
- `~/.sarthi/plugins/sarthi-achievements/`
