...
``{title, url, description, position}`` rows every other Sarthi web
provider produces.
...
prefers Sarthi-managed xAI Grok OAuth (via ``sarthi auth``) and falls back
to ``XAI_API_KEY`` (resolved through ``~/.sarthi/.env``, then
``os.environ``).
"""
...
class XAIWebSearchProvider(WebSearchProvider):
    ...
    def is_available(self) -> bool:
        ...
        every ``sarthi tools`` repaint and at tool-registration time.
    ...
    def search(self, query: str, limit: int = 5) -> Dict[str, Any]:
        ...
        if not api_key:
            return {
                "success": False,
                "error": (
                    "No xAI credentials found. Run `sarthi auth` to sign in with "
