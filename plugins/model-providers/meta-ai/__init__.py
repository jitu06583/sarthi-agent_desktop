"""Meta Model API (Muse Spark) provider plugin for Sarthi Agent.
...
Bundled from https://github.com/albertodepaola/sarthi-meta-provider. Sarthi'
...
* **Zero core edits.** Everything rides on ``ProviderProfile`` hooks. No changes
  to sarthi' ``model_metadata.py`` / ``models.py`` / ``run_agent.py`` are needed:
...
def _resolve_effort(reasoning_config: dict | None) -> str:
    """Map Sarthi' reasoning_config to a Meta-safe ``reasoning_effort`` value.
...
meta_ai = MetaAIProfile(
    ...
    # in sarthi_cli/providers.py and the retention hint in
