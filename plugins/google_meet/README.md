...
Let the sarthi agent join a Google Meet call, transcribe it, optionally speak
...
```
┌─ gateway (Linux box, where sarthi runs) ────────────────────────────┐
│                                                                      │
...
│                                                                      │
│   NodeServer (from `sarthi meet node run`)                           │
│     │                                                                │
...
| `__init__.py` | `register(ctx)` — registers 5 tools + `on_session_end` hook + `sarthi meet` CLI |
...
| `cli.py` | `sarthi meet setup / auth / join / status / transcript / say / stop / node ...` |
...
| `node/registry.py` | v3: `$SARTHI_HOME/workspace/meetings/nodes.json` |
| `node/server.py` | v3: `NodeServer` (runs on host machine) |
| `node/client.py` | v3: `NodeClient` (used by tool handlers + CLI on gateway) |
| `node/cli.py` | v3: `sarthi meet node {run,list,approve,remove,status,ping}` |
...
sarthi meet install                                      # pip + Chromium
sarthi meet setup                                        # preflight
sarthi meet auth                                         # optional
sarthi meet join https://meet.google.com/abc-defg-hij    # transcribe
```
...
```bash
sarthi meet install --realtime                     # installs pulseaudio-utils
echo 'OPENAI_API_KEY=sk-...' >> ~/.sarthi/.env
sarthi meet join https://meet.google.com/abc-defg-hij --mode realtime
# then from the agent or CLI:
sarthi meet say "Good morning everyone, I'm the note-taker bot."
```

macOS:
```bash
sarthi meet install --realtime     # runs: brew install blackhole-2ch ffmpeg
# then — manually! — open System Settings → Sound → Input → BlackHole 2ch
echo 'OPENAI_API_KEY=sk-...' >> ~/.sarthi/.env
sarthi meet join https://meet.google.com/abc-defg-hij --mode realtime
```

On macOS, sarthi will **not** switch your system audio input automatically — the
...
sarthi plugins enable google_meet
sarthi meet node run --display-name my-mac --host 0.0.0.0 --port 18789
