...
A standards-compliant OpenID Connect Relying Party for the ``sarthi dashboard``
...
:class:`~sarthi_cli.dashboard_auth.DashboardAuthProvider` methods and touches
...
``sarthi_cli/dashboard_auth/routes.py``; this provider only:
  ...
  4. verifies the **ID token** (RS256/ES256) against the discovered
     ...
     onto a :class:`~sarthi_cli.dashboard_auth.Session`.
    ...
    dashboard:
      oauth:
        ...
        self_hosted:
          ...
          # credential — prefer the env var / ~/.sarthi/.env over config.yaml.

    # Environment overrides (Docker/Fly secret injection)
    SARTHI_DASHBOARD_OIDC_ISSUER
    SARTHI_DASHBOARD_OIDC_CLIENT_ID
    SARTHI_DASHBOARD_OIDC_SCOPES        # optional; defaults to "openid profile email"
    SARTHI_DASHBOARD_OIDC_CLIENT_SECRET # optional; set for a confidential client
...
class SelfHostedOIDCProvider(DashboardAuthProvider):
    ...
    def start_login(self, *, redirect_uri: str) -> LoginStart:
        ...
        cookie_payload = {
            "sarthi_session_pkce": f"state={state};verifier={code_verifier}",
        }
    ...
    def _session_from_tokens(
        ...
        *,
        ...
        OAuth access token is intentionally NOT stored — Sarthi does not call
...
def register(ctx) -> None:
    ...
    client_id are configured (via ``SARTHI_DASHBOARD_OIDC_*`` env vars or the
    ...
    issuer = _resolve_setting(
        "SARTHI_DASHBOARD_OIDC_ISSUER", oidc_cfg.get("issuer")
    )
    client_id = _resolve_setting(
        "SARTHI_DASHBOARD_OIDC_CLIENT_ID", oidc_cfg.get("client_id")
    )
    scopes = (
        _resolve_setting("SARTHI_DASHBOARD_OIDC_SCOPES", oidc_cfg.get("scopes"))
        or _DEFAULT_SCOPES
    )
    # Optional — set only for a confidential client. A credential, so the
    # canonical home is the env var / ~/.sarthi/.env; config.yaml is supported
    # for precedence symmetry. Empty ⇒ public client (unchanged behaviour).
    client_secret = _resolve_setting(
        "SARTHI_DASHBOARD_OIDC_CLIENT_SECRET", oidc_cfg.get("client_secret")
    )
    ...
    if not issuer or not client_id:
        LAST_SKIP_REASON = (
            ...
            "(SARTHI_DASHBOARD_OIDC_ISSUER + SARTHI_DASHBOARD_OIDC_CLIENT_ID) "
