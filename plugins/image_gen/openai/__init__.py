...
three virtual model IDs so the ``sarthi tools`` model picker and the
    ...
    def generate(
        ...
        **kwargs: Any,
        ...
        if not api_key:
            return error_response(
                error=(
                    "OPENAI_API_KEY not set. Run `sarthi tools` → Image "
                    "Generation → OpenAI to configure, or `sarthi setup` "
                    "to add the key."
                ),
