...
Auto-tracks and cleans up ephemeral files created during Sarthi Agent
sessions — test scripts, temp outputs, cron logs, stale chrome profiles.
Scoped strictly to `$SARTHI_HOME` and `/tmp/sarthi-*`.
...
| Hook | Behaviour |
|---|---|
| `post_tool_call` | When `write_file` / `terminal` / `patch` creates a file matching `test_*`, `tmp_*`, or `*.test.*` inside `SARTHI_HOME`, track it silently as `test` / `temp` / `cron-output`. |
...
- `is_safe_path()` rejects anything outside `SARTHI_HOME` or `/tmp/sarthi-*`
- Windows mounts (`/mnt/c` etc.) are rejected
- The state directory `$SARTHI_HOME/disk-cleanup/` is itself excluded
- `$SARTHI_HOME/logs/`, `memories/`, `sessions/`, `skills/`, `plugins/`,
