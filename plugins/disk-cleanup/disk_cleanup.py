"""disk_cleanup — ephemeral file cleanup for Sarthi Agent.
...
Rules:
  ...
  - empty dirs    → always delete (under SARTHI_HOME)
  - research      → keep 10 newest, prompt for older (deep only)
  - chrome-profile→ prompt after 14 days (deep only)
  - >500 MB files → prompt always (deep only)

Scope: strictly SARTHI_HOME and /tmp/sarthi-*
Never touches: ~/.sarthi/logs/ or any system directory.
"""
...
try:
    ...
    def get_sarthi_home() -> Path:  # type: ignore[no-redef]
        val = (os.environ.get("SARTHI_HOME") or "").strip()
        return Path(val).resolve() if val else (Path.home() / ".sarthi").resolve()
...
def get_state_dir() -> Path:
    """State dir — separate from ``$SARTHI_HOME/logs/``."""
    return get_sarthi_home() / "disk-cleanup"
...
def get_log_file() -> Path:
    """Audit log — intentionally NOT under ``$SARTHI_HOME/logs/``."""
...
def is_safe_path(path: Path) -> bool:
    """Accept only paths under SARTHI_HOME or ``/tmp/sarthi-*``.

    Rejects Windows mounts (``/mnt/c`` etc.) and any system directory.
    """
    sarthi_home = get_sarthi_home()
    try:
        path.resolve().relative_to(sarthi_home)
        return True
    except (ValueError, OSError):
        pass
    # Allow /tmp/sarthi-* explicitly
    parts = path.parts
    if len(parts) >= 3 and parts[1] == "tmp" and parts[2].startswith("sarthi-"):
        return True
    return False
...
_EMPTY_DIR_PROTECTED_TOP_LEVEL = frozenset({
    ...
    "sarthi-agent", "backups", "profiles", ".worktrees",
})
...
# Paths under $SARTHI_HOME that must NEVER be deleted by quick(),
...
def _is_protected_cron_path(p: Path) -> bool:
    ...
    history at once.
    """
    # Lazily build the set once per process so SARTHI_HOME is resolved
    # exactly once.
    if not _PROTECTED_CRON_PATHS:
        sarthi_home = get_sarthi_home()
        for parent in ("cron", "cronjobs"):
            base = sarthi_home / parent
...
def track(path_str: str, category: str, silent: bool = False) -> bool:
    ...
    if not is_safe_path(path):
        _log(f"REJECT: {path} (outside SARTHI_HOME)")
        return False
...
def quick() -> Dict[str, Any]:
    ...
    # Remove empty dirs under SARTHI_HOME, but never recurse into known
    # durable state trees.  Some installs place the Sarthi checkout, venv,
    # and desktop build under SARTHI_HOME; a full rglob over that tree can
    # stall the gateway event loop for minutes.
    sarthi_home = get_sarthi_home()
    empty_removed = 0
    sweep_stack: List[Tuple[Path, bool]] = []
    try:
        for top in sarthi_home.iterdir():
            if (
                top.is_dir()
...
def guess_category(path: Path) -> Optional[str]:
    ...
    sarthi_home = get_sarthi_home()
    try:
        rel = path.resolve().relative_to(sarthi_home)
        top = rel.parts[0] if rel.parts else ""
        if top in {
            "disk-cleanup", "logs", "memories", "sessions", "config.yaml",
            "skills", "plugins", ".env", "USER.md", "MEMORY.md", "SOUL.md",
            "auth.json", "sarthi-agent",
        }:
            return None
        ...
        # Path isn't under SARTHI_HOME (e.g. /tmp/sarthi-*) — fall through.
        pass
