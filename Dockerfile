...
# node:26 image instead (Sarthi pins its toolchain to Node 26 everywhere).
...
# Do not write .pyc files at runtime: /opt/sarthi is immutable in the
...
ENV PLAYWRIGHT_BROWSERS_PATH=/opt/sarthi/.playwright

# Install system dependencies in one layer, clear APT cache.
# tini was previously PID 1 to reap orphaned zombie processes (MCP stdio
# subprocesses, git, bun, etc.) that would otherwise accumulate when sarthi
# ran as PID 1. See #15012. Phase 2 of the s6-overlay supervision plan
# replaces tini with s6-overlay's /init (PID 1 = s6-svscan), which reaps
# zombies non-blockingly on SIGCHLD and additionally supervises the main
# sarthi process, the dashboard, and per-profile gateways.
RUN apt-get -o Acquire::Retries=3 update && \
    apt-get -o Acquire::Retries=3 install -y --no-install-recommends \
...
db.execute(\"INSERT INTO docs VALUES ('sarthi')\"); \
...
# ---------- s6-overlay install ----------
# s6-overlay provides supervision for the main sarthi process, the dashboard,
...
# 'Sarthi WebUI' catalog, NAS compose projects that preserve an old
...
# Non-root user for runtime; UID can be overridden via SARTHI_UID at runtime
RUN useradd -u 10000 -m -d /opt/data sarthi
...
#
# ui-tui/packages/sarthi-ink/ is copied IN FULL (not just its manifests)
...
COPY ui-tui/packages/sarthi-ink/ ui-tui/packages/sarthi-ink/
# apps/shared/ is copied IN FULL because web/package.json references it as a
# `file:` workspace dependency (same pattern as sarthi-ink above).
...
# lock on the @sarthi/ink entry, tripped the TUI launcher's
...
#
# The [otlp] extra contains the SDK/exporter imported by Sarthi when Gateway
# Health export is enabled. Collector and observability-backend dependencies
# remain external and are not part of the Sarthi production image.
#
# The hindsight memory provider's client (hindsight-client) is baked in
# for the same reason: it lazy-installs into /opt/sarthi/.venv at first
...
# gives the non-root sarthi user read + traverse but no write; root retains
# write so the build steps below don't need chmod u+w dances.
COPY --link --chmod=a+rX,go-w . .

# ---------- Permissions ----------
# Link sarthi-agent itself (editable). Deps are already installed in the
# cached layer above; `--no-deps` makes this a fast egg-link creation with no
# resolution or downloads.
RUN uv pip install --no-cache-dir --no-deps -e "."

# Wire the exec shim and install-method stamp.  Files under /opt/sarthi are
# already root-owned (COPY, uv sync, npm install all run as root) and
# read-only for the sarthi user (go-w from the --chmod above).

USER root
RUN mkdir -p /opt/sarthi/bin && \
    cp /opt/sarthi/docker/sarthi-exec-shim.sh /opt/sarthi/bin/sarthi && \
    chmod 0755 /opt/sarthi/bin/sarthi && \
    printf 'docker\n' > /opt/sarthi/.install_method
# The ``.install_method`` stamp is baked next to the running code (the install
# tree), NOT into $SARTHI_HOME. $SARTHI_HOME (/opt/data) is a shared data
# volume that is commonly bind-mounted from the host and even shared with a
# host-side Desktop/CLI install; stamping it at boot used to clobber that
# host install's marker and wrongly block its ``sarthi update``. A code-scoped
# stamp is read first by detect_install_method() and is immune to the share.
# Start as root so the s6-overlay stage2 hook can usermod/groupmod and chown
# the data volume. Each supervised service then drops to the sarthi user via
# `s6-setuidgid sarthi` in its run script. If SARTHI_UID is unset, services
# run as the default sarthi user (UID 10000).

# ---------- Bake build-time git revision ----------
# .dockerignore excludes .git, so `git rev-parse HEAD` from inside the
# container always returns nothing — meaning `sarthi dump` reports
# "(unknown)" and the startup banner drops its `· upstream <sha>` suffix.
# That makes support triage from container bug reports impossible:
# we can't tell which commit the user is actually running.
#
# Fix: write the commit SHA passed via the SARTHI_GIT_SHA build-arg to
# /opt/sarthi/.sarthi_build_sha at build time, and have
# sarthi_cli/build_info.py read it at runtime.  Both `sarthi dump` and
...
ARG SARTHI_GIT_SHA=
RUN if [ -n "${SARTHI_GIT_SHA}" ]; then \
        printf '%s\n' "${SARTHI_GIT_SHA}" > /opt/sarthi/.sarthi_build_sha; \
    fi

# ---------- s6-overlay service wiring ----------
# Static services declared at build time: main-sarthi + dashboard.
...
# `exec sarthi`. Wired in as cont-init.d/01- so it
# runs before user services start.
#
# 02-reconcile-profiles re-creates per-profile gateway s6 service
# slots from $SARTHI_HOME/profiles/<name>/ after a container restart
# (the /run/service/ scandir is tmpfs and wiped on restart). Phase 4.
RUN mkdir -p /etc/cont-init.d && \
    printf '#!/command/with-contenv sh\nexec /opt/sarthi/docker/stage2-hook.sh\n' \
        > /etc/cont-init.d/01-sarthi-setup && \
    chmod +x /etc/cont-init.d/01-sarthi-setup
COPY --chmod=0755 docker/cont-init.d/015-supervise-perms /etc/cont-init.d/015-supervise-perms
COPY --chmod=0755 docker/cont-init.d/02-reconcile-profiles /etc/cont-init.d/02-reconcile-profiles

# ---------- Runtime ----------
ENV SARTHI_WEB_DIST=/opt/sarthi/sarthi_cli/web_dist
# Point the TUI launcher at the prebuilt bundle baked at build time (Layer 8:
# `ui-tui && npm run build`). This makes _make_tui_argv take the prebuilt-bundle
# fast path (`node --expose-gc /opt/sarthi/ui-tui/dist/entry.js`) and skip the
...
ENV SARTHI_TUI_DIR=/opt/sarthi/ui-tui
ENV SARTHI_HOME=/opt/data
ENV SARTHI_WRITE_SAFE_ROOT=/opt/data
ENV SARTHI_DISABLE_LAZY_INSTALLS=1
# The published image seals /opt/sarthi (root-owned, read-only) so a runtime
...
# is seeded + chowned to the sarthi user by docker/stage2-hook.sh and lives
...
ENV SARTHI_LAZY_INSTALL_TARGET=/opt/data/lazy-packages

# `docker exec` privilege-drop shim. When operators run
# `docker exec <c> sarthi ...` they default to root, and any file the
# command writes under $SARTHI_HOME (auth.json, .env, config.yaml) ends
# up root-owned and unreadable to the supervised gateway (UID 10000).
# The shim lives at /opt/sarthi/bin/sarthi, sits earliest on PATH, and
# transparently re-exec's the real venv binary via `s6-setuidgid sarthi`
# when invoked as root. Non-root callers (supervised processes,
# `--user sarthi`, etc.) hit the short-circuit path with no overhead.
# Recursion is impossible because the shim exec's the venv binary by
# absolute path (/opt/sarthi/.venv/bin/sarthi). See the shim source for
# the opt-out env var (SARTHI_DOCKER_EXEC_AS_ROOT=1).
COPY --chmod=0755 docker/sarthi-exec-shim.sh /opt/sarthi/bin/sarthi
COPY --chmod=0755 docker/entrypoint-dispatch.sh /opt/sarthi/docker/entrypoint-dispatch.sh

# Pre-s6 entrypoint.sh did `source .venv/bin/activate` which exported
# the venv bin onto PATH; Architecture B's main-wrapper.sh does the
# same for the container's main process, but `docker exec` and our
# cont-init.d scripts don't pass through the wrapper. Expose the venv
# bin globally so `docker exec <container> sarthi ...` and any
# subprocess that doesn't activate the venv first still find sarthi.
#
# /opt/sarthi/bin is prepended ahead of the venv so the privilege-drop
# shim wins PATH resolution. The shim's last act is to exec the venv
# binary by absolute path, so this PATH ordering is transparent to
# every other consumer.
ENV PATH="/opt/sarthi/bin:/opt/sarthi/.venv/bin:/opt/data/.local/bin:${PATH}"
...
#
# main-wrapper.sh handles arg routing (bare-exec vs. sarthi
# subcommand vs. no-args), drops to the sarthi user via s6-setuidgid,
...
ENTRYPOINT [ "/opt/sarthi/docker/entrypoint-dispatch.sh" ]
CMD [ ]
