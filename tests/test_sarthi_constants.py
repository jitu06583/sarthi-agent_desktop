"""Tests for sarthi_constants module."""
...
from sarthi_constants import (
    ...
    find_sarthi_node_executable,
    find_node_executable,
    find_node_executable_on_path,
    get_default_sarthi_root,
    get_sarthi_dir,
    get_sarthi_home,
    heal_sarthi_managed_node,
    ...
    secure_parent_dir,
    with_sarthi_node_path,
)


class TestGetDefaultSarthiRoot:
    """Tests for get_default_sarthi_root() — Docker/custom deployment awareness."""

    def test_no_sarthi_home_returns_native(self, tmp_path, monkeypatch):
        """When SARTHI_HOME is not set, returns ~/.sarthi."""
        monkeypatch.delenv("SARTHI_HOME", raising=False)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)

        assert get_default_sarthi_root() == tmp_path / ".sarthi"

    def test_sarthi_home_is_native(self, tmp_path, monkeypatch):
        """When SARTHI_HOME = ~/.sarthi, returns ~/.sarthi."""
        native = tmp_path / ".sarthi"
        native.mkdir()
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("SARTHI_HOME", str(native))
        assert get_default_sarthi_root() == native

    def test_sarthi_home_is_profile(self, tmp_path, monkeypatch):
        """When SARTHI_HOME is a profile under ~/.sarthi, returns ~/.sarthi."""
        native = tmp_path / ".sarthi"
    ...
    def test_sarthi_home_is_docker(self, tmp_path, monkeypatch):
        """When SARTHI_HOME points outside ~/.sarthi (Docker), returns SARTHI_HOME."""
        docker_home = tmp_path / "opt" / "data"
        docker_home.mkdir(parents=True)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("SARTHI_HOME", str(docker_home))
        assert get_default_sarthi_root() == docker_home

    def test_sarthi_home_is_custom_path(self, tmp_path, monkeypatch):
        """Any SARTHI_HOME outside ~/.sarthi is treated as the root."""
        custom = tmp_path / "my-sarthi-data"
        custom.mkdir()
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("SARTHI_HOME", str(custom))
        assert get_default_sarthi_root() == custom

    def test_docker_profile_active(self, tmp_path, monkeypatch):
        """When a Docker profile is active (SARTHI_HOME=<root>/profiles/<name>),
        ...
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("SARTHI_HOME", str(profile))
        assert get_default_sarthi_root() == docker_root

    def test_no_sarthi_home_returns_localappdata_root_on_windows(self, tmp_path, monkeypatch):
        """Native Windows falls back to %LOCALAPPDATA%\\sarthi, not ~/.sarthi."""
        local_appdata = tmp_path / "LocalAppData"
        monkeypatch.delenv("SARTHI_HOME", raising=False)
        monkeypatch.setenv("LOCALAPPDATA", str(local_appdata))
        monkeypatch.setattr(Path, "home", lambda: tmp_path / "Home")
        monkeypatch.setattr(sarthi_constants.sys, "platform", "win32")

        assert get_default_sarthi_root() == local_appdata / "sarthi"

    def test_no_sarthi_home_uses_windows_path_when_localappdata_missing(self, tmp_path, monkeypatch):
        """Windows fallback still uses AppData/Local/sarthi without LOCALAPPDATA."""
        home = tmp_path / "Home"
        monkeypatch.delenv("SARTHI_HOME", raising=False)
        ...
        assert get_default_sarthi_root() == home / "AppData" / "Local" / "sarthi"


class TestGetSarthiHome:
    """Tests for get_sarthi_home() platform-aware fallback."""

    def test_windows_fallback_uses_localappdata(self, tmp_path, monkeypatch):
        """When SARTHI_HOME is unset on Windows, use %LOCALAPPDATA%\\sarthi."""
        local_appdata = tmp_path / "LocalAppData"
        monkeypatch.delenv("SARTHI_HOME", raising=False)
        monkeypatch.setenv("LOCALAPPDATA", str(local_appdata))
        monkeypatch.setattr(Path, "home", lambda: tmp_path / "Home")
        monkeypatch.setattr(sarthi_constants.sys, "platform", "win32")
        monkeypatch.setattr(sarthi_constants, "_profile_fallback_warned", False)

        assert get_sarthi_home() == local_appdata / "sarthi"


class TestSarthiManagedNode:
    def test_windows_node_dir_prefers_portable_root(self, tmp_path, monkeypatch):
        ...
        assert iter_sarthi_node_dirs() == [node_dir, bin_dir]
    ...
    def test_windows_finds_npm_cmd_before_path(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(sarthi_constants.sys, "platform", "win32")
        monkeypatch.setenv("SARTHI_HOME", str(home))
        monkeypatch.setattr(sarthi_constants, "node_tool_runnable", lambda path: True)

        assert find_sarthi_node_executable("npm") == str(npm_cmd)
    ...
    def test_windows_skips_broken_managed_npm_without_path_fallback(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("SARTHI_HOME", str(home))
        monkeypatch.setenv("PATH", str(bin_dir))
        monkeypatch.setattr(sarthi_constants, "_managed_node_heal_attempted", False)
        monkeypatch.setattr(sarthi_constants, "heal_sarthi_managed_node", lambda: False)
        monkeypatch.setattr(
            sarthi_constants,
            "node_tool_runnable",
            lambda path: False,
        ...
        assert sarthi_managed_node_tree_present() is True
        assert find_node_executable("npm") is None
        assert find_node_executable("npm") != str(path_npm)

    def test_with_sarthi_node_path_prepends_existing_managed_dirs(self, tmp_path, monkeypatch):
        home = tmp_path / "sarthi"
        node_dir = home / "node"
        bin_dir = node_dir / "bin"
        node_dir.mkdir(parents=True)
        ...
        env = with_sarthi_node_path({"PATH": "system-node"})
...
class TestNodeToolRunnable:
    """node_tool_runnable() rejects broken Sarthi-managed npm/node wrappers."""
    ...
    def test_broken_managed_npm_heals_when_node_still_runs(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(sarthi_constants, "heal_sarthi_managed_node", _heal)
    ...
    def test_broken_managed_npm_heals_instead_of_path_fallback(self, tmp_path, monkeypatch):
        ...
        assert find_sarthi_node_executable("npm") == str(healed_npm)
...
class TestIsContainer:
    ...
    def _reset_cache(self, monkeypatch):
        ...
        monkeypatch.setattr(sarthi_constants, "_container_detected", None)
    ...
    def test_caches_result(self, monkeypatch):
        ...
        monkeypatch.setattr(sarthi_constants, "_container_detected", True)
        assert is_container() is True
...
class TestResolvePerModelReasoningEffort:
    ...
    def test_exact_match(self):
        ...
        from sarthi_constants import resolve_per_model_reasoning_effort
...
class TestReasoningOverridesDefaultConfig:
    ...
    def test_load_config_preserves_user_reasoning_overrides(self, tmp_path, monkeypatch):
        ...
        import yaml
        from sarthi_cli.config import load_config, get_config_path
...
class TestSecureParentDir:
    ...
    def test_safe_path_calls_chmod(self, tmp_path, monkeypatch):
        ...
        safe_dir = tmp_path / "home" / "user" / ".sarthi"
...
class TestAgentBrowserRunnable:
    ...
    def test_version_probe_uses_windows_hide_flags(self, tmp_path, monkeypatch):
        ...
        import sarthi_cli._subprocess_compat as subprocess_compat
...
class TestGetSarthiDir:
    """Tests for ``get_sarthi_dir(new_subpath, old_name)``.
    ...
    def test_neither_exists_returns_new(self, tmp_path, monkeypatch):
        ...
        result = get_sarthi_dir("platforms/pairing", "pairing")
    ...
    def test_legacy_populated_returns_legacy(self, tmp_path, monkeypatch):
        ...
        result = get_sarthi_dir("cache/images", "image_cache")
    ...
    def test_legacy_populated_with_subdir_returns_legacy(self, tmp_path, monkeypatch):
        ...
        result = get_sarthi_dir("platforms/matrix/store", "matrix/store")
        assert result == legacy
    ...
    def test_legacy_empty_and_new_missing_returns_new(self, tmp_path, monkeypatch):
        ...
        legacy.mkdir()
        result = get_sarthi_dir("cache/audio", "audio_cache")
...
class TestWslPathTranslation:
    ...
    def test_windows_drive_to_wsl_mount(self):
        assert sarthi_constants.windows_path_to_wsl(r"C:\Users\alex") == "/mnt/c/Users/alex"
        assert sarthi_constants.windows_path_to_wsl("C:/Users/alex") == "/mnt/c/Users/alex"
        assert sarthi_constants.windows_path_to_wsl("D:\\") == "/mnt/d/"

    def test_windows_drive_ignores_non_drive_paths(self):
        assert sarthi_constants.windows_path_to_wsl("/home/alex") is None
        assert sarthi_constants.windows_path_to_wsl("relative\\dir") is None

    def test_wsl_unc_to_posix_both_spellings(self):
        assert sarthi_constants.wsl_unc_path_to_posix(r"\\wsl.localhost\Ubuntu\home\alex") == "/home/alex"
        assert sarthi_constants.wsl_unc_path_to_posix(r"\\wsl$\Ubuntu\home\alex") == "/home/alex"
        # Forward-slash spelling and distro root.
        assert sarthi_constants.wsl_unc_path_to_posix("//wsl.localhost/Debian/srv/app") == "/srv/app"
        assert sarthi_constants.wsl_unc_path_to_posix("\\\\wsl.localhost\\Ubuntu\\") == "/"

    def test_wsl_unc_ignores_non_unc_paths(self):
        assert sarthi_constants.wsl_unc_path_to_posix(r"C:\Users\alex") is None
        assert sarthi_constants.wsl_unc_path_to_posix("/home/alex") is None

    def test_translate_is_noop_off_wsl(self, monkeypatch):
        monkeypatch.setattr(sarthi_constants, "is_wsl", lambda: False)
        assert sarthi_constants.translate_cwd_for_wsl_backend(r"C:\Users\alex") == r"C:\Users\alex"

    def test_translate_maps_windows_and_unc_on_wsl(self, monkeypatch):
        monkeypatch.setattr(sarthi_constants, "is_wsl", lambda: True)
        assert sarthi_constants.translate_cwd_for_wsl_backend(r"C:\Users\alex") == "/mnt/c/Users/alex"
        assert sarthi_constants.translate_cwd_for_wsl_backend(r"\\wsl.localhost\Ubuntu\home\alex") == "/home/alex"
        # Already-POSIX paths pass through untouched.
        assert sarthi_constants.translate_cwd_for_wsl_backend("/home/alex") == "/home/alex"
