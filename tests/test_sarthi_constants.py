...
class TestGetDefaultSarthiRoot:
    ...
    def test_result_memoised_until_env_or_home_changes(self, tmp_path, monkeypatch):
        """Repeated calls reuse the memo; SARTHI_HOME / home changes invalidate.

        get_default_sarthi_root() resolves SARTHI_HOME against the native
        ...
        (native home, SARTHI_HOME) compared for free each call.
        """
        # SARTHI_HOME set to a Docker-profile path: every call resolves the
        ...
        # _get_platform_default_sarthi_home() on every call (even hits), so
        ...
        monkeypatch.setattr(
            sarthi_constants, "_default_sarthi_root_memo", None, raising=False
        )

        first = get_default_sarthi_root()
        ...
        # SARTHI_HOME change invalidates the memo (fresh resolution).
        other_profile = docker_root / "profiles" / "writer"
        other_profile.mkdir(parents=True)
        monkeypatch.setenv("SARTHI_HOME", str(other_profile))
        before = resolve_calls["n"]
        assert get_default_sarthi_root() == docker_root
        assert resolve_calls["n"] > before, (
            "SARTHI_HOME change must force a fresh resolution"
        )
...
class TestGetProcessSarthiHome:
    """Tests for get_process_sarthi_home() — process launch scope.
    ...
    def test_env_set_returns_that_path(self, tmp_path, monkeypatch):
        ...
        assert get_process_sarthi_home() == home
...
class TestNodeToolRunnable:
    ...
    def test_outdated_managed_node_heals_to_target_major(self, tmp_path, monkeypatch):
        ...
        target = sarthi_constants._SARTHI_NODE_TARGET_MAJOR
        profile_home = tmp_path / "profiles" / "assistant"
        managed_bin = profile_home / "node" / "bin"
        managed_bin.mkdir(parents=True)
        old_node = self._stub(
        ...
        resolved = sarthi_constants.find_sarthi_node_executable("node")
        assert heal_called["value"] is True
    ...
    def test_outdated_managed_node_survives_failed_heal(self, tmp_path, monkeypatch):
        ...
        assert sarthi_constants.find_sarthi_node_executable("node") == str(old_node)
    ...
    def test_target_major_managed_node_does_not_heal(self, tmp_path, monkeypatch):
        ...
        assert sarthi_constants.find_sarthi_node_executable("node") == str(node)
...
class TestManagedNodeTreeInUse:
    ...
    def test_always_false_off_windows(self, tmp_path, monkeypatch):
        monkeypatch.setattr(sarthi_constants.sys, "platform", "darwin")
        monkeypatch.setenv("SARTHI_HOME", str(tmp_path))
        assert sarthi_constants.managed_node_tree_in_use() is False
    ...
    def test_exe_under_node_dir_counts(self, tmp_path, monkeypatch):
        ...
        assert sarthi_constants.managed_node_tree_in_use() is True
...
class TestWindowsHealStageSwap:
    ...
    def _stub_env(
        ...
        monkeypatch.setenv("SARTHI_HOME", str(home))
        monkeypatch.setenv(
            "SARTHI_NODE_TARGET_MAJOR",
            str(sarthi_constants._SARTHI_NODE_TARGET_MAJOR),
        )
        monkeypatch.setattr(sarthi_constants, "_managed_node_heal_attempted", False)
        monkeypatch.setattr(
            sarthi_constants, "_managed_node_in_use_notice_printed", False
        )
        monkeypatch.setattr(
            sarthi_constants,
        ...
        monkeypatch.setattr(
            sarthi_constants, "node_tool_runnable", lambda path: True
        )
    ...
    def test_in_use_defers_without_touching_tree(self, tmp_path, monkeypatch):
        ...
        zip_name, zip_bytes = _make_node_zip(sarthi_constants._SARTHI_NODE_TARGET_MAJOR)
        ...
        result = sarthi_constants._heal_managed_node_windows()
...
class TestHealAttemptFlagSemantics:
    ...
    def test_deferral_keeps_flag_clear_and_retries(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(sarthi_constants, "_heal_managed_node_windows", fake_heal)

        assert heal_sarthi_managed_node() is False
        assert sarthi_constants._managed_node_heal_attempted is False
    ...
    def test_real_failure_records_attempt(self, tmp_path, monkeypatch):
        ...
        assert heal_sarthi_managed_node() is False
        assert sarthi_constants._managed_node_heal_attempted is True
