...
from sarthi_cli.plugins import PluginManager, VALID_HOOKS


def _make_enabled_plugin(sarthi_home: Path, name: str, register_body: str) -> Path:
    """Create a plugin under <sarthi_home>/plugins/<name> and opt it in."""
    plugin_dir = sarthi_home / "plugins" / name
...
def test_no_plugins_returns_empty_results(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("SARTHI_HOME", str(tmp_path / "sarthi_empty"))
