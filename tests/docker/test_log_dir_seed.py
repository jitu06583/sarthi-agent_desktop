"""Runtime smoke test for Docker $SARTHI_HOME/logs/gateways seeding.

Build the real image and verify logs/ and logs/gateways/ exist and are
owned by the sarthi user after container boot.
...
registered later runs its log service as the dropped sarthi user and
...
def test_logs_gateways_seeded_and_sarthi_owned(
    built_image: str, container_name: str,
) -> None:
    """logs/ and logs/gateways/ must exist and be owned by sarthi after boot."""
    ...
    assert "logs=sarthi" in r.stdout, (
        f"logs/ not owned by sarthi: {r.stdout}"
    )
    assert "gateways=sarthi" in r.stdout, (
        f"logs/gateways/ not owned by sarthi: {r.stdout}"
    )
