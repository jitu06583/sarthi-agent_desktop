...
Override the image with ``SARTHI_TEST_IMAGE`` env var to point at a pre-built
...
IMAGE_TAG = os.environ.get("SARTHI_TEST_IMAGE", "sarthi-agent-harness:latest")
...
def built_image() -> str:
    ...
    Override with ``SARTHI_TEST_IMAGE`` env var to point at a pre-built
    image (faster local iteration).
    """
    if os.environ.get("SARTHI_TEST_IMAGE"):
        return IMAGE_TAG
...
# ---------------------------------------------------------------------------
# docker_exec — default to the unprivileged sarthi user
# ---------------------------------------------------------------------------
#
# Background: every Sarthi runtime path inside the container drops to UID
# 10000 (the ``sarthi`` user) via ``s6-setuidgid sarthi``. ``docker exec``
...
# fails (PermissionError swallowed) for sarthi, so a test that ran as root
...
# helpers below run every probe under ``-u sarthi`` unless a specific
...
def docker_exec(
    ...
    *args: str,
    ...
    """Run a command inside ``container`` as ``user`` (default: sarthi).
