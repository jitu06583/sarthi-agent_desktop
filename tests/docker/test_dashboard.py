"""Harness: dashboard opt-in via SARTHI_DASHBOARD.

Today (tini): dashboard starts once when SARTHI_DASHBOARD=1; if it crashes
...
Every ``docker exec`` here runs as the unprivileged ``sarthi`` user
...
def test_dashboard_not_running_by_default(
    ...
    """Without SARTHI_DASHBOARD, no dashboard process should be running."""
    start_container(built_image, container_name, cmd="sleep 60")
    r = docker_exec(container_name, "pgrep", "-f", "sarthi dashboard")
    # pgrep exits non-zero when no match found
    assert r.returncode != 0, (
        "Dashboard should not be running without SARTHI_DASHBOARD"
    )
...
def test_dashboard_slot_reports_down_when_disabled(
    ...
    """Without SARTHI_DASHBOARD, s6-svstat should report the dashboard
    slot as DOWN (not up-with-sleep-infinity, which would
    false-positive `sarthi doctor` and any other health check).
    ...
    SARTHI_DASHBOARD is unset, so the slot reflects reality.
    """
    start_container(built_image, container_name, cmd="sleep 60")
    ...
    assert "down" in r.stdout, (
        f"Dashboard slot should be 'down' without SARTHI_DASHBOARD; "
...
def test_dashboard_slot_reports_up_when_enabled(
    ...
    """Symmetry: with SARTHI_DASHBOARD=1, s6-svstat reports the slot as up."""
    # The default dashboard host is 0.0.0.0, which now engages the
    # OAuth auth gate. Without a provider registered (no
    # SARTHI_DASHBOARD_OAUTH_CLIENT_ID in this test env), start_server
    ...
    start_container(
        ...
        "SARTHI_DASHBOARD_BASIC_AUTH_PASSWORD=test-dashboard-pw",
        cmd="sleep 120",
    )
...
def test_dashboard_opt_in_starts(
    ...
    """With SARTHI_DASHBOARD=1, a dashboard process should be visible."""
    ...
    ok, _ = poll_container(
        container_name, "pgrep -f 'sarthi dashboard'", deadline_s=30.0,
    )
    assert ok, "Dashboard should be running with SARTHI_DASHBOARD=1"


def test_dashboard_port_override(
    built_image: str, container_name: str,
) -> None:
    """SARTHI_DASHBOARD_PORT changes the dashboard's listen port."""
...
def test_dashboard_restarts_after_crash(
    ...
    for _attempt in range(10):
        first_pid_result = docker_exec(
            container_name, "pgrep", "-f", "sarthi dashboard",
        )
    ...
    # Kill the dashboard. The dashboard process runs as sarthi, so the
    # sarthi user can kill it (same UID).
...
# whenever `SARTHI_DASHBOARD_HOST` was non-loopback, silently disabling
...
def test_dashboard_oauth_gate_engages_on_non_loopback_bind(
    ...
    provider activates whenever ``SARTHI_DASHBOARD_OAUTH_CLIENT_ID`` is
    set).
    ...
    start_container(
        built_image, container_name,
        "SARTHI_DASHBOARD=1",
        "SARTHI_DASHBOARD_HOST=0.0.0.0",
        "SARTHI_DASHBOARD_OAUTH_CLIENT_ID=agent:test-instance",
        cmd="sleep 120",
    )
    ...
    assert "nous" in provider_names, (
        ...
        f"SARTHI_DASHBOARD_OAUTH_CLIENT_ID is set. Got: {payload!r}"
    )
    ...
    assert status_code == 401, (
        ...
        "when a provider is registered and SARTHI_DASHBOARD_INSECURE "
...
def test_dashboard_insecure_env_var_no_longer_bypasses_gate(
    ...
    """``SARTHI_DASHBOARD_INSECURE=1`` NO LONGER disables the auth gate
