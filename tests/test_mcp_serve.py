"""
Tests for mcp_serve — Sarthi MCP server.
...
def _isolate_sarthi_home(tmp_path, monkeypatch):
    """Redirect SARTHI_HOME to a temp directory."""
...
def _create_test_db(db_path, session_id, messages):
    """Create a minimal SQLite DB mimicking sarthi_state schema."""
...
class TestEventBridgePollE2E:
    ...
    def test_poll_picks_up_new_conversation_on_db_change(
        ...
        # _poll_once reads <SARTHI_HOME>/state.db for its mtime gate; the autouse
        # fixture points SARTHI_HOME at tmp_path.
        db_path = tmp_path / "state.db"
