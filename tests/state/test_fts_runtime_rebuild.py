...
class TestRuntimeFtsRebuild:
    def test_foreign_holder_detection_includes_deleted_wal(
        ...
        monkeypatch.setattr(sarthi_state, "psutil", FakePsutil)
        monkeypatch.setattr(sarthi_state, "_IS_WINDOWS", False)
        monkeypatch.setattr(sarthi_state.os, "getpid", lambda: 111)
    ...
    def test_foreign_holder_detection_proc_readlink_deleted_wal(
        ...
        monkeypatch.setattr(sarthi_state.os, "listdir", _listdir)
        real_readlink = os.readlink
        def _readlink(path):
            path = path.replace("/proc", str(proc_root))
            return real_readlink(path)
        monkeypatch.setattr(sarthi_state.os, "readlink", _readlink)
    ...
    def test_foreign_holder_uninspectable_process_cmdline_fallback(
        ...
        flagged when /proc/<pid>/cmdline identifies it as a Sarthi process."""
        db_path = tmp_path / "state.db"
        ...
        # PID 222's cmdline is world-readable and looks like Sarthi
        cmdline_path = proc_root / "222" / "cmdline"
        cmdline_path.write_bytes(b"python3\x00sarthi_cli.main\x00chat\x00")
        ...
        def _fake_cmdline(pid):
            ...
            try:
                ...
                return None
        monkeypatch.setattr(sarthi_state, "_read_proc_cmdline", _fake_cmdline)

        holders = db._foreign_state_db_holders()
        # Should include PID 222 with the cmdline info
        assert len(holders) == 1
        assert holders[0][0] == 222
        assert "sarthi_cli.main" in holders[0][1]
