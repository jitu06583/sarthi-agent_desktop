...
Sarthi state stays profile-scoped through SARTHI_HOME. Host subprocesses should
...
class TestGetSubprocessHome:
    """Unit tests for sarthi_constants.get_subprocess_home()."""

    def _host_mode(self, monkeypatch):
        monkeypatch.setattr(sarthi_constants, "is_container", lambda: False)
    ...
    def test_host_auto_keeps_real_home_when_profile_home_exists(self, tmp_path, monkeypatch):
        ...
        sarthi_home = real_home / ".sarthi" / "profiles" / "coder"
        profile_home = sarthi_home / "home"
    ...
    def test_real_mode_repairs_parent_home_already_pointing_at_profile(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("SARTHI_REAL_HOME", str(real_home))

        from sarthi_constants import get_subprocess_home, get_real_home
    ...
    def test_two_profiles_get_different_homes(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("SARTHI_HOME", str(base / "alpha"))
        home_a = get_subprocess_home()

        monkeypatch.setenv("SARTHI_HOME", str(base / "beta"))
...
class TestMakeRunEnvHomeInjection:
    ...
    def test_host_auto_preserves_real_home_when_profile_home_exists(self, tmp_path, monkeypatch):
        ...
        assert result["SARTHI_REAL_HOME"] == str(real_home)
    ...
    def test_profile_mode_injects_profile_home_when_profile_home_exists(self, tmp_path, monkeypatch):
        ...
        assert result["HOME"] == str(sarthi_home / "home")
        assert result["SARTHI_REAL_HOME"] == str(real_home)
    ...
    def test_no_injection_when_sarthi_home_unset(self, monkeypatch):
        monkeypatch.delenv("SARTHI_HOME", raising=False)
...
class TestProfileBootstrap:
    ...
    def test_profile_dirs_includes_home(self):
        from sarthi_cli.profiles import _PROFILE_DIRS
