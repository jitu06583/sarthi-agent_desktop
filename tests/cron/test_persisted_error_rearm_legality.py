...
def cron_store(tmp_path, monkeypatch):
    ...
    (sarthi_home / "cron").mkdir(parents=True)
    monkeypatch.setenv("SARTHI_HOME", str(sarthi_home))
    monkeypatch.setattr(J, "SARTHI_DIR", sarthi_home)
    monkeypatch.setattr(J, "CRON_DIR", sarthi_home / "cron")
    monkeypatch.setattr(J, "JOBS_FILE", sarthi_home / "cron" / "jobs.json")
    monkeypatch.setattr(J, "OUTPUT_DIR", sarthi_home / "cron" / "output")
...
class TestCronRearmRespectsScheduleLegality:
    def test_weekday_job_errored_friday_does_not_fire_on_saturday(self, cron_store):
        ...
        with mock.patch.object(J, "_sarthi_now", lambda: now):
            due = J.get_due_jobs()
