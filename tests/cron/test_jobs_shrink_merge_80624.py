...
def test_stale_empty_save_preserves_concurrent_no_agent_create(sarthi_env):
...
def test_remove_other_job_preserves_concurrent_create(sarthi_env):
...
def test_intentional_remove_still_deletes(sarthi_env):
...
def test_replace_flag_allows_wholesale_rewrite(sarthi_env):
    from cron.jobs import create_job, load_jobs, save_jobs
...
def test_jobs_json_on_disk_matches_merge(sarthi_env):
    ...
    save_jobs([])
    payload = json.loads((Path(sarthi_env) / "cron" / "jobs.json").read_text())
    assert [j["id"] for j in payload["jobs"]] == [job["id"]]


def test_stamp_fast_path_skips_merge_when_file_unchanged(sarthi_env, monkeypatch):
...
def test_merge_does_not_mutate_caller_list(sarthi_env):
...
def test_corrupt_disk_file_does_not_break_save(sarthi_env):
...
def test_nested_create_survives_outer_stale_save(sarthi_env):
