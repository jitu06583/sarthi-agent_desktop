...
#
# Installs a real, earlier Sarthi the way a user does, applies ONE update route,
# and requires the checkout to land on this commit with a working `sarthi`.
#
...
# Keep sandbox state out of the default .sarthi-sandbox so a run never clobbers
...
SANDBOX_DIR_NAME=".sarthi-sandbox-e2e-$ROUTE"
export SARTHI_DEV_SANDBOX_DIR="$SANDBOX_DIR_NAME"

SANDBOX_ROOT="$REPO_ROOT/$SANDBOX_DIR_NAME"
INSTALL_DIR="/home/sarthi/.sarthi/sarthi-agent"   # user-level layout (sandbox default)
FAKE_REMOTE="/work/repos/sarthi-agent.git"
...
# both together.
UPSTREAM_URL="${SARTHI_DEV_SANDBOX_UPSTREAM:-https://github.com/NousResearch/hermes-agent.git}"
...
LOG_DIR="${SARTHI_E2E_LOG_DIR:-$(mktemp -d -t sarthi-install-e2e-logs.XXXXXX)}"
...
# ── helpers ────────────────────────────────────────────────────────────────
# Does the INSTALLED sarthi accept FLAG on `sarthi update`?
#
...
# that silently rots. `sarthi update --help` is the same surface a user meets,
# and argparse prints every option it accepts.
update_supports() {
  local flag="$1"
  in_sandbox "sarthi update --help 2>&1" | grep -qF -- "$flag"
}
...
require_sarthi_works() {
  local when="$1" out
  out="$(in_sandbox "sarthi --version" 2>&1)" \
    || { printf '%s\n' "$out" >&2; fail "sarthi --version failed $when"; }
  printf '%s\n' "$out" | sed 's/^/    /'
  ok "sarthi runs $when"
}

# ── install the earlier Sarthi ─────────────────────────────────────────────
...
require_sarthi_works 'after install'

# ── apply exactly one update route ─────────────────────────────────────────
case "$ROUTE" in
  update)
    step 'ROUTE: sarthi update'
    ...
    # sarthi which it accepts; older ones read the prompt from stdin, so close it.
    if update_supports --yes; then
      update_cmd="sarthi update --yes"
    else
      update_cmd="sarthi update </dev/null"
    fi
    if ! in_sandbox "cd $INSTALL_DIR && $update_cmd"; then
      collect_sandbox_logs update
      fail "sarthi update failed ($update_cmd)"
    fi
    require_landed_on_target 'sarthi update'
    require_sarthi_works 'after sarthi update'
    ;;
  installer)
    ...
    require_sarthi_works 'after installer re-run'
    ;;
esac
