...
1. ``load_sarthi_dotenv()`` auto-loads ``~/.sarthi/.op.env`` so the
...
def test_op_env_autoloads_bootstrap_token_in_cron_context(tmp_path, monkeypatch):
    ...
    env_loader.load_sarthi_dotenv(sarthi_home=home)
...
def _seed_openrouter_token(monkeypatch, dotenv_value, environ_value):
    ...
    monkeypatch.setattr(
        "sarthi_cli.auth.is_source_suppressed", lambda _p, _s: False
    )
