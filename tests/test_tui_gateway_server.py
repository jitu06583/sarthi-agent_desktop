...
def test_session_create_rejects_at_active_session_limit(monkeypatch, tmp_path):
    ...
    try:
        ...
        assert second["error"]["message"] == (
            "Sarthi is at the active session limit (1/1). "
...
def test_voice_toggle_tts_branch_also_carries_record_key(monkeypatch):
    ...
    monkeypatch.setenv("SARTHI_VOICE", "1")
    monkeypatch.delenv("SARTHI_VOICE_TTS", raising=False)
...
def test_session_compress_syncs_session_key_after_rotation(monkeypatch):
    ...
    continuation session — mirrors SarthiCLI._manual_compress's
...
def test_file_attach_uploads_remote_file_into_session_workspace(monkeypatch, tmp_path):
    ...
    try:
        ...
        stored = workspace / ".sarthi" / "desktop-attachments" / "report.txt"
        assert resp["result"]["attached"] is True
        assert resp["result"]["uploaded"] is True
        assert resp["result"]["path"] == str(stored)
        assert resp["result"]["ref_text"] == "@file:.sarthi/desktop-attachments/report.txt"
...
def test_file_attach_copies_gateway_visible_file_outside_workspace(monkeypatch, tmp_path):
    ...
    try:
        ...
        stored = workspace / ".sarthi" / "desktop-attachments" / "outside.txt"
        assert resp["result"]["attached"] is True
        assert resp["result"]["uploaded"] is True
        assert resp["result"]["ref_text"] == "@file:.sarthi/desktop-attachments/outside.txt"
...
def test_file_attach_quotes_ref_with_spaces(monkeypatch, tmp_path):
    ...
    try:
        ...
        assert resp["result"]["attached"] is True
        assert resp["result"]["ref_text"] == "@file:`.sarthi/desktop-attachments/my exam schedule.csv`"
    finally:
        server._sessions.pop("sid", None)
...
def test_session_info_includes_install_warning_for_pip(monkeypatch):
    ...
    monkeypatch.setattr("sarthi_cli.config.detect_install_method", lambda: "pip")
...
def test_session_info_omits_install_warning_for_git(monkeypatch):
    monkeypatch.setattr("sarthi_cli.config.detect_install_method", lambda: "git")
...
def test_browser_manage_connect_default_local_retries_after_launch(monkeypatch):
    ...
    with patch.dict(sys.modules, {"tools.browser_tool": fake}):
        with patch(
            "sarthi_cli.browser_connect.try_launch_chrome_debug", return_value=True
        ):
            resp = server.handle_request(
                {"id": "1", "method": "browser.manage", "params": {"action": "connect"}}
            )
...
def test_persist_model_switch_preserves_sibling_model_keys(tmp_path, monkeypatch):
    ...
    # save_config_value() resolves the config path from cli._sarthi_home, which
    # is captured at import time — patch it directly (set_sarthi_home_override
