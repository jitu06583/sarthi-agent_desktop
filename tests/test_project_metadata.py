...
def test_every_lazy_deps_exact_pin_matches_uv_lock():
    ...
    install ships the locked version, and the ``sarthi update`` lazy-refresh
    ...
    assert not drift, (
        ...
        "every package the core lock also ships — otherwise `sarthi update` "
...
def test_huggingface_hub_lazy_pin_matches_uv_lock():
    ...
    a feature from mere package presence, the `sarthi update` lazy-refresh
    ...
    assert lazy_pin == locked, (
        ...
        "huggingface-hub`), or `sarthi update` will churn/downgrade the "
