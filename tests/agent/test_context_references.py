...
@pytest.mark.asyncio
async def test_blocks_sensitive_home_and_sarthi_paths(tmp_path: Path, monkeypatch):
    from agent.context_references import preprocess_context_references_async

    monkeypatch.setenv("HOME", str(tmp_path))
    monkeypatch.setenv("SARTHI_HOME", str(tmp_path / ".sarthi"))

    sarthi_env = tmp_path / ".sarthi" / ".env"
    sarthi_env.parent.mkdir(parents=True)
    sarthi_env.write_text("API_KEY=super-secret\n", encoding="utf-8")
    ...
    result = await preprocess_context_references_async(
        "read @file:.sarthi/.env and @file:.ssh/id_rsa",
        cwd=tmp_path,
