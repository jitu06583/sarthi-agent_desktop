...
def sample_repo(tmp_path: Path) -> Path:
    ...
    repo.mkdir()
    _git(repo, "init")
    _git(repo, "config", "user.name", "Sarthi Tests")
...
def test_binary_reference_block_maps_host_attachment_to_container_path(tmp_path: Path, monkeypatch):
    ...
    attachments = sarthi_home / "attachments"
    ...
    assert result.expanded
    # Default container base for the docker backend is /root/.sarthi.
    assert "/root/.sarthi/attachments/archive.zip" in result.message
...
def test_binary_reference_block_keeps_host_path_on_local_backend(tmp_path: Path, monkeypatch):
    ...
    assert "/root/.sarthi/attachments/" not in result.message
...
async def test_blocks_canonical_read_denylist_credential_stores(tmp_path: Path, monkeypatch):
    ...
    a chat peer could otherwise attach `@file:~/.sarthi/auth.json` and read the
    ...
    oauth = sarthi_home / ".anthropic_oauth.json"
    oauth.write_text('{"access_token": "OAUTH-SECRET"}\n', encoding="utf-8")

    mcp_token = sarthi_home / "mcp-tokens" / "github.json"
    ...
    result = await preprocess_context_references_async(
        "inspect @file:.sarthi/auth.json and @file:.sarthi/.anthropic_oauth.json "
        "and @file:.sarthi/mcp-tokens/github.json and @file:project/.env",
        cwd=tmp_path,
...
@pytest.mark.parametrize(
    ...
    [
        "/tmp/plain.png",
        "/Users/me/Library/Application Support/Sarthi/composer-images/a.png",
