...
path via ``sarthi chat --query``) couldn't see what the aggregator actually
produced without joining to the session DB by hand.

These exercise the real ``consume_and_save_trace`` → ``save_moa_turn`` path
with real file I/O against a temp SARTHI_HOME — no mocks on the write path.
"""
...
def _enable_traces(tmp_path, monkeypatch):
    """Point SARTHI_HOME at a temp dir and turn moa.save_traces on."""
    sarthi_home = tmp_path / ".sarthi"
    sarthi_home.mkdir()
    monkeypatch.setenv("SARTHI_HOME", str(sarthi_home))

    # save_moa_turn reads config via sarthi_cli.config.load_config; stub it to
