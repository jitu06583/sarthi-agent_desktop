...
class TestEnvFileReadBlocking:
    ...
    def test_allowed_sarthi_env(self):
        """Sarthi' own .env inside SARTHI_HOME is NOT blocked by this rule
        (it's handled by other mechanisms). Only project-local .env is blocked."""
        # Note: sarthi internal .env is in ~/.sarthi/.env which is NOT a project-local
        # path, but the basename check applies to ANY .env. This is intentional —
        # even ~/.sarthi/.env should not be readable via read_file.
        error = get_read_block_error(os.path.expanduser("~/.sarthi/.env"))
        assert error is not None
...
class TestCacheFileReadBlocking:
    """Internal Sarthi cache files must remain blocked."""

    def test_hub_index_cache_blocked(self, tmp_path):
        """Hub index-cache reads are blocked."""
        sarthi_home = tmp_path / ".sarthi"
        cache = sarthi_home / "skills" / ".hub" / "index-cache" / "data.json"
        cache.parent.mkdir(parents=True)
        cache.write_text("{}")

        with patch("agent.file_safety._sarthi_home_path", return_value=sarthi_home):
            error = get_read_block_error(str(cache))
            assert error is not None
            assert "internal Sarthi cache" in error

    def test_hub_directory_blocked(self, tmp_path):
        """Hub directory reads are blocked."""
        sarthi_home = tmp_path / ".sarthi"
        hub = sarthi_home / "skills" / ".hub" / "metadata.json"
...
class TestCombinedGuards:
    ...
    def test_env_guard_works_regardless_of_sarthi_home(self, tmp_path):
        """The env basename guard does not depend on SARTHI_HOME resolution."""
        sarthi_home = tmp_path / ".sarthi"
        sarthi_home.mkdir()
    ...
    def test_cache_guard_still_works_with_env_guard(self, tmp_path):
        ...
        sarthi_home = tmp_path / ".sarthi"
        cache = sarthi_home / "skills" / ".hub" / "index-cache" / "x"
        cache.parent.mkdir(parents=True)
        cache.write_text("")
