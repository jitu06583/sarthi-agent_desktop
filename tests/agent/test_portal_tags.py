...
def test_sarthi_client_tag_includes_current_version():
    """The client tag must reflect sarthi_cli.__version__ verbatim."""
    from sarthi_cli import __version__
    from agent.portal_tags import sarthi_client_tag

    assert sarthi_client_tag() == f"client=sarthi-client-v{__version__}"


def test_sarthi_client_tag_format():
    """The client tag has the exact shape Nous Portal expects."""
    from agent.portal_tags import sarthi_client_tag

    tag = sarthi_client_tag()
    assert tag.startswith("client=sarthi-client-v")
...
def test_nous_portal_tags_contains_product_and_client():
    ...
    from agent.portal_tags import sarthi_client_tag, nous_portal_tags

    tags = nous_portal_tags()
    assert "product=sarthi-agent" in tags
    assert sarthi_client_tag() in tags
    assert len(tags) == 2
