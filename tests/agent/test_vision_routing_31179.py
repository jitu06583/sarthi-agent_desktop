...
def isolated_home(monkeypatch):
    """Temp SARTHI_HOME with config + clean credential env vars."""
    test_home = tempfile.mkdtemp(prefix="sarthi_test_31179_")
    sarthi_home = os.path.join(test_home, ".sarthi")
    os.makedirs(sarthi_home)
...
def _fresh_modules():
    """Drop cached sarthi modules so each test reloads against current env."""
