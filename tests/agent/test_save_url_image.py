...
def http_server(tmp_path, monkeypatch):
    """Spin up a localhost HTTP server and isolate SARTHI_HOME under tmp_path."""
    monkeypatch.setenv("SARTHI_HOME", str(tmp_path / ".sarthi"))
    (tmp_path / ".sarthi").mkdir()

    # Force the constants/image cache helpers to re-read SARTHI_HOME.
    import sys
    for mod in list(sys.modules):
        if mod.startswith("sarthi_constants") or mod.startswith("agent.image_gen_provider"):
...
class TestSaveUrlImage:
    def test_writes_real_bytes_to_sarthi_home_cache(self, http_server):
        base, _ = http_server
        ...
        # The cache directory must be under SARTHI_HOME — gateway cleanup
