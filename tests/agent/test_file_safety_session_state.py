...
manipulation. Sarthi keeps canonical conversation history in state.db and may
...
def fake_homes(tmp_path, monkeypatch):
    """Point SARTHI_HOME at a temp profile dir.

    Uses the real env-var resolution chain (get_sarthi_home /
    get_default_sarthi_root) instead of monkeypatching private helpers —
    ...
    SARTHI_HOME=<root>/profiles/<name> makes get_default_sarthi_root()
