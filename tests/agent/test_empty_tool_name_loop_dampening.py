...
@pytest.fixture()
def agent_env():
    """Spin up the mock provider + an isolated SARTHI_HOME, yield (agent, helpers)."""
    ...
    test_home = tempfile.mkdtemp(prefix="sarthi_e2e_47967_")
    os.makedirs(os.path.join(test_home, ".sarthi"))
    prev_home = os.environ.get("SARTHI_HOME")
    os.environ["SARTHI_HOME"] = os.path.join(test_home, ".sarthi")
    ...
    for mod in list(sys.modules):
        if mod == "run_agent" or mod.startswith("agent.") or mod.startswith("tools.") or mod.startswith("sarthi_"):
            del sys.modules[mod]
    ...
    try:
        ...
        if prev_home is None:
            ...
            os.environ["SARTHI_HOME"] = prev_home
