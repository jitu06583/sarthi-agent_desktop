...
class TestKimiEffortVocabulary:
    def test_k3_maps_full_sarthi_ladder(self):
        expected = {
            "minimal": "low",
            "low": "low",
            "medium": "high",
        ...
        for sarthi_level, wire_level in expected.items():
            kw = _kimi_kwargs(
                "kimi-k3", {"enabled": True, "effort": sarthi_level}
            )
            assert kw["reasoning_effort"] == wire_level, sarthi_level
