...
class TestSpawnEnvIsolation:
    ...
    def test_kanban_worker_adds_only_kanban_writable_root(self, monkeypatch):
        ...
        danger-full-access. Sarthi passes a narrow app-server config override
        ...
        monkeypatch.setattr(subprocess, "Popen", FakePopen)
        monkeypatch.setenv("HOME", "/users/alice")
        monkeypatch.setenv("SARTHI_HOME", "/users/alice/.sarthi/profiles/backend-worker")
        monkeypatch.setenv("SARTHI_KANBAN_TASK", "t_smoke")
        monkeypatch.setenv(
            "SARTHI_KANBAN_DB",
            "/users/alice/.sarthi/kanban/boards/smoke/kanban.db",
        )
        ...
        assert (
            'sandbox_workspace_write.writable_roots=["/users/alice/.sarthi/kanban/boards/smoke"]'
            in cmd
        )
...
class TestSpawnEnvSecretStripping:
    """codex app-server routes its spawn env through sarthi_subprocess_env(
    ...
    credentials to authenticate, but it must NOT inherit Tier-1 Sarthi secrets
    ...
    def test_tier1_and_internal_secrets_stripped_from_spawn_env(self, monkeypatch):
        ...
        for var in (
            ...
            "SARTHI_DASHBOARD_SESSION_TOKEN", "AUXILIARY_VISION_API_KEY",
    ...
    def test_home_still_preserved_through_helper(self, monkeypatch):
        """Regression guard: routing through sarthi_subprocess_env must not
