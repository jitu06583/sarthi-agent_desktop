...
* ``urllib``-based ``/models`` catalog discovery probe
  (``sarthi_cli.models.probe_api_models`` via ``_custom_provider_ssl_context``).
...
from sarthi_cli.models import _custom_provider_ssl_context
...
class TestResolveRequestsVerifyProviderScoped:
    ...
    def test_provider_ca_used_for_matching_base_url(self, clean_env, bundle_file):
        with patch(
            "sarthi_cli.config.get_compatible_custom_providers",
    ...
    def test_no_base_url_does_not_consult_config(self, clean_env, bundle_file):
        ...
        with patch("sarthi_cli.config.get_compatible_custom_providers", probe):
