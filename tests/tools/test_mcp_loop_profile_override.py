"""Regression tests for SARTHI_HOME override propagation onto the MCP loop.
...
get_sarthi_home() inside the coroutine, most visibly OAuth token-store
...
def test_oauth_token_paths_follow_override(tmp_path, monkeypatch, mcp_loop):
    """The actual symptom path: SarthiTokenStorage resolving inside the
    ...
    async def token_path():
        ...
        return str(SarthiTokenStorage("probe-srv")._tokens_path())
