...
was gated on ``[ -f "$SARTHI_HOME/.env" ]`` while the first-boot seed that
...
def _run_keygen(
    ...
    script = (
        ...
        f"{env_setup}"
        f'SARTHI_HOME="{home}"\n'
        # In tests we run unprivileged; as_sarthi is a passthrough then.
        'as_sarthi() { "$@"; }\n'
...
def test_keygen_skips_when_container_env_provides_key(
    ...
    Sarthi loads $SARTHI_HOME/.env with override=True, so a key generated
...
def test_dockerignore_keeps_env_example_template() -> None:
    """The first-boot seed copies /opt/sarthi/.env.example -> $SARTHI_HOME/.env.
    ...
    assert verdict == "included", (
        ...
        "seeds $SARTHI_HOME/.env from it on first boot"
    )
