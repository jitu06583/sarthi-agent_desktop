...
class TestScanMemoryContent:
    ...
    def test_sarthi_config_mod_blocked(self):
        result = _scan_memory_content("edit .sarthi/config.yaml to change settings")
        assert "Blocked" in result
        assert "sarthi_config_mod" in result
        result = _scan_memory_content("update .sarthi/SOUL.md with new personality")
        assert "Blocked" in result
        assert "sarthi_config_mod" in result
    ...
    def test_sarthi_config_mod_no_false_positives(self):
        """Merely mentioning sarthi config files should not trigger; only modify intent should."""
        assert _scan_memory_content("Check .sarthi/config.yaml for settings") is None
        assert _scan_memory_content("Read .sarthi/SOUL.md for agent personality") is None
        assert _scan_memory_content("The .sarthi/config.yaml file contains runtime options") is None
