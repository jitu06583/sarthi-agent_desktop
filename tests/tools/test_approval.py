...
class TestApprovalModeParsing:
    ...
    def test_config_bool_false_maps_to_off(self):
        with mock_patch("sarthi_cli.config.load_config_readonly", return_value={"approvals": {"mode": False}}):
...
class TestWindowsShellDestructiveCommands:
    def test_windows_destructive_requires_approval(self):
        cases = [
            (r"cmd /c del /f /q C:\tmp\sarthi-victim\file.txt", "Windows cmd destructive delete"),
            (r"cmd.exe /k rmdir /s /q C:\tmp\sarthi-victim", "Windows cmd destructive delete"),
...
class TestTeePattern:
    ...
    def test_tee_to_sensitive_target(self):
        for command in (
            ...
            "echo x | tee /dev/sda",
            "echo x | tee ~/.sarthi/.env",
            "echo x | tee $SARTHI_HOME/.env",
            'echo x | tee "$SARTHI_HOME/.env"',
        ):
            dangerous, key, desc = detect_dangerous_command(command)
            assert dangerous is True, command
...
class TestSarthiConfigWriteProtection:
    ...
    def test_write_idioms_against_config(self):
        for command in (
            "echo 'approvals:' > ~/.sarthi/config.yaml",
            "echo '  mode: off' >> ~/.sarthi/config.yaml",
            "echo x | tee ~/.sarthi/config.yaml",
            "echo x | tee $SARTHI_HOME/config.yaml",
            "cp /tmp/evil.yaml ~/.sarthi/config.yaml",
        ):
            dangerous, key, desc = detect_dangerous_command(command)
            assert dangerous is True, command
            assert key is not None, command
    ...
    def test_reads_and_unrelated_writes_are_safe(self):
        # Reading config is not a write; a non-Sarthi absolute config.yaml is
        # handled by the project patterns, not the Sarthi-home rule.
        for cmd in (
            "cat ~/.sarthi/config.yaml",
...
class TestSmartDeniedPrompt:
    ...
    def test_smart_deny_uses_locale_specific_once_deny_choices(self, monkeypatch, capsys):
        monkeypatch.setenv("SARTHI_LANGUAGE", "tr")
...
class TestGatewayProtection:
    ...
    def test_gateway_run_backgrounded_detected(self):
        ...
        for variant in (
            "python -m sarthi_cli.main gateway run --replace &",
            "nohup python -m sarthi_cli.main gateway run --replace",
        ):
...
class TestIFSWhitespaceBypass:
    ...
    def test_ifs_forms_still_flagged_dangerous(self):
        for cmd in (
            ...
            # In-place edit of the Sarthi security config via IFS.
            "sed${IFS}-i ~/.sarthi/config.yaml",
        ):
            dangerous, key, desc = detect_dangerous_command(cmd)
...
class TestPgrepKillExpansion:
    ...
    def test_kill_pidof_expansion_detected(self):
        ...
        dangerous, _, desc = detect_dangerous_command("kill -TERM $(pidof sarthi_cli.main)")
        assert dangerous is True
        assert "pidof" in desc.lower() or "pgrep" in desc.lower()
        assert detect_dangerous_command("kill -9 `pidof sarthi`")[0] is True
...
class TestLaunchctlGatewayLifecycle:
    ...
    def test_launchctl_against_sarthi_label_detected(self):
        for cmd in (
            "launchctl stop ai.sarthi.gateway",
            "launchctl kickstart -k system/ai.sarthi.gateway",
            "launchctl bootout system/ai.sarthi.gateway",
            "launchctl unload ~/Library/LaunchAgents/ai.sarthi.gateway.plist",
        ):
            dangerous, _, desc = detect_dangerous_command(cmd)
            assert dangerous is True, cmd

    def test_unrelated_labels_not_flagged(self):
        """Read-only inspection, and lifecycle ops on non-Sarthi labels, are
    ...
    * ``tirith_fail_open: false`` → surface a Tirith-style warning through
    ...
    def test_import_error_allows_when_fail_open_or_disabled(self, enabled, fail_open):
        ...
        with _patch("builtins.__import__", side_effect=self._make_failing_import(real_import)):
            with _patch("sarthi_cli.config.load_config_readonly", return_value=cfg):
