...
Covers the canonical fix for issues #4146, #27303, #30882, #33057:
  ...
  4. tools.code_execution_tool._scrub_child_env — broad SARTHI_ prefix dropped,
...
def gw_session(monkeypatch):
    """A clean gateway session: SARTHI_GATEWAY_SESSION set, a bound session
...
def test_env_scrub_sarthi_allowlist_and_secret_blocks():
    from tools.code_execution_tool import _scrub_child_env

    env = {
        # operational allowlist → kept
        "SARTHI_HOME": "/h", "SARTHI_PROFILE": "p",
        "SARTHI_CONFIG": "/c.yaml", "SARTHI_ENV": "/e",
        # other SARTHI_* → dropped (broad prefix removed)
        "SARTHI_BASE_URL": "https://x", "SARTHI_INTERACTIVE": "1",
        "SARTHI_KANBAN_DB": "postgres://u:p@h/db",
    ...
    for kept in ("SARTHI_HOME", "SARTHI_PROFILE", "SARTHI_CONFIG", "SARTHI_ENV", "PATH"):
        assert kept in out, f"{kept} should be kept"
    for dropped in (
        "SARTHI_BASE_URL", "SARTHI_INTERACTIVE", "SARTHI_KANBAN_DB",
...
def test_env_scrub_logs_dropped_sarthi_vars(caplog):
    """Dropping a non-allowlisted, non-secret SARTHI_* var must be diagnosable:
    ...
    leave users guessing why a sandbox script sees an unset SARTHI_* var."""
    import logging

    from tools.code_execution_tool import _scrub_child_env

    env = {
        "SARTHI_HOME": "/h",          # allowlisted → kept, not logged
        "SARTHI_BASE_URL": "https://x",   # dropped → logged
        "SARTHI_KANBAN_DB": "postgres://u:p@h/db",  # dropped → logged
        "SARTHI_API_KEY": "sk",       # secret → dropped silently (not logged)
        "PATH": "/usr/bin",           # safe prefix → kept
    }
    with caplog.at_level(logging.DEBUG, logger="tools.code_execution_tool"):
        out = _scrub_child_env(env, is_passthrough=lambda _: False, is_windows=False)

    assert "SARTHI_HOME" in out and "PATH" in out
    assert "SARTHI_BASE_URL" not in out and "SARTHI_KANBAN_DB" not in out

    msgs = "\n".join(r.getMessage() for r in caplog.records)
    assert "SARTHI_BASE_URL" in msgs and "SARTHI_KANBAN_DB" in msgs
    assert "env_passthrough" in msgs
    # Secret vars are dropped but must NOT be named in the diagnostic log.
    assert "SARTHI_API_KEY" not in msgs


def test_env_scrub_no_log_when_nothing_dropped(caplog):
    """No diagnostic noise when there are no dropped SARTHI_* vars."""
    import logging
