...
def _make_backend(session: _FakeSession):
    ...
    backend._session_id = "sarthi-session"
...
def test_backends_are_isolated_by_sarthi_session_and_reused_within_it():
...
def test_concurrent_sarthi_sessions_do_not_share_backend_state():
    from tools.computer_use import tool as computer_use
...
def _browser_route(driver: _BrowserDriver, session_id: str = "sarthi-a"):
...
def test_exact_browser_binding_injects_sarthi_session_capability():
    driver = _BrowserDriver()
    route = _browser_route(driver, session_id="sarthi-owned-session")
    ...
    assert driver.calls == [
        (
            "get_browser_state",
            {"pid": 101, "window_id": 202, "session": "sarthi-owned-session"},
        )
    ]
...
def test_typed_browser_refs_do_not_cross_route_sessions():
    driver = _BrowserDriver()
    first = _browser_route(driver, session_id="sarthi-a")
    second = _browser_route(driver, session_id="sarthi-b")
