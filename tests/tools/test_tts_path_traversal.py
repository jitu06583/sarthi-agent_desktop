...
def test_output_path_rejects_sarthi_oauth_store(tmp_path, monkeypatch):
    """TTS output_path must not bypass the shared protected-file write guard."""
    import agent.file_safety as file_safety

    sarthi_home = tmp_path / "sarthi-home"
    sarthi_home.mkdir()
    monkeypatch.setattr(file_safety, "_sarthi_home_path", lambda: sarthi_home)
    monkeypatch.setattr(file_safety, "_sarthi_root_path", lambda: sarthi_home)

    target = sarthi_home / ".anthropic_oauth.json"
    result = json.loads(text_to_speech_tool(
        text="hello",
...
def test_output_path_rejects_mcp_token_directory(tmp_path, monkeypatch):
    ...
    sarthi_home = tmp_path / "sarthi-home"
    token_dir = sarthi_home / "mcp-tokens"
