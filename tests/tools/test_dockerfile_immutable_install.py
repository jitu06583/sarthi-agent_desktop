"""Contract tests for the Docker image's immutable /opt/sarthi install tree."""
...
def test_dockerfile_makes_opt_sarthi_readonly_for_sarthi_user() -> None:
    ...
    assert "chown -R root:root /opt/sarthi" not in text
    assert "chmod -R a+rX /opt/sarthi" not in text
    assert "chmod -R a-w /opt/sarthi" not in text


def test_dockerfile_keeps_mutable_state_under_opt_data() -> None:
    text = _dockerfile_text()

    assert "ENV SARTHI_HOME=/opt/data" in text
    assert "ENV SARTHI_WRITE_SAFE_ROOT=/opt/data" in text
...
def test_dockerfile_disables_runtime_install_mutations() -> None:
    ...
    assert "ENV SARTHI_DISABLE_LAZY_INSTALLS=1" in text
    assert "SARTHI_TUI_DIR=/opt/sarthi/ui-tui" in text


def test_dockerfile_does_not_chown_install_trees_to_sarthi() -> None:
    text = _dockerfile_text()
    forbidden_patterns = (
        r"chown\s+-R\s+sarthi:sarthi\s+/opt/sarthi/\.venv",
        r"chown\s+-R\s+sarthi:sarthi\s+/opt/sarthi/ui-tui",
        r"chown\s+-R\s+sarthi:sarthi\s+/opt/sarthi/gateway",
        r"chown\s+-R\s+sarthi:sarthi\s+/opt/sarthi/node_modules",
    )
    for pattern in forbidden_patterns:
        assert not re.search(pattern, text), (
            "runtime install trees under /opt/sarthi must stay immutable; "
...
def test_dockerfile_bakes_code_scoped_install_method_stamp() -> None:
    ...
    (/opt/sarthi/.install_method) first; baking it at build time keeps the
    published image self-identifying as 'docker' WITHOUT writing into the
    shared $SARTHI_HOME data volume (which a host install may also use).
    The stamp is created by root in the shim-wiring RUN block; the sarthi
    user can't modify it (go-w from the --chmod on the source COPY).
    """
    text = _dockerfile_text()
    assert "printf 'docker\\n' > /opt/sarthi/.install_method" in text

    # The stamp must be in the RUN block that wires the exec shim.
    shim_block = re.search(
        r"RUN mkdir -p /opt/sarthi/bin && \\\n"
        r"(?:.*\\\n)+?"
...
def test_dockerfile_redirects_lazy_installs_to_durable_target() -> None:
    ...
    # never under the immutable /opt/sarthi tree.
    assert f"ENV SARTHI_LAZY_INSTALL_TARGET={target}" in text
    assert target.startswith("/opt/data/"), "target must be on the durable volume"
    assert "ENV SARTHI_LAZY_INSTALL_TARGET=/opt/sarthi" not in text
    ...
    # succeed as the unprivileged sarthi runtime user.
    stage2 = (REPO_ROOT / "docker" / "stage2-hook.sh").read_text()
    assert '"$SARTHI_HOME/lazy-packages"' in stage2, (
