...
def test_cannot_message_self(tmp_path):
    ...
    result = json.loads(
        bot_mode_dm.message_agent_tool(target="sarthi", message="hi", agent=agent)
    )
    assert "error" in result
...
def test_local_delivery_command_and_ack(tmp_path, monkeypatch):
    ...
    assert command.startswith("sarthi -p researcher chat --in ~ -c \"Bot Chat\"")
    ...
    assert content.startswith("Message from 🤖 sarthi (@sarthi): ")
...
def test_peer_delivery_command(tmp_path, monkeypatch):
    ...
    assert command.startswith("sarthi peer dm spark/researcher < ")
    ...
    assert calls[1]["command"].startswith("sarthi peer dm spark < ")


def test_named_profile_sender_prefix(tmp_path, monkeypatch):
    """A named-profile bot signs with its own handle, not @sarthi."""
    calls = _capture_spawn(monkeypatch)
    home = _managed_home(tmp_path, teammates=("researcher", "coder"))
