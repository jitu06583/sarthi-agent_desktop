...
def _restore_tool_modules():
    original_sarthi_home = os.environ.get("SARTHI_HOME")
    original_modules = {
        name: module
    ...
    try:
        ...
        if original_sarthi_home is None:
            os.environ.pop("SARTHI_HOME", None)
        else:
            os.environ["SARTHI_HOME"] = original_sarthi_home
        _reset_modules(("tools", "sarthi_cli", "modal"))
...
def _install_modal_test_modules(
    ...
    *,
    ...
    sarthi_cli = types.ModuleType("sarthi_cli")
    sarthi_cli.__path__ = []  # type: ignore[attr-defined]
    sys.modules["sarthi_cli"] = sarthi_cli
    sarthi_home = tmp_path / "sarthi-home"
    os.environ["SARTHI_HOME"] = str(sarthi_home)
    sys.modules["sarthi_cli.config"] = types.SimpleNamespace(
        get_sarthi_home=lambda: sarthi_home,
    )
    ...
    async def _lookup_aio(_name: str, create_if_missing: bool = False):
        return types.SimpleNamespace(name="sarthi-agent", create_if_missing=create_if_missing)
    ...
    return {
        "snapshot_store": sarthi_home / "modal_snapshots.json",
