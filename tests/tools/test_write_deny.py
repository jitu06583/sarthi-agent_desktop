...
class TestWriteDenyExactPaths:
    ...
    def test_sarthi_env(self):
        # ``.env`` under the active SARTHI_HOME (profile-aware, not just
        # ``~/.sarthi``) must be write-denied. The hermetic test conftest
        # points SARTHI_HOME at a tempdir — resolve via get_sarthi_home()
        # to match the denylist.
        from sarthi_constants import get_sarthi_home
        path = str(get_sarthi_home() / ".env")
        assert _is_write_denied(path) is True

    def test_sarthi_root_env_when_running_under_profile(self, tmp_path, monkeypatch):
        ...
        ``~/.sarthi/.env`` (whose credentials are inherited by every profile)
        could be silently overwritten by ``write_file`` while a profile was
        active.
        """
        root = tmp_path / "sarthi_root"
        ...
        # Sanity check: SARTHI_HOME does point to the profile dir, not the root.
        from sarthi_constants import get_sarthi_home, get_default_sarthi_root
...
class TestWriteAllowed:
    ...
    def test_sarthi_control_files_requested_writable(self):
        from sarthi_constants import get_sarthi_home
