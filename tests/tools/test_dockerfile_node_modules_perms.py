...
Older images made /opt/sarthi/ui-tui and /opt/sarthi/node_modules writable so a
...
def test_dockerfile_uses_prebuilt_tui_instead_of_writable_node_modules() -> None:
    ...
    assert "ENV SARTHI_TUI_DIR=/opt/sarthi/ui-tui" in text
    assert "cd ../ui-tui && npm run build" in text
    assert "chown -R sarthi:sarthi /opt/sarthi/ui-tui" not in text
    assert "chown -R sarthi:sarthi /opt/sarthi/node_modules" not in text
