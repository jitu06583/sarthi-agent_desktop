...
class TestEnsureInstalled:
    ...
    def test_not_found_returns_none(self, mock_cfg):
        ...
        with patch("tools.tirith_security.shutil.which", return_value=None), \
             patch("tools.tirith_security._sarthi_bin_dir", return_value="/nonexistent"), \
...
class TestInstallArchiveMemberValidation:
    ...
    def test_install_extracts_regular_tirith_member(self, mock_target, mock_which,
        ...
        assert reason == ""
        assert path == str(sarthi_home / "bin" / "tirith")
    ...
    def test_install_rejects_non_regular_tirith_member(self, mock_target, mock_which,
        ...
        assert not os.path.lexists(sarthi_home / "bin" / "tirith")
...
class TestDiskFailureMarker:
    ...
    def test_install_failed_recovers_from_sarthi_bin(self):
        """After _INSTALL_FAILED, manual install in SARTHI_HOME/bin is picked up."""
        from tools.tirith_security import _resolve_tirith_path, _INSTALL_FAILED
        import tempfile
        tmpdir = tempfile.mkdtemp()
        sarthi_bin = os.path.join(tmpdir, "tirith")
        # Create a fake executable
        with open(sarthi_bin, "w") as f:
            f.write("#!/bin/sh\n")
        os.chmod(sarthi_bin, 0o755)

        _tirith_mod._resolved_path = _INSTALL_FAILED

        with patch("tools.tirith_security.shutil.which", return_value=None), \
             patch("tools.tirith_security._sarthi_bin_dir", return_value=tmpdir), \
             patch("tools.tirith_security._clear_install_failed") as mock_clear:
            result = _resolve_tirith_path("tirith")
            assert result == sarthi_bin
            assert _tirith_mod._resolved_path == sarthi_bin
            mock_clear.assert_called_once()
...
# ---------------------------------------------------------------------------
# SARTHI_HOME isolation
# ---------------------------------------------------------------------------

class TestSarthiHomeIsolation:
    def test_sarthi_bin_dir_respects_sarthi_home(self):
        """_sarthi_bin_dir must use SARTHI_HOME, not hardcoded ~/.sarthi."""
        from tools.tirith_security import _sarthi_bin_dir
        import tempfile
        tmpdir = tempfile.mkdtemp()
        with patch.dict(os.environ, {"SARTHI_HOME": tmpdir}):
            result = _sarthi_bin_dir()
        assert result == os.path.join(tmpdir, "bin")
        assert os.path.isdir(result)

    def test_failure_marker_respects_sarthi_home(self):
        """_failure_marker_path must use SARTHI_HOME, not hardcoded ~/.sarthi."""
        from tools.tirith_security import _failure_marker_path
        with patch.dict(os.environ, {"SARTHI_HOME": "/custom/sarthi"}):
            result = _failure_marker_path()
        assert result == "/custom/sarthi/.tirith-install-failed"

    def test_conftest_isolation_prevents_real_home_writes(self):
        """The conftest autouse fixture sets SARTHI_HOME; verify it's active."""
        sarthi_home = os.getenv("SARTHI_HOME")
        assert sarthi_home is not None, "SARTHI_HOME should be set by conftest"
        assert "sarthi_test" in sarthi_home, "Should point to test temp dir"

    def test_get_sarthi_home_fallback(self):
        """Without SARTHI_HOME set, falls back to the active OS home."""
        from tools.tirith_security import _get_sarthi_home
        with patch.dict(os.environ, {}, clear=True):
            # Remove SARTHI_HOME entirely. With HOME also absent, expanduser
            ...
            expected = os.path.join(os.path.expanduser("~"), ".sarthi")
            result = _get_sarthi_home()
    ...
    def test_mkdtemp_oserror_propagates_to_ensure_installed(self):
        ...
        with patch("tools.tirith_security.tempfile.mkdtemp",
             ...
             patch("tools.tirith_security.shutil.which",
                   return_value=None), \
             patch("tools.tirith_security._sarthi_bin_dir",
