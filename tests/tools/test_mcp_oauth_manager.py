...
def test_sarthi_provider_subclass_exists():
    """SarthiMCPOAuthProvider is defined and subclasses OAuthClientProvider."""
    from tools.mcp_oauth_manager import _SARTHI_PROVIDER_CLS
    from mcp.client.auth.oauth2 import OAuthClientProvider

    assert _SARTHI_PROVIDER_CLS is not None
    assert issubclass(_SARTHI_PROVIDER_CLS, OAuthClientProvider)
...
def test_manager_builds_sarthi_provider_subclass(tmp_path, monkeypatch):
    """get_or_build_provider returns SarthiMCPOAuthProvider, not plain OAuthClientProvider."""
    from tools.mcp_oauth_manager import (
        MCPOAuthManager, _SARTHI_PROVIDER_CLS, reset_manager_for_tests,
    )
    reset_manager_for_tests()
    monkeypatch.setenv("SARTHI_HOME", str(tmp_path))
    _set_interactive_stdin(monkeypatch)
    ...
    assert _SARTHI_PROVIDER_CLS is not None
    assert isinstance(provider, _SARTHI_PROVIDER_CLS)
    assert provider._sarthi_server_name == "srv"
