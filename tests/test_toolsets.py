...
class TestToolsetConsistency:
    ...
    def test_sarthi_platforms_share_core_tools(self):
        """All sarthi-* platform toolsets share the same core tools.

        Platform-specific additions (e.g. ``discord`` / ``discord_admin``
        on sarthi-discord, gated on DISCORD_BOT_TOKEN) are allowed on top —
        the invariant is that the core set is identical across platforms.
        """
        platforms = ["sarthi-cli", "sarthi-telegram", "sarthi-discord", "sarthi-whatsapp", "sarthi-slack", "sarthi-signal", "sarthi-homeassistant"]
...
class TestDefaultPlatformWebSearchCoverage:
    def test_sarthi_whatsapp_toolset_includes_web_search(self):
        assert "web_search" in resolve_toolset("sarthi-whatsapp")
...
class TestResolveToolsetMemo:
    ...
    def test_second_resolution_is_cached(self, monkeypatch):
        ...
        first = resolve_toolset("sarthi-cli")
        second = resolve_toolset("sarthi-cli")
        ...
        assert (
            "sarthi-cli", True, registry_id, generation
    ...
    def test_memo_result_matches_fresh_resolution(self):
        ...
        toolsets_mod._resolve_toolset_memo.clear()
        first = resolve_toolset("sarthi-cli", include_registry=False)
        second = resolve_toolset("sarthi-cli", include_registry=False)
        assert first == second
