...
def progress(tmp_path):
    ...
    status = tmp_path / "sarthi-update-status"
...
def test_every_state_carries_a_clock(progress):
    ...
    for state, message in [
        ...
        ("done", ""),
        ("manual", "Reopen Sarthi to finish."),
        ("error", "Update failed."),
    ]:
...
# Stands in for `sarthi update`, and reports the stage that was on screen
...
n="$(cat "$SARTHI_TEST_CALLS" 2>/dev/null || echo 0)"; n=$((n + 1))
printf '%s' "$n" > "$SARTHI_TEST_CALLS"
for f in "$TMPDIR"/sarthi-update-status.[0-9]*; do
  case "$f" in *.tmp) continue ;; esac
  cp "$f" "$SARTHI_TEST_CAPTURE.$n" 2>/dev/null
done
exit "$(cat "$SARTHI_TEST_EXITS.$n" 2>/dev/null || echo 0)"
"""
...
def _run_handoff(tmp_path, exits: dict[int, int]) -> list[dict]:
    ...
    install_root = tmp_path / "sarthi-agent"
    (install_root / "venv" / "bin").mkdir(parents=True)
    sarthi = install_root / "venv" / "bin" / "sarthi"
    sarthi.write_text(FAKE_SARTHI)
    sarthi.chmod(0o755)
    ...
    env = {
        **os.environ,
        "TMPDIR": str(tmp_path),
        "SARTHI_TEST_CAPTURE": str(capture),
        "SARTHI_TEST_CALLS": str(calls),
        "SARTHI_TEST_EXITS": str(tmp_path / "exits"),
    }
    ...
    result = tmp_path / ".sarthi-update-result.json"
...
def test_update_gate_publishes_its_stage_before_running(tmp_path):
    """`sarthi update` is the longest wait in the hand-off and the one the
