...
The ``sarthi update`` Python path already clears the conflict with ``git reset``
...
def test_install_ps1_stops_venv_resident_processes_before_parking_venv() -> None:
    ...
    ``venv\\Scripts\\pythonw.exe -m sarthi_cli.main gateway run`` — image name
    ``pythonw``, not ``sarthi.exe`` — so the ``taskkill /IM sarthi.exe`` guard
    ...
    # The sarthi.exe tree-kill is preserved (kills spawned child processes too).
    assert 'taskkill /F /T /IM sarthi.exe' in text
