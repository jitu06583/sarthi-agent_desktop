...
def test_tui_gateway_fuzzy_file_listing_hides_git_windows(monkeypatch):
    from sarthi_cli import _subprocess_compat
...
def test_gateway_pid_scan_hides_wmic_and_powershell_windows(monkeypatch):
    from sarthi_cli import gateway
    from sarthi_cli import _subprocess_compat
    ...
    def fake_run(cmd, **kwargs):
        ...
        return _Completed(stdout="CommandLine=sarthi gateway\nProcessId=123\n")
...
def test_stale_dashboard_windows_scan_hides_wmic(monkeypatch):
    ...
    def fake_run(cmd, **kwargs):
        captured.append((cmd, kwargs))
        return _Completed(stdout="CommandLine=sarthi dashboard\nProcessId=123\n")
