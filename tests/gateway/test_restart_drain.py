...
def test_load_busy_input_mode_prefers_env_then_config_then_default(tmp_path, monkeypatch):
    monkeypatch.setattr(gateway_run, "_sarthi_home", tmp_path)
    monkeypatch.delenv("SARTHI_GATEWAY_BUSY_INPUT_MODE", raising=False)
    ...
    monkeypatch.setenv("SARTHI_GATEWAY_BUSY_INPUT_MODE", "interrupt")
    assert gateway_run.GatewayRunner._load_busy_input_mode() == "interrupt"

    monkeypatch.setenv("SARTHI_GATEWAY_BUSY_INPUT_MODE", "steer")
    assert gateway_run.GatewayRunner._load_busy_input_mode() == "steer"

    # Unknown values fall through to the safe default
    monkeypatch.setenv("SARTHI_GATEWAY_BUSY_INPUT_MODE", "bogus")
    assert gateway_run.GatewayRunner._load_busy_input_mode() == "interrupt"


def test_load_busy_text_mode_follows_input_mode_and_honors_legacy(tmp_path, monkeypatch):
    monkeypatch.setattr(gateway_run, "_sarthi_home", tmp_path)
    monkeypatch.delenv("SARTHI_GATEWAY_BUSY_TEXT_MODE", raising=False)
    monkeypatch.delenv("SARTHI_GATEWAY_BUSY_INPUT_MODE", raising=False)
    ...
    monkeypatch.setenv("SARTHI_GATEWAY_BUSY_TEXT_MODE", "queue")
    assert gateway_run.GatewayRunner._load_busy_text_mode() == "queue"

    # Bogus legacy value is ignored → falls through to busy_input_mode (interrupt).
    monkeypatch.setenv("SARTHI_GATEWAY_BUSY_TEXT_MODE", "bogus")
    assert gateway_run.GatewayRunner._load_busy_text_mode() == "interrupt"
...
def test_load_restart_drain_timeout_prefers_env_then_config_then_default(
    ...
    monkeypatch.setenv("SARTHI_RESTART_DRAIN_TIMEOUT", "7")
...
async def test_launch_detached_restart_command_uses_setsid(monkeypatch):
    ...
    monkeypatch.setattr(gateway_run, "_resolve_sarthi_bin", lambda: ["/usr/bin/sarthi"])
    ...
    # self-restart loop guard refuses to run `sarthi gateway restart`.
    assert kwargs["env"].get("_SARTHI_GATEWAY") is None
...
async def test_windows_detached_restart_scrubs_gateway_marker(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(gateway_run.sys, "platform", "win32")
    monkeypatch.setattr(gateway_run, "_resolve_sarthi_bin", lambda: ["sarthi"])
    monkeypatch.setattr(gateway_run.os, "getpid", lambda: 321)
    monkeypatch.setenv("_SARTHI_GATEWAY", "1")
    monkeypatch.setenv("VIRTUAL_ENV", str(venv_dir))
    ...
    assert len(popen_calls) == 1
    cmd, kwargs = popen_calls[0]
    assert cmd[-3:] == ["sarthi", "gateway", "restart"]
    assert kwargs["env"].get("_SARTHI_GATEWAY") is None
