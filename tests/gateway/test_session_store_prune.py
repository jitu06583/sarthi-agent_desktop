...
def test_session_store_default_db_uses_runtime_sarthi_home(tmp_path, monkeypatch):
    """SessionStore must honor runtime SARTHI_HOME when opening the default DB.

    Regression for the import-time DEFAULT_DB_PATH freeze: importing
    sarthi_state before a fixture redirected SARTHI_HOME used to pin every
    default SessionDB() at the developer's real ~/.sarthi/state.db.
    """
    config = GatewayConfig(default_reset_policy=SessionResetPolicy(mode="none"))
    fake_home = tmp_path / "alt_sarthi_home"
    fake_home.mkdir()
...
class TestReadmeSentinel:
    ...
    def test_save_writes_readme_sentinel_first(self, tmp_path):
        ...
        assert "sarthi sessions list" in raw["_README"]
