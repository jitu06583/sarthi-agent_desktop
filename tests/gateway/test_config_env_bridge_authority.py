...
def _run_gateway_import(sarthi_home: Path, initial_env: dict[str, str]) -> dict[str, str]:
    ...
    script = textwrap.dedent(
        ...
        for k in (
            ...
            "SARTHI_TURN_LEASE_TIMEOUT",
            "SARTHI_SESSION_STALL_TIMEOUT",
    ...
    # otherwise intentionally clean so stale Sarthi settings cannot leak in.
    for k in (
        "PATH",
        "PYTHONPATH",
        "VIRTUAL_ENV",
...
def test_config_gateway_timeout_wins_over_stale_env(sarthi_home: Path) -> None:
    ...
    _write_env(sarthi_home, {
        ...
        "SARTHI_SESSION_STALL_TIMEOUT": "15",
    })
    ...
    assert env.get("SARTHI_SESSION_STALL_TIMEOUT") == "300"


def test_config_turn_lease_timeout_wins_over_stale_env(sarthi_home: Path) -> None:
    ...
    _write_env(
        sarthi_home,
        {"SARTHI_TURN_LEASE_TIMEOUT": "60"},
    )

    env = _run_gateway_import(sarthi_home, initial_env={})

    assert env.get("SARTHI_TURN_LEASE_TIMEOUT") == "600"
...
def test_default_turn_lease_timeout_overrides_stale_env_when_key_is_omitted(
    ...
    assert env.get("SARTHI_TURN_LEASE_TIMEOUT") == "1800"
