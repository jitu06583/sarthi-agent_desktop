...
def test_get_flush_dir_uses_get_sarthi_home(tmp_path, monkeypatch):
    """Flush dir must use get_sarthi_home(), not hardcoded Path.home()."""
    import gateway.shutdown_flush as mod

    captured = {}

    def fake_get_sarthi_home():
        from pathlib import Path
        captured["called"] = True
        return tmp_path

    monkeypatch.setattr(
        "sarthi_constants.get_sarthi_home", fake_get_sarthi_home
    )
