...
class TestDMTopicFallbackReplyToMode:
    ...
    reply_to_mode 'off' was ignored when sending via Sarthi-created DM topic
...
class TestDMTopicSyntheticSendRouting:
    ...
    response in a different chat lane than the Sarthi topic the session runs
    in. The no-anchor branch must prefer the Sarthi topic's
    ``message_thread_id`` whenever it resolves.
    """

    def test_no_anchor_prefers_sarthi_topic_thread_id(self):
