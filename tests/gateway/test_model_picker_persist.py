...
closure the PR changed, against a real temp ``SARTHI_HOME``.
"""
...
def _fake_switch_result():
    ...
    from sarthi_cli.model_switch import ModelSwitchResult
...
def _stub_picker_dependencies(monkeypatch):
    ...
    monkeypatch.setattr(
        "sarthi_cli.model_switch.list_picker_providers",
    ...
    monkeypatch.setattr(
        "sarthi_cli.model_switch.resolve_display_context_length",
        lambda *a, **k: 272000,
    )
...
def _setup_isolated_home(tmp_path, monkeypatch, model_yaml_value):
    ...
    # save_config writes to ``get_sarthi_home() / config.yaml`` — point it here.
    monkeypatch.setattr("sarthi_constants.get_sarthi_home", lambda: sarthi_home)
    monkeypatch.setattr("sarthi_cli.config.get_sarthi_home", lambda: sarthi_home)
    return cfg_path
...
async def test_multiplex_picker_keeps_profile_adapter_and_callback_scope(
    ...
    monkeypatch.setattr("sarthi_cli.model_switch.switch_model", _profile_switch)
...
async def test_multiplex_picker_global_persists_only_named_profile(
    ...
    named_adapter = _FakePickerAdapter()
    runner = _make_named_runner(monkeypatch, default_adapter, named_adapter, named_home)
    monkeypatch.setattr(gateway_run, "_sarthi_home", default_home)
    _stub_picker_dependencies(monkeypatch)
