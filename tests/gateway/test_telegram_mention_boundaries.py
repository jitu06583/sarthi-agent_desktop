...
substrings like 'foo@sarthi_bot.example'.
...
def _make_adapter():
    ...
    adapter._bot = SimpleNamespace(id=999, username="sarthi_bot")
    return adapter
...
class TestSubstringFalsePositivesAreRejected:
    ...
    def test_email_like_substring(self):
        """bug #12545 exact repro: 'foo@sarthi_bot.example'."""
        adapter = _make_adapter()
    ...
    def test_email_substring_in_caption(self):
        adapter = _make_adapter()
        msg = _message(caption="foo@sarthi_bot.example")
        assert adapter._message_mentions_bot(msg) is False
