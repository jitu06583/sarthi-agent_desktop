...
def test_registered_observable_metric_names_cover_snapshot_metrics(monkeypatch):
    ...
    gateway_snapshot = type("S", (), {"metrics": [
        _M("sarthi.gateway.up"), _M("sarthi.gateway.active_agents"),
        _M("sarthi.gateway.busy"), _M("sarthi.gateway.drainable"),
        _M("sarthi.gateway.restart_requested"),
        _M("sarthi.platform.up"), _M("sarthi.platform.degraded"),
    ]})()
    cron_snapshot = type("S", (), {"metrics": [
        _M("sarthi.cron.scheduler.heartbeat_age_seconds"),
        _M("sarthi.cron.scheduler.last_success_age_seconds"),
        _M("sarthi.cron.scheduler.catch_up_occurrences"),
        _M("sarthi.cron.jobs.enabled"), _M("sarthi.cron.jobs.running"),
        _M("sarthi.cron.jobs.overdue"),
    ]})()
...
def test_monitoring_docs_distinguish_relay_health_scope_and_terminal_flush():
    ...
    assert "Sarthi Agent-owned Relay transport health" in text
