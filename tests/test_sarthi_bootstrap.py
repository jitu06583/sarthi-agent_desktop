"""Tests for sarthi_bootstrap — Windows UTF-8 stdio shim.

The bootstrap module is imported at the top of every Sarthi entry point
(sarthi, sarthi-agent, sarthi-acp, gateway, batch_runner, cli.py).  It
...
Key invariants covered by these tests:
  ...
  5. Load order: every Sarthi entry point imports sarthi_bootstrap as its
...
def _fresh_import():
    """Return a freshly-imported sarthi_bootstrap module.

    Drops any cached copy from sys.modules first so module-level code
    runs again and the platform check re-evaluates.
    """
    sys.modules.pop("sarthi_bootstrap", None)
    import sarthi_bootstrap  # noqa: WPS433
    return sarthi_bootstrap
...
class TestWindowsBehavior:
    ...
    def test_stdout_reconfigured_to_utf8_on_windows(self):
        # The live process's stdout should now be UTF-8 (the Sarthi CLI
...
class TestEntryPointsImportBootstrap:
    """Every Sarthi entry point must import sarthi_bootstrap as its
    ...
    # Entry points that invoke Sarthi as a process.  Each one must
    # import sarthi_bootstrap before doing any file I/O or stdout writes.
    ENTRY_POINTS = [
        "sarthi_cli/main.py",   # sarthi CLI (console_script)
        "run_agent.py",          # sarthi-agent (console_script)
        "acp_adapter/entry.py",  # sarthi-acp (console_script)
    ...
    def test_entry_point_imports_bootstrap(self, path):
        """The file must contain 'import sarthi_bootstrap' and that
        ...
        half-finished ``sarthi update`` (git-reset landed new code but
        ``uv pip install -e .`` didn't finish re-registering
        ``sarthi_bootstrap`` as a top-level module) leaves sarthi
        ...
        """
        # Resolve relative to the sarthi-agent repo root.  Tests live
        # at tests/test_sarthi_bootstrap.py, so go up one dir.
        import pathlib
        ...
        for node in ast.iter_child_nodes(tree):
            ...
            # sarthi start even when sarthi_bootstrap hasn't been
        ...
        assert first_import_name == "sarthi_bootstrap", (
            f"{path}: first top-level import is {first_import_name!r}, "
            f"but it must be 'sarthi_bootstrap' so UTF-8 stdio is "
            f"configured before anything else initializes.  Move the "
            f"'import sarthi_bootstrap' line to be the first import."
        )


class TestHardenImportPath:
    """harden_import_path() must keep a same-named package in the launch
    directory from shadowing Sarthi's own top-level modules — covering both
    the relative ('' / '.') and absolute-path forms the cwd can take on
    sys.path (issue #51286)."""

    def _run(self, hb, path_seed, env=None):
        original = sys.path[:]
        original_env = os.environ.get("SARTHI_PYTHON_SRC_ROOT")
        try:
            sys.path[:] = path_seed
            if env is not None:
                os.environ["SARTHI_PYTHON_SRC_ROOT"] = env
            elif "SARTHI_PYTHON_SRC_ROOT" in os.environ:
                del os.environ["SARTHI_PYTHON_SRC_ROOT"]
            hb.harden_import_path(src_root="/opt/sarthi")
            return sys.path[:]
        finally:
            sys.path[:] = original
            if original_env is None:
                os.environ.pop("SARTHI_PYTHON_SRC_ROOT", None)
            else:
                os.environ["SARTHI_PYTHON_SRC_ROOT"] = original_env

    def test_relative_cwd_forms_removed(self):
        hb = _fresh_import()
        result = self._run(hb, ["", ".", "/opt/sarthi", "/usr/lib/python"])
    ...
    def test_absolute_cwd_path_loses_to_src_root(self):
        ...
        # Sarthi root.  The guard must relocate Sarthi to the front.
        hb = _fresh_import()
        result = self._run(hb, ["/home/user/tg-ws-proxy", "/opt/sarthi"])
        assert result[0] == "/opt/sarthi"
        # The cwd absolute path may still appear (it can hold legit deps),
        # but only AFTER the Sarthi root.
        assert result.index("/opt/sarthi") < result.index("/home/user/tg-ws-proxy")

    def test_src_root_not_duplicated(self):
        hb = _fresh_import()
        result = self._run(hb, ["/opt/sarthi", "/opt/sarthi", ""])
        assert result.count("/opt/sarthi") == 1
    ...
    def test_env_var_used_when_no_arg(self):
        ...
        try:
            ...
            os.environ["SARTHI_PYTHON_SRC_ROOT"] = "/env/sarthi"
            hb.harden_import_path()
            assert sys.path[0] == "/env/sarthi"
        finally:
            sys.path[:] = original
            if original_env is None:
                os.environ.pop("SARTHI_PYTHON_SRC_ROOT", None)
