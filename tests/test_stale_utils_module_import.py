...
Real incident (gateway session 1518671026962174144)::
    ...
    cannot import name 'env_float' from 'utils' (~/.sarthi/sarthi-agent/utils.py)
...
2. ``sarthi update`` ran ``git pull``, updating ``utils.py`` (now defining
...
``sarthi_cli/main.py`` (the ``sarthi update`` flow, ~line 9326) already
acknowledges this exact hazard -- "source files on disk are newer than cached
Python modules in this process" -- and reloads ``sarthi_constants`` after the
