"""Shared fixtures for the sarthi-agent test suite.
...
2. **Isolated SARTHI_HOME.** SARTHI_HOME points to a per-test tempdir so
   code reading ``~/.sarthi/*`` via ``get_sarthi_home()`` can't see the
   real one. (We do NOT also redirect HOME — that broke subprocesses in
   CI. Code using ``Path.home() / ".sarthi"`` instead of the canonical
   ``get_sarthi_home()`` is a bug to fix at the callsite.)
3. **Deterministic runtime.** TZ=UTC, LANG=C.UTF-8, PYTHONHASHSEED=0.
4. **No SARTHI_SESSION_* inheritance** — the agent's current gateway
...
# ── Sandbox SARTHI_HOME before ANY test module is imported ──────────────────
# `sarthi_cli/main.py` calls `setup_logging()` at MODULE level, which resolves
# `get_sarthi_home()` and attaches rotating file handlers to the ROOT logger.
...
# real `~/.sarthi/logs/agent.log` and `errors.log`.
#
# The `_isolate_env` fixture below also sandboxes SARTHI_HOME, but fixtures run
...
# the REAL Sarthi root — capture it BEFORE the sandbox rewires SARTHI_HOME,
# otherwise the deny-list would point at the throwaway tempdir and the guard
# would silently stop protecting the operator's actual ~/.sarthi (#69385).
_PRE_SANDBOX_KANBAN_OVERRIDE = os.environ.get("SARTHI_KANBAN_HOME", "").strip()
_PRE_SANDBOX_SARTHI_HOME = os.environ.get("SARTHI_HOME", "")


def _sarthi_home_points_at_production(value: str) -> bool:
    """True when a pre-set SARTHI_HOME resolves to the real production root.

    Gateway-launched shells (and developer shells that ``export
    SARTHI_HOME=~/.sarthi``) hand pytest the PRODUCTION home. Historically
    the session sandbox below honored any pre-set value, so collection-time
    imports (logging handlers, ``sarthi_state.DEFAULT_DB_PATH``) froze paths
    inside the real ``~/.sarthi`` — the escape vector that landed pytest
    ...
    SARTHI_HOME is honored now.
    """
    if not value:
        return True
    try:
        resolved = Path(value).expanduser().resolve()
        real_root = (Path.home() / ".sarthi").resolve()
    except Exception:
        return True
...
if _sarthi_home_points_at_production(os.environ.get("SARTHI_HOME", "")):
    _SESSION_SARTHI_HOME = tempfile.mkdtemp(prefix="sarthi-test-home-")
    os.environ["SARTHI_HOME"] = _SESSION_SARTHI_HOME
    atexit.register(shutil.rmtree, _SESSION_SARTHI_HOME, True)
...
# like a real CLI") — which used to disarm sarthi_state's live-DB guard in
# the child at the same moment the child lost the SARTHI_HOME redirect.
# SARTHI_TEST_ISOLATION is OUR marker: exported here (before any test module
# imports), inherited by every child by default, and honored by
# sarthi_state._running_under_pytest() as a test-context signal. A child
...
# open a real DB must export SARTHI_STATE_DB_GUARD_BYPASS=1 in that child's
# env instead of stripping markers.
os.environ["SARTHI_TEST_ISOLATION"] = os.environ.get("SARTHI_HOME", "") or "1"

#: SARTHI_HOME as it stood when conftest was imported - i.e. before any test
...
SARTHI_HOME_AT_CONFTEST_IMPORT = os.environ.get("SARTHI_HOME", "")
...
# SARTHI_* vars that change test behavior by being set. Unset all of these
# unconditionally — individual tests that need them set do so explicitly.
_SARTHI_BEHAVIORAL_VARS = frozenset({
    ...
    # final response text to ``sarthi_cli.voice.speak_text`` — real synthesis,
    ...
    # any test run launched FROM a Sarthi agent session inherits them and
    # sarthi_constants home-resolution helpers prefer them over monkeypatched
    # HOME (test_subprocess_home_isolation red locally, green on CI).
    "SARTHI_REAL_HOME",
    "TERMINAL_HOME_MODE",
    "SARTHI_INTERACTIVE",
    "SARTHI_QUIET",
    "SARTHI_TOOL_PROGRESS",
    "SARTHI_TOOL_PROGRESS_MODE",
    "SARTHI_MAX_ITERATIONS",
    "SARTHI_SESSION_PLATFORM",
    "SARTHI_SESSION_CHAT_ID",
    "SARTHI_SESSION_CHAT_NAME",
    "SARTHI_SESSION_CHAT_TYPE",
    "SARTHI_SESSION_THREAD_ID",
    "SARTHI_SESSION_SOURCE",
    "SARTHI_SESSION_KEY",
    ...
    "SARTHI_PLATFORM",
    "SARTHI_MODEL",
    "SARTHI_INFERENCE_MODEL",
    "SARTHI_INFERENCE_PROVIDER",
    "SARTHI_TUI_PROVIDER",
    "SARTHI_MANAGED",
    "SARTHI_MANAGED_DIR",
    "SARTHI_DEV",
    ...
    "SARTHI_TIMEZONE",
    "SARTHI_REDACT_SECRETS",
    "SARTHI_BACKGROUND_NOTIFICATIONS",
    "SARTHI_EXEC_ASK",
    "SARTHI_HOME_MODE",
    "SARTHI_AGENT_USE_LEGACY_SESSION_KEYS",
    # Kanban path/board pins must never leak from a developer shell or
    # dispatched worker into tests; otherwise tests can write fake tasks to
    # the real ~/.sarthi/kanban.db instead of the per-test SARTHI_HOME.
    "SARTHI_KANBAN_DB",
    "SARTHI_KANBAN_BOARD",
    "SARTHI_KANBAN_HOME",
    "SARTHI_KANBAN_WORKSPACES_ROOT",
    "SARTHI_KANBAN_LOGS_ROOT",
    "SARTHI_KANBAN_TASK",
    "SARTHI_KANBAN_WORKSPACE",
    "SARTHI_KANBAN_RUN_ID",
    "SARTHI_KANBAN_CLAIM_LOCK",
    "SARTHI_KANBAN_DISPATCH_IN_GATEWAY",
    ...
    "SARTHI_DELEGATED_CHILD_CONTEXT",
    "SARTHI_TENANT",
    ...
    # otherwise-unrelated config tests away from the default "sarthi" host.
    "SARTHI_HONCHO_HOST",
    ...
    "SARTHI_DASHBOARD_OAUTH_CLIENT_ID",
    "SARTHI_DASHBOARD_PORTAL_URL",
    "TERMINAL_CWD",
    "TERMINAL_ENV",
...
def _hermetic_environment(tmp_path, monkeypatch):
    ...
    Also redirects HOME and SARTHI_HOME to per-test tempdirs so code that
    reads ``~/.sarthi/*`` can't touch the real one, and pins TZ/LANG so
    ...
    # 2. Blank behavioral SARTHI_* vars that could change test semantics.
    for name in _SARTHI_BEHAVIORAL_VARS:
        monkeypatch.delenv(name, raising=False)
    ...
    monkeypatch.setenv("SARTHI_HONCHO_HOST", "sarthi")

    # 3. Redirect SARTHI_HOME to a per-test tempdir. Code that reads
    #    ``~/.sarthi/*`` via ``get_sarthi_home()`` now gets the tempdir.
    #
    #    NOTE: We do NOT also redirect HOME. Doing so broke CI because
    #    some tests (and their transitive deps) spawn subprocesses that
    #    inherit HOME and expect it to be stable. If a test genuinely
    #    needs HOME isolated, it should set it explicitly in its own
    #    fixture. Any code in the codebase reading ``~/.sarthi/*`` via
    #    ``Path.home() / ".sarthi"`` instead of ``get_sarthi_home()``
    #    is a bug to fix at the callsite.
    fake_sarthi_home = tmp_path / "sarthi_test"
    fake_sarthi_home.mkdir()
    (fake_sarthi_home / "sessions").mkdir()
    (fake_sarthi_home / "cron").mkdir()
    (fake_sarthi_home / "memories").mkdir()
    (fake_sarthi_home / "skills").mkdir()
    monkeypatch.setenv("SARTHI_HOME", str(fake_sarthi_home))
    ...
    # sarthi_state's live-DB guard stays armed in them even when the test
    # strips pytest's own PYTEST_* vars from the child env.
    monkeypatch.setenv("SARTHI_TEST_ISOLATION", str(fake_sarthi_home))
    # And never let a developer-shell (or leaked child) bypass disarm the
    # guard for in-process code under test.
    monkeypatch.delenv("SARTHI_STATE_DB_GUARD_BYPASS", raising=False)

    # 3b. sarthi_state computes ``DEFAULT_DB_PATH = get_sarthi_home() / "state.db"``
    #     at import time. When the module is first imported at collection (any
    #     test file with a top-level ``from sarthi_state import ...``) that
    ...
    sarthi_state_mod = sys.modules.get("sarthi_state")
    if sarthi_state_mod is not None and hasattr(sarthi_state_mod, "DEFAULT_DB_PATH"):
        monkeypatch.setattr(
            sarthi_state_mod, "DEFAULT_DB_PATH", fake_sarthi_home / "state.db"
        )
    ...
    monkeypatch.setenv("SARTHI_DISABLE_LAZY_INSTALLS", "1")

    # 5. Reset plugin singleton so tests don't leak plugins from
    #    ~/.sarthi/plugins/ (which, per step 3, is now empty — but the
    #    singleton might still be cached from a previous test).
    try:
        import sarthi_cli.plugins as _plugins_mod
        ...
        # previous test's tmp_path SARTHI_HOME can't leak forward. Paths
...
@pytest.fixture(autouse=True)
def _isolate_sarthi_home(_hermetic_environment):
...
def _neutralize_kanban_memory_guard(request, monkeypatch):
    ...
    if request.node.get_closest_marker("real_memory_guard"):
        return
    try:
        from sarthi_cli import kanban_db as _kb_mod
    except Exception:
        return
...
# invocation), kanban writes silently pollute the real ~/.sarthi. This autouse
...
# allow-list because test-level fixtures legitimately move SARTHI_HOME to
...
def _capture_real_kanban_root() -> Path:
    ...
    file (before the session SARTHI_HOME sandbox rewired the env), so the
    deny-list keeps pointing at the operator's actual root. Mirrors
    ``kanban_db.kanban_home()`` resolution order:
    1. ``SARTHI_KANBAN_HOME`` env var when set and non-empty
    2. the real (pre-sandbox) Sarthi root otherwise
    """
    if _PRE_SANDBOX_KANBAN_OVERRIDE:
        return Path(_PRE_SANDBOX_KANBAN_OVERRIDE).expanduser().resolve()
    if _PRE_SANDBOX_SARTHI_HOME and not _sarthi_home_points_at_production(
        _PRE_SANDBOX_SARTHI_HOME
    ):
        # SARTHI_HOME was genuinely set to a CUSTOM root before the sandbox
        ...
        return get_default_sarthi_root().resolve()
    # No pre-existing SARTHI_HOME: the real root is the platform default,
    # NOT the sandbox tempdir now sitting in the env.
    return (Path.home() / ".sarthi").resolve()
...
def _kanban_write_guard(_hermetic_environment, monkeypatch):
    ...
    ``~/.sarthi`` captured at import time. Hermetic tests that legitimately
    move SARTHI_HOME to sibling tempdirs are unaffected.

    Only patches when ``sarthi_cli.kanban_db`` is *already imported* — a
    ...
    """
    _kdb = sys.modules.get("sarthi_cli.kanban_db")
    if _kdb is None:
        return

    # The sys.modules probe can observe the module MID-IMPORT: a fixture
    # boundary firing while another test's lazy `import sarthi_cli.kanban_db`
    ...
    def _guarded_connect(db_path=None, *args, **kwargs):
        ...
        raise RuntimeError(
            ...
            f"to the real ~/.sarthi. See #69283."
        )
...
# ``sarthi_state._ensure_test_isolation`` (the single choke point every
# ``SessionDB()`` construction goes through) refuses, under pytest, any DB
# path that resolves inside the REAL Sarthi root. This fixture wires the
# test-side knobs:
...
#     installs where SARTHI_HOME is not ~/.sarthi) into the guard's
...
# so subprocess children that import sarthi_state directly are covered even
# without this fixture.


@pytest.fixture(autouse=True)
def _state_db_write_guard(request, monkeypatch):
    _hs = sys.modules.get("sarthi_state")
    ...
    if _PRE_SANDBOX_SARTHI_HOME and not _sarthi_home_points_at_production(
        ...
        extra_roots.append(
            Path(_PRE_SANDBOX_SARTHI_HOME).expanduser().resolve()
        )
    monkeypatch.setattr(
...
def _reset_tui_gateway_server_state():
    ...
    # A leaked context-local Sarthi home override redirects every later
    # ``get_sarthi_home()`` call (active-session registry, config paths)
    # to a stale per-test tmpdir. Force the main-thread ContextVar back
    # to its default.
    try:
        from sarthi_constants import get_sarthi_home_override, set_sarthi_home_override

        if get_sarthi_home_override() is not None:
            set_sarthi_home_override(None)
    except Exception:
        pass
...
@pytest.fixture()
def mock_config():
    """Return a minimal sarthi config dict suitable for unit tests."""
    return {
...
# environment and finds the developer's live ``sarthi-gateway`` process
...
# correlated with a ``tests/sarthi_cli/`` pytest run starting up.
#
...
#    ``check_output`` reject any ``systemctl ... <verb> sarthi-gateway``
...
def _wal_is_usable() -> bool:
    """True when Sarthi will actually put a database into WAL mode here.

    Sarthi refuses journal_mode=WAL on SQLite builds carrying the upstream
    ...
    running Sarthi can link DIFFERENT SQLite versions: a repo ``.venv`` on
    3.50.4 (vulnerable → DELETE) alongside a Sarthi managed runtime on 3.53.1
    (fixed → WAL). The same test then passes in one and fails in the other.

    IMPORTANT: this must NOT import ``sarthi_state``. That module computes
    ``DEFAULT_DB_PATH`` from ``get_sarthi_home()`` at import time, so importing
    it during collection — before the per-test ``_isolate_sarthi_home`` fixture
    redirects ``SARTHI_HOME`` — permanently caches the DEVELOPER'S REAL
    ``~/.sarthi/state.db`` for the whole session. Tests then read live
    production sessions instead of a tempdir. The version predicate is
    duplicated from ``sarthi_state._is_sqlite_wal_reset_vulnerable`` (upstream
...
#      process environment: ``os.environ["SARTHI_VOICE_TTS"] = "1"``. The
...
#      ``sarthi_cli.voice.speak_text(final_response)`` on a daemon thread.
...
# to mock it:
#
#  • ``sarthi_cli.voice.speak_text`` — the synth+playback entry point both
#    gateway call sites late-import, so patching the module attribute catches
#    them wherever they import it from.
#  • ``sarthi_cli.voice.play_audio_file`` — the module-level binding
#    ``speak_text`` actually plays through. Patching the binding inside
#    ``sarthi_cli.voice`` (not ``tools.voice_mode``) keeps the real function
...
# OS gating
#
# Sarthi runs on Linux, macOS and native Windows, and a lot of its behaviour
# genuinely differs per host: PTY vs pywinpty, taskkill vs SIGTERM, launchd
# vs systemd, Keychain vs libsecret, ``%LOCALAPPDATA%`` vs ``~/.sarthi``.
#
...
def pytest_configure(config):  # noqa: D401 — pytest hook
    ...
    config.addinivalue_line(
        ...
        "SQLite WAL mode; skipped on builds where Sarthi falls back to "
...
def pytest_collection_modifyitems(config, items):  # noqa: D401 — pytest hook
    ...
    reason = (
        f"SQLite {sqlite3.sqlite_version} has the WAL-reset bug — Sarthi uses "
...
def _live_system_guard(request, monkeypatch):
    ...
    tokens[0]), so ``bash -c "systemctl restart sarthi-gateway"``,
    ``sudo systemctl ...``, ``env systemctl ...``, ``setsid systemctl ...``
    are all caught. ``pkill``/``killall``/``taskkill`` invocations
    targeting sarthi/python patterns are also blocked.
    """
    ...
    def _matches_sarthi_gateway(cmd_str: str) -> bool:
        low = cmd_str.lower()
        return any(tok in low for tok in _SARTHI_TOKENS)

    def _is_blocked_systemctl(cmd) -> bool:
        cmd_str = _cmd_to_string(cmd)
        if "systemctl" not in cmd_str:
            return False
        if not _matches_sarthi_gateway(cmd_str):
            return False
        try:
    ...
    def _is_process_killer(cmd) -> bool:
        ...
        for tok in killer_tokens:
            ...
            if head in _PROCESS_KILLERS:
                low = cmd_str.lower()
                # pkill -f pattern: catch sarthi-themed patterns + a
                # plain "python" -f which would catch the live gateway
                # whose cmdline contains "python -m sarthi_cli.main".
                if (
                    "sarthi" in low
                    or "gateway" in low
    ...
    def _check_subprocess_cmd(name, cmd):
        if _is_blocked_systemctl(cmd):
            raise RuntimeError(
                ...
                "live sarthi-gateway systemd unit. Mock "
        ...
        if _is_process_killer(cmd):
            raise RuntimeError(
                ...
                "targeting sarthi/python could hit the live gateway. "
                "Mark with @pytest.mark.live_system_guard_bypass if "
                "intentional."
            )
        # Block any subprocess that would run `sarthi update` (or the
        # equivalent `python -m sarthi_cli.main update`).  These commands
        ...
        if "update" in low and (
            # sarthi update / sarthi update --gateway / setsid bash -c ... sarthi update
            ("sarthi" in low and "update" in low.split())
            or
            # python -m sarthi_cli.main update --gateway
            ("sarthi_cli" in low and "update" in low.split())
            or
            # venv/bin/sarthi update  (absolute path variant used in tests)
            (".venv/bin/sarthi" in low and "update" in low)
        ):
            raise RuntimeError(
                f"tests/conftest.py live-system guard: blocked "
                f"subprocess.{name}({cmd!r}) — this command would run "
                "`sarthi update` against the real checkout, fetching "
...
def _audio_playback_guard(request, monkeypatch):
    ...
    depth behind ``_SARTHI_BEHAVIORAL_VARS``: the env blanking stops the flag
    ...
    try:
        import sarthi_cli.voice as _voice
    except Exception:
