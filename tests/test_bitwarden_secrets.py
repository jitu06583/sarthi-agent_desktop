...
binary download are exercised manually by `sarthi secrets bitwarden
...
def sarthi_home(tmp_path, monkeypatch):
    """Point Sarthi at an isolated home directory."""
    home = tmp_path / ".sarthi"
    home.mkdir()
    monkeypatch.setenv("SARTHI_HOME", str(home))
    # Some modules cache get_sarthi_home; clear if needed.
    import sarthi_constants
    if hasattr(sarthi_constants, "_SARTHI_HOME_CACHE"):
        sarthi_constants._SARTHI_HOME_CACHE = None  # type: ignore[attr-defined]
    return home
...
def test_install_bws_rejects_malicious_member(sarthi_home, monkeypatch):
...
def test_install_bws_happy_path(sarthi_home, monkeypatch):
...
def test_install_bws_checksum_mismatch(sarthi_home, monkeypatch):
...
def test_install_bws_missing_checksum_entry(sarthi_home, monkeypatch):
...
def test_env_loader_skips_when_disabled(tmp_path, monkeypatch):
    ...
    from sarthi_cli.env_loader import _apply_external_secret_sources
...
def test_disk_cache_written_after_first_fetch(monkeypatch, tmp_path):
    """First fetch hits bws AND writes a 0600 file under sarthi_home/cache/."""
    home = tmp_path / ".sarthi"
    home.mkdir()
    fake_binary = tmp_path / "bws"
    fake_binary.write_text("")
