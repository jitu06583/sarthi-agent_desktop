...
import sarthi_cli.doctor as doctor
import sarthi_cli.gateway as gateway_cli
from sarthi_cli import doctor as doctor_mod
from sarthi_cli.doctor import _has_provider_env_config
...
class TestDoctorEnvFileEncoding:
    """Regression for #18637 (bug 3): `sarthi doctor` crashed on Windows
    ...
    def test_doctor_reads_env_as_utf8_even_when_locale_is_not_utf8(
        ...
        monkeypatch.setattr(doctor_mod, "SARTHI_HOME", sarthi_home)
...
class TestDoctorToolAvailabilityOverrides:
    ...
    def test_leaves_kanban_unavailable_when_worker_env_is_set(self, monkeypatch):
        monkeypatch.setenv("SARTHI_KANBAN_TASK", "probe")
...
class TestDoctorMemoryProviderSection:
    ...
    def _make_sarthi_home(self, tmp_path, provider=""):
        """Create a minimal SARTHI_HOME with config.yaml."""
    ...
    def _run_doctor_and_capture(self, monkeypatch, tmp_path, provider=""):
        ...
        home = self._make_sarthi_home(tmp_path, provider)
        monkeypatch.setattr(doctor_mod, "SARTHI_HOME", home)
        ...
        try:
            from sarthi_cli import auth as _auth_mod
...
def test_run_doctor_accepts_sarthi_provider_ids_that_catalog_aliases(
...
def _doctor_env_for_agent_browser(monkeypatch, tmp_path):
    ...
    branch in run_doctor (sarthi_cli/doctor.py ~1557-1605)."""
    home = tmp_path / ".sarthi"
    home.mkdir(parents=True, exist_ok=True)
    (home / "config.yaml").write_text("memory: {}\n", encoding="utf-8")
    project = tmp_path / "project"
...
def test_run_doctor_fix_warms_npx_cache_when_agent_browser_resolves_via_npx(
    ...
    """`sarthi doctor --fix` must actually call warm_agent_browser_npx_cache()
...
class TestGitHubTokenCheck:
    ...
    def _isolate_home(monkeypatch, home):
        """Point doctor at the temp SARTHI_HOME.

        ``run_doctor`` reads the module-level ``SARTHI_HOME`` constant (cached
        at import time), NOT the env var — so ``setenv("SARTHI_HOME")`` alone
        leaves doctor probing the REAL ~/.sarthi. On a dev machine with a
...
def test_has_healthy_oauth_fallback_returns_false_for_unknown_provider():
    from sarthi_cli.doctor import _has_healthy_oauth_fallback_for_apikey_provider
...
class TestHasHealthyOauthFallbackForXai:
    ...
    def test_returns_false_when_xai_import_unavailable(self, monkeypatch):
        ...
        monkeypatch.delattr("sarthi_cli.auth.get_xai_oauth_auth_status", raising=False)
        # Force doctor module to re-import the function
        monkeypatch.delitem(sys.modules, "sarthi_cli.doctor", raising=False)
        from sarthi_cli.doctor import _has_healthy_oauth_fallback_for_apikey_provider
...
class TestDoctorStaleMaxIterationsDrift:
    """Regression for #17534: a stale SARTHI_MAX_ITERATIONS in .env shadows
    ...
    def _run_config_section(self, monkeypatch, tmp_path, *, fix, ghost, cfg_turns,
        ...
        sarthi_home = tmp_path / ".sarthi"
        sarthi_home.mkdir(parents=True)
        (sarthi_home / "config.yaml").write_text(
            f"agent:\n  max_turns: {cfg_turns}\n", encoding="utf-8"
        )
        env_lines = ["OPENAI_API_KEY=sk-test\n"]
        if ghost is not None:
            env_lines.append(f"SARTHI_MAX_ITERATIONS={ghost}\n")
        (sarthi_home / ".env").write_text("".join(env_lines), encoding="utf-8")

        monkeypatch.setattr(doctor_mod, "SARTHI_HOME", sarthi_home)
        monkeypatch.setattr(doctor_mod, "get_sarthi_home", lambda: sarthi_home)
        ...
        if os_environ_value is not None:
            ...
            monkeypatch.setenv("SARTHI_MAX_ITERATIONS", str(os_environ_value))
        else:
            monkeypatch.delenv("SARTHI_MAX_ITERATIONS", raising=False)
    ...
    def test_fix_removes_ghost(self, monkeypatch, tmp_path):
        ...
        assert "Removed stale SARTHI_MAX_ITERATIONS" in out
        env_after = (sarthi_home / ".env").read_text(encoding="utf-8")
        assert "SARTHI_MAX_ITERATIONS" not in env_after
...
class TestDoctorDeprecatedConfigAndEnv:
    ...
    def test_sarthi_tool_progress_warning_says_unsupported_since_floor(self):
        """SARTHI_TOOL_PROGRESS lost its last consumer (the retired v3→4
        ...
        findings = dict(
            doctor_mod.collect_deprecated_env_vars({"SARTHI_TOOL_PROGRESS": "true"})
        )
        assert "ignored/unsupported since config floor v12" in findings["SARTHI_TOOL_PROGRESS"]
        # The MODE variant is still read by the gateway fallback → keeps the
        # plain deprecation wording.
        mode = dict(
            doctor_mod.collect_deprecated_env_vars({"SARTHI_TOOL_PROGRESS_MODE": "all"})
        )
        assert mode["SARTHI_TOOL_PROGRESS_MODE"] == "display.tool_progress in config.yaml"

    def _run_doctor_with_config(self, monkeypatch, tmp_path, *, config_yaml: str, env_text: str = ""):
        sarthi_home = tmp_path / ".sarthi"
        sarthi_home.mkdir(parents=True)
        (sarthi_home / "config.yaml").write_text(config_yaml, encoding="utf-8")
        env_body = env_text if env_text else "OPENAI_API_KEY=sk-test\n"
        (sarthi_home / ".env").write_text(env_body, encoding="utf-8")
    ...
    def test_report_does_not_count_as_blocking_issue(self, monkeypatch, tmp_path, capsys):
        ...
        findings = doctor_mod.report_deprecated_config_and_env(
            ...
            {"SARTHI_TOOL_PROGRESS_MODE": "verbose"},
        )
        out = capsys.readouterr().out
        assert len(findings) == 2
        assert "Deprecated: delegation.max_async_children" in out
        assert "Deprecated: SARTHI_TOOL_PROGRESS_MODE" in out
