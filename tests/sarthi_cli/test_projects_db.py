"""Tests for the per-profile Projects store (sarthi_cli/projects_db)."""
...
def test_create_get_list(conn):
    pid = pdb.create_project(conn, name="Sarthi Agent", folders=["/tmp/sarthi"])
    proj = pdb.get_project(conn, pid)

    assert proj is not None
    assert proj.slug == "sarthi-agent"
    assert proj.name == "Sarthi Agent"
    # First folder becomes primary.
    assert proj.primary_path == "/tmp/sarthi"
    assert [f.path for f in proj.folders] == ["/tmp/sarthi"]
    assert proj.folders[0].is_primary is True

    # Lookup by slug too.
    assert pdb.get_project(conn, "sarthi-agent").id == pid
    assert len(pdb.list_projects(conn)) == 1


def test_slug_collision_disambiguates(conn):
    pdb.create_project(conn, name="Sarthi Agent")
    pdb.create_project(conn, name="Sarthi Agent")
    slugs = sorted(p.slug for p in pdb.list_projects(conn))

    assert slugs == ["sarthi-agent", "sarthi-agent-2"]
...
def test_per_profile_isolation(tmp_path):
    # Two distinct DB paths stand in for two profiles' SARTHI_HOME.
...
def test_db_path_under_sarthi_home():
    # Resolves under SARTHI_HOME (set by the autouse isolation fixture).
