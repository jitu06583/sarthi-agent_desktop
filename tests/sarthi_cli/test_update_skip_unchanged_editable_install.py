"""``sarthi update`` skips the editable reinstall when the pull can't affect it.
...
that rewrite is the only reason the running ``sarthi.exe`` gets quarantined,
...
from sarthi_cli.update_cmd import _editable_install_is_current
