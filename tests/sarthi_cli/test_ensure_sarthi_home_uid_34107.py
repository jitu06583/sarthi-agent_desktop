"""Regression tests for #34107 — Docker UID/GID handling in ensure_sarthi_home.

When Sarthi runs in Docker with ``SARTHI_UID=1000`` / ``SARTHI_GID=911``,
the entrypoint chowns the top-level ``SARTHI_HOME`` once at startup. But
...
The fix is a ``_chown_to_sarthi_uid`` helper that reads the env vars and
...
# ---------------------------------------------------------------------------
# _resolve_sarthi_uid_gid
# ---------------------------------------------------------------------------


class TestResolveSarthiUidGid:
...
# ---------------------------------------------------------------------------
# _chown_to_sarthi_uid
# ---------------------------------------------------------------------------


class TestChownToSarthiUid:
    ...
    def test_attributeerror_swallowed_for_windows_compat(self, tmp_path, monkeypatch):
        ...
        with patch.object(cfg.os, "chown", side_effect=AttributeError("no chown on this platform")):
            cfg._chown_to_sarthi_uid(d)  # must not raise
