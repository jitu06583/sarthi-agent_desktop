...
from sarthi_cli.nous_account import NousPortalAccountInfo, NousToolAccessInfo
...
class TestAgentBrowserPostSetup:
    ...
    tools.browser_tool._find_agent_browser (PATH -> Homebrew/Sarthi-managed
    ...
    def _stub_browser_use_install(self):
        ...
        logic under test."""
        with patch("sarthi_cli.tools_config._ensure_browser_use_cli") as stub:
            yield stub

    def test_warns_when_neither_npx_nor_agent_browser_on_path(self):
        with patch("shutil.which", return_value=None), patch(
            "subprocess.run"
        ) as run, patch("sarthi_cli.tools_config._print_warning") as warn:
    ...
    def test_chromium_already_installed_skips_subprocess(self):
        with patch("shutil.which", return_value="/usr/bin/npx"), patch(
            ...
            "sarthi_cli.tools_config._print_success"
        ) as success:
            _run_post_setup("agent_browser")
    ...
    def test_docker_with_missing_chromium_warns_instead_of_installing(self):
        with patch("shutil.which", return_value="/usr/bin/npx"), patch(
            ...
            "sarthi_cli.tools_config._print_warning"
        ) as warn:
            _run_post_setup("agent_browser")
    ...
    def test_installs_chromium_via_npx_resolved_only_through_extended_path(self):
        """Sarthi-managed-Node-only setups: npx resolves via
        ...
        sarthi_npx = "/home/user/.sarthi/node/bin/npx"
        with patch("shutil.which", return_value=None), patch(
            "subprocess.run"
        ) as run, patch(
            "tools.browser_tool._chromium_installed", return_value=False
            ...
            "tools.browser_tool._resolve_npx_bin", return_value=sarthi_npx
        ), patch(
            "sarthi_cli.tools_config._print_success"
        ):
            run.return_value = SimpleNamespace(returncode=0, stdout="", stderr="")
        ...
        run.assert_called_once()
        assert run.call_args.args[0] == [
            sarthi_npx, "--ignore-scripts", "-y", AGENT_BROWSER_NPX_SPEC, "install", "--with-deps",
        ]
    ...
    def test_install_failure_prints_stderr_tail_and_does_not_invalidate_cache(self):
        ...
        with patch("shutil.which", return_value="/usr/bin/npx"), patch(
            ...
            "sarthi_cli.tools_config._print_info"
        ) as info:
...
class TestBrowserUseCliInstalledForAllNonCamofoxBackends:
    ...
    def test_browser_post_setup_attempts_cli_install(self, key):
        with patch("sarthi_cli.tools_config._ensure_browser_use_cli") as ensure, patch(
    ...
    def test_camofox_post_setup_never_touches_browser_use(self):
        ...
        the CLI in."""
        with patch("sarthi_cli.tools_config._ensure_browser_use_cli") as ensure, patch(
            "sarthi_constants.find_node_executable", return_value=None
        ), patch("subprocess.run"):
    ...
    def test_ensure_helper_always_delegates_to_install_cli(self):
        ...
        $SARTHI_HOME/bin when only side installs exist."""
        with patch(
            "sarthi_cli.tools_config.shutil.which", return_value="/usr/bin/browser-use"
        ), patch(
            "tools.browser_use_cli.install_cli",
            return_value=(True, "browser-use CLI already installed (/managed/bin/browser-use)"),
        ) as install:
            from sarthi_cli.tools_config import _ensure_browser_use_cli
    ...
    def test_ensure_helper_install_failure_is_non_fatal(self):
        ...
        with patch(
            "sarthi_cli.tools_config.shutil.which", return_value=None
        ), patch(
            "tools.browser_use_cli.install_cli",
            return_value=(False, "`uv tool install browser-use` failed:\nboom"),
        ), patch("sarthi_cli.tools_config._print_warning") as warn:
...
class TestImagegenModelPicker:
    ...
    def test_plugin_picker_falls_back_when_default_is_missing_from_catalog(self):
        ...
        from sarthi_cli.tools_config import _configure_imagegen_model_for_plugin
        ...
        with (
            patch(
                "sarthi_cli.tools_config._plugin_image_gen_catalog",
                return_value=(catalog, "also-missing"),
            ),
            patch("sarthi_cli.tools_config._prompt_choice", return_value=0),
        ):
...
# `sarthi tools post-setup <key>` child. On Windows each console child (npm,
...
# ── Toolsets that shipped after a platform's last `sarthi tools` save ────────
#
# Saving the picker (or one toggle in the desktop Toolsets UI) replaces a
# platform's composite (``[sarthi-cli]``) with a frozen explicit list, and
...
def test_agent_disabled_toolsets_json_array_string_form_still_wins():
    ...
    `sarthi config set agent.disabled_toolsets '["memory"]'`). It must be
...
def test_platforms_whose_composite_excludes_it_are_left_narrow():
    ...
    composite (sarthi-acp, sarthi-webhook) that never carried the toolset."""
    from toolsets import TOOLSETS, resolve_toolset

    narrow = [
        platform
        for platform in ("acp", "webhook")
        if f"sarthi-{platform}" in TOOLSETS
        and not any(
            set(resolve_toolset(ts, include_registry=False))
            <= set(resolve_toolset(f"sarthi-{platform}"))
...
def test_explicit_plugin_toolset_admitted_in_platform_toolsets(monkeypatch):
    ...
    ``sarthi-cli``), it MUST be admitted as a configurable key instead of
    ...
    import sarthi_cli.tools_config as _tc_mod
    ...
    # standard composite — exactly the "I want sarthi-cli AND a2a in my CLI
    # session" config the issue's user was trying to write.
    config = {"platform_toolsets": {"cli": ["sarthi-cli", "dplat_client"]}}
...
def test_explicit_plugin_toolset_admitted_against_real_a2a_plugin(monkeypatch):
    ...
    a real config like ``platform_toolsets.cli: [sarthi-cli, a2a]``, ``a2a``
    ...
    config = {"platform_toolsets": {"cli": ["sarthi-cli", "a2a"]}}
    enabled = _get_platform_tools(config, "cli")
