...
def test_build_gateway_argv_keeps_venv_console_python_for_uv_venv(monkeypatch, tmp_path):
    ...
    assert argv[:3] == [str(venv_python), "-m", "sarthi_cli.main"]
...
def test_spawn_detached_marks_primary_breakaway_success(monkeypatch, tmp_path, caplog):
    ...
    argv = ["python.exe", "-m", "sarthi_cli.main", "gateway", "run"]
    cwd = str(tmp_path)
    calls = []
    ...
    monkeypatch.setattr(
        gateway_windows,
        "_build_gateway_argv",
        lambda: (argv, cwd, {"SARTHI_GATEWAY_DETACHED": "1"}),
    )
    monkeypatch.setattr("sarthi_cli.config.get_sarthi_home", lambda: tmp_path)
...
def test_spawn_detached_warns_and_marks_no_breakaway_fallback(
    ...
    monkeypatch.setattr(
        ...
        lambda: (
            argv,
            cwd,
            {"SARTHI_GATEWAY_DETACHED": "1", "SECRET_SENTINEL": "do-not-log"},
        ),
    )
    monkeypatch.setattr("sarthi_cli.config.get_sarthi_home", lambda: tmp_path)
    monkeypatch.setattr(gateway_windows.subprocess, "Popen", fake_popen)
...
def test_elevated_gateway_command_uses_hidden_console_python(monkeypatch):
    ...
    assert verb == "runas"
    assert executable == r"C:\Sarthi\venv\Scripts\python.exe"
