...
def _run_sarthi(home: Path, *args: str, marker: bool = False) -> subprocess.CompletedProcess[str]:
    env = os.environ.copy()
    env["SARTHI_HOME"] = str(home)
    env["SARTHI_KANBAN_HOME"] = str(home)
    for name in (
        "SARTHI_KANBAN_BOARD",
        "SARTHI_KANBAN_DB",
    ...
    if marker:
        env["SARTHI_DELEGATED_CHILD_CONTEXT"] = "1"
    else:
        env.pop("SARTHI_DELEGATED_CHILD_CONTEXT", None)
...
def test_delegated_child_kanban_cli_refusal_returns_nonzero_exit_status(tmp_path):
    ...
    created = _run_sarthi(home, "kanban", "create", "exit status probe", "--json")
