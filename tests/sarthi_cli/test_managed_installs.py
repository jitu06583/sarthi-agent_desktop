...
def test_get_managed_system_homebrew(monkeypatch):
    ...
    assert recommended_update_command() == "brew upgrade sarthi-agent"


def test_format_managed_message_homebrew(monkeypatch):
    monkeypatch.setenv("SARTHI_MANAGED", "homebrew")

    message = format_managed_message("update Sarthi Agent")

    assert "managed by Homebrew" in message
    assert "brew upgrade sarthi-agent" in message
...
def test_cmd_update_blocks_managed_homebrew(monkeypatch, capsys):
    ...
    assert "brew upgrade sarthi-agent" in captured.err
