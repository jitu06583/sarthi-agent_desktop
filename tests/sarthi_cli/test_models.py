...
class TestUnionWithPortalFreeRecommendations:
    ...
    def test_empty_payload_returns_inputs_unchanged(self):
        ...
        with patch("sarthi_cli.models.fetch_nous_recommended_models", return_value={}):
            ids, p = union_with_portal_free_recommendations(curated, pricing, "")
        assert ids == curated
        assert p == pricing
...
class TestNousRecommendedModels:
    ...
    def test_fetch_returns_empty_on_network_failure(self):
        from sarthi_cli.models import fetch_nous_recommended_models
        with patch("sarthi_cli.models._urlopen_model_catalog_request", side_effect=OSError("boom")):
    ...
    def test_auto_detects_tier_when_not_supplied(self):
        ...
        with (
            ...
            patch("sarthi_cli.models.check_nous_free_tier", return_value=True),
        ):
            assert get_nous_recommended_aux_model(vision=False) == "free-model"
        with (
            patch("sarthi_cli.models.fetch_nous_recommended_models", return_value=payload),
            patch("sarthi_cli.models.check_nous_free_tier", return_value=False),
        ):
