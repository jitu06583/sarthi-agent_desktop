"""Tests for the top-level `./sarthi` launcher script."""
...
def test_launcher_delegates_to_argparse_entrypoint(monkeypatch):
    """`./sarthi` should use `sarthi_cli.main`, not the legacy Fire wrapper."""
    launcher_path = Path(__file__).resolve().parents[2] / "sarthi"
    called = []

    fake_main_module = types.ModuleType("sarthi_cli.main")

    def fake_main():
        called.append("sarthi_cli.main")

    fake_main_module.main = fake_main
    monkeypatch.setitem(sys.modules, "sarthi_cli.main", fake_main_module)
