...
def test_default_spawn_pins_assignee_profile_cli_toolsets(monkeypatch, tmp_path):
    ...
    SARTHI_KANBAN_TASK: the worker must not collapse to only kanban lifecycle
...
""".lstrip(),
    ...
    assert pid == 4242
    assert captured["env"]["SARTHI_HOME"] == str(profile)
    assert captured["env"]["SARTHI_KANBAN_TASK"] == "t_spawn_tools"
...
def test_default_spawn_never_boots_the_tui(monkeypatch, tmp_path):
    """Workers are headless: an inherited SARTHI_TUI=1 (or a TUI-default
    ...
    flag) and strips SARTHI_TUI from the child env."""
    root = tmp_path / ".sarthi"
    ...
    assert "SARTHI_TUI" not in captured["env"]
