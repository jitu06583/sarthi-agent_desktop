"""Tests for the `sarthi memory reset` CLI command.
...
- Profile-scoped reset (uses SARTHI_HOME)
"""
...
def memory_env(tmp_path, monkeypatch):
    """Set up a fake SARTHI_HOME with memory files."""
    sarthi_home = tmp_path / ".sarthi"
    memories = sarthi_home / "memories"
    memories.mkdir(parents=True)
    monkeypatch.setenv("SARTHI_HOME", str(sarthi_home))

    # Create sample memory files
    (memories / "MEMORY.md").write_text(
        "§\nSarthi repo is at ~/.sarthi/sarthi-agent\n§\nUser prefers dark themes",
        encoding="utf-8",
    )
...
def _run_memory_reset(target="all", yes=False, monkeypatch=None, confirm_input="no"):
    ...
    Simulates what happens when `sarthi memory reset` is run.
    """
...
class TestMemoryReset:
    ...
    def test_reset_no_files_exist(self, tmp_path, monkeypatch):
        ...
        sarthi_home = tmp_path / ".sarthi"
        (sarthi_home / "memories").mkdir(parents=True)
        monkeypatch.setenv("SARTHI_HOME", str(sarthi_home))
    ...
    def test_reset_profile_scoped(self, tmp_path, monkeypatch):
        """Reset should work on the active profile's SARTHI_HOME."""
    ...
    def test_reset_empty_memories_dir(self, tmp_path, monkeypatch):
        ...
        # The memories dir won't exist; get_sarthi_home() / "memories" won't have files
        result = _run_memory_reset(target="all", yes=True)
        assert result == "nothing"
