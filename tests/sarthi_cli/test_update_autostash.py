...
def _setup_update_mocks(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(sarthi_main, "_upgrade_pip_before_lazy_refresh", lambda *a, **kw: None)
    monkeypatch.setattr(sarthi_main, "_refresh_active_lazy_features", lambda *a, **kw: True)
...
def test_refresh_active_memory_provider_dependencies_reinstalls_active_provider(monkeypatch):
    ...
    monkeypatch.setattr(
        "sarthi_cli.memory_setup._install_dependencies",
        lambda provider_name, force=False: recorded.append((provider_name, force)),
    )

    sarthi_main._refresh_active_memory_provider_dependencies()
...
def test_reload_updated_runtime_modules_restores_new_sarthi_constants_symbol(monkeypatch):
    """A pre-pull module object missing a new helper is repaired by reload."""
    import sarthi_constants

    monkeypatch.delattr(sarthi_constants, "apply_subprocess_home_env", raising=False)
    assert not hasattr(sarthi_constants, "apply_subprocess_home_env")

    sarthi_main._reload_updated_runtime_modules()

    assert callable(sarthi_constants.apply_subprocess_home_env)
...
def _setup_keep_stash_test(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(
        sarthi_main, "_park_stashed_changes",
    ...
    monkeypatch.setattr(
        "sarthi_cli.gateway.find_gateway_pids", lambda **kw: [], raising=False
    )
...
def test_update_keep_stash_parks_instead_of_restoring(monkeypatch, tmp_path):
    ...
    sarthi_main.cmd_update(SimpleNamespace(yes=True, keep_stash=True))
...
def test_update_without_keep_stash_still_restores(monkeypatch, tmp_path):
    ...
    sarthi_main.cmd_update(SimpleNamespace(yes=True, keep_stash=False))
...
def test_update_autostash_survives_undeletable_untracked_dir(tmp_path):
    ...
    (pkg / "sarthi-agent.rb").write_text("formula\n")
