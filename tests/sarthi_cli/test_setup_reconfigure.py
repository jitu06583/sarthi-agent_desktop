...
- Bare `sarthi setup` drops straight into the full reconfigure wizard
  (every prompt shows the current value as its default).
- `sarthi setup --quick` runs the narrower "fill in missing items" flow.
- `sarthi setup --reconfigure` is a backwards-compat alias for the
  bare-setup default.
...
def _enter_existing_install_patches(stack, **extra):
    ...
    for target, kwargs in [
        ("sarthi_cli.setup.ensure_sarthi_home", {}),
        ("sarthi_cli.setup.is_interactive_stdin", {"return_value": True}),
        ("sarthi_cli.config.is_managed", {"return_value": False}),
        ("sarthi_cli.setup.load_config", {"return_value": {}}),
        ("sarthi_cli.setup.save_config", {}),
        ("sarthi_cli.setup.get_env_value", {"return_value": None}),
        ("sarthi_cli.auth.get_active_provider", {"return_value": "openrouter"}),
        ("sarthi_cli.setup._print_setup_summary", {}),
        ("sarthi_cli.setup._offer_openclaw_migration", {"return_value": False}),
    ]:
...
def _enter_fresh_install_patches(stack, **extra):
    for target, kwargs in [
        ...
        ("sarthi_cli.setup.load_config", {"return_value": {}}),
        ("sarthi_cli.setup.save_config", {}),
        ("sarthi_cli.auth.get_active_provider", {"return_value": None}),
        ("sarthi_cli.setup.get_env_value", {"return_value": None}),
        ("sarthi_cli.setup._offer_openclaw_migration", {"return_value": False}),
    ]:
        stack.enter_context(patch(target, **kwargs))
...
class TestExistingInstallDefault:
    """Bare `sarthi setup` on an existing install = full reconfigure wizard."""
    ...
    def test_bare_setup_runs_full_reconfigure_without_menu(self, existing_install):
        ...
        with ExitStack() as stack:
            m = _enter_existing_install_patches(
                stack,
                prompt_choice="sarthi_cli.setup.prompt_choice",
                quick="sarthi_cli.setup._run_quick_setup",
                model="sarthi_cli.setup.setup_model_provider",
                terminal="sarthi_cli.setup.setup_terminal_backend",
                agent="sarthi_cli.setup.setup_agent_settings",
                gateway="sarthi_cli.setup.setup_gateway",
                tools="sarthi_cli.setup.setup_tools",
            )
    ...
    def test_reconfigure_flag_is_backwards_compat_noop(self, existing_install):
        """`sarthi setup --reconfigure` behaves the same as bare `sarthi setup`."""
    ...
    def test_bare_setup_runs_first_time_flow(self, fresh_install):
        ...
        with ExitStack() as stack:
            m = _enter_fresh_install_patches(
                stack,
                prompt=("sarthi_cli.setup.prompt_choice", {"return_value": 0}),
                first="sarthi_cli.setup._run_first_time_quick_setup",
            )
            from sarthi_cli.setup import run_setup_wizard
            run_setup_wizard(args)
...
class TestArgparse:
    ...
    def test_reconfigure_flag_reaches_cmd_setup(self, monkeypatch):
        ...
        captured = {}
        monkeypatch.setattr(
            "sarthi_cli.setup.run_setup_wizard",
            lambda args: captured.setdefault("args", args),
        )
        monkeypatch.setattr(sys, "argv", ["sarthi", "setup", "--reconfigure"])
        try:
            main()
        except SystemExit:
            pass
    ...
    def test_quick_flag_reaches_cmd_setup(self, monkeypatch):
        ...
        monkeypatch.setattr(sys, "argv", ["sarthi", "setup", "--quick"])
        try:
            main()
        except SystemExit:
            pass
    ...
    def test_bare_setup_has_both_flags_false(self, monkeypatch):
        ...
        monkeypatch.setattr(sys, "argv", ["sarthi", "setup"])
        try:
            main()
        except SystemExit:
            pass
