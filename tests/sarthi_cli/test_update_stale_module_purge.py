"""Tests for _purge_stale_sarthi_modules — the class fix for stale
sys.modules breaking the gateway auto-restart after `sarthi update`.

Field failure (2026-08-20, Teknium's Linux box): `sarthi update` pulled a
checkout where sarthi_cli/gateway.py newly imports `line_input` from
sarthi_cli.cli_output, but the updater process had cli_output cached from
before that symbol existed. The function-level `from sarthi_cli.gateway
...
the Sarthi package prefixes so later imports rebuild a self-consistent
...
def _restore_sys_modules():
    ...
    The purge under test evicts real Sarthi modules from the cache; later
...
def test_purge_evicts_sarthi_prefixed_modules():
    victims = [
        "sarthi_cli.cli_output",
        "sarthi_cli.gateway",
        "gateway.status",
        "tools.ansi_strip",
        "tui_gateway.server",
    ...
    for name in victims:
        if name not in sys.modules:
            ...
            added.append(name)
    try:
        cli_main._purge_stale_sarthi_modules()
        for name in victims:
...
def test_purge_protects_executing_modules():
    ...
    cli_main._purge_stale_sarthi_modules()
    assert sys.modules.get("sarthi_cli.update_cmd") is update_cmd
    assert sys.modules.get("sarthi_cli.main") is cli_main
    assert "sarthi_cli" in sys.modules
...
def test_purge_never_raises_on_weird_sys_modules():
    ...
    sys.modules["sarthi_cli._purge_test_none"] = None  # type: ignore[assignment]
    try:
        cli_main._purge_stale_sarthi_modules()
    finally:
        sys.modules.pop("sarthi_cli._purge_test_none", None)
...
def test_stale_symbol_scenario_end_to_end():
    ...
    try:
        ...
        try:
            from sarthi_cli.cli_output import line_input  # noqa: F401
            raised = False
        except ImportError:
            raised = True
