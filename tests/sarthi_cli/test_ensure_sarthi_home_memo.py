"""ensure_sarthi_home is memoized per home path (perf: it runs on every
...
def test_distinct_home_paths_each_get_the_skeleton(tmp_path, monkeypatch):
    first = tmp_path / "a" / ".sarthi"
    second = tmp_path / "b" / ".sarthi"

    monkeypatch.setenv("SARTHI_HOME", str(first))
    cfg.ensure_sarthi_home()

    # Profile switch: SARTHI_HOME moves → the new path is ensured too.
    monkeypatch.setenv("SARTHI_HOME", str(second))
    cfg.ensure_sarthi_home()
