...
def test_single_handler_builders(name, builder, kw, argv):
    parser = argparse.ArgumentParser(prog="sarthi")
...
def test_config_get_unset_subcommands_parse():
    """`sarthi config get/unset` parse key args (and --json for get)."""
    parser = argparse.ArgumentParser(prog="sarthi")
    sub = parser.add_subparsers(dest="command")
    handler = _h("config")
...
# ── deprecated `sarthi login` fails gracefully, not with argparse error ────
#
# `sarthi login` is a removed command; its handler (`login_command` in
# `sarthi_cli/auth.py`) prints a deprecation notice pointing at `sarthi auth` /
# `sarthi model` and exits 0.  Two behavior contracts guard the UX:
...
def test_login_subparser_help_is_suppressed():
    """The deprecated `login` row must not appear in `sarthi --help`.
