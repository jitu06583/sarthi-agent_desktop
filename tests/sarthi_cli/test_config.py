...
class TestGetSarthiHome:
    ...
    def test_env_override(self):
        with patch.dict(os.environ, {"SARTHI_HOME": "/custom/path"}):
            home = get_sarthi_home()
...
class TestEnsureSarthiHome:
    ...
    def test_preserves_legacy_template_with_user_persona(self, tmp_path):
        ...
        from sarthi_cli.default_soul import _LEGACY_TEMPLATE_SOULS
    ...
    def test_existing_named_profile_still_bootstraps_subdirs(self, tmp_path):
        profile_home = tmp_path / ".sarthi" / "profiles" / "coder"
        profile_home.mkdir(parents=True)
        with patch.dict(os.environ, {"SARTHI_HOME": str(profile_home)}):
            ensure_sarthi_home()
...
class TestLoadConfigParseFailure:
    ...
    def test_logs_and_warns_on_parse_failure(self, tmp_path, caplog, capsys):
        ...
        with patch.dict(os.environ, {"SARTHI_HOME": str(tmp_path)}):
            ...
            import logging
            with caplog.at_level(logging.WARNING, logger="sarthi_cli.config"):
                config = load_config()
            ...
            # stands out at sarthi startup before logging is configured)
            captured = capsys.readouterr()
            assert "sarthi config:" in captured.err
    ...
    def test_rewarns_after_file_edit(self, tmp_path, capsys):
        ...
        with patch.dict(os.environ, {"SARTHI_HOME": str(tmp_path)}):
            ...
            assert "sarthi config:" in after_edit, "edited file should re-warn"
...
class TestRemoveEnvValue:
    ...
    def test_clears_os_environ(self, tmp_path):
        ...
        with patch.dict(os.environ, {"SARTHI_HOME": str(tmp_path), "MY_KEY": "my_value"}):
    ...
    def test_handles_missing_env_file(self, tmp_path):
        with patch.dict(os.environ, {"SARTHI_HOME": str(tmp_path), "GHOST_KEY": "ghost"}):
...
class TestEnvWriteDenylist:
    ...
    def test_arbitrary_user_key_still_works(self):
        ...
        denylist and outside ``SARTHI_*``) keep working. The denylist
