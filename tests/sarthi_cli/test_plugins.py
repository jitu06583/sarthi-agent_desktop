"""Tests for the Sarthi plugin system (sarthi_cli.plugins)."""
...
from sarthi_cli.relay_plugin_cutover import RELAY_PLUGINS_CONFIG_ENV
from sarthi_cli.middleware import (
    VALID_MIDDLEWARE,
...
def _make_plugin_dir(base: Path, name: str, *, register_body: str = "pass",
    ...
    ``<sarthi_home>/config.yaml`` under ``plugins.enabled``. Plugins are
    ...
    *base* is expected to be ``<sarthi_home>/plugins/``; we derive
    ``<sarthi_home>`` from it by walking one level up unless *home* is
    given explicitly.

    Pass *home* explicitly whenever the target Sarthi home for this
    plugin isn't necessarily the current ``SARTHI_HOME`` env var — e.g.
    when writing fixtures for two profiles up front and only switching
    ``SARTHI_HOME``/``set_sarthi_home_override()`` per-profile afterwards
    ...
    if auto_enable:
        # Write/merge plugins.enabled in <SARTHI_HOME>/config.yaml.
        # Config is always read from SARTHI_HOME (not from the project
        # dir for project plugins), so that's where we opt in.
        if home is not None:
            sarthi_home = Path(home)
        else:
            import os
            sarthi_home_str = os.environ.get("SARTHI_HOME")
            if sarthi_home_str:
                sarthi_home = Path(sarthi_home_str)
            else:
                sarthi_home = base.parent
        sarthi_home.mkdir(parents=True, exist_ok=True)
        cfg_path = sarthi_home / "config.yaml"
        cfg: dict = {}
        if cfg_path.exists():
            try:
...
class TestPluginDiscovery:
    ...
    def test_removed_relay_plugin_identity_cannot_be_reloaded(
        ...
        from sarthi_cli import plugins as plugins_mod
        ...
        assert "Relay lifecycle is owned by Sarthi core" in state.error
        assert RELAY_PLUGINS_CONFIG_ENV in state.error
        assert "Refusing to load removed Sarthi Relay plugin" in caplog.text

    def test_enabled_portable_plugin_registers_components(
        self, tmp_path, monkeypatch
    ):
        from sarthi_cli.agent_plugins import MCP_SCHEMA_V1, PLUGIN_SCHEMA_V1
        from sarthi_cli import plugins as plugins_mod
    ...
    def test_disabled_portable_plugin_registers_nothing(self, tmp_path, monkeypatch):
        from sarthi_cli.agent_plugins import PLUGIN_SCHEMA_V1
        from sarthi_cli import plugins as plugins_mod
    ...
    def test_portable_author_object_is_normalized_to_stable_string(
        ...
        bundled.mkdir()
        monkeypatch.setenv("SARTHI_HOME", str(home))
        monkeypatch.setenv("SARTHI_BUNDLED_PLUGINS", str(bundled))
    ...
    def test_plugin_can_register_and_invoke_middleware(self, tmp_path, monkeypatch):
        plugins_dir = tmp_path / "sarthi_test" / "plugins"
        _make_plugin_dir(
            plugins_dir,
            "mw_plugin",
            register_body=(
        ...
        monkeypatch.setenv("SARTHI_HOME", str(tmp_path / "sarthi_test"))
    ...
    def test_middleware_helpers_skip_no_listener_work(self, monkeypatch):
        ...
        monkeypatch.setattr("sarthi_cli.plugins.get_plugin_manager", lambda: manager)
...
class TestPluginLoading:
    ...
    def test_load_registers_namespace_module(self, tmp_path, monkeypatch):
        """Directory plugins are importable under sarthi_plugins.<name>."""
        plugins_dir = tmp_path / "sarthi_test" / "plugins"
        _make_plugin_dir(plugins_dir, "ns_plugin")
        monkeypatch.setenv("SARTHI_HOME", str(tmp_path / "sarthi_test"))

        # Clean up any prior namespace module
        sys.modules.pop("sarthi_plugins.ns_plugin", None)

        mgr = PluginManager()
        mgr.discover_and_load()

        assert "sarthi_plugins.ns_plugin" in sys.modules
    ...
    def test_user_memory_plugin_auto_coerced_to_exclusive(self, tmp_path, monkeypatch):
        ...
        sarthi_home = tmp_path / "sarthi_test"
    ...
    def test_entrypoint_memory_provider_auto_coerced_to_exclusive(
        ...
        ``sarthi_agent.plugins`` entry point but exposing
        ...
        monkeypatch.setattr(
            "sarthi_cli.plugins.importlib.metadata.entry_points",
    ...
    def test_entrypoint_duplicate_does_not_block_directory_provider_activation(
        ...
        # Same-name directory provider under $SARTHI_HOME/plugins/.
        sarthi_home = tmp_path / "sarthi_test"
        plugins_dir = sarthi_home / "plugins"
...
class TestDeliveryParity:
    ...
    def _fresh_manager(self, monkeypatch, register_body):
        ...
        import sarthi_cli.plugins as plugins_mod
...
class TestForceReloadSymmetry:
    ...
    def test_re_register_config_hooks_clears_idempotence_set(self, monkeypatch):
        ...
        monkeypatch.setattr(
            "sarthi_cli.config.load_config", lambda: {"hooks": {}}
        )
...
class TestPreToolCallBlocking:
    ...
    def test_block_message_returned_for_valid_directive(self, monkeypatch):
        monkeypatch.setattr(
            "sarthi_cli.plugins.invoke_hook",
...
class TestPreToolCallDirective:
    ...
    def test_first_party_observer_receives_pre_tool_call(self, monkeypatch):
        from sarthi_cli import observability
        from sarthi_cli.plugins import get_pre_tool_call_directive
...
class TestResolvePreToolBlock:
    ...
    def test_approve_gate_receives_tool_observability_context(self, monkeypatch):
        from sarthi_cli.plugins import resolve_pre_tool_block
...
class TestGetPreVerifyContinueMessage:
    ...
    def test_none_when_no_hooks(self, monkeypatch):
        monkeypatch.setattr("sarthi_cli.plugins.invoke_hook", lambda hook_name, **kwargs: [])
    ...
    def test_forwards_scope_signals_to_hooks(self, monkeypatch):
        ...
        monkeypatch.setattr("sarthi_cli.plugins.invoke_hook", capture)
...
class TestPluginContext:
    ...
    def test_register_tool_override_blocked_without_operator_opt_in(self, tmp_path, monkeypatch):
        ...
        from sarthi_cli.plugins import PluginToolOverrideError
        ...
        try:
            ...
            from sarthi_cli.plugins import PluginContext, PluginManifest
    ...
    def test_register_tool_override_blocked_via_delayed_callback(self, tmp_path, monkeypatch):
        ...
        try:
            ...
            import sys as _sys
            mod = _sys.modules.get("sarthi_plugins.delayed_override_plugin")
...
class TestPluginManagerList:
    ...
    def test_shared_hook_name_credited_to_every_plugin(self, tmp_path, monkeypatch):
        ...
        `sarthi plugins list`. Attribution now counts what each plugin's own
...
class TestPluginCommands:
    ...
    def test_register_command_empty_name_rejected(self, caplog):
        ...
        with caplog.at_level(logging.WARNING, logger="sarthi_cli.plugins"):
    ...
    def test_plugin_manager_scoped_by_sarthi_home_override(self, tmp_path):
        """set_sarthi_home_override() must get its own manager per profile.

        This is the production path used by the gateway multiplexer
        (``gateway/run.py``'s ``_profile_scope`` context manager) and by
        subagent/embedded callers: it swaps ``SARTHI_HOME`` via a
        context-local ContextVar, which — per
        ``sarthi_constants.set_sarthi_home_override`` — deliberately does
        NOT touch ``os.environ``. A regression test that only flips the
        ``SARTHI_HOME`` env var never exercises this path.
        """
        ...
        def write_engine_plugin(home: Path) -> None:
            _make_plugin_dir(
                ...
                register_body=(
                    ...
                    "        def __init__(self):\n"
                    "            self.home = os.environ.get('SARTHI_HOME')\n\n"
                    "        @property\n"
                    "        def name(self):\n"
                    "            return 'home-engine'\n\n"
                    "        def update_from_response(self, usage):\n"
        ...
        # Note: HomeEngine reads os.environ['SARTHI_HOME'] itself (simulating
        # a real plugin like sarthi-lcm capturing its home at registration),
        ...
        # not last-write-wins.
        token_a2 = set_sarthi_home_override(str(home_a))
        try:
            manager_a2 = plugins_mod.get_plugin_manager()
        finally:
            reset_sarthi_home_override(token_a2)
    ...
    def test_relative_import_not_leaked_across_home_switch(self, tmp_path):
        ...
        ``sarthi_plugins.<slug>``, but a relative import inside the
        plugin's ``__init__.py`` (``from .state import STATE``) is cached
        separately in ``sys.modules`` under
        ``sarthi_plugins.<slug>.state``. If a profile switch replaces only
        ...
        def write_stateful_plugin(home: Path, marker: str) -> None:
            ...
            # `sarthi_plugins.stateful_plugin.state`.
...
class TestPluginCommandResultResolution:
    ...
    def test_awaits_async_result_with_running_loop(self, monkeypatch):
        ...
        monkeypatch.setattr("sarthi_cli.plugins.asyncio.get_running_loop", lambda: _Loop())
    ...
    def test_running_loop_timeout_does_not_hang_forever(self, monkeypatch):
        ...
        monkeypatch.setattr("sarthi_cli.plugins.asyncio.get_running_loop", lambda: _Loop())
        monkeypatch.setattr("sarthi_cli.plugins._PLUGIN_COMMAND_AWAIT_TIMEOUT_SECS", 0.1)
...
class TestPluginDispatchTool:
    ...
    def test_dispatch_tool_calls_registry(self):
        ...
        with patch("sarthi_cli.plugins.PluginContext.dispatch_tool.__module__", "sarthi_cli.plugins"):
...
class TestPluginDebugLogging:
    """SARTHI_PLUGINS_DEBUG opt-in stderr handler for plugin developers."""
    ...
    def test_debug_handler_not_installed_when_env_var_absent(self, monkeypatch):
        ...
        monkeypatch.delenv("SARTHI_PLUGINS_DEBUG", raising=False)
        from sarthi_cli import plugins as plugins_mod
...
class TestPluginContextProfileName:
    """ctx.profile_name resolves from SARTHI_HOME in every context."""
    ...
    def test_default_profile(self, tmp_path, monkeypatch):
        """SARTHI_HOME at the root resolves to 'default'."""
    ...
    def test_named_profile(self, tmp_path, monkeypatch):
        """SARTHI_HOME under profiles/<name> resolves to that name."""
        prof = tmp_path / ".sarthi" / "profiles" / "coder"
        prof.mkdir(parents=True)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("SARTHI_HOME", str(prof))
