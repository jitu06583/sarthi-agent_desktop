...
def _no_real_gateway_service(monkeypatch):
    ...
    assert the wiring."""
    import sarthi_cli.gateway as gateway_mod
...
def _advance_backup_clock(seconds: float = 1.1) -> None:
    """Skew sarthi_cli.backup's datetime forward instead of sleeping.
    ...
    import sarthi_cli.backup as _backup

    shim = getattr(_backup.datetime, "_sarthi_test_shim", None)
    if shim is None:
        class _ShimDatetime(_dt.datetime):
            _sarthi_test_shim = True
...
def _make_sarthi_tree(root: Path) -> None:
    ...
    for db_name in ("memory_store.db", "sarthi_state.db"):
...
class TestShouldExclude:
    ...
    def test_excludes_state_snapshots_dir(self):
        ...
        from sarthi_cli.backup import _EXCLUDED_DIRS, _QUICK_SNAPSHOTS_DIR, _should_exclude
...
class TestBackup:
    ...
    def test_state_snapshots_not_nested_into_backup(self, tmp_path, monkeypatch):
        ...
        sarthi_home.mkdir()
        _make_sarthi_tree(sarthi_home)
        with sqlite3.connect(sarthi_home / "state.db") as conn:
            conn.execute("CREATE TABLE sessions (id TEXT PRIMARY KEY)")
            conn.execute("INSERT INTO sessions VALUES ('s1')")

        from sarthi_cli.backup import _QUICK_SNAPSHOTS_DIR, create_quick_snapshot, run_backup
        ...
        assert snap_id and (sarthi_home / _QUICK_SNAPSHOTS_DIR / snap_id / "state.db").exists()
...
class TestImport:
    ...
    def test_import_survives_service_layer_import_failure(self, tmp_path, monkeypatch, capsys):
        ...
        assert "Done. Your Sarthi configuration has been restored." in out
...
class TestImportAtomicWrites:
    """`sarthi import` must never leave a user's file truncated.
    ...
    tests pin the invariant for both restore branches: the SARTHI_HOME branch
    ...
    def test_failed_member_leaves_existing_file_intact(self, tmp_path, monkeypatch):
        ...
        (sarthi_home / "config.yaml").write_text(original)
        monkeypatch.setenv("SARTHI_HOME", str(sarthi_home))
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        ...
        assert (sarthi_home / "config.yaml").read_text() == original
        # And the aborted write must not litter the directory it staged in.
        assert list(sarthi_home.glob(".config.yaml.*")) == []

    def test_failed_external_member_leaves_existing_file_intact(self, tmp_path, monkeypatch):
        """Same invariant on the `_external/` branch, which writes outside SARTHI_HOME."""
    ...
    def test_restore_preserves_existing_file_mode(self, tmp_path, monkeypatch):
        ...
        sarthi_home = tmp_path / ".sarthi"
        sarthi_home.mkdir()
        target = sarthi_home / "config.yaml"
    ...
    def test_restore_preserves_existing_file_owner(self, tmp_path, monkeypatch):
        ...
        ``sudo sarthi import`` onto a user-owned (or Docker/NAS volume-owned)
        SARTHI_HOME would hand every restored file to root. The uid/gid is
        ...
        monkeypatch.setattr(
            "sarthi_cli.backup._preserve_file_owner",
    ...
    def test_restore_does_not_carry_setuid_onto_archive_content(
        ...
        re-serialize content this process produced; ``sarthi import`` is the
...
class TestSafeCopyDb:
    ...
    def test_aborts_when_source_remains_busy_past_deadline(
        ...
        from sarthi_cli import backup as backup_mod
    ...
    def test_is_zeroed_sqlite_file_detects_nul_header(self, tmp_path):
        from sarthi_cli.backup import is_zeroed_sqlite_file
        p = tmp_path / "state.db"
...
class TestQuickSnapshot:
    ...
    def test_failed_state_db_copy_is_loud(self, sarthi_home, monkeypatch, capsys):
        ...
        snap_id = backup_mod.create_quick_snapshot(sarthi_home=sarthi_home)
        err = capsys.readouterr().out
        assert "SQLite safe copy FAILED" in err or "CRITICAL" in err
        assert "state.db" in err
        # Other small files may still snapshot
        if snap_id:
            manifest = (sarthi_home / "state-snapshots" / snap_id / "manifest.json")
...
# ---------------------------------------------------------------------------
    ...
    def test_oversized_db_suppresses_pruning(self, sarthi_home, capsys):
        ...
        first_id = create_quick_snapshot(label="complete", sarthi_home=sarthi_home)
        assert first_id is not None
        first_dir = sarthi_home / "state-snapshots" / first_id
        ...
        second_id = create_quick_snapshot(
            label="oversized", sarthi_home=sarthi_home, max_file_size=1024, keep=1
        )
        assert second_id is not None
        second_dir = sarthi_home / "state-snapshots" / second_id
        ...
        all_snaps = list_quick_snapshots(limit=100, sarthi_home=sarthi_home)
...
class TestPreUpdateBackup:
    ...
    def test_pre_update_zip_does_not_nest_the_pre_update_snapshot(self, sarthi_home):
        """``sarthi update`` in ``full`` mode takes the quick snapshot *before*
        ...
        snap_id = create_quick_snapshot(label="pre-update", sarthi_home=sarthi_home)
        assert snap_id and (sarthi_home / _QUICK_SNAPSHOTS_DIR / snap_id / "state.db").exists()
...
class TestRunPreUpdateBackup:
    ...
    def sarthi_home(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        # Config reads resolve SARTHI_HOME dynamically and their caches are
    ...
    @staticmethod
    def _set_mode(sarthi_home, value):
        import yaml
    ...
    @staticmethod
    def _zips(sarthi_home):
        d = sarthi_home / "backups"
        return list(d.glob("pre-update-*.zip")) if d.exists() else []

    @staticmethod
    def _snaps(sarthi_home):
        d = sarthi_home / "state-snapshots"
        return [p for p in d.iterdir() if p.is_dir()] if d.exists() else []




    def test_config_off_disables_everything_silently(self, sarthi_home, capsys):
        ...
        self._set_mode(sarthi_home, "off")
        ...
        assert out == ""
        assert not self._snaps(sarthi_home)
        assert not self._zips(sarthi_home)



    def test_config_full_mode(self, sarthi_home, capsys):
        self._set_mode(sarthi_home, "full")
        from sarthi_cli.main import _run_pre_update_backup
        snap_id = _run_pre_update_backup(Namespace(no_backup=False, backup=False))
        out = capsys.readouterr().out
        assert snap_id is not None
        ...
        assert len(self._zips(sarthi_home)) == 1
