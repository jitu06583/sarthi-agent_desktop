"""``sarthi config set`` must parse list/mapping literals, not store them as strings.

Before this fix, ``sarthi config set platform_toolsets.discord '["file","web"]'``
...
def test_round_trip_through_load_config(user_home):
    ...
    from sarthi_cli.config import set_config_value, load_config
