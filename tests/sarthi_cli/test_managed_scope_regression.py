...
def sarthi_home(tmp_path, monkeypatch):
    ...
    # /etc/sarthi on the dev/CI box can't influence the test.
    monkeypatch.setenv("SARTHI_MANAGED_DIR", str(tmp_path / "no_such_managed_dir"))
...
def test_user_config_overrides_default(sarthi_home, monkeypatch):
...
def test_env_expansion_in_user_config(sarthi_home, monkeypatch):
    from sarthi_cli.config import load_config, cfg_get
...
def test_no_managed_dir_means_user_value_wins(sarthi_home):
...
def test_user_env_overrides_shell(tmp_path, monkeypatch):
    ...
    load_sarthi_dotenv(sarthi_home=str(home))
