...
Before the fix, ``sarthi whatsapp`` wrote ``WHATSAPP_ENABLED=true`` at
...
``sarthi gateway`` then paid a 30s bridge-bootstrap timeout and queued
...
def isolated_home(tmp_path, monkeypatch):
    home = tmp_path / "home"
    sarthi = home / ".sarthi"
    sarthi.mkdir(parents=True)
    monkeypatch.setattr(Path, "home", lambda: home)
    monkeypatch.setenv("SARTHI_HOME", str(sarthi))
...
def _env_value(sarthi_home: Path, key: str) -> str | None:
...
def test_aborted_setup_does_not_enable_whatsapp(isolated_home, monkeypatch):
    ...
    """
    from sarthi_cli.main import cmd_whatsapp
    ...
    monkeypatch.setattr("sarthi_cli.main._require_tty", lambda *_a, **_kw: None)
...
def test_existing_pairing_skip_branch_enables_whatsapp(isolated_home, monkeypatch):
    """User runs ``sarthi whatsapp`` with an existing paired session and
