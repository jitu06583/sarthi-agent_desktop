"""Tests for IRC gateway configuration via `sarthi setup gateway` UI.

Covers the full plugin-platform discovery → status → configure flow so that
a fresh Sarthi install (no state, no env vars) can set up IRC through the
...
class TestIRCFreshInstallDiscovery:
    """IRC appears in the setup menu on a brand-new Sarthi install."""
    ...
    def test_irc_status_configured_when_env_set(self, monkeypatch):
        ...
        try:
            ...
            monkeypatch.setenv("IRC_CHANNEL", "#sarthi")
            monkeypatch.setenv("IRC_NICKNAME", "sarthi-bot")
...
class TestIRCGatewaySetupFreshInstall:
    """Simulate the full `sarthi setup gateway` experience with IRC present."""
