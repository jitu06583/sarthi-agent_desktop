...
def test_sessions_delete_accepts_unique_id_prefix(monkeypatch, capsys):
    ...
    monkeypatch.setattr(
        sys,
        "argv",
        ["sarthi", "sessions", "delete", "20260315_092437_c9a6", "--yes"],
    )
...
def test_sessions_delete_reports_not_found_when_prefix_is_unknown(monkeypatch, capsys):
    ...
    monkeypatch.setattr(
        sys,
        "argv",
        ["sarthi", "sessions", "delete", "missing-prefix", "--yes"],
    )
...
def test_sessions_delete_handles_eoferror_on_confirm(monkeypatch, capsys):
    ...
    monkeypatch.setattr(
        sys, "argv",
        ["sarthi", "sessions", "delete", "20260315_092437_c9a6"],
    )
...
def _run_prune(monkeypatch, capsys, argv_tail, candidates=None):
    """Run `sarthi sessions prune <argv_tail>` against a FakeDB, capturing
    ...
    monkeypatch.setattr(sarthi_state, "SessionDB", lambda: FakeDB())
    monkeypatch.setattr(
        sys, "argv", ["sarthi", "sessions", "prune", *argv_tail]
    )
...
def test_sessions_prune_bare_keeps_90_day_default(monkeypatch, capsys):
    """A truly bare `sarthi sessions prune` keeps the implicit 90-day cutoff."""
...
def test_sessions_prune_preview_shows_oldest_newest(monkeypatch, capsys):
    ...
    from sarthi_cli.session_filters import format_epoch
