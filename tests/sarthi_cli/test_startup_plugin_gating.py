...
``sarthi_cli.main`` skips eager plugin discovery at argparse-setup time
when the invocation is clearly targeting a known built-in subcommand.
This saves 500-650ms on ``sarthi --help``, ``sarthi version``,
``sarthi logs``, etc., by not importing ``google.cloud.pubsub_v1``,
...
def _live_subcommand_names() -> set[str]:
    """Run ``sarthi --help`` in-process and parse the subcommand block.
    ...
    built-in-only set.
    """
    from sarthi_cli import main as _main

    argv_backup = sys.argv[:]
    sys.argv = ["sarthi", "--help"]
    buf = io.StringIO()
    try:
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (["sarthi", "-w", "--tui"], None),
        (["sarthi", "version"], "version"),
        (["sarthi", "--tui", "chat"], "chat"),
        (["sarthi", "-w", "logs"], "logs"),
        (["sarthi", "chat", "hello world"], "chat"),
        (["sarthi", "gateway", "run"], "gateway"),
        # Top-level value-taking flags: the value should be skipped.
        (["sarthi", "-m", "gpt5", "chat"], "chat"),
        (["sarthi", "--model", "gpt5", "chat", "hi"], "chat"),
        (["sarthi", "-m", "gpt5", "--provider", "openai", "chat"], "chat"),
        (["sarthi", "-z", "hello world"], None),
        (["sarthi", "-z", "hello", "chat"], "chat"),
        (["sarthi", "--model=gpt5", "chat"], "chat"),     # inline form
        (["sarthi", "--", "chat"], "chat"),               # -- terminator
        (["sarthi", "-w", "--"], None),
        # Unknown positional after skipped flags → plugin-cmd candidate.
        (["sarthi", "some-plugin-cmd"], "some-plugin-cmd"),
        (["sarthi", "-m", "gpt5", "some-plugin-cmd"], "some-plugin-cmd"),
    ],
)
...
@pytest.mark.parametrize(
    ...
    [
        ["sarthi"],                          # bare → chat
        ["sarthi", "--help"],                # top-level help
        ["sarthi", "-h"],
        ["sarthi", "version"],               # known built-in
        ["sarthi", "logs"],
        ["sarthi", "gateway", "run"],
        ["sarthi", "--tui"],
        ["sarthi", "-w", "--tui"],
        ["sarthi", "chat", "hi"],
        ["sarthi", "help"],                  # accepted built-in-ish
        ["sarthi", "-m", "gpt5", "chat"],    # flag-value-skipping
    ],
)
...
@pytest.mark.parametrize(
    "argv",
    [
        ["sarthi", "meet", "join"],          # potential google_meet plugin
        ["sarthi", "honcho", "status"],      # potential memory plugin
        ["sarthi", "unknown-subcmd"],
    ],
)
...
def test_builtin_set_covers_every_registered_subcommand():
    ...
    # in the set so ``sarthi help <cmd>`` skips discovery; it won't show
    ...
    assert not missing_from_declaration, (
        ...
        f"sarthi_cli/main.py::_BUILTIN_SUBCOMMANDS so plugin discovery "
