"""Tests for Codex auth — tokens stored in Sarthi auth store (~/.sarthi/auth.json)."""
...
def _setup_sarthi_auth(sarthi_home: Path, *, access_token: str = "access", refresh_token: str = "refresh"):
    """Write Codex tokens into the Sarthi auth store."""
...
def test_read_codex_tokens_success(tmp_path, monkeypatch):
    ...
    _setup_sarthi_auth(sarthi_home)
...
def test_resolve_codex_runtime_credentials_refreshes_expiring_token(tmp_path, monkeypatch):
    ...
    _setup_sarthi_auth(sarthi_home, access_token=expiring_token, refresh_token="refresh-old")
    monkeypatch.setenv("SARTHI_HOME", str(sarthi_home))
    ...
    monkeypatch.setattr("sarthi_cli.auth._refresh_codex_auth_tokens", _fake_refresh)
...
def test_resolve_codex_runtime_credentials_force_refresh(tmp_path, monkeypatch):
    sarthi_home = tmp_path / "sarthi"
    _setup_sarthi_auth(sarthi_home, access_token="access-current", refresh_token="refresh-old")
    monkeypatch.setenv("SARTHI_HOME", str(sarthi_home))
...
def test_save_codex_tokens_syncs_credential_pool(tmp_path, monkeypatch):
    ...
    auth = json.loads((sarthi_home / "auth.json").read_text())
...
def test_save_codex_tokens_syncs_manual_device_code_entries(tmp_path, monkeypatch):
    ...
    fix landed would have run ``sarthi auth add openai-codex`` as a
    ...
    clobbered independent accounts added via ``sarthi auth add openai-codex``.
    ...
    (sarthi_home / "auth.json").write_text(json.dumps({
        ...
        "credential_pool": {
            "openai-codex": [
                ...
                # Independent account from `sarthi auth add openai-codex` —
...
def test_save_codex_tokens_does_not_overwrite_independent_manual_entries(tmp_path, monkeypatch):
    ...
    Regression for #39236: ``sarthi auth add openai-codex`` for accounts B and C
    ...
    (sarthi_home / "auth.json").write_text(json.dumps({
        ...
        "credential_pool": {
            "openai-codex": [
                ...
                # ``sarthi auth add openai-codex`` (account B and account C).
...
def test_save_codex_tokens_still_refreshes_legacy_manual_alias(tmp_path, monkeypatch):
    ...
    ``sarthi auth add openai-codex`` as a workaround when there was no
...
def test_codex_tokens_not_written_to_shared_file(tmp_path, monkeypatch):
    """Verify _save_codex_tokens writes only to Sarthi auth store, not ~/.codex/."""
    sarthi_home = tmp_path / "sarthi"
    codex_home = tmp_path / "codex-cli"
    sarthi_home.mkdir(parents=True, exist_ok=True)
    codex_home.mkdir(parents=True, exist_ok=True)
    ...
    _save_codex_tokens({"access_token": "sarthi-at", "refresh_token": "sarthi-rt"})

    # ~/.codex/auth.json should NOT exist — _save_codex_tokens only touches Sarthi store
    assert not (codex_home / "auth.json").exists()

    # Sarthi auth store should have the tokens
    data = _read_codex_tokens()
    assert data["tokens"]["access_token"] == "sarthi-at"


def test_resolve_returns_sarthi_auth_store_source(tmp_path, monkeypatch):
    sarthi_home = tmp_path / "sarthi"
    _setup_sarthi_auth(sarthi_home)
    monkeypatch.setenv("SARTHI_HOME", str(sarthi_home))
...
def test_refresh_429_classified_as_quota_not_auth_failure(monkeypatch):
    ...
    than a misleading "run sarthi auth".
    """
    from sarthi_cli.auth import (
    ...
    assert "sarthi auth" not in rendered
...
def test_refresh_429_without_retry_after_header(monkeypatch):
    ...
    from sarthi_cli.auth import CODEX_RATE_LIMITED_CODE
...
def test_is_rate_limited_auth_error_distinguishes_credential_errors():
    ...
    from sarthi_cli.auth import CODEX_RATE_LIMITED_CODE, is_rate_limited_auth_error
...
def test_login_openai_codex_force_new_login_skips_existing_reuse_prompt(monkeypatch):
    ...
    monkeypatch.setattr(
        "sarthi_cli.auth.resolve_codex_runtime_credentials",
    ...
    monkeypatch.setattr("sarthi_cli.auth._save_codex_tokens", _fake_save)
    monkeypatch.setattr("sarthi_cli.auth._update_config_for_provider", lambda *args, **kwargs: "/tmp/config.yaml")
    monkeypatch.setattr(
        "builtins.input",
...
def _patch_httpx_post(monkeypatch, responses):
    """Patch sarthi_cli.auth.httpx.Client so .post() returns queued responses."""
    seq = iter(responses)
    ...
    monkeypatch.setattr("sarthi_cli.auth.httpx.Client", lambda *a, **k: _FakeClient())
