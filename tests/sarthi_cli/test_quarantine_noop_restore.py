...
``sarthi*.exe`` shims aside (``sarthi.exe.old.<ms>``) before invoking the
...
quarantined shims stayed renamed aside and ``sarthi`` vanished from PATH
...
def _make_scripts_dir(tmp_path: Path) -> Path:
    ...
    for name in ("sarthi", "sarthi-agent", "sarthi-acp", "sarthi-gateway"):
...
# ---------------------------------------------------------------------------
# sarthi_cli.main._run_quarantined_install
# ---------------------------------------------------------------------------
...
def test_main_noop_success_restores_shims(tmp_path):
    ...
    assert "sarthi.exe" in names, "sarthi.exe must be restored after a no-op install"
    assert "sarthi-acp.exe" in names
    assert "sarthi-gateway.exe" in names
    assert (scripts / "sarthi.exe").read_bytes() == b"MZ-old-sarthi"
...
def test_main_rewriting_success_keeps_fresh_shims(tmp_path):
    ...
    assert (scripts / "sarthi.exe").read_bytes() == b"MZ-new-sarthi"
...
# ---------------------------------------------------------------------------
# sarthi_cli._install_repair._run_install_cmd (the deferred-recovery path)
# ---------------------------------------------------------------------------
...
def test_repair_noop_success_restores_shims(tmp_path):
    ...
    names = _shim_names(scripts)
    assert "sarthi.exe" in names, "sarthi.exe must be restored after a no-op recovery install"
    assert (scripts / "sarthi.exe").read_bytes() == b"MZ-old-sarthi"
...
def test_repair_rewriting_success_keeps_fresh_shims(tmp_path):
    ...
    def fake_run(cmd, cwd=None, check=None, env=None):
        (scripts / "sarthi.exe").write_bytes(b"MZ-new-sarthi")
