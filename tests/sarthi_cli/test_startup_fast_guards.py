"""Guards for sarthi_cli._startup_fast — the pre-import version fast path.
...
1. IMPORT WEIGHT: _startup_fast must stay stdlib-only. The whole point of
   ...
   ``from sarthi_cli.config import ...`` silently makes `sarthi --version`
...
def test_startup_fast_import_weight():
    ...
    probe = (
        "import sys, json\n"
        "import sarthi_cli._startup_fast\n"
    ...
    assert not offenders, (
        f"sarthi_cli._startup_fast imported heavy modules: {offenders} — "
...
def _run_version(env_overrides: dict) -> subprocess.CompletedProcess:
    ...
    env.pop("SARTHI_DEV", None)
    return subprocess.run(
        [sys.executable, "-m", "sarthi_cli.main", "--version"],
        capture_output=True,
        text=True,
        timeout=60,
        cwd=REPO_ROOT,
...
def test_fast_version_parity_off_termux(tmp_path):
    ...
    home.mkdir()
    result = _run_version({"SARTHI_HOME": str(home), "TERMUX_VERSION": ""})
    assert result.returncode == 0, result.stderr
    out = result.stdout
    for field in ("Sarthi Agent v", "Install directory:", "Python:", "OpenAI SDK:"):
...
def test_fast_version_parity_on_termux(tmp_path):
    ...
    home.mkdir()
    result = _run_version(
        {"SARTHI_HOME": str(home), "TERMUX_VERSION": "0.118"}
    )
    assert result.returncode == 0, result.stderr
    assert "Sarthi Agent v" in result.stdout
