...
class TestSlackNativeSlashes:
    ...
    def test_sarthi_catchall_is_first(self):
        """``/sarthi`` must be reserved as the first slot so the legacy
        ``/sarthi <subcommand>`` form keeps working after we add new
        commands and hit the 50-slash cap."""
        slashes = slack_native_slashes()
        assert slashes[0][0] == "sarthi"
    ...
    def test_excludes_slack_reserved_commands(self):
        ...
        Users can still reach them via /sarthi <command>."""
        names = {n for n, _d, _h in slack_native_slashes()}
    ...
    def test_includes_aliases_as_first_class_slashes(self):
        ...
        aliases stay reachable via ``/sarthi <alias>``.
        """
        slashes = slack_native_slashes()
    ...
    def test_telegram_parity(self):
        ...
        # Commands deliberately routed through /sarthi <command> on Slack only
        # (Slack's 50-slash cap) are expected to be absent from native slashes.
        via_sarthi_norm = {_norm(n) for n in _SLACK_VIA_SARTHI_ONLY}
        missing = (tg_norm - slack_norm) - reserved_norm - via_sarthi_norm
        assert not missing, (
...
class TestSlackAppManifest:
    """Generated Slack app manifest (used by `sarthi slack manifest`)."""
...
class TestSubcommandCompletion:
    ...
    def test_tools_enable_completes_toolset_names(self, monkeypatch):
        ...
        from sarthi_cli import commands as commands_mod
        ...
        monkeypatch.setattr(
            "sarthi_cli.tools_config._get_plugin_toolset_keys",
            lambda: set(),
        )
...
from sarthi_cli.commands import discord_skill_commands_by_category  # noqa: E402
...
class TestPluginCommandEnumeration:
    ...
    def _patch_plugin_commands(self, monkeypatch, commands):
        """Monkeypatch sarthi_cli.plugins.get_plugin_commands() to a fixed dict."""
        from sarthi_cli import plugins as _plugins_mod
    ...
    def test_plugin_command_appears_in_slack_subcommand_map(self, monkeypatch):
        """/sarthi metricas must route through the Slack subcommand map."""
        self._patch_plugin_commands(monkeypatch, {
            "metricas": {
                "handler": lambda _a: "ok",
                "description": "Metrics",
