...
the SAME provider universe as ``sarthi model`` (the CLI/TUI picker).
...
def test_every_sarthi_model_provider_is_configurable_in_desktop():
    """PARITY CONTRACT: GUI (keys ∪ accounts) ⊇ `sarthi model` universe."""
    ...
    assert not missing, (
        "providers shown in `sarthi model` but not configurable in the desktop "
