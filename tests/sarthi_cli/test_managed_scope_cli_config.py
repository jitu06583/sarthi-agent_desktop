...
sarthi_cli.config._load_config_impl, so the managed-scope merge has to be
applied in BOTH places or the interactive CLI/TUI surface (skin, display prefs)
silently ignores administrator-pinned values while `sarthi config`/`doctor`
...
def _load_cli_config(home):
    ...
    cli.py binds ``_sarthi_home = get_sarthi_home()`` at import time (module
    singleton), so monkeypatching SARTHI_HOME after import doesn't move it.
    ...
    this only matters for tests that swap SARTHI_HOME.)
    """
    import cli

    cli._sarthi_home = home
