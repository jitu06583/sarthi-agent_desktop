...
Covers the three paths changed by fix/bedrock-provider-model-ids-live-discovery:
  ...
  2. list_authenticated_providers() Section 2 (SARTHI_OVERLAYS) — bedrock
...
# ---------------------------------------------------------------------------
# 2. list_authenticated_providers() — bedrock via SARTHI_OVERLAYS (Section 2)
# ---------------------------------------------------------------------------
...
class TestBedrockOverlayRegistration:
    """bedrock entry in SARTHI_OVERLAYS is correctly configured."""

    def test_bedrock_overlay_exists(self):
        from sarthi_cli.providers import SARTHI_OVERLAYS
        assert "bedrock" in SARTHI_OVERLAYS

    def test_bedrock_overlay_transport(self):
        from sarthi_cli.providers import SARTHI_OVERLAYS
        assert SARTHI_OVERLAYS["bedrock"].transport == "bedrock_converse"

    def test_bedrock_overlay_auth_type(self):
        from sarthi_cli.providers import SARTHI_OVERLAYS
        assert SARTHI_OVERLAYS["bedrock"].auth_type == "aws_sdk"
