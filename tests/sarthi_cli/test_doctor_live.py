"""Tests for ``sarthi doctor --live`` — opt-in bounded real-call tool-backend probes.
...
from sarthi_cli import doctor_live
from sarthi_cli.doctor_live import (
    ProbeResult,
...
class TestBrowserAvailableNpxRung:
    ...
    with. It must fall through to the same cascade `sarthi doctor` uses."""

    def _block_path_and_node_modules_checks(self, monkeypatch, tmp_path):
        monkeypatch.setattr("shutil.which", lambda *a, **k: None)
        monkeypatch.setattr("sarthi_cli.doctor.SARTHI_HOME", tmp_path / "home")
        monkeypatch.setattr("sarthi_cli.doctor.PROJECT_ROOT", tmp_path / "root")
