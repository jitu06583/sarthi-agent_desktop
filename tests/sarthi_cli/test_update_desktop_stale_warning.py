...
#88251: ``sarthi update`` treated a failed desktop pack as non-fatal, printed
...
def desktop_env(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "sarthi_constants.with_sarthi_node_path", lambda: {}, raising=False
    )
    monkeypatch.setattr(
        "sarthi_constants.display_sarthi_home", lambda: str(tmp_path), raising=False
    )
...
def test_gateway_exit_code_file_tracks_desktop_rebuild(tmp_path, monkeypatch):
    monkeypatch.setattr(update_cmd, "get_sarthi_home", lambda: tmp_path)
