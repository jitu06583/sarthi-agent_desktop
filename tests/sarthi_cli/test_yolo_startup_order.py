"""Regression tests for #60328: --yolo must set SARTHI_YOLO_MODE in
...
def _run_main_and_capture_yolo_at_startup(monkeypatch, argv):
    """Run main() with *argv*, capturing SARTHI_YOLO_MODE at the
    ...
    def spy_prepare_startup(args):
        yolo_at_startup["value"] = os.environ.get("SARTHI_YOLO_MODE")

    monkeypatch.setattr(
        "sarthi_cli.main._prepare_agent_startup", spy_prepare_startup
    )
    # Stub cmd_chat so main() returns cleanly without entering chat.
    monkeypatch.setattr("sarthi_cli.main.cmd_chat", lambda args: None)
    monkeypatch.delenv("SARTHI_YOLO_MODE", raising=False)
    monkeypatch.setattr(sys, "argv", argv)

    from sarthi_cli.main import main as cli_main
...
def test_top_level_yolo_flag_sets_env_before_startup(monkeypatch):
    """sarthi --yolo must set SARTHI_YOLO_MODE before
    _prepare_agent_startup imports tools.approval."""
    result = _run_main_and_capture_yolo_at_startup(
        monkeypatch, ["sarthi", "--yolo"]
    )
    assert result == "1", (
        "SARTHI_YOLO_MODE was not '1' when _prepare_agent_startup was "
...
def test_chat_subcommand_yolo_flag_sets_env_before_startup(monkeypatch):
    """sarthi chat --yolo must also set SARTHI_YOLO_MODE before
    _prepare_agent_startup."""
    result = _run_main_and_capture_yolo_at_startup(
        monkeypatch, ["sarthi", "chat", "--yolo"]
    )
    assert result == "1", (
        "SARTHI_YOLO_MODE was not '1' when _prepare_agent_startup was "
...
def test_no_yolo_flag_leaves_env_unset_at_startup(monkeypatch):
    """Without --yolo, SARTHI_YOLO_MODE must not be set at startup."""
    result = _run_main_and_capture_yolo_at_startup(
        monkeypatch, ["sarthi"]
    )
    assert result is None, (
        "SARTHI_YOLO_MODE was unexpectedly set at startup without --yolo."
    )
