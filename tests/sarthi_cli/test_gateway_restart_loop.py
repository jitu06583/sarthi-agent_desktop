...
Covers:
- Defense 1: gateway stop/restart refuse when _SARTHI_GATEWAY=1
...
class TestGatewayLifecyclePattern:
    ...
    @pytest.mark.parametrize("text", [
        ...
        "sarthi gateway stop",
        "sarthi  gateway  restart",         # double spaces
        "Hermez Gateway Restart".lower().replace("z", "s"),  # case handled
        "SARTHI GATEWAY RESTART",           # uppercase
    ])
    def test_sarthi_gateway_commands(self, text):
    ...
    @pytest.mark.parametrize("text", [
        ...
        "launchctl submit -l ai.sarthi.gateway-hard-restart-no-photon-notice -- /bin/sh ~/.sarthi/scripts/hard_restart_gateway_no_photon_notice.sh",
        "launchctl submit -l sarthi-gateway-restart-helper -- /bin/sh helper.sh",
        # bootstrap loads an arbitrary plist — same laundering shape.
        "launchctl bootstrap gui/501 ~/Library/LaunchAgents/ai.sarthi.gateway.restart-once.plist",
        ...
        (
            "launchctl submit \\\n"
            "  -l ai.sarthi.gateway-hard-restart-no-photon-notice \\\n"
            "  -- /bin/sh ~/.sarthi/scripts/hard_restart_gateway_no_photon_notice.sh"
        ),
    ])
    ...
    def test_line_continuation_does_not_bridge_unrelated_lines(self):
        ...
        text = (
            ...
            "unrelated sarthi note on the next line"
        )
    ...
    @pytest.mark.parametrize("text", [
        ...
        "sarthi cron list",
        "sarthi update",
        "sarthi config set model claude",
        "echo 'just a normal cron job'",
        "run the backup script",
        "gateway is running fine",
        # `sarthi gateway start` is benign — starting a gateway from inside a
        ...
        "sarthi gateway start --all",
        # Tightened launchctl/systemctl branches: ops on NON-gateway sarthi
        # services must not be falsely blocked (the old `.*sarthi` matched any
        # sarthi token).
        "launchctl unload ai.sarthi.update-checker.plist",
        "launchctl restart ai.sarthi.daemon",
        ...
        "systemctl restart sarthi-meta.service",
        "systemctl restart sarthi-cron-helper",
...
class TestCronCreateLifecycleBlock:
    ...
    def test_block_sarthi_gateway_restart(self, capsys):
        args = Namespace(
            cron_command="create",
            schedule="30m",
            prompt="Upgrade sarthi then run sarthi gateway restart",
            name=None,
            deliver=None,
            repeat=None,
            skill=None,
    ...
    def test_block_script_with_lifecycle_command(self, tmp_path, capsys, monkeypatch):
        ...
        # restart_sarthi_gateway_once.sh). The script must live under
        # SARTHI_HOME/scripts so the scheduler — and the guard — resolve it.
        monkeypatch.setenv("SARTHI_HOME", str(tmp_path / ".sarthi"))
        scripts_dir = tmp_path / ".sarthi" / "scripts"
...
class TestGatewaySelfTargetingGuard:
    """Verify sarthi gateway stop/restart refuse when _SARTHI_GATEWAY=1."""

    def test_stop_refuses_inside_gateway(self, monkeypatch):
        monkeypatch.setenv("_SARTHI_GATEWAY", "1")
    ...
    def test_stop_allows_outside_gateway(self, monkeypatch):
        ...
        monkeypatch.delenv("_SARTHI_GATEWAY", raising=False)
...
class TestTerminalToolGatewayLifecycleGuard:
    """terminal_tool must refuse gateway lifecycle commands when _SARTHI_GATEWAY=1.

    Issue #37453: systemctl --user restart sarthi-gateway runs as a child of the
    ...
    @pytest.mark.parametrize("cmd", [
        "systemctl restart sarthi-gateway",
        "systemctl --user restart sarthi-gateway",
        "systemctl stop sarthi-gateway.service",
        "sarthi gateway restart",
        "launchctl kickstart gui/501/ai.sarthi.gateway",
        # #62891 exact reported shape and its bootstrap sibling.
        "launchctl submit -l ai.sarthi.gateway-hard-restart-no-photon-notice -- /bin/sh ~/.sarthi/scripts/hard_restart_gateway_no_photon_notice.sh",
        "launchctl submit -l com.foo -- /path/gateway",
        "launchctl bootstrap gui/501 ~/Library/LaunchAgents/ai.sarthi.gateway.restart-once.plist",
        "pkill -f sarthi.*gateway",
    ])
    ...
    def test_force_true_cannot_bypass_block(self, monkeypatch):
        ...
        result = json.loads(tt.terminal_tool(
            command="systemctl restart sarthi-gateway", force=True
        ))
    ...
    def test_blocks_launchctl_submit_inside_gateway(self, monkeypatch, tmp_path):
        ...
        result = json.loads(tt.terminal_tool(
            command=(
                "launchctl submit -l ai.sarthi.delayed-ops -- "
                f"/bin/bash {script}"
            )
        ))
    ...
    @pytest.mark.parametrize("command", [
        # Neutral, non-sarthi label: label-independent detection is the point
        # (#62891 second reproduction used `ai.sarthi.svc-reload-tmp`).
        "launchctl submit -l com.foo -- /path/gateway",
        "launchctl submit -l ai.sarthi.svc-reload-tmp -- /bin/sh /tmp/h-svc-reload.sh",
    ...
    def test_blocks_launchctl_submit_hidden_in_referenced_script(
        ...
        script.write_text(
            "#!/bin/bash\nlaunchctl submit -l ai.sarthi.loop -- /bin/true\n"
        )
        self._patch_env(monkeypatch, self._make_fake_env(), inside_gateway=True)
    ...
    def test_launchctl_submit_parser_handles_shell_quoting(self, monkeypatch):
        ...
        self._patch_env(monkeypatch, self._make_fake_env(), inside_gateway=True)
        result = json.loads(tt.terminal_tool(
            command="launchctl sub\"\"mit -l ai.sarthi.loop -- /bin/true"
        ))
    ...
    def test_safe_systemctl_commands_pass_through(self, monkeypatch):
        """Non-sarthi systemctl commands must not be blocked by this guard."""
        import tools.terminal_tool as tt
...
class TestLifecycleGuardModule:
    ...
    def test_prompt_with_command_raises(self):
        ...
        with pytest.raises(GatewayLifecycleBlocked) as exc:
            check_gateway_lifecycle("please run sarthi gateway restart", None)
    ...
    def test_split_across_prompt_and_script_still_blocks(self, tmp_path):
        ...
        script.write_text("sarthi gateway stop\n", encoding="utf-8")
        with pytest.raises(GatewayLifecycleBlocked):
    ...
    def test_binary_script_does_not_silently_bypass(self, tmp_path):
        ...
        script.write_bytes(b"\xfesarthi gateway restart\xff")
        with pytest.raises(GatewayLifecycleBlocked):
            check_gateway_lifecycle("", str(script))


    def test_relative_script_resolved_under_scripts_dir(self, tmp_path, monkeypatch):
        """A bare/relative script name resolves under SARTHI_HOME/scripts (the
        ...
        (scripts_dir / "restart.sh").write_text(
            "launchctl kickstart -k gui/501/ai.sarthi.gateway\n"
        )
        with pytest.raises(GatewayLifecycleBlocked):
    ...
    def test_python_script_with_literal_lifecycle_command_still_blocked(
        ...
        script.write_text('import os\nos.system("sarthi gateway restart")\n', encoding="utf-8")
        with pytest.raises(GatewayLifecycleBlocked):
            check_gateway_lifecycle("clean prompt", str(script))
    ...
    def test_read_referenced_script_tolerates_nul_in_path(self):
        ...
        text, unsafe = _read_referenced_script(Path("/tmp/sarthi\x00binary"))
        assert text is None
    ...
    def test_binary_from_remote_callback_never_false_positives(self):
        ...
        def _remote_read(_path: str):
            return "MZ\x00\x00\x90\x00 sarthi gateway restart \x00\x00junk"
    ...
    def test_cron_guard_total_when_home_unresolvable(self, monkeypatch):
        """`get_sarthi_home()` falls back to Path.home(), which raises
        RuntimeError when neither SARTHI_HOME nor HOME resolves
...
class TestCreateJobBlocksLifecycleCommands:
    ...
    sarthi_cli.cron.cron_create. Enforcing at create_job covers both."""
    ...
    def test_create_job_blocks_prompt_command(self):
        ...
        with pytest.raises(GatewayLifecycleBlocked):
            create_job(prompt="then run sarthi gateway restart", schedule="30m")
    ...
    def test_cronjob_tool_surfaces_block_as_error(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("SARTHI_HOME", str(tmp_path / ".sarthi"))
        (tmp_path / ".sarthi").mkdir(parents=True)
        from tools.cronjob_tools import cronjob
        result = json.loads(cronjob(
            action="create", schedule="0 9 * * *",
            prompt="please run sarthi gateway restart nightly",
        ))
...
class TestLifecycleGuardDataArgumentExemption:
    ...
    @pytest.mark.parametrize("command", [
        ...
        "'systemctl stop sarthi-gateway'\"",
        # grep/rg pattern arguments hunting for the lifecycle string.
        "grep -c 'systemctl restart sarthi-gateway' /var/log/syslog",
        "rg 'sarthi gateway restart' /home/user/.sarthi/logs/",
        "journalctl -u sarthi-gateway --grep 'systemctl restart sarthi-gateway'",
        # SQL with stop/restart column/value words but no command shape.
        'sqlite3 stats.db "SELECT stop_time, restart_reason FROM '
        'sarthi_gateway_restarts"',
    ...
    @pytest.mark.parametrize("command", [
        ...
        'sqlite3 db ".shell sarthi gateway restart"',
        'psql -c "\\! systemctl restart sarthi-gateway"',
        "grep 'systemctl restart sarthi-gateway' cmds.txt | sh",
        "grep gateway f | xargs systemctl restart sarthi-gateway",
        'grep "$(systemctl restart sarthi-gateway)" f',
        "grep 'restart' log; systemctl restart sarthi-gateway",
        'sqlite3 db "SELECT 1"; sarthi gateway stop',
        # Plain lifecycle commands are unaffected by the exemption.
        "sarthi gateway restart",
        "sudo systemctl stop sarthi-gateway",
    ])
    ...
    def test_python_script_branch_gets_the_same_exemption(self, tmp_path):
        ...
        prompt = (
            ...
            "WHERE msg LIKE '%systemctl restart sarthi-gateway%'\""
        )
