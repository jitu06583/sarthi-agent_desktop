...
class TestWebServerEndpoints:
    ...
    def test_profiles_sidebar_heals_stale_schema_store(self):
        ...
        array — the desktop rendered "No sessions yet" after `sarthi update`
    ...
    def test_startup_eager_reconcile_heals_stale_store(self):
        ...
        #79531/#80037: after `sarthi update` an old-schema state.db used to
    ...
    def test_startup_eager_reconcile_never_raises(self, monkeypatch):
        ...
        monkeypatch.setattr(sarthi_state, "SessionDB", boom)
    ...
    def test_heal_gives_up_when_reconcile_cannot_fix_the_store(self, monkeypatch):
        ...
        original_init = sarthi_state.SessionDB.__init__
        ...
        # the class on sarthi_state covers every open the helper makes.
        monkeypatch.setattr(sarthi_state.SessionDB, "__init__", counting_init)
    ...
    def test_generic_corruption_does_not_trigger_writable_heal(
        ...
        monkeypatch.setattr(sarthi_state, "SessionDB", corrupt_open)
    ...
    def test_messaging_platforms_profile_scopes_gateway_reads(self, monkeypatch):
        ...
        SARTHI_HOME contextvar override (#56986), so /api/messaging/platforms
    ...
    def _isolate_honcho_config(self):
        # Honcho tests write the suite-wide SARTHI_HOME honcho.json; snapshot and
        # restore it so provider status/config state never leaks across tests.
        from sarthi_constants import get_sarthi_home

        path = get_sarthi_home() / "honcho.json"
    ...
    def test_put_honcho_writes_host_block_root_and_secret(self, monkeypatch, tmp_path):
        ...
        cfg = json.loads((get_sarthi_home() / "honcho.json").read_text(encoding="utf-8"))
        # baseUrl is root-scoped; the rest live in the active host block.
        assert cfg["baseUrl"] == "https://honcho.example.dev"
        assert cfg["hosts"]["sarthi"]["workspace"] == "myws"
        assert cfg["hosts"]["sarthi"]["peerName"] == "eri"
        assert cfg["hosts"]["sarthi"]["environment"] == "local"
        assert cfg["hosts"]["sarthi"]["sessionStrategy"] == "per-repo"
        # The key lands where the client reads first; GET keeps it write-only.
        assert cfg["hosts"]["sarthi"]["apiKey"] == "hch-test-key"
    ...
    def test_update_sarthi_returns_apt_guidance_without_spawning(self, monkeypatch):
        import sarthi_cli.web_server as web_server
        ...
        def fail_spawn(*_args, **_kwargs):
            nonlocal spawned
            spawned = True
            raise AssertionError("APT-managed update guard should not spawn sarthi update")
        ...
        assert data["update_command"] == "pkg upgrade sarthi-agent"
        assert spawned is False

        check = self.client.get("/api/sarthi/update/check")
        assert check.status_code == 200
        check_data = check.json()
        assert check_data["install_method"] == "apt"
        assert check_data["can_apply"] is False
        assert check_data["update_command"] == "pkg upgrade sarthi-agent"
        assert "Termux APT" in check_data["message"]

    def test_update_sarthi_spawns_with_action_id(self, monkeypatch):
        import sarthi_cli.web_server as web_server
        ...
        assert calls == [
            (["update"], "sarthi-update", {"SARTHI_ACTION_ID": "a" * 32})
        ]

    def test_update_sarthi_reuses_running_action(self, monkeypatch):
        import sarthi_cli.web_server as web_server
    ...
    def test_platform_scoped_messaging_env_vars_are_channel_managed(self):
        ...
        tests/sarthi_cli/test_setup_hidden_env.py.
        """
    ...
    def test_deleting_the_active_custom_endpoint_clears_its_model_mirror(self):
        ...
        """
        from sarthi_cli.config import custom_endpoint_key_env, get_env_value, load_config
    ...
    def test_custom_endpoint_save_scopes_to_the_requested_profile(self):
        ...
        """
        from sarthi_cli import profiles as profiles_mod
        from sarthi_cli.config import custom_endpoint_key_env
        from sarthi_constants import get_sarthi_home

        default_home = get_sarthi_home()
        worker_home = profiles_mod.get_profile_dir("worker")
        worker_home.mkdir(parents=True)
    ...
    def test_custom_endpoint_save_leaves_a_hand_written_env_ref_alone(self, monkeypatch):
        ...
        from sarthi_cli.config import custom_endpoint_key_env, get_config_path, get_env_value
    ...
    def test_two_endpoints_on_one_host_keep_separate_credentials(self):
        ...
        the first's key.
        """
        from sarthi_cli.config import custom_endpoint_key_env, get_env_value
...
class TestBuildSchemaFromConfig:
    ...
    def test_timezone_field_is_searchable_select(self):
        ...
        """
        from sarthi_cli.web_server import CONFIG_SCHEMA, _timezone_options
...
class TestNewEndpoints:
    ...
    def test_toolsets_resolve_subscription_features_once(self, monkeypatch):
        ...
        from sarthi_cli.nous_subscription import NousSubscriptionFeatures
    ...
    def test_get_toolset_config_reports_truthful_provider_status(self, monkeypatch):
        ...
        """
        import sarthi_cli.tools_config as tools_config
        from sarthi_cli.nous_account import NousPortalAccountInfo

        # Logged out of Nous Portal → managed subscription rows need sign-in.
        monkeypatch.setattr(
            "sarthi_cli.nous_subscription.get_nous_portal_account_info",
    ...
    def test_terminal_ssh_probe_ready_when_configured(self, monkeypatch):
        ...
        config["terminal"]["ssh_user"] = "sarthi"
        save_config(config)
        ...
        assert "sarthi@devbox.example.com" in ssh["detail"]
...
class TestStatusInstallId:
    ...
    Sarthi home (not the profile home), survives a fresh process-cache read,
    ...
    def test_status_reports_persistent_install_id(self, monkeypatch):
        ...
        id_file = get_default_sarthi_root() / "install_id"
    ...
    def test_all_profiles_of_one_install_share_the_id(self, monkeypatch, tmp_path):
        """SARTHI_HOME=<root> and SARTHI_HOME=<root>/profiles/<name> resolve to
        the same id file — profiles share one physical install identity."""
        import sarthi_cli.web_server as ws

        root = tmp_path / "sarthi-root"
...
class TestDiscoverUserThemes:
    ...
    def test_ignores_transient_profile_override(self, tmp_path, monkeypatch):
        ...
        token = set_sarthi_home_override(str(other))
        try:
            results = web_server._discover_user_themes()
        finally:
...
class TestDashboardPluginManifestExtensions:
    ...
    def test_user_plugins_ignore_profile_home_override(self, tmp_path, monkeypatch):
        ...
        SARTHI_HOME override scopes a request to another profile."""
        from sarthi_constants import (
            reset_sarthi_home_override,
            set_sarthi_home_override,
        )
        ...
        monkeypatch.setenv("SARTHI_HOME", str(launch_home))
        from sarthi_cli import web_server
        token = set_sarthi_home_override(str(other))
        try:
    ...
    def test_user_plugins_found_under_profile_scoped_process(self, tmp_path, monkeypatch):
        ...
        sets SARTHI_HOME=<root>/profiles/<name>) must still discover user
        plugins installed in the sarthi root's plugins/ directory."""
        root = tmp_path / "sarthi-root"
...
class TestPtyWebSocket:
    ...
    def test_tui_python_command_uses_child_path(self, tmp_path):
        ...
        command = f"sarthi-review-python{Path(sys.executable).suffix}"
        ...
        env = {
            "SARTHI_CWD": str(tmp_path),
            "SARTHI_PYTHON": command,
            "PATH": str(bin_dir),
        }

        main_mod._apply_tui_python_env(env)

        assert env["SARTHI_PYTHON"] == command
...
class TestServeIndexMissingIndex:
    ...
    def _client_with_dist(tmp_path, monkeypatch, *, write_index: bool):
        ...
        monkeypatch.setattr(ws, "WEB_DIST", dist)
        monkeypatch.delenv("SARTHI_SERVE_HEADLESS", raising=False)
        spa_app = FastAPI()
        ws.mount_spa(spa_app)
...
class TestHashedAssetCacheHeaders:
    ...
    def test_serve_css_is_immutable_and_keeps_prefix_rewrites(
        ...
        prefixed = client.get(
            ...
            headers={"X-Forwarded-Prefix": "/sarthi"},
        )
        assert prefixed.status_code == 200
        assert prefixed.headers["cache-control"] == self._IMMUTABLE
        assert "url(/sarthi/ds-assets/bg.png)" in prefixed.text
        assert "url(/sarthi/fonts-terminal/x.woff2)" in prefixed.text
