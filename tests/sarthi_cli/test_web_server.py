"""Tests for sarthi_cli.web_server and related config utilities."""
...
@pytest.fixture
def _install_example_plugin(_isolate_sarthi_home):
    """Drop the example-dashboard fixture into the per-test SARTHI_HOME
    ...
    isolated ``SARTHI_HOME``.

    The user-plugin source is preferred over a transient
    ``SARTHI_BUNDLED_PLUGINS`` override because the bundled dir is
    resolved per-call (other tests in the suite implicitly rely on the
    real bundled plugins — kanban, sarthi-achievements, model providers
    ...
    # plugin in exactly as a real operator would with `sarthi plugins
    # enable example`.
    ...
    #      and start serving requests against a torn-down SARTHI_HOME.
    app = web_server.app
...
class TestReloadEnv:
    ...
    def test_removes_deleted_known_vars(self, tmp_path):
        """reload_env() removes known Sarthi vars not present in .env."""
        env_file = tmp_path / ".env"
    ...
    def test_does_not_remove_unknown_vars(self, tmp_path):
        """reload_env() preserves non-Sarthi env vars even when absent from .env."""
        env_file = tmp_path / ".env"
        env_file.write_text("")
        with patch.dict(reload_env.__globals__, {"get_env_path": lambda: env_file}):
...
class TestSessionTokenInjection:
    """The desktop shell mints SARTHI_DASHBOARD_SESSION_TOKEN and signs its
    ...
    def test_honors_injected_token(self, monkeypatch):
        import importlib
        import sarthi_cli.web_server as ws

        monkeypatch.setenv("SARTHI_DASHBOARD_SESSION_TOKEN", "desktop-seeded-token")
        try:
            importlib.reload(ws)
            assert ws._SESSION_TOKEN == "desktop-seeded-token"
        finally:
            monkeypatch.delenv("SARTHI_DASHBOARD_SESSION_TOKEN", raising=False)
            importlib.reload(ws)
...
class TestWebServerEndpoints:
    ...
    def _setup_test_client(self, monkeypatch, _isolate_sarthi_home):
        """Create a TestClient and isolate the state DB under the test SARTHI_HOME."""
        try:
        ...
        import sarthi_state
        from sarthi_constants import get_sarthi_home
        from sarthi_cli.web_server import app, _SESSION_HEADER_NAME, _SESSION_TOKEN

        monkeypatch.setattr(sarthi_state, "DEFAULT_DB_PATH", get_sarthi_home() / "state.db")
    ...
    def test_get_status(self):
        ...
        assert "sarthi_home" in data
        assert "active_sessions" in data
        assert data["can_update_sarthi"] is True

    def test_status_active_session_count_uses_read_only_db(self, monkeypatch, tmp_path):
        import sarthi_cli.web_server as web_server
        import sarthi_state
        ...
        fake_db_path.touch()
        monkeypatch.setattr(sarthi_state, "DEFAULT_DB_PATH", fake_db_path)
        ...
        monkeypatch.setattr("sarthi_state.SessionDB", _FakeDB)
    ...
    def test_status_active_session_count_fresh_install_returns_zero(self, monkeypatch, tmp_path):
        ...
        monkeypatch.setattr(sarthi_state, "DEFAULT_DB_PATH", tmp_path / "absent.db")

        def _boom(*a, **k):
            raise AssertionError("SessionDB must not be constructed when db file is absent")

        monkeypatch.setattr("sarthi_state.SessionDB", _boom)
    ...
    def test_dashboard_update_capability_allows_git_in_container(self, monkeypatch):
        """A git checkout inside a container (e.g. bind-mounted in sarthi-webui)
    ...
    def test_get_memory_provider_config_returns_safe_defaults(self):
        ...
        assert fields["bank_id"]["value"] == "sarthi"
    ...
    def test_declared_surface_put_writes_config_and_secret(self):
        from sarthi_constants import get_sarthi_home
        from sarthi_cli.config import load_env
        ...
        config_path = get_sarthi_home() / "hindsight" / "config.json"
    ...
    def test_post_memory_provider_setup_persists_values_without_activation(self):
        from sarthi_cli.config import load_config, load_env
    ...
    def test_get_media_404_for_missing_file(self):
        ...
        missing = get_sarthi_home() / "images" / "nope.png"
        resp = self.client.get("/api/media", params={"path": str(missing)})
        assert resp.status_code == 404

    def test_get_media_requires_auth(self):
        from sarthi_cli.web_server import _SESSION_HEADER_NAME
    ...
    def test_chat_image_upload_writes_to_default_profile_images(self):
        ...
        assert target.parent == get_sarthi_home() / "images"
    ...
    def test_desktop_audio_routes_registered(self):
        ...
        without failing CI.
        """
        from sarthi_cli.web_server import app
    ...
    def test_update_sarthi_returns_docker_guidance_without_spawning(self, monkeypatch):
        import sarthi_cli.web_server as web_server
        ...
        def fail_spawn(*_args, **_kwargs):
            nonlocal spawned
            spawned = True
            raise AssertionError("docker update guard should not spawn sarthi update")

        # Bypass the managed-externally gate so we reach the docker install check.
        monkeypatch.setattr(web_server, "_dashboard_local_update_managed_externally", lambda: False)
        monkeypatch.setattr(web_server, "detect_install_method", lambda _root: "docker")
        monkeypatch.setattr(web_server, "_spawn_sarthi_action", fail_spawn)
        web_server._ACTION_PROCS.pop("sarthi-update", None)
        web_server._ACTION_RESULTS.pop("sarthi-update", None)

        resp = self.client.post("/api/sarthi/update")

        assert resp.status_code == 200
        data = resp.json()
        assert data["ok"] is False
        assert data["name"] == "sarthi-update"
        assert data["pid"] is None
        assert data["error"] == "docker_update_unsupported"
        assert "docker pull nousresearch/hermes-agent:latest" in data["message"]
        assert spawned is False

        status = self.client.get("/api/actions/sarthi-update/status")
    ...
    def test_update_sarthi_returns_managed_runtime_guidance_without_spawning(self, monkeypatch):
        import sarthi_cli.web_server as web_server
        ...
        def fail_spawn(*_args, **_kwargs):
            nonlocal spawned
            spawned = True
            raise AssertionError("managed runtime update guard should not spawn sarthi update")
    ...
    def test_update_sarthi_spawns_on_non_docker_install(self, monkeypatch):
        import sarthi_cli.web_server as web_server
        ...
        monkeypatch.setattr(web_server, "detect_install_method", lambda _root: "git")
        monkeypatch.setattr(web_server, "_spawn_sarthi_action", fake_spawn)
        web_server._ACTION_PROCS.pop("sarthi-update", None)
        web_server._ACTION_RESULTS.pop("sarthi-update", None)
        ...
        assert resp.status_code == 200
        assert resp.json() == {"ok": True, "pid": 12345, "name": "sarthi-update"}
        assert calls == [(["update"], "sarthi-update")]
    ...
    def test_action_status_reaps_completed_process(self, monkeypatch):
        ...
        web_server._ACTION_PROCS.pop("sarthi-update", None)
        web_server._ACTION_RESULTS.pop("sarthi-update", None)
        web_server._ACTION_PROCS["sarthi-update"] = proc

        resp = self.client.get("/api/actions/sarthi-update/status")
        assert resp.status_code == 200
        data = resp.json()
        ...
        assert "sarthi-update" not in web_server._ACTION_PROCS
        assert web_server._ACTION_RESULTS["sarthi-update"] == {
            "exit_code": 0,
            "pid": 42424,
        }
    ...
    def test_action_status_tails_large_log_without_read_text(self, tmp_path, monkeypatch):
        ...
        log_path = tmp_path / web_server._ACTION_LOG_FILES["sarthi-update"]
        log_path.write_text(
            "stale-start\n"
        ...
        resp = self.client.get("/api/actions/sarthi-update/status?lines=3")
    ...
    def test_get_env_vars_marks_channel_managed_keys(self):
        from sarthi_cli.web_server import _channel_managed_env_keys
    ...
    def test_get_env_vars_surfaces_catalog_providers(self):
        ...
        tencent-tokenhub, copilot were configurable via `sarthi model` but
    ...
    def test_platform_scoped_messaging_env_vars_are_channel_managed(self):
        from sarthi_cli.web_server import (
    ...
    def test_model_set_requires_confirmation_for_expensive_model(self, monkeypatch):
        monkeypatch.setattr(
            "sarthi_cli.model_cost_guard.expensive_model_warning",
    ...
    def test_model_set_maps_unknown_vendor_to_aggregator(self, monkeypatch):
        ...
        a Sarthi provider — keep the user's aggregator instead of writing a
    ...
    def test_ops_import_passes_force_flag(self, tmp_path, monkeypatch):
        ...
        `sarthi import` doesn't auto-abort at the overwrite prompt."""
        import sarthi_cli.web_server as ws
        ...
        monkeypatch.setattr(ws, "_spawn_sarthi_action", fake_spawn)
    ...
    def test_ops_backup_defaults_to_dashboard_downloadable_archive(self, monkeypatch):
        ...
        assert archive.parent == get_sarthi_home() / "backups"
        assert archive.name.startswith("sarthi-backup-")
        assert archive.suffix == ".zip"

    def test_ops_backup_uses_hosted_sarthi_home(self, tmp_path, monkeypatch):
        from pathlib import Path

        import sarthi_cli.web_server as ws

        hosted_home = tmp_path / "opt-data"
        monkeypatch.setenv("SARTHI_HOME", str(hosted_home))
        captured = {}
    ...
    def test_ops_backup_download_streams_dashboard_backup(self, tmp_path):
        ...
        archive = backup_dir / "sarthi-backup-test.zip"
    ...
    def test_ops_import_upload_rejects_invalid_zip(self, monkeypatch):
        ...
        monkeypatch.setattr(ws, "_spawn_sarthi_action", fail_spawn)
    ...
    def test_reveal_env_var(self, tmp_path):
        ...
        from sarthi_cli.web_server import _SESSION_HEADER_NAME, _SESSION_TOKEN
    ...
    def test_reveal_env_var_legacy_authorization_header_still_works(self, tmp_path):
        ...
        from sarthi_cli.config import save_env_value
        from sarthi_cli.web_server import _SESSION_TOKEN
    ...
    def test_weixin_messaging_metadata_describes_personal_ilink_setup(self):
        ...
        assert weixin["docs_url"] == (
            "https://sarthi-agent.vercel.app/docs/user-guide/messaging/weixin/"
        )
    ...
    def test_teams_messaging_metadata_links_setup_guide(self):
        ...
        from sarthi_cli.web_server import _build_catalog_entry

        teams = _build_catalog_entry("teams")
        assert teams["docs_url"] == (
            "https://sarthi-agent.vercel.app/docs/user-guide/messaging/teams"
        )
    ...
    def test_google_chat_messaging_metadata_links_setup_guide(self):
        ...
        assert google_chat["docs_url"] == (
            "https://sarthi-agent.vercel.app/docs/user-guide/messaging/google_chat"
        )
    ...
    def test_telegram_onboarding_worker_request_uses_httpx(self, monkeypatch):
        ...
        assert kwargs["json"] == {"bot_name": "Sarthi Agent"}
        assert kwargs["headers"]["Accept"] == "application/json"
        assert kwargs["headers"]["Authorization"] == "Bearer poll-secret"
        assert kwargs["headers"]["Content-Type"] == "application/json"
        assert kwargs["headers"]["User-Agent"].startswith("SarthiDashboard/")
    ...
    def test_telegram_onboarding_start_strips_poll_token(self, monkeypatch):
        ...
        def fake_request(method, path, *, body=None, bearer_token=None):
            ...
            return {
                ...
                "suggested_username": "sarthi_pair123_bot",
                "deep_link": "https://t.me/newbot/SarthiSetupBot/sarthi_pair123_bot",
                "qr_payload": "https://t.me/newbot/SarthiSetupBot/sarthi_pair123_bot",
    ...
    def test_telegram_onboarding_ready_and_apply_never_returns_bot_token(self, monkeypatch):
        ...
        def fake_request(method, path, *, body=None, bearer_token=None):
            if method == "POST":
                return {
                    ...
                    "poll_token": "poll-secret",
                    "suggested_username": "sarthi_pair_ready_bot",
                    "deep_link": "https://t.me/newbot/SarthiSetupBot/sarthi_pair_ready_bot",
                    "qr_payload": "https://t.me/newbot/SarthiSetupBot/sarthi_pair_ready_bot",
                    "expires_at": "2027-05-18T00:00:00.000Z",
                }
            assert method == "GET"
            ...
            return {
                "status": "ready",
                "bot_username": "sarthi_pair_ready_bot",
        ...
        monkeypatch.setattr(ws, "_spawn_sarthi_action", fake_spawn_action)
    ...
    def test_telegram_onboarding_apply_reports_restart_failure_after_save(
        ...
        def fake_request(method, path, *, body=None, bearer_token=None):
            if method == "POST":
                return {
                    ...
                    "poll_token": "poll-secret",
                    "suggested_username": "sarthi_pair_restart_fails_bot",
                    "deep_link": "https://t.me/newbot/SarthiSetupBot/sarthi_pair_restart_fails_bot",
                    "qr_payload": "https://t.me/newbot/SarthiSetupBot/sarthi_pair_restart_fails_bot",
                    "expires_at": "2027-05-18T00:00:00.000Z",
                }
            assert method == "GET"
            ...
            return {
                "status": "ready",
                "bot_username": "sarthi_pair_restart_fails_bot",
                "owner_user_id": 123456789,
                "token": "123456:SECRET",
            }
        ...
        monkeypatch.setattr(ws, "_spawn_sarthi_action", fail_spawn_action)
    ...
    def test_telegram_onboarding_apply_reuses_inflight_gateway_restart(
        ...
        second racing ``sarthi gateway restart`` child (e.g. when a stale
        ...
        def fake_request(method, path, *, body=None, bearer_token=None):
            if method == "POST":
                return {
                    ...
                    "poll_token": "poll-secret",
                    "suggested_username": "sarthi_pair_reuse_bot",
                    "deep_link": "https://t.me/newbot/SarthiSetupBot/sarthi_pair_reuse_bot",
                    "qr_payload": "https://t.me/newbot/SarthiSetupBot/sarthi_pair_reuse_bot",
                    "expires_at": "2027-05-18T00:00:00.000Z",
                }
            return {
                "status": "ready",
                "bot_username": "sarthi_pair_reuse_bot",
                "owner_user_id": 123456789,
                "token": "123456:SECRET",
            }
    ...
    def test_telegram_onboarding_apply_requires_ready_pairing(self, monkeypatch):
        ...
        def fake_request(method, path, *, body=None, bearer_token=None):
            return {
                ...
                "poll_token": "poll-secret",
                "suggested_username": "sarthi_pair_waiting_bot",
                "deep_link": "https://t.me/newbot/SarthiSetupBot/sarthi_pair_waiting_bot",
                "qr_payload": "https://t.me/newbot/SarthiSetupBot/sarthi_pair_waiting_bot",
                "expires_at": "2027-05-18T00:00:00.000Z",
            }
    ...
    def test_telegram_onboarding_cancel_clears_local_session(self, monkeypatch):
        ...
        def fake_request(method, path, *, body=None, bearer_token=None):
            return {
                ...
                "poll_token": "poll-secret",
                "suggested_username": "sarthi_pair_cancel_bot",
                "deep_link": "https://t.me/newbot/SarthiSetupBot/sarthi_pair_cancel_bot",
                "qr_payload": "https://t.me/newbot/SarthiSetupBot/sarthi_pair_cancel_bot",
                "expires_at": "2027-05-18T00:00:00.000Z",
            }
    ...
    def test_path_traversal_dotdot_blocked(self):
        ...
        resp = self.client.get("/%2e%2e/sarthi_cli/web_server.py")
        assert resp.status_code in {200, 404}
        if resp.status_code == 200:
    ...
    def test_spa_assets_are_read_as_utf8(self, monkeypatch, tmp_path):
        ...
        css_resp = spa_client.get("/assets/app.css", headers={"x-forwarded-prefix": "/sarthi"})
    ...
    def test_headless_serve_disables_spa_even_with_a_dist(self, monkeypatch, tmp_path):
        """`sarthi serve` (SARTHI_SERVE_HEADLESS) must NOT serve the SPA even
        ...
        monkeypatch.setattr(ws, "WEB_DIST", dist)
        monkeypatch.setenv("SARTHI_SERVE_HEADLESS", "1")
        app_ = FastAPI()
        ws.mount_spa(app_)
    ...
    def test_set_model_main_nous_applies_gateway_defaults(self, monkeypatch):
        ...
        import sarthi_cli.nous_subscription as ns
    ...
    def test_apply_main_model_assignment_base_url_and_context_reconcile(self):
        ...
        from sarthi_cli.web_server import _apply_main_model_assignment
    ...
    def test_parse_model_ids_handles_openai_and_bare_shapes(self):
        ...
        from sarthi_cli.web_server import _parse_model_ids
    ...
    def test_set_model_main_custom_persists_api_key_and_registers_provider(self):
        ...
        ``sarthi model`` custom flow. Regression for the desktop loop where a
    ...
    def test_recommended_default_nous_honors_free_tier(self, monkeypatch):
        ...
        model (mirroring `sarthi model`), not the first curated paid entry."""
        import sarthi_cli.models as models_mod
...
class TestBuildSchemaFromConfig:
    def test_produces_expected_field_count(self):
        from sarthi_cli.web_server import CONFIG_SCHEMA
    ...
    def test_approvals_mode_options_match_config_values(self):
        ...
        are 'manual', 'smart', and 'off' (see sarthi_cli/config.py).
    ...
    def test_empty_prefix_produces_correct_keys(self):
        from sarthi_cli.web_server import _build_schema_from_config
...
class TestConfigRoundTrip:
    ...
    def test_round_trip_preserves_schema_invisible_nested_keys(self):
        ...
        from sarthi_cli.config import load_config, read_raw_config, save_config
...
class TestNewEndpoints:
    ...
    @pytest.fixture(autouse=True)
    def _setup(self, monkeypatch, _isolate_sarthi_home):
        try:
            from starlette.testclient import TestClient
        except ImportError:
            pytest.skip("fastapi/starlette not installed")
    ...
    def test_cron_blueprints_list(self):
        ...
        assert first["appUrl"].startswith("sarthi://")
    ...
    def test_profiles_list_includes_default(self):
        from sarthi_constants import get_sarthi_home
        get_sarthi_home().mkdir(parents=True, exist_ok=True)
    ...
    def test_profiles_list_falls_back_when_profile_listing_fails(self, monkeypatch):
        from sarthi_constants import get_sarthi_home
        import sarthi_cli.profiles as profiles_mod

        sarthi_home = get_sarthi_home()
        sarthi_home.mkdir(parents=True, exist_ok=True)
        (sarthi_home / "config.yaml").write_text(
            "model:\n  provider: openrouter\n  name: anthropic/claude-sonnet-4.6\n",
            encoding="utf-8",
        )
        named = sarthi_home / "profiles" / "multi-agent"
    ...
    def test_profile_setup_command_uses_named_profile_wrapper(self):
        ...
        (get_sarthi_home() / "profiles" / "coder").mkdir(parents=True)
    ...
    def test_profile_setup_command_uses_sarthi_for_default_profile(self):
        from sarthi_constants import get_sarthi_home
        ...
        assert resp.status_code == 200
        assert resp.json()["command"] == "sarthi setup"
    ...
    def test_profiles_create_creates_wrapper_alias_when_safe(self, monkeypatch, tmp_path):
        ...
        monkeypatch.setattr(profiles_mod.shutil, "which", lambda name: "/opt/sarthi/bin/sarthi")
        ...
        if is_windows:
            assert lines == ["@echo off", "sarthi -p writer %*"]
        else:
            assert lines == ["#!/bin/sh", 'exec /opt/sarthi/bin/sarthi -p writer "$@"']
    ...
    def test_profiles_create_with_clone_from_copies_source_skills(self, monkeypatch):
        ...
        (get_sarthi_home() / "config.yaml").write_text(
            "model:\n  provider: openrouter\n",
            encoding="utf-8",
        )
        default_skill = get_sarthi_home() / "skills" / "custom" / "new-skill"
        ...
        assert resp.status_code == 200
        cloned_root = get_sarthi_home() / "profiles" / "cloned"
    ...
    def test_profiles_create_with_clone_from_duplicates_source(self, monkeypatch):
        ...
        source_skill = get_sarthi_home() / "profiles" / "source-prof" / "skills" / "custom" / "src-skill"
        ...
        assert resp.status_code == 200
        cloned_skill = (
            get_sarthi_home() / "profiles" / "source-prof-copy" / "skills" / "custom" / "src-skill" / "SKILL.md"
        )
        assert cloned_skill.exists()
    ...
    def test_profiles_create_clone_all_from_named_source(self, monkeypatch):
        ...
        source_dir = get_sarthi_home() / "profiles" / "full-src"
        ...
        assert resp.status_code == 200
        target_dir = get_sarthi_home() / "profiles" / "full-copy"
    ...
    def test_profiles_create_without_clone_seeds_bundled_skills(self, monkeypatch):
        ...
        assert resp.status_code == 200
        seeded_skill = get_sarthi_home() / "profiles" / "fresh" / "skills" / "software-development" / "plan" / "SKILL.md"
    ...
    def test_profiles_create_builder_fields_model_mcp_and_keep_skills(self, monkeypatch):
        ...
        prof_dir = get_sarthi_home() / "profiles" / "builder"
        token = set_sarthi_home_override(str(prof_dir))
        try:
            cfg = load_config()
    ...
    def test_profile_model_round_trip(self, monkeypatch):
        ...
        import yaml
        cfg_path = get_sarthi_home() / "profiles" / "model-prof" / "config.yaml"
    ...
    def test_skills_list_includes_disabled_skills(self, monkeypatch):
        ...
        import sarthi_cli.skills_config as skills_config
        import sarthi_cli.web_server as web_server
    ...
    def test_toolsets_list_matches_cli_enabled_state(self, monkeypatch):
        import sarthi_cli.tools_config as tools_config
...
class TestModelContextLength:
    ...
    def test_normalize_extracts_context_length_from_dict(self):
        ...
        from sarthi_cli.web_server import _normalize_config_for_web
    ...
    def test_denormalize_writes_context_length_into_model_dict(self):
        ...
        from sarthi_cli.web_server import _denormalize_config_from_web
        from sarthi_cli.config import save_config
...
class TestGatewayBusyReadout:
    ...
    def test_restart_drain_timeout_surfaced_and_numeric(self, monkeypatch):
        ...
        monkeypatch.setenv("SARTHI_RESTART_DRAIN_TIMEOUT", "90")
...
class TestNormaliseThemeDefinition:
    ...
    def test_rejects_missing_name(self):
        from sarthi_cli.web_server import _normalise_theme_definition
...
class TestDiscoverUserThemes:
    """Tests for _discover_user_themes() — scans ~/.sarthi/dashboard-themes/."""
...
class TestThemeBootstrapCSS:
    ...
    @staticmethod
    def _write_theme(sarthi_home, name="ocean"):
        themes_dir = sarthi_home / "dashboard-themes"
    ...
    def test_user_theme_renders_bundle_vars(self, tmp_path, monkeypatch):
        ...
        assert css.startswith('<style id="sarthi-theme-bootstrap">')
    ...
    def test_serve_index_no_bootstrap_for_builtin_theme(self, tmp_path, monkeypatch):
        ...
        resp = client.get("/chat")
        assert resp.status_code == 200
        assert "sarthi-theme-bootstrap" not in resp.text
...
class TestDeleteSessionEndpoint:
    ...
    def _setup_test_client(self, monkeypatch, _isolate_sarthi_home):
        ...
        monkeypatch.setattr(
            sarthi_state, "DEFAULT_DB_PATH", get_sarthi_home() / "state.db"
        )
...
class TestBulkDeleteSessionsEndpoint:
    ...
    1. Route-ordering: ``/api/sessions/bulk-delete`` must shadow the
       ...
       the block comment in ``sarthi_cli/web_server.py``).
    ...
    def test_route_order_not_shadowed_by_session_id(self):
        ...
        assert "deleted" in body, (
            ...
            "registration order in sarthi_cli/web_server.py."
        )
...
class TestDeleteEmptySessionsEndpoint:
    ...
    def _setup_test_client(self, monkeypatch, _isolate_sarthi_home):
        ...
        # Pin the SessionDB to the isolated SARTHI_HOME so each test
...
class TestPluginAPIAuth:
    ...
    @pytest.fixture(autouse=True)
    def _setup_test_client(self, monkeypatch, _isolate_sarthi_home, _install_example_plugin):
        ...
        installs it into the per-test ``SARTHI_HOME``.
        """
        try:
            from starlette.testclient import TestClient
        except ImportError:
    ...
    def test_plugin_route_allows_auth(self):
        ...
        test fixture (installed into SARTHI_HOME by the class-level
    ...
    def test_non_kanban_plugin_route_requires_auth(self):
        ...
        so any plugin's API surface — kanban, sarthi-achievements, future
        plugins — must require the session token. Hit a non-kanban plugin
        path to lock that in.
        """
        # Real plugin path (sarthi-achievements is loaded by default).
        resp = self.client.get("/api/plugins/sarthi-achievements/overview")
        assert resp.status_code == 401
...
# or ``sh -c 'printf …'``) instead of the real ``sarthi --tui`` binary.  The
...
class TestPtyWebSocket:
    ...
    def test_resolve_chat_argv_uses_dashboard_scroll_env(self, monkeypatch):
        ...
        import sarthi_cli.main as main_mod
        ...
        assert env["SARTHI_TUI_DASHBOARD"] == "1"
        assert env["SARTHI_TUI_INLINE"] == "1"
        assert env["SARTHI_TUI_DISABLE_MOUSE"] == "1"
    ...
    def test_resolve_chat_argv_applies_terminal_backend_config(
        self, monkeypatch, _isolate_sarthi_home
    ):
        import sarthi_cli.main as main_mod

        config_path = Path(os.environ["SARTHI_HOME"]) / "config.yaml"
        config_path.write_text(
            "\n".join(
                [
                    "terminal:",
                    "  backend: docker",
                    "  docker_image: example/sarthi-tools:latest",
                    "  docker_extra_args:",
                    "    - --network=host",
                ]
            ),
        ...
        assert env["TERMINAL_DOCKER_IMAGE"] == "example/sarthi-tools:latest"
    ...
    def test_streams_child_stdout_to_client(self, monkeypatch):
        ...
        with self.client.websocket_connect(self._url()) as conn:
            ...
            while time.monotonic() < deadline:
                ...
                if frame:
                    buf += frame
                if b"sarthi-ws-ok" in buf:
                    break
            assert b"sarthi-ws-ok" in buf
    ...
    def test_unavailable_platform_closes_with_message(self, monkeypatch):
        from sarthi_cli.pty_bridge import PtyUnavailableError
        ...
        import sarthi_cli.web_server as ws_mod
    ...
    def test_channel_param_propagates_sidecar_url(self, monkeypatch):
        ...
        SARTHI_TUI_SIDECAR_URL env var pointing back at /api/pub on the
    ...
    def test_pub_broadcasts_to_events_subscribers(self):
        ...
        """
        import asyncio
        from sarthi_cli import web_server as ws_mod
...
def test_resolve_chat_argv_injects_gateway_ws_url(monkeypatch):
    ...
    gateway_url = env.get("SARTHI_TUI_GATEWAY_URL", "")
...
class TestDashboardPluginStaticAssetAllowlist:
    ...
    def _setup_test_client(self, monkeypatch, _isolate_sarthi_home, _install_example_plugin):
        ...
        the per-test ``SARTHI_HOME`` user-plugins dir.
        """
        try:
            from starlette.testclient import TestClient
        except ImportError:
...
class TestDesktopCronTicker:
    ...
    def test_ticker_runs_when_desktop(self, monkeypatch, _isolate_sarthi_home):
        import threading
        import cron.scheduler as sched

        called = threading.Event()
        monkeypatch.setattr(sched, "tick", lambda *a, **k: called.set())
        monkeypatch.setenv("SARTHI_DESKTOP", "1")

        with self._client():
            assert called.wait(3.0), "expected cron tick under SARTHI_DESKTOP=1"

    def test_ticker_skipped_without_desktop(self, monkeypatch, _isolate_sarthi_home):
        import threading
        import cron.scheduler as sched

        called = threading.Event()
        monkeypatch.setattr(sched, "tick", lambda *a, **k: called.set())
        monkeypatch.delenv("SARTHI_DESKTOP", raising=False)
