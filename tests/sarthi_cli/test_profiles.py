...
class TestDeleteProfile:
    ...
    def test_backend_scan_matches_shebang_exec_of_sarthi_shim(self, profile_env, monkeypatch):
        """A `sarthi` console-script shim spawned directly (e.g. Electron's
        findOnPath('sarthi') resolution) reports argv[0] as the interpreter
        (python3) and argv[1] as the shim's path -- not "sarthi" -- because
        ...
        procs = [
            ...
            # being the python interpreter, not "sarthi".
            FakeProc(201, ["/usr/bin/python3", "/Users/x/.local/bin/sarthi", "--profile", "coder", "serve",
                            "--host", "127.0.0.1", "--port", "0"]),
            # Same shape but a different profile → skipped.
            FakeProc(202, ["/usr/bin/python3", "/Users/x/.local/bin/sarthi", "--profile", "other", "serve"]),
            # Non-sarthi script run by python3 → skipped.
    ...
    def test_backend_scan_rejects_unrelated_sarthi_prefixed_script(self, profile_env, monkeypatch):
        """A user's own script that happens to start with "sarthi" (e.g.
        sarthi-notes.py, sarthi-unrelated-tool) must NOT be misidentified as
        the console-script shim just because argv[0] is a python interpreter
        and argv[1]'s basename starts with "sarthi" -- only the actual known
        console-script entry points (sarthi, sarthi-agent, sarthi-acp) count.
        """
        create_profile("coder", no_alias=True)
        profile_dir = get_profile_dir("coder")
        ...
        procs = [
            ...
            FakeProc(301, ["/usr/bin/python3", "/Users/x/scripts/sarthi-notes.py",
                            "--profile", "coder", "serve"]),
            FakeProc(302, ["/usr/bin/python3", "/Users/x/scripts/sarthi-unrelated-tool",
                            "--profile", "coder", "serve"]),
        ]
    ...
    def test_backend_scan_matches_all_known_console_script_shims(self, profile_env, monkeypatch):
        """The other two real console-script entry points (sarthi-agent,
        sarthi-acp -- see pyproject.toml [project.scripts]) must also be
        recognized via the shebang-exec path, not just the primary "sarthi"
        shim.
        """
        create_profile("coder", no_alias=True)
        profile_dir = get_profile_dir("coder")
        ...
        self_pid = os.getpid()
        procs = [
            FakeProc(401, ["/usr/bin/python3", "/Users/x/.local/bin/sarthi-agent",
                            "--profile", "coder", "serve"]),
            FakeProc(402, ["/usr/bin/python3", "/Users/x/.local/bin/sarthi-acp",
                            "--profile", "coder", "serve"]),
        ]
...
class TestResolveProfileEnvSpelling:
    """resolve_profile_env() keeps the configured SARTHI_HOME spelling as
    ...
    def test_resolution_matrix_preserves_configured_spelling(self, monkeypatch, tmp_path):
        ...
        for env_home, profile, expected in cases:
            monkeypatch.setenv("SARTHI_HOME", str(env_home))
    ...
    def test_unset_env_falls_back_to_default_root(self, monkeypatch):
        # No SARTHI_HOME: the platform default root applies (existing contract).
        monkeypatch.delenv("SARTHI_HOME", raising=False)
        assert Path(resolve_profile_env("default")) == _get_default_sarthi_home()
