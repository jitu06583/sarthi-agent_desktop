...
subparser, placing the flag BEFORE the subcommand (e.g. 'sarthi --yolo chat')
...
def _build_parser():
    """Build the sarthi argument parser from the real code.
...
class TestChatVerboseArg:
    ...
    def test_cmd_chat_forwards_none_when_verbose_is_absent(self, monkeypatch):
        ...
        fake_banner = types.ModuleType("sarthi_cli.banner")
        ...
        monkeypatch.setitem(sys.modules, "sarthi_cli.banner", fake_banner)
...
class TestYoloEnvVar:
    """Verify --yolo sets SARTHI_YOLO_MODE regardless of flag position.
...
class TestAcceptHooksOnAgentSubparsers:
    ...
    parser and `chat`, so `sarthi gateway run --accept-hooks` failed
    ...
    # One driver subprocess parses ALL argvs: sarthi_cli.main is a very heavy
    # import (previously 11 separate `python -m sarthi_cli.main` spawns with a
...
"""
    ...
    def test_accepted_at_every_position(self):
        """Every `sarthi <argv>` must exit 0 (help) rather than failing
        with `unrecognized arguments`."""
        import json
        import subprocess
...
class TestChatSubparserInheritedValueFlags:
    ...
    Regression test for #28780: `sarthi -t web chat` silently dropped the
    ...
    Uses the real `sarthi_cli._parser.build_top_level_parser()` rather than
    ...
    def test_flag_before_chat_is_preserved(self, real_parser, flag, attr, value):
        ...
        assert getattr(args, attr, None) == value, (
            f"`sarthi {flag} {value} chat` lost the flag — got "
    ...
    def test_store_true_flag_before_chat_is_preserved(
        ...
        would clobber the parent's `True` when used as `sarthi --tui chat`."""
        args, _ = real_parser.parse_known_args([flag, "chat"])
        assert getattr(args, attr, None) is True, (
            f"`sarthi {flag} chat` lost the flag — got "
    ...
    def test_chat_subparser_inherited_value_flags_use_suppress(self):
        ...
        assert not offenders, (
            ...
            "top-level value when used as `sarthi <flag> <value> chat`:\n  "
