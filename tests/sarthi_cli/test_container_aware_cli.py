...
def container_env(tmp_path, monkeypatch):
    """Set up a fake SARTHI_HOME with .container-mode file."""
    sarthi_home = tmp_path / ".sarthi"
    sarthi_home.mkdir()
    monkeypatch.setenv("SARTHI_HOME", str(sarthi_home))
    monkeypatch.delenv("SARTHI_DEV", raising=False)

    container_mode = sarthi_home / ".container-mode"
    container_mode.write_text(
        "# Written by NixOS activation script. Do not edit manually.\n"
        "backend=podman\n"
        "container_name=sarthi-agent\n"
        "exec_user=sarthi\n"
        "sarthi_bin=/data/current-package/bin/sarthi\n"
    )
    return sarthi_home
...
def test_get_container_exec_info_returns_metadata(container_env):
    ...
    with patch("sarthi_constants.is_container", return_value=False):
        info = get_container_exec_info()

    assert info is not None
    assert info["backend"] == "podman"
    assert info["container_name"] == "sarthi-agent"
    assert info["exec_user"] == "sarthi"
    assert info["sarthi_bin"] == "/data/current-package/bin/sarthi"


def test_get_container_exec_info_none_inside_container(container_env):
    """Returns None when we're already inside a container."""
    with patch("sarthi_constants.is_container", return_value=True):
        info = get_container_exec_info()
...
def test_get_container_exec_info_skipped_when_sarthi_dev(container_env, monkeypatch):
    """Returns None when SARTHI_DEV=1 is set (dev mode bypass)."""
    monkeypatch.setenv("SARTHI_DEV", "1")
...
def test_get_container_exec_info_not_skipped_when_sarthi_dev_zero(container_env, monkeypatch):
    """SARTHI_DEV=0 does NOT trigger bypass — only '1' does."""
    monkeypatch.setenv("SARTHI_DEV", "0")
...
def test_get_container_exec_info_defaults():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        sarthi_home = Path(tmpdir) / ".sarthi"
        sarthi_home.mkdir()
        (sarthi_home / ".container-mode").write_text(
            "# minimal file with no keys\n"
        )

        with patch("sarthi_constants.is_container", return_value=False), \
             patch.dict(get_container_exec_info.__globals__, {"get_sarthi_home": lambda: sarthi_home}), \
             patch.dict(os.environ, {}, clear=False):
            os.environ.pop("SARTHI_DEV", None)
            info = get_container_exec_info()
...
def test_get_container_exec_info_docker_backend(container_env):
    ...
    (container_env / ".container-mode").write_text(
        "backend=docker\n"
        "container_name=sarthi-custom\n"
        "exec_user=myuser\n"
        "sarthi_bin=/opt/sarthi/bin/sarthi\n"
    )
    ...
    assert info["backend"] == "docker"
    assert info["container_name"] == "sarthi-custom"
    assert info["exec_user"] == "myuser"
    assert info["sarthi_bin"] == "/opt/sarthi/bin/sarthi"
...
def test_exec_in_container_calls_execvp(docker_container_info):
    ...
    from sarthi_cli.main import _exec_in_container
