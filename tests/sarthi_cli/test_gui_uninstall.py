"""Tests for sarthi_cli.gui_uninstall — GUI-only uninstall + install discovery.
...
import sarthi_cli.gui_uninstall as gu


def _make_agent(sarthi_home: Path) -> Path:
    ...
    (agent_root / "sarthi_cli" / "__init__.py").write_text("")
    (agent_root / "venv" / "bin").mkdir(parents=True)
    return agent_root


def _make_gui_build(sarthi_home: Path) -> None:
    """Create the source-built GUI artifacts a `sarthi desktop` run produces."""
    desktop = sarthi_home / "sarthi-agent" / "apps" / "desktop"
    ...
    (sarthi_home / "sarthi-agent" / "node_modules").mkdir(parents=True)
    (sarthi_home / "desktop-build-stamp.json").write_text("{}")


def _make_user_data(sarthi_home: Path) -> None:
    (sarthi_home / "config.yaml").write_text("x: 1\n")
    (sarthi_home / ".env").write_text("KEY=secret\n")
...
def test_gui_install_summary_shape(tmp_path, monkeypatch):
    ...
    summary = gu.gui_install_summary(sarthi_home)
    ...
    assert summary["sarthi_home"] == str(sarthi_home)
