...
PROVIDER_ENV_VARS = (
    ...
    "OPENAI_BASE_URL", "SARTHI_COPILOT_ACP_COMMAND", "COPILOT_CLI_PATH",
...
class TestApiKeyProviderStatus:
    ...
    def test_copilot_status_uses_gh_cli_token(self, monkeypatch):
        monkeypatch.setattr("sarthi_cli.copilot_auth._try_gh_cli_token", lambda: "gho_gh_cli_token")
    ...
    def test_get_auth_status_dispatches_to_external_process(self, monkeypatch):
        monkeypatch.setattr("sarthi_cli.auth.shutil.which", lambda command: f"/opt/bin/{command}")
...
class TestResolveApiKeyProviderCredentials:
    ...
    def test_resolve_copilot_acp_with_local_cli(self, monkeypatch):
        monkeypatch.setenv("SARTHI_COPILOT_ACP_ARGS", "--acp --stdio")
        monkeypatch.setattr("sarthi_cli.auth.shutil.which", lambda command: f"/usr/local/bin/{command}")
...
class TestHasAnyProviderConfigured:
    ...
    def test_claude_code_creds_counted_when_sarthi_configured(self, monkeypatch, tmp_path):
        """Claude Code credentials should count when Sarthi has been explicitly configured."""
        import yaml
        from sarthi_cli import config as config_module
        sarthi_home = tmp_path / ".sarthi"
        sarthi_home.mkdir()
...
class TestZaiEndpointAutoDetect:
    ...
    def test_probe_success_returns_detected_url(self, monkeypatch):
        ...
        monkeypatch.setattr(
            "sarthi_cli.auth.detect_zai_endpoint",
            lambda *a, **kw: {
                "id": "coding-global",
...
class TestDeepInfraTagFiltering:
    ...
    def test_returns_none_on_network_failure(self, monkeypatch):
        ...
        from sarthi_cli.models import _fetch_deepinfra_models_by_tag, _fetch_deepinfra_pricing
