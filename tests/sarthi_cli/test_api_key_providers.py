...
from sarthi_cli.copilot_auth import _try_gh_cli_token
...
from sarthi_cli.auth import PROVIDER_REGISTRY as _REGISTRY
...
_EXTRA_ENV_VARS = (
    ...
    "SARTHI_COPILOT_ACP_COMMAND", "COPILOT_CLI_PATH",
    "SARTHI_COPILOT_ACP_ARGS", "COPILOT_ACP_BASE_URL",
)
...
def _clear_provider_env(monkeypatch):
    ...
    monkeypatch.setattr("sarthi_cli.auth._load_auth_store", lambda: {})
...
class TestResolveApiKeyProviderCredentials:
    ...
    def test_try_gh_cli_token_uses_homebrew_path_when_not_on_path(self, monkeypatch):
        from sarthi_cli.copilot_auth import _invalidate_gh_cli_token_cache

        _invalidate_gh_cli_token_cache()
        monkeypatch.setattr("sarthi_cli.copilot_auth.shutil.which", lambda command: None)
        monkeypatch.setattr(
            "sarthi_cli.copilot_auth.os.path.isfile",
            lambda path: path == "/opt/homebrew/bin/gh",
        )
        monkeypatch.setattr(
            "sarthi_cli.copilot_auth.os.access",
        ...
        monkeypatch.setattr("sarthi_cli.copilot_auth.subprocess.run", _fake_run)
...
class TestRuntimeProviderResolution:
    ...
    def test_runtime_copilot_uses_gh_cli_token(self, monkeypatch):
        monkeypatch.setattr("sarthi_cli.copilot_auth._try_gh_cli_token", lambda: "gho_cli_secret")
        from sarthi_cli.runtime_provider import resolve_runtime_provider
    ...
    def test_runtime_copilot_uses_responses_for_gpt_5_4(self, monkeypatch):
        monkeypatch.setattr("sarthi_cli.copilot_auth._try_gh_cli_token", lambda: "gho_cli_secret")
        monkeypatch.setattr(
            "sarthi_cli.runtime_provider._get_model_config",
            lambda: {"provider": "copilot", "default": "gpt-5.4"},
        )
        monkeypatch.setattr(
            "sarthi_cli.models.fetch_github_model_catalog",
    ...
    def test_runtime_copilot_acp_uses_process_runtime(self, monkeypatch):
        monkeypatch.setattr("sarthi_cli.auth.shutil.which", lambda command: f"/usr/local/bin/{command}")
        monkeypatch.setenv("SARTHI_COPILOT_ACP_ARGS", "--acp --stdio --debug")
...
class TestHasAnyProviderConfigured:
    ...
    def test_claude_code_creds_ignored_on_fresh_install(self, monkeypatch, tmp_path):
        """Claude Code credentials should NOT skip the wizard when Sarthi is unconfigured."""
        from sarthi_cli import config as config_module
        from sarthi_cli.auth import PROVIDER_REGISTRY
        sarthi_home = tmp_path / ".sarthi"
        sarthi_home.mkdir()
        monkeypatch.setattr(config_module, "get_env_path", lambda: sarthi_home / ".env")
        monkeypatch.setattr(config_module, "get_sarthi_home", lambda: sarthi_home)
        monkeypatch.setattr("sarthi_cli.copilot_auth.resolve_copilot_token", lambda: ("", ""))
        ...
        monkeypatch.setattr("sarthi_cli.auth.get_auth_status", lambda _pid: {})
    ...
    def test_config_provider_counts(self, monkeypatch, tmp_path):
        ...
        sarthi_home = tmp_path / ".sarthi"
        sarthi_home.mkdir()
        config_file = sarthi_home / "config.yaml"
    ...
    def test_config_provider_skips_registry_sweep(self, monkeypatch, tmp_path):
        ...
        """
        import yaml
        sarthi_home = self._setup_home(monkeypatch, tmp_path)
        ...
        monkeypatch.setattr("sarthi_cli.auth.get_auth_status", _trap)
        from sarthi_cli.main import _has_any_provider_configured
        assert _has_any_provider_configured() is True
        assert sweep_calls == [], (
    ...
    def test_auth_json_skips_registry_sweep(self, monkeypatch, tmp_path):
        ...
        monkeypatch.setattr("sarthi_cli.auth.get_auth_status", _guarded_status)
        from sarthi_cli.main import _has_any_provider_configured
        assert _has_any_provider_configured() is True
        assert calls == ["nous"], (
...
class TestKimiCodeCredentialAutoDetect:
    ...
    def test_non_kimi_providers_unaffected(self, monkeypatch):
        ...
        monkeypatch.setattr("sarthi_cli.auth.detect_zai_endpoint", lambda *a, **kw: None)
...
class TestZaiEndpointAutoDetect:
    ...
    def test_env_override_skips_probe(self, monkeypatch):
        ...
        monkeypatch.setattr("sarthi_cli.auth.detect_zai_endpoint", _never_called)
        creds = resolve_api_key_provider_credentials("zai")
...
class TestZaiParallelProbe:
    ...
    def test_candidate_model_fallback_within_endpoint(self, monkeypatch):
        ...
        from sarthi_cli.auth import ZAI_ENDPOINTS, detect_zai_endpoint
    ...
    def test_priority_order_wins_over_completion_order(self, monkeypatch):
        ...
        monkeypatch.setattr("sarthi_cli.auth.httpx.post", _slow_first)
        result = detect_zai_endpoint("test-key", timeout=1.0)
        assert result is not None
        assert result["id"] == first[0]

    def test_all_fail_returns_none(self, monkeypatch):
        from sarthi_cli.auth import detect_zai_endpoint

        monkeypatch.setattr("sarthi_cli.auth.httpx.post", self._mock_post({}))
    ...
    def test_early_exit_does_not_wait_for_slow_losers(self, monkeypatch):
        ...
        monkeypatch.setattr("sarthi_cli.auth.httpx.post", _slow_losers)
...
class TestKimiMoonshotModelListIsolation:
    ...
    def test_moonshot_list_excludes_coding_plan_only_models(self):
        from sarthi_cli.main import _PROVIDER_MODELS
...
class TestHuggingFaceModels:
    ...
    def test_model_lists_match(self):
        ...
        from sarthi_cli.main import _PROVIDER_MODELS as main_models
        from sarthi_cli.models import _PROVIDER_MODELS as models_models
...
class TestNovitaProvider:
    ...
    def test_novita_pricing_cache(self, monkeypatch):
        ...
        from sarthi_cli import models as models_mod
...
class TestFetchDeepInfraModels:
    ...
    def test_returns_filtered_models_on_success(self, monkeypatch):
        ...
        from sarthi_cli.models import _fetch_deepinfra_models
...
class TestDeepInfraTagFiltering:
    ...
    def test_filters_by_surface_tag_and_handles_rollout_states(self, monkeypatch):
        ...
        from sarthi_cli.models import _fetch_deepinfra_models_by_tag
        import sarthi_cli.models as _m
...
class TestDeepInfraProviderProfile:
    ...
    def test_profile_registered_with_alias_and_aux(self):
        ...
        from sarthi_cli.auth import PROVIDER_REGISTRY, resolve_provider
        from sarthi_cli.config import OPTIONAL_ENV_VARS
        from sarthi_cli.models import CANONICAL_PROVIDERS
