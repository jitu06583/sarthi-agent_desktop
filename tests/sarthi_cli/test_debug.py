...
class TestMissingLogNote:
    ...
    `sarthi debug share` runs on the backend, so a desktop connected to a
    ...
    def test_backend_written_log_reports_plain_absence(self, sarthi_home):
        from sarthi_cli.debug import _capture_log_snapshot

        (sarthi_home / "logs" / "agent.log").unlink()

        snap = _capture_log_snapshot("agent", tail_lines=10)
        assert snap.full_text is None
        assert snap.tail_text == "(file not found)"

    def test_client_written_log_names_its_writer_and_path(self, sarthi_home):
        from sarthi_cli.debug import _capture_log_snapshot
        ...
        assert "Sarthi Desktop" in snap.tail_text
        # The reader needs the path to collect by hand on the client machine.
        assert str(sarthi_home / "logs" / "desktop.log") in snap.tail_text

    def test_present_client_log_is_captured_normally(self, sarthi_home):
    ...
    def test_empty_client_log_is_empty_not_absent(self, sarthi_home):
        """An empty file means the app ran and logged nothing — a different fact."""
        from sarthi_cli.debug import _capture_log_snapshot

        (sarthi_home / "logs" / "desktop.log").write_text("")

        snap = _capture_log_snapshot("desktop", tail_lines=10)
        assert snap.tail_text == "(file empty)"

    def test_report_carries_the_note_for_a_remote_backend(self, sarthi_home):
