...
When ``SARTHI_HOME`` points to a named profile, ``read_credential_pool()``
...
def profile_env(tmp_path, monkeypatch):
    """Set up a global root + an active profile under Path.home()/.sarthi/profiles/coder.

    * Path.home() -> tmp_path
    * Global root -> tmp_path/.sarthi            (has its own auth.json fixture)
    * Profile     -> tmp_path/.sarthi/profiles/coder   (active, SARTHI_HOME points here)
...
# ``_load_provider_state`` itself, those helpers raise ``"Sarthi is not
...
def test_load_provider_state_falls_back_to_global(profile_env):
    ...
    from sarthi_cli.auth import _load_auth_store, _load_provider_state
...
def test_load_provider_state_classic_mode_no_fallback(tmp_path, monkeypatch):
    ...
    sarthi_home = tmp_path / "classic"
    sarthi_home.mkdir()
    monkeypatch.setenv("SARTHI_HOME", str(sarthi_home))

    _write(sarthi_home / "auth.json", _make_auth_store(providers={
...
def test_classic_mode_does_not_double_read_same_file(tmp_path, monkeypatch):
    """In classic mode (SARTHI_HOME == global root), no fallback path runs.
    ...
    # sees tmp_path/home/.sarthi as the "real home" — which is NOT equal
    # to the SARTHI_HOME we set (tmp_path/classic), so the guard passes.
    fake_home = tmp_path / "home"
    fake_home.mkdir()
    monkeypatch.setattr(Path, "home", lambda: fake_home)
    sarthi_home = tmp_path / "classic"
    ...
    _write(sarthi_home / "auth.json", _make_auth_store(pool={
        "openrouter": [{
            "id": "only",
            "label": "classic",
            "auth_type": "api_key",
    ...
    from sarthi_cli.auth import read_credential_pool, _global_auth_file_path

    # Classic mode: SARTHI_HOME is set to a custom path that is NOT under
    # ~/.sarthi/profiles/ — get_default_sarthi_root() returns SARTHI_HOME
...
def test_auth_lock_reentrancy_is_scoped_after_profile_context_switch(profile_env):
    ...
    with auth._auth_store_lock():
        ...
        token = set_sarthi_home_override(profile_b)
        try:
