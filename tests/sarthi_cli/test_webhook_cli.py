"""Tests for sarthi_cli/webhook.py — webhook subscription CLI."""
...
from sarthi_cli.webhook import (
    webhook_command,
...
def _isolate(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "sarthi_cli.webhook._is_webhook_enabled", lambda: True
    )
...
def test_webhook_base_url_maps_wildcard_hosts_to_localhost(monkeypatch, host):
    monkeypatch.setattr(
        "sarthi_cli.webhook._get_webhook_config",
...
class TestWebhookEnabledGate:
    def test_blocks_when_disabled(self, capsys, monkeypatch):
        monkeypatch.setattr("sarthi_cli.webhook._is_webhook_enabled", lambda: False)
        webhook_command(_make_args(webhook_action="subscribe", name="blocked"))
        out = capsys.readouterr().out
        assert "not enabled" in out.lower()
        assert "sarthi gateway setup" in out
        assert _load_subscriptions() == {}
    ...
    def test_real_check_disabled(self, monkeypatch):
        ...
        monkeypatch.setattr(
            "sarthi_cli.webhook._is_webhook_enabled",
            lambda: bool({}.get("enabled")),
        )
        import sarthi_cli.webhook as wh_mod
