...
import sarthi_cli.gateway as gateway_cli
...
class TestSystemdServiceRefresh:
    def test_systemd_install_repairs_outdated_unit_without_force(self, tmp_path, monkeypatch):
        unit_path = tmp_path / "sarthi-gateway.service"
    ...
    def test_systemd_stop_timeout_prints_status_guidance(self, monkeypatch, capsys):
        ...
        assert "sarthi gateway status" in output

    def test_systemd_restart_timeout_prints_status_guidance(self, monkeypatch, capsys):
        """`sarthi gateway restart` must not surface a raw TimeoutExpired traceback.

        The dashboard spawns `sarthi gateway restart` in the background; when a
    ...
    def test_run_gateway_refreshes_outdated_unit_on_boot(self, tmp_path, monkeypatch):
        ...
        via exit-code-75 (bypassing `sarthi gateway restart`)."""
        unit_path = tmp_path / "sarthi-gateway.service"
        unit_path.write_text("old unit\n", encoding="utf-8")
    ...
    def test_refresh_refuses_to_bake_pytest_tmpdir_into_real_user_unit(
        ...
        ``generate_systemd_unit()`` bakes ``SARTHI_HOME`` into the unit's
        ...
        ``/tmp/pytest-of-.../sarthi_test`` SARTHI_HOME — silently breaking
        ...
        # Realistic generated unit referencing a pytest tmpdir SARTHI_HOME
        polluted_unit = (
            "[Service]\n"
            'Environment="SARTHI_HOME=/tmp/pytest-of-alice/pytest-42/'
            'popen-gw0/test_x/sarthi_test"\n'
        )
        monkeypatch.setattr(
            gateway_cli,
    ...
    def test_refresh_refuses_to_bake_any_tempdir_home_into_real_user_unit(
        ...
        """Structural guard: a manual E2E SARTHI_HOME like
        ``/tmp/sarthi-e2e-41264`` carries none of the pytest markers but
        poisons the unit identically (seen live 2026-06-11 — an E2E probe ran
        ``sarthi gateway restart`` with a /tmp SARTHI_HOME exported; the
        ...
        must refuse ANY temp-dir SARTHI_HOME, not just pytest-shaped ones.
        """
        unit_path = tmp_path / "sarthi-gateway.service"
        unit_path.write_text("old unit\n", encoding="utf-8")
        ...
        polluted_unit = (
            "[Service]\n"
            'Environment="SARTHI_HOME=/tmp/sarthi-e2e-41264"\n'
            "WorkingDirectory=/tmp/sarthi-e2e-41264\n"
        )
        monkeypatch.setattr(
            gateway_cli,
            "generate_systemd_unit",
...
class TestTempHomeServiceDefinitionGuard:
    ...
    def test_detects_tmp_home_in_systemd_unit(self):
        ...
        assert (
            ...
            == "/tmp/sarthi-e2e-41264"
        )

    def test_detects_var_tmp_home(self):
        unit = '[Service]\nEnvironment="SARTHI_HOME=/var/tmp/sarthi-x"\n'
    ...
    def test_tmp_prefixed_non_temp_path_is_accepted(self):
        ...
        unit = '[Service]\nEnvironment="SARTHI_HOME=/tmpfs-data/.sarthi"\n'
        assert gateway_cli._temp_home_in_service_definition(unit) is None
...
class TestRequireServiceInstalled:
    def test_exits_with_install_hint_when_unit_missing(self, tmp_path, monkeypatch, capsys):
        ...
        assert "sarthi gateway install" in out
...
class TestGeneratedSystemdUnits:
    ...
    def test_user_unit_does_not_leak_profile_node_symlink_target(self, tmp_path, monkeypatch):
        ...
        profile_node_bin = tmp_path / ".sarthi" / "profiles" / "jarvis" / "node" / "bin"
    ...
    def test_system_unit_includes_wsl_windows_interop_paths(self, monkeypatch):
        ...
        monkeypatch.setattr(gateway_cli, "_sarthi_home_for_target_user", lambda home: "/home/alice/.sarthi")
        monkeypatch.setenv("PATH", "/usr/local/bin:/mnt/c/WINDOWS/system32")
        monkeypatch.setattr(gateway_cli.shutil, "which", lambda cmd: None)
...
class TestLaunchdServiceRecovery:
    def test_get_restart_drain_timeout_prefers_env_then_config_then_default(self, monkeypatch):
        monkeypatch.delenv("SARTHI_RESTART_DRAIN_TIMEOUT", raising=False)
        ...
        monkeypatch.setenv("SARTHI_RESTART_DRAIN_TIMEOUT", "9")
        assert gateway_cli._get_restart_drain_timeout() == 9.0

        monkeypatch.setenv("SARTHI_RESTART_DRAIN_TIMEOUT", "invalid")
        assert (
            gateway_cli._get_restart_drain_timeout()
            == DEFAULT_GATEWAY_RESTART_DRAIN_TIMEOUT
        )
    ...
    def test_launchd_install_repairs_outdated_plist_without_force(self, tmp_path, monkeypatch):
        plist_path = tmp_path / "ai.sarthi.gateway.plist"
        ...
        # SARTHI_HOME resolves under the (pytest tmp) test SARTHI_HOME.
        monkeypatch.setattr(
            gateway_cli,
            "generate_launchd_plist",
            lambda: (
                "<plist>--replace\n<key>SARTHI_HOME</key>"
                "<string>/Users/alice/.sarthi</string></plist>"
            ),
        )
    ...
    def test_launchd_install_falls_back_to_detached_on_bootstrap_5(self, tmp_path, monkeypatch, capsys):
        ...
        # (which would trip on the pytest-tmp test SARTHI_HOME) stays out of
        # the way — this test exercises the bootstrap-error fallback.
        monkeypatch.setattr(
            gateway_cli,
            "generate_launchd_plist",
            lambda: (
                "<plist><key>SARTHI_HOME</key>"
                "<string>/Users/alice/.sarthi</string></plist>"
            ),
        )
    ...
    def test_launchd_fallback_exits_when_spawn_fails(self, monkeypatch, capsys):
        ...
        out = capsys.readouterr().out
        assert "nohup sarthi gateway run" in out
    ...
    def test_launchd_unsupported_marker_write_and_clear(self, tmp_path, monkeypatch):
        monkeypatch.setattr(gateway_cli, "get_sarthi_home", lambda: tmp_path)
...
class TestGatewaySystemServiceRouting:
    ...
    def test_systemd_status_surfaces_planned_restart_failure(self, monkeypatch, capsys):
        ...
        monkeypatch.setattr(gateway_cli, "has_legacy_sarthi_units", lambda: False)
    ...
    def test_gateway_install_reports_termux_manual_mode(self, monkeypatch, capsys):
        ...
        assert "Run manually: sarthi gateway" in out
    ...
    def test_gateway_status_on_termux_shows_manual_guidance(self, monkeypatch, capsys):
        ...
        assert "nohup sarthi gateway" in out
    ...
    def test_gateway_restart_does_not_fallback_to_foreground_when_launchd_restart_fails(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(
            ...
            lambda: (_ for _ in ()).throw(
                gateway_cli.subprocess.CalledProcessError(5, ["launchctl", "kickstart", "-k", "gui/501/ai.sarthi.gateway"])
            ),
        )
...
class TestSystemUnitSarthiHome:
    """SARTHI_HOME in system units must reference the target user, not root."""
    ...
    def test_system_unit_uses_target_user_home_not_calling_user(self, monkeypatch):
        ...
        assert 'SARTHI_HOME=/home/alice/.sarthi' in unit
        assert '/root/.sarthi' not in unit

    def test_system_unit_remaps_profile_to_target_user(self, monkeypatch):
        # Simulate sudo with a profile: SARTHI_HOME was resolved under root
        monkeypatch.setattr(Path, "home", staticmethod(lambda: Path("/root")))
        monkeypatch.setenv("SARTHI_HOME", "/root/.sarthi/profiles/coder")
        monkeypatch.setattr(
            gateway_cli, "_system_service_identity",
            lambda run_as_user=None: ("alice", "alice", "/home/alice"),
        )
        ...
        assert 'SARTHI_HOME=/home/alice/.sarthi/profiles/coder' in unit
        assert '/root/' not in unit

    def test_system_unit_preserves_custom_sarthi_home(self, monkeypatch):
        # Custom SARTHI_HOME not under any user's home — keep as-is
        monkeypatch.setattr(Path, "home", staticmethod(lambda: Path("/root")))
        monkeypatch.setenv("SARTHI_HOME", "/opt/sarthi-shared")
        monkeypatch.setattr(
            gateway_cli, "_system_service_identity",
            lambda run_as_user=None: ("alice", "alice", "/home/alice"),
        )
        ...
        assert 'SARTHI_HOME=/opt/sarthi-shared' in unit

    def test_user_unit_unaffected_by_change(self):
        # User-scope units should still use the calling user's SARTHI_HOME
        unit = gateway_cli.generate_systemd_unit(system=False)

        sarthi_home = str(gateway_cli.get_sarthi_home().resolve())
        assert f'SARTHI_HOME={sarthi_home}' in unit


class TestSystemUnitRefreshSyncsSarthiHome:
    """sudo system refresh must not flip TimeoutStopSec via /root/.sarthi."""

    def test_refresh_adopts_unit_sarthi_home_before_rewriting(self, tmp_path, monkeypatch):
        root_home = tmp_path / "root"
        alice_home = tmp_path / "alice"
        root_sarthi = root_home / ".sarthi"
        alice_sarthi = alice_home / ".sarthi"
        root_sarthi.mkdir(parents=True)
        alice_sarthi.mkdir(parents=True)
        (root_sarthi / "config.yaml").write_text(
            "agent:\n  restart_drain_timeout: 60\n", encoding="utf-8"
        )
        (alice_sarthi / "config.yaml").write_text(
        ...
        # Correct installed unit (operator's SARTHI_HOME + drain timeout).
        monkeypatch.setenv("SARTHI_HOME", str(alice_sarthi))
        ...
        # Simulate sudo without inherited SARTHI_HOME (falls back to root).
        monkeypatch.setenv("SARTHI_HOME", str(root_sarthi))
        ...
        assert os.environ["SARTHI_HOME"] == str(alice_sarthi)
    ...
    def test_is_current_syncs_before_reading_unit(self, tmp_path, monkeypatch):
        ...
        unit's pinned SARTHI_HOME *before* it reads/compares the unit.
        ...
        monkeypatch.setattr(gateway_cli, "_sync_sarthi_home_from_systemd_unit", tracking_sync)
    ...
    def test_start_and_restart_delegate_sync_to_chokepoint(self, monkeypatch):
        ...
        for entry in ("systemd_start", "systemd_restart"):
            ...
            monkeypatch.setattr(
                gateway_cli,
                "_sync_sarthi_home_from_systemd_unit",
...
class TestSarthiHomeForTargetUser:
    """Unit tests for _sarthi_home_for_target_user()."""

    def test_remaps_default_home(self, monkeypatch):
        monkeypatch.setattr(Path, "home", staticmethod(lambda: Path("/root")))
        monkeypatch.delenv("SARTHI_HOME", raising=False)

        result = gateway_cli._sarthi_home_for_target_user("/home/alice")
        assert result == "/home/alice/.sarthi"
    ...
    def test_remaps_profile_path(self, monkeypatch):
        ...
        result = gateway_cli._sarthi_home_for_target_user("/home/alice")
        assert result == "/home/alice/.sarthi/profiles/coder"

    def test_keeps_custom_path(self, monkeypatch):
        monkeypatch.setattr(Path, "home", staticmethod(lambda: Path("/root")))
        monkeypatch.setenv("SARTHI_HOME", "/opt/sarthi")

        result = gateway_cli._sarthi_home_for_target_user("/home/alice")
        assert result == "/opt/sarthi"
...
class TestPreflightUserSystemd:
    ...
    def test_raises_when_linger_disabled_and_loginctl_denied(self, monkeypatch):
        ...
        assert "sarthi gateway run" in msg  # foreground fallback mentioned
...
class TestProfileArg:
    ...
    def test_default_sarthi_home_returns_empty(self, tmp_path, monkeypatch):
        """Default ~/.sarthi should not produce a --profile flag."""
        sarthi_home = tmp_path / ".sarthi"
        sarthi_home.mkdir()
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("SARTHI_HOME", str(sarthi_home))
        result = gateway_cli._profile_arg(str(sarthi_home))
        assert result == ""

    def test_named_profile_returns_flag(self, tmp_path, monkeypatch):
        """~/.sarthi/profiles/mybot should return '--profile mybot'."""
        profile_dir = tmp_path / ".sarthi" / "profiles" / "mybot"
        profile_dir.mkdir(parents=True)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("SARTHI_HOME", str(tmp_path / ".sarthi"))
    ...
    def test_named_profile_under_target_user_root_returns_flag(self, tmp_path):
        ...
        target_root = tmp_path / "home" / "alice" / ".sarthi"
    ...
    def test_hash_path_returns_empty(self, tmp_path, monkeypatch):
        """Arbitrary non-profile SARTHI_HOME should return empty string."""
        custom_home = tmp_path / "custom" / "sarthi"
    ...
    def test_nested_profile_path_returns_empty(self, tmp_path, monkeypatch):
        """~/.sarthi/profiles/mybot/subdir should NOT match — too deep."""
        nested = tmp_path / ".sarthi" / "profiles" / "mybot" / "subdir"
    ...
    def test_invalid_profile_name_returns_empty(self, tmp_path, monkeypatch):
        ...
        bad_profile = tmp_path / ".sarthi" / "profiles" / "My Bot!"
    ...
    def test_systemd_unit_includes_profile(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(gateway_cli, "get_sarthi_home", lambda: profile_dir)
        unit = gateway_cli.generate_systemd_unit(system=False)
    ...
    def test_systemd_unit_for_target_user_includes_named_profile(self, tmp_path, monkeypatch):
        ...
        root_profile = root_home / ".sarthi" / "profiles" / "mybot"
        root_profile.mkdir(parents=True)

        monkeypatch.setattr(Path, "home", lambda: root_home)
        monkeypatch.setenv("SARTHI_HOME", str(root_profile))
        monkeypatch.setattr(gateway_cli, "get_sarthi_home", lambda: root_profile)
        monkeypatch.setattr(
            gateway_cli,
            "_system_service_identity",
        ...
        assert f'SARTHI_HOME={target_home / ".sarthi" / "profiles" / "mybot"}' in unit
    ...
    def test_launchd_plist_path_uses_real_user_home_not_profile_home(self, tmp_path, monkeypatch):
        profile_dir = tmp_path / ".sarthi" / "profiles" / "orcha"
        profile_dir.mkdir(parents=True)
        ...
        assert plist_path == machine_home / "Library" / "LaunchAgents" / "ai.sarthi.gateway-orcha.plist"
...
class TestRemapPathForUser:
    ...
    def test_remaps_path_under_current_home(self, monkeypatch, tmp_path):
        ...
        result = gateway_cli._remap_path_for_user(
            str(tmp_path / "root" / ".sarthi" / "sarthi-agent"),
            str(tmp_path / "alice"),
        )
        assert result == str(tmp_path / "alice" / ".sarthi" / "sarthi-agent")

    def test_keeps_system_path_unchanged(self, monkeypatch, tmp_path):
        monkeypatch.setattr(Path, "home", lambda: tmp_path / "root")
        (tmp_path / "root").mkdir()
        result = gateway_cli._remap_path_for_user("/opt/sarthi", str(tmp_path / "alice"))
        assert result == "/opt/sarthi"

    def test_noop_when_same_user(self, monkeypatch, tmp_path):
        monkeypatch.setattr(Path, "home", lambda: tmp_path / "alice")
        (tmp_path / "alice").mkdir()
        original = str(tmp_path / "alice" / ".sarthi" / "sarthi-agent")
...
class TestSystemUnitPathRemapping:
    ...
    def test_system_unit_has_no_root_paths(self, monkeypatch, tmp_path):
        root_home = tmp_path / "root"
        root_home.mkdir()
        project = root_home / ".sarthi" / "sarthi-agent"
        ...
        monkeypatch.setattr(Path, "home", lambda: root_home)
        monkeypatch.setenv("SARTHI_HOME", str(root_home / ".sarthi"))
        monkeypatch.setattr(gateway_cli, "get_sarthi_home", lambda: root_home / ".sarthi")
        ...
        # WorkingDirectory is anchored at the target user's SARTHI_HOME (stable,
        ...
        assert "WorkingDirectory=/home/alice/.sarthi" in unit
        assert "WorkingDirectory=/home/alice/.sarthi/sarthi-agent" not in unit
...
class TestDockerAwareGateway:
    ...
    def test_run_systemctl_raises_runtimeerror_when_missing(self, monkeypatch):
        ...
        with pytest.raises(RuntimeError, match="systemctl is not available"):
            gateway_cli._run_systemctl(["start", "sarthi-gateway"])
    ...
    def test_run_systemctl_passes_through_on_success(self, monkeypatch):
        ...
        result = gateway_cli._run_systemctl(["status", "sarthi-gateway"])
        assert result.returncode == 0
        assert len(calls) == 1
        assert "status" in calls[0]

    def test_install_in_container_prints_docker_guidance(self, monkeypatch, capsys):
        """'sarthi gateway install' inside Docker exits 0 with container guidance."""
        import pytest
    ...
    def test_uninstall_in_container_prints_docker_guidance(self, monkeypatch, capsys):
        """'sarthi gateway uninstall' inside Docker exits 0 with container guidance."""
        import pytest
    ...
    def test_start_in_container_prints_docker_guidance(self, monkeypatch, capsys):
        """'sarthi gateway start' inside Docker exits 0 with container guidance."""
        import pytest
        ...
        out = capsys.readouterr().out
        assert "docker" in out.lower()
        assert "sarthi gateway run" in out


class TestLegacySarthiUnitDetection:
    """Tests for _find_legacy_sarthi_units / has_legacy_sarthi_units.

    These guard against the scenario that tripped Luis in April 2026: an
    older install left a ``sarthi.service`` unit behind when the service was
    renamed to ``sarthi-gateway.service``. After PR #5646 (signal recovery
    via systemd), the two services began SIGTERM-flapping over the same
    Telegram bot token in a 30-second cycle.

    The detector must flag ``sarthi.service`` ONLY when it actually runs our
    gateway, and must NEVER flag profile units
    (``sarthi-gateway-<profile>.service``) or unrelated third-party services.
    """

    # Minimal ExecStart that looks like our gateway
    _OUR_UNIT_TEXT = (
        "[Unit]\nDescription=Sarthi Gateway\n[Service]\n"
        "ExecStart=/usr/bin/python -m sarthi_cli.main gateway run --replace\n"
    )
    ...
    def test_detects_legacy_sarthi_service_in_user_scope(self, tmp_path, monkeypatch):
        user_dir, _ = self._setup_search_paths(tmp_path, monkeypatch)
        legacy = user_dir / "sarthi.service"
        legacy.write_text(self._OUR_UNIT_TEXT, encoding="utf-8")

        results = gateway_cli._find_legacy_sarthi_units()

        assert len(results) == 1
        name, path, is_system = results[0]
        assert name == "sarthi.service"
        assert path == legacy
        assert is_system is False
        assert gateway_cli.has_legacy_sarthi_units() is True

    def test_detects_legacy_sarthi_service_in_system_scope(self, tmp_path, monkeypatch):
        _, system_dir = self._setup_search_paths(tmp_path, monkeypatch)
        legacy = system_dir / "sarthi.service"
        legacy.write_text(self._OUR_UNIT_TEXT, encoding="utf-8")
    ...
    def test_ignores_profile_unit_sarthi_gateway_coder(self, tmp_path, monkeypatch):
        """CRITICAL: profile units must NOT be flagged as legacy.

        Teknium's concern — ``sarthi-gateway-coder.service`` is our standard
        ...
        for base in (user_dir, system_dir):
            (base / "sarthi-gateway-coder.service").write_text(
                self._OUR_UNIT_TEXT, encoding="utf-8"
            )
            (base / "sarthi-gateway-orcha.service").write_text(
                self._OUR_UNIT_TEXT, encoding="utf-8"
            )
            (base / "sarthi-gateway.service").write_text(
                self._OUR_UNIT_TEXT, encoding="utf-8"
            )
        ...
        assert results == []
        assert gateway_cli.has_legacy_sarthi_units() is False

    def test_ignores_unrelated_sarthi_service(self, tmp_path, monkeypatch):
        """Third-party ``sarthi.service`` that isn't ours stays untouched.

        If a user has some other package named ``sarthi`` installed as a
        service, we must not flag it.
        """
        user_dir, _ = self._setup_search_paths(tmp_path, monkeypatch)
        (user_dir / "sarthi.service").write_text(
            "[Unit]\nDescription=Some Other Sarthi\n[Service]\n"
            "ExecStart=/opt/other-sarthi/bin/daemon --foreground\n",
            encoding="utf-8",
        )
    ...
    def test_returns_empty_when_no_legacy_files_exist(self, tmp_path, monkeypatch):
        ...
        assert gateway_cli._find_legacy_sarthi_units() == []
        assert gateway_cli.has_legacy_sarthi_units() is False
    ...
    def test_detects_both_scopes_simultaneously(self, tmp_path, monkeypatch):
        ...
        user_dir, system_dir = self._setup_search_paths(tmp_path, monkeypatch)
        (user_dir / "sarthi.service").write_text(self._OUR_UNIT_TEXT, encoding="utf-8")
        (system_dir / "sarthi.service").write_text(self._OUR_UNIT_TEXT, encoding="utf-8")
    ...
    def test_accepts_alternate_execstart_formats(self, tmp_path, monkeypatch):
        ...
        ExecStart variants we've seen in the wild:
          - python -m sarthi_cli.main gateway run
          - python path/to/sarthi_cli/main.py gateway run
          - sarthi gateway run   (direct binary)
          - python path/to/gateway/run.py
        """
        user_dir, _ = self._setup_search_paths(tmp_path, monkeypatch)
        variants = [
            "ExecStart=/venv/bin/python -m sarthi_cli.main gateway run --replace",
            "ExecStart=/venv/bin/python /opt/sarthi/sarthi_cli/main.py gateway run",
            "ExecStart=/usr/local/bin/sarthi gateway run --replace",
            "ExecStart=/venv/bin/python /opt/sarthi/gateway/run.py",
        ]
        for i, execstart in enumerate(variants):
            name = "sarthi.service" if i == 0 else "sarthi.service"  # same name
            # Test each variant fresh
            (user_dir / "sarthi.service").write_text(
                f"[Unit]\nDescription=Old Sarthi\n[Service]\n{execstart}\n",
                encoding="utf-8",
            )
            results = gateway_cli._find_legacy_sarthi_units()
    ...
    def test_print_legacy_unit_warning_shows_migration_hint(self, tmp_path, monkeypatch, capsys):
        ...
        assert "sarthi.service" in out
        assert "sarthi gateway migrate-legacy" in out
    ...
    def test_handles_unreadable_unit_file_gracefully(self, tmp_path, monkeypatch):
        ...
        user_dir, _ = self._setup_search_paths(tmp_path, monkeypatch)
        unreadable = user_dir / "sarthi.service"
...
class TestRemoveLegacySarthiUnits:
    """Tests for remove_legacy_sarthi_units (the migration action)."""
    ...
    def test_returns_zero_when_no_legacy_units(self, tmp_path, monkeypatch, capsys):
        ...
        removed, remaining = gateway_cli.remove_legacy_sarthi_units(interactive=False)
    ...
    def test_dry_run_lists_without_removing(self, tmp_path, monkeypatch, capsys):
        ...
        removed, remaining = gateway_cli.remove_legacy_sarthi_units(
    ...
    def test_removes_user_scope_legacy_unit(self, tmp_path, monkeypatch, capsys):
        ...
        assert any("--user stop sarthi.service" in c for c in cmds_joined)
        assert any("--user disable sarthi.service" in c for c in cmds_joined)
    ...
    def test_system_scope_without_root_defers_removal(self, tmp_path, monkeypatch, capsys):
        ...
        out = capsys.readouterr().out
        assert "sudo sarthi gateway migrate-legacy" in out
    ...
    def test_system_scope_with_root_removes(self, tmp_path, monkeypatch, capsys):
        ...
        assert any(
            c.startswith("systemctl stop sarthi.service") for c in cmds_joined
        )
        assert any(
            c.startswith("systemctl disable sarthi.service") for c in cmds_joined
        )
    ...
    def test_removes_both_scopes_with_root(self, tmp_path, monkeypatch, capsys):
        ...
        user_legacy = user_dir / "sarthi.service"
        system_legacy = system_dir / "sarthi.service"
    ...
    def test_does_not_touch_profile_units_during_migration(
        ...
        """Teknium's constraint: profile units (sarthi-gateway-coder.service)
        ...
        profile_unit = user_dir / "sarthi-gateway-coder.service"
        profile_unit.write_text(self._OUR_UNIT_TEXT, encoding="utf-8")
        default_unit = user_dir / "sarthi-gateway.service"
    ...
    def test_interactive_prompt_no_skips_removal(self, tmp_path, monkeypatch, capsys):
        ...
        removed, remaining = gateway_cli.remove_legacy_sarthi_units(interactive=True)
...
class TestMigrateLegacyCommand:
    """Tests for the `sarthi gateway migrate-legacy` subcommand dispatch."""
    ...
    def test_migrate_legacy_subparser_accepts_dry_run_and_yes(self):
        ...
        import sarthi_cli.main as cli_main
        ...
        # public, just confirm that `sarthi gateway --help` shows it.
        import subprocess
        import sys

        project_root = cli_main.PROJECT_ROOT if hasattr(cli_main, "PROJECT_ROOT") else None
        if project_root is None:
            import sarthi_cli.gateway as gw
            project_root = gw.PROJECT_ROOT

        result = subprocess.run(
            [sys.executable, "-m", "sarthi_cli.main", "gateway", "--help"],
    ...
    def test_gateway_command_migrate_legacy_dispatches(
        ...
        monkeypatch.setattr(gateway_cli, "remove_legacy_sarthi_units", fake_remove)
        monkeypatch.setattr(gateway_cli, "supports_systemd_services", lambda: True)
        monkeypatch.setattr(gateway_cli, "is_macos", lambda: False)
...
class TestGatewayStatusParser:
    def test_gateway_status_subparser_accepts_full_flag(self):
        ...
        result = subprocess.run(
            [sys.executable, "-m", "sarthi_cli.main", "gateway", "status", "-l", "--help"],
...
class TestSystemdInstallOffersLegacyRemoval:
    ...
    def test_install_offers_removal_when_legacy_detected(
        ...
        # has_legacy_sarthi_units must return True
        monkeypatch.setattr(gateway_cli, "has_legacy_sarthi_units", lambda: True)
        monkeypatch.setattr(gateway_cli, "remove_legacy_sarthi_units", fake_remove)
...
class TestSystemScopeWizardPreCheck:
    ...
    def _setup_units(tmp_path, monkeypatch, system_present: bool, user_present: bool):
        ...
        usr_dir.mkdir()
        if system_present:
            (sys_dir / "sarthi-gateway.service").write_text("[Unit]\n")
        if user_present:
            (usr_dir / "sarthi-gateway.service").write_text("[Unit]\n")
        monkeypatch.setattr(
            gateway_cli,
            "get_systemd_unit_path",
            lambda system=False: (sys_dir if system else usr_dir) / "sarthi-gateway.service",
        )
    ...
    def test_non_root_with_explicit_system_arg_returns_true(self, tmp_path, monkeypatch):
        # Caller passed system=True explicitly (e.g. ``sarthi gateway start --system``).
        self._setup_units(tmp_path, monkeypatch, system_present=False, user_present=False)
        monkeypatch.setattr(gateway_cli.os, "geteuid", lambda: 1000)
...
class TestSystemScopeRemediationOutput:
    ...
    def test_start_remediation_mentions_sudo_systemctl_and_uninstall(self, capsys, monkeypatch):
        monkeypatch.setattr(gateway_cli, "get_service_name", lambda: "sarthi-gateway")
        ...
        assert "sudo systemctl start sarthi-gateway" in out
        assert "sudo sarthi gateway uninstall --system" in out
        assert "sarthi gateway install" in out
    ...
    def test_restart_remediation_uses_systemctl_restart(self, capsys, monkeypatch):
        ...
        assert "sudo systemctl restart sarthi-gateway" in out
    ...
    def test_stop_remediation_uses_systemctl_stop(self, capsys, monkeypatch):
        ...
        assert "sudo systemctl stop sarthi-gateway" in out


class TestGatewayCommandCatchesSystemScopeError:
    """The direct CLI path (``sarthi gateway start --system`` etc.) must
...
class TestServiceWorkingDirIsStable:
    ...
    (SARTHI_HOME), never the source checkout / worktree, so a relocated or
    deleted checkout can't crash-loop the unit on CHDIR (status=200).
    """

    def test_stable_working_dir_uses_sarthi_home(self, tmp_path, monkeypatch):
        home = tmp_path / ".sarthi"
        home.mkdir()
        monkeypatch.setattr(gateway_cli, "get_sarthi_home", lambda: home)
    ...
    def test_stable_working_dir_falls_back_to_project_root(self, tmp_path, monkeypatch):
        # SARTHI_HOME points somewhere that does not exist -> fall back.
        missing = tmp_path / "does-not-exist" / ".sarthi"
        monkeypatch.setattr(gateway_cli, "get_sarthi_home", lambda: missing)
        assert gateway_cli._stable_service_working_dir() == str(gateway_cli.PROJECT_ROOT)

    def test_user_unit_workingdirectory_is_sarthi_home_not_checkout(self, tmp_path, monkeypatch):
        home = tmp_path / ".sarthi"
        home.mkdir()
        monkeypatch.setattr(gateway_cli, "get_sarthi_home", lambda: home)
        unit = gateway_cli.generate_systemd_unit(system=False)
    ...
    def test_launchd_workingdirectory_is_sarthi_home(self, tmp_path, monkeypatch):
        import re
...
class TestLaunchctlBootstrapEioRetry:
    ...
    PLIST = "/tmp/ai.sarthi.gateway.plist"
    DOMAIN = "gui/501"
    LABEL = "ai.sarthi.gateway"
