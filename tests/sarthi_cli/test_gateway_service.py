...
class TestGeneratedSystemdUnits:
    ...
    def test_launchd_plist_persists_configured_nofile_soft_limit(self, monkeypatch):
        ...
        plist rewrite by `sarthi gateway start` cannot strip the FD floor and
        reintroduce EMFILE crashes (launchd default soft limit is 256)."""
        import sarthi_cli.resource_limits as resource_limits
...
class TestSystemUnitSarthiHome:
    ...
    def test_empty_managed_node_dir_uses_only_ambient_fallback(
        ...
        managed_bin = tmp_path / ".sarthi" / "node" / "bin"
        ...
        gateway_cli._append_node_dir_for_service(entries, tmp_path / ".sarthi")
    ...
    def test_managed_node_makes_system_unit_independent_of_callers_path(
        ...
        target_sarthi = target_home / ".sarthi"
        root_home = tmp_path / "root"
        root_sarthi = root_home / ".sarthi"
        managed_bin = target_sarthi / "node" / "bin"
        managed_bin.mkdir(parents=True)
        node = managed_bin / "node"
        node.write_text("#!/bin/sh\n")
        node.chmod(0o755)
        ...
        monkeypatch.setattr(gateway_cli, "get_sarthi_home", lambda: root_sarthi)
    ...
    def test_node_path_lookup_remains_fallback_without_managed_node(
        ...
        monkeypatch.setattr(
            "sarthi_constants.iter_sarthi_node_dirs", lambda root=None: []
        )
        monkeypatch.setattr(
            "sarthi_constants.sarthi_managed_node_tree_present",
