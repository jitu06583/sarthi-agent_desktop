...
def test_discover_all_plugins_includes_entrypoint_plugins(monkeypatch, tmp_path):
    ...
    dist = SimpleNamespace(
        version="0.1.0",
        metadata={"Summary": "Karpathy-style LLM Wikis for Sarthi"},
    )
    entry_point = SimpleNamespace(
        name="wiki",
        value="adapters.sarthi.cli_plugin",
    ...
    monkeypatch.setattr(
        "sarthi_cli.plugins.get_bundled_plugins_dir",
        lambda: bundled_dir,
    )
    monkeypatch.setattr(
    ...
    assert entries == [
        (
            "wiki",
            "0.1.0",
            "Karpathy-style LLM Wikis for Sarthi",
            "entrypoint",
            "adapters.sarthi.cli_plugin",
            "wiki",
        )
    ]
...
def test_declared_capabilities_for_entrypoint_uses_distribution_metadata(
    ...
    monkeypatch.setattr(plugins_cmd, "_plugins_dir", lambda: user_dir)
    monkeypatch.setattr(
        "sarthi_cli.plugins.get_bundled_plugins_dir", lambda: bundled_dir
    )
    monkeypatch.setattr(
        plugins_cmd.importlib.metadata,
        "entry_points",
