...
def test_sessions_export_rejects_oversized_single_before_touching_output(
    ...
    monkeypatch.setattr(sarthi_state, "resolved_max_export_messages", lambda: 2)
    materialized = []
    ...
    result = SarthiConsoleEngine().execute(
...
def test_capture_output_surfaces_string_exit_code_as_command_error():
    from sarthi_cli.console_engine import ConsoleCommandError, _capture_output
...
def test_execute_handler_string_exit_returns_error_not_crash(_isolate_sarthi_home):
    result = SarthiConsoleEngine().execute(
...
def test_console_checkpoints_prune_does_not_reprompt_for_orphans(
    _isolate_sarthi_home, monkeypatch
):
    ...
    result = SarthiConsoleEngine().execute("checkpoints prune", confirmed=True)
