...
SessionDB.session_gateway_runtime (sarthi_state.py) — the round trip that
makes `sarthi --resume` reopen a session on the model/provider it actually
...
def _make_stub(**overrides):
    """Bare SarthiCLI the way resume paths see it (no __init__)."""
    stub = object.__new__(cli_mod.SarthiCLI)
