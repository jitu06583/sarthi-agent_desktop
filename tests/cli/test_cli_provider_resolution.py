...
def test_sarthi_cli_init_does_not_eagerly_resolve_runtime_provider(monkeypatch):
    cli = _import_cli()
    calls = {"count": 0}

    def _unexpected_runtime_resolve(**kwargs):
        calls["count"] += 1
        raise AssertionError("resolve_runtime_provider should not be called in SarthiCLI.__init__")

    monkeypatch.setattr("sarthi_cli.runtime_provider.resolve_runtime_provider", _unexpected_runtime_resolve)
    monkeypatch.setattr("sarthi_cli.runtime_provider.format_runtime_provider_error", lambda exc: str(exc))

    shell = cli.SarthiCLI(model="gpt-5", compact=True, max_turns=1)
...
def test_runtime_resolution_failure_is_not_sticky(monkeypatch):
    ...
    monkeypatch.setattr("sarthi_cli.runtime_provider.resolve_runtime_provider", _runtime_resolve)
    monkeypatch.setattr("sarthi_cli.runtime_provider.format_runtime_provider_error", lambda exc: str(exc))
...
def test_cli_prefers_config_provider_over_stale_env_override(monkeypatch):
    ...
    shell = cli.SarthiCLI(model="fireworks/minimax-m2p5", compact=True, max_turns=1)
...
def test_codex_provider_replaces_incompatible_default_model(monkeypatch):
    ...
    monkeypatch.setattr("sarthi_cli.runtime_provider.format_runtime_provider_error", lambda exc: str(exc))
    monkeypatch.setattr(
        "sarthi_cli.codex_models.get_codex_model_ids",
        lambda access_token=None: ["gpt-5.2-codex", "gpt-5.1-codex-mini"],
    )

    shell = cli.SarthiCLI(compact=True, max_turns=1)
...
def test_model_flow_nous_prints_subscription_guidance_without_mutating_explicit_tts(monkeypatch, capsys):
    monkeypatch.setattr(
        "sarthi_cli.nous_subscription.managed_nous_tools_enabled",
    ...
    monkeypatch.setattr(
        "sarthi_cli.auth.get_provider_auth_state",
        lambda provider: {"access_token": "nous-token"},
    )
    monkeypatch.setattr(
        "sarthi_cli.auth.resolve_nous_runtime_credentials",
        lambda *args, **kwargs: {
    ...
    monkeypatch.setattr(
        "sarthi_cli.auth.fetch_nous_models",
        lambda *args, **kwargs: ["claude-opus-4-6"],
    )
    monkeypatch.setattr("sarthi_cli.auth._prompt_model_selection", lambda model_ids, current_model="", pricing=None, **kw: "claude-opus-4-6")
    monkeypatch.setattr("sarthi_cli.auth._save_model_choice", lambda model: None)
    monkeypatch.setattr("sarthi_cli.auth._update_config_for_provider", lambda provider, url: None)

    sarthi_main._model_flow_nous(config, current_model="claude-opus-4-6")
...
def test_model_flow_nous_does_not_restore_stale_custom_api_key(tmp_path, monkeypatch):
    ...
    config_home = tmp_path / "sarthi"
    config_home.mkdir()
    monkeypatch.setenv("SARTHI_HOME", str(config_home))
    ...
    monkeypatch.setattr(
        "sarthi_cli.models.get_curated_nous_model_ids",
        lambda: [selected_model],
    )
    monkeypatch.setattr("sarthi_cli.models.get_pricing_for_provider", lambda provider: {})
    monkeypatch.setattr("sarthi_cli.models.check_nous_free_tier", lambda **kwargs: False)
    monkeypatch.setattr(
        "sarthi_cli.models.union_with_portal_paid_recommendations",
        lambda model_ids, pricing, portal_url: (model_ids, pricing),
    )
    monkeypatch.setattr(
        "sarthi_cli.auth._prompt_model_selection",
        lambda *args, **kwargs: selected_model,
    )
    monkeypatch.setattr(
        "sarthi_cli.nous_subscription.prompt_enable_tool_gateway",
        lambda config: None,
    )

    sarthi_main._model_flow_nous(stale_config, current_model="glm-5.2")
...
def test_model_flow_openrouter_clears_stale_custom_key(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "sarthi_cli.main._prompt_api_key",
        lambda *args, **kwargs: ("sk-openrouter", False),
    )
    monkeypatch.setattr(
        "sarthi_cli.models.model_ids",
        lambda **kwargs: ["anthropic/claude-sonnet-4.6"],
    )
    monkeypatch.setattr("sarthi_cli.models.get_pricing_for_provider", lambda *a, **k: {})
    monkeypatch.setattr(
        "sarthi_cli.auth._prompt_model_selection",
        lambda *args, **kwargs: "anthropic/claude-sonnet-4.6",
    )
    monkeypatch.setattr("sarthi_cli.auth.deactivate_provider", lambda: None)

    sarthi_main._model_flow_openrouter({}, current_model="glm-5.2")
...
def test_model_flow_anthropic_clears_stale_custom_key_and_mode(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("sarthi_cli.auth.get_anthropic_key", lambda: "sk-ant-api03-test")
    monkeypatch.setattr(
    ...
    monkeypatch.setattr(
        "sarthi_cli.model_setup_flows._prompt_auth_credentials_choice",
        lambda title: "use",
    )
    monkeypatch.setattr(
        "sarthi_cli.auth._prompt_model_selection",
    ...
    sarthi_main._model_flow_anthropic({}, current_model="glm-5.2")
...
def test_model_flow_nous_offers_tool_gateway_prompt_when_unconfigured(monkeypatch, capsys):
    ...
    monkeypatch.setattr("sarthi_cli.setup.prompt_checklist", _fake_checklist, raising=False)
...
def test_codex_provider_preserves_explicit_codex_model(monkeypatch):
    ...
    shell = cli.SarthiCLI(model="gpt-5.1-codex-mini", compact=True, max_turns=1)
...
def test_codex_provider_strips_provider_prefix_from_model(monkeypatch):
    ...
    shell = cli.SarthiCLI(model="openai/gpt-5.3-codex", compact=True, max_turns=1)
...
def test_cmd_model_falls_back_to_auto_on_invalid_provider(monkeypatch, capsys):
    monkeypatch.setattr(
        "sarthi_cli.config.load_config",
        lambda: {"model": {"default": "gpt-5", "provider": "invalid-provider"}},
    )
    monkeypatch.setattr("sarthi_cli.config.save_config", lambda cfg: None)
    monkeypatch.setattr("sarthi_cli.config.get_env_value", lambda key: "")
    monkeypatch.setattr("sarthi_cli.config.save_env_value", lambda key, value: None)
    ...
    monkeypatch.setattr("sarthi_cli.auth.resolve_provider", _resolve_provider)
    monkeypatch.setattr(sarthi_main, "_prompt_provider_choice", lambda choices, **kwargs: len(choices) - 1)
    monkeypatch.setattr("sys.stdin", type("FakeTTY", (), {"isatty": lambda self: True})())

    sarthi_main.cmd_model(SimpleNamespace())
...
def test_model_flow_custom_saves_verified_v1_base_url(monkeypatch, capsys):
    monkeypatch.setattr(
        "sarthi_cli.config.get_env_value",
        lambda key: "" if key in {"OPENAI_BASE_URL", "OPENAI_API_KEY"} else "",
    )
    saved_env = {}
    monkeypatch.setattr("sarthi_cli.config.save_env_value", lambda key, value: saved_env.__setitem__(key, value))
    monkeypatch.setattr("sarthi_cli.auth._save_model_choice", lambda model: saved_env.__setitem__("MODEL", model))
    monkeypatch.setattr("sarthi_cli.auth.deactivate_provider", lambda: None)
    monkeypatch.setattr("sarthi_cli.main._save_custom_provider", lambda *args, **kwargs: None)
    monkeypatch.setattr(
        "sarthi_cli.models.probe_api_models",
    ...
    monkeypatch.setattr("sarthi_cli.secret_prompt.masked_secret_prompt", lambda _prompt="": next(answers))

    sarthi_main._model_flow_custom({})
    output = capsys.readouterr().out
...
def test_model_flow_custom_persists_selected_api_mode(monkeypatch):
    ...
    monkeypatch.setattr("sarthi_cli.config.load_config", lambda: saved_cfg)
    monkeypatch.setattr("sarthi_cli.config.save_config", lambda cfg: saved_cfg.update(cfg))
    monkeypatch.setattr(
        "sarthi_cli.main._save_custom_provider",
    ...
    monkeypatch.setattr("builtins.input", lambda _prompt="": next(answers))
    monkeypatch.setattr("sarthi_cli.secret_prompt.masked_secret_prompt", lambda _prompt="": "test-key")

    sarthi_main._model_flow_custom({"model": {"provider": "custom"}})
...
def test_cmd_model_forwards_nous_login_tls_options(monkeypatch):
    monkeypatch.setattr(sarthi_main, "_require_tty", lambda *a: None)
    monkeypatch.setattr(
        "sarthi_cli.config.load_config",
    ...
    monkeypatch.setattr("sarthi_cli.config.get_env_value", lambda key: "")
    monkeypatch.setattr("sarthi_cli.config.save_env_value", lambda key, value: None)
    monkeypatch.setattr("sarthi_cli.auth.resolve_provider", lambda requested, **kwargs: "nous")
    monkeypatch.setattr("sarthi_cli.auth.get_provider_auth_state", lambda provider_id: None)
    monkeypatch.setattr(sarthi_main, "_prompt_provider_choice", lambda choices, **kwargs: 0)
    ...
    monkeypatch.setattr("sarthi_cli.auth._login_nous", _fake_login)

    sarthi_main.cmd_model(
        SimpleNamespace(
            portal_url="https://portal.nousresearch.com",
            inference_url="https://inference.nousresearch.com/v1",
            client_id="sarthi-local",
...
def test_auto_provider_name_localhost():
    from sarthi_cli.main import _auto_provider_name
...
def test_save_custom_provider_uses_provided_name(monkeypatch, tmp_path):
    ...
    import yaml
    from sarthi_cli.main import _save_custom_provider

    cfg_path = tmp_path / "config.yaml"
    cfg_path.write_text(yaml.dump({}))

    monkeypatch.setattr(
        "sarthi_cli.config.load_config", lambda: yaml.safe_load(cfg_path.read_text()) or {},
    )
    saved = {}
    def _save(cfg):
        saved.update(cfg)
    monkeypatch.setattr("sarthi_cli.config.save_config", _save)
