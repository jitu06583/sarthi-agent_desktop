...
class TestLightModeDetection:
    def test_sarthi_light_env_true_forces_light(self, cli_mod, monkeypatch):
        monkeypatch.setenv("SARTHI_LIGHT", "1")
        assert cli_mod._detect_light_mode() is True

    def test_sarthi_light_env_false_forces_dark(self, cli_mod, monkeypatch):
        monkeypatch.setenv("SARTHI_LIGHT", "0")
        # Also blank out other signals so nothing else flips it light.
        monkeypatch.delenv("SARTHI_TUI_LIGHT", raising=False)
        monkeypatch.delenv("SARTHI_TUI_THEME", raising=False)
        monkeypatch.delenv("SARTHI_TUI_BACKGROUND", raising=False)
        monkeypatch.delenv("COLORFGBG", raising=False)
        assert cli_mod._detect_light_mode() is False

    def test_theme_hint_light(self, cli_mod, monkeypatch):
        monkeypatch.delenv("SARTHI_LIGHT", raising=False)
        monkeypatch.delenv("SARTHI_TUI_LIGHT", raising=False)
        monkeypatch.setenv("SARTHI_TUI_THEME", "light")
        assert cli_mod._detect_light_mode() is True
    ...
    def test_background_hex_hint_light(self, cli_mod, monkeypatch):
        ...
        monkeypatch.delenv("SARTHI_TUI_LIGHT", raising=False)
        monkeypatch.delenv("SARTHI_TUI_THEME", raising=False)
        monkeypatch.setenv("SARTHI_TUI_BACKGROUND", "#FFFFFF")
        assert cli_mod._detect_light_mode() is True
    ...
    def test_background_hex_hint_dark(self, cli_mod, monkeypatch):
        ...
        monkeypatch.delenv("SARTHI_TUI_LIGHT", raising=False)
        monkeypatch.delenv("SARTHI_TUI_THEME", raising=False)
        monkeypatch.setenv("SARTHI_TUI_BACKGROUND", "#1a1a2e")
        monkeypatch.delenv("COLORFGBG", raising=False)
        assert cli_mod._detect_light_mode() is False
...
class TestLightModeRemap:
    ...
    def test_remap_known_dark_color(self, cli_mod, monkeypatch):
        ...
        assert cli_mod._maybe_remap_for_light_mode("#2DD4BF") == "#9A6B00"
...
class TestSkinConfigHook:
    ...
    def test_hook_installed(self, cli_mod):
        from sarthi_cli.skin_engine import SkinConfig

        assert getattr(SkinConfig, "_sarthi_light_mode_hook_installed", False) is True
    ...
    def test_skin_color_remaps_through_wrapper_in_light_mode(
        ...
        skin = SkinConfig(
            name="test",
            colors={"banner_text": "#FFF8DC", "response_border": "#2DD4BF"},
        )
