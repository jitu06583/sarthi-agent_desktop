...
def session_db(tmp_path):
    ...
    os.environ["SARTHI_HOME"] = str(tmp_path / ".sarthi")
    os.makedirs(tmp_path / ".sarthi", exist_ok=True)
    from sarthi_state import SessionDB
    db = SessionDB(db_path=tmp_path / ".sarthi" / "test_sessions.db")
    yield db
    db.close()
...
def cli_instance(tmp_path, session_db):
    """Create a minimal SarthiCLI-like object for testing _handle_branch_command."""
...
class TestBranchCommandCLI:
    ...
    def test_branch_creates_new_session(self, cli_instance, session_db):
        ...
        SarthiCLI._handle_branch_command(cli_instance, "/branch")
    ...
    def test_branch_with_custom_name(self, cli_instance, session_db):
        ...
        SarthiCLI._handle_branch_command(cli_instance, "/branch refactor approach")
    ...
    def test_branch_rotates_sarthi_session_id_env_and_context(self, cli_instance, session_db):
        """Branching must update process-local session-id readers too."""
        from cli import SarthiCLI
        from gateway.session_context import _UNSET, _VAR_MAP, get_session_env

        old_session_id = cli_instance.session_id
        os.environ["SARTHI_SESSION_ID"] = old_session_id
        _VAR_MAP["SARTHI_SESSION_ID"].set(old_session_id)

        try:
            SarthiCLI._handle_branch_command(cli_instance, "/branch")

            assert cli_instance.session_id != old_session_id
            assert os.environ["SARTHI_SESSION_ID"] == cli_instance.session_id
            assert get_session_env("SARTHI_SESSION_ID") == cli_instance.session_id
        finally:
...
class TestBranchCommandDef:
    ...
    def test_branch_in_registry(self):
        ...
        from sarthi_cli.commands import COMMAND_REGISTRY
