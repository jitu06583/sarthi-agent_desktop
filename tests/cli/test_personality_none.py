...
class TestGatewayPersonalityNone:
    ...
    async def test_empty_personality_list_uses_profile_display_path(self, tmp_path):
        ...
        with patch("gateway.run._sarthi_home", tmp_path), \
             patch("sarthi_constants.display_sarthi_home", return_value="~/.sarthi/profiles/coder"):
            event = self._make_event("")
            result = await runner._handle_personality_command(event)

        assert result == "No personalities configured in `~/.sarthi/profiles/coder/config.yaml`"
