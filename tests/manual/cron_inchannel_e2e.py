...
Uses a throwaway SARTHI_HOME so it never touches ~/.sarthi. No real names.
"""
...
def _fresh_home():
    """Point SARTHI_HOME at a throwaway dir BEFORE importing gateway modules
    (mirror.py binds _SESSIONS_INDEX from get_sarthi_home() at import time)."""
...
# Import AFTER SARTHI_HOME is set.
...
# a different get_sarthi_home() at import if something imported it earlier).
...
def main():
    ...
    print(f"SARTHI_HOME (throwaway): {HOME}")
