...
def test_root_session_no_compression(db):
    ...
    assert prov["currentSarthiSessionId"] == "root1"
    assert prov["rootSarthiSessionId"] == "root1"
    assert prov["parentSarthiSessionId"] is None
...
def test_compression_split_continuation(db):
    ...
    prov = build_session_provenance(
        db, "acp-1", "new", previous_sarthi_session_id="old"
    )
    assert prov["sessionKind"] == "continuation"
    assert prov["parentSarthiSessionId"] == "old"
    assert prov["rootSarthiSessionId"] == "old"
    assert prov["compressionDepth"] == 1
    assert prov["previousSarthiSessionId"] == "old"
...
def test_multi_depth_chain(db):
    ...
    assert prov["rootSarthiSessionId"] == "s0"
...
def test_non_compression_parent_is_root_not_continuation(db):
    ...
    assert prov["sessionKind"] == "root"
    assert prov["compressionDepth"] == 0
    assert prov["rootSarthiSessionId"] == "p"  # lineage root still walked
...
def test_no_false_rotation_when_head_unchanged(db):
    ...
    prov = build_session_provenance(
        db, "acp-1", "s", previous_sarthi_session_id="s"
    )
    assert "reason" not in prov
    assert "creatorKind" not in prov
    assert prov["previousSarthiSessionId"] == "s"
...
def test_meta_wrapper_shape(db):
    ...
    assert set(meta.keys()) == {"sarthi"}
    assert "sessionProvenance" in meta["sarthi"]
    assert meta["sarthi"]["sessionProvenance"]["currentSarthiSessionId"] == "root1"
