"""Tests for ACP Registry metadata shipped with Sarthi."""
...
def test_agent_json_matches_official_registry_required_fields():
    ...
    assert data["id"] == "sarthi-agent"
    assert re.fullmatch(r"[a-z][a-z0-9-]*", data["id"])
    assert data["name"] == "Sarthi Agent"
    assert data["description"]
    assert data["repository"] == "https://github.com/jitendra-singh-thakur/sarthi-agent"
    assert data["website"].startswith("https://sarthi-agent.vercel.app/")
...
def test_agent_json_uses_uvx_distribution_without_local_command_fields():
    ...
    assert uvx["package"] == f"sarthi-agent[acp]=={data['version']}"
    assert uvx["args"] == ["sarthi-acp"]
...
def test_agent_json_pins_uvx_package_to_pyproject_version():
    ...
    assert _manifest()["distribution"]["uvx"]["package"] == (
        f"sarthi-agent[acp]=={_pyproject_version()}"
    )
