"""Guard: Sarthi-owned subprocesses must not resolve managed runtimes by bare PATH.

Sarthi installs runtimes for itself — ``uv`` at ``$SARTHI_HOME/bin/uv``, Node at
``$SARTHI_HOME/node``. Neither directory is on the ambient PATH of an arbitrary
process, so ``shutil.which("uv")`` / ``shutil.which("node")`` in Sarthi's own
...
* when a system copy also exists, the one Sarthi does not own wins — which is
...
``iter_sarthi_node_dirs()``, ``resolve_uv()``, or ``ensure_uv()``. This test is
...
# Runtimes Sarthi provisions into SARTHI_HOME and must therefore resolve
# through a managed-aware helper rather than PATH.
_MANAGED_COMMANDS = frozenset({"uv", "node", "npm", "npx"})

# Directories that are not Sarthi-owned subprocess code: plugins ship their own
...
def test_no_unreviewed_bare_managed_runtime_lookups():
    ...
    assert not unexpected, (
        "Bare PATH lookup for a Sarthi-managed runtime.\n\n"
        + "\n".join(f"  {rel}:{lineno}  which({cmd!r})" for rel, cmd, lineno in unexpected)
        + "\n\n$SARTHI_HOME/bin (uv) and $SARTHI_HOME/node are not on an "
        ...
        "  node/npm -> sarthi_constants.find_node_executable()\n"
        "  PATH env -> sarthi_constants.iter_sarthi_node_dirs()\n"
...
def test_managed_node_helpers_exist(helper):
    ...
    assert callable(getattr(sarthi_constants, helper))


def test_managed_uv_helpers_exist():
    from sarthi_cli.managed_uv import ensure_uv, managed_uv_path, resolve_uv
