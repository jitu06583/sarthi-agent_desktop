    ...
    with patch.dict(
        "sys.modules",
        {
            "sarthi_constants": MagicMock(
                get_sarthi_home=MagicMock(return_value="/tmp/sarthi_test_review_summary")
            ),
...
def test_init_session_attaches_background_review_callback(server, monkeypatch):
    ...
    cb("💾 Self-improvement review: Skill 'sarthi-release' patched")
    ...
    assert sid == "sid-abc"
    assert payload == {
        "text": "💾 Self-improvement review: Skill 'sarthi-release' patched"
    }
