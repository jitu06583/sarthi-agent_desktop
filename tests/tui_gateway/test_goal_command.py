...
def sarthi_home(tmp_path, monkeypatch):
    ...
    # Bust the goal-module DB cache so it re-resolves SARTHI_HOME.
...
@pytest.fixture()
def server(sarthi_home, monkeypatch):
    ...
    # Pin config resolution to the isolated SARTHI_HOME. Sibling test
    ...
    # module-level ``_sarthi_home = get_sarthi_home()`` snapshot freezes
    ...
    monkeypatch.setattr(mod, "_sarthi_home", sarthi_home)
...
def test_moa_bare_returns_usage(server, session, sarthi_home):
    _write_moa_config(sarthi_home, """
moa:
