...
``platform="tui"`` / ``chat_id=SARTHI_SESSION_KEY``, but no component ever
...
class TestCollectKanbanNotifications:
    ...
    def test_profile_scoped_session_reads_the_shared_board(self, tmp_path):
        ...
        sarthi_cli/kanban_db.py module docstring): ``kanban_home()`` anchors on
        ``get_default_sarthi_root()``, which resolves the process env and
        ...
        token = set_sarthi_home_override(str(other_profile_home))
        try:
