...
def test_deleted_sibling_worktree_folds_into_parent_home_checkout():
    ...
    resolve = _resolver({"/www/sarthi-agent": ("/www/sarthi-agent", "/www/sarthi-agent")})
    sessions = [
        _session("/www/sarthi-agent", branch="main"),
        _session("/www/sarthi-agent-session-links"),
    ]

    tree = pt.build_tree([], sessions, [], resolve, hydrate=True)
    project = tree["projects"][0]

    assert [p["id"] for p in tree["projects"]] == ["/www/sarthi-agent"]
    assert _lane_ids(project) == ["/www/sarthi-agent::branch::main"]
    main = project["repos"][0]["groups"][0]
    assert main["isMain"] and main["path"] == "/www/sarthi-agent"
