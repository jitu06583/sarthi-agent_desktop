"""The test suite must never write into the operator's real Sarthi logs.

`sarthi_cli/main.py` calls `setup_logging()` at module scope, which resolves
...
the whole pytest session's logging to `<SARTHI_HOME>/logs/agent.log`.

If SARTHI_HOME is not already sandboxed at that moment, that is the
...
`tests/conftest.py` sets SARTHI_HOME at module scope for that reason - this
...
def _all_file_destinations() -> list[str]:
    ...
    try:
        ...
        listener = getattr(sarthi_logging, "_queue_listener", None)
        if listener is not None:
            collect(getattr(listener, "handlers", ()))
    except Exception:
        pass
...
class TestLogIsolation:
    def test_sarthi_home_is_sandboxed_before_imports(self):
        # Deliberately NOT os.environ: by test time the per-test `_isolate_env`
        # fixture has sandboxed SARTHI_HOME, so reading it here would pass even
        # with the conftest block deleted. Assert the value captured at conftest
        # import, which is the moment that actually matters.
        from tests.conftest import SARTHI_HOME_AT_CONFTEST_IMPORT as home

        assert home, "conftest must set SARTHI_HOME before test modules import"
        assert Path(home).resolve() != _real_sarthi_home().resolve(), (
            f"SARTHI_HOME pointed at the operator's real home ({home}) when "
    ...
    def test_importing_the_cli_does_not_target_the_real_logs(self):
        pytest.importorskip("sarthi_cli.main")

        real_logs = str(_real_sarthi_home() / "logs")
        offenders = [p for p in _all_file_destinations() if p.startswith(real_logs)]

        assert offenders == [], (
            "the test session is writing into the operator's real Sarthi logs:\n  "
            + "\n  ".join(offenders)
        )
