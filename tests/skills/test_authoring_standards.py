...
def test_required_frontmatter_fields(p):
    ...
    sarthi = (fm.get("metadata") or {}).get("sarthi") or {}
    if not (sarthi.get("tags") or fm.get("tags")) and not _grandfathered(p, "tags"):
        pytest.fail(f"{_rel(p)}: no tags (metadata.sarthi.tags or top-level tags)")
...
def test_related_skills_resolve(p):
    ...
    sarthi = (fm.get("metadata") or {}).get("sarthi") or {}
    dangling = [
        rs for rs in (sarthi.get("related_skills") or []) if rs not in _all_names()
    ]
