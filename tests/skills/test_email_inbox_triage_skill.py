...
def test_frontmatter_required_fields():
    ...
    sarthi = fm["metadata"]["sarthi"]
    assert sarthi["tags"]
    assert "related_skills" in sarthi
...
def test_author_credits_human_first():
    fm, _ = _frontmatter_and_body()
    assert not fm["author"].startswith("Sarthi Agent"), "human contributor must be credited first"
...
def test_related_skills_resolve_in_repo():
    ...
    for name in fm["metadata"]["sarthi"]["related_skills"]:
        hits = (
