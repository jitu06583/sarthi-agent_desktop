...
class TestRuntimeGate:
    ...
    def test_get_langfuse_does_not_import_sarthi_config(self, monkeypatch):
        ...
        # Drop any cached import of sarthi_cli.config.
        sys.modules.pop("sarthi_cli.config", None)

        langfuse_plugin = self._fresh_plugin()
        for _ in range(20):
            langfuse_plugin._get_langfuse()

        assert "sarthi_cli.config" not in sys.modules, (
            "langfuse plugin imported sarthi_cli.config — regression toward "
...
class TestPlaceholderKeyDetection:
    ...
    def test_validate_langfuse_key_rejects_wrong_prefix(self, monkeypatch):
        ...
        msg = plugin._validate_langfuse_key(
            "SARTHI_LANGFUSE_PUBLIC_KEY", "placeholder"
        )
        assert msg is not None
        assert "SARTHI_LANGFUSE_PUBLIC_KEY" in msg
        assert "pk-lf-" in msg
    ...
    def test_validate_langfuse_key_unknown_name_passes(self, monkeypatch):
        ...
        self._clear_env(monkeypatch)
        plugin = self._fresh_plugin()
        assert plugin._validate_langfuse_key("SARTHI_LANGFUSE_BASE_URL", "anything") is None
    ...
    def test_common_placeholders_detected(self, monkeypatch, caplog, placeholder):
        ...
        with caplog.at_level(logging.WARNING, logger=self.LOGGER_NAME):
            assert plugin._get_langfuse() is None
        assert "SARTHI_LANGFUSE_PUBLIC_KEY" in caplog.text

    def test_legacy_LANGFUSE_PUBLIC_KEY_also_validated(self, monkeypatch, caplog):
        """The plugin reads both the canonical SARTHI_-prefixed env var and
        ...
        # SARTHI_-prefixed one — operators set the SARTHI_-prefixed one).
        assert "SARTHI_LANGFUSE_PUBLIC_KEY" in caplog.text
