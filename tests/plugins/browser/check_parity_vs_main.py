...
Run from the PR worktree:
    ...
    cd ~/.sarthi/sarthi-agent/.worktrees/browser-providers-plugin
...
# checkout lives two levels up at ``~/.sarthi/sarthi-agent``.
MAIN_DIR = REPO_ROOT.parent.parent  # ~/.sarthi/sarthi-agent
PR_DIR = REPO_ROOT  # the worktree we're in
assert (MAIN_DIR / "tools" / "browser_tool.py").exists(), (
    f"MAIN_DIR={MAIN_DIR} doesn't look like a sarthi-agent checkout"
)
...
# Isolated SARTHI_HOME for the config write.
