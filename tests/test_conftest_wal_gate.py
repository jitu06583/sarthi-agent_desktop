"""The conftest WAL gate must agree with sarthi_state, and must not import it.

``tests/conftest.py::_wal_is_usable`` duplicates the SQLite WAL-reset version
predicate instead of importing ``sarthi_state``. That is deliberate: importing
``sarthi_state`` during collection caches ``DEFAULT_DB_PATH`` from the real
``~/.sarthi`` before the per-test ``SARTHI_HOME`` redirect, which makes tests
...
def test_conftest_gate_agrees_with_sarthi_state(version_info, monkeypatch):
    ...
    assert _wal_is_usable() is not is_sqlite_wal_reset_vulnerable(version_info), (
        f"conftest gate and sarthi_state disagree for SQLite {version_info}"
    )


def test_conftest_does_not_import_sarthi_state_at_collection():
    """The gate must stay import-free of sarthi_state.

    Importing it during collection caches DEFAULT_DB_PATH from the real
    ~/.sarthi, so tests read live production sessions instead of a tempdir.
    Reading the source is not an option here (banned), so assert on behavior:
    the gate must work with ``sarthi_state`` absent from ``sys.modules`` and
    ...
    def guard(name, *args, **kwargs):
        if name == "sarthi_state" or name.startswith("sarthi_state."):
            blocked.append(name)
            raise AssertionError(
                "conftest._wal_is_usable imported sarthi_state — this caches "
                "DEFAULT_DB_PATH from the real ~/.sarthi during collection"
            )
        return real_import(name, *args, **kwargs)

    saved = sys.modules.pop("sarthi_state", None)
    ...
    try:
        ...
        if saved is not None:
            sys.modules["sarthi_state"] = saved
    assert not blocked
