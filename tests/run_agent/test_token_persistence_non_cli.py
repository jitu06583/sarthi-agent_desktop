...
def test_session_search_lazily_opens_db_when_entrypoint_did_not_pass_one(monkeypatch):
    ...
    sarthi_state = ModuleType("sarthi_state")
    sarthi_state.SessionDB = FakeSessionDB
    monkeypatch.setitem(sys.modules, "sarthi_state", sarthi_state)
    ...
    result = json.loads(agent._invoke_tool("session_search", {"query": "Sarthi"}, "task-id"))

    assert result["success"] is True
    assert captured["db"] is sentinel_db
    assert captured["query"] == "Sarthi"
