...
//!
//! Resolution order:
//!   1. Dev shortcut: a sibling repo checkout via $SARTHI_SETUP_DEV_REPO_ROOT
...
//! Electron app's APP_ROOT/../.. trick, because Sarthi-Setup.exe is meant
...
///
/// `pin` is the commit-or-branch from either Sarthi-Setup's build-time
...
pub async fn resolve(
    ...
    // 1. Dev shortcut.
    if let Ok(repo_root) = std::env::var("SARTHI_SETUP_DEV_REPO_ROOT") {
...
async fn download(kind: ScriptKind, commit_or_ref: &str, dest_path: &Path) -> Result<()> {
    ...
    let response = reqwest::Client::builder()
        ...
        .get(&url)
        .header("User-Agent", "sarthi-setup/0.0.1")
        .send()
        .await
...
mod tests {
    ...
    fn upgrade_cached_script_adds_bom_to_legacy_ps1() {
        ...
        let dir = std::env::temp_dir().join(format!("sarthi-bom-test-{}", std::process::id()));
    ...
    fn upgrade_cached_script_leaves_sh_untouched() {
        let dir = std::env::temp_dir().join(format!("sarthi-bom-sh-test-{}", std::process::id()));
        std::fs::create_dir_all(&dir).unwrap();
