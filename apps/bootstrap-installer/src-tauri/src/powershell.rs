...
/// read side does not see EOF until the last of them closes it. `sarthi update`
...
/// stranded with it.
///
/// Same bound `Invoke-SarthiStep` grew in `scripts/desktop-update/windows.ps1`
...
///
/// `sarthi_home_override` propagates to the child as $SARTHI_HOME so the
...
pub async fn run_script(
    ...
    if let Some(cwd) = stable_script_cwd(script_path, sarthi_home_override) {
        cmd.current_dir(cwd);
    }

    if let Some(home) = sarthi_home_override {
        cmd.env("SARTHI_HOME", home);
    }
...
fn stable_script_cwd<'a>(script_path: &'a Path, sarthi_home_override: Option<&'a str>) -> Option<&'a Path> {
    if let Some(home) = sarthi_home_override {
...
"#;
    ...
    #[test]
    fn stable_script_cwd_prefers_existing_sarthi_home() {
    ...
    /// child. `sarthi update` is this shape -- the Electron build alone is
    /// megabytes.
    #[cfg(unix)]
    #[tokio::test]
