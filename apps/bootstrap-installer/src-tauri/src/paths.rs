...
//!
//! Mirrors `sarthi_constants.get_sarthi_home()` from the Python CLI:
//!   Windows: %LOCALAPPDATA%\sarthi
//!   macOS:   ~/.sarthi
//!   Linux:   ~/.sarthi  (override via $SARTHI_HOME)
//!
//! NOTE (macOS): Python's get_sarthi_home(), scripts/install.sh, and the
//! Electron desktop's resolveSarthiHome() ALL use ~/.sarthi on macOS — there
...
/// Returns the canonical Sarthi home directory, respecting $SARTHI_HOME if set.
pub fn sarthi_home() -> PathBuf {
    if let Ok(override_path) = std::env::var("SARTHI_HOME") {
    ...
    {
        // %LOCALAPPDATA%\sarthi — matches scripts/install.ps1's $SarthiHome.
        if let Some(local_app_data) = dirs::data_local_dir() {
            return local_app_data.join("sarthi");
        }
    }

    // macOS + Linux + fallback: ~/.sarthi (matches Python get_sarthi_home(),
    // install.sh, and the Electron desktop's resolveSarthiHome()).
    if let Some(home) = dirs::home_dir() {
        return home.join(".sarthi");
    }
...
pub fn bootstrap_cache_dir() -> PathBuf {
    sarthi_home().join("bootstrap-cache")
}
...
/// SARTHI_HOME so it survives repo checkout deletion (unlike anything under
/// sarthi-agent/).
///
/// On Windows this is `%LOCALAPPDATA%\sarthi\sarthi-setup.exe`; on other
...
/// mid-update re-locks the venv shim and triggers `force_kill_other_sarthi`,
/// which then kills that legitimate backend in a respawn loop (#50238).
///
/// Lives directly under SARTHI_HOME (same rationale as `installer_dest`) so the
/// Electron desktop — which resolves SARTHI_HOME identically and pins it into
/// the updater's env — agrees on the exact path.
pub fn update_in_progress_marker() -> PathBuf {
    sarthi_home().join(".sarthi-update-in-progress")
}
...
pub fn copy_self_to_sarthi_home() -> std::io::Result<()> {
    ...
    tracing::info!(?src, ?dest, "copied installer to SARTHI_HOME");
    Ok(())
}
...
/// We don't always know ACTIVE_SARTHI_ROOT until install.ps1 reports it, so
/// this is a probe helper, not a definitive path.
pub fn likely_bootstrap_marker(install_root: &Path) -> PathBuf {
    install_root.join(".sarthi-bootstrap-complete")
}

/// Initializes tracing to bootstrap-installer.log under SARTHI_HOME/logs/.
...
pub fn init_logging() -> Option<WorkerGuard> {
    ...
    if let Err(err) = std::fs::create_dir_all(&dir) {
        ...
        eprintln!("[sarthi-setup] could not create log dir {dir:?}: {err}");
        return None;
    }
    ...
    let env_filter = tracing_subscriber::EnvFilter::try_from_env("SARTHI_BOOTSTRAP_LOG")
...
#[tauri::command]
pub fn get_sarthi_home() -> String {
    sarthi_home().to_string_lossy().into_owned()
}
