...
export const $sarthiHome = atom<string | null>(null)
...
export async function initialize(): Promise<void> {
  ...
  if (fake) {
    unlisten = () => {}
    $logPath.set('~/.sarthi/logs/bootstrap-installer.log')
    $sarthiHome.set('~/.sarthi')
  ...
  try {
    const [logPath, sarthiHome, mode] = await Promise.all([
      invoke<string>('get_log_path'),
      invoke<string>('get_sarthi_home'),
      invoke<AppMode>('get_mode')
    ])

    $logPath.set(logPath)
    $sarthiHome.set(sarthiHome)
    $mode.set(mode)
  } catch (err) {
  ...
  unlisten = await listen<BootstrapEvent>('bootstrap', (event) => {
    ...
    switch (payload.type) {
      ...
      case 'complete':
        ...
        // Install: show the "launch Sarthi" success screen. Update: this is a
...
export async function startUpdate(): Promise<void> {
  ...
  // Update is driven by the desktop handing off (Sarthi-Setup.exe --update);
...
export async function launchSarthiDesktop(): Promise<void> {
  ...
  await invoke('launch_sarthi_desktop', { installRoot })
}
...
const FAKE_INSTALL_STAGES: FakeStage[] = [
  ...
  { name: 'repo', title: 'Sarthi repository' },
