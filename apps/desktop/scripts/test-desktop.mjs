...
// no longer bundles the Sarthi Agent Python payload (that's fetched at first
// launch via install.ps1 / install.sh, per the Phase 1 thin-installer flow).
const APP = (() => {
  if (PLATFORM === 'darwin') {
    const appPath = path.join(RELEASE_ROOT, `mac-${ARCH}`, 'Sarthi.app')
    return {
      appPath,
      binary: path.join(appPath, 'Contents', 'MacOS', 'Sarthi'),
  ...
  if (PLATFORM === 'win32') {
    ...
    return {
      appPath: unpacked,
      binary: path.join(unpacked, 'Sarthi.exe'),
  ...
  return {
    appPath: unpacked,
    binary: path.join(unpacked, 'Sarthi'),
    resourcesPath: path.join(unpacked, 'resources'),
    asarPath: path.join(unpacked, 'resources', 'app.asar'),
    unpackedDistIndex: path.join(unpacked, 'resources', 'app.asar.unpacked', 'dist', 'index.html')
  }
...
// Default SARTHI_HOME for non-sandboxed runs -- matches main.ts's
// resolveSarthiHome(). On Windows it's %LOCALAPPDATA%\sarthi; elsewhere
// it's ~/.sarthi. The fresh-install sandbox launchFresh() sets its own
// SARTHI_HOME and never touches this.
const DEFAULT_SARTHI_HOME = (() => {
  if (PLATFORM === 'win32' && process.env.LOCALAPPDATA) {
    return path.join(process.env.LOCALAPPDATA, 'sarthi')
  }
  return path.join(os.homedir(), '.sarthi')
})()
const VENV_ROOT = path.join(DEFAULT_SARTHI_HOME, 'sarthi-agent', 'venv')
const FRESH_SANDBOX_ROOT = path.join(os.tmpdir(), 'sarthi-desktop-fresh-install')
...
function ensurePackagedApp() {
  if (process.env.SARTHI_DESKTOP_SKIP_BUILD === '1' && exists(APP.binary)) {
    return
  }
...
function resolveDmgPath() {
  if (!exists(RELEASE_ROOT)) {
    return path.join(RELEASE_ROOT, `Sarthi-${PACKAGE_JSON.version}-${ARCH}.dmg`)
  }

  const prefix = `Sarthi-${PACKAGE_JSON.version}`
  const candidates = fs
  ...
  return candidates.length > 0
    ...
    : path.join(RELEASE_ROOT, `Sarthi-${PACKAGE_JSON.version}-${ARCH}.dmg`)
}

function resolveNsisPath() {
  // electron-builder NSIS artifactName template is 'Sarthi-${version}-${os}-${arch}.${ext}'
...
function ensureDmg() {
  ...
  if (process.env.SARTHI_DESKTOP_SKIP_BUILD === '1' && exists(resolveDmgPath())) {
    return
  }
...
function ensureNsis() {
  ...
  if (process.env.SARTHI_DESKTOP_SKIP_BUILD === '1' && resolveNsisPath()) {
    return
  }
...
function launchFresh() {
  ...
  const sarthiHome = path.join(sandbox, 'sarthi-home')
  ...
  env.SARTHI_DESKTOP_CWD = cwd
  env.SARTHI_DESKTOP_IGNORE_EXISTING = '1'
  env.SARTHI_DESKTOP_TEST_MODE = 'fresh-install'
  env.SARTHI_DESKTOP_USER_DATA_DIR = userDataDir
  env.SARTHI_HOME = sarthiHome
  delete env.SARTHI_DESKTOP_SARTHI
  delete env.SARTHI_DESKTOP_SARTHI_ROOT
  ...
  console.log(`  SARTHI_HOME: ${sarthiHome}`)
  console.log(`  cwd: ${cwd}`)

  return { runtimeRoot: path.join(sarthiHome, 'sarthi-agent', 'venv') }
}

// Validate the packaged bundle matches the thin-installer architecture:
//   - The Sarthi Agent Python payload is NOT shipped (it's fetched at first
...
function validateBundle() {
  ...
  // present anymore. If a stray ship of sarthi_cli sneaks back in we want
  // to fail loudly rather than re-introduce the 400MB delta we just removed.
  const staleFactoryMarker = path.join(APP.resourcesPath, 'sarthi-agent', 'sarthi_cli', 'main.py')
...
function help() {
  console.log(`Usage:
  npm run test:desktop:existing  # build packaged app, launch with normal PATH/existing Sarthi
  npm run test:desktop:fresh     # build packaged app, launch with temp userData + SARTHI_HOME
...
Fast rerun (skip rebuild if the packaged app already exists):
  SARTHI_DESKTOP_SKIP_BUILD=1 npm run test:desktop:fresh
`)
}
