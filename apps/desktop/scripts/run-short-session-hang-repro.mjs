...
const LEGACY_STORAGE_KEY = 'sarthi.desktop.inflightTurnJournal.v1'
const LEGACY_MIGRATION_KEY = 'sarthi.desktop.inflightTurnJournal.v2.migrated'
...
async function verifyInteractiveSurfaces(cdp, timed, measure, label) {
  ...
  const version = await timed(`version.ipc.${label}`, () => cdp.eval('window.sarthiDesktop.getVersion()'))
...
async function withTemporarySandbox(label, body) {
  const sandbox = mkdtempSync(join(tmpdir(), `sarthi-short-session-${label}-`))
...
async function executeRunInSandboxAttempt(target, index, warmup, mock, output, sandbox, attempt) {
  ...
  const sarthiHome = join(sandbox, 'sarthi-home')
  const userData = join(sandbox, 'electron-user-data')
  const desktopLog = join(sarthiHome, 'logs', 'desktop.log')
  ...
  writeSandboxConfig(sarthiHome, mock.url)
  ...
  const child = spawn(
    ...
    [target.targetDesktop, `--user-data-dir=${userData}`, `--remote-debugging-port=${port}`],
    {
      ...
      env: sanitizedEnv({
        SARTHI_DESKTOP_APP_NAME: `SarthiShortSession-${target.label}-${process.pid}-${index}-${warmup ? 'w' : 'm'}-${attempt}`,
        SARTHI_DESKTOP_SARTHI_ROOT: target.targetRoot,
        SARTHI_DESKTOP_IGNORE_EXISTING: '1',
        SARTHI_DESKTOP_USER_DATA_DIR: userData,
...
async function main() {
  ...
  const ephemeralRoot = mkdtempSync(join(tmpdir(), 'sarthi-short-session-ab-'))
  const prepared = []
