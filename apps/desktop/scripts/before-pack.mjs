 ...
 * product name (`Sarthi`). If a PREVIOUS `npm run pack` was interrupted
 ...
 *   '.../release/linux-unpacked/electron' -> '.../release/linux-unpacked/Sarthi'
 *
 * This is a hard failure with no obvious cause for the user — `sarthi desktop`
 ...
 * (the mac-unpacked Sarthi.app bundle) and Windows (win-unpacked), so we
 ...
 * Sarthi.exe that Windows can't load (truncated PE from a corrupt cached
 * Electron zip, wrong arch), the updater's integrity gate in
 * `sarthi desktop --build-only` (sarthi_cli/main.py
 ...
 */
export function preserveRollbackBackup(appOutDir, productExeName = 'Sarthi.exe') {
...
export default async function beforePack(context) {
  ...
  try {
    ...
    // rename fails.
    const productExe = `${(context && context.packager?.appInfo?.productFilename) || 'Sarthi'}.exe`
