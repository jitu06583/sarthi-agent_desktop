...
function createSandbox() {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), `sarthi-dev-mock-${Date.now()}`))
  ...
  return { root, sarthiHome, userDataDir, cleanup: () => fs.rmSync(root, { recursive: true, force: true }) }
}

function writeMockConfig(sarthiHome, mockUrl) {
  fs.writeFileSync(
    path.join(sarthiHome, 'config.yaml'),
...
`,
  ...
  fs.writeFileSync(path.join(sarthiHome, '.env'), 'MOCK_API_KEY=e2e-mock-key\n', 'utf8')
}
...
async function main() {
  ...
  writeMockConfig(sandbox.sarthiHome, mock.url)
  console.log(`  SARTHI_HOME: ${sandbox.sarthiHome}`)
  ...
  const env = {
    ...
    SARTHI_DESKTOP_APP_NAME: `SarthiDevMock-${Date.now()}`,
  }
