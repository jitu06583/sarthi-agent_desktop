...
test('terminalScriptEnv drops PATH in any casing and keeps the rest', () => {
  ...
  assert.deepEqual(env, { PYTHONPATH: '/repo', PYTHONUTF8: '1', SARTHI_HOME: '/home/b/.sarthi' })
})
...
test('buildTerminalScript execs the resolved runtime with its env', () => {
  const script = buildTerminalScript({
    args: ['-m', 'sarthi_cli.main', '--tui', '--resume', 'sess'],
    command: '/home/b/.sarthi/sarthi-agent/venv/bin/python',
    cwd: "/home/b/o'brien",
    env: { PYTHONPATH: '/home/b/.sarthi/sarthi-agent' },
    platform: 'darwin'
  })
  ...
  assert.equal(
    ...
    [
      ...
      `export PYTHONPATH='/home/b/.sarthi/sarthi-agent'`,
      `exec '/home/b/.sarthi/sarthi-agent/venv/bin/python' '-m' 'sarthi_cli.main' '--tui' '--resume' 'sess'`,
      ''
    ].join('\n')
  )
})
...
test('buildTerminalScript emits a cmd script on Windows', () => {
  const script = buildTerminalScript({
    ...
    command: 'C:\\sarthi\\venv\\Scripts\\sarthi.exe',
    cwd: 'C:\\Users\\b',
    env: { PYTHONUTF8: '1' },
    platform: 'win32'
  })
  ...
  assert.deepEqual(script.split('\r\n'), [
    ...
    'cd /d "C:\\Users\\b"',
    'set "PYTHONUTF8=1"',
    '"C:\\sarthi\\venv\\Scripts\\sarthi.exe" "--tui" "--resume" "sess"',
    ''
  ])
})
