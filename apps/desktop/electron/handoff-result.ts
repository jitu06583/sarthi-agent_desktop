 ...
 * console. It writes SARTHI_HOME/.sarthi-update-result.json on every exit
 ...
 * ever surfaces, and the user may not reopen Sarthi within 30 minutes.
...
export function handoffResultPath(sarthiHome: string): string {
  return path.join(sarthiHome, '.sarthi-update-result.json')
}
...
export function readAndConsumeHandoffResult(
  ...
  const file = handoffResultPath(sarthiHome)
  let raw: string
