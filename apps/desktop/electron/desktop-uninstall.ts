 ...
 * the three user-facing uninstall modes to the `sarthi uninstall` CLI flags,
...
/**
 * Map an uninstall mode to the `python -m sarthi_cli.uninstall` argv (after the
 * python executable). Uses the dedicated lightweight module entrypoint (not
 * `sarthi_cli.main`) so it can run under a system Python OUTSIDE the venv that
...
function uninstallArgsForMode(mode) {
  ...
  return ['-m', 'sarthi_cli.uninstall', '--mode', mode]
}
 ...
 *
 *   macOS:   …/Sarthi.app/Contents/MacOS/Sarthi  → …/Sarthi.app
 *   Windows: …\Sarthi\Sarthi.exe                 → …\Sarthi  (install dir)
...
function resolveRemovableAppPath(execPath, platform, env: any = {}) {
  ...
  if (platform === 'darwin') {
    // …/Sarthi.app/Contents/MacOS/Sarthi → strip 3 segments to the .app
    const macOsDir = p.dirname(exe) // …/Contents/MacOS
    const contents = p.dirname(macOsDir) // …/Contents
    const appBundle = p.dirname(contents) // …/Sarthi.app
  ...
  if (platform === 'win32') {
    // NSIS per-user installs Sarthi.exe directly in the install dir.
    const dir = p.dirname(exe)

    if (/[\\/]Sarthi$/i.test(dir) || /[\\/]sarthi-desktop$/i.test(dir)) {
      return dir
    }
  ...
  // Unpacked electron-builder tree: …/linux-unpacked/sarthi
  const dir = p.dirname(exe)
 ...
 * being deleted); `pythonPath` is prepended to PYTHONPATH so `import sarthi_cli`
 ...
 */
function buildPosixCleanupScript({ desktopPid, pythonExe, pythonPath, agentRoot, uninstallArgs, appPath, sarthiHome }) {
  ...
  const lines = [
    ...
    '  done',
    'fi',
    `export SARTHI_HOME=${q(sarthiHome)}`
  ]
 ...
 * modes + `pythonPath`=agentRoot so `import sarthi_cli` resolves from source
...
function buildWindowsCleanupScript({
  ...
  // a problem, but Sarthi install paths don't use them.
  const q = s => `"${String(s).replace(/"/g, '')}"`

  const lines = [
    '@echo off',
    'setlocal enableextensions',
    `set "SARTHI_HOME=${String(sarthiHome).replace(/"/g, '')}"`,
    `set "PID=${pid}"`
  ]
