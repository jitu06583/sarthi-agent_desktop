...
import { buildDesktopBackendEnv, sarthiManagedNodePathEntries, normalizeSarthiHomeRoot } from './backend-env'
import { isReauthRequiredError, makeNousCloudBackendDownError, waitForSarthiReady } from './backend-health'
import { backendCommandMatches, createBackendOwnership, createBackendShutdownCoordinator } from './backend-ownership'
import {
  canImportSarthiCli,
  execProbeSync,
  PROBE_TIMEOUT_MS,
  shouldTrustSarthiOverride,
  verifySarthiCli
} from './backend-probes'
...
import {
  ...
  resolveVenvSarthiCommand
} from './windows-sarthi-path'
...
const USER_DATA_OVERRIDE = process.env.SARTHI_DESKTOP_USER_DATA_DIR
...
const DEV_SERVER = process.env.SARTHI_DESKTOP_DEV_SERVER
const IS_PACKAGED = app.isPackaged || Boolean(process.env.SARTHI_DESKTOP_IS_PACKAGED)
...
// switches only apply pre-launch. Override with SARTHI_DESKTOP_DISABLE_GPU
...
if (REMOTE_DISPLAY_REASON) {
  ...
  console.log(
    `[sarthi] remote display detected (${REMOTE_DISPLAY_REASON}); disabling GPU hardware acceleration to prevent flicker`
  )
}
...
if (DEV_CDP.port) {
  ...
  console.log(
    `[sarthi] renderer debugging on http://127.0.0.1:${DEV_CDP.port} — anything that can reach it ` +
      'can run code in the renderer. SARTHI_DESKTOP_CDP_PORT=off to disable.'
  )
} else {
  const why = describeDevCdpDecision(DEV_CDP)

  if (why) {
    console.warn(`[sarthi] ${why}`)
  }
}
...
if (IS_WSL && !REMOTE_DISPLAY_REASON && fs.existsSync('/dev/dxg')) {
  ...
  console.log('[sarthi] WSL GPU passthrough (/dev/dxg) detected; enabling GPU acceleration')
}
...
// it). The value arrives via SARTHI_DESKTOP_PASSWORD_STORE, bridged by the
// `sarthi desktop` launcher from detection or `desktop.password_store` in
...
if (PASSWORD_STORE.warning) {
  console.warn(`[sarthi] ${PASSWORD_STORE.warning}`)
}

if (PASSWORD_STORE.store) {
  app.commandLine.appendSwitch('password-store', PASSWORD_STORE.store)
  console.log(`[sarthi] using password-store backend: ${PASSWORD_STORE.store}`)
}
...
// never go through `sarthi desktop`; it is version-scoped so an app update
...
if (IS_WINDOWS) {
  ...
  // expose Sarthi sessions/config to every packaged app on the machine.
  if (shouldAttemptAclRepair(priorMarker)) {
    const exeDir = path.dirname(process.execPath)
    const acl = grantAllApplicationPackagesAcl(exeDir, { execFileSync })

    if (acl.ok) {
      console.log(`[sarthi] granted ALL APPLICATION PACKAGES RX on ${exeDir} (#38216)`)
    } else if (acl.error && acl.error !== 'missing-target-or-exec') {
      console.warn(`[sarthi] AppContainer ACL grant failed on ${exeDir}: ${acl.error}`)
    }
  }
  ...
  if (sandboxDecision.enable && sandboxDecision.reason !== 'already-enabled') {
    ...
    console.log(
      `[sarthi] Windows sandbox fallback enabled (${sandboxDecision.reason}); launching with --no-sandbox (#38216)`
    )
  }
  ...
  app.on('child-process-gone', (_event, details) => {
    ...
    console.warn(
      `[sarthi] Windows GPU sandbox crashed (exit=${details?.exitCode}); relaunching once with --no-sandbox (#38216)`
    )
    ...
    try {
      ...
      console.error(`[sarthi] --no-sandbox relaunch failed: ${error?.message || error}`)
    }
  })
}

ipcMain.handle('sarthi:get-remote-display-reason', () => REMOTE_DISPLAY_REASON)
...
// always-on timers. A completely idle, minimized Sarthi burned ~20% CPU
    ...
    try {
      ...
      if (parsed && typeof parsed === 'object' && typeof parsed.commit === 'string' && parsed.commit.length >= 7) {
        if (parsed.schemaVersion !== INSTALL_STAMP_SCHEMA_VERSION) {
          console.warn(
            `[sarthi] install-stamp.json schemaVersion ${parsed.schemaVersion} != expected ${INSTALL_STAMP_SCHEMA_VERSION}; ignoring`
          )
      ...
      console.warn(`[sarthi] install-stamp.json found at ${p} , but parsing failed with ${e}`)
...
if (INSTALL_STAMP) {
  console.log(
    `[sarthi] install stamp: ${INSTALL_STAMP.commit.slice(0, 12)}${INSTALL_STAMP.branch ? ` (${INSTALL_STAMP.branch})` : ''}${INSTALL_STAMP.dirty ? ' [DIRTY]' : ''} from ${INSTALL_STAMP.source || 'unknown'}`
  )
} else if (IS_PACKAGED) {
  ...
  console.error(
    '[sarthi] WARNING: no install-stamp.json found in packaged build. First-launch bootstrap will not have a pinned ref to install.'
  )
}

// SARTHI_HOME — the user-facing root for everything Sarthi-related. Mirrors
// scripts/install.ps1's $SarthiHome and scripts/install.sh's $SARTHI_HOME.
//
// Defaults:
//   Windows: %LOCALAPPDATA%\sarthi (matches install.ps1)
//   macOS / Linux: ~/.sarthi (matches install.sh)
//
// Special case for Windows: if the user has a legacy ~/.sarthi directory
// (e.g., from a prior pip install or a manual setup) AND no
// %LOCALAPPDATA%\sarthi yet, prefer the legacy path so we don't orphan their
// existing config / sessions / .env. New installs go to %LOCALAPPDATA%.
//
// SARTHI_DESKTOP_USER_DATA_DIR (used by test:desktop:fresh) puts the sandbox
// SARTHI_HOME beneath the throwaway userData dir so a fresh-install run never
// touches the user's real ~/.sarthi / %LOCALAPPDATA%\sarthi.
function resolveSarthiHome() {
  if (process.env.SARTHI_HOME) {
    return normalizeSarthiHomeRoot(process.env.SARTHI_HOME)
  }

  if (USER_DATA_OVERRIDE) {
    return path.join(path.resolve(USER_DATA_OVERRIDE), 'sarthi-home')
  }

  if (IS_WINDOWS) {
    // A GUI app launched from Explorer inherits the environment block captured
    // at login, so a SARTHI_HOME set via `setx` AFTER login is invisible in
    // process.env even though the CLI (a fresh shell) sees it. Without this the
    // backend silently falls back to %LOCALAPPDATA%\sarthi and reports "No
    // inference provider configured" despite a valid configured home (#45471).
    // Consult the live User-scoped registry value before the default below.
    const fromRegistry = readWindowsUserEnvVar('SARTHI_HOME')

    if (fromRegistry) {
      return normalizeSarthiHomeRoot(fromRegistry)
    }
  }

  if (IS_WINDOWS && process.env.LOCALAPPDATA) {
    const localappdata = path.join(process.env.LOCALAPPDATA, 'sarthi')
    const legacy = path.join(app.getPath('home'), '.sarthi')

    // Migrate transparently to LOCALAPPDATA, but honour an existing legacy
    // ~/.sarthi setup (no LOCALAPPDATA install yet) so users don't lose state.
  ...
  return path.join(app.getPath('home'), '.sarthi')
}

const SARTHI_HOME = resolveSarthiHome()

function pathWithSarthiManagedNode(...entries) {
  const managed = sarthiManagedNodePathEntries(SARTHI_HOME).filter(directoryExists)

  return [...managed, ...entries, process.env.PATH].filter(Boolean).join(path.delimiter)
}

// ACTIVE_SARTHI_ROOT — the canonical mutable Sarthi install. Same path
// install.ps1 / install.sh use, so a desktop-only user and a CLI-only user end
// up with identical layouts and can share one install.
const ACTIVE_SARTHI_ROOT = path.join(SARTHI_HOME, 'sarthi-agent')
// VENV_ROOT — venv lives inside the repo, exactly like install.ps1 does it.
const VENV_ROOT = path.join(ACTIVE_SARTHI_ROOT, 'venv')
...
// boots, going straight to `resolveSarthiBackend()`. Missing or stale marker
...
//
// We deliberately put the marker INSIDE ACTIVE_SARTHI_ROOT (not alongside)
...
const BOOTSTRAP_COMPLETE_MARKER = path.join(ACTIVE_SARTHI_ROOT, '.sarthi-bootstrap-complete')
...
// active-profile.json records which Sarthi profile the desktop launches its
// local backend as. When set, startSarthi() passes `sarthi --profile <name>
// dashboard …`, which deterministically pins SARTHI_HOME (see
// _apply_profile_override in sarthi_cli/main.py) and bypasses the sticky
// ~/.sarthi/active_profile file. Unset (null) preserves the legacy behavior:
// no --profile flag, so the backend honors active_profile / default.
const DESKTOP_PROFILE_CONFIG_PATH = path.join(app.getPath('userData'), 'active-profile.json')
// Mirrors sarthi_cli.profiles._PROFILE_ID_RE so we never hand the backend a
// value its profile resolver would reject and exit on.
const PROFILE_NAME_RE = /^[a-z0-9][a-z0-9_-]{0,63}$/
// Branch we track for self-update. The GUI work has merged to main, so this
// tracks main. User can also override at runtime via
// sarthiDesktop.updates.setBranch().
const DEFAULT_UPDATE_BRANCH = 'main'
// desktop.log lives under SARTHI_HOME/logs/ so it sits next to agent.log,
// errors.log, gateway.log produced by sarthi_logging.setup_logging — one log
// directory per user, regardless of which UI surface produced the line.
const DESKTOP_LOG_PATH = path.join(SARTHI_HOME, 'logs', 'desktop.log')
...
//
// Mirror the Python logs (sarthi_logging.py RotatingFileHandler, maxBytes x
...
const BOOT_FAKE_MODE = process.env.SARTHI_DESKTOP_BOOT_FAKE === '1'
const BOOT_FAKE_ERROR = process.env.SARTHI_DESKTOP_BOOT_FAKE_ERROR || ''
...
const SKIP_QUIT_CONFIRM = process.env.SARTHI_DESKTOP_SKIP_QUIT_CONFIRM === '1'

const BOOT_FAKE_STEP_MS = (() => {
  const raw = Number.parseInt(String(process.env.SARTHI_DESKTOP_BOOT_FAKE_STEP_MS || ''), 10)
...
const APP_NAME = process.env.SARTHI_DESKTOP_APP_NAME || 'Sarthi'
...
// 'sarthi:native-theme' ('dark' | 'light' | 'system'); we pin
...
// electron-builder derives from the build `appId` (com.nousresearch.sarthi) —
// keep this string in sync with package.json `build.appId`. macOS/Linux don't
// need this, so gate it on Windows. (Fixes: desktop approval/turn notifications
// never firing on Windows.)
if (IS_WINDOWS) {
  app.setAppUserModelId('com.nousresearch.sarthi')
}

// Seed the native About panel with the live Sarthi version. This is refreshed
// on every open via the explicit "About" menu handler (refreshAboutPanel), so
// an in-place `sarthi update` mid-session is reflected without an app restart;
// the seed here just covers the first open and any non-menu invocation path.
app.setAboutPanelOptions({
  applicationName: APP_NAME,
  applicationVersion: resolveSarthiVersion(),
...
// startSarthi(); this pool only holds EXTRA profile
...
const POOL_MAX_BACKENDS = Math.max(1, Number(process.env.SARTHI_DESKTOP_POOL_MAX) || 3)
const POOL_IDLE_MS = Math.max(60_000, Number(process.env.SARTHI_DESKTOP_POOL_IDLE_MS) || 10 * 60_000)
...
// onto the error so subsequent startSarthi() calls (e.g. the renderer's
// ensureGatewayOpen retrying after the WS won't open) return the same error
// instead of re-running install.ps1 in a hot loop. Cleared explicitly by
// the renderer's "Reload and retry" path or by quitting the app.
let bootstrapFailure = null
// Latched non-bootstrap backend spawn failure — stops getConnection() from
// respawning sarthi serve backend children in a tight loop while boot is broken.
...
let bootProgressState = {
  ...
  message: 'Waiting to start Sarthi backend',
  phase: 'idle',
  progress: 0,
  retryable: false,
  running: false,
...
function rememberLog(chunk) {
  ...
  sarthiLog.push(...lines)

  if (sarthiLog.length > 300) {
    sarthiLog.splice(0, sarthiLog.length - 300)
  }
...
function ensureWslWindowsFonts() {
  ...
  try {
    ...
    const confPath = path.join(confDir, '99-sarthi-wsl-windows-fonts.conf')
    let existing = ''
...
function broadcastBootProgress() {
  ...
  webContents.send('sarthi:boot-progress', bootProgressState)
}
...
//
// The snapshot is queryable via the sarthi:bootstrap:get IPC handler so a
...
function broadcastBootstrapEvent(ev) {
  ...
  webContents.send('sarthi:bootstrap:event', ev)
}
...
function promptFirstRunSetupChoice(backend) {
  broadcastBootstrapEvent({
    ...
    activeRoot: backend.activeRoot || ACTIVE_SARTHI_ROOT
  })
}
...
// The Tauri updater writes SARTHI_HOME/.sarthi-update-in-progress for the whole
...
// (`force_kill_other_sarthi`, taskkill /IM sarthi.exe) kills it, the launch
...
function updateGateDeps() {
  return {
    hasLiveMarker: () => Boolean(readLiveUpdateMarker(SARTHI_HOME)),
...
async function waitForUpdateToFinish() {
  ...
  const outcome = await waitForUpdateClearance(updateGateDeps(), {
    onWaitTick: async reason => {
      ...
      await advanceBootProgress(
        'backend.update-wait',
        'An update is finishing — Sarthi will start automatically when it completes…',
        12
      )
    },
  ...
  // "nothing happened").
  try {
    const result = readAndConsumeHandoffResult(SARTHI_HOME)
    ...
    if (result && result.ok && result.manual) {
      ...
      dialog.showErrorBox(
        'Sarthi update did not finish',
        `${result.message}\n\nDetails: ${path.join(SARTHI_HOME, 'logs', 'desktop-update-handoff.log')}`
      )
    }
  } catch (err) {
...
function findOnPath(command) {
  ...
  // shell-script shim named `sarthi` — must not shadow `sarthi.cmd`/`sarthi.exe`.
...
function unwrapWindowsVenvSarthiCommand(command, backendArgs) {
  return resolveVenvSarthiCommand(command, backendArgs, {
...
// spawns `sarthi serve`; runtimes older than serve only have `dashboard`. We
...
// (covers a bare `sarthi` resolved from PATH with no known source root). Result
...
function backendSupportsServe(backend) {
  ...
  if (backend.root) {
    try {
      const src = fs.readFileSync(path.join(backend.root, 'sarthi_cli', 'subcommands', 'dashboard.py'), 'utf8')
  ...
  if (supported === null) {
    try {
      ...
      // `sarthi --version` (~10.5s measured cold), and a false negative here
      ...
      execProbeSync(backend.command, [...prefix, 'serve', '--help'], {
        ...
        env: { ...process.env, SARTHI_HOME, ...(backend.env || {}) },
        timeout: PROBE_TIMEOUT_MS,
        stdio: 'ignore',
        // `.cmd`/`.bat` shim backends carry shell: true in their descriptor
        // (see resolveSarthiBackend step 4); execFileSync of a .cmd without
...
function isSarthiSourceRoot(root) {
  return directoryExists(root) && fileExists(path.join(root, 'sarthi_cli', 'main.py'))
}

function findPythonForRoot(root) {
  const override = process.env.SARTHI_DESKTOP_PYTHON
...
function findSystemPython() {
  ...
  // CPython but several Sarthi deps (notably pywinpty's Rust-built
      ...
      try {
        const out = execFileSync(
          ...
          hiddenWindowsChildOptions({
            ...
            // Bare interpreter startup — much lighter than the sarthi-import
...
// findGitBash — locate bash.exe on Windows. Resolves SARTHI_GIT_BASH_PATH
...
// normal SARTHI_DASHBOARD_READY stdout line and no ready-file side channel is
// needed.
...
// install only has PortableGit under %LOCALAPPDATA%\sarthi\git (never on
...
function resolveGitBinary() {
  ...
  if (localAppData) {
    candidates.push(path.join(localAppData, 'sarthi', 'git', 'cmd', 'git.exe'))
    candidates.push(path.join(localAppData, 'sarthi', 'git', 'bin', 'git.exe'))
  }
...
function recentSarthiLog() {
  return sarthiLog.slice(-20).join('\n')
}

// ─── Self-update (git-pull against the running backend's sarthi root) ──────
...
// Dev → SOURCE_REPO_ROOT. Packaged/CLI install → ACTIVE_SARTHI_ROOT.
// SARTHI_DESKTOP_SARTHI_ROOT always wins so devs can pin a worktree.
function resolveUpdateRoot() {
  const candidates = [
    process.env.SARTHI_DESKTOP_SARTHI_ROOT && path.resolve(process.env.SARTHI_DESKTOP_SARTHI_ROOT),
    !IS_PACKAGED && isSarthiSourceRoot(SOURCE_REPO_ROOT) ? SOURCE_REPO_ROOT : null,
    isSarthiSourceRoot(ACTIVE_SARTHI_ROOT) ? ACTIVE_SARTHI_ROOT : null
  ].filter(Boolean)

  return candidates.find(c => directoryExists(path.join(c, '.git'))) || candidates[0] || ACTIVE_SARTHI_ROOT
}
...
function emitUpdateProgress(payload) {
  ...
  for (const window of BrowserWindow.getAllWindows()) {
    window.webContents.send('sarthi:updates:progress', merged)
  }
}
      ...
      const req = https.get(
        url,
        {
          headers: {
            ...
            'User-Agent': 'sarthi-desktop-update-check'
          },
          timeout: 10_000
        },
        res => {
...
// Windows that binary owns ALL repo mutation — running `sarthi update` +
...
function resolveUpdaterBinary() {
  return resolveStagedUpdaterBinary(SARTHI_HOME, { fileExists, isWindows: IS_WINDOWS })
}
...
// Path to the venv shim whose lock decides whether `sarthi update` can write
// fresh entry points. On Windows this is the file the running backend
// `sarthi.exe` holds open; on POSIX it's never mandatory-locked.
function venvSarthiShimPath(updateRoot) {
  return IS_WINDOWS
    ? path.join(updateRoot, 'venv', 'Scripts', 'sarthi.exe')
    : path.join(updateRoot, 'venv', 'bin', 'sarthi')
}
...
// only signals the direct child, so on Windows a backend `sarthi.exe` that
// spawned its own grandchildren (a `sarthi` REPL, a pty terminal session, the
...
async function claimBackendChild(child, command, profile, nonce) {
  try {
    ...
    child.sarthiBackendIdentity = identity

    return identity
  } catch (error) {
    stopBackendChild(child)
    await waitForBackendExit(child)
    throw new Error(`Could not persist ownership for the Sarthi backend: ${error.message}`)
  }
}

function releaseBackendChild(child) {
  const identity = child?.sarthiBackendIdentity
...
// `sarthiProcess.kill('SIGTERM')` + `app.quit()` fire-and-forget: SIGTERM on
// Windows doesn't reap the backend's grandchildren, and quit didn't wait for
// teardown, so the updater raced a still-locked `sarthi.exe`, the quarantine
...
// problem: the desktop's backend (and the grandchildren IT spawned — a sarthi
// REPL, a pty terminal, the gateway) keep `sarthi.exe` and other files in the
...
async function releaseBackendLock(updateRoot, tag) {
  ...
  const sarthiProcess = backendConnectionState.getProcess()

  stopBackendTreesForUpdate(sarthiProcess, {
    forceKillProcessTree,
    stopAllPoolBackends
  })

  const shim = venvSarthiShimPath(updateRoot)
  ...
  while (Date.now() < deadlineMs) {
    ...
    const currentSarthiProcess = backendConnectionState.getProcess()

    if (currentSarthiProcess && Number.isInteger(currentSarthiProcess.pid)) {
      stragglers.push(currentSarthiProcess.pid)
    }
...
// `sarthi update`). Instead we spawn the staged Sarthi-Setup binary with
// --update and quit, so it can run `sarthi update` (which refuses while we
...
async function applyUpdates(opts: { stopSafeBlockers?: boolean } = {}) {
  ...
  try {
    ...
    if (!updater && !IS_WINDOWS) {
      ...
      // Windows (quit → detached orchestrator → `sarthi update` → relaunch),
      ...
      // in: the SARTHI_DESKTOP_CHILD_PID reaper-exclusion dance (#37532),
      ...
      // get the manual `sarthi update` card once; their next update pulls it.
      return await applyUpdatesPosixHandoff(opts)
    }

    if (!updater) {
      // No staged updater binary — this is a CLI-installed user (they ran
      // `sarthi desktop`, never the Tauri installer that self-copies
      // sarthi-setup.exe into SARTHI_HOME). On Windows the repo hand-off
      ...
      if (!resolveUpdateScriptHandoff(updateRoot)) {
        // They DO have a working `sarthi` on PATH / in the venv, so the
        ...
        try {
          ...
          if (head.code === 0 && current && current !== 'HEAD') {
            ...
            if (branch !== 'main') {
              command = `sarthi update --branch ${branch}`
            }
          }
        } catch {
          // Best-effort: fall back to bare `sarthi update` if branch detection fails.
        }

        rememberLog(`[updates] no staged updater; surfacing manual \`${command}\` for CLI install at ${updateRoot}`)
        emitUpdateProgress({ stage: 'manual', message: command, percent: null })

        return { ok: true, manual: true, command, sarthiRoot: updateRoot }
      }

      rememberLog('[updates] no staged updater; using repo hand-off script for CLI install')
    }

    const handoffConflict = updateHandoffConflict(SARTHI_HOME)
    ...
    emitUpdateProgress({
      stage: 'restart',
      message:
        'Updating Sarthi — this window will close and the updater will open. Don’t reopen Sarthi yourself; it restarts automatically when the update finishes.',
      percent: 100
    })
    ...
    preflightStateDb(SARTHI_HOME, rememberLog)

    // Stop our own backend(s) and wait for the venv shim to unlock BEFORE we
    // spawn the updater. Without this the updater races a still-locked
    // sarthi.exe (held by the backend child / its grandchildren) and the update
    ...
    if (!lock.unlocked) {
      ...
      // terminal running sarthi, an unkillable child). Handing off anyway
      ...
      const message =
        'Update aborted: another process is holding the Sarthi install open ' +
        '(a second Sarthi window or a terminal running sarthi?). Close it and retry.'
    ...
    // Sarthi processes running from this venv.  The updater normally refuses
    ...
    // `sarthi update` will run (the venv shim is locked while we live).
    //
    ...
    // checkout, so each `sarthi update` refreshes the code that drives the
    ...
    if (scriptHandoff) {
      ...
      child = spawnUpdaterProcess(wrapped.command, wrapped.args, {
        ...
        env: {
          ...process.env,
          SARTHI_HOME,
          SARTHI_UPDATE_STARTED_AT: String(updateStartedAt),
          PATH: pathWithSarthiManagedNode(venvBin)
        },
        detached: true,
        stdio: 'ignore'
      })
      ...
      // The `sarthi update` child adopts the SCRIPT's claim via
      // update_lock.py's process-ancestry rule; no mtime heuristics needed.
      if (Number.isInteger(child.pid)) {
        writeUpdateMarker(SARTHI_HOME, child.pid, { startedAt: updateStartedAt })
      }
      ...
      // abort with "Another Sarthi update is already running (PID <itself>)" —
      ...
      if (Number.isInteger(child.pid) && stagedUpdaterSupportsPrewrittenMarker(updater)) {
        writeUpdateMarker(SARTHI_HOME, child.pid)
    ...
    if (!handoffOutcome.ok) {
      const message = `Update failed to start: ${handoffOutcome.message}. Sarthi will keep running — try again, or run \`sarthi update\` from a terminal.`
...
async function handOffWindowsBootstrapRecovery(reason) {
  ...
  const venvBin = path.join(updateRoot, 'venv', IS_WINDOWS ? 'Scripts' : 'bin')
  const venvSarthi = path.join(venvBin, IS_WINDOWS ? 'sarthi.exe' : 'sarthi')
  const venvPython = path.join(venvBin, IS_WINDOWS ? 'python.exe' : 'python')

  // The updater invokes the venv's Sarthi launcher, which in turn requires the
  ...
  // marker-only install through --update dead-ends at "Could not find the sarthi
  ...
  const updaterArgs = chooseUpdaterArgs(
    {
      hasBootstrapMarker: fileExists(path.join(updateRoot, '.sarthi-bootstrap-complete')),
      hasVenvSarthi: fileExists(venvSarthi),
...
// Python-level pre-update snapshot inside `sarthi update`.
function preflightStateDb(sarthiHome, rememberLog) {
  const stateDbPath = path.join(sarthiHome, 'state.db')
  ...
  try {
    ...
    if (stat.size > 100) {
      ...
      const emergencyPath = path.join(sarthiHome, `state.db.pre-update-emergency-${ts}.bak`)
      ...
      try {
        ...
        try {
          const homeDir = fs.readdirSync(sarthiHome)
          ...
          for (const old of backups.slice(2)) {
            try {
              fs.unlinkSync(path.join(sarthiHome, old))
            } catch {
              void 0
            }
          }
...
// out, runs `sarthi update`, swaps/relaunches the app bundle, and writes
// .sarthi-update-result.json for the relaunched Desktop to surface. It shows
...
async function applyUpdatesPosixHandoff(opts: any) {
  ...
  if (!handoff) {
    emitUpdateProgress({ stage: 'manual', message: 'sarthi update', percent: null })

    return { ok: true, manual: true, command: 'sarthi update', sarthiRoot: updateRoot }
  }
  ...
  const child = spawnUpdaterProcess(handoff.command, args, {
    ...
    env: {
      ...
      SARTHI_HOME,
      SARTHI_UPDATE_STARTED_AT: String(updateStartedAt),
      PATH: pathWithSarthiManagedNode(path.join(updateRoot, 'venv', 'bin'))
    },
    detached: true,
    stdio: 'ignore'
  })
  ...
  emitUpdateProgress({
    stage: 'restart',
    message:
      'Updating Sarthi — this window will close. Don’t reopen Sarthi yourself; it restarts automatically when the update finishes.',
    percent: 100
  })
...
// Marker-independent: is the canonical install at ACTIVE_SARTHI_ROOT actually
...
function isActiveRuntimeUsable() {
  ...
  return (
    isSarthiSourceRoot(ACTIVE_SARTHI_ROOT) &&
    fileExists(venvPython) &&
    canImportSarthiCli(venvPython, {
      env: {
        PYTHONPATH: [ACTIVE_SARTHI_ROOT, process.env.PYTHONPATH].filter(Boolean).join(path.delimiter)
      }
    })
  )
}
...
function activeRuntimeState() {
  ...
  // pinned commit -- users update via the in-app update path or `sarthi
...
function resolveWebDist() {
  const override = process.env.SARTHI_DESKTOP_WEB_DIST
  ...
  if (IS_PACKAGED && /app\.asar(?=$|[\\/])/.test(fallback) && !directoryExists(fallback)) {
    rememberLog(
      `[web-dist] dashboard frontend dir resolved to an asar-internal path that ` +
        ...
        `Ensure dist/** is unpacked (asarUnpack) or set SARTHI_DESKTOP_WEB_DIST.`
    )
  }
...
function resolveRendererIndex() {
  ...
  if (present.length > 0) {
    ...
    rememberLog(
      `[renderer] every renderer bundle is incomplete (${present.join(', ')}). ` +
        ...
        `Repair with: sarthi desktop --force-build`
    )
  ...
  rememberLog(
    `[renderer] index.html not found — the desktop app was packaged without a ` +
      ...
      `Rebuild with: sarthi desktop --force-build`
  )
...
function resolveSarthiCwd() {
  // In a packaged build, `process.cwd()` resolves to the install root (e.g.
  // `…/win-unpacked` on Windows or `/Applications/Sarthi.app/Contents/...`
  ...
  const candidates = [
    ...
    process.env.SARTHI_DESKTOP_CWD,
...
function sanitizeWorkspaceCwd(cwd) {
  ...
  if (!trimmed || isPackagedInstallPath(trimmed)) {
    return { cwd: resolveSarthiCwd(), sanitized: Boolean(trimmed) }
  }
...
function createPythonBackend(root, label, backendArgs, options: any = {}) {
  ...
  return {
    ...
    label,
    command,
    args: ['-m', 'sarthi_cli.main', ...backendArgs],
...
// createActiveBackend — build a backend pointing at ACTIVE_SARTHI_ROOT, the
...
function createActiveBackend(backendArgs) {
  ...
  return {
    kind: 'python',
    label: `Sarthi at ${ACTIVE_SARTHI_ROOT}`,
    command,
    args: ['-m', 'sarthi_cli.main', ...backendArgs],
    env: buildDesktopBackendEnv({
      sarthiHome: SARTHI_HOME,
      pythonPathEntries: [ACTIVE_SARTHI_ROOT, ...getVenvSitePackagesEntries(VENV_ROOT)],
      venvRoot: VENV_ROOT
    }),
    root: ACTIVE_SARTHI_ROOT,
    bootstrap: true,
    shell: false
  }
}

function resolveSarthiBackend(backendArgs) {
  // 1. Explicit override -- SARTHI_DESKTOP_SARTHI_ROOT points at a developer
  //    checkout. Honour it as-is (no bootstrap; the user is driving).
  const overrideRoot = process.env.SARTHI_DESKTOP_SARTHI_ROOT && path.resolve(process.env.SARTHI_DESKTOP_SARTHI_ROOT)

  if (overrideRoot && isSarthiSourceRoot(overrideRoot)) {
    const backend = createPythonBackend(overrideRoot, `Sarthi source at ${overrideRoot}`, backendArgs)
  ...
  //    installed `sarthi` on PATH so local Python edits are actually exercised.
  //    (In dev with no checkout, SOURCE_REPO_ROOT won't pass isSarthiSourceRoot.)
  if (!IS_PACKAGED && isSarthiSourceRoot(SOURCE_REPO_ROOT)) {
    const backend = createPythonBackend(SOURCE_REPO_ROOT, `Sarthi source at ${SOURCE_REPO_ROOT}`, backendArgs)
  ...
  // 3. ACTIVE_SARTHI_ROOT — the canonical install at
  //    %LOCALAPPDATA%\\sarthi\\sarthi-agent (Windows) or ~/.sarthi/sarthi-agent.
  ...
  if (activeRuntime.shouldUseActiveRuntime && !bootstrapRepairRequested) {
    if (!activeRuntime.hasValidMarker) {
      rememberLog(
        `[bootstrap] Active Sarthi runtime at ${ACTIVE_SARTHI_ROOT} is usable but the bootstrap marker is missing or stale; skipping first-run bootstrap.`
      )
    }
  ...
  // 4. Existing `sarthi` on PATH -- installed via install.ps1 / install.sh from
  //    a previous tool-only setup, or pip-installed system-wide. Use it but
  //    do NOT write a bootstrap marker; the user did this themselves and we
  //    don't want to take ownership of an install we didn't perform.
  //    SARTHI_DESKTOP_IGNORE_EXISTING=1 forces the bootstrap path for testing.
  if (process.env.SARTHI_DESKTOP_IGNORE_EXISTING !== '1') {
    let sarthiCommand = null
    const sarthiOverride = process.env.SARTHI_DESKTOP_SARTHI

    if (sarthiOverride) {
      const resolvedOverride = findOnPath(sarthiOverride)

      if (resolvedOverride) {
        sarthiCommand = resolvedOverride
      } else if (!isWindowsBinaryPathInWsl(sarthiOverride, { isWsl: IS_WSL })) {
        sarthiCommand = sarthiOverride
      } else {
        rememberLog(`Ignoring Windows Sarthi override under WSL: ${sarthiOverride}`)
      }
    } else {
      sarthiCommand = findOnPath('sarthi')
    }

    if (sarthiCommand) {
      if (looksLikeDesktopAppBinary(sarthiCommand)) {
        rememberLog(`Ignoring desktop app executable on PATH while resolving Sarthi CLI: ${sarthiCommand}`)
        sarthiCommand = null
      }
    }

    if (sarthiCommand) {
      const unwrapped = unwrapWindowsVenvSarthiCommand(sarthiCommand, backendArgs)

      if (unwrapped) {
        return unwrapped
      }

      // Smoke-test the candidate before trusting it. A `sarthi` shim
      ...
      const shellForProbe = isCommandScript(sarthiCommand)

      // SARTHI_DESKTOP_SARTHI is an explicit deployment override (used by
      ...
      if (shouldTrustSarthiOverride(sarthiOverride) || verifySarthiCli(sarthiCommand, { shell: shellForProbe })) {
        ...
        // unwrapWindowsVenvSarthiCommand here: the second call repeats the
        // same un-memoized import probe, costing up to another full probe
        // timeout on the boot path for an answer we already have.
        return {
          label: `existing Sarthi CLI at ${sarthiCommand}`,
          command: sarthiCommand,
          args: backendArgs,
          bootstrap: false,
          env: {},
          kind: 'command',
      ...
      rememberLog(
        `Ignoring existing Sarthi CLI at ${sarthiCommand}: --version probe failed; falling through to bootstrap.`
      )
    }
  }

  // 5. Last-ditch: pip-installed sarthi_cli module via system Python.
  ...
  if (python) {
    ...
    // having sarthi_cli installed -- common on dev boxes that have
    ...
    // a uv-managed 3.11 into %LOCALAPPDATA%\sarthi\sarthi-agent\venv.
    if (canImportSarthiCli(python)) {
      return {
        kind: 'python',
        label: `installed sarthi_cli module via ${python}`,
        command: python,
        args: ['-m', 'sarthi_cli.main', ...backendArgs],
        bootstrap: false,
        env: {},
    ...
    rememberLog(`Ignoring system Python ${python}: sarthi_cli is not importable; falling through to bootstrap.`)
  }
  ...
  //    resolveSarthiBackend was the old "no payload" path and forced the
  //    user into a dead end. With the bootstrap protocol, "no install yet"
  //    is a recoverable state the GUI can drive through.
  return {
    kind: 'bootstrap-needed',
    label: 'Sarthi Agent not installed yet; bootstrap required',
    command: null,
    args: backendArgs,
    bootstrap: true,
    env: {},
...
async function ensureRuntime(backend) {
  ...
  // backend.kind === 'bootstrap-needed' means resolveSarthiBackend couldn't
  ...
  if (backend.kind === 'bootstrap-needed') {
    rememberLog('[bootstrap] no Sarthi install found; starting first-launch bootstrap')

    if (await handOffWindowsBootstrapRecovery('bootstrap-needed')) {
      const handoffError: Error & { isBootstrapFailure?: boolean; bootstrapHandedOff?: boolean } = new Error(
        'Sarthi recovery was handed off to Sarthi Setup. The desktop will restart when recovery completes.'
      )
    ...
    const bootstrapResult = await runBootstrap({
      ...
      sarthiHome: SARTHI_HOME,
      logRoot: path.join(SARTHI_HOME, 'logs'),
    ...
    if (bootstrapResult.cancelled) {
      const cancelledError = new Error('Sarthi install was cancelled.') as any
    ...
    if (!bootstrapResult.ok) {
      const bootstrapError = new Error(
        `Sarthi bootstrap failed${bootstrapResult.failedStage ? ` at stage '${bootstrapResult.failedStage}'` : ''}: ` +
          `${bootstrapResult.error || 'unknown error'}. ` +
          `Check ${path.join(SARTHI_HOME, 'logs', 'desktop.log')} for the full transcript.`
      ) as any

      bootstrapError.isBootstrapFailure = true
      bootstrapError.failedStage = bootstrapResult.failedStage || null
      // Latch the failure so subsequent startSarthi() calls return this
      // same error without re-running install.ps1.  Cleared by the
      // sarthi:bootstrap:reset IPC (renderer's "Reload and retry").
    ...
    return ensureRuntime(resolveSarthiBackend(backend.args))
  }
  ...
  if (!isSarthiSourceRoot(ACTIVE_SARTHI_ROOT)) {
    throw new Error(
      `Sarthi install at ${ACTIVE_SARTHI_ROOT} is missing or incomplete. ` +
        'Reinstall via the desktop installer or scripts/install.ps1.'
    )
  }

  // On Windows, preflight Git Bash. Sarthi' terminal tool calls bash.exe
  ...
  // %LOCALAPPDATA%\sarthi\git\, which findGitBash() picks up, so for any
  // user who completed the bootstrap this is a no-op. For users who got
  // here via an external `sarthi` on PATH, this check still helps.
  if (IS_WINDOWS && !findGitBash()) {
    throw new Error(
      'Git for Windows is required for Sarthi on Windows (provides Git Bash, ' +
        "which the agent's terminal tool uses). Install it from " +
        'https://git-scm.com/download/win or run `winget install -e --id Git.Git`, ' +
        'then relaunch Sarthi.'
    )
  }
  ...
  if (!fileExists(venvPython)) {
    ...
    // normal flow because activeRuntimeState() requires isSarthiSourceRoot()
    // plus an importable sarthi_cli before it hands back the active runtime.
    // If we hit this, the user (or a deleted venv) broke the invariant; tell
    // them to re-run the install.
    throw new Error(
      `Sarthi venv missing at ${VENV_ROOT}. Re-run the desktop installer or ` + '`scripts/install.ps1` to rebuild it.'
    )
  }

  backend.command = getVenvPython(VENV_ROOT)
  backend.label = `Sarthi at ${ACTIVE_SARTHI_ROOT} (venv: ${VENV_ROOT})`
  updateBootProgress({
    phase: 'runtime.ready',
    message: 'Sarthi runtime is ready',
    progress: 82,
    running: true,
    error: null
  })
...
function multipartBody(upload) {
  const boundary = `----sarthi-${crypto.randomBytes(12).toString('hex')}`
...
function fetchJson(url, token, options: any = {}) {
  return new Promise((resolve, reject) => {
    ...
    if (parsed.protocol !== 'http:' && parsed.protocol !== 'https:') {
      reject(new Error(`Unsupported Sarthi backend URL protocol: ${parsed.protocol}`))
            ...
            reject(
              new Error(
                `Expected JSON from ${url} but got HTML (status ${res.statusCode}). ` +
                  'The endpoint is likely missing on the Sarthi backend.'
              )
            )
    ...
    req.setTimeout(timeoutMs, () => {
      req.destroy(new Error(`Timed out connecting to Sarthi backend after ${timeoutMs}ms`))
    })
...
function downloadViaTokenToFile(url, token, ctx, options: any = {}) {
  return new Promise((resolve, reject) => {
    ...
    const req = client.request(
      ...
      {
        method: 'GET',
        headers: options.bearer ? { Authorization: `Bearer ${options.bearer}` } : { 'X-Sarthi-Session-Token': token }
      },
      res => {
...
function fetchPublicJson(url, options: any = {}) {
  ...
  // NO ``X-Sarthi-Session-Token`` header — used by the auth-mode probe before
...
function getLinkTitleSession() {
  ...
  linkTitleSession = session.fromPartition('sarthi:link-titles', { cache: false })
...
async function previewFileTarget(rawTarget, baseDir) {
  ...
  const base = baseDir ? path.resolve(expandUserPath(baseDir)) : resolveSarthiCwd()
...
function sendPreviewFileChanged(payload) {
  ...
  webContents.send('sarthi:preview-file-changed', payload)
}
...
// waitForSarthiReady how to read a 401 — rejected session vs gated route.
...
async function waitForSarthi(baseUrl, token, signal?, authMode?, headers = {}) {
  const { probeHealth, probeIsCredentialed } = await buildReadinessHealthProbe(baseUrl, authMode, token)

  return waitForSarthiReady(baseUrl, {
    token,
    signal,
    fetchPublicJson,
...
function sendBackendExit(payload) {
  ...
  webContents.send('sarthi:backend-exit', payload)
}
...
function sendClosePreviewRequested() {
  ...
  webContents.send('sarthi:close-preview-requested')
}
...
function sendPreviewNavCommand(command: 'back' | 'forward' | 'reload') {
  ...
  webContents.send('sarthi:preview-nav', command)
}
...
function sendOpenFolderRequested() {
  ...
  webContents.send('sarthi:open-folder-requested')
}

// Tell the renderer the machine just woke. Sleep silently drops the
// renderer's WebSocket to the local backend; the renderer reconnects on this
// signal so the chat composer doesn't stay stuck on "Starting Sarthi...".
function sendPowerResume() {
  ...
  webContents.send('sarthi:power-resume')
}
...
// 'sarthi:power-battery' push below.
ipcMain.handle('sarthi:power-battery:get', () => onBatteryPower === true)
...
function broadcastBatteryState(next: boolean) {
  ...
  for (const win of BrowserWindow.getAllWindows()) {
    ...
    if (webContents && !webContents.isDestroyed()) {
      webContents.send('sarthi:power-battery', next)
    }
  }
}
...
function sendOpenUpdatesRequested() {
  ...
  webContents.send('sarthi:open-updates')
...
function sendWindowStateChanged(nextIsFullscreen?: boolean, target = mainWindow) {
  ...
  webContents.send('sarthi:window-state-changed', state)
}
...
function installContextMenuBridge(window: BrowserWindow) {
  window.webContents.on('context-menu', (_event, params) => {
    ...
    if (params.isEditable && params.misspelledWord) {
      window.webContents.send('sarthi:context-menu-spellcheck', {
...
//
// Hosted Sarthi gateways gate the dashboard behind an OAuth provider (e.g.
// Nous Research) instead of a static session token. The auth model is
// fundamentally different from the token path:
//
//   * REST is authed by HttpOnly session cookies (``sarthi_session_at``),
...
//     alongside the ~15-min access token (Portal NAS #293 / sarthi #37247).
//     Both are set as HttpOnly cookies (``sarthi_session_at`` ~15 min,
//     ``sarthi_session_rt`` 24h). When the AT cookie lapses but the RT cookie
...
const OAUTH_SESSION_PARTITION = 'persist:sarthi-remote-oauth'
...
// "not signed in", which on the initial boot path (startSarthi → the renderer's
// single-shot boot() with no retry) surfaced as the "Sarthi couldn't start"
...
async function hasLiveOauthSession(baseUrl) {
  ...
  // for a signed-in user — the exact race that produced the transient "Sarthi
  ...
  return new Promise((resolve, reject) => {
    ...
    try {
      win = new BrowserWindow({
        width: 520,
        height: 720,
        title: silent ? 'Connecting to Sarthi Cloud agent…' : 'Sign in to Sarthi gateway',
...
function fetchJsonViaOauthSession(url, options: any = {}) {
  return new Promise((resolve, reject) => {
    ...
    const timer = setTimeout(() => {
      ...
      reject(new Error(`Timed out connecting to Sarthi backend after ${timeoutMs}ms`))
    }, timeoutMs)
...
// feature; the server half lives in sarthi_cli/dashboard_auth/native_flow.py.
// ---------------------------------------------------------------------------
...
// --- Sarthi Cloud discovery + silent per-agent sign-in (cloud-auto-discovery
// Phase 3) ---------------------------------------------------------------
//
...
// portal session cookie living in `persist:sarthi-remote-oauth`:
...
// convention (sarthi_cli/auth.py DEFAULT_NOUS_PORTAL_URL + the same env names)
// so a single override flips every Sarthi surface to the same portal.
const DEFAULT_NOUS_PORTAL_URL = 'https://portal.nousresearch.com'

function resolvePortalBaseUrl() {
  const raw = process.env.SARTHI_PORTAL_BASE_URL || process.env.NOUS_PORTAL_BASE_URL || DEFAULT_NOUS_PORTAL_URL
...
// authenticates via PRIVY, not the Sarthi gateway session cookies, so this
// checks for the `privy-token` cookie on the portal host (NOT
// hasLiveOauthSession, which looks for sarthi_session_at/rt that the portal
    ...
    return await new Promise<boolean>(resolve => {
      ...
      try {
        win = new BrowserWindow({
          ...
          height: 720,
          show: false,
          title: 'Renewing Sarthi Cloud session…',
          autoHideMenuBar: true,
          webPreferences: {
            contextIsolation: true,
            nodeIntegration: false,
...
function openPortalLoginWindow() {
  ...
  return new Promise((resolve, reject) => {
    if (!app.isReady()) {
      reject(new Error('Desktop is not ready to start a Sarthi Cloud sign-in.'))
...
// Discover the hosted (Sarthi Cloud) agents the signed-in user can see. Calls
...
async function discoverCloudAgents(org?: string) {
  ...
  if (!(await hasLivePortalSession())) {
    const err = new Error(
      'You are not signed in to Sarthi Cloud. Open Settings → Gateway, choose Sarthi Cloud, and sign in.'
    ) as any
  ...
  try {
    ...
    if (body === undefined) {
      ...
      if (error && error.statusCode === 401) {
        const err = new Error(
          'Your Sarthi Cloud session has expired. Open Settings → Gateway and sign in again.'
        ) as any
...
async function cloudAgentSilentSignIn(dashboardUrl) {
  ...
  if (!(await hasLivePortalSession())) {
    const err = new Error('Your Sarthi Cloud session has expired. Sign in to Sarthi Cloud again.') as any
    err.needsCloudLogin = true
    throw err
  }
...
function sanitizeConnectionProfiles(raw: Record<string, any>) {
  ...
  for (const [name, entry] of Object.entries(raw)) {
    ...
    // Preserve the Sarthi Cloud org tag on cloud-mode entries so Settings can
...
function readDesktopConnectionsRegistry() {
  ...
  if (mtime === null) {
    ...
    try {
      ...
      // hard-failing every sarthi:connections:* call.
 ...
 * editor doesn't carry — cloud `org`, ssh `remoteSarthiPath`/`remoteProfile` —
...
// a valid stored value (pins the root SARTHI_HOME explicitly); null means "no
...
async function sanitizeDesktopConnectionConfig(config = readDesktopConnectionConfig(), profile = null) {
  ...
  const envOverride = key ? false : Boolean(process.env.SARTHI_DESKTOP_REMOTE_URL)
  ...
  const remoteUrl = envOverride ? String(process.env.SARTHI_DESKTOP_REMOTE_URL || '') : String(block.url || '')
  ...
  return {
    ...
    remoteUrl,
    // The persisted Sarthi Cloud org (slug/id) for a cloud connection, or '' for
    ...
    sshRemoteSarthiPath: (ssh || savedSsh)?.remoteSarthiPath || '',
    sshRemoteProfile: (ssh || savedSsh)?.remoteProfile || '',
    // The env override only forces the global/primary connection; a per-profile
    // scope is never overridden by SARTHI_DESKTOP_REMOTE_URL.
    envOverride
  }
}
...
// `org` (optional) is the Sarthi Cloud org slug/id the instance was discovered
...
function coerceDesktopConnectionConfig(input: any = {}, existing = readDesktopConnectionConfig(), options: any = {}) {
  ...
  // describe a discovered Sarthi Cloud instance, NOT a user-owned remote gateway,
...
function buildSshBlock(input: any, existingBlock: any = {}) {
  ...
  const merged = normalizeSshConfig({
    ...
    remoteSarthiPath: input.sshRemoteSarthiPath ?? existingBlock.remoteSarthiPath,
...
async function buildRemoteConnection(
  ...
  if (authMode === 'oauth') {
    ...
    // refresh token (sarthi #37247), and the gateway middleware transparently
    ...
    if (
      ...
      const err = new Error(
        'Remote Sarthi gateway uses OAuth, but you are not signed in. ' +
    ...
    try {
      ...
      throw gatewayTicketFailure(
        ...
        'Could not reach the remote Sarthi gateway while refreshing its WebSocket ticket. Try reconnecting.'
      )
    }
  ...
  if (!token) {
    throw new Error(
      'Remote Sarthi gateway is selected, but no session token is saved. ' +
...
function activeSshTerminalTarget() {
  ...
  if (process.env.SARTHI_DESKTOP_REMOTE_URL) {
    return null
  }
...
async function bootstrapSshConnectionInner(profile, sshConfig, reuseToken, source, fingerprint, lease) {
  ...
  try {
    const platform = await detectRemotePlatform(ssh, sshConfig.remoteSarthiPath || '')
    ...
    result = await lifecycle({
      ...
      remoteSarthiPath: sshConfig.remoteSarthiPath || '',
      ownershipId: sshOwnershipKey(profile),
      ...
      pickLocalPort,
      waitForSarthi: (baseUrl, token) => waitForSarthi(baseUrl, token, lease.signal, 'token'),
  ...
  sshConnections.set(scope, {
    ...
    host: sshConfig.host,
    hostLabel,
    sarthiVersion: result.sarthiVersion || '',
  ...
  sshRememberLog(
    `[ssh] connection ${result.reused ? 'REUSED' : 'spawned'} dashboard: ` +
      `${result.sarthiVersion || 'sarthi (version unknown)'} at ${result.sarthiPath || '?'}`
  )
  ...
  return { ...connection, remoteSarthiVersion: result.sarthiVersion || '' }
}
...
//   2. env override (SARTHI_DESKTOP_REMOTE_URL/_TOKEN) — applies app-wide
...
async function resolveRemoteBackend(profile) {
  ...
  const route = resolveDesktopRemoteRoute({
    config,
    env: {
      token: process.env.SARTHI_DESKTOP_REMOTE_TOKEN,
      url: process.env.SARTHI_DESKTOP_REMOTE_URL
    },
    profile,
...
// env → global) so a startSarthi() failure can be classified as remote (never
...
async function probeRemoteAuthMode(rawUrl) {
  ...
  // sending any credentials. ``/api/status`` is public on every Sarthi
...
async function testDesktopConnectionConfig(input: any = {}) {
  if (input.mode === 'ssh') {
    const sshConfig = normalizeSshConfig({
      ...
      remoteSarthiPath: input.sshRemoteSarthiPath
    })
    ...
    try {
      ...
      for (;;) {
        try {
          await ssh.open()
          const platform: any = await detectRemotePlatform(ssh, sshConfig.remoteSarthiPath || '')
          let sarthiPath
          let sarthiVersion
          let supported
          ...
          if (platform.os === 'Windows') {
            ...
            sarthiPath = runtime.sarthiPath
            const inspection = await helper(ssh, runtime, 'inspect', [runtime.sarthiPath])
            sarthiVersion = inspection.version
            supported = inspection.supported
          } else {
            sarthiPath = await remoteLifecycle.locateSarthi(ssh, sshConfig.remoteSarthiPath || '')
            sarthiVersion = await remoteLifecycle.probeSarthiVersion(ssh, sarthiPath)
            supported = await remoteLifecycle.remoteSupportsSshOwnership(ssh, sarthiPath)
          }
          ...
          if (!supported) {
            return {
              ...
              error: 'Update Sarthi on the remote host before connecting with Desktop SSH.'
            }
          }
          ...
          return {
            ...
            remoteSarthiPath: sarthiPath,
            remoteSarthiVersion: sarthiVersion,
  ...
  if (wantRemote && block?.url) {
    ...
    const remote = (await resolveRemoteBackend(key)) || (await startSarthi())
  ...
  // connect to Sarthi gateway". Mirror the renderer's connect here so the test
...
function resetSarthiConnection({ soft = false } = {}) {
  backendStartFailure = null
  remoteReauthFailure = null
  remoteLiveness.clear()
  const sarthiProcess = backendConnectionState.invalidate()
  stopBackendChild(sarthiProcess)
...
// startSarthi() spawns fresh instead of racing the dying one. Shared by the
// connection-config and profile switch flows.
async function teardownPrimaryBackendAndWait({ soft = false } = {}) {
  // Capture the reference before resetSarthiConnection() invalidates it.
  const sarthiProcess = backendConnectionState.getProcess()
  const dying = sarthiProcess && !sarthiProcess.killed ? sarthiProcess : null
...
function sendConnectionApplied() {
  ...
  webContents.send('sarthi:connection:applied')
}
...
function broadcastConnectionsChanged(payload: { connectionId: string; reason: 'removed' | 'updated' }) {
  for (const win of BrowserWindow.getAllWindows()) {
    ...
    if (webContents && !webContents.isDestroyed()) {
      webContents.send('sarthi:connections:changed', payload)
    }
  }
}
...
async function ensureBackend(profile) {
  ...
  if (route.backend === 'primary') {
    const connection = await startSarthi()
  ...
  // spawning, so two children never share one profile's SARTHI_HOME.
  ...
  entry.connectionPromise = spawnPoolBackend(key, entry).catch(async error => {
    ...
    rememberLog(
      `Sarthi backend for profile "${key}" failed to start: ${error instanceof Error ? error.message : String(error)}`
    )
...
async function ensureRegistryBackend(connectionId, profile) {
  ...
  if (source.kind === 'local') {
    ...
    localEntry.connectionPromise = spawnPoolBackend(profileKey, localEntry, {
      ...
      rememberLog(
        `Sarthi backend for profile "${profileKey}" (forced-local) failed to start: ${error instanceof Error ? error.message : String(error)}`
      )
...
async function connectRegistryBackend(source, profile, key, poolEntry) {
  ...
  if (source.kind === 'ssh') {
    ...
    const sshConfig = normalizeSshConfig({
      ...
      remoteSarthiPath: source.remoteSarthiPath,
  ...
  await waitForSarthi(connection.baseUrl, connection.token, undefined, connection.authMode, connection.headers)
  poolEntry.remoteBaseUrl = connection.baseUrl
...
// local-spawn portion of startSarthi() but without the boot-progress UI,
...
async function spawnPoolBackend(profile, entry, opts: { forceLocal?: boolean; poolKey?: string } = {}) {
  ...
  if (remote) {
    await waitForSarthi(remote.baseUrl, remote.token, undefined, remote.authMode, remote.headers)
  ...
  // --profile wins over the inherited SARTHI_HOME env (see _apply_profile_override
  // step 3 in sarthi_cli/main.py), so the child re-homes to this profile.
  // --port 0: the OS assigns an ephemeral port; the child announces it on stdout.
  const backendArgs = ['--profile', profile, 'serve', '--host', '127.0.0.1', '--port', '0']
  const backend = await ensureRuntime(resolveSarthiBackend(backendArgs))
  // Route old runtimes (no `serve`) through the legacy `dashboard --no-open`.
  backend.args = getBackendArgsForRuntime(backend)
  const sarthiCwd = resolveSarthiCwd()
  ...
  assertLocalProfileCanStart(profile, profileDeletionGate, key =>
    directoryExists(path.join(SARTHI_HOME, 'profiles', key))
  )

  rememberLog(`Starting Sarthi backend for profile "${profile}" via ${backend.label}`)
  ...
  const child = spawn(
    ...
    hiddenWindowsChildOptions({
      ...
      env: {
        ...
        TERMINAL_CWD: sarthiCwd,
        SARTHI_DASHBOARD_SESSION_TOKEN: token,
        ...
        ...parentIdentityEnv,
        SARTHI_WEB_DIST: webDist,
        ...(readyFile ? { SARTHI_DESKTOP_READY_FILE: readyFile } : {})
      },
      shell: backend.shell,
      stdio: ['ignore', 'pipe', 'pipe']
    })
  ...
  child.once('error', error => {
    rememberLog(`Sarthi backend for profile "${profile}" failed to start: ${error.message}`)
    releaseBackendChild(child)
    backendPool.delete(poolKey)
    rejectStart?.(error)
  })
  child.once('exit', (code, signal) => {
    rememberLog(`Sarthi backend for profile "${profile}" exited (${signal || code})`)
    releaseBackendChild(child)
    backendPool.delete(poolKey)

    if (!ready) {
      rejectStart?.(
        new Error(`Sarthi backend for profile "${profile}" exited before it became ready (${signal || code}).`)
      )
    }
  })
  ...
  await Promise.race([waitForSarthi(baseUrl, token), startFailed])
  ready = true

  const authToken = await adoptServedDashboardToken(baseUrl, token, {
    childAlive: () => child.exitCode === null && !child.killed,
    label: `Sarthi backend for profile "${profile}"`,
    rememberLog
  })
  ...
  if (!wsProbe.ok) {
    throw new Error(
      `Sarthi backend for profile "${profile}" is HTTP-reachable but the WebSocket (/api/ws) rejected the session token: ${wsProbe.reason}`
    )
  }
...
// backend whose ensure_sarthi_home() recreates the deleted profile directory.
//
...
async function startSarthi() {
  ...
  if (!isPrimaryInstance) {
    ...
    throw new Error('Sarthi Desktop is already running in another window.')
  }

  await reapOrphanedBackendsOnce()

  // Latched-failure short-circuit: once bootstrap has failed in this
  // process, every subsequent startSarthi() call re-throws the same error
  ...
  if (BOOT_FAKE_ERROR) {
    await advanceBootProgress('backend.resolve', 'Resolving Sarthi backend', 8)
  ...
  const connectionPromise = (async () => {
    const connectRemote = async remote => {
      ...
      if (!backendConnectionState.isCurrentAttempt(connectionAttempt)) {
        throw new Error('Sarthi backend start was superseded by a newer connection attempt.')
      }

      await advanceBootProgress('backend.remote', `Connecting to remote Sarthi backend at ${remote.baseUrl}`, 24)
      await waitForSarthi(remote.baseUrl, remote.token, undefined, remote.authMode, remote.headers)
      ...
      updateBootProgress({
        phase: 'backend.ready',
        message: 'Remote Sarthi backend is ready',
        progress: 94,
        running: true,
        error: null
      })

      return createPrimaryRemoteConnection(remote, sarthiLog.slice(-80), getWindowState())
    }
    ...
    // deterministic (it wins over the sticky ~/.sarthi/active_profile file) and
    // resolves SARTHI_HOME the same way `sarthi -p <name>` does on the CLI. An
    ...
    const setup = await runPrimaryBackendStartup({
      ...
      prepareLocalBackend: async () => {
        await advanceBootProgress('backend.runtime', 'Resolving Sarthi runtime', 28)

        return resolveSarthiBackend(backendArgs)
      },
      resolveRemote: () => {
    ...
    await advanceBootProgress('backend.spawn', `Starting Sarthi backend via ${backend.label}`, 84)
    rememberLog(`Starting Sarthi backend via ${backend.label}`)
    ...
    const sarthiProcess = spawn(
      ...
      hiddenWindowsChildOptions({
        ...
        env: {
          ...process.env,
          // Explicitly pin SARTHI_HOME for the child so Python's get_sarthi_home()
          // resolves to the SAME location our resolveSarthiHome() picked. Without
          // this pin, Python falls back to ~/.sarthi on every platform — fine on
          // mac/linux (where our default matches), but on Windows our default is
          // %LOCALAPPDATA%\sarthi, which differs from C:\Users\<u>\.sarthi.
          // Mismatch would split config / sessions / .env / logs across two
          // directories. install.ps1 sets SARTHI_HOME via setx; the desktop
    ...
    await claimBackendChild(sarthiProcess, `${backend.command} ${backend.args.join(' ')}`, profile, backendNonce)
    const processOwner = backendConnectionState.attachProcess(connectionAttempt, sarthiProcess)

    if (!processOwner) {
      stopBackendChild(sarthiProcess)
      await waitForBackendExit(sarthiProcess)
      releaseBackendChild(sarthiProcess)
      throw new Error('Sarthi backend start was superseded by a newer connection attempt.')
    }

    sarthiProcess.stdout.on('data', rememberLog)
    sarthiProcess.stderr.on('data', rememberLog)
    ...
    sarthiProcess.once('error', error => {
      releaseBackendChild(sarthiProcess)

      if (!backendConnectionState.clearForCurrentProcess(processOwner)) {
        rememberLog(`Ignoring stale Sarthi backend error: ${error.message}`)
        rejectBackendStart?.(new Error('Sarthi backend start was superseded by a newer connection attempt.'))

        return
      }

      rememberLog(`Sarthi backend failed to start: ${error.message}`)
      updateBootProgress(
        {
          error: error.message,
          message: `Sarthi backend failed to start: ${error.message}`,
          phase: 'backend.error',
          running: false
        },
        { allowDecrease: true }
    ...
    sarthiProcess.once('exit', (code, signal) => {
      releaseBackendChild(sarthiProcess)

      if (!backendConnectionState.clearForCurrentProcess(processOwner)) {
        rememberLog(`Ignoring stale Sarthi backend exit (${signal || code})`)
      ...
      rememberLog(`Sarthi backend exited (${signal || code})`)
      sendBackendExit({ code, signal })

      if (!backendReady) {
        const message = `Sarthi backend exited before it became ready (${signal || code}).`
        updateBootProgress(
          {
            error: message,
            message,
        ...
        rejectBackendStart?.(
          new Error(
            `Sarthi backend exited before it became ready (${signal || code}). Log: ${DESKTOP_LOG_PATH}\n${recentSarthiLog()}`
          )
        )
      }
    })

    await advanceBootProgress('backend.port', 'Waiting for Sarthi backend to launch', 86)

    // Discover the ephemeral port the child bound to
    const port = await Promise.race([
      waitForDashboardPortAnnouncement(sarthiProcess, { readyFile }),
      backendStartFailed
    ])
    ...
    const baseUrl = `http://127.0.0.1:${port}`
    await advanceBootProgress('backend.wait', 'Waiting for Sarthi backend to become ready', 90)
    await Promise.race([waitForSarthi(baseUrl, token), backendStartFailed])
    backendReady = true
    backendStartFailure = null

    const authToken = await adoptServedDashboardToken(baseUrl, token, {
      childAlive: () => sarthiProcess.exitCode === null && !sarthiProcess.killed,
      rememberLog
    })
    ...
    if (!wsProbe.ok) {
      throw new Error(
        `Local Sarthi backend is HTTP-reachable but the WebSocket (/api/ws) rejected the session token: ${wsProbe.reason}`
      )
    }

    updateBootProgress({
      phase: 'backend.ready',
      message: 'Sarthi backend is ready. Finalizing desktop startup',
      progress: 94,
      running: true,
      error: null
    })
...
// here so it can leave the app's bounds and stay visible while Sarthi is
...
// pushes pet state over IPC (sarthi:pet-overlay:state); the overlay just renders
// it. Control flows back (pop-in, composer submit) via sarthi:pet-overlay:control.
...
function spawnPetOverlayWindow(bounds) {
  const win = new BrowserWindow({
    ...
    // instead, leaving the main Sarthi app as the Dock/cmd-tab anchor.
    skipTaskbar: !IS_MAC,
    hasShadow: false,
    alwaysOnTop: true,
    ...
    // anchor and the Sarthi icon drops out of cmd/alt-tab — especially when the
    // main window is minimized. We flip this on only while the composer needs
    // the keyboard (see sarthi:pet-overlay:set-focusable).
    focusable: false,
    show: false,
  ...
  try {
    ...
    // which briefly hides the Dock/cmd-tab presence. Keep Sarthi in the normal
  ...
  win.on('closed', () => {
    ...
    if (mainWindow && !mainWindow.isDestroyed()) {
      mainWindow.webContents.send('sarthi:pet-overlay:control', { type: 'pop-in' })
    }
  })
...
// window showing only the composer and its scrollback, so Sarthi can be driven
...
function startHudCursorFeed(win: BrowserWindow) {
  ...
  const timer = setInterval(() => {
    ...
    last = key
    win.webContents.send('sarthi:hud:cursor', point)
  }, HUD_CURSOR_POLL_MS)
...
function broadcastHudState(open) {
  ...
  for (const win of BrowserWindow.getAllWindows()) {
    if (!win.isDestroyed()) {
      win.webContents.send('sarthi:hud:changed', payload)
    }
  }
}
...
function spawnHudWindow(sessionId, profile) {
  const win = new BrowserWindow({
    ...
    // `sarthi:hud:set-bounds`, which flips resizable on for the call — the
...
function openHudWindow(sessionId, profile) {
  ...
  if (hudWindow && !hudWindow.isDestroyed()) {
    ...
    if (sessionId && sessionId !== hudSessionId) {
      ...
      hudWindow.webContents.send('sarthi:hud:goto', sessionId)
...
function spawnQuickEntryWindow() {
  ...
  win.webContents.on('did-finish-load', () => {
    if (!win.isDestroyed() && quickEntryLastState) {
      win.webContents.send('sarthi:quick-entry:state', quickEntryLastState)
    }
  })
...
function showQuickEntryWindow() {
  ...
  quickEntryWindow.webContents.send('sarthi:quick-entry:shown')
}
...
function createWindow() {
  ...
  startSarthi().catch(error => rememberLog(error.stack || error.message))
...
ipcMain.handle('sarthi:connection', async (_event, profile) => {
  const connection = await ensureBackend(profile)
...
ipcMain.handle('sarthi:connection:for', async (_event, payload) => {
...
// "Starting Sarthi…". Before the renderer's backoff loop reconnects, it asks us
// to confirm the cached PRIMARY backend is still reachable; if a remote one is
// not, we drop the cache so the next getConnection() rebuilds it. Local backends
// self-heal via their child 'exit' handler, so we never touch them here.
ipcMain.handle('sarthi:connection:revalidate', async () => {
  ...
  return remoteRevalidation.run(connectionPromise, async () => {
    const [result] = await Promise.all([
      revalidateRemoteConnection({
        ...
        resetConnection: resetSarthiConnection,
...
ipcMain.handle('sarthi:backend:touch', async (_event, profile) => {
  touchPoolBackend(profile)

  return { ok: true }
})
ipcMain.handle('sarthi:gateway:ws-url', async (_event, profile) => {
  return gatewayWsUrlIpcResult(() => freshGatewayWsUrl(profile))
})
ipcMain.handle('sarthi:window:openSession', async (_event, sessionId, opts) => {
...
ipcMain.handle('sarthi:window:openInstance', async () => {
...
ipcMain.handle('sarthi:window:openInTerminal', async (_event, sessionId, opts) => {
  ...
  try {
    ...
    const backend = resolveSarthiBackend(tuiResumeArgs(sessionId.trim(), profile || undefined))

    if (!backend.command) {
      return { ok: false, error: 'Sarthi is not installed yet' }
    }
    ...
    const scriptPath = path.join(
      scriptDir,
      `sarthi-${crypto.randomBytes(6).toString('hex')}${terminalScriptExtension()}`
    )
    ...
    fs.writeFileSync(
      ...
      buildTerminalScript({
        ...
        cwd,
        env: terminalScriptEnv(backend.env, SARTHI_HOME)
      }),
      { mode: 0o700 }
    )
...
ipcMain.handle('sarthi:wake-indicator:get', () => wakeIndicatorController.getState())
ipcMain.on('sarthi:wake-indicator:set', (_event, state) => {
...
ipcMain.handle('sarthi:zoom:get', event => {
...
ipcMain.on('sarthi:zoom:set-percent', (event, percent) => {
  const window = BrowserWindow.fromWebContents(event.sender)
...
ipcMain.handle('sarthi:bootstrap:reset', async () => {
  // Renderer's "Reload and retry" path. Clear the latched failure and
  // reset connection state so the next startSarthi() call restarts the
...
ipcMain.handle('sarthi:bootstrap:repair', async () => {
  // Forceful repair: force the next startSarthi() through the full installer
  ...
  // "bypass active runtime" path inside startSarthi() and fall through
...
ipcMain.handle('sarthi:boot-progress:get', async () => bootProgressState)
ipcMain.handle('sarthi:bootstrap:get', async () => getBootstrapState())
ipcMain.handle('sarthi:connection-config:get', async (_event, profile) =>
  sanitizeDesktopConnectionConfig(readDesktopConnectionConfig(), profile)
)
ipcMain.handle('sarthi:plugin-profile-routes', async (_event, rawProfileNames) => {
...
ipcMain.handle('sarthi:ssh-config:hosts', async () => ({ hosts: collectSshConfigHosts() }))
ipcMain.handle('sarthi:ssh-config:resolve', async (_event, host) => {
...
ipcMain.handle('sarthi:connection-config:test', async (_event, payload) => testDesktopConnectionConfig(payload))
...
ipcMain.handle('sarthi:connections:list', async () => sanitizeConnectionsRegistry())
ipcMain.handle('sarthi:connections:save', async (_event, payload) => {
  const saved = await saveRegistryConnection(payload)

  return { ok: true, connection: saved, registry: sanitizeConnectionsRegistry() }
})
ipcMain.handle('sarthi:connections:remove', async (_event, id) => {
  const key = String(id || '')
...
ipcMain.handle('sarthi:connections:set-primary', async (_event, id) => {
  const registry = setPrimaryConnection(readDesktopConnectionsRegistry(), String(id || ''))
  writeDesktopConnectionsRegistry(registry)

  return { ok: true, registry: sanitizeConnectionsRegistry(registry) }
})
ipcMain.handle('sarthi:connections:set-launch-mode', async (_event, mode) => {
  const registry = setConnectionLaunchMode(readDesktopConnectionsRegistry(), String(mode || ''))
  writeDesktopConnectionsRegistry(registry)

  return { ok: true, registry: sanitizeConnectionsRegistry(registry) }
})
ipcMain.handle('sarthi:connections:set-last-used', async (_event, id) => {
  const registry = setLastUsedConnection(readDesktopConnectionsRegistry(), String(id || ''))
  writeDesktopConnectionsRegistry(registry)

  return { ok: true, registry: sanitizeConnectionsRegistry(registry) }
})
ipcMain.handle('sarthi:connections:test', async (_event, id) => {
  const registry = readDesktopConnectionsRegistry()
  ...
  if (entry.kind === 'ssh') {
    const result = await testDesktopConnectionConfig({
      ...
      sshRemoteSarthiPath: entry.remoteSarthiPath
    })
...
async function probeSshProfileInventory(connection) {
  ...
  const sshConfig = normalizeSshConfig({
    ...
    remoteSarthiPath: connection.remoteSarthiPath
  })
  ...
  try {
    await ssh.open()
    const profiles = await remoteLifecycle.listRemoteSarthiProfiles(ssh)
    ...
    registry.connections.map(async connection => {
      ...
      try {
        ...
        if (connection.kind === 'ssh') {
          ...
          // The root SARTHI_HOME is an agent too; enumerations that omit it
...
ipcMain.handle('sarthi:agents:roster', async () => {
  const registry = readDesktopConnectionsRegistry()
  const enumerations = await enumerateRegistryAgentSources(registry)
...
// sarthi:gateway:ws-url. Same single-use-ticket discipline for OAuth sources.
ipcMain.handle('sarthi:gateway:ws-url-for', async (_event, payload) => {
  const { connectionId, profile } = payload && typeof payload === 'object' ? (payload as any) : ({} as any)
...
// Fan out `sarthi update` to every eligible registered connection at once.
...
// /api/sarthi/update endpoint (the dashboard updater), which runs
// `sarthi update` on THAT machine.
ipcMain.handle('sarthi:connections:update-all', async (_event, payload) => {
  const registry = readDesktopConnectionsRegistry()
    ...
    registry.connections
      ...
      .map(async connection => {
        ...
        try {
          ...
          const body: any = await postJsonForBackend(descriptor, '/api/sarthi/update', {}, { timeoutMs: 15_000 })
...
// analogue of the sarthi:api handler's own auth split: OAuth backends prefer a
...
ipcMain.handle('sarthi:connection-config:probe', async (_event, rawUrl) => probeRemoteAuthMode(rawUrl))
ipcMain.handle('sarthi:connection-config:oauth-login', async (_event, rawUrl) => {
  ...
  if (strategy === 'native') {
    try {
      ...
      // startSarthi() re-dials instead of replaying the stale rejection.
      remoteReauthFailure = null
...
ipcMain.handle('sarthi:connection-config:oauth-logout', async (_event, rawUrl) => {
...
// --- Sarthi Cloud (cloud-auto-discovery Phase 3) ---
...
ipcMain.handle('sarthi:cloud:agent-sign-in', async (_event, dashboardUrl) => {
...
ipcMain.handle('sarthi:connection-config:save', async (_event, payload) => {
  const config = coerceDesktopConnectionConfig(payload)
  writeDesktopConnectionConfig(config)

  return sanitizeDesktopConnectionConfig(config, payload?.profile)
})
ipcMain.handle('sarthi:connection-config:apply', async (_event, payload) => {
  const config = coerceDesktopConnectionConfig(payload)
  writeDesktopConnectionConfig(config)
...
ipcMain.handle('sarthi:profile:get', async () => ({ profile: readActiveDesktopProfile() }))
ipcMain.handle('sarthi:profile:set', async (_event, name) => {
  const next = writeActiveDesktopProfile(name)

  // Switching profiles is a backend re-home: relaunch the dashboard under the
  // new SARTHI_HOME. Pool backends keep their own homes, so only the primary
  // is torn down.
  await teardownPrimaryBackendAndWait()
  mainWindow?.reload()
...
ipcMain.on('sarthi:previewShortcutActive', (_event, active) => {
  previewShortcutActive = Boolean(active)
})

ipcMain.handle('sarthi:requestMicrophoneAccess', async () => {
...
ipcMain.handle('sarthi:window:readBelow', async event => {
...
async function handleSarthiApiRequest(request) {
  ...
  // backend calls ensure_sarthi_home() which recreates the profile directory,
...
ipcMain.handle('sarthi:api', async (_event, request) => {
  ...
  // respawn the old-name backend and recreate its SARTHI_HOME (#45474).
  ...
  if (!mutatingProfile) {
    return handleSarthiApiRequest(request)
  }

  const releaseProfileDeletion = profileDeletionGate.acquire(mutatingProfile)

  return handleSarthiApiRequest(request).finally(releaseProfileDeletion)
})
...
ipcMain.handle('sarthi:ambient:claim', (_event, key) => !claimedAmbientCue(String(key ?? '')))

ipcMain.handle('sarthi:notify', (_event, payload) => {
  ...
  notification.on('click', () => {
    ...
    if (payload?.sessionId) {
      mainWindow.webContents.send('sarthi:focus-session', payload.sessionId)
    }

    // Plugin / session-less activation — serializable path (+ optional notifyId
    // for renderer callbacks). Same vocabulary as sarthi://index-network/….
    if (payload?.activate || payload?.notifyId) {
      mainWindow.webContents.send('sarthi:notification-activate', {
  ...
  notification.on('action', (_actionEvent, index) => {
    ...
    if (payload?.sessionId && !payload?.notifyId && !payload?.activate) {
      mainWindow.webContents.send('sarthi:notification-action', { sessionId: payload.sessionId, actionId: action.id })
...
ipcMain.handle('sarthi:data-url-read-max:get', () => ({
...
ipcMain.handle('sarthi:data-url-read-max:set', (_event, maxMb) => {
...
ipcMain.handle('sarthi:readFileDataUrl', async (_event, filePath) => {
...
ipcMain.handle('sarthi:readFileDataUrlForAttach', async (_event, filePath) => {
  return readFileDataUrlForIpc(filePath, {
...
ipcMain.handle('sarthi:readFileText', async (_event, filePath) => {
...
ipcMain.handle('sarthi:selectPaths', async (_event, options: any = {}) => {
...
ipcMain.handle('sarthi:writeClipboard', (_event, text) => {
...
ipcMain.handle('sarthi:selectSavePath', async (_event, options: any = {}) => {
  const result = await dialog.showSaveDialog(mainWindow, {
...
ipcMain.handle('sarthi:readClipboard', () => clipboard.readText())

ipcMain.handle('sarthi:saveGatewayFile', (_event, payload) => saveGatewayFile(payload))

ipcMain.handle('sarthi:saveImageFromUrl', (_event, url) => saveImageFromUrl(String(url || '')))
...
ipcMain.handle('sarthi:context-menu:edit', (event, command) => {
...
ipcMain.handle('sarthi:context-menu:copy-image', event => {
...
ipcMain.handle('sarthi:context-menu:spellcheck', (event, action) => {
...
ipcMain.handle('sarthi:context-menu:guest-add-word', (_event, payload) => {
...
ipcMain.handle('sarthi:saveImageBuffer', async (_event, payload) => {
...
ipcMain.handle('sarthi:saveClipboardImage', async () => {
...
ipcMain.handle('sarthi:normalizePreviewTarget', (_event, target, baseDir) =>
  normalizePreviewTarget(String(target || ''), baseDir ? String(baseDir) : '')
)

ipcMain.handle('sarthi:watchPreviewFile', (_event, url) => watchPreviewFile(String(url || '')))

ipcMain.handle('sarthi:watchDirectory', (_event, dir) => watchDirectory(String(dir || '')))

ipcMain.handle('sarthi:stopPreviewFileWatch', (_event, id) => stopPreviewFileWatch(String(id || '')))
...
ipcMain.on('sarthi:active-work', (event, payload) => {
...
ipcMain.on('sarthi:titlebar-theme', (_event, payload) => {
...
ipcMain.on('sarthi:native-theme', (_event, mode) => {
...
ipcMain.on('sarthi:translucency:support', event => {
  event.returnValue = { glass: GLASS_SUPPORTED, translucency: TRANSLUCENCY_SUPPORTED }
})

ipcMain.on('sarthi:translucency', (_event, payload) => {
...
ipcMain.on('sarthi:keep-awake', (_event, on) => {
...
ipcMain.handle('sarthi:quick-entry:settings:get', async () => {
...
ipcMain.handle('sarthi:quick-entry:settings:set', async (_event, patch) => {
...
ipcMain.on('sarthi:quick-entry:submit', (_event, payload) => {
  hideQuickEntryWindow()
  ...
  mainWindow.webContents.send('sarthi:quick-entry:submit', {
...
ipcMain.on('sarthi:quick-entry:state', (_event, payload) => {
  quickEntryLastState = payload ?? null

  if (quickEntryWindow && !quickEntryWindow.isDestroyed()) {
    quickEntryWindow.webContents.send('sarthi:quick-entry:state', payload)
  }
})

ipcMain.on('sarthi:quick-entry:dismiss', () => hideQuickEntryWindow())
...
ipcMain.on('sarthi:devtools:disable-f12', (_event, on) => {
...
ipcMain.handle('sarthi:openExternal', (_event, url) => {
...
// per-session secondary windows spawned via `sarthi:window:openSession`).
...
ipcMain.handle('sarthi:find-in-page', async (event, query, options) => {
  const win = BrowserWindow.fromWebContents(event.sender)
...
ipcMain.handle('sarthi:stop-find-in-page', event => {
  const win = BrowserWindow.fromWebContents(event.sender)
...
ipcMain.handle('sarthi:preview:reach', async (_event, url) => reachablePreviewUrl(String(url || '')))

ipcMain.handle('sarthi:openPreviewInBrowser', async (_event, url) => {
...
// it via writeDefaultProjectDir so resolveSarthiCwd picks it up on the next
// session spawn (no app restart needed).
ipcMain.handle('sarthi:setting:defaultProjectDir:get', async () => ({
  dir: readDefaultProjectDir(),
  defaultLabel: app.getPath('home'),
  resolvedCwd: resolveSarthiCwd()
}))

ipcMain.handle('sarthi:workspace:sanitize', async (_event, cwd) => sanitizeWorkspaceCwd(cwd))

ipcMain.handle('sarthi:setting:defaultProjectDir:set', async (_event, dir) => {
...
ipcMain.handle('sarthi:setting:defaultProjectDir:pick', async () => {
...
ipcMain.handle('sarthi:fetchLinkTitle', (_event, url) => fetchLinkTitle(url))

ipcMain.handle('sarthi:resolveFavicon', (_event, url) => resolveFaviconCached(url))

ipcMain.handle('sarthi:logs:reveal', async () => {
  try {
    await fs.promises.mkdir(path.dirname(DESKTOP_LOG_PATH), { recursive: true })
...
ipcMain.handle('sarthi:logs:recent', async () => ({ path: DESKTOP_LOG_PATH, lines: sarthiLog.slice(-200) }))
...
ipcMain.on('sarthi:logs:renderer-error', (_event, report) => {
...
// Embedded terminal PTY host (sarthi:terminal:*) — see terminal-ipc.ts.
...
ipcMain.handle('sarthi:updates:check', async () =>
...
ipcMain.handle('sarthi:updates:apply', async (_event, payload) =>
...
ipcMain.handle('sarthi:updates:branch:get', async () => readDesktopUpdateConfig())

ipcMain.handle('sarthi:updates:branch:set', async (_event, name) => {
...
// Resolve the canonical Sarthi version (the one `release.py` bumps in
// sarthi_cli/__init__.py + pyproject.toml) so the desktop About panel shows the
// real Sarthi version instead of the Electron app's own package.json version,
// which historically drifted (stuck at 0.0.2). Falls back to app.getVersion()
// when the source tree can't be read (e.g. a packaged build without the repo).
function resolveSarthiVersion() {
  try {
    const root = resolveUpdateRoot()
    const initPath = path.join(root, 'sarthi_cli', '__init__.py')
...
// Renderer-bundle skew: `sarthi update` moves the SOURCE TREE, but the UI
...
// Re-resolve the live Sarthi version and push it into the native About panel
// just before showing it, so an in-place `sarthi update` is reflected without
  ...
  void detectRendererSkew().then(skew => {
    app.setAboutPanelOptions({
      ...
      applicationVersion: skew.outOfSync
        ? `${resolveSarthiVersion()} — app build out of date, update the desktop app`
        : resolveSarthiVersion(),
      copyright: 'Copyright © 2026 Nous Research'
    })
    app.showAboutPanel()
  })
...
ipcMain.handle('sarthi:version', async () => {
  const skew = await detectRendererSkew()

  return {
    appVersion: resolveSarthiVersion(),
    electronVersion: process.versions.electron,
    nodeVersion: process.versions.node,
    platform: process.platform,
    sarthiRoot: resolveUpdateRoot(),
...
// the actual removal via `sarthi uninstall …` so the cross-platform PATH /
// registry / service / node-symlink cleanup all lives in one place
// (sarthi_cli/uninstall.py + sarthi_cli/gui_uninstall.py).
//
...
async function getUninstallSummary() {
  ...
  const agentRoot = ACTIVE_SARTHI_ROOT
  ...
  const fallback = () => ({
    sarthi_home: SARTHI_HOME,
    agent_installed: isSarthiSourceRoot(agentRoot) && fileExists(py),
    gui_installed: true,
  ...
  return new Promise(resolve => {
    ...
    try {
      const child = spawn(
        py,
        ['-m', 'sarthi_cli.main', 'uninstall', '--gui-summary'],
        hiddenWindowsChildOptions({
          cwd: agentRoot,
          env: { ...process.env, SARTHI_HOME, NO_COLOR: '1' },
          stdio: ['ignore', 'pipe', 'ignore']
        })
      )
...
async function runDesktopUninstall(mode) {
  ...
  if (!fileExists(venvPy)) {
    return {
      ok: false,
      error: 'agent-missing',
      message: `Can't run the uninstaller: no Sarthi agent venv at ${VENV_ROOT}.`
    }
  }
  ...
  // Python with PYTHONPATH=<agentRoot> so `import sarthi_cli` resolves from
  ...
  try {
    await releaseBackendLock(ACTIVE_SARTHI_ROOT, 'uninstall')
  } catch (error) {
  ...
  try {
    if (IS_WINDOWS) {
      scriptPath = path.join(app.getPath('temp'), `sarthi-uninstall-${Date.now()}.cmd`)
      ...
      scriptPath = path.join(app.getPath('temp'), `sarthi-uninstall-${Date.now()}.sh`)
...
ipcMain.handle('sarthi:uninstall:summary', async () => getUninstallSummary())
ipcMain.handle('sarthi:uninstall:run', async (_event, payload) => {
...
ipcMain.handle('sarthi:vscode-theme:fetch', async (_event, id) => fetchMarketplaceThemes(String(id || '')))

// Search the Marketplace for color-theme extensions (empty query = top installs).
ipcMain.handle('sarthi:vscode-theme:search', async (_event, query) => searchMarketplaceThemes(String(query || ''), 20))

// ---------------------------------------------------------------------------
// sarthi:// deep links (e.g. sarthi://blueprint/morning-brief?time=08:00,
// sarthi://mcp/install?name=NAME&config=B64 — the vendor "Add to Sarthi"
// button, or sarthi://plugin/install?repo=owner/repo). Dev
// (`SARTHI_DESKTOP_DEV_SERVER`) registers sarthi-dev:// instead — bare
// Electron or a stale OS handler often owns sarthi:// on dev machines.
...
// ---------------------------------------------------------------------------
const SARTHI_PROTOCOL = DEV_SERVER ? 'sarthi-dev' : 'sarthi'
/** Schemes accepted when parsing inbound URLs (dev accepts both). */
const DEEPLINK_SCHEMES = DEV_SERVER ? ['sarthi-dev', 'sarthi'] : ['sarthi']
...
function handleDeepLink(url) {
  ...
  // sarthi://blueprint/<key>?slot=val  -> host="blueprint", path="/<key>"
  ...
  try {
    ...
    mainWindow.focus()
    mainWindow.webContents.send('sarthi:deep-link', payload)
...
ipcMain.handle('sarthi:deep-link-ready', () => {
  ...
  if (_pendingDeepLink) {
    ...
    handleDeepLink(
      `${SARTHI_PROTOCOL}://${queued.kind}/${encodeURIComponent(queued.name)}` +
...
function registerDeepLinkProtocol() {
  try {
    if (process.defaultApp && process.argv.length >= 2) {
      ...
      app.setAsDefaultProtocolClient(SARTHI_PROTOCOL, process.execPath, [entry])
    } else {
      app.setAsDefaultProtocolClient(SARTHI_PROTOCOL)
    }

    rememberLog(`[deeplink] registered ${SARTHI_PROTOCOL}:// handler`)
  } catch (err) {
...
// second-instance argv. Without the lock a second `sarthi://` launch spawns a
...
if (!isPrimaryInstance) {
  ...
  // startup (shortcut registration, createWindow → startSarthi), whose
...
app.whenReady().then(() => {
  ...
  // startSarthi() finishes resolving the configured backend.
  const primaryProfile = primaryProfileKey()
  ...
  // Win/Linux cold start: the launching sarthi:// URL is in our own argv.
...
app.on('before-quit', event => {
  ...
  // quitting Sarthi never keeps another app's chord hostage.
