...
import { buildDesktopBackendEnv, normalizeSarthiHomeRoot } from './backend-env'
import { canImportSarthiCli, verifySarthiCli } from './backend-probes'
...
function sarthiManagedNodePathEntries() {
  // NOTE: keep this ordering in sync with iter_sarthi_node_dirs() in
  // sarthi_constants.py — this Node main process cannot import the Python
  // module, so the platform-ordering rule is mirrored here.
  const root = path.join(SARTHI_HOME, 'node')
...
function pathWithSarthiManagedNode(...entries) {
  return [...sarthiManagedNodePathEntries(), ...entries, process.env.PATH].filter(Boolean).join(path.delimiter)
}
...
const MEDIA_PROTOCOL = 'sarthi-media'

// Only audio/video may be streamed. Without this the handler would read any
// non-blocklisted local file (no size cap) for any `fetch(sarthi-media://…)`.
...
let mainWindow = null
let sarthiProcess = null
let connectionPromise = null
// True while connection-config:apply soft-rehomes the primary — suppresses the
// backend-exit toast so an intentional kill doesn't look like a crash.
let softRehomeInProgress = false
// Additional per-profile backends, keyed by profile name. The PRIMARY backend
// (the desktop's launch profile) stays managed by sarthiProcess +
// connectionPromise + startSarthi(); this pool only holds EXTRA profile
...
async function waitForUpdateToFinish() {
  let marker = readLiveUpdateMarker(SARTHI_HOME)
  ...
  while (marker && Date.now() < deadline) {
    ...
    marker = readLiveUpdateMarker(SARTHI_HOME)
  }
...
// findGitBash — locate bash.exe on Windows. Sarthi' terminal tool requires
...
function findGitBash() {
  ...
  // install.ps1 drops PortableGit at %LOCALAPPDATA%\sarthi\git\... — checked
  ...
  if (localAppData) {
    candidates.push(path.join(localAppData, 'sarthi', 'git', 'bin', 'bash.exe'))
    candidates.push(path.join(localAppData, 'sarthi', 'git', 'usr', 'bin', 'bash.exe'))
  }
...
// SARTHI_HOME/sarthi-setup.exe on a successful install (see
// apps/bootstrap-installer paths::copy_self_to_sarthi_home). That binary owns
// ALL repo mutation — running `sarthi update` + rebuilding the desktop — so
...
function resolveUpdaterBinary() {
  const name = IS_WINDOWS ? 'sarthi-setup.exe' : 'sarthi-setup'
  const candidate = path.join(SARTHI_HOME, name)
...
async function releaseBackendLock(updateRoot, tag) {
  ...
  if (sarthiProcess && Number.isInteger(sarthiProcess.pid)) {
    pids.push(sarthiProcess.pid)
  }
  ...
  if (sarthiProcess && !sarthiProcess.killed) {
    try {
      sarthiProcess.kill('SIGTERM')
    } catch {
      void 0
    }
  }
  ...
  while (Date.now() < deadlineMs) {
    ...
    if (sarthiProcess && Number.isInteger(sarthiProcess.pid)) {
      stragglers.push(sarthiProcess.pid)
    }
...
async function applyUpdates(opts = {}) {
  ...
  try {
    ...
    if (!updater && !IS_WINDOWS) {
      // macOS/Linux drag-install: no staged Tauri sarthi-setup. Unlike Windows
      ...
      // whole update itself: `sarthi update` (backend) + `sarthi desktop
    ...
    if (!updater) {
      ...
      // sarthi-setup.exe into SARTHI_HOME). They DO have a working `sarthi`
      ...
      // checkout they're on — bare `sarthi update` defaults to main and would
...
async function handOffWindowsBootstrapRecovery(reason) {
  ...
  // not just the `sarthi.exe` console-script shim. That shim is generated at the
  ...
  const haveRealInstall =
    fileExists(venvPython) || fileExists(venvSarthi) || fileExists(path.join(updateRoot, '.sarthi-bootstrap-complete'))
...
// Resolve the sarthi CLI to drive an in-app update: prefer the venv shim in
// the install we're updating, fall back to `sarthi` on PATH.
function resolveSarthiCliBinary(updateRoot) {
  const venvSarthi = path.join(updateRoot, 'venv', 'bin', 'sarthi')

  if (fileExists(venvSarthi)) {
    return venvSarthi
  }

  return findOnPath('sarthi') || null
}
...
// macOS/Linux in-app update: backend (`sarthi update`) + OS-aware GUI rebuild
// (`sarthi desktop --build-only`), then atomically swap the running .app bundle
...
async function applyUpdatesPosixInApp(opts: any) {
  const updateRoot = resolveUpdateRoot()
  const sarthi = resolveSarthiCliBinary(updateRoot)
  ...
  // Put the Sarthi-managed Node and the venv on PATH so `sarthi desktop`'s
  // npm build can find them on a machine with no system Node. Windows portable
  // Node lives directly under %LOCALAPPDATA%\sarthi\node, not node\bin.
  const env: Record<string, string> = {
    SARTHI_HOME,
    PATH: pathWithSarthiManagedNode(path.join(updateRoot, 'venv', 'bin'))
  }

  // `sarthi update` reaps stale `sarthi serve` backends (a code update
  ...
  // _kill_stale_dashboard_processes reads SARTHI_DESKTOP_CHILD_PID and excludes
  ...
  if (sarthiProcess && Number.isInteger(sarthiProcess.pid)) {
    desktopChildPids.push(sarthiProcess.pid)
  }
  ...
  if (desktopChildPids.length) {
    env.SARTHI_DESKTOP_CHILD_PID = desktopChildPids.join(',')
  }
  ...
  emitUpdateProgress({ stage: 'update', message: 'Updating Sarthi (git + dependencies)…', percent: 10 })

  const updated = (await runStreamedUpdate(sarthi, ['update', '--yes', ...branchArgs], {
    cwd: updateRoot,
    env,
    stage: 'update'
  })) as any
  ...
  if (updated.code !== 0) {
    emitUpdateProgress({ stage: 'error', message: 'sarthi update failed.', error: updated.error || 'update-failed' })

    return { ok: false, error: 'sarthi update failed' }
  }
  ...
  const rebuilt = await runRebuildWithRetry(attempt => {
    ...
    return runStreamedUpdate(sarthi, ['desktop', '--build-only'], { cwd: updateRoot, env, stage: 'rebuild' })
  })

  if (rebuilt.code !== 0) {
    emitUpdateProgress({
      stage: 'error',
      message: 'Backend updated, but the desktop rebuild failed. Restart Sarthi to retry.',
  ...
  // Linux in-app update terminal state (#45205). `sarthi desktop --build-only`
  ...
  if (!IS_MAC) {
    ...
    if (outcome === 'relaunch') {
      emitUpdateProgress({ stage: 'restart', message: 'Restarting Sarthi…', percent: 100 })
      ...
      const scriptPath = path.join(app.getPath('temp'), `sarthi-desktop-update-${Date.now()}.sh`)
      ...
      try {
        ...
        return {
          ...
          guiUpdated: false,
          manualRestart: true,
          message: 'Backend updated. Quit and reopen Sarthi to load the new version.'
        }
      }
    }
    ...
    if (outcome === 'guiSkew') {
      emitUpdateProgress({
        ...
        message:
          ...
          'Update or reinstall the Sarthi desktop app to match.',
        percent: 100
      })
      rememberLog(
    ...
    return {
      ...
      message:
        ...
        '(sandbox helper needs root). Quit and reopen Sarthi to finish.'
    }
  }

  const rebuiltApp = [
    path.join(updateRoot, 'apps', 'desktop', 'release', 'mac-arm64', 'Sarthi.app'),
    path.join(updateRoot, 'apps', 'desktop', 'release', 'mac', 'Sarthi.app')
  ...
  if (!rebuiltApp || !targetApp) {
    emitUpdateProgress({
      stage: 'done',
      message: 'Backend updated. Restart Sarthi to load the new version.',
      percent: 100
    })
...
if [ "$SRC" != "$DST" ]; then
  if /usr/bin/ditto "$SRC" "$DST.sarthi-update-new"; then
    rm -rf "$DST.sarthi-update-old" 2>/dev/null || true
    mv "$DST" "$DST.sarthi-update-old" 2>/dev/null || rm -rf "$DST"
    mv "$DST.sarthi-update-new" "$DST"
    rm -rf "$DST.sarthi-update-old" 2>/dev/null || true
  fi
fi
...
`
  ...
  try {
    ...
    emitUpdateProgress({
      stage: 'done',
      message: 'Backend + app updated. Restart Sarthi to load the new version.',
      percent: 100
    })
...
function resolveSarthiBackend(backendArgs) {
  ...
  // 3. Bootstrap-complete ACTIVE_SARTHI_ROOT -- the canonical install at
  ...
  //    to spawning sarthi. Updates flow through the in-app update path
  //    (applyUpdates -> git pull) or `sarthi update` from the CLI.
  ...
  if (process.env.SARTHI_DESKTOP_IGNORE_EXISTING !== '1') {
    ...
    if (sarthiCommand) {
      ...
      if (verifySarthiCli(sarthiCommand, { shell: shellForProbe })) {
        return (
          unwrapWindowsVenvSarthiCommand(sarthiCommand, backendArgs) || {
...
async function ensureRuntime(backend) {
  ...
  if (!fileExists(venvPython)) {
    ...
    // isSarthiSourceRoot() and the bootstrap writes the marker only after
...
async function waitForSarthi(baseUrl, token) {
  ...
  throw new Error(`Sarthi backend did not become ready: ${lastError?.message || 'timeout'}`)
}
...
async function discoverCloudAgents(org?: string) {
  ...
  try {
    ...
    if (error && error.statusCode === 401) {
      const err = new Error('Your Sarthi Cloud session has expired. Open Settings → Gateway and sign in again.') as any
      err.needsCloudLogin = true
      err.cause = error
      throw err
    }
...
async function sanitizeDesktopConnectionConfig(config = readDesktopConnectionConfig(), profile = null) {
  ...
  // saved mode, preserving 'cloud' (a Sarthi Cloud connection — Q6) so the UI
...
async function buildRemoteConnection(rawUrl, authMode, token, source) {
  ...
  if (authMode === 'oauth') {
    ...
    // Portal issues a 24h rotating refresh token (sarthi #37247), and the
...
async function resolveRemoteBackend(profile) {
  ...
  const rawEnvUrl = process.env.SARTHI_DESKTOP_REMOTE_URL
  const rawEnvToken = process.env.SARTHI_DESKTOP_REMOTE_TOKEN

  if (rawEnvUrl) {
    if (!rawEnvToken) {
      throw new Error(
        'SARTHI_DESKTOP_REMOTE_URL is set but SARTHI_DESKTOP_REMOTE_TOKEN is not. ' +
          'Both must be provided to connect to a remote Sarthi backend.'
      )
    }
...
async function teardownPrimaryBackendAndWait({ soft = false } = {}) {
  // Capture the reference before resetSarthiConnection() nulls sarthiProcess.
...
// profile to startSarthi() (the window backend: boot UI, bootstrap, remote
...
async function spawnPoolBackend(profile, entry) {
  ...
  if (remote) {
    await waitForSarthi(remote.baseUrl, remote.token)
...
async function startSarthi() {
  ...
  connectionPromise = (async () => {
    ...
    const authToken = await adoptServedDashboardToken(baseUrl, token, {
      // The exit/error handlers null sarthiProcess when the child dies.
      childAlive: () => sarthiProcess !== null && sarthiProcess.exitCode === null && !sarthiProcess.killed,
      rememberLog
    })
...
ipcMain.handle('sarthi:connection', async (_event, profile) => ensureBackend(profile))
...
ipcMain.handle('sarthi:connection:revalidate', async () => {
  ...
  try {
    ...
    // tick rebuilds a fresh, reachable descriptor. resetSarthiConnection only
    // nulls connectionPromise for a remote (no child to SIGTERM).
    rememberLog('Cached remote Sarthi backend failed liveness probe; dropping stale connection.')
...
ipcMain.handle('sarthi:gateway:ws-url', async (_event, profile) => freshGatewayWsUrl(profile))
...
ipcMain.handle('sarthi:window:openNewSession', async () => {
...
ipcMain.handle('sarthi:pet-overlay:open', async (_event, request) => {
...
ipcMain.handle('sarthi:pet-overlay:close', async () => {
  closePetOverlay()
...
ipcMain.on('sarthi:pet-overlay:set-bounds', (_event, bounds) => {
...
ipcMain.on('sarthi:pet-overlay:ignore-mouse', (_event, ignore) => {
  if (petOverlayWindow && !petOverlayWindow.isDestroyed()) {
...
ipcMain.on('sarthi:pet-overlay:set-focusable', (_event, focusable) => {
  if (!petOverlayWindow || petOverlayWindow.isDestroyed()) {
    return
  }
...
ipcMain.on('sarthi:pet-overlay:state', (_event, payload) => {
  if (petOverlayWindow && !petOverlayWindow.isDestroyed()) {
    petOverlayWindow.webContents.send('sarthi:pet-overlay:state', payload)
  }
})
// Overlay → main renderer: control messages (pop back in, composer submit).
ipcMain.on('sarthi:pet-overlay:control', (_event, payload) => {
  if (!mainWindow || mainWindow.isDestroyed()) {
    return
  }
  ...
  mainWindow.webContents.send('sarthi:pet-overlay:control', payload)
})
...
ipcMain.handle('sarthi:bootstrap:repair', async () => {
  ...
  // startSarthi() re-runs the full installer (refreshing a broken/partial
...
ipcMain.handle('sarthi:connection-config:apply', async (_event, payload) => {
  ...
  if (key && key !== primaryProfileKey()) {
    ...
    // wipes session lists (skeletons) and re-dials on sarthi:connection:applied.
...
function terminalShellCommand() {
  // SARTHI_DESKTOP_SHELL is the cross-platform escape hatch (a path or a bare
  ...
  const override = (process.env.SARTHI_DESKTOP_SHELL || (IS_WINDOWS ? '' : process.env.SHELL) || '').trim()
...
function terminalShellEnv() {
  ...
  // /TERM=dumb; some terminals set COLORFGBG which would flip Sarthi' TUI into
  ...
  env.TERM_PROGRAM = 'Sarthi'
  env.TERM_PROGRAM_VERSION = app.getVersion()

  // Let a sarthi/--tui launched in this pane know it's embedded in the desktop
  // GUI (build_environment_hints surfaces this). Distinct from SARTHI_DESKTOP,
  // which marks the agent *backend* and gates cron/gateway behavior.
  env.SARTHI_DESKTOP_TERMINAL = '1'

  return env
}

function terminalChannel(id, suffix) {
  return `sarthi:terminal:${id}:${suffix}`
}
...
ipcMain.handle('sarthi:fs:readDir', async (_event, dirPath) => readDirForIpc(dirPath))

ipcMain.handle('sarthi:fs:gitRoot', async (_event, startPath) => gitRootForIpc(startPath))

// Reveal a path in the OS file manager (Finder / Explorer / Files).
ipcMain.handle('sarthi:fs:reveal', async (_event, targetPath) => {
...
ipcMain.handle('sarthi:fs:openDir', async (_event, dirPath) => {
...
ipcMain.handle('sarthi:fs:rename', async (_event, targetPath, newName) => {
...
ipcMain.handle('sarthi:fs:writeText', async (_event, filePath, content) => {
...
ipcMain.handle('sarthi:fs:trash', async (_event, targetPath) => {
  const target = String(targetPath || '').trim()
...
ipcMain.handle('sarthi:git:worktreeList', async (_event, repoPath) => listWorktrees(repoPath, resolveGitBinary()))

ipcMain.handle('sarthi:git:worktreeAdd', async (_event, repoPath, options) =>
  addWorktree(repoPath, options || {}, resolveGitBinary())
)

ipcMain.handle('sarthi:git:worktreeRemove', async (_event, repoPath, worktreePath, options) =>
  removeWorktree(repoPath, worktreePath, options || {}, resolveGitBinary())
)

ipcMain.handle('sarthi:git:branchSwitch', async (_event, repoPath, branch) =>
  switchBranch(repoPath, branch, resolveGitBinary())
)

ipcMain.handle('sarthi:git:branchList', async (_event, repoPath) => listBranches(repoPath, resolveGitBinary()))

ipcMain.handle('sarthi:git:baseBranchList', async (_event, repoPath) => listBaseBranches(repoPath, resolveGitBinary()))
...
ipcMain.handle('sarthi:git:repoStatus', async (_event, repoPath) => repoStatus(repoPath, resolveGitBinary()))
...
ipcMain.handle('sarthi:git:review:list', async (_event, repoPath, scope, baseRef) =>
  reviewList(repoPath, scope, baseRef, resolveGitBinary())
)
ipcMain.handle('sarthi:git:review:diff', async (_event, repoPath, filePath, scope, baseRef, staged) =>
...
ipcMain.handle('sarthi:git:fileDiff', async (_event, repoPath, filePath) =>
  fileDiffVsHead(repoPath, filePath, resolveGitBinary())
)
ipcMain.handle('sarthi:git:review:stage', async (_event, repoPath, filePath) =>
  reviewStage(repoPath, filePath ?? null, resolveGitBinary())
)
ipcMain.handle('sarthi:git:review:unstage', async (_event, repoPath, filePath) =>
  reviewUnstage(repoPath, filePath ?? null, resolveGitBinary())
)
ipcMain.handle('sarthi:git:review:revert', async (_event, repoPath, filePath) =>
  reviewRevert(repoPath, filePath ?? null, resolveGitBinary())
)
ipcMain.handle('sarthi:git:review:revParse', async (_event, repoPath, ref) =>
  reviewRevParse(repoPath, ref, resolveGitBinary())
)
ipcMain.handle('sarthi:git:review:commit', async (_event, repoPath, message, push) =>
  reviewCommit(repoPath, message, Boolean(push), resolveGitBinary())
)
ipcMain.handle('sarthi:git:review:commitContext', async (_event, repoPath) =>
  reviewCommitContext(repoPath, resolveGitBinary())
)
ipcMain.handle('sarthi:git:review:push', async (_event, repoPath) => reviewPush(repoPath, resolveGitBinary()))
ipcMain.handle('sarthi:git:review:shipInfo', async (_event, repoPath) => reviewShipInfo(repoPath, resolveGhBinary()))
ipcMain.handle('sarthi:git:review:createPr', async (_event, repoPath) =>
...
ipcMain.handle('sarthi:git:scanRepos', async (_event, roots, options) => {
  try {
...
ipcMain.handle('sarthi:terminal:start', async (event, payload = {}) => {
...
ipcMain.handle('sarthi:terminal:write', (_event, id, data) => {
...
ipcMain.handle('sarthi:terminal:resize', (_event, id, size = {}) => {
  const sessionInfo = terminalSessions.get(String(id || ''))
...
ipcMain.handle('sarthi:terminal:cwd', async (_event, id) => {
  const sessionInfo = terminalSessions.get(String(id || ''))
...
ipcMain.handle('sarthi:terminal:dispose', (_event, id) => disposeTerminalSession(String(id || '')))
...
function _extractDeepLink(argv) {
  ...
  return argv.find(a => typeof a === 'string' && a.startsWith(`${SARTHI_PROTOCOL}://`)) || null
}
...
function registerDeepLinkProtocol() {
  try {
    if (process.defaultApp && process.argv.length >= 2) {
      ...
      app.setAsDefaultProtocolClient(SARTHI_PROTOCOL, process.execPath, [path.resolve(process.argv[1])])
    } else {
