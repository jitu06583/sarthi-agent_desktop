...
function ownershipEntry(overrides: Partial<BackendIdentity> = {}) {
  return { command: 'sarthi serve --port 0', ...identity(overrides) }
}
...
test('backend identity check matches only serve and dashboard invocation shapes', () => {
  assert.equal(backendCommandMatches('/venv/bin/sarthi serve --port 0'), true)
  assert.equal(backendCommandMatches('python -m sarthi_cli.main dashboard --no-open'), true)
  assert.equal(backendCommandMatches('/venv/bin/sarthi --profile work serve --port 0'), true)
  assert.equal(backendCommandMatches('"C:\\Sarthi Runtime\\sarthi.exe" dashboard --no-open'), true)
  assert.equal(backendCommandMatches('sarthi chat --query serve'), false)
