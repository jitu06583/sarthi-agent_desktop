...
test('desktop backend PATH adds Sarthi-managed bins and missing POSIX sane entries', () => {
  ...
  assert.equal(entries[0], '/Users/test/.sarthi/node/bin')
  assert.equal(entries[1], '/Users/test/.sarthi/sarthi-agent/venv/bin')
...
test('buildDesktopBackendEnv extends PYTHONPATH and backend PATH together', () => {
  ...
  assert.ok(env.PATH.startsWith('/Users/test/.sarthi/node/bin:/Users/test/.sarthi/sarthi-agent/venv/bin:'))
...
test('Windows PATH casing and delimiter are preserved without POSIX sane entries', () => {
  ...
  assert.ok(env.Path.startsWith('C:\\Users\\test\\AppData\\Local\\sarthi\\node\\bin;'))
