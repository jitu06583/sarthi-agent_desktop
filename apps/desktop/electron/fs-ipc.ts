...
// hardening, SARTHI_HOME resolution, and the git binary stay injected.
...
export function registerFsIpc({
  ...
  // The LOCAL Desktop runtime-plugin root: `<SARTHI_HOME>/desktop-plugins`,
  // resolved from the main-process SARTHI_HOME (see resolveSarthiHome) — NOT from
  // the connected backend. A remote backend reports its own `sarthi_home` over
  ...
  async function localPluginsRoot(dirName: string): Promise<string> {
    ...
    // profiles/<name>/, matching the profile-scoped sarthi_home the backend
    // reported before this resolver existed. 'default'/unset pins the global root.
    const profile = readActiveDesktopProfile()
    const base = profile && profile !== 'default' ? path.join(sarthiHome, 'profiles', profile) : sarthiHome
  ...
  ipcMain.handle('sarthi:fs:desktopPluginsRoot', async () => localPluginsRoot('desktop-plugins'))

  // The LOCAL logs root (`<SARTHI_HOME>/logs`, profile-aware) — the error
  // card's "Open Logs" action reveals agent.log/gateway.log without the user
  // knowing where SARTHI_HOME lives. Same Electron-local resolution as the
  // plugin roots: valid in every connection mode, created on demand.
  ipcMain.handle('sarthi:fs:logsRoot', async () => localPluginsRoot('logs'))

  // The LOCAL agent-plugin root (`<SARTHI_HOME>/plugins`), same Electron-local
  ...
  ipcMain.handle('sarthi:fs:agentPluginsRoot', async () => localPluginsRoot('plugins'))

  ipcMain.handle('sarthi:plugin:probe', async (_event, payload) => {
  ...
  ipcMain.handle('sarthi:plugin:installDesktop', async (_event, payload) => {
    const identifier = String(payload?.identifier || payload?.repo || '').trim()
