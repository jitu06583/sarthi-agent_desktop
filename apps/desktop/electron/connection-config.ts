 ...
 *     `X-Sarthi-Session-Token` header; WS uses `?token=`.
...
// sarthi_cli/dashboard_auth/cookies.py.
//
// Two cookies are in play (see that module):
//   - sarthi_session_at: the OAuth access token. Short-lived (~15 min); its
//     Max-Age tracks the access-token TTL, so the cookie jar drops it the
//     instant the AT expires.
//   - sarthi_session_rt: the OAuth refresh token. Long-lived (24h rotating,
//     reuse-detected — Portal NAS #293 / sarthi #37247). When the AT cookie
...
const AT_COOKIE_VARIANTS = ['__Host-sarthi_session_at', '__Secure-sarthi_session_at', 'sarthi_session_at']
const RT_COOKIE_VARIANTS = ['__Host-sarthi_session_rt', '__Secure-sarthi_session_rt', 'sarthi_session_rt']

// The Nous portal (NAS) does NOT use Sarthi gateway session cookies — it is a
...
// Keep this aligned with sarthi_cli.profiles.validate_profile_name(). `default`
// is the built-in root alias; these names cannot be created as profiles.
const RESERVED_REMOTE_PROFILES = new Set(['sarthi', 'test', 'tmp', 'root', 'sudo'])
...
/** Which Sarthi profile the remote SSH dashboard should actually run as.
...
// Sarthi Cloud connection (cloud-auto-discovery Q3/Q6): it carries a
...
function normalizeSshConfig(entry) {
  ...
  const remoteSarthiPath = String(entry.remoteSarthiPath || '').trim()

  if (remoteSarthiPath) {
    out.remoteSarthiPath = remoteSarthiPath
  }

  // A Desktop profile can be a local routing label rather than the profile
  // name used by the remote Sarthi installation. Preserve an explicit mapping
  // when it is a valid Sarthi profile identifier; otherwise fall back to the
...
function localPrimaryRequestScope(opts: ProfileRouteOptions): boolean | null {
  ...
  // spawned them: `_spawn_sarthi_action` registers the (often dynamic, e.g.
 ...
 *     `--profile`, so its `SARTHI_HOME` scopes it.
 *
 ...
 * and by the registry SSH branch of the `sarthi:api` handler — both routes
...
function translateSelfProfileQuery(path, profile, backendProfile) {
  ...
  try {
    parsed = new URL(rawPath, 'http://sarthi.local')
  } catch {
    return path
  }
...
/**
 * Resolve the two decisions made by the `sarthi:api` IPC handler from the same
...
/**
 * True if any cookie in `cookies` is a sarthi session ACCESS-token cookie
 ...
 * the Sarthi gateway session cookies, so cloud sign-in / discovery liveness
