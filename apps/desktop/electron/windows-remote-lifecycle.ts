...
const READY_RE = /^SARTHI_(?:BACKEND|DASHBOARD)_READY port=(\d+)/gm
...
async function probeWindowsRemote(ssh, explicitSarthiPath = '') {
  const explicit = psLiteral(explicitSarthiPath)

  const script = [
    '$ErrorActionPreference="Stop"',
    `$explicit=${explicit}`,
    '$sarthiHome=$env:SARTHI_HOME',
    'if(-not $sarthiHome){$sarthiHome=Join-Path $env:LOCALAPPDATA "sarthi"}',
    '$candidates=@()',
    'if($explicit){$candidates+=$explicit}',
    '$cmd=Get-Command sarthi.exe -ErrorAction SilentlyContinue',
    'if($cmd){$candidates+=$cmd.Source}',
    '$candidates+=(Join-Path $sarthiHome "sarthi-agent\\venv\\Scripts\\sarthi.exe")',
    '$candidates+=(Join-Path $HOME "sarthi-agent\\.venv\\Scripts\\sarthi.exe")',
    '$sarthi=$candidates|Where-Object{Test-Path -LiteralPath $_ -PathType Leaf}|Select-Object -First 1',
    'if(-not $sarthi){throw "Sarthi is not installed on the remote Windows host."}',
    'if($explicit -and $sarthi -ne $explicit){throw "The configured Sarthi path is not an executable file."}',
    '$python=Join-Path (Split-Path $sarthi) "python.exe"',
    'if(-not (Test-Path -LiteralPath $python -PathType Leaf)){throw "The remote Sarthi Python runtime was not found."}',
    '[ordered]@{os="Windows";arch=$env:PROCESSOR_ARCHITECTURE;sarthiHome=$sarthiHome;sarthiPath=$sarthi;python=$python}|ConvertTo-Json -Compress'
  ].join(';')
...
async function detectRemotePlatform(ssh, explicitSarthiPath = '') {
  try {
  ...
  try {
    return await probeWindowsRemote(ssh, explicitSarthiPath)
  } catch (cause: any) {
...
function helperCommand(runtime, operation, args = []) {
  const argv = [runtime.python, '-m', 'sarthi_cli.windows_ssh_runtime', operation, ...args]
...
function validLock(lock, ownershipId) {
  ...
  return Boolean(
    ...
    typeof lock.sarthiPath === 'string' &&
    typeof lock.sarthiHome === 'string'
  )
}
...
function reusableWindowsLock(lock, state, profile, reuseToken, runtime) {
  return Boolean(
    ...
    lock.sarthiPath === runtime.sarthiPath &&
    lock.sarthiHome === runtime.sarthiHome
  )
}
...
async function connectWindowsRemote(deps) {
  ...
  assertBootstrapNotSuperseded(signal)
  const runtime = await probeWindowsRemote(ssh, remoteSarthiPath)
  const inspection = await helper(ssh, runtime, 'inspect', [runtime.sarthiPath])

  if (!inspection.supported) {
    const error: any = new Error('Update Sarthi on the remote Windows host before connecting with Desktop SSH.')
    error.kind = 'update-required'
    throw error
  }

  runtime.sarthiPath = inspection.path
  const sarthiVersion = inspection.version || ''
  rememberLog(`[ssh-lifecycle] remote platform Windows/${runtime.arch}`)
  rememberLog(`[ssh-lifecycle] located sarthi at ${runtime.sarthiPath}`)
  ...
  try {
    spawned = await helper(
      ...
      'spawn',
      [],
      JSON.stringify({ ownershipId, spawnNonce, profile, sarthiPath: runtime.sarthiPath })
    )
  } catch (error) {
  ...
  try {
    ...
    const baseUrl = `http://127.0.0.1:${localPort}`
    await waitForSarthi(baseUrl, token)
    assertBootstrapNotSuperseded(signal)
...
function buildWindowsInteractiveCommand(remoteCwd = '') {
  ...
  script.push('$host.UI.RawUI.WindowTitle="Sarthi SSH"', 'powershell.exe -NoLogo')
