// Profile-delete routing logic for the `sarthi:api` IPC handler.
//
...
// be rejected: spawning a fresh backend would call ensure_sarthi_home() and
...
/** Parse a `sarthi:api` request into the profile name a DELETE targets. */
 ...
 * still removing the profile and recreate its SARTHI_HOME.
 *
...
/**
 * Route the next `sarthi:api` request away from the primary/window backend
 ...
 * ensure_sarthi_home() recreates the directory the delete just removed.
 */
