...
test('buildNativeAuthorizeUrl omits provider when not given and preserves prefix', () => {
  const url = buildNativeAuthorizeUrl('https://gw.example.com/sarthi', {
    challenge: 'C',
  ...
  assert.equal(parsed.pathname, '/sarthi/auth/native/authorize')
...
test('nativeTokenUrl / nativeRefreshUrl build the right endpoints', () => {
  ...
  assert.equal(nativeRefreshUrl('https://gw.example.com/sarthi'), 'https://gw.example.com/sarthi/auth/native/refresh')
})
