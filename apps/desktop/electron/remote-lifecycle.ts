 ...
 * remote-lifecycle.ts
 *
 * Pure, electron-free remote Sarthi dashboard lifecycle over SSH for Desktop
 ...
 *
 *   - locate the Sarthi install on the remote (login-shell probe),
 ...
 *     `SARTHI_DASHBOARD_READY port=<n>` readiness line,
 ...
 * the real SshConnection, fetch, adoptServedDashboardToken, and waitForSarthi in.
 *
 * The minted SARTHI_DASHBOARD_SESSION_TOKEN is the SPAWN credential. After
...
const READY_RE = /^SARTHI_(?:BACKEND|DASHBOARD)_READY port=(\d+)/m
const REMOTE_LOCK_DIR = '~/.sarthi/desktop-ssh'
...
// Resolve the remote sarthi executable. An EXPLICIT path is honored strictly
...
async function locateSarthi(ssh, remoteSarthiPath) {
  const resolveLauncher = async (candidate: string) => {
    // Return the candidate path directly. The sarthi binary or wrapper script
  ...
  if (remoteSarthiPath) {
    if (await isExecutable(remoteSarthiPath)) {
      return resolveLauncher(remoteSarthiPath)
    }

    const err: any = new Error(
      `The Sarthi path you set is not an executable on the remote host: "${remoteSarthiPath}". ` +
        'Check the path (it must be the full path to the `sarthi` binary on the remote, e.g. ' +
        '~/sarthi-agent/.venv/bin/sarthi), or clear it to auto-detect.'
    )

    err.kind = 'sarthi-not-found'
    throw err
  }
  ...
  try {
    const found = (await ssh.exec(`bash -lc ${shq('command -v sarthi')}`)).trim()
  ...
  candidates.push('~/.local/bin/sarthi')
  candidates.push('/usr/local/bin/sarthi')
  candidates.push('~/.sarthi/sarthi-agent/venv/bin/sarthi')
  ...
  const err: any = new Error(
    'Sarthi is not installed on the remote host (could not find a `sarthi` executable). ' +
      'Install it on the remote with:  curl -fsSL https://hermes-agent.nousresearch.com/install.sh | sh  ' +
      '— or set the Sarthi path explicitly in the SSH connection settings.'
  )
...
// Probe the resolved binary's version string (first line of `<sarthi> --version`,
// e.g. "Sarthi Agent v0.18.2 ..."), or '' on failure. Surfaces WHICH sarthi a
// connection uses, so a stale/unexpected install is visible.
async function probeSarthiVersion(ssh, sarthiPath) {
  try {
    const out = (await ssh.exec(`${expandRemotePath(sarthiPath)} --version 2>&1`)).trim()
...
async function probeRemotePlatform(ssh) {
  ...
  if (!SUPPORTED_REMOTE_OS.has(osName)) {
    const err: any = new Error(
      `Unsupported remote platform "${osName || 'unknown'}". Sarthi Desktop SSH mode supports Linux, macOS, and Windows remote hosts.`
    )
...
// The SARTHI_HOME the remote dashboard will use (explicit env wins, else
// ~/.sarthi). Recorded in the lockfile so a future reuse can tell it's the same
// state store; best-effort.
async function probeRemoteSarthiHome(ssh) {
  try {
    const out = (await ssh.exec('echo "${SARTHI_HOME:-$HOME/.sarthi}"')).trim().split('\n').pop()

    return out || '~/.sarthi'
  } catch (cause) {
    const error: any = new Error('Could not resolve the remote Sarthi home.')
...
async function listRemoteSarthiProfiles(ssh) {
  const home = assertSafeRemoteHome(await probeRemoteSarthiHome(ssh))
  ...
  try {
    ...
    const error: any = new Error('Could not list remote Sarthi profiles.')
    error.kind = 'transient-transport-error'
    error.cause = cause
    throw error
  }
...
function assertSafeRemoteHome(home) {
  ...
  if (!/^(\/|~\/)[A-Za-z0-9._/+-]+$/.test(value) || value.includes('..')) {
    const error: any = new Error('Unsafe remote Sarthi home.')
...
async function readLockfile(ssh, ownershipId) {
  ...
  for (const field of ['profile', 'sarthiPath', 'sarthiHome', 'logPath', 'startedAt']) {
...
async function pidIsOurDashboard(
  ...
  if (!pid || !/^[0-9a-f]{16}$/.test(String(spawnNonce || '')) || !sarthiPath) {
    return false
  }
  ...
  try {
    const script =
      ...
      `pid=${Number(pid)}\n` +
      `expected=os.path.expanduser(${shq(sarthiPath)})\n` +
      // The installer-facing launcher is intentionally preserved for invocation
      // (#74411), but it may `exec python <install-dir>/sarthi`, leaving neither
      // launcher nor SARTHI_HOME-derived entrypoint in argv. The ownership-scoped
      // token path + random nonce + exact profile below are the alternative proof.
      `sarthi_home=os.path.expanduser(${shq(sarthiHome)}) if ${shq(sarthiHome)} else ""\n` +
      'expected_entries={expected}\n' +
      'if sarthi_home:\n' +
      ' expected_entries.add(os.path.join(sarthi_home,"sarthi-agent","venv","bin","sarthi"))\n' +
...
function buildSpawnCommand(sarthiPath, profile, opts: any = {}) {
  const sarthi = expandRemotePath(sarthiPath)
  ...
  const dashCmd =
    ...
    `exec env SARTHI_DESKTOP=1 ${sarthi} ${profileArgs}${subCmd}`
...
async function remoteSupportsSshOwnership(ssh, sarthiPath) {
  const sarthi = expandRemotePath(sarthiPath)

  const out = await ssh.exec(
    `help="$(${sarthi} serve --help 2>&1)"; ` +
...
async function spawnRemoteDashboard(ssh, { sarthiPath, profile, token, ownershipId }) {
  if (!(await remoteSupportsSshOwnership(ssh, sarthiPath))) {
    const err: any = new Error(
      'The remote Sarthi install does not support --ssh-session-token-file and --ssh-owner-nonce. ' +
        'Update Sarthi on the remote host to continue using Desktop SSH mode.'
    )
  ...
  try {
    out = await ssh.exec(buildSpawnCommand(sarthiPath, profile, { spawnNonce, tokenFilePath, logPath }))
  } catch (error) {
    try {
      await ssh.exec(`rm -f ${expandRemotePath(tokenFilePath)}`)
    } catch {
 ...
 * opened SshConnection, forward/pickLocalPort/waitForSarthi, a token-gated
...
async function connect(deps) {
  ...
  const sarthiPath = await locateSarthi(ssh, remoteSarthiPath)
  log(`located sarthi at ${sarthiPath}`)
  const sarthiVersion = await probeSarthiVersion(ssh, sarthiPath)

  if (sarthiVersion) {
    log(`remote sarthi version: ${sarthiVersion}`)
  }

  const reuseToken = deps.reuseToken || ''
  const sarthiHome = await probeRemoteSarthiHome(ssh)
  ...
  if (lock) {
    ...
    const reusable =
      ...
      lock.sarthiPath === sarthiPath &&
      lock.sarthiHome === sarthiHome
  ...
  try {
    ...
    assertBootstrapNotSuperseded(signal)
    const baseUrl = `http://127.0.0.1:${localPort}`
    await waitForSarthi(baseUrl, spawnToken)
    assertBootstrapNotSuperseded(signal)
