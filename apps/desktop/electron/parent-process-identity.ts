export type ParentWatchdogEnv = {
  SARTHI_PARENT_PID: string
  SARTHI_PARENT_START_MARKER?: string
  SARTHI_PARENT_NONCE?: string
  /** Spawn tag consumed by sarthi_cli.process_identity (`v1:<install>:<purpose>:<spawner_pid>:<spawner_create_s>`).
...
export function parentWatchdogEnv(pid: number, startMarker: string | null, nonce: string): ParentWatchdogEnv {
  ...
  const env: ParentWatchdogEnv = { SARTHI_PARENT_PID: String(pid) }
  env.SARTHI_SPAWN = spawnTag(pid, startMarker)
  ...
  env.SARTHI_PARENT_START_MARKER = startMarker
  env.SARTHI_PARENT_NONCE = nonce

  return env
}

/** Build the `SARTHI_SPAWN` tag mirrored by sarthi_cli/process_identity.py.
