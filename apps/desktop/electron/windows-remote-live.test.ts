...
//   SARTHI_WIN_SSH_HOST   ssh alias/host of the Windows box
//   SARTHI_WIN_SSH_USER   remote user
//   SARTHI_WIN_SSH_SARTHI absolute path to the remote sarthi.exe under test
const liveHost = process.env.SARTHI_WIN_SSH_HOST || ''
const liveUser = process.env.SARTHI_WIN_SSH_USER || ''
const configuredSarthi = process.env.SARTHI_WIN_SSH_SARTHI || ''
const ownershipId = '89abcdef0123456789abcdef01234567'

function fetchJson(url, token, path) {
  return fetch(`${url}${path}`, { headers: { 'X-Sarthi-Session-Token': token } }).then(async response => {
    if (!response.ok) {
...
test.skipIf(!liveHost || !liveUser || !configuredSarthi)(
  ...
  async () => {
    ...
    try {
      ...
      const runtimeScript = `& '${configuredSarthi.replace('sarthi.exe', 'python.exe')}' -m sarthi_cli.windows_ssh_runtime read-lock '${ownershipId}'`
      ...
      if (lock) {
        const python = configuredSarthi.replace('sarthi.exe', 'python.exe')
        const terminate = `& '${python}' -m sarthi_cli.windows_ssh_runtime terminate '${lock.pid}' '${lock.creationTimeNs}' '${lock.sarthiPath}' '${lock.spawnNonce}'`
        await ssh.exec(`powershell.exe -NoProfile -NonInteractive -Command "${terminate}"`)
        await ssh.exec(
          `powershell.exe -NoProfile -NonInteractive -Command "& '${python}' -m sarthi_cli.windows_ssh_runtime remove-lock '${ownershipId}'"`
        )
        await ssh.exec(
          `powershell.exe -NoProfile -NonInteractive -Command "& '${python}' -m sarthi_cli.windows_ssh_runtime remove-log '${ownershipId}' '${lock.spawnNonce}'"`
        )
      }
