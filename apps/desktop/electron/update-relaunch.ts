 ...
 * Background
 * ----------
 * After `sarthi update` + `sarthi desktop --build-only`, the freshly-rebuilt
...
// backend/profile/root it talks to). Anything SARTHI_DESKTOP_* is preserved
// plus SARTHI_HOME. We snapshot the values, not the live env, so the new
...
const PRESERVED_ENV_KEYS = ['SARTHI_HOME', 'ELECTRON_DISABLE_SANDBOX']
const PRESERVED_ENV_PREFIXES = ['SARTHI_DESKTOP_']
