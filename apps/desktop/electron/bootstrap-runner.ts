 ...
 * bootstrap-runner.ts
 *
 * Drives apps/desktop's first-launch install of Sarthi Agent by spawning
...
function bootstrapCacheDir(sarthiHome) {
  return path.join(sarthiHome, 'bootstrap-cache')
}

// The install.sh / install.ps1 that ships inside the already-installed agent
// checkout under ~/.sarthi/sarthi-agent. Used as a last-resort fallback when
// the pinned commit can't be fetched from GitHub (e.g. a locally-built desktop
// app stamped to an unpushed HEAD).
function installedAgentInstallScript(sarthiHome) {
  if (!sarthiHome) {
    return null
  }

  const candidate = path.join(sarthiHome, 'sarthi-agent', 'scripts', installScriptName())
...
function cachedScriptPath(sarthiHome, commit) {
  return path.join(bootstrapCacheDir(sarthiHome), `install-${commit}.${process.platform === 'win32' ? 'ps1' : 'sh'}`)
}
...
function downloadInstallScript(commit, destPath) {
  ...
  const url = `https://raw.githubusercontent.com/jitendra-singh-thakur/sarthi-agent/${commit}/scripts/${scriptName}`
...
async function resolveInstallScript({
  ...
  const cached = cachedScriptPath(sarthiHome, installStamp.commit)
  ...
  try {
    ...
    const installed = installedAgentInstallScript(sarthiHome)
...
function spawnPowerShell(scriptPath, args, { emit, stageName, abortSignal, sarthiHome }: any = {}) {
  return new Promise<any>((resolve, reject) => {
    ...
    const child = spawn(
      ...
      hiddenWindowsChildOptions({
        ...
        env: {
          ...process.env,
          // Pass SARTHI_HOME through so install.ps1 respects the caller's
          // choice rather than re-computing the default.
          SARTHI_HOME: sarthiHome || process.env.SARTHI_HOME || ''
        }
      })
    )
...
function spawnBash(scriptPath, args, { emit, stageName, abortSignal, sarthiHome }: any = {}) {
  return new Promise<any>((resolve, reject) => {
...
function buildPosixPinArgs({ installStamp, activeRoot, sarthiHome, pinCommit = true }) {
  const args = ['--dir', activeRoot, '--sarthi-home', sarthiHome]
...
async function fetchManifest({ scriptPath, installerKind, emit, sarthiHome, activeRoot, installStamp, pinCommit }) {
  const isPosix = installerKind === 'posix'

  const args = isPosix
    ? ['--manifest', ...buildPosixPinArgs({ installStamp, activeRoot, sarthiHome, pinCommit })]
...
async function runStage({
  ...
  const args = isPosix
    ? [
        ...
        '--non-interactive',
        '--json',
        ...buildPosixPinArgs({ installStamp, activeRoot, sarthiHome, pinCommit })
      ]
...
async function runBootstrap(opts) {
  ...
  const runLog = openRunLog(logRoot || path.join(sarthiHome, 'logs'))
  ...
  try {
    ...
    const scriptInfo = await resolveInstallScript({ installStamp, sourceRepoRoot, sarthiHome, emit })
