 ...
 * runtime, remote gateways, Sarthi Cloud instances, SSH hosts) that are all
 ...
 * apps/shared/src/backend-scope.ts (`@sarthi/shared`) — tsconfig project
...
/** Turn `ls ~/.sarthi/profiles` output into roster names. Always includes
...
/**
 * Whether "Update all instances" may drive this connection. Sarthi Cloud
 * instances are platform-managed — we never run `sarthi update` against them.
...
export function normalizeConnectionInput(input: ConnectionInput, registry: ConnectionRegistry): RegistryConnection {
  ...
  if (kind === 'ssh') {
    const ssh = normalizeSshConfig({
      ...
      remoteSarthiPath: input.remoteSarthiPath,
  ...
  if (kind === 'remote' || kind === 'cloud') {
    ...
    // transport-/Sarthi-managed names; an empty result stores nothing.
 ...
 * `remoteSarthiPath`/`remoteProfile`. Only fields the payload explicitly
...
export function migrateV1ToRegistry(v1: unknown): ConnectionRegistry {
  ...
  const addRemoteLike = (block: Record<string, any>, kind: 'cloud' | 'remote'): null | RegistryConnection => {
    ...
    const label = uniqueLabel(
      hostLabelFromBaseUrl(url) || (kind === 'cloud' ? 'Sarthi Cloud' : 'Remote gateway'),
