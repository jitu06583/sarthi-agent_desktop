 ...
 *
 * The staged Tauri `sarthi-setup.exe` has no self-update path, so every
 ...
 * checkout instead: every `sarthi update` refreshes the code that drives the
 ...
 * and QUITS, the script waits it out, runs `sarthi update`, swaps/relaunches
 * the app, and writes .sarthi-update-result.json. With the app gone before
 * the update starts, the SARTHI_DESKTOP_CHILD_PID reaper-exclusion dance is
 ...
 * `sarthi update` card — old checkouts pull the script on their next update).
 */
 ...
 *
 * The Tauri installer self-copies into SARTHI_HOME on *every* platform
 * (`sarthi-setup.exe` on Windows, `sarthi-setup` elsewhere — see
 * apps/bootstrap-installer `paths::installer_dest` and
 * `bootstrap::copy_self_to_sarthi_home`), so finding that binary on macOS or
 ...
 * than the hand-off protocol holds the update marker, spawns `sarthi update`,
...
export function resolveStagedUpdaterBinary(
  ...
  const candidate = path.join(sarthiHome, 'sarthi-setup.exe')
 ...
 *
 * `copy_self_to_sarthi_home` deliberately no-ops during `--update`
 ...
 * "Another Sarthi update is already running (PID <itself>, started 1s ago)" —
 * the observed infinite "Install didn't finish" loop. Skipping the pre-write
 * for those binaries lets them acquire cleanly and run `sarthi update`, which
 ...
 * the installer is written to SARTHI_HOME at install/repair time, making mtime
