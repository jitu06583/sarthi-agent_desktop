...
function encryptDesktopSecret(value, safeStorageApi) {
  ...
  if (!encryptionAvailable) {
    throw new Error(
      'Secure token storage is unavailable, so Sarthi Desktop cannot save remote gateway tokens. ' +
        'Set SARTHI_DESKTOP_REMOTE_URL and SARTHI_DESKTOP_REMOTE_TOKEN in your environment, or enable OS keychain access and try again.'
    )
  }
