...
function mkTmpHome() {
  return fs.mkdtempSync(path.join(os.tmpdir(), 'sarthi-bootstrap-test-'))
}
...
test('runBootstrap bails immediately when the signal is already aborted', async () => {
  ...
  const result = await runBootstrap({
    installStamp: null,
    activeRoot: '/tmp/sarthi-runner-test',
    sourceRepoRoot: null,
    sarthiHome: '/tmp/sarthi-runner-test',
    logRoot: '/tmp/sarthi-runner-test',
...
test('installedAgentInstallScript resolves the installer in the agent checkout', () => {
  ...
  try {
    ...
    const scriptsDir = path.join(home, 'sarthi-agent', 'scripts')
...
test('existing checkout detection requires git metadata', () => {
  ...
  try {
    const activeRoot = path.join(home, 'sarthi-agent')
...
test('fresh bootstrap args include the packaged commit pin', () => {
  ...
  assert.deepEqual(
    buildPosixPinArgs({
      installStamp,
      activeRoot: '/tmp/sarthi-agent',
      sarthiHome: '/tmp/sarthi'
    }),
    ['--dir', '/tmp/sarthi-agent', '--sarthi-home', '/tmp/sarthi', '--branch', 'main', '--commit', installStamp.commit]
  )
})
...
test('existing-checkout bootstrap args keep branch but skip the packaged commit pin', () => {
  ...
  assert.deepEqual(
    ...
    ['--dir', '/tmp/sarthi-agent', '--sarthi-home', '/tmp/sarthi', '--branch', 'main']
  )
})
...
test('fallback install stamps use an unpinned branch ref', () => {
  ...
  assert.deepEqual(
    buildPosixPinArgs({
      installStamp: stamp,
      activeRoot: '/tmp/sarthi',
      sarthiHome: '/tmp/home'
    }),
    ['--dir', '/tmp/sarthi', '--sarthi-home', '/tmp/home', '--branch', 'main']
  )
})
