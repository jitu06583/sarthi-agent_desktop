 ...
 *   1. SARTHI_GIT_BASH_PATH env var override
 *   2. PortableGit under %LOCALAPPDATA%\sarthi\git\ (install.ps1)
...
export function findGitBash(opts: GitBashOptions): string | null {
  ...
  // Respect SARTHI_GIT_BASH_PATH if set (mirrors tools/environments/local.py:_find_bash).
  const gitBashPath = env.SARTHI_GIT_BASH_PATH
  ...
  if (localAppData) {
    candidates.push(joinWin(localAppData, 'sarthi', 'git', 'bin', 'bash.exe'))
    candidates.push(joinWin(localAppData, 'sarthi', 'git', 'usr', 'bin', 'bash.exe'))
  }
