...
function makeRepo() {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'sarthi-desktop-git-status-'))

  tempDirs.push(dir)
  execFileSync('git', ['init', '-q'], { cwd: dir })
  execFileSync('git', ['config', 'user.email', 'sarthi-test@example.com'], { cwd: dir })
  execFileSync('git', ['config', 'user.name', 'Sarthi Test'], { cwd: dir })
