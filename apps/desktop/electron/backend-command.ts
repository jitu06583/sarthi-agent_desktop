// Backend subcommand routing for the desktop-managed Sarthi process.
//
// The desktop app launches its own headless backend via `sarthi serve` — it
...
// app hasn't updated yet, or an older `sarthi` resolved from PATH) only knows
 ...
 * @param {string} [profile] optional Sarthi profile to pin via `--profile`.
 */
 ...
 * `-m sarthi_cli.main` and any `--profile <name>`). Returns a copy; if there is
...
/**
 * True when a runtime's `sarthi_cli/subcommands/dashboard.py` source registers
