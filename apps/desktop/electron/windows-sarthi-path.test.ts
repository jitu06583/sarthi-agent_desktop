// Unit tests for the pure Windows `sarthi` resolution helpers extracted from
// main.ts's findOnPath(), handOffWindowsBootstrapRecovery(), and
// unwrapWindowsVenvSarthiCommand(). These pin the two Windows resolution bugs
// that caused desktop reinstall loops:
//   1. buildPathExtCandidates() — PATHEXT extensions must be tried BEFORE the
//      empty extension, or an extensionless Git-Bash `sarthi` shim shadows
//      the real sarthi.cmd/sarthi.exe.
...
//   3. resolveVenvSarthiCommand() — must probe the venv python via
//      canImportSarthiCli() before trusting it, or a broken venv gets
...
test('chooseUpdaterArgs: gentle --update when both updater runtime files exist', () => {
  assert.deepEqual(chooseUpdaterArgs({ hasBootstrapMarker: true, hasVenvSarthi: true, hasVenvPython: true }, 'main'), [
    '--update',
    '--branch',
    'main'
  ])
...
test('chooseUpdaterArgs: marker-only install uses --repair when the venv is gone', () => {
  assert.deepEqual(
    chooseUpdaterArgs({ hasBootstrapMarker: true, hasVenvSarthi: false, hasVenvPython: false }, 'main'),
    ['--repair', '--branch', 'main']
  )
})

test('chooseUpdaterArgs: partial updater runtimes use --repair', () => {
  assert.deepEqual(chooseUpdaterArgs({ hasBootstrapMarker: true, hasVenvSarthi: false, hasVenvPython: true }, 'main'), [
    '--repair',
    '--branch',
    'main'
  ])
  assert.deepEqual(chooseUpdaterArgs({ hasBootstrapMarker: true, hasVenvSarthi: true, hasVenvPython: false }, 'main'), [
    '--repair',
    '--branch',
    'main'
  ])
...
test('chooseUpdaterArgs: passes the branch through unchanged in both modes', () => {
  assert.deepEqual(
    chooseUpdaterArgs({ hasBootstrapMarker: false, hasVenvSarthi: true, hasVenvPython: true }, 'release/1.2'),
    ['--update', '--branch', 'release/1.2']
  )
  assert.deepEqual(
    chooseUpdaterArgs({ hasBootstrapMarker: false, hasVenvSarthi: false, hasVenvPython: false }, 'release/1.2'),
    ['--repair', '--branch', 'release/1.2']
  )
})

function makeDeps(overrides: Partial<Parameters<typeof resolveVenvSarthiCommand>[2]> = {}) {
  return {
    isWindows: true,
    isCommandScript: () => false,
    fileExists: () => true,
    directoryExists: () => false,
    canImportSarthiCli: () => true,
    getVenvPython: (venvRoot: string) => `${venvRoot}/Scripts/python.exe`,
    getVenvSitePackagesEntries: () => [],
    buildDesktopBackendEnv: () => ({ FAKE_ENV: '1' }),
    sarthiHome: '/fake/sarthi-home',
...
test('resolveVenvSarthiCommand: returns null off Windows', () => {
  const deps = makeDeps({ isWindows: false })

  assert.equal(resolveVenvSarthiCommand('/root/venv/Scripts/sarthi.exe', [], deps), null)
})

test('resolveVenvSarthiCommand: returns null for a .cmd/.bat script command', () => {
  const deps = makeDeps({ isCommandScript: () => true })

  assert.equal(resolveVenvSarthiCommand('/root/venv/Scripts/sarthi.cmd', [], deps), null)
})

test('resolveVenvSarthiCommand: returns null when the basename is not sarthi/sarthi.exe', () => {
  const deps = makeDeps()

  assert.equal(resolveVenvSarthiCommand('/root/venv/Scripts/python.exe', [], deps), null)
})

test('resolveVenvSarthiCommand: returns null when the parent dir is not Scripts', () => {
  const deps = makeDeps()

  assert.equal(resolveVenvSarthiCommand('/root/venv/bin/sarthi.exe', [], deps), null)
})

test('resolveVenvSarthiCommand: returns null when the venv python does not exist on disk', () => {
  const deps = makeDeps({ fileExists: () => false })

  assert.equal(resolveVenvSarthiCommand('/root/venv/Scripts/sarthi.exe', [], deps), null)
})

test('resolveVenvSarthiCommand: probes the venv python before trusting it (returns null on failed probe)', () => {
  let probed = false

  const deps = makeDeps({
    canImportSarthiCli: (python: string) => {
      probed = true
  ...
  const result = resolveVenvSarthiCommand('/root/venv/Scripts/sarthi.exe', ['serve'], deps)
...
test('resolveVenvSarthiCommand: returns the resolved python backend descriptor when the probe passes', () => {
  const deps = makeDeps()
  const result = resolveVenvSarthiCommand('/root/venv/Scripts/sarthi.exe', ['serve', '--port', '0'], deps)
  ...
  assert.deepEqual(result.args, ['-m', 'sarthi_cli.main', 'serve', '--port', '0'])
...
test('resolveVenvSarthiCommand: is case-insensitive on sarthi.exe and the Scripts dir name', () => {
  const deps = makeDeps()

  assert.ok(resolveVenvSarthiCommand('/root/venv/Scripts/SARTHI.EXE', [], deps))
  assert.ok(resolveVenvSarthiCommand('/root/venv/SCRIPTS/sarthi.exe', [], deps))
})
