...
export interface SarthiReadyOptions {
   ...
   * two very different meanings of a 401 (see `waitForSarthiReady`).
   */
 ...
 *  - readiness-probe exhaustion in waitForSarthiReady().
 *
...
/**
 * True when the backend URL points at a Nous-managed Sarthi Cloud instance
...
export async function waitForSarthiReady(baseUrl: string, options: SarthiReadyOptions): Promise<void> {
  ...
  throw new Error(`Sarthi backend did not become ready: ${detail}`)
}
