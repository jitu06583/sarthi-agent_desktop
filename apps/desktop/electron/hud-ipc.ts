...
export function registerHudIpc({
  ...
  ipcMain.handle('sarthi:hud:open', async (_event, request) => {
    openHudWindow(
  ...
  ipcMain.handle('sarthi:hud:frost', (_event, showing) => {
  ...
  // reaches the bar.
  ipcMain.on('sarthi:hud:ignore-mouse', (_event, ignore) => {
    const hudWindow = getHudWindow()
  ...
  ipcMain.on('sarthi:hud:move-by', (event, delta) => {
    const hudWindow = getHudWindow()
  ...
  ipcMain.on('sarthi:hud:set-bounds', (event, bounds) => {
    const hudWindow = getHudWindow()
  ...
  ipcMain.on('sarthi:hud:session', (event, sessionId) => {
    const hudWindow = getHudWindow()
  ...
  ipcMain.handle('sarthi:hud:close', async () => {
    closeHudWindow()
