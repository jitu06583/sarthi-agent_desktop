...
const _REDACTIONS: Array<[RegExp, string]> = [
  [/(SARTHI_DASHBOARD_SESSION_TOKEN=)(\S+)/g, '$1<redacted>'],
  [/(X-Sarthi-Session-Token["']?\s*[:=]\s*["']?)([^\s"'&]+)/gi, '$1<redacted>'],
...
// root under a short per-user base (`~/.sarthi/desktop-ssh`) so even worst case
...
function defaultControlDir() {
  ...
  if (process.platform === 'win32') {
    return path.join(os.tmpdir(), 'sarthi-desktop-ssh')
  }

  return path.join(os.homedir(), '.sarthi', 'desktop-ssh')
}
