...
test('redactSecrets scrubs the spawn-time session token env var', () => {
  const line = 'setsid env SARTHI_DASHBOARD_SESSION_TOKEN=abc123deadbeef SARTHI_DESKTOP=1 sarthi dashboard'
  const out = redactSecrets(line)
  assert.ok(!out.includes('abc123deadbeef'))
  assert.match(out, /SARTHI_DASHBOARD_SESSION_TOKEN=<redacted>/)
  // non-secret env vars are preserved
  assert.match(out, /SARTHI_DESKTOP=1/)
})
...
test('redactSecrets scrubs Authorization and X-Sarthi-Session-Token headers', () => {
  ...
  assert.match(redactSecrets('X-Sarthi-Session-Token: hdr_888'), /X-Sarthi-Session-Token: ?<redacted>/)
  assert.ok(!redactSecrets('X-Sarthi-Session-Token: hdr_888').includes('hdr_888'))
})
...
test('controlSocketPath default base stays under sun_path even with the temp-listener suffix', () => {
  ...
  const p = controlSocketPath('sarthi', 'remote-build-server', 22) // no baseDir → default
...
test('buildExecArgs ends with host then the remote command', () => {
  ...
  const args = buildExecArgs(conn, 'command -v sarthi', 15000)
  assert.equal(args[args.length - 1], 'command -v sarthi')
...
test('close() removes the control socket when -O exit fails', async () => {
  const dir = path.join(os.tmpdir(), `sarthi-ssh-close-${process.pid}-${Date.now()}`)
...
test('open() creates the control-socket directory if it does not exist', async () => {
  const dir = path.join(os.tmpdir(), `sarthi-ssh-test-${process.pid}-${Date.now()}`)
