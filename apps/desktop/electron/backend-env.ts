 ...
 * `iter_sarthi_node_dirs()` in sarthi_constants.py, which the Electron main
...
function sarthiManagedNodePathEntries(
  ...
  const root = pathModule.join(sarthiHome, 'node')
...
function buildDesktopBackendPath({
  ...
  const sarthiNodeDirs = sarthiManagedNodePathEntries(sarthiHome, { platform, pathModule })
  const venvBin = venvRoot ? pathModule.join(venvRoot, platform === 'win32' ? 'Scripts' : 'bin') : null
  const saneEntries = platform === 'win32' ? [] : POSIX_SANE_PATH_ENTRIES

  return appendUniquePathEntries([sarthiNodeDirs, venvBin, currentPath, saneEntries], { delimiter })
}

function normalizeSarthiHomeRoot(sarthiHome, { pathModule = pathModuleForPlatform(process.platform) }: any = {}) {
  if (!sarthiHome) {
    return sarthiHome
  }

  const resolved = pathModule.resolve(String(sarthiHome))
...
function buildDesktopBackendEnv({
  ...
  return {
    ...
    // cp1252, ...). sarthi_bootstrap sets this inside the child too, but only
