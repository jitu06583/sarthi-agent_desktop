...
export function markerPath(sarthiHome) {
  return path.join(sarthiHome, '.sarthi-update-in-progress')
}
...
export function readLiveUpdateMarker(
  ...
  const file = markerPath(sarthiHome)
  let raw
 ...
 *
 * The Tauri-based sarthi-setup.exe takes several seconds to initialise its
...
export function writeUpdateMarker(
  ...
  const file = markerPath(sarthiHome)
  const nowMs = now()
  const owner = readLiveUpdateMarker(sarthiHome, { kill, maxAgeMs, now: () => nowMs })
 ...
 * Sarthi to exit…") clobbers that still-running updater's claim: the
...
export function updateHandoffConflict(
  ...
  const owner = readLiveUpdateMarker(sarthiHome, opts)
