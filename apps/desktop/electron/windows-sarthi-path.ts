/**
 * windows-sarthi-path.ts
 *
 * Pure, dependency-injected pieces of Windows `sarthi` resolution pulled out
 * of main.ts's findOnPath(), handOffWindowsBootstrapRecovery(), and
 * unwrapWindowsVenvSarthiCommand(). Each of the three functions here pins one
 ...
 *      FIRST, so an extensionless Git-Bash `sarthi` shim shadowed the real
 *      sarthi.cmd/sarthi.exe; the shim then failed the --version probe and
 ...
 *      and Sarthi launcher. Marker-only or partial runtimes must use --repair;
 *      only a runnable pair can use --update.
 *   3. resolveVenvSarthiCommand() — unwrapWindowsVenvSarthiCommand() returned
 ...
 * extensionless file (e.g. a Git-Bash shell-script shim named `sarthi`) must
 * not shadow `sarthi.cmd`/`sarthi.exe`. The empty entry is kept LAST so
 ...
 * venv Python interpreter and the Sarthi launcher that drives `sarthi update`.
...
export interface ResolveVenvSarthiCommandDeps {
  isWindows: boolean
  isCommandScript: (command: string) => boolean
  fileExists: (filePath: string) => boolean
  directoryExists: (filePath: string) => boolean
  canImportSarthiCli: (python: string, opts?: { env?: Record<string, string> }) => boolean
...
/**
 * If `command` is a Windows venv `sarthi`/`sarthi.exe` console-script shim
 * (i.e. `<venvRoot>/Scripts/sarthi(.exe)`), resolve it to the underlying
 * venv python invoked as `python -m sarthi_cli.main <backendArgs>` — but
 * ONLY after smoke-testing that interpreter with canImportSarthiCli(). A
 * venv whose update died mid-`pip install` still has python.exe + sarthi.exe
 ...
 *
 * Returns null when `command` is not a venv sarthi shim, the underlying
 * python doesn't exist, or the import probe fails. Otherwise returns the
 * resolved backend descriptor.
 */
export function resolveVenvSarthiCommand(
  command: string,
  backendArgs: string[],
  deps: ResolveVenvSarthiCommandDeps
): {
  label: string
  command: string
  args: string[]
  ...
  if (!/^sarthi(?:\.exe)?$/i.test(basename(resolved))) {
    return null
  }
  ...
  if (
    ...
    rememberLog?.(
      `Ignoring venv Sarthi at ${python}: runtime import probe failed (broken/partial venv); falling through to bootstrap.`
    )
  ...
  return {
    label: `existing Sarthi Python at ${python}`,
    command: python,
