...
const SCAN_MODULE = 'sarthi_cli._scan_venv_blockers'
 ...
 * preview servers. Unknown Python/Sarthi processes are deliberately ignored.
 */
export async function stopSafeVenvBlockers(
  ...
  for (const process of result.processes) {
    ...
    try {
      await execFn(
        pythonPath,
        ['-m', 'sarthi_cli._scan_venv_blockers', '--terminate-safe', String(process.pid), String(process.createTime)],
...
export function formatBlockerMessage(result: VenvBlockerScanResult): string {
  const lines = [
    'Update aborted: another Sarthi process is using this installation.',
    '',
...
export function formatProbeFailedMessage(): string {
  return (
    'Update aborted: Desktop could not verify the Sarthi installation is free.\n' +
    '\n' +
    'Close other Sarthi windows and terminals, then retry.  If the problem\n' +
    'persists, run `sarthi update` in a terminal for detailed diagnostics.'
  )
}
