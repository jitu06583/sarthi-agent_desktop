// Launching the Sarthi TUI in the user's OWN terminal emulator.
//
...
// `sarthi --tui --resume <id>` there. Two problems have to be solved for that
// to work anywhere:
//
//  1. WHAT to run. The desktop's Sarthi runtime is often a venv Python invoked
//     as `python -m sarthi_cli.main`, not a `sarthi` on PATH — so the command
 ...
 * headless child) is the wrong answer for an interactive terminal. The Sarthi
