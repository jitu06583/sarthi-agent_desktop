...
How to build Sarthi Desktop well. This is a judgment guide, not an inventory —
...
- The **backend** is authoritative for anything another Sarthi surface can also
...
- A **runtime home change** (switching the underlying `SARTHI_HOME` profile) is
...
its shape. "Plugin" means several unrelated things across Sarthi — do not assume
