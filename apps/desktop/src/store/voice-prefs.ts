...
import { getSarthiConfigRecord, saveSarthiConfig } from '@/sarthi'
...
export async function setAutoSpeakReplies(enabled: boolean): Promise<void> {
  ...
  try {
    const record = await getSarthiConfigRecord()
    const voice = record.voice && typeof record.voice === 'object' ? (record.voice as Record<string, unknown>) : {}

    await saveSarthiConfig({ ...record, voice: { ...voice, auto_tts: enabled } })
  } catch (error) {
