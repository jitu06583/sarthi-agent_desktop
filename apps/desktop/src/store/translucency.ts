 ...
 * `@sarthi/shared/translucency`, which owns the mapping both this store and the
 * main process read.
 *
 ...
 * material underneath the web contents to read (see the `[data-sarthi-glass]`
 * block in styles.css).
 */
...
} from '@sarthi/shared/translucency'
 ...
 * sets `sarthiDesktop.glassSupported` from `os.release()` so Win10 cannot
...
export const GLASS_SUPPORTED =
  typeof window !== 'undefined' && typeof window.sarthiDesktop?.glassSupported === 'boolean'
    ? window.sarthiDesktop.glassSupported
...
export const TRANSLUCENCY_SUPPORTED =
  typeof window !== 'undefined' && typeof window.sarthiDesktop?.translucencySupported === 'boolean'
    ? window.sarthiDesktop.translucencySupported
    : isMacPlatform() || isWindowsPlatform()
...
/* Peek: while the user is actively adjusting translucency from Settings, the
   ...
   [data-sarthi-translucency-peek] rules in styles.css). A counter rather
   than a boolean: a held slider drag and a timed pulse from a picker click
   can overlap. */
const PEEK_ATTR = 'data-sarthi-translucency-peek'
...
const applyGlassSurfaces = ({ intensity, mode, scope }: TranslucencyState): void => {
  ...
  root.toggleAttribute('data-sarthi-glass-on', glassLive)
  root.toggleAttribute('data-sarthi-glass', glassOn)
  root.toggleAttribute('data-sarthi-clear', clearOn)
  ...
  if (glassOn) {
    root.setAttribute('data-sarthi-glass-scope', scope)
  } else {
    root.removeAttribute('data-sarthi-glass-scope')
  }
...
if (typeof window !== 'undefined') {
  ...
  $translucency.subscribe(state => {
    ...
    window.sarthiDesktop?.setTranslucency?.(state)
  })
