import type { ConnectionState } from '@sarthi/shared'
...
import type { SessionInfo, UsageStats } from '@/types/sarthi'
...
const COMPOSER_MODEL_KEY = 'sarthi.desktop.composer.model'
const COMPOSER_PROVIDER_KEY = 'sarthi.desktop.composer.provider'
const COMPOSER_MODEL_SOURCE_KEY = 'sarthi.desktop.composer.model-source'
const COMPOSER_EFFORT_KEY = 'sarthi.desktop.composer.reasoning-effort'
const COMPOSER_FAST_KEY = 'sarthi.desktop.composer.fast'
...
const LAST_SESSION_KEY = 'sarthi.desktop.lastSessionId'
const LAST_ROUTE_KEY = 'sarthi.desktop.lastRoute'
...
function workspaceCwdKey(connection: SarthiConnection | null = $connection.get()): string {
...
export async function syncConfiguredDefaultProjectDir(): Promise<string> {
  const settings = window.sarthiDesktop?.settings?.getDefaultProjectDir
...
export async function ensureDefaultWorkspaceCwd(): Promise<void> {
  const sanitize = window.sarthiDesktop?.sanitizeWorkspaceCwd
...
export const $connection = atom<SarthiConnection | null>(null)
...
export const setConnection = (next: Updater<SarthiConnection | null>) => {
...
// until config loads, and re-seeded on every profile switch by useSarthiConfig.
