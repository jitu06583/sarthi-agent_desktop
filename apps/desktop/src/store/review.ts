...
import type { SarthiReviewFile, SarthiReviewShipInfo } from '@/global'
...
//
// Scope is always "uncommitted" — Sarthi' flow is agent edits you review BEFORE
...
const OPEN_KEY = 'sarthi.desktop.reviewOpen'
const COMMIT_DEFAULT_KEY = 'sarthi.desktop.reviewCommitDefault'
const TREE_MODE_KEY = 'sarthi.desktop.reviewTreeMode'
const SELECTED_KEY = 'sarthi.desktop.reviewSelectedPath'
...
export const $reviewFiles = atom<SarthiReviewFile[]>([])
...
export const $reviewShipInfo = atom<SarthiReviewShipInfo>({ ghReady: false, pr: null })
...
type ReviewBridge = NonNullable<NonNullable<NonNullable<Window['sarthiDesktop']>['git']>['review']>
...
export async function selectReviewFile(file: SarthiReviewFile): Promise<void> {
...
function matchReviewFile(files: readonly SarthiReviewFile[], path: string): SarthiReviewFile | undefined {
...
export async function createOrOpenPr(): Promise<void> {
  ...
  if (existing?.url) {
    void window.sarthiDesktop?.openExternal?.(existing.url)
  ...
  await runShip(async () => {
    ...
    if (url) {
      void window.sarthiDesktop?.openExternal?.(url)
    }
