 ...
 * composer with the reply scrolling above it, so Sarthi can be driven while
...
export const canUseHud = (): boolean =>
  typeof window !== 'undefined' && typeof window.sarthiDesktop?.hud?.open === 'function'

export function openHud(sessionId?: null | string): void {
  const api = window.sarthiDesktop?.hud
...
export const reportHudSession = (sessionId: null | string): void => window.sarthiDesktop?.hud?.setSession?.(sessionId)
...
export function watchHudState(onClosed?: (sessionId: null | string) => void): () => void {
  const off = window.sarthiDesktop?.hud?.onChanged?.(({ open, sessionId }) => {
    $hudActive.set(open)
