...
import type { SarthiBranchPullRequest } from '@/global'
import { scanSessionPullRequests, type SessionInfo } from '@/sarthi'
...
export const $pullRequestsByBranch = atom<Record<string, SarthiBranchPullRequest>>({})
...
export const $prBranchBySession = persistentAtom<Record<string, string>>(
  'sarthi.desktop.prBranchBySession',
  {},
  Codecs.stringRecord
)
...
const $prScannedSessions = persistentAtom<string[]>('sarthi.desktop.prScannedSessions', [], Codecs.stringArray)
...
export function pullRequestBucket(pr: SarthiBranchPullRequest | undefined): PullRequestBucket {
  if (!pr) {
    return 'none'
  }
