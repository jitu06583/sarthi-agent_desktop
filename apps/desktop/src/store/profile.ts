...
import { getProfiles, sarthiApi, setApiRequestProfile, STARTUP_REQUEST_TIMEOUT_MS } from '@/sarthi'
...
import type { ProfileInfo } from '@/types/sarthi'
...
// /api/profiles/active `current`). "default" is the root ~/.sarthi. This is the
...
const PROFILE_ORDER_STORAGE_KEY = 'sarthi.desktop.profileOrder'
...
const PROFILE_COLORS_STORAGE_KEY = 'sarthi.desktop.profileColors'
...
export async function refreshActiveProfile(): Promise<void> {
  ...
  try {
    const res = await sarthiApi<ActiveProfileResponse>({
...
// Persist the choice and relaunch the backend under the new SARTHI_HOME. The
...
export async function switchProfile(name: string): Promise<void> {
  ...
  await window.sarthiDesktop.profile.set(name)
}
...
async function resolveConnectionForProfile(profile: string): Promise<SarthiConnection | null> {
  const getConnection = window.sarthiDesktop?.getConnection
...
// previous descriptor.
async function resolveConnectionForAgent(connectionId: string, profile: string): Promise<SarthiConnection | null> {
  const getConnectionFor = window.sarthiDesktop?.getConnectionFor
...
const SHOW_ALL_PROFILES_STORAGE_KEY = 'sarthi.desktop.showAllProfiles'
...
// Switch to the default (root ~/.sarthi) profile — bound to ⌘1.
...
export function touchActiveGatewayBackend(): void {
  ...
  void window.sarthiDesktop?.touchBackend?.(target).catch(() => undefined)
}
