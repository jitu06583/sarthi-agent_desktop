...
vi.mock('@/sarthi', () => ({
  getSarthiConfigRecord: vi.fn(async () => ({})),
  saveSarthiConfig: vi.fn(async () => undefined)
}))
...
describe('applyVoiceStopPhraseFromConfig', () => {
  ...
  it('uses the first configured phrase so a custom phrase renders correctly', () => {
    applyVoiceStopPhraseFromConfig({ voice: { stop_phrases: ['goodbye sarthi', 'stop'] } })
    expect($voiceStopPhrase.get()).toBe('goodbye sarthi')
  })
