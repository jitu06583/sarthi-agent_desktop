import { type ConnectionState, type GatewayEvent, registryBackendScopeKey, resolveGatewayWsUrl } from '@sarthi/shared'
import { atom } from 'nanostores'

import type { SarthiConnection } from '@/global'
import { SarthiGateway, setApiRequestConnection } from '@/sarthi'
...
const isOpen = (gateway: SarthiGateway | null): boolean => gateway?.connectionState === 'open'

interface RegistryConfig {
  onEvent: (event: GatewayEvent) => void
  onActiveConnectionInvalidated?: (fallbackProfile: string, activationEpoch: number) => void
  onActiveConnectionChanged?: (connection: SarthiConnection) => void
  /**
...
interface Secondary {
  ...
  connection: SarthiConnection | null
  gateway: SarthiGateway
...
interface GatewayRegistryState {
  ...
  primaryGateway: SarthiGateway | null
  primaryProfile: string
  activeKey: string
  activationEpoch: number
  secondaries: Map<string, Secondary>
  $gateway: ReturnType<typeof atom<SarthiGateway | null>>
  $activeProfile: ReturnType<typeof atom<string>>
}

const STATE_KEY = Symbol.for('sarthi.desktop.gatewayRegistryState')
...
function createRegistryState(): GatewayRegistryState {
  return {
    ...
    $gateway: atom<SarthiGateway | null>(null),
...
export function setPrimaryGateway(gateway: SarthiGateway | null, profile = 'default'): void {
...
export function activeGateway(): SarthiGateway | null {
...
function applyActive(profile: string, activationEpoch: number): boolean {
  ...
  // Push the active scope's registry connection into the sarthi module (null
...
function publishActiveConnection(connection: SarthiConnection): void {
...
async function reconnectSecondary(entry: Secondary): Promise<void> {
  ...
  try {
    ...
    // under partial vi.mock('@/sarthi') harnesses the transitive graph can
...
function createSecondary(profile: string, connectionId: null | string = null): Secondary {
  const gateway = new SarthiGateway()
...
async function gatewayForProfile(
  profile: string,
  leaseRequest = false
): Promise<{ gateway: SarthiGateway | null; key: string; release: () => void; scopeProfile: boolean }> {
...
export async function requestGatewayForProfile<T>(
  ...
  try {
    if (!route.gateway) {
      throw new Error(`Sarthi gateway unavailable for profile "${route.key}"`)
    }
...
export async function requestGatewayForAgent<T>(
  ...
  if (!window.sarthiDesktop?.getConnectionFor) {
    throw new Error('This Desktop build cannot dial registry connections. Update Sarthi Desktop.')
  }
...
export async function ensureActiveGatewayOpen(): Promise<SarthiGateway | null> {
  if (g.activeKey === g.primaryProfile) {
    return g.primaryGateway
  }
  ...
  if (!isOpen(entry.gateway)) {
    ...
    // "Sarthi gateway is not connected" on the Sessions `+` action (#88880).
