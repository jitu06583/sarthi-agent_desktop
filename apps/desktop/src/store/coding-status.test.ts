...
import type { SarthiRepoStatus } from '@/global'
...
const sampleStatus: SarthiRepoStatus = {
...
const otherStatus: SarthiRepoStatus = {
  ...sampleStatus,
...
function stubProbe(impl: (cwd: string) => Promise<SarthiRepoStatus | null>) {
  ;(window as unknown as { sarthiDesktop?: unknown }).sarthiDesktop = { git: { repoStatus: impl } }
}
...
describe('refreshRepoStatus', () => {
  ...
  it('never publishes an old worktree status onto the primary after the active cwd moves', async () => {
    let resolveOld!: (status: SarthiRepoStatus | null) => void
    stubProbe(
      () =>
        new Promise(resolve => {
          resolveOld = resolve
  ...
  it('runs one probe at a time and coalesces overlap into one trailing refresh per drain', async () => {
    const resolvers: Array<(status: SarthiRepoStatus | null) => void> = []
    const calls: string[] = []
    let active = 0
    let maxActive = 0
...
describe('repoChangeKindForPath', () => {
  it('does not notify a row when only another path changes', () => {
    ...
    $repoStatusByCwd.set({
      '/repo': {
        ...sampleStatus,
        files: [{ path: 'b.ts', untracked: true } as SarthiRepoStatus['files'][number]]
      }
    })
    ...
    $repoStatusByCwd.set({
      '/repo': {
        ...sampleStatus,
        files: [{ path: 'a.ts', untracked: true } as SarthiRepoStatus['files'][number]]
      }
    })
