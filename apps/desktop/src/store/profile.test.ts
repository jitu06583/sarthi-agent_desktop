...
const { getProfiles } = await import('@/sarthi')
...
const profile = (name: string, isDefault = false): ProfileInfo => ({
  ...
  model: null,
  name,
  path: `/tmp/sarthi/${name}`,
  provider: null,
  skill_count: 0
})

const remoteConn = (over: Partial<SarthiConnection> = {}): SarthiConnection =>
  ({ baseUrl: 'https://sarthi-roy.tail.ts.net', mode: 'remote', profile: 'vps-remote', ...over }) as SarthiConnection

const localConn = (over: Partial<SarthiConnection> = {}): SarthiConnection =>
  ({ baseUrl: '', mode: 'local', profile: 'default', ...over }) as SarthiConnection

const getConnection = vi.fn<(profile?: string | null) => Promise<SarthiConnection>>()
...
beforeEach(() => {
  ...
  $profiles.set([])
  vi.stubGlobal('window', { sarthiDesktop: { getConnection } })
