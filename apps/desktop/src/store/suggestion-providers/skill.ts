 ...
 *  Working in `~/www/sarthi-agent`, the draft says "sarthi-agent" constantly
 *  — a "Use skill: sarthi-agent" pill on every mention is noise (the reported
 *  annoyance that prompted this guard). Boundary containment, not exact
 *  segment equality, so worktree dirs (`sarthi-agent-suggest`) suppress too. */
 ...
 *  segment of a qualified form (`github/sarthi-agent-dev`, `plugin:skill`). */
