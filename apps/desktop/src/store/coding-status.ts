...
import type { SarthiGitWorktree, SarthiRepoStatus } from '@/global'
...
const EMPTY_WORKTREES: SarthiGitWorktree[] = []
...
export const $repoStatusByCwd = atom<Record<string, SarthiRepoStatus | null>>({})
export const $repoWorktreesByCwd = atom<Record<string, SarthiGitWorktree[]>>({})
...
export const $repoStatus: ReadableAtom<SarthiRepoStatus | null> = computed(
...
export const $repoWorktrees: ReadableAtom<SarthiGitWorktree[]> = computed(
...
const statusAtomByCwd = new Map<string, ReadableAtom<SarthiRepoStatus | null>>()
const worktreesAtomByCwd = new Map<string, ReadableAtom<SarthiGitWorktree[]>>()
const $noRepoStatus = atom<SarthiRepoStatus | null>(null)
const $noWorktrees = atom<SarthiGitWorktree[]>(EMPTY_WORKTREES)

/** Reactive status for one repo cwd (a tile's worktree). Stable per cwd. */
export function repoStatusForCwd(cwd?: null | string): ReadableAtom<SarthiRepoStatus | null> {
...
export function repoWorktreesForCwd(cwd?: null | string): ReadableAtom<SarthiGitWorktree[]> {
  const key = normalizeCwd(cwd)
...
function setRepoStatusEntry(target: string, status: SarthiRepoStatus | null): void {
...
function setRepoWorktreesEntry(target: string, worktrees: SarthiGitWorktree[]): void {
...
interface RepoStatusRefreshRequest {
  probe: (cwd: string) => Promise<SarthiRepoStatus | null>
  seq: number
  target: string
}
