 ...
 * app's bounds and stays visible while Sarthi is minimized. That window carries
...
const OVERLAY_ACTIVE_KEY = 'sarthi.desktop.pet-overlay-active.v1'
const OVERLAY_BOUNDS_KEY = 'sarthi.desktop.pet-overlay-bounds.v1'
...
function pushNow(): void {
  window.sarthiDesktop?.petOverlay?.pushState(currentPayload())
}
...
function openOverlay(request: PetOverlayOpenRequest): void {
  const api = window.sarthiDesktop?.petOverlay
...
export function restorePetOverlay(): void {
  if (!window.sarthiDesktop?.petOverlay || !$petOverlayActive.get() || stateUnsubs.length) {
    return
  }
...
export function popInPet(): void {
  ...
  stateUnsubs = []
  $petOverlayActive.set(false)
  void window.sarthiDesktop?.petOverlay?.close()
}
