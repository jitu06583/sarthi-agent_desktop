 ...
 *
 * The archive is the CLI's own `sarthi profile export` tar.gz (config, skills,
...
import { exportProfileArchive, importProfileArchive } from '@/sarthi'
...
import type { ProfileDesktopOverlay } from '@/types/sarthi'
...
const ARCHIVE_FILTERS = [{ extensions: ['tar.gz', 'tgz'], name: 'Sarthi profile' }]
...
export async function runExportProfileFlow(profile?: string): Promise<null | string> {
  ...
  const pick = window.sarthiDesktop?.selectSavePath
...
export async function runImportProfileFlow(): Promise<null | string> {
  const paths = await window.sarthiDesktop?.selectPaths?.({
