...
import type { SarthiGitBaseBranch, SarthiGitBranch } from '@/global'
import { getSarthiConfig, sarthiApi, type SarthiGateway } from '@/sarthi'
...
import type { ProjectInfo, ProjectsPayload } from '@/types/sarthi'
...
const PROJECT_SCOPE_KEY = 'sarthi.desktop.projectScope'
...
async function gatewayRequest<T>(method: string, params: Record<string, unknown> = {}): Promise<T> {
  ...
  if (!gateway) {
    throw new Error('Sarthi gateway is not connected')
  }
...
async function activeProjectsContext(): Promise<ActiveProjectsContext> {
  ...
  if (!gateway || gateway !== activeGateway() || profile !== projectProfile()) {
    throw new Error('Active Sarthi profile changed while connecting')
  }
...
async function refreshProjectTreeAcrossProfiles(): Promise<void> {
  ...
  try {
    const res = await sarthiApi<ProjectTreePayload>({
...
const repoScanStates = new WeakMap<SarthiGateway, RepoScanState>()
const scanningGatewayGenerations = new WeakMap<SarthiGateway, number>()
...
export async function scanAndRecordRepos(force = false): Promise<void> {
  if (isDesktopFsRemoteMode()) {
    ...
    // with `scan: true` — added in #81723) so repos with zero Sarthi
  ...
  try {
    const policy = repoDiscoveryPolicyFromConfig(await getSarthiConfig(context.profile))
...
// repo, returning where Sarthi should start working. Git is the source of
...
export async function listRepoBranches(repoPath: string): Promise<SarthiGitBranch[]> {
  const git = desktopGit()
...
export async function listBaseBranches(repoPath: string): Promise<SarthiGitBaseBranch[]> {
  const git = desktopGit()
...
export async function revealPath(path: null | string): Promise<void> {
  if (path) {
    await window.sarthiDesktop?.revealPath?.(path)
  }
}
...
export async function copyPath(path: null | string): Promise<void> {
  if (path) {
    await window.sarthiDesktop?.writeClipboard?.(path)
  }
}
