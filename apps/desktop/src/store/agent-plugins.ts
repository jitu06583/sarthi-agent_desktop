...
/**
 * Feature store for backend (agent) plugins — the native Sarthi plugins plus
 ...
 * primitives `sarthi plugins` and the dashboard use, so all surfaces agree on
...
export interface AgentPluginRow {
  ...
  /** Agent Plugins v1 package (portable skills/MCP format) vs native Sarthi. */
  portable?: boolean
}
 ...
 *  SARTHI_HOME. Always refetches (it's a cheap local disk scan on the
