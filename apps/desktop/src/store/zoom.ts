...
export function setZoomPercent(percent: number): void {
  window.sarthiDesktop?.zoom?.setPercent(percent)
}

if (typeof window !== 'undefined' && window.sarthiDesktop?.zoom) {
  void window.sarthiDesktop.zoom.get().then(({ percent }) => $zoomPercent.set(percent))
  window.sarthiDesktop.zoom.onChanged(({ percent }) => $zoomPercent.set(percent))
}
