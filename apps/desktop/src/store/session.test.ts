...
describe('workspaceCwdForNewSession', () => {
  afterEach(() => {
    ...
    window.localStorage.removeItem('sarthi.desktop.workspace-cwd')
    window.localStorage.removeItem('sarthi.desktop.workspace-cwd.remote.http%3A%2F%2Fbackend-a.default')
    window.localStorage.removeItem('sarthi.desktop.workspace-cwd.remote.http%3A%2F%2Fbackend-b.default')
  })

  it('prefers the configured default over the sticky remembered workspace', () => {
    window.localStorage.setItem('sarthi.desktop.workspace-cwd', '/home/user/sticky')
  ...
  it('keeps remote workspace memory separate from local and other remotes', () => {
    window.localStorage.setItem('sarthi.desktop.workspace-cwd', '/local/project')
    $currentCwd.set('/live/session/path')
...
describe('remembered session id (per profile)', () => {
  ...
  it('discards legacy unsuffixed keys on first read (zero-migration, refuse-to-guess)', () => {
    ...
    localStorage.setItem('sarthi.desktop.lastSessionId', 'legacy-session')
    ...
    expect(localStorage.getItem('sarthi.desktop.lastSessionId')).toBeNull()
  })
  ...
  it('uses encodeURIComponent so profile names with reserved chars are isolated', () => {
    ...
    expect(localStorage.getItem('sarthi.desktop.lastSessionId.profile.research%2Fops')).toBe('ops-session')
...
describe('remembered route (per profile)', () => {
  ...
  it('discards legacy unsuffixed keys on first read (zero-migration, refuse-to-guess)', () => {
    localStorage.setItem('sarthi.desktop.lastRoute', '/skills')
    ...
    expect(localStorage.getItem('sarthi.desktop.lastRoute')).toBeNull()
  })
  ...
  it('uses encodeURIComponent so profile names with reserved chars are isolated', () => {
    ...
    expect(localStorage.getItem('sarthi.desktop.lastRoute.profile.research%2Fops')).toBe('/cron')
    expect(getRememberedRoute('research')).toBeNull()
  })
