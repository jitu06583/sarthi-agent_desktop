 ...
 *   (`@sarthi/plugin-sdk` / `react*` -> live shim blobs, see sdk/runtime.ts)
 *   -> blob `import()` -> validate default SarthiPlugin -> register(ctx)
 *
 ...
 * and the two on-disk doors — `<sarthi home>/desktop-plugins/<name>/plugin.js`
 * and the unified agent-plugin half `<sarthi home>/plugins/<name>/desktop/
...
import { createPluginContext, type SarthiPlugin } from './plugin'
...
interface LoadOptions {
   ...
   *  unified agent-plugin root sets this so `~/.sarthi/plugins` keeps its
...
/** Rewrite ONLY mapped import specifiers (@sarthi/plugin-sdk, react*) to their
...
export async function loadRuntimePlugin(
  ...
  try {
    ...
    if (unsupported.length > 0) {
      throw new Error(
        `unsupported import${unsupported.length > 1 ? 's' : ''}: ${unsupported.join(', ')} — ` +
          `runtime plugins may only import @sarthi/plugin-sdk and react`
      )
    }

    const url = URL.createObjectURL(new Blob([rewriteSpecifiers(source)], { type: 'text/javascript' }))

    let mod: { default?: SarthiPlugin }
    ...
    if (!plugin?.id || typeof plugin.register !== 'function') {
      throw new Error(`${origin} has no valid default SarthiPlugin export`)
    }

    // A disk/runtime copy of a plugin that now ships BUNDLED (e.g. a
    // standalone install of sarthi-bots predating its adoption in-tree) must
...
//  - `<sarthi home>/desktop-plugins/<name>/plugin.js` — the standalone door
//    (agent- or user-written desktop-only plugins);
//  - `<sarthi home>/plugins/<name>/desktop/plugin.js` — the desktop HALF of a
 ...
 *  backend's sarthi_home — #66899). `agentPluginsRoot` is optional: older
...
async function diskRoots(): Promise<DiskRoot[]> {
  ...
  if (unified) {
    // Opt-in by default: `~/.sarthi/plugins` is installed-but-inert until the
