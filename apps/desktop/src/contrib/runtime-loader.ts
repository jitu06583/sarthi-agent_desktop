 ...
 * and `<sarthi home>/desktop-plugins/<name>/plugin.js` on disk — the door the
...
// ---------------------------------------------------------------------------
// The on-disk plugin door: `<sarthi home>/desktop-plugins/<name>/plugin.js`
...
async function scanDiskPlugins(): Promise<void> {
  ...
  try {
    const { sarthi_home } = await getStatus()
    const { entries } = await desktop.readDir(`${sarthi_home}/desktop-plugins`)
