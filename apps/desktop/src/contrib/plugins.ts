 ...
 *    `SarthiPlugin` registers automatically (vite glob — drop a folder in).
 *    None ship in-tree today; reference/demo plugins live in the companion
 *    `sarthi-example-plugins` repo.
 *  - RUNTIME: the on-disk door (`<sarthi home>/desktop-plugins/<name>/plugin.js`)
...
const modules = import.meta.glob<{ default: SarthiPlugin }>('../plugins/*/plugin.{ts,tsx}', { eager: true })
