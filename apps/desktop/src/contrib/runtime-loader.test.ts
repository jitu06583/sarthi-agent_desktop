...
import type { SarthiReadDirResult } from '@/global'
import type * as SarthiModule from '@/sarthi'
...
// getStatus would supply the connected backend's sarthi_home — a REMOTE path in
// remote mode. The disk scanner must NOT derive the plugin root from it (#66899).
const getStatus = vi.fn(async () => ({ sarthi_home: '/remote/box/.sarthi' }))

vi.mock('@/sarthi', async importActual => ({
  ...(await importActual<typeof SarthiModule>()),
  getStatus: () => getStatus()
}))

const desktopPluginsRoot = vi.fn<() => Promise<string>>()
const agentPluginsRoot = vi.fn<() => Promise<string>>()
const readDir = vi.fn<(path: string) => Promise<SarthiReadDirResult>>()
...
beforeEach(() => {
  ...
  getStatus.mockClear()
  ;(window as unknown as { sarthiDesktop: unknown }).sarthiDesktop = {
    agentPluginsRoot,
    desktopPluginsRoot,
...
afterEach(() => {
  delete (window as unknown as { sarthiDesktop?: unknown }).sarthiDesktop
})

describe('scanDiskPlugins (#66899)', () => {
  it('scans the Electron-resolved local roots, never the backend sarthi_home', async () => {
    desktopPluginsRoot.mockResolvedValue('/local/.sarthi/desktop-plugins')
    agentPluginsRoot.mockResolvedValue('/local/.sarthi/plugins')
    readDir.mockResolvedValue({ entries: [] })

    await discoverRuntimePlugins()

    expect(desktopPluginsRoot).toHaveBeenCalled()
    expect(readDir).toHaveBeenCalledWith('/local/.sarthi/desktop-plugins')
    expect(readDir).toHaveBeenCalledWith('/local/.sarthi/plugins')
    // The remote backend's sarthi_home must never feed the local plugin scan.
    expect(getStatus).not.toHaveBeenCalled()
    expect(readDir).not.toHaveBeenCalledWith('/remote/box/.sarthi/desktop-plugins')
  })
  ...
  it('probes desktop/plugin.js inside agent-plugin packages (unified packaging)', async () => {
    ...
    readDir.mockImplementation(async dir =>
      dir === '/local/.sarthi/plugins'
        ? { entries: [{ isDirectory: true, name: 'my-feature', path: '/local/.sarthi/plugins/my-feature' }] }
        : { entries: [] }
    )
    ...
    expect(readFileText).toHaveBeenCalledWith('/local/.sarthi/plugins/my-feature/desktop/plugin.js')
    // The Python half's files must never be probed as a desktop entry.
    expect(readFileText).not.toHaveBeenCalledWith('/local/.sarthi/plugins/my-feature/plugin.js')
  })

  it('still scans the standalone root when agentPluginsRoot is absent (older shell)', async () => {
    delete (window.sarthiDesktop as unknown as { agentPluginsRoot?: unknown }).agentPluginsRoot
    desktopPluginsRoot.mockResolvedValue('/local/.sarthi/desktop-plugins')
    readDir.mockResolvedValue({ entries: [] })
  ...
  it('loads a unified desktop half OPT-IN: inventoried but not activated by default', async () => {
    ...
    readDir.mockImplementation(async dir =>
      dir === '/local/.sarthi/plugins'
        ? { entries: [{ isDirectory: true, name: 'uni', path: '/local/.sarthi/plugins/uni' }] }
        : { entries: [] }
    )
    ...
    try {
      ...
      // ~/.sarthi/plugins stays installed-but-inert until the user toggles it.
...
describe('watchRuntimePlugins dir watch (#66899)', () => {
  it('watches both Electron-resolved local roots, never the backend sarthi_home', async () => {
    desktopPluginsRoot.mockResolvedValue('/local/.sarthi/desktop-plugins')
    agentPluginsRoot.mockResolvedValue('/local/.sarthi/plugins')
    readDir.mockResolvedValue({ entries: [] })
    ...
    expect(watchDirectory).toHaveBeenCalledWith('/local/.sarthi/desktop-plugins')
    expect(watchDirectory).toHaveBeenCalledWith('/local/.sarthi/plugins')
    expect(watchDirectory).not.toHaveBeenCalledWith('/remote/box/.sarthi/desktop-plugins')
    expect(getStatus).not.toHaveBeenCalled()
  })
})
...
describe('bundled-shadowed disk copies', () => {
  it('skips a disk copy of a bundled plugin but publishes a visible inventory row', async () => {
    ...
    publishPlugin({ id: 'sarthi-bots', name: 'Bot Mode', kind: 'bundled', status: 'loaded' })
    ...
    try {
      const id = await loadRuntimePlugin(
        'export default { id: "sarthi-bots", name: "Bot Mode", register() {} }',
        'sarthi-bots',
        { file: '/local/.sarthi/desktop-plugins/sarthi-bots/plugin.js' }
      )

      // Skipped — the bundled copy stays the only live registration...
      expect(id).toBeNull()
      expect($pluginRecords.get()['sarthi-bots']).toMatchObject({ kind: 'bundled', status: 'loaded' })
      ...
      expect($pluginRecords.get()['sarthi-bots:disk-shadowed']).toMatchObject({
        kind: 'disk',
        status: 'disabled',
        file: '/local/.sarthi/desktop-plugins/sarthi-bots/plugin.js'
      })
