...
export const ja = defineLocale({
  ...
  settings: {
    ...
    gateway: {
      ...
      intro:
        'Sarthi Desktop はデフォルトで独自のローカルゲートウェイを起動します。別のマシンや信頼できるプロキシの背後で既に動作している Sarthi バックエンドをこのアプリで制御する場合は、リモートゲートウェイを使用してください。以下でプロファイルを選択して、それぞれのリモートホストを設定します。',
      appliesTo: '適用対象',
  ...
  shell: {
    ...
    statusbar: {
      ...
      gatewayTitle: 'Sarthi 推論ゲートウェイのステータス',
      agents: 'エージェント',
