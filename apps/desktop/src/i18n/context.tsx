...
import { getSarthiConfigRecord, type SarthiConfigRecord, saveSarthiConfig } from '@/sarthi'
...
export interface I18nConfigClient {
  getConfig: () => Promise<SarthiConfigRecord>
  saveConfig: (config: SarthiConfigRecord) => Promise<{ ok: boolean }>
}

const defaultConfigClient: I18nConfigClient = {
  getConfig: () => {
    if (typeof window === 'undefined' || !window.sarthiDesktop?.api) {
      return Promise.resolve({})
    }

    return getSarthiConfigRecord()
  },
  saveConfig: config => {
    if (typeof window === 'undefined' || !window.sarthiDesktop?.api) {
    ...
    return saveSarthiConfig(config)
  }
}
...
export function getConfigDisplayLanguage(config: SarthiConfigRecord): unknown {
  return isRecord(config.display) ? config.display.language : undefined
}

export function withConfigDisplayLanguage(config: SarthiConfigRecord, locale: Locale): SarthiConfigRecord {
