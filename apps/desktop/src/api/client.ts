...
import type { SarthiApiRequest } from '@/global'
...
// process to route them to that host (sarthi:api's registry branch). Null
...
export function sarthiApi<T>(request: SarthiApiRequest): Promise<T> {
  return window.sarthiDesktop.api<T>({ ...connectionScoped(), ...request })
}
