...
import { capabilityScoped, sarthiApi, type ProfileScope, profileScoped, STARTUP_REQUEST_TIMEOUT_MS } from './client'

export function getStatus(): Promise<StatusResponse> {
  return sarthiApi<StatusResponse>({
    ...profileScoped(),
    path: '/api/status'
  })
}
...
export function getLogs(params: {
  ...
  return sarthiApi<LogsResponse>({
    ...profileScoped(),
    path: suffix ? `/api/logs?${suffix}` : '/api/logs'
  })
}

export function getSarthiConfig(profile?: string): Promise<SarthiConfig> {
  return sarthiApi<SarthiConfig>({
...
export function getSarthiConfigRecord(profile?: ProfileScope): Promise<SarthiConfigRecord> {
...
export function getSarthiConfigDefaults(): Promise<SarthiConfigRecord> {
  return sarthiApi<SarthiConfigRecord>({
    ...profileScoped(),
...
export function getSarthiConfigSchema(profile?: null | string): Promise<ConfigSchemaResponse> {
  return sarthiApi<ConfigSchemaResponse>({
    ...profileScoped(profile),
    path: '/api/config/schema'
  })
}

export function saveSarthiConfig(config: SarthiConfigRecord, profile?: null | string): Promise<{ ok: boolean }> {
  return sarthiApi<{ ok: boolean }>({
    ...profileScoped(profile),
    path: '/api/config',
    method: 'PUT',
    body: { config }
...
export function getEnvVars(profile?: null | string): Promise<Record<string, EnvVarInfo>> {
  return sarthiApi<Record<string, EnvVarInfo>>({
    ...profileScoped(profile),
    path: '/api/env'
  })
}
...
export function validateProviderCredential(
  ...
  return sarthiApi<{ ok: boolean; reachable: boolean; message: string; models?: string[] }>({
    ...profileScoped(),
...
export function getCustomEndpoints(): Promise<CustomEndpointsResponse> {
  return sarthiApi<CustomEndpointsResponse>({
    ...profileScoped(),
...
export function validateCustomEndpoint(endpoint: CustomEndpointUpdate): Promise<CustomEndpointValidationResponse> {
  return sarthiApi<CustomEndpointValidationResponse>({
...
export function activateCustomEndpoint(id: string): Promise<{ ok: boolean; provider: string; model: string }> {
  return sarthiApi<{ ok: boolean; provider: string; model: string }>({
    ...profileScoped(),
...
export function listOAuthProviders(): Promise<OAuthProvidersResponse> {
  return sarthiApi<OAuthProvidersResponse>({
    ...profileScoped(),
...
export function disconnectOAuthProvider(providerId: string): Promise<{ ok: boolean; provider: string }> {
  return sarthiApi<{ ok: boolean; provider: string }>({
    ...profileScoped(),
...
export function submitOAuthCode(providerId: string, sessionId: string, code: string): Promise<OAuthSubmitResponse> {
  return sarthiApi<OAuthSubmitResponse>({
    ...profileScoped(),
