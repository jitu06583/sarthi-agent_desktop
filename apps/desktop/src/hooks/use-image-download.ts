...
function isMissingIpcHandler(error: unknown): boolean {
  ...
  return message.includes("No handler registered for 'sarthi:saveImageFromUrl'")
}
...
export function useImageDownload(src?: string) {
  ...
  const download = useCallback(async () => {
    ...
    try {
      if (window.sarthiDesktop?.saveImageFromUrl) {
        if (await window.sarthiDesktop.saveImageFromUrl(src)) {
