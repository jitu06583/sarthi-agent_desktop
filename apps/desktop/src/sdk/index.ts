/**
 * @sarthi/plugin-sdk — THE plugin language. The vscode-module model: plugin
 ...
 *    object as `window.__SARTHI_PLUGIN_SDK__` and maps the import to it, so a
...
import { deleteProfile, getLogs, getStatus, type SarthiGateway } from '@/sarthi'
import {
  $gateway,
...
import type { UsageStats } from '@/types/sarthi'
...
export interface PluginProfileRoute {
  ...
  profile: string
  /** Backend Sarthi profile served by that route. */
...
async function requestPluginProfile<T>(
  ...
  const getAgentRoster = window.sarthiDesktop?.getAgentRoster
...
//
// ensureGatewayProfile awaits the store's dial, and SarthiGateway.connect() has
...
export const host = {
  ...
  connections: async () => {
    const bridge = window.sarthiDesktop?.connections

    if (!bridge) {
      throw new Error('This Desktop build has no connection registry. Update Sarthi Desktop.')
    }
  ...
  agents: async () => {
    const roster = window.sarthiDesktop?.getAgentRoster

    if (!roster) {
      throw new Error('This Desktop build cannot enumerate multi-source agents. Update Sarthi Desktop.')
    }
  ...
  profileRoutes: async () => {
    ...
    if (!getProfileRoutes) {
      throw new Error('Sarthi Desktop connection routing unavailable')
    }
  ...
  request: async <T>(method: string, params: Record<string, unknown> = {}): Promise<T> => {
    ...
    if (!gateway) {
      throw new Error('Sarthi gateway unavailable')
    }
...
export type { SarthiGateway } from '@/sarthi'
...
export type { SarthiOpenTarget } from '@/lib/sarthi-open-target'
...
/** Sarthi' reasoning levels + their compact labels, so a plugin surfacing a
...
export type { RpcEvent, StatusResponse } from '@/types/sarthi'
