import type { GatewayWsUrlResult } from '@sarthi/shared'
import type { TranslucencyState } from '@sarthi/shared/translucency'
...
declare global {
  interface Window {
    sarthiDesktop: {
      ...
      getConnection: (profile?: string | null) => Promise<SarthiConnection>
      ...
      // Resume this session in the user's own terminal emulator (`sarthi --tui
      ...
      // Sarthi Cloud: one portal login powers discovery + silent per-agent
      ...
      profile: {
        ...
        // backend under the new SARTHI_HOME (reloads the window). Pass null to
        // clear the preference.
        set: (name: string | null) => Promise<DesktopActiveProfile>
      }
      api: <T>(request: SarthiApiRequest) => Promise<T>
      notify: (payload: SarthiNotification) => Promise<boolean>
      ...
      readFileText: (filePath: string) => Promise<SarthiReadFileTextResult>
      selectPaths: (options?: SarthiSelectPathsOptions) => Promise<string[]>
      ...
      normalizePreviewTarget: (target: string, baseDir?: string) => Promise<SarthiPreviewTarget | null>
      watchPreviewFile: (url: string) => Promise<SarthiPreviewWatch>
      ...
      watchDirectory?: (dir: string) => Promise<SarthiPreviewWatch>
      stopPreviewFileWatch: (id: string) => Promise<boolean>
      setActiveWork?: (payload: SarthiActiveWork) => void
      setTitleBarTheme?: (payload: SarthiTitleBarTheme) => void
      ...
      readDir: (path: string) => Promise<SarthiReadDirResult>
      ...
      // Local Desktop runtime-plugin root (<SARTHI_HOME>/desktop-plugins),
      ...
      /** LOCAL `<SARTHI_HOME>/logs` (profile-aware) — error card "Open Logs". */
      logsRoot?: () => Promise<string>
      // Local AGENT-plugin root (<SARTHI_HOME>/plugins), same Electron-local
      ...
      git?: {
        worktreeList: (repoPath: string) => Promise<SarthiGitWorktree[]>
        worktreeAdd: (
          repoPath: string,
        ...
        branchList: (repoPath: string) => Promise<SarthiGitBranch[]>
        ...
        baseBranchList: (repoPath: string) => Promise<SarthiGitBaseBranch[]>
        ...
        repoStatus: (repoPath: string) => Promise<SarthiRepoStatus | null>
        ...
        review: {
          list: (repoPath: string, scope: SarthiReviewScope, baseRef?: null | string) => Promise<SarthiReviewList>
          diff: (
            repoPath: string,
            filePath: string,
            scope: SarthiReviewScope,
            baseRef?: null | string,
            staged?: boolean
          ) => Promise<string>
          ...
          shipInfo: (repoPath: string) => Promise<SarthiReviewShipInfo>
          ...
          prList: (repoPath: string, branches: string[], numbers?: number[]) => Promise<SarthiRepoPullRequests>
          ...
          fetchPrComment: (repoPath: string, url: string) => Promise<SarthiPrComment | null>
      ...
      terminal: {
        ...
        onExit: (id: string, callback: (payload: SarthiTerminalExit) => void) => () => void
        resize: (id: string, size: { cols: number; rows: number }) => Promise<boolean>
        start: (options?: { cols?: number; cwd?: string; rows?: number }) => Promise<SarthiTerminalSession>
      ...
      onWindowStateChanged?: (callback: (payload: SarthiWindowState) => void) => () => void
      ...
      onPreviewFileChanged: (callback: (payload: SarthiPreviewFileChanged) => void) => () => void
...
export interface SarthiTerminalSession {
  cwd: string
  id: string
  shell: string
}

export interface SarthiTerminalExit {
  code: number | null
  signal: string | null
}
...
export interface DesktopUpdateApplyResult {
   ...
   *  `sarthi update` themselves. `command` is the exact line to run. */
  manual?: boolean
  command?: string
  sarthiRoot?: string
...
export interface SarthiConnection {
  baseUrl: string
  darwinMajor?: number
...
export interface SarthiTitleBarTheme {
  background: string
  foreground: string
}

/** Turns in flight, so the main process can confirm before a quit kills them. */
export interface SarthiActiveWork {
  count: number
  titles: string[]
}

export interface SarthiWindowState {
  darwinMajor?: number
  isFullscreen: boolean
  isMinimized?: boolean
  isVisible?: boolean
...
export interface DesktopConnectionConfig {
  envOverride: boolean
  // The saved connection mode. 'cloud' is a Sarthi Cloud connection: it carries
  ...
  remoteUrl: string
  // For a 'cloud' connection: the persisted Sarthi Cloud org (slug or id) the
...
export interface DesktopConnectionConfigInput {
  ...
  remoteUrl?: string
  // For a 'cloud' connection: the selected Sarthi Cloud org (slug or id) to
...
export interface DesktopCloudStatus {
  ...
  // the sarthi gateway session cookies. See cookiesHavePrivySession.
  signedIn: boolean
}

// A discovered Sarthi Cloud agent — the trimmed DTO from NAS GET /api/agents.
...
export interface SarthiApiRequest {
  path: string
  method?: string
  body?: unknown
...
export interface SarthiNotification {
  title?: string
  body?: string
  silent?: boolean
  kind?: string
...
export interface SarthiPreviewTarget {
  binary?: boolean
  byteSize?: number
  kind: 'file' | 'url'
  label: string
...
export interface SarthiReadFileTextResult {
  binary?: boolean
  byteSize?: number
  language?: string
  mimeType?: string
...
export interface SarthiPreviewWatch {
  id: string
  path: string
}
...
export interface SarthiGitWorktree {
  path: string
  branch: null | string
  isMain: boolean
  detached: boolean
...
export interface SarthiGitBranch {
  name: string
  checkedOut: boolean
  isDefault: boolean
  isRemote: boolean
...
// preselect it.
export interface SarthiGitBaseBranch {
  name: string
  isRemote: boolean
  isDefault: boolean
}
...
export interface SarthiRepoStatusFile {
  path: string
  staged: boolean
  unstaged: boolean
  untracked: boolean
...
export interface SarthiRepoStatus {
  branch: null | string
...
// turn began.
export type SarthiReviewScope = 'branch' | 'lastTurn' | 'uncommitted'

// One changed file in the review pane (status letter, +/- lines, staged flag).
export interface SarthiReviewFile {
  path: string
  added: number
  removed: number
...
export interface SarthiReviewList {
  files: SarthiReviewFile[]
...
export interface SarthiReviewPr {
  url: string
  state: string
  number: number
}
...
export interface SarthiBranchPullRequest {
  branch: string
  draft: boolean
  number: number
...
export interface SarthiRepoPullRequests {
  ghReady: boolean
  prs: SarthiBranchPullRequest[]
}
...
export interface SarthiPrComment {
  author: string
  body: string
  diffHunk: string
  kind: 'issue' | 'review'
...
export interface SarthiReviewShipInfo {
  ghReady: boolean
  pr: SarthiReviewPr | null
}

export interface SarthiReadDirEntry {
  name: string
  path: string
  isDirectory: boolean
}

export interface SarthiReadDirResult {
  entries: SarthiReadDirEntry[]
  error?: string
}

export interface SarthiPreviewFileChanged {
  id: string
  path: string
  url: string
}

export interface SarthiSelectPathsOptions {
  title?: string
  defaultPath?: string
  directories?: boolean
  multiple?: boolean
