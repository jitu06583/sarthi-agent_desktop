/**
 * Live skin sync from the Sarthi backend.
 *
 * The backend resolves the active skin (built-in or `$SARTHI_HOME/skins/*.yaml`)
 ...
 * stomps the user's persisted desktop theme; only a genuine name change (Sarthi
 * authoring/activating a skin from a prompt, or `/skin` elsewhere) repaints.
 */

import type { SarthiSkin } from '@sarthi/shared/skin'
 ...
 */
export function ingestBackendSkin(skin: SarthiSkin | undefined | null, { apply }: { apply: boolean }): void {
  ...
  if (name !== 'default' && !BUILTIN_THEMES[name]) {
    const theme = skinToDesktopTheme(skin as SarthiSkin)
