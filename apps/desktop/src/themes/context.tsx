...
const SKIN_KEY = 'sarthi-desktop-theme-v2'
const MODE_KEY = 'sarthi-desktop-mode-v1'
...
const PROFILE_SKINS_KEY = 'sarthi-desktop-profile-themes-v1'
const PROFILE_MODES_KEY = 'sarthi-desktop-profile-modes-v1'
// Last active profile, recorded so the boot-time paint can pick that profile's
// theme before the gateway reports which profile actually launched.
const LAST_PROFILE_KEY = 'sarthi-desktop-active-profile-v1'
...
function applyTheme(theme: DesktopTheme, mode: 'light' | 'dark') {
  ...
  root.dataset.sarthiTheme = skinName
  root.dataset.sarthiMode = rendered
  ...
  window.sarthiDesktop?.setTitleBarTheme?.({
    background: chromeBg,
  ...
  try {
    window.localStorage.setItem('sarthi-boot-background', chromeBg)
    window.localStorage.setItem('sarthi-boot-color-scheme', rendered)
  } catch {
  ...
  if (typo.fontUrl && !INJECTED_FONT_URLS.has(typo.fontUrl)) {
    ...
    link.dataset.sarthiThemeFont = 'true'
...
const syncNativeTheme = (pref: ThemeMode, rendered: 'light' | 'dark') =>
  window.sarthiDesktop?.setNativeTheme?.(pref === 'system' ? 'system' : rendered)
...
export function ThemeProvider({ children }: { children: ReactNode }) {
  ...
  // Drain a backend-driven skin switch (Sarthi authoring/activating a skin from a
