...
export async function resolveMediaDisplaySrc(path: string): Promise<string> {
  ...
  if (window.sarthiDesktop && isRemoteGateway()) {
    return gatewayMediaDataUrl(path)
  }

  if (!window.sarthiDesktop?.readFileDataUrl) {
    return mediaExternalUrl(path)
  }

  return window.sarthiDesktop.readFileDataUrl(filePathFromMediaPath(path))
}
...
export async function resolveMediaPlaybackSrc(path: string): Promise<string> {
  ...
  if (window.sarthiDesktop && ['audio', 'video'].includes(mediaKind(path))) {
...
export function mediaGatewayStreamUrl(path: string): string {
  ...
  if (isRemoteGateway()) {
    ...
    return `sarthi-media://remote/${file}${profile}`
  }
...
export function mediaStreamUrl(path: string): string {
  return `sarthi-media://stream/${encodeURIComponent(filePathFromMediaPath(path))}`
}
...
// (workspace, skills, ~/.sarthi/cache, etc.); /api/media is intentionally
...
export async function downloadGatewayMediaFile(
  ...
  if (!window.sarthiDesktop?.saveGatewayFile) {
    throw new Error('Desktop file download bridge is unavailable')
  }

  return window.sarthiDesktop.saveGatewayFile({
    path: file,
