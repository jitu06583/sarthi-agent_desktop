...
// Sarthi is rendering, and when the two are opposite schemes it takes over
...
function renderedMode(doc: Document): RenderedMode {
  const attr = doc.documentElement.dataset.sarthiMode
 ...
 * the only carriers of Sarthi' palette — are dropped so the paste target's
