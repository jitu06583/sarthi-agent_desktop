import type {
  SarthiConnection,
  SarthiReadDirResult,
  SarthiReadFileTextResult,
  SarthiSelectPathsOptions
} from '@/global'
...
function connectionCacheKey(connection: SarthiConnection | null) {
  if (!connection) {
    return 'local:'
  }
...
export function desktopFsCacheKey(connection: SarthiConnection | null = $connection.get()) {
...
export async function readDesktopDir(path: string): Promise<SarthiReadDirResult> {
  if (!isDesktopFsRemoteMode()) {
    return bridge().readDir(path)
  }

  return remoteFsApi<SarthiReadDirResult>(fsPath('list', path))
}

export async function readDesktopFileText(path: string): Promise<SarthiReadFileTextResult> {
  if (!isDesktopFsRemoteMode()) {
    return bridge().readFileText(path)
  }

  return remoteFsApi<SarthiReadFileTextResult>(fsPath('read-text', path))
}
...
export async function readDesktopFileDataUrlLocalFirst(path: string): Promise<string> {
  try {
    const local = await window.sarthiDesktop?.readFileDataUrl?.(path)
...
export async function selectDesktopPaths(options?: SarthiSelectPathsOptions): Promise<string[]> {
  const desktop = bridge()
