...
//
// When someone is in a hands-free "Hey Sarthi" voice chat, the natural way to
...
// phrase (optionally addressed to Sarthi), so a real turn that merely contains
...
// Optional address prefixes so "sarthi stop" / "ok stop" / "hey sarthi, stop"
// still count. Stripped before matching the core phrase.
const ADDRESS_PREFIXES: readonly string[] = ['hey sarthi', 'hey sarthi,', 'sarthi', 'sarthi,', 'ok', 'okay', 'hey']
...
function stripAddress(text: string): string {
  for (const prefix of ADDRESS_PREFIXES) {
    if (text === prefix) {
      // Bare address ("sarthi") is not a stop command on its own.
      continue
    }
 ...
 * to Sarthi). Returns false for anything that merely contains "stop" as part of
