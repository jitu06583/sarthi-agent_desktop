...
/** Why a known Sarthi command has no desktop UI surface. */
...
const DESKTOP_COMMAND_SPECS: readonly DesktopCommandSpec[] = [
  ...
  { name: '/profile', description: 'Switch the active Sarthi profile', surface: action('profile') },
  {
    name: '/skin',
  ...
  { name: '/version', description: 'Show Sarthi Agent version', surface: exec() },
...
function isKnownSarthiSlashCommand(command: string): boolean {
  const normalized = normalizeCommand(command)
 ...
 * Sarthi' built-in slash commands — i.e. skill commands (`/gif-search`,
...
export function isDesktopSlashExtensionCommand(command: string): boolean {
  ...
  return !isKnownSarthiSlashCommand(normalized)
}
 ...
 * activity — the ones that ship with Sarthi and were never asked for. It is
