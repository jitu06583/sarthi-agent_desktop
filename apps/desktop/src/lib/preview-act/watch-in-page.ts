...
export function watchInPage(doc: Document, holder: WatchHolder, stage: WatchStage, label?: string): void {
  const win = doc.defaultView as (Window & { __sarthiWatch?: Record<string, unknown> }) | null
  ...
  const busy = 25
  const state = win.__sarthiWatch || (win.__sarthiWatch = {})
  ...
  const build = () => {
    ...
    const stale = doc.querySelectorAll('sarthi-watch')

    for (let i = 0; i < stale.length; i++) {
      stale[i].remove()
    }

    const host = doc.createElement('sarthi-watch')
  ...
  // this up" in dev.
  const stamp = (win as unknown as { __sarthiWatchTag?: number }).__sarthiWatchTag
