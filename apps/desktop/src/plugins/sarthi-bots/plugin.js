/**
 * Sarthi Bot Mode — a "one chat per agent" roster for the Sarthi desktop.
 *
 * Left pane "Bots": one row per Sarthi profile (a bot = an agent profile) with
 ...
 *
 * Right tile "Routines": scheduled tasks (Sarthi cron jobs) scoped to the
 ...
 * bot-initiated sends use `sarthi -p <bot> chat --in ~ -c "Bot Chat"`.
 */

import * as sdk from '@sarthi/plugin-sdk'
import {
  atom,
  Button,
  Checkbox,
...
const GROUP_CHAT_SYNC_META_KEY = 'sarthi-bots-groups'
 ...
 *  clients (Sarthi Go, headless backends) see rooms regardless of which
...
async function saveBotMeta(name, patch) {
  ...
  try {
    ...
    serverRequest = Promise.resolve(host.request('profiles.configure', { name, ui_meta: { 'sarthi-bots': rest } }))
  } catch {
...
// handoffs (`sarthi -p <bot> chat --in ~ -c "Bot Chat" --create-if-missing`)
  ...
  for (const bot of roster) {
    ...
    if (image && typeof image === 'string' && image.startsWith('data:')) {
      ...
      host
        ...
        .then(() => queryClient.invalidateQueries({ queryKey: ['sarthi-bots', 'roster'] }))
...
function mergeServerMeta(roster, fetchedAt = 0) {
  ...
  for (const bot of roster) {
    const server = bot.ui_meta?.['sarthi-bots']
...
/** Permanently delete a bot's Sarthi profile, then remove plugin-local state
 ...
 * directory (sarthi-agent#52279). That is the "can't delete a bot" error. */
...
if (typeof document !== 'undefined' && !document.getElementById('sarthi-bots-roster-css')) {
  const style = document.createElement('style')
  style.id = 'sarthi-bots-roster-css'
  style.textContent =
    '.sarthi-bots-roster [data-radix-scroll-area-viewport] > div {' +
    ' display: block !important; width: 100%; min-width: 0; }' +
    '.sarthi-scroll-cap > [data-radix-scroll-area-viewport] { max-height: inherit; }' +
    '@keyframes sarthi-bots-pulse { 0%,100% { opacity: 1; } 50% { opacity: 0.35; } }' +
    '.sarthi-bots-pulse { animation: sarthi-bots-pulse 1.2s ease-in-out infinite; }'
...
// and the row falls back to the "run sarthi mcp / Settings" hint. profile is
...
function McpSetupButton({ profile, entry, onDone, ensureProfile }) {
  ...
  const beginOAuth = async () => {
    ...
    try {
      ...
      } else if (typeof window !== 'undefined' && window.sarthiDesktop && window.sarthiDesktop.openExternal) {
        window.sarthiDesktop.openExternal(authUrl)
      } else {
      ...
      tab === 'generate'
        ? imagen
          ...
          : jsx('div', {
              ...
              children:
                imagen === false
                  ? 'No image model available. If you just enabled one (or updated Sarthi), restart the gateway: Ctrl+K → "Restart gateway".'
...
function PetTab({ image, onImage }) {
  ...
  if (!pets.length) {
    return jsx('div', {
      ...
      children: 'No pets in the petdex gallery. Run `sarthi pets` to explore.'
    })
  }
 ...
 *  protocol into the system prompt itself (sarthi-agent bot_mode_probe).
...
function useRoster() {
  ...
  return useQuery({
    ...
    queryFn: async () => {
      ...
      // Multi-source desktops (sarthi-agent #86875) also expose the union
 ...
 *  callable alias is 'sarthi' — the mention middleware resolves it back to
...
function botHandle(name, bot) {
  ...
  return (name || '').trim().toLowerCase() === 'default' ? 'sarthi' : name
}

/** Taggable @-forms derived from a bot's friendly names — the core profile
 *  display name (`sarthi profile rename`) and the Bot Mode title. Free text
 ...
 *  (researchbuddy). Reserved tokens are dropped so a bot renamed "Sarthi"
 *  can never hijack the primary profile's @sarthi alias. */
function mentionNameForms(value) {
  ...
  return [...new Set([slug, collapsed])].filter(
    form => /^[a-z0-9][a-z0-9_-]*$/.test(form) && !['all', 'everyone', 'user', 'default', 'sarthi'].includes(form)
  )
}
...
function botFriendlyNames(bot) {
  ...
  return [bot?.ui_meta?.['sarthi-bots']?.title, localTitle, bot?.title, bot?.display_name]
}
...
function resolveRosterMentions(text, roster, active = {}) {
  ...
  for (const match of prose.matchAll(/(^|\s)@([a-z0-9][a-z0-9_-]*)/gi)) {
    ...
    if (token === 'sarthi') {
      token = byForm.has('sarthi') ? 'sarthi' : token
    }
...
// grandfather, or recover. Stored-id pins (ui_meta['sarthi-bots'].chat) were
...
async function openStoredBotChat(name, storedId, summary) {
  if (!storedId || typeof host.openSession !== 'function') {
    throw new Error('This Sarthi Desktop version cannot open stored sessions')
  }
  ...
  // even though the session is fine (sarthi-agent#89617) — clicking Retry
...
async function prepareBotSource(bot) {
  ...
  if (typeof host.ensureAgent !== 'function') {
    throw new Error('Update Sarthi Desktop to chat with agents on other connections.')
  }
...
function displayName(bot, meta) {
  ...
  // "Sarthi". Annotated active rows carry sourceScoped too, and keying this
  ...
  // Core-profile display name (profile.yaml, set via `sarthi profile rename
  ...
  // that reads like nobody bothered. Present it as Sarthi (the agent it is)
...
function durableGroupChatMembers(bots) {
  return (bots || []).map(bot => {
    ...
    const title = String(botRosterMeta(bot, $botMeta.get())?.title || bot.ui_meta?.['sarthi-bots']?.title || bot.title || '').trim()
...
// turn in its OWN persistent per-group Sarthi session and is fed only the
 ...
 *  literally named "default" — render it as Sarthi (matching displayName and
 *  the @sarthi handle) so the main agent never loses its name in rooms. */
...
function messagingProtocolSection(name, roster) {
  ...
  return [
    ...
    '',
    '```',
    'sarthi -p <agent-name> chat --in ~ -c "Bot Chat" --create-if-missing -Q -q "Message from \uD83E\uDD16 ' + handle + ' (@' + handle + '): your message"',
    '',
    ...
    '',
    'The roster grows over time — run `sarthi profile list` for the LIVE',
...
function previewKind(preview) {
  ...
  if (match) {
    ...
    // profile reads @sarthi, never @default (#89484).
 ...
 *  (sarthi-agent#90268). Older gateways omit worker_session — always false. */
        ...
        children: [
          jsxs('div', {
            ...
            children: [
              ...
              activeNow
                ? jsx('span', {
                    className: 'sarthi-bots-pulse size-1.5 shrink-0 rounded-full bg-(--ui-accent,#4f9cf9)',
...
function AdvancedProfileConfig({ bot, state, setState }) {
  ...
  if (unsupported) {
    return jsx('div', {
      className: 'px-2 py-3 text-center text-xs text-(--ui-text-tertiary)',
      children: 'Full configuration needs a newer gateway (restart it after updating Sarthi).'
    })
  }
  ...
  // (sarthi-agent#87317): Skills (installed list + one-click hub installs +
...
// {type: 'sarthi-skill-pick', ...} to us (sarthi-agent#86243). We validate
...
function HubSkillsSection({ forProfile, onInstalled }) {
  ...
  useEffect(() => {
    ...
    const onMessage = event => {
      ...
      if (!data || data.type !== 'sarthi-skill-pick' || !data.name) {
        return
      }
  ...
  const ensureAgentCreated = () => {
    ...
    const flight = singleFlight(flightRef, async () => {
      ...
      if (remoteTarget) {
        ...
        try {
          void requestForTarget('profiles.configure', { name: slug, ui_meta: { 'sarthi-bots': look } }).catch(() => undefined)
                    ...
                    advTab === 'general'
                      ...
                      : capsFailed
                        ? jsx('div', {
                            className: 'px-2 py-3 text-center text-xs text-(--ui-text-tertiary)',
                            children:
                              'Capability catalog needs a newer gateway (restart it after updating Sarthi).'
                          })
                        : !caps
                          ? jsx('div', {
                              className: 'flex justify-center py-4',
...
// sarthi -p <bot> chat delegation wrapper so the run reaches that bot's
...
function routinePrompt(bot, title, instruction, activeProfile) {
  ...
  return (
    ...
    `sarthi -p ${shellQuote(bot)} chat -c ${shellQuote(`Routine: ${title}`)} -q ${shellQuote(`[Scheduled routine] ${instruction}`)}\n\n` +
...
// Sarthi-native schedule string; Advanced exposes it raw.
const FREQUENCIES = [
...
/** Compose the Sarthi schedule string from picker state. */
...
function GroupChatWorkspace({ group, members, onBack, visible = true }) {
  ...
  const renderEntry = (entry, index) => {
                  ...
                  // primary "default" profile renders as Sarthi, remote dupes
...
function GroupRow({ active, group, members, needsYou, onOpen, onDisband }) {
  ...
  // and the group prompt: the primary profile is @sarthi, not @default.
...
function BotsPane() {
  ...
  const activityOf = bot => {
    const created = botRosterMeta(bot, allMeta)?.created || bot.ui_meta?.['sarthi-bots']?.created || 0
        ...
        : error && !roster.length
          ? jsxs('div', {
              ...
              children: [
                jsx('div', {
                  children: gatewayUp
                    ? `Roster unavailable: ${error instanceof Error ? error.message : 'gateway error'}. If your gateway predates profiles.list, update Sarthi and restart the gateway.`
          ...
          : roster.length === 0
            ...
            : filteredRoster.length === 0 && rosterRows.length === 0
              ...
              : jsx(ScrollArea, {
                  className: 'sarthi-bots-roster min-h-0 flex-1',
                  children: jsx('div', {
        ...
        description: deleting
          ? jsxs('span', {
              children: [
                ...
                ' and its associated Sarthi profile at ',
...
export default {
  ...
  register(ctx) {
    ...
    // excluded (a bot doesn't @ itself); 'default' surfaces as @sarthi.
    ctx.register({
    ...
    if (!document.getElementById('sarthi-bots-keyframes')) {
      const style = document.createElement('style')
      style.id = 'sarthi-bots-keyframes'
      style.textContent = '@keyframes sarthi-bots-bob { from { transform: translateY(0); } to { transform: translateY(-3px); } }'
      document.head.appendChild(style)
    }
