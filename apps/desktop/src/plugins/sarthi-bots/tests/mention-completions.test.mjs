...
function loadProvide({ roster, active = 'default', focused = active, meta = {} } = {}) {
  ...
  const context = {
    ...
    queryClient: {
      ...
      // key-ignoring mock.
      getQueryData: key =>
        key.length === 3 && key[0] === 'sarthi-bots' && key[1] === 'roster'
          ? roster
          : undefined,
      getQueriesData: () => (Array.isArray(roster?.profiles) ? [[['sarthi-bots', 'roster', 'local'], roster]] : []),
...
test('offers roster handles for a matching prefix, excluding the active profile', () => {
  ...
  assert.ok(inserts.includes('@sarthi'), 'default surfaces as @sarthi')
