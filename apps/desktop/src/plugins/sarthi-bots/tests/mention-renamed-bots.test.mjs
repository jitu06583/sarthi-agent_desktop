...
// Bot Mode title or `sarthi profile rename` display_name — must update what
...
test('mentionNameForms: slugged + collapsed, reserved tokens dropped', () => {
  ...
  // A bot renamed "Sarthi" or "everyone" cannot hijack reserved tags.
  assert.equal(mentionNameForms('Sarthi').length, 0)
...
test('botMentionTag: renamed slug wins, profile handle is the fallback', () => {
  ...
  assert.equal(botMentionTag({ name: 'default' }), 'sarthi')
  // display_name (sarthi profile rename) drives the tag too.
...
test('parseGroupChatMentions: display_name and ui_meta title forms address a member', () => {
  ...
  const members = [
    ...
    { name: 'ops', title: '', ui_meta: { 'sarthi-bots': { title: 'Night Watch' } } }
  ]
