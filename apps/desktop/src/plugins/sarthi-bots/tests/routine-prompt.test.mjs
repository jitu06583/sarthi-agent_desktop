...
test('integration: a different active profile retains the delegated routine wrapper', () => {
  ...
  assert.match(prompt, /sarthi -p 'research' chat/)
...
test('security: delegated routine arguments remain literal shell values', () => {
  ...
  const command = prompt.slice(prompt.indexOf('sarthi '), prompt.lastIndexOf('\n\nIf the command'))
  const script = `sarthi() { printf '%s\\037' "$@"; }\n${command}`
...
test('security: upgrade pauses persisted delegated routines before they can execute', async () => {
  const titleSentinel = `/tmp/sarthi-bot-mode-title-${process.pid}`
  const taskSentinel = `/tmp/sarthi-bot-mode-task-${process.pid}`
  ...
  const oldPrompt =
    ...
    `sarthi -p research chat -c "Routine: ${title}" -q ${JSON.stringify(`[Scheduled routine] ${instruction}`)}\n\n` +
  ...
  const command = recreated.slice(recreated.indexOf('sarthi '), recreated.lastIndexOf('\n\nIf the command'))
  const script = `sarthi() { printf '%s\\037' "$@"; }\n${command}`
...
test('regression: Create Cronjob passes the active profile to routinePrompt', () => {
  ...
  assert.doesNotMatch(__routines.routinePrompt('ops', 'Check', instruction, 'ops'), /sarthi -p/)
})
