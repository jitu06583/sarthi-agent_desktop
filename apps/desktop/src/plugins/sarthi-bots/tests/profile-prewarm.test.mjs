    ...
    host: {
      ...
      request: async method =>
        method === 'profiles.list'
          ? { profiles: [{ name, ui_meta: { 'sarthi-bots': { chat: 'owner-chat' } } }] }
          : { sessions: [] },
      notify: () => undefined,
...
test('behavior: remote default does not open this-device chat when the source did not activate', async () => {
  ...
  const context = {
    ...
    host: {
      ...
      request: async () => ({ profiles: [{ name: 'default', ui_meta: { 'sarthi-bots': { chat: 'this-device-chat' } } }] }),
      notify: () => undefined,
