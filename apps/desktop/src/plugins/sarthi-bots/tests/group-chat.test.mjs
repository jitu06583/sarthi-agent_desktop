...
function load(turnScript, { busyUntilResumeCall, clarifyUntilResumeCall, approvalUntilResumeCall, conflictOnce = false, deferredTimers = false } = {}) {
      ...
      request: async (method, params) => {
        ...
        if (method === 'profiles.configure') {
          if (conflictOnce && !injectedConflict) {
            ...
            sharedUiMeta['sarthi-bots-groups'] = {
              version: 2,
              rooms: {
                Shared: {
                  revision: 1,
            ...
            uiMetaRevisions['sarthi-bots-groups'] = 1
            return {
              applied: {
                ui_meta: false,
                ui_meta_conflicts: { 'sarthi-bots-groups': { expected: 0, actual: 1 } },
                ui_meta_revisions: { 'sarthi-bots-groups': 1 }
              }
            }
          }
  ...
  const source = pluginSource
    .replace(/^import\s+\*\s+as\s+sdk\s+from '@sarthi\/plugin-sdk'\r?\n/m, '')
    .replace(/^import\s+\{[\s\S]*?\}\s+from '@sarthi\/plugin-sdk'\r?\n/m, '')
...
test('group room messages and members mirror through bounded gateway profile metadata', async () => {
  ...
  const envelope = configure.params.ui_meta['sarthi-bots-groups']
...
test('an explicit final-room disband may clear the gateway room mirror', async () => {
  ...
  const configure = gc.requests.filter(call => call.method === 'profiles.configure').at(-1)
  assert.deepEqual(Object.keys(configure.params.ui_meta['sarthi-bots-groups'].rooms), [])
  assert.ok(configure.params.ui_meta['sarthi-bots-groups'].deleted['name:Research'] > 0)
})
...
test('sync worker retries a gateway CAS conflict and publishes the merged room', async () => {
  ...
  for (let i = 0; i < 20 && (gc.uiMetaRevisions['sarthi-bots-groups'] || 0) < 2; i++) {
    await new Promise(resolve => setImmediate(resolve))
  }

  const stored = gc.sharedUiMeta['sarthi-bots-groups']
  assert.equal(gc.uiMetaRevisions['sarthi-bots-groups'], 2)
  assert.equal(
...
test('fan-out class: a room write is mirrored to every reachable default-profile gateway', async () => {
  ...
  gc.host.requestProfile = async (route, method) => {
    ...
    if (method === 'profiles.configure') {
      return { applied: { ui_meta: true, ui_meta_revisions: { 'sarthi-bots-groups': 1 } } }
    }
    return {}
  }
...
test('default profile speaks as Sarthi in room transcripts, not @default', () => {
  const gc = load(() => '(pass)')
  const line = gc.formatGroupChatLine({ from: { kind: 'member', name: 'default' }, text: 'hello room' }, 'builder')
  assert.equal(line, 'Sarthi: hello room')
...
test('turn prompt addresses the default profile as @sarthi', () => {
  ...
  assert.match(prompt, /You are @sarthi,/)
  ...
  assert.match(peerView, /group chat with @sarthi/)
})

test('mention routing: @sarthi resolves to the default member', () => {
  const gc = load(() => '(pass)')
  const members = [{ name: 'default', title: '' }, { name: 'builder', title: '' }]
  const parsed = gc.parseGroupChatMentions('@sarthi take a look', members)
...
test('source contract: workspace speaker labels use displayName with a click-to-reveal handle', () => {
  // Speaker labels come from the roster displayName (default → Sarthi)…
...
test('group room preview renders the bot HANDLE, not the raw profile name', () => {
  ...
  // @sarthi, so users concluded mention routing was broken.
