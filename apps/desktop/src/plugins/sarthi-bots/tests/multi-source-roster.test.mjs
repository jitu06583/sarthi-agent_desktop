...
function runtime() {
  ...
  const code = source
    .replace(/^import \* as sdk from '@sarthi\/plugin-sdk'\r?\n/m, '')
...
test('default rows use source identity without borrowing another source title', () => {
  ...
  // label. Titled: the title wins. Untitled: it stays "Sarthi". Regression:
  ...
  assert.equal(name(active, undefined), 'Sarthi')
})

test('botHandle: precomputed multi-source handle wins; default stays sarthi', () => {
  ...
  assert.equal(botHandle('default'), 'sarthi')
})
...
test('displayName: local default stays Sarthi; remote default uses the device label', () => {
...
test('resolveRosterMentions: @sarthi in this chat is not a handoff to yourself', () => {
  const { __resolveRosterMentions: resolve } = runtime()
  const roster = [
    { name: 'default', connectionId: 'local', handle: 'sarthi' },
    {
      name: 'default',
      connectionId: 'mac-mini',
      connectionLabel: 'Mac Mini',
  ...
  const hits = resolve('ask @sarthi and @default-mac-mini', roster, {
    name: 'default',
    connectionId: 'local'
  })
