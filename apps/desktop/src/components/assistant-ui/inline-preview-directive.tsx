 ...
 * own design.
 *
 * WIDGETS TALK BACK OFF-SCREEN. `window.sarthi.send(prompt)` (or declarative
 * `data-sarthi-send` on any clickable element) routes the prompt through the
...
const SIZE_MESSAGE_TYPE = 'sarthi-inline-preview-size'
const INTENT_MESSAGE_TYPE = 'sarthi-inline-preview-intent'
...
/** The script that gives the widget its ONE voice: `sarthi.send(prompt)`.
 ...
 *  wires `data-sarthi-send` so declarative HTML works with zero script:
 *  `<button data-sarthi-send="get-price eth">ETH</button>`. */
export function intentScript(token: string): string {
  return (
    ...
    ')},"*");return true}' +
    'window.sarthi={send:send};' +
    'addEventListener("click",function(e){var el=e.target&&e.target.closest?' +
    'e.target.closest("[data-sarthi-send]"):null;' +
    'if(el)send(el.getAttribute("data-sarthi-send")||"")},true)})()</script>'
  )
}
...
function InlineHtmlFrame({
  ...
  useEffect(() => {
    ...
    void Promise.resolve(window.sarthiDesktop?.readFileText(path))
      .then(result => {
        if (!alive) {
          return
        }
