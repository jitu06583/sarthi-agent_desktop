...
const SARTHI_REF_TYPES = ['file', 'folder', 'url', 'image', 'tool', 'line', 'terminal', 'session'] as const
type SarthiRefType = (typeof SARTHI_REF_TYPES)[number]
...
const ICON_PATHS: Record<SarthiRefType, string[]> = {
  file: [
...
const iconPathsFor = (type: string) => ICON_PATHS[type as SarthiRefType] ?? ICON_FALLBACK
...
const SARTHI_DIRECTIVE_RE = new RegExp(
...
export const sarthiDirectiveFormatter: Unstable_DirectiveFormatter = {
...
function parseDirectiveText(text: string): Unstable_DirectiveSegment[] {
  const matches = [
    ...
    ...Array.from(text.matchAll(SARTHI_DIRECTIVE_RE)).map(match => {
      ...
      return {
        ...
        type: match[1] || 'file',
        label: shortLabel(match[1] as SarthiRefType, id),
        id
      }
    })
  ]
...
function shortLabel(type: SarthiRefType, id: string): string {
  if (type === 'terminal') {
    return id || 'terminal'
  }
...
function safeDirectiveSegments(text: string): Unstable_DirectiveSegment[] {
  try {
    return [...sarthiDirectiveFormatter.parse(text)]
  } catch {
...
/**
 * Renders text containing Sarthi directives (`@file:...`, `@image:...`) as
...
const DirectiveImage: FC<{ id: string; label: string }> = ({ id, label }) => {
  ...
  useEffect(() => {
    ...
    const load =
      window.sarthiDesktop && isRemoteGateway() ? gatewayMediaDataUrl(id) : window.sarthiDesktop?.readFileDataUrl(id)
