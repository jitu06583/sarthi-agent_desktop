...
const ErrorRecoveryActions: FC = () => {
  ...
  // Open Logs reveals the LOCAL Electron profile's SARTHI_HOME/logs. On a
  ...
  const openLogs = useCallback(async () => {
    try {
      const root = await window.sarthiDesktop?.logsRoot?.()
      ...
      const result = await window.sarthiDesktop?.openDir?.(root)
