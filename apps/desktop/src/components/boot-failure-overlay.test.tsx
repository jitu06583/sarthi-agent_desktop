...
function failBoot() {
  $desktopBoot.set({
    error: 'Could not connect to Sarthi gateway',
    fakeMode: false,
    message: 'boot failed',
...
function stubDesktop(config: Record<string, unknown>) {
  const original = window.sarthiDesktop
  Object.defineProperty(window, 'sarthiDesktop', {
    configurable: true,
    value: { getRecentLogs: async () => ({ lines: [] }), getConnectionConfig: async () => config }
  })

  return () => Object.defineProperty(window, 'sarthiDesktop', { configurable: true, value: original })
}
