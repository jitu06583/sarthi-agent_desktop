...
function sessionsBotsTree() {
  ...
  registerPane('sarthi-bots:pane', { placement: 'left', hideOnly: true })
  registerPane('workspace', { placement: 'main', uncloseable: true })
  $layoutTree.set(
    split('row', [
      group(['sessions', 'sarthi-bots:pane'], { active: 'sessions', id: 'g-side' }),
...
describe('hide-only strip tabs', () => {
  it('hides and shows a chrome tab, keeping the pane in the tree', () => {
    ...
    expect(setStripTabHidden('sarthi-bots:pane', true)).toBe(true)
    expect($hiddenTreePanes.get()).toContain('sarthi-bots:pane')
    expect($hiddenStripTabs.get()).toContain('sarthi-bots:pane')
    // Hidden, not dismissed: the pane stays in the layout tree.
    expect(allPaneIds($layoutTree.get()!)).toContain('sarthi-bots:pane')

    expect(setStripTabHidden('sarthi-bots:pane', false)).toBe(true)
    expect($hiddenTreePanes.get()).not.toContain('sarthi-bots:pane')
    expect($hiddenStripTabs.get()).not.toContain('sarthi-bots:pane')
  })

  it('refuses to hide the zone last visible tab', () => {
    sessionsBotsTree()
    setStripTabHidden('sarthi-bots:pane', true)
  ...
  it('persists hides and clears them on reveal', () => {
    ...
    const persisted = JSON.parse(window.localStorage.getItem('sarthi.desktop.hiddenStripTabs.v1') ?? '[]')

    expect(persisted).toContain('sarthi-bots:pane')
    ...
    revealTreePane('sarthi-bots:pane')
    expect($hiddenTreePanes.get()).not.toContain('sarthi-bots:pane')
    expect(window.localStorage.getItem('sarthi.desktop.hiddenStripTabs.v1')).toBeNull()
  })
  ...
  it('lists the zone hide-only tabs with live hidden state for the menu', () => {
    ...
    expect(hideOnlyZoneTabs('g-side')).toEqual([
      ...
      { hidden: true, id: 'sarthi-bots:pane', title: 'sarthi-bots:pane' }
    ])
  ...
  it('excludes hide-only tabs from every close verb', () => {
    ...
    $layoutTree.set(group(['sessions', 'sarthi-bots:pane', 'session-tile:x'], { active: 'sessions', id: 'g-mixed' }))
