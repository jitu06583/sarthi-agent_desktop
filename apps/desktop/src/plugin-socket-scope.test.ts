...
// (#73044). Exercises the REAL sarthi + store/gateway + store/profile chain:
...
vi.mock('@/sarthi', async importOriginal => {
...
const { pluginSocket, setApiRequestConnection, setApiRequestProfile } = await import('@/sarthi')
...
const conn = (over: Partial<SarthiConnection> = {}): SarthiConnection =>
  ({
    authMode: 'oauth',
