...
import type { SkillInfo, ToolsetInfo } from '@/types/sarthi'
...
export function SkillsView({
  ...
  const registryBridge = window.sarthiDesktop?.connections
  const rosterBridge = window.sarthiDesktop?.getAgentRoster
  ...
  const scopeOptions: { key: string; label: string; value: string }[] = useMemo(() => {
    ...
    return (profilesData?.profiles ?? []).map(p => ({
      key: p.name,
      label: p.is_default ? 'Sarthi (default)' : p.name,
      value: p.name
    }))
