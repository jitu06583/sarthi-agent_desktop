...
import { pathFromSarthiDeepLink, resolveSarthiOpenPath } from '@/lib/sarthi-open-target'
...
export function useDesktopIntegrations({
  ...
  useEffect(() => {
    ...
    const unsubscribe = window.sarthiDesktop?.onOpenUpdatesRequested?.(() => openUpdatesWindow())
  ...
  useEffect(() => {
    window.sarthiDesktop?.setPreviewShortcutActive?.(true)
  }, [])
  ...
  useEffect(() => {
    const unsubscribe = window.sarthiDesktop?.onFocusSession?.(sessionId => {
      if (sessionId) {
  ...
  useEffect(() => {
    const unsubscribe = window.sarthiDesktop?.onNotificationAction?.(({ actionId, sessionId }) => {
  ...
  // sarthi://index-network/intent/1 vocabulary with deep links.
  useEffect(() => {
    const unsubscribe = window.sarthiDesktop?.onNotificationActivate?.(payload => {
      if (!payload) {
        return
      }
      ...
      if (payload.activate) {
        ...
        // the pre-IPC validation — any future sarthiDesktop.notify caller gets
        // funneled through the same resolver.
        const path = resolveSarthiOpenPath(payload.activate)
  ...
  useEffect(() => {
    const unsubscribe = window.sarthiDesktop?.onDeepLink?.(payload => {
      if (!payload?.kind) {
        return
      }
      ...
      // link (sarthi://index-network/intent/1, sarthi://open/…). The resolver
      // rejects reserved kinds and unsafe paths.
      const path = pathFromSarthiDeepLink(payload.kind, payload.name || '', payload.params || {})
    ...
    void window.sarthiDesktop?.signalDeepLinkReady?.()
  ...
  useEffect(() => {
    const unsubscribe = window.sarthiDesktop?.onClosePreviewRequested?.(
  ...
  useEffect(() => {
    const unsubscribe = window.sarthiDesktop?.onPreviewNav?.(command => {
  ...
  useEffect(() => {
    const unsubscribe = window.sarthiDesktop?.onOpenFolderRequested?.(() => void openFolderAsProject())
