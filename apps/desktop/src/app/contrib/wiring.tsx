...
import { getLatestSessionMessages } from '@/sarthi'
...
import { useSarthiConfig } from '../session/hooks/use-sarthi-config'
...
export function ContribWiring({ children }: { children: ReactNode }) {
  ...
  const { refreshSarthiConfig, sttEnabled, voiceMaxRecordingSeconds } = useSarthiConfig({ activeSessionIdRef })
  ...
  useEffect(() => {
    ...
    window.addEventListener('sarthi:open-keybinds', onOpenKeybinds)

    return () => window.removeEventListener('sarthi:open-keybinds', onOpenKeybinds)
  }, [navigate])
  ...
  useEffect(() => {
    ...
    void refreshSarthiConfig(true)
    void refreshActiveProfile()
    resetProjectTreeState()
  }, [gatewayScope, refreshCurrentModel, refreshSarthiConfig])
