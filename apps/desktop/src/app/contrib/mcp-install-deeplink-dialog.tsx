...
import { getSarthiConfigRecord, saveMcpServers } from '@/sarthi'
...
import { setSarthiConfigCache } from '../hooks/use-config-record'

/**
 * Explicit-confirm gate for `sarthi://mcp/install` deep links. The payload is
 ...
 * confirming lets Sarthi spawn that local process. An existing server name is
...
export function McpInstallDeepLinkDialog() {
  ...
  const confirm = async () => {
    ...
    try {
      ...
      const current = getServers(await getSarthiConfigRecord())
      ...
      setSarthiConfigCache(previous => (previous ? { ...previous, mcp_servers: nextServers } : previous))
