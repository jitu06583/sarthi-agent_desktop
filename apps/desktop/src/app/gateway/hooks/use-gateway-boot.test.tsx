...
// bug that drives the REAL useGatewayBoot hook + REAL SarthiGateway through a
...
function Harness({
  ...
  useGatewayBoot({
    ...
    refreshSarthiConfig: async () => undefined,
...
beforeEach(() => {
  ...
  ;(window as { sarthiDesktop?: unknown }).sarthiDesktop = fakeDesktop()
...
afterEach(() => {
  ...
  delete (window as { sarthiDesktop?: unknown }).sarthiDesktop
...
describe('useGatewayBoot remote reconnect loop (real hook, fake socket)', () => {
  it('INITIAL boot against a dead VPS: getConnection hangs (waitForSarthi) → app sits in the connecting combo, then fails', async () => {
    // The report's actual path: a fresh launch pointed at an unreachable VPS.
    // startSarthi()'s remote branch awaits waitForSarthi() for 45s before it
    ...
    ;(window as { sarthiDesktop?: unknown }).sarthiDesktop = desktop
    ...
    // After ~45s waitForSarthi gives up and getConnection rejects → boot()
    // catch → failDesktopBoot → the BootFailureOverlay recovery surface.
    await act(async () => {
      rejectConn(new Error('Sarthi backend did not become ready: timeout'))
      await vi.advanceTimersByTimeAsync(0)
    })
  ...
  it('FIX: a failed session-list fetch during boot is non-fatal — the app still boots', async () => {
    ...
    // failDesktopBoot's "Sarthi couldn't start" overlay — the socket is open
  ...
  it('seeds the configured default project dir pre-connect — no route-resume race (#71873)', async () => {
    ...
    expect(cwdAtConnect).toBe('C:\\Sarthi')
    expect($currentCwd.get()).toBe('C:\\Sarthi')
  })
  ...
  it('FIX #82679: a transient remote boot failure self-heals — the next attempt rebuilds the dropped connection', async () => {
    ...
    // SSH/HTTP remote whose transport dropped. startSarthi() rejects with a
