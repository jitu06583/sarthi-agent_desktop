...
import type { SarthiGateway } from '@/sarthi'
...
export function useGatewayRequest() {
  ...
  const gateway = useStore($gateway) as SarthiGateway | null
  const gatewayRef = useRef<SarthiGateway | null>(null)

  const connectionRef = useRef<Awaited<ReturnType<NonNullable<typeof window.sarthiDesktop>['getConnection']>> | null>(
    null
  )

  const gatewayStateRef = useRef(gatewayState)
  const reconnectingRef = useRef<Promise<SarthiGateway | null> | null>(null)
  ...
  useEffect(
    () =>
      $gateway.subscribe(gateway => {
        gatewayRef.current = gateway as SarthiGateway | null
      }),
    []
  )
