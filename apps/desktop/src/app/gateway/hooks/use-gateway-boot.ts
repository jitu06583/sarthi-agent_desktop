import { isGatewayReauthRequired, resolveGatewayWsUrl } from '@sarthi/shared'
import { useEffect, useRef } from 'react'

import type { SarthiConnection } from '@/global'
import { SarthiGateway } from '@/sarthi'
...
import type { RpcEvent } from '@/types/sarthi'
...
interface GatewayBootOptions {
  ...
  onConnectionReady: (
    connection: Awaited<ReturnType<NonNullable<typeof window.sarthiDesktop>['getConnection']>> | null
  ) => void
  onGatewayReady: (gateway: SarthiGateway | null) => void
  refreshSarthiConfig: () => Promise<void>
...
export function useGatewayBoot({
  ...
  useEffect(() => {
    ...
    const publish = (next: SarthiConnection | null) => {
    ...
    // composer stayed disabled forever on "Starting Sarthi...". Once the
    ...
    const attemptReconnect = async () => {
      ...
      try {
        ...
        // "Starting Sarthi…". The probe is a no-op for a healthy or local backend.
        ...
        // as an opaque "Could not connect to Sarthi gateway". resolveGatewayWsUrl
        ...
        await callbacksRef.current.refreshSarthiConfig().catch(() => undefined)
    ...
    const softSwitch = async () => {
      ...
      try {
        ...
        await Promise.all([
          ...
          callbacksRef.current.refreshSarthiConfig().catch(() => undefined),
    ...
    const offBootProgress = desktop.onBootProgress(payload => {
      // Soft switch / post-boot startSarthi re-emits progress — ignore so the
    ...
    const gateway = adoptedFromHmr ? survivor!.gateway : new SarthiGateway()
    ...
    async function boot() {
      try {
        ...
        await Promise.all([
          ...
          callbacksRef.current.refreshSarthiConfig(),
          ...
          // retry it), not brick boot behind the "Sarthi couldn't start"
