...
import { getSarthiConfig } from '@/sarthi'
...
import { useSarthiConfig } from './use-sarthi-config'

vi.mock('@/sarthi', () => ({
  getSarthiConfig: vi.fn(),
  getSarthiConfigDefaults: vi.fn().mockResolvedValue({})
}))

const WORKSPACE_CWD_KEY = 'sarthi.desktop.workspace-cwd'

const mockConfig = (config: Record<string, unknown>) =>
  vi.mocked(getSarthiConfig).mockResolvedValue(config as Awaited<ReturnType<typeof getSarthiConfig>>)

describe('useSarthiConfig refreshSarthiConfig', () => {
  beforeEach(() => {
  ...
  // `agent.reasoning_effort: high` to Sarthi' built-in medium.
  it('publishes the profile default effort even when a manual pick blocks the composer reseed', async () => {
    ...
    const { result } = renderHook(() => useSarthiConfig({ activeSessionIdRef: { current: null } }))

    await act(async () => {
      await result.current.refreshSarthiConfig()
    })
  ...
  it('does not let terminal.cwd replace an active session workspace', async () => {
    ...
    mockConfig({ terminal: { cwd: '/Users/example/new-workspace' } })
    const { result } = renderHook(() => useSarthiConfig({ activeSessionIdRef: { current: 'session-1' } }))
  ...
  it('does not let a stale forced config refresh overwrite newer draft selector intent', async () => {
    const profileConfig = deferred<Awaited<ReturnType<typeof getSarthiConfig>>>()
    vi.mocked(getSarthiConfig).mockReturnValueOnce(profileConfig.promise)

    const { result } = renderHook(() => useSarthiConfig({ activeSessionIdRef: { current: null } }))

    let pendingRefresh!: Promise<void>
    act(() => {
      pendingRefresh = result.current.refreshSarthiConfig(true)
    })
    expect(getSarthiConfig).toHaveBeenCalled()
    ...
    } as Awaited<ReturnType<typeof getSarthiConfig>>)
  ...
  it('does not let an older profile config overwrite a newer profile', async () => {
    const profileB = deferred<Awaited<ReturnType<typeof getSarthiConfig>>>()
    const profileC = deferred<Awaited<ReturnType<typeof getSarthiConfig>>>()
    vi.mocked(getSarthiConfig).mockReturnValueOnce(profileB.promise).mockReturnValueOnce(profileC.promise)
    ...
    act(() => {
      refreshB = result.current.refreshSarthiConfig(true)
      refreshC = result.current.refreshSarthiConfig(true)
    })
