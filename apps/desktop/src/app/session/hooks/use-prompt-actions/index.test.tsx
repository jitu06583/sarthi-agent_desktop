import { JsonRpcGatewayError } from '@sarthi/shared'
import { act, cleanup, render, waitFor } from '@testing-library/react'
import type { MutableRefObject } from 'react'
import { useEffect, useRef } from 'react'
import { afterEach, beforeEach, describe, expect, it, vi } from 'vitest'

import { getSession } from '@/sarthi'
...
// it, rather than reaching for Sarthi's own browser and panes.
  ...
  it('uploads file bytes via file.attach on a remote gateway and submits the rewritten ref', async () => {
    ...
    const requestGateway = vi.fn(async (method: string, params?: Record<string, unknown>) => {
      ...
      if (method === 'file.attach') {
        return {
          attached: true,
          path: '/remote/work/.sarthi/desktop-attachments/report.txt',
          ref_text: '@file:.sarthi/desktop-attachments/report.txt',
          uploaded: true
        } as never
      }
    ...
    expect(calls[1]?.params).toEqual({
      session_id: RUNTIME_SESSION_ID,
      text: '@file:.sarthi/desktop-attachments/report.txt\n\nconvert this to epub'
    })
  })
  ...
  it('uploads Windows file bytes when local mode fronts a POSIX WSL/Docker backend', async () => {
    ...
    const requestGateway = vi.fn(async (method: string, params?: Record<string, unknown>) => {
      ...
      if (method === 'file.attach') {
        return {
          attached: true,
          path: '/root/.sarthi/desktop-attachments/report.txt',
          ref_text: '@file:.sarthi/desktop-attachments/report.txt',
          uploaded: true
        } as never
      }
    ...
    expect(calls[1]).toEqual({
      ...
      params: { session_id: RUNTIME_SESSION_ID, text: '@file:.sarthi/desktop-attachments/report.txt\n\nsummarize' }
    })
  })
  ...
  it('uploads file bytes when the terminal backend is a container (docker)', async () => {
    ...
    const requestGateway = vi.fn(async (method: string, params?: Record<string, unknown>) => {
      ...
      if (method === 'file.attach') {
        return {
          attached: true,
          path: '/root/.sarthi/attachments/report.txt',
          ref_text: '@file:/root/.sarthi/attachments/report.txt',
          uploaded: true
        } as never
      }
...
describe('usePromptActions eager-upload races', () => {
  ...
  it('joins an in-flight eager upload at submit instead of staging the file twice', async () => {
    ...
    // under .sarthi/desktop-attachments/. Submit must await the in-flight upload
    ...
    const requestGateway = vi.fn(async (method: string) => {
      ...
      if (method === 'file.attach') {
        ...
        return { attached: true, ref_text: '@file:.sarthi/desktop-attachments/doc.pdf', uploaded: true } as never
      }
  ...
  it('still aborts when the user genuinely switches chats after create, during attachment sync (#62805)', async () => {
    ...
    const requestGateway = vi.fn(async (method: string, params?: Record<string, unknown>) => {
      ...
      if (method === 'file.attach') {
        ...
        return {
          attached: true,
          ref_text: '@file:.sarthi/desktop-attachments/test.pdf',
          uploaded: true
        } as never
      }
...
describe('usePromptActions eager attachment upload (drop-time)', () => {
  ...
  it('uploads a dropped file the moment it lands (active session) and rewrites the chip with the gateway ref', async () => {
    ...
    $connection.set({ mode: 'remote' } as never)
    const readFileDataUrl = vi.fn(async () => 'data:application/pdf;base64,JVBERi0=')
    Object.defineProperty(window, 'sarthiDesktop', { configurable: true, value: { readFileDataUrl } })
    ...
    const requestGateway = vi.fn(async (method: string) => {
      ...
      if (method === 'file.attach') {
        return {
          attached: true,
          ref_text: '@file:.sarthi/desktop-attachments/DEVIS_signed.pdf',
          uploaded: true
        } as never
      }
    ...
    expect(chip.refText).toBe('@file:.sarthi/desktop-attachments/DEVIS_signed.pdf')
