import type { BillingBlock } from '@sarthi/shared'
 ...
 * billing-specific toast — never the generic "Sarthi error" — with a smart CTA
  ...
  if (event.type === 'message.complete') {
    ...
    const failure =
      payload?.status === 'error'
        ? {
            error: coerceGatewayText(payload.error).trim() || finalText || 'Sarthi reported an error',
