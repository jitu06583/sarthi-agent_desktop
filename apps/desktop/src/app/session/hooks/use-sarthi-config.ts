...
import { getSarthiConfig, getSarthiConfigDefaults } from '@/sarthi'
...
interface SarthiConfigOptions {
  activeSessionIdRef: MutableRefObject<string | null>
}

export function useSarthiConfig({ activeSessionIdRef }: SarthiConfigOptions) {
  const [voiceMaxRecordingSeconds, setVoiceMaxRecordingSeconds] = useState(DEFAULT_VOICE_SECONDS)
  const [sttEnabled, setSttEnabled] = useState(true)
  const profileRefreshEpochRef = useRef(0)

  const refreshSarthiConfig = useCallback(
    async (force = false) => {
      if (force) {
      ...
      try {
        const [config, defaults] = await Promise.all([getSarthiConfig(), getSarthiConfigDefaults().catch(() => ({}))])
        ...
        // rendering/applying Sarthi' built-in medium over the user's config.
  ...
  return { refreshSarthiConfig, sttEnabled, voiceMaxRecordingSeconds }
}
