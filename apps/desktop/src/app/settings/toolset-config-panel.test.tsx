...
import type { ToolsetConfig } from '@/types/sarthi'
...
const getSarthiConfigSchema = vi.fn()
...
vi.mock('@/sarthi', () => ({
  ...
  getSarthiConfigSchema: () => getSarthiConfigSchema(),
...
beforeEach(() => {
  ...
  getSarthiConfigRecord.mockResolvedValue({
    tts: {
      provider: 'edge',
  ...
  getSarthiConfigSchema.mockResolvedValue({ fields: {}, category_order: [] })
...
describe('ToolsetConfigPanel', () => {
  it('renders inline voice/model fields for a TTS provider row carrying tts_provider', async () => {
    ...
    await waitFor(() => expect(saveSarthiConfig).toHaveBeenCalled(), { timeout: 3000 })
    const saved = saveSarthiConfig.mock.calls.at(-1)?.[0] as Record<string, Record<string, Record<string, string>>>
