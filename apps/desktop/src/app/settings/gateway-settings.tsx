...
// Sarthi Cloud discovery lifecycle for the cloud-mode panel.
...
export function GatewaySettings({ embedded = false }: { embedded?: boolean } = {}) {
  ...
  useEffect(() => {
    let cancelled = false
    const desktop = window.sarthiDesktop
  ...
  useEffect(() => {
    if (state.mode !== 'ssh' || !window.sarthiDesktop?.sshConfigHosts) {
      return
    }

    let cancelled = false
    void window.sarthiDesktop
      .sshConfigHosts()
      .then(result => {
        if (!cancelled) {
  ...
  const payload = (allowPlainTextToken?: boolean) => ({
    ...
    sshRemoteSarthiPath: state.sshRemoteSarthiPath.trim(),
  ...
  const performSave = async (apply: boolean, allowPlainTextToken: boolean) => {
    ...
    try {
      const next = apply
        ? await window.sarthiDesktop.applyConnectionConfig(payload(allowPlainTextToken))
        : await window.sarthiDesktop.saveConnectionConfig(payload(allowPlainTextToken))
      ...
      const errors = {
        ...
        'sarthi-not-found': g.sshErrNotInstalled,
  ...
  const signIn = async () => {
    ...
    try {
      ...
      const saved = await window.sarthiDesktop.saveConnectionConfig({
        mode: state.mode,
      ...
      const result = await window.sarthiDesktop.oauthLoginConnectionConfig(trimmedUrl)
      ...
      if (result.connected) {
        const refreshed = await window.sarthiDesktop.getConnectionConfig(null)
  ...
  const signOut = async () => {
    ...
    try {
      await window.sarthiDesktop.oauthLogoutConnectionConfig(trimmedUrl || undefined)
      const refreshed = await window.sarthiDesktop.getConnectionConfig(null)
  ...
  const resolveSshHost = async (host: string) => {
    if (!host || !window.sarthiDesktop?.sshResolveHost) {
      return
    }
    ...
    try {
      const resolved = await window.sarthiDesktop.sshResolveHost(host)
  ...
  const testSsh = async () => {
    ...
    try {
      const result = await window.sarthiDesktop.testConnectionConfig(payload())
  ...
  const testRemote = async () => {
    ...
    try {
      const result = await window.sarthiDesktop.testConnectionConfig({
        mode: 'remote',
        remoteAuthMode: authMode,
        remoteToken: authMode === 'token' ? remoteToken.trim() || undefined : undefined,
        remoteUrl: trimmedUrl
  ...
  if (!window.sarthiDesktop?.getConnectionConfig) {
  ...
  return (
    <SettingsContent bare={embedded}>
      ...
      {/* Sarthi Cloud panel: one portal sign-in, then a discovered-agent picker
          ...
          <ListRow
            action={
              <Input
                ...
                placeholder="https://gateway.example.com/sarthi"
        ...
        <div className="mt-5 grid gap-1">
          ...
          <ListRow
            action={
              <Input
                className={cn('h-8 font-mono', CONTROL_TEXT)}
                onChange={event => setState(current => ({ ...current, sshRemoteSarthiPath: event.target.value }))}
                placeholder={g.sshSarthiPathPlaceholder}
                value={state.sshRemoteSarthiPath}
              />
            }
            description={g.sshSarthiPathDesc}
            title={g.sshSarthiPathTitle}
          />
        </div>
      ) : null}
      ...
      {embedded ? null : (
        <div className="mt-6 grid gap-1">
          <ListRow
            action={
              <Button onClick={() => void window.sarthiDesktop?.revealLogs()} size="sm" variant="textStrong">
                <FileText />
                {g.openLogs}
              </Button>
            }
