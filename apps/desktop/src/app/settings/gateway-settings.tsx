...
export function GatewaySettings({ embedded = false }: { embedded?: boolean } = {}) {
  ...
  const save = async (apply: boolean) => {
    ...
    try {
      const next = apply
        ? await window.sarthiDesktop.applyConnectionConfig(payload())
        : await window.sarthiDesktop.saveConnectionConfig(payload())
  ...
  const signIn = async () => {
    ...
    try {
      ...
      if (result.connected) {
        const refreshed = await window.sarthiDesktop.getConnectionConfig(scope)
        setState(refreshed)
