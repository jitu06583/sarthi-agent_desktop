...
import { getProfiles } from '@/sarthi'
...
// User-installed plugins first — mirrors `sarthi plugins list --user`.
...
function reveal(file: string) {
  void window.sarthiDesktop?.revealPath?.(file)?.catch(() => undefined)
}

async function revealPluginsDir() {
  try {
    // Electron owns the local plugin root — deriving it from the backend's
    // sarthi_home breaks against a remote backend (#66899).
    const dir = await window.sarthiDesktop?.desktopPluginsRoot?.()
    ...
    const result = await window.sarthiDesktop?.openDir?.(dir)
...
// Agent plugins live under the BACKEND's sarthi home (profile-aware), so the
// path comes from the gateway — not from the renderer's local SARTHI_HOME.
...
async function revealAgentPluginsDir(request: GatewayRequest) {
  try {
    ...
    const opened = await window.sarthiDesktop?.openDir?.(`${home}/plugins`)
            ...
            <SelectContent>
              {profiles.map(profile => (
                <SelectItem key={profile.name} value={profile.name}>
                  {profile.is_default ? 'Sarthi (default)' : profile.name}
                </SelectItem>
              ))}
            </SelectContent>
          </Select>
