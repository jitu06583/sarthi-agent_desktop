...
import { disconnectOAuthProvider, listOAuthProviders } from '@/sarthi'
...
import type { EnvVarInfo, OAuthProvider } from '@/types/sarthi'
...
const canRunInTerminal = () => typeof window !== 'undefined' && Boolean(window.sarthiDesktop?.terminal)
...
//      in sarthi_cli/provider_catalog.py) — the SAME provider identity
//      `sarthi model` uses. This is authoritative: a provider tagged by the
...
function ConnectedProviderRow({
  ...
  // Sarthi can clear this provider's creds via the API.
  const canDisconnect = provider.disconnectable ?? provider.flow !== 'external'
  // External (CLI-managed) provider Sarthi can't clear via the API, but ships a
...
export function ProvidersSettings({ onClose, onViewChange, view }: ProvidersSettingsProps) {
  ...
  // Sarthi never deletes creds another tool owns behind a silent API call.
