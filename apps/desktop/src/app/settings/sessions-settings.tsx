...
import type { SarthiConfigRecord, SessionInfo } from '@/types/sarthi'
...
function AutoArchiveSetting() {
  ...
  const persist = useCallback(
    async (autoArchive: boolean, archiveDays: number) => {
      ...
      try {
        await saveSarthiConfig(updated)
      } catch (err) {
...
// / Program Files), which buried any files Sarthi wrote there.
function DefaultProjectDirSetting() {
  ...
  useEffect(() => {
    ...
    // / Storybook / non-Electron context `window.sarthiDesktop` is
    ...
    const settings = window.sarthiDesktop?.settings
