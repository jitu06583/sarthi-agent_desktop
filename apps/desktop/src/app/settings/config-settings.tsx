...
import { getElevenLabsVoices, getSarthiConfigSchema, saveSarthiConfig } from '@/sarthi'
...
import type { ConfigFieldSchema, SarthiConfigRecord } from '@/types/sarthi'

import { sarthiConfigCacheWriter, useSarthiConfigRecord } from '../hooks/use-config-record'
...
function ConfigSettingsInner({
  ...
  const [config, setConfig] = useState<SarthiConfigRecord | null>(null)
  const { data: loadedConfig, isError: configLoadFailed, refetch: refetchConfig } = useSarthiConfigRecord(scopeProfile)
  ...
  const writeConfigCache = useMemo(() => sarthiConfigCacheWriter(scopeProfile), [scopeProfile])
  ...
  const {
    ...
    queryKey:
      scopeProfile == null ? ['sarthi-config-schema'] : ['sarthi-config-schema', normalizeProfileKey(scopeProfile)],
    queryFn: () => getSarthiConfigSchema(scopeProfile ?? undefined),
    staleTime: 5 * 60 * 1000
  })
  ...
  useEffect(() => {
    ...
    const t = window.setTimeout(() => {
      void (async () => {
        try {
          const result = await saveSarthiConfig(config, scopeProfile ?? undefined)
  ...
  const applyConfig = (next: SarthiConfigRecord) => {
    saveVersionRef.current += 1
    setConfig(next)
    setSaveVersion(saveVersionRef.current)
  }

  const updateConfig = (next: SarthiConfigRecord) => {
