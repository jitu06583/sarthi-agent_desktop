...
import { invalidateSarthiConfig, setSarthiConfigCache, useSarthiConfigRecord } from '../hooks/use-config-record'
...
// Sarthi' reasoning levels (VALID_REASONING_EFFORTS); `none` = thinking off.
// Empty config = Sarthi default (medium), shown as Medium.
...
// backend now surfaces the full `sarthi model` universe (every canonical
...
// Mirrors `_AUX_TASK_SLOTS` in sarthi_cli/web_server.py. Friendly labels and
...
export function ModelSettings({ onMainModelChanged }: ModelSettingsProps) {
  ...
  const { data: config } = useSarthiConfigRecord()
  const setConfig = setSarthiConfigCache
  ...
  const activateApiKeyProvider = useCallback(async () => {
    ...
    try {
      ...
      // `sarthi model` curation). Best-effort — fall through to the refreshed
        ...
        {needsSetup && !setupIsApiKey && (
          <p className="mt-2 text-xs text-muted-foreground">
            {selectedProviderRow?.auth_type === 'api_key'
              ...
              : `${selectedProviderRow?.name} signs in through your browser — Sarthi runs the flow for you.`}
          </p>
        )}
