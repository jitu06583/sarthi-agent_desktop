...
import { sarthiConfigCacheWriter, invalidateSarthiConfig, useSarthiConfigRecord } from '../hooks/use-config-record'
...
export function ModelSettings({ onMainModelChanged, scopeProfile = null }: ModelSettingsProps) {
  ...
  const { data: config } = useSarthiConfigRecord(scopeProfile)
  const setConfig = useMemo(() => sarthiConfigCacheWriter(scopeProfile), [scopeProfile])
  ...
  const refresh = useCallback(
    async ({ replaceSelection = false }: { replaceSelection?: boolean } = {}) => {
      ...
      try {
        ...
        void invalidateSarthiConfig(scopeProfile)
      } catch (err) {
        if (profileEpoch.current === epoch) {
  ...
  const writeAgentDefault = useCallback(
    async (key: string, value: string) => {
      ...
      try {
        await saveSarthiConfig(next, scopeProfile ?? undefined)
      } catch (err) {
        setConfig(prev)
