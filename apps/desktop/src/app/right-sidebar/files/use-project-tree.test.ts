...
beforeEach(() => {
  ...
  readDir.mockReset()
  ;(window as unknown as { sarthiDesktop: { readDir: typeof readDir } }).sarthiDesktop = { readDir }
})
...
function ok(entries: { name: string; path: string; isDirectory: boolean }[]): SarthiReadDirResult {
  return { entries }
}
...
describe('useProjectTree', () => {
  ...
  it('reads gitignore from the real path while caching per connection', async () => {
    ...
    ;(window as unknown as { sarthiDesktop: unknown }).sarthiDesktop = { gitRoot, readDir, readFileDataUrl }
  ...
  it('dedupes concurrent loadChildren calls for the same id', async () => {
    ...
    let resolveChildren: ((value: SarthiReadDirResult) => void) | undefined
    readDir.mockImplementationOnce(
      () =>
        new Promise<SarthiReadDirResult>(resolve => {
  ...
  it('falls back to the sanitized workspace dir when the session cwd is gone', async () => {
    ...
    ;(window as unknown as { sarthiDesktop: unknown }).sarthiDesktop = { readDir, sanitizeWorkspaceCwd }
  ...
  it('returns no-bridge gracefully when window.sarthiDesktop is missing', async () => {
    delete (window as unknown as { sarthiDesktop?: unknown }).sarthiDesktop
