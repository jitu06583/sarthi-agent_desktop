...
import type { SarthiReadDirEntry, SarthiReadDirResult } from '@/global'
...
export type ProjectTreeEntry = SarthiReadDirEntry
...
function ignoredBy(rules: GitignoreRule[], entry: SarthiReadDirEntry) {
...
async function filterIgnored(entries: SarthiReadDirEntry[], rootPath: string, dirPath: string) {
...
export async function readProjectDir(dirPath: string, rootPath = dirPath): Promise<SarthiReadDirResult> {
  if (!window.sarthiDesktop) {
