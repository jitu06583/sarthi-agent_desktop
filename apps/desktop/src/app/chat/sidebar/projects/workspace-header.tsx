...
import type { SarthiGitBranch } from '@/global'
...
const branchActionLabel = (branch: SarthiGitBranch, copy: BranchActionCopy) => {
...
// inside it. Naming is explicit — no auto-generated `sarthi/work-<ts>` trees.
...
export function StartWorkButton({ repoPath, onStarted }: { repoPath: string; onStarted: (path: string) => void }) {
  ...
  const [branches, setBranches] = useState<SarthiGitBranch[]>([])
  ...
  const convert = async (branch: SarthiGitBranch) => {
    if (pending || !repoPath || !branch) {
      return
    }
