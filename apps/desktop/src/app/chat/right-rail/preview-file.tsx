...
import { SARTHI_PATHS_MIME } from '@/app/chat/hooks/use-composer-actions'
...
async function readTextPreview(filePath: string) {
  try {
    ...
    if (!message.includes("No handler registered for 'sarthi:readFileText'")) {
      throw error
    }
  }
  ...
  const dataUrl = await window.sarthiDesktop.readFileDataUrl(filePath)
...
function startLineDrag(event: ReactDragEvent<HTMLElement>, filePath: string, { end, start }: LineSelection) {
  ...
  event.dataTransfer.setData(SARTHI_PATHS_MIME, JSON.stringify([{ line: start, lineEnd, path: filePath }]))
