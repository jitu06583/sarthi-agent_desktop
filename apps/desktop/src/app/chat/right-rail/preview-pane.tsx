        ...
        <>
          <a
            ...
            onClick={event => {
              ...
              void window.sarthiDesktop?.openExternal(error.url)
            }}
          >
...
export function PreviewPane({ embedded = false, onRestartServer, reloadRequest = 0, tabId, target }: PreviewPaneProps) {
  ...
  const restartServer = useCallback(async () => {
    ...
    // "Ask Sarthi to restart the server" looked like it did nothing —
  ...
  useEffect(() => {
    if (
      ...
      !window.sarthiDesktop?.watchPreviewFile ||
      !window.sarthiDesktop?.onPreviewFileChanged
    ) {
      return
    }
    ...
    const unsubscribe = window.sarthiDesktop.onPreviewFileChanged(payload => {
    ...
    void window.sarthiDesktop
      ...
      .then(watch => {
        if (!active) {
          void window.sarthiDesktop?.stopPreviewFileWatch?.(watch.id)
    ...
    return () => {
      ...
      if (watchId) {
        void window.sarthiDesktop?.stopPreviewFileWatch?.(watchId)
      }
    }
  ...
  useEffect(() => {
    ...
    webview.setAttribute('partition', 'persist:sarthi-preview')
    ...
    const onGuestContextMenu = (event: Event) => {
      ...
      const zoom = window.sarthiDesktop?.zoom?.factor?.() || 1
        ...
        },
        {
          addToDictionary: (word: string) => {
            ...
            if (typeof webContentsId === 'number') {
              void window.sarthiDesktop?.contextMenuGuestAddWord?.({ webContentsId, word })
            }
          },
          copyImage: () => void window.sarthiDesktop?.contextMenuCopyImage?.(),
      ...
      <div className="flex min-h-0 flex-1 flex-col overflow-hidden">
        ...
        {isWebPreview && !isRemoteHtml && (
          <PreviewBrowserBar
            ...
            onOpenExternal={() => void window.sarthiDesktop?.openExternal(currentUrl)}
