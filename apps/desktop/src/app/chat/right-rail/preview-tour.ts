...
function buildTourScript(action: TourAction): string {
  return `(function () {
  var w = window;
  if (!w.__sarthiTourEngine) {
    ${driverIife}
    w.__sarthiTourHolder = {};
    w.__sarthiTourCollect = (${collectTourTargets.toString()});
    w.__sarthiTourEngine = (${runTourEngine.toString()});
  }
  if (!document.getElementById('__sarthi-tour-style')) {
    var style = document.createElement('style');
    style.id = '__sarthi-tour-style';
    style.textContent = ${JSON.stringify(driverCss)};
    (document.head || document.documentElement).appendChild(style);
  }
  return JSON.stringify(w.__sarthiTourEngine(
    w.driver.js.driver,
    w.__sarthiTourHolder,
    ${JSON.stringify(action)},
    w.__sarthiTourCollect,
    document
  ));
})()`
}
