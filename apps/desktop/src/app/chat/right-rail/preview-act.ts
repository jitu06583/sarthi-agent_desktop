...
const preamble = () => `  var w = window;
  ...
  w.__sarthiActHolder = w.__sarthiActHolder || {};
  w.__sarthiAct = ${actEngineSource()};
  w.__sarthiWatch_fn = (${watchInPage.toString()});
  w.__sarthiWatchTag = ${WATCH_TAG};
  var holder = w.__sarthiActHolder;
  var act = function (a) { return w.__sarthiAct(document, holder, a); };
  var watch = function (stage, label) {
    try { w.__sarthiWatch_fn(document, holder, stage, label); } catch (err) {}
  };
...
${preamble()}
  ...
  document.addEventListener('pointerdown', function (e) {
    w.__sarthiHit = { tag: e.target ? e.target.tagName : '?', trusted: e.isTrusted === true };
...
function hitNote(hit?: { tag: string; trusted: boolean } | null): string | undefined {
  return hit && hit.tag === 'SARTHI-WATCH' ? 'The action overlay intercepted the click instead of the page.' : undefined
}
