...
export interface OAuthProvider {
   ...
   *  embedded terminal. Null when Sarthi doesn't know how to remove it. */
...
export interface EnvVarInfo {
  ...
  // in sarthi_cli/provider_catalog.py). When present, the Keys tab groups by
  // this provider identity — the SAME one `sarthi model` uses — instead of
...
export type SarthiConfigRecord = Record<string, unknown>
...
export interface ModelOptionProvider {
   ...
   *  the `sarthi model` CLI / onboarding OAuth flow. */
  auth_type?: string
...
// non-desktop) Sarthi simply carries none of it. See store/profile-share.ts.
export interface ProfileDesktopOverlay {
  ...
  /** Layout tree (sarthi.desktop.layoutTree.v2 shape). */
  layoutTree?: unknown
...
// folders. Mirrors sarthi_cli/projects_db.Project.to_dict().
...
export interface SkillInfo {
  ...
  usage?: number
  /** 'agent' = learned/local (editable), 'bundled' = ships with Sarthi, 'hub' = installed. */
...
export interface ToolProvider {
   ...
   *  CLI `sarthi tools` active-provider detection). */
  is_active: boolean
 ...
 *  not Sarthi); elsewhere, driver health from `cua-driver doctor`. `null`
...
/** Shape of `GET /api/sarthi/update/check` — the backend's own update state.
