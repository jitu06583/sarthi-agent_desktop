 ...
 * packaged binary) with a real `sarthi serve` backend pointed at a mock
 * inference server. The full chain is exercised:
 *
 *   electron → sarthi serve (python) → mock provider → renderer
 *
...
test.describe('dev-mode boot with mock backend', () => {
  test('window opens with Sarthi title', async () => {
    const title = await fixture!.page.title()
    expect(title).toContain('Sarthi')
  })
  ...
  test('the preload bridge reaches the renderer', async () => {
    const bridge = await fixture!.page.evaluate(() => {
      const desktop = (window as unknown as { sarthiDesktop?: Record<string, unknown> }).sarthiDesktop
  ...
  test('backend boots and app becomes ready', async () => {
    ...
    // electron starts → sarthi serve is spawned → WS connects → config
