/**
 * Shared E2E fixtures for the Sarthi desktop Playwright suite.
 *
 * Two fixture modes:
 *
 ...
 *     (electron → sarthi serve → provider → inference → renderer) is
 ...
 * `electron-builder --dir` step and matches `sarthi desktop --source`. The
...
export function createSandbox(prefix: string): Sandbox {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), `sarthi-e2e-${prefix}-${Math.random()}`))
  const sarthiHome = path.join(root, 'sarthi-home')
  const userDataDir = path.join(root, 'electron-user-data')

  fs.mkdirSync(sarthiHome, { recursive: true })
...
export function writeMockProviderConfig(
  ...
  const configPath = path.join(sarthiHome, 'config.yaml')
 ...
 */
export function writeEnvFile(sarthiHome: string, apiKey = 'e2e-mock-key'): void {
  const envPath = path.join(sarthiHome, '.env')
 ...
 */
function writeEmptyConfig(sarthiHome: string): void {
  const configPath = path.join(sarthiHome, 'config.yaml')
 ...
 *
 * Key env vars:
 *  - SARTHI_HOME → sandbox sarthi-home (isolated config/sessions)
 *  - SARTHI_DESKTOP_USER_DATA_DIR → sandbox electron-user-data
 *  - SARTHI_DESKTOP_IGNORE_EXISTING=1 → don't pick up `sarthi` from PATH
 *    (we want the dev checkout at REPO_ROOT)
 *  - SARTHI_DESKTOP_SARTHI_ROOT → REPO_ROOT (dev checkout resolution)
 *  - SARTHI_DESKTOP_APP_NAME → unique-ish per test (avoids single-instance lock)
...
export function buildAppEnv(sandbox: Sandbox, extra: Record<string, string> = {}): Record<string, string> {
  ...
  return {
    ...clean,
    SARTHI_HOME: sandbox.sarthiHome,
    SARTHI_DESKTOP_USER_DATA_DIR: sandbox.userDataDir,
    SARTHI_DESKTOP_IGNORE_EXISTING: '1',
    SARTHI_DESKTOP_SARTHI_ROOT: REPO_ROOT,
    SARTHI_DESKTOP_APP_NAME: `SarthiE2E-${Date.now()}`,
    ...
    SARTHI_DESKTOP_SKIP_QUIT_CONFIRM: '1',
 ...
 *
 * @param sandbox  - isolated SARTHI_HOME + userData
 * @param env      - the process environment (already has SARTHI_HOME etc.)
...
export async function setupMockBackend(options: MockBackendOptions = {}): Promise<MockBackendFixture> {
  ...
  writeEnvFile(sandbox.sarthiHome)
...
export async function setupNoProvider(): Promise<NoProviderFixture> {
  ...
  writeEmptyConfig(sandbox.sarthiHome)
...
export interface DeadBackendOptions {
  /**
   * When true, inject a fake boot error via SARTHI_DESKTOP_BOOT_FAKE_ERROR
 ...
 * nothing listens on). By default the backend still boots (`sarthi serve`
...
export async function setupDeadBackend(options: DeadBackendOptions = {}): Promise<DeadBackendFixture> {
  ...
  const configPath = path.join(sandbox.sarthiHome, 'config.yaml')
  fs.writeFileSync(
    configPath,
...
`,
  ...
  const env = buildAppEnv(sandbox, options.fakeError ? { SARTHI_DESKTOP_BOOT_FAKE_ERROR: 'Failed to connect to Sarthi backend: connection refused' } : {})
  const { app, page } = await launchDesktop(env)
...
function resolvePackagedBinaryPath(): string {
  if (process.platform === 'win32') {
    return path.join(RELEASE_ROOT, 'win-unpacked', 'Sarthi.exe')
  }

  if (process.platform === 'darwin') {
    const arch = process.arch === 'arm64' ? 'arm64' : 'x64'

    return path.join(RELEASE_ROOT, `mac-${arch}`, 'Sarthi.app', 'Contents', 'MacOS', 'Sarthi')
  }

  return path.join(RELEASE_ROOT, 'linux-unpacked', 'sarthi')
}
 ...
 * progress without spawning a real Sarthi backend.
 *
 * Uses the same sandbox isolation (credential stripping, isolated
 * SARTHI_HOME + userData, unique app name) as the dev-mode fixtures.
 *
...
export async function setupPackagedApp(): Promise<PackagedAppFixture> {
  ...
  const env = buildAppEnv(sandbox, {
    ...
    SARTHI_DESKTOP_BOOT_FAKE: '1',
    SARTHI_DESKTOP_BOOT_FAKE_STEP_MS: '120',
  })

  // Clear dev-server + sarthi-root overrides — the packaged binary
  // should use its own bundled renderer, not the dev checkout.
  delete (env as Record<string, string | undefined>).SARTHI_DESKTOP_DEV_SERVER
  delete (env as Record<string, string | undefined>).SARTHI_DESKTOP_SARTHI
  delete (env as Record<string, string | undefined>).SARTHI_DESKTOP_SARTHI_ROOT
