...
/**
 * E2E smoke tests for the packaged Sarthi desktop app.
 *
 ...
 * (credential stripping, isolated SARTHI_HOME + userData, unique app name).
 *
...
test('window opens with the Sarthi title', async () => {
...
test('HUD composer remains fully inside the transparent window', async () => {
  ...
  await fixture!.page.evaluate(() =>
    (window as typeof window & {
      sarthiDesktop?: { hud?: { open: (options: { sessionId: null }) => Promise<void> } }
    }).sarthiDesktop?.hud?.open({ sessionId: null })
  )
