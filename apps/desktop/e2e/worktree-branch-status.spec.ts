...
function createGitRepo(root: string): string {
  ...
  execFileSync('git', ['config', 'user.name', 'Sarthi E2E'], { cwd: repo })
...
function configureRepoCwd(sarthiHome: string, mockUrl: string, repo: string): void {
  writeMockProviderConfig(sarthiHome, mockUrl)
  fs.appendFileSync(path.join(sarthiHome, 'config.yaml'), `\nterminal:\n  cwd: ${repo}\n`, 'utf8')
  writeEnvFile(sarthiHome)
}
...
test.beforeAll(async () => {
  ...
  configureRepoCwd(sandbox.sarthiHome, mock.url, repo)
