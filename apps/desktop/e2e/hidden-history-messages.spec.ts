...
async function setupSeededMockBackend(): Promise<MockBackendFixture> {
  ...
  writeMockProviderConfig(sandbox.sarthiHome, mock.url)
  fs.appendFileSync(
    path.join(sandbox.sarthiHome, 'config.yaml'),
    '\ncompression:\n  threshold_tokens: 1\n',
    'utf8',
  )
  writeEnvFile(sandbox.sarthiHome)
  const builder = await RealSessionBuilder.start(sandbox.sarthiHome)
  try {
...
test('live verify-on-stop continuations stay out of the transcript', async ({}, testInfo) => {
  ...
  writeMockProviderConfig(sandbox.sarthiHome, mock.url)
  fs.appendFileSync(path.join(sandbox.sarthiHome, 'config.yaml'), '\nagent:\n  verify_on_stop: true\n', 'utf8')
  writeEnvFile(sandbox.sarthiHome)
  const { app, page } = await launchDesktop(buildAppEnv(sandbox))
