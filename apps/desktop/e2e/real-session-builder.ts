 ...
 * AIAgent loop, using the E2E mock provider configured in `sarthiHome`.
 *
...
export class RealSessionBuilder {
  ...
  private constructor(sarthiHome: string) {
  ...
  static async start(sarthiHome: string): Promise<RealSessionBuilder> {
    const builder = new RealSessionBuilder(sarthiHome)
