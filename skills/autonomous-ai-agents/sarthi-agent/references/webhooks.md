...
Create dynamic webhook subscriptions so external services (GitHub, GitLab, Stripe, CI/CD, IoT sensors, monitoring tools) can trigger Sarthi agent runs by POSTing events to a URL.
...
All management is via the `sarthi webhook` CLI command:

### Create a subscription
```bash
sarthi webhook subscribe <name> \
...
- **Route scripts** (`--script` on subscribe, or `script:` on a config route): a script under `~/.sarthi/scripts/` receives the payload as JSON on stdin. JSON stdout replaces the payload before prompt templating; empty stdout, `[SILENT]`, or a nonzero exit ignores the webhook. `.sh`/`.bash` run with bash, everything else with Python. Scripts cannot live outside `~/.sarthi/scripts/` (path traversal is blocked).

```bash
sarthi webhook subscribe todoist-sarthi \
  --prompt "Task changed: {payload.content}" \
  --script "todoist-sarthi-label.py" \
...
```bash
sarthi webhook remove <name>
```

### Test a subscription
```bash
sarthi webhook test <name>
sarthi webhook test <name> --payload '{"key": "value"}'
```
...
### GitHub: PR reviews
```bash
sarthi webhook subscribe github-prs \
  --events "pull_request" \
...
```bash
sarthi webhook subscribe ci-builds \
  --events "pipeline" \
...
```bash
sarthi webhook subscribe alerts \
...
- Subscriptions persist to `~/.sarthi/webhook_subscriptions.json`

## How It Works

1. `sarthi webhook subscribe` writes to `~/.sarthi/webhook_subscriptions.json`
...
1. **Is the gateway running?** Check with `systemctl --user status sarthi-gateway` or `ps aux | grep gateway`
2. **Is the webhook server listening?** `curl http://localhost:8644/health` should return `{"status": "ok"}`
3. **Check gateway logs:** `grep webhook ~/.sarthi/logs/gateway.log | tail -20`
4. **Signature mismatch?** Verify the secret in your service matches the one from `sarthi webhook list`. GitHub sends `X-Hub-Signature-256`, GitLab sends `X-Gitlab-Token`.
5. **Firewall/NAT?** The webhook URL must be reachable from the service. For local development, use a tunnel (ngrok, cloudflared).
6. **Wrong event type?** Check `--events` filter matches what the service sends. Use `sarthi webhook test <name>` to verify the route works.
