# Themes / Skins — Author a Sarthi Color Theme

Author a Sarthi **skin** — one YAML file that themes the CLI, the TUI, and the
desktop GUI at once. The skin engine (`sarthi_cli/skin_engine.py`) resolves the
active skin and the gateway pushes it to every surface, so a file dropped in
`~/.sarthi/skins/` is the theme analogue of a plugin: no code, all surfaces. This
...
- The user asks for a custom look ("make me a synthwave theme", "dark forest
  vibes", "match my brand colors") for Sarthi itself.
...
- Write access to the Sarthi home dir — `~/.sarthi` by default, or `$SARTHI_HOME`
  / the active profile's dir. Skins live in `<sarthi-home>/skins/`.
- Native tools: `write_file` (create the YAML), `read_file` / `search_files`
  (inspect existing skins), `terminal` (activate via `sarthi config set`).
...
3. `write_file` it to `<sarthi-home>/skins/<name>.yaml`.
...
4. **Confirm the new look landed** and tell the user how to revert: run
   `sarthi config set display.skin default` (or they can `/skin default`).
...
```
sarthi skin set <key> <hex>      # e.g. sarthi skin set ui_tool "#00FFFF"
```
...
palette and resets the background. `sarthi skin set background "#08201f"` changes
only the background; `sarthi skin use <name>` / `sarthi skin list` switch and
enumerate.

## Pitfalls

- **Don't hardcode `~/.sarthi`** when a profile is active — resolve the real home
  from `$SARTHI_HOME` first, falling back to `~/.sarthi`.
...
- **Never hand-edit `config.yaml` to activate.** Use `sarthi config set
...
- **You apply it, not the user.** `sarthi config set display.skin <name>` is
...
- `read_file` the written `<sarthi-home>/skins/<name>.yaml` and confirm valid
  YAML with the intended `name` and `colors`.
- Run `sarthi config get display.skin` and confirm it reports `<name>`.
