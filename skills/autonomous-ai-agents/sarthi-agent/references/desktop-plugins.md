...
Write plugins for the Sarthi desktop app: statusbar items, layout panes,
...
system (`~/.sarthi/plugins/`) is otherwise documented separately.

There are TWO on-disk doors, same contract and hot reload:

- `$SARTHI_HOME/desktop-plugins/<id>/plugin.js` — standalone desktop plugin.
  Loads enabled by default.
- `$SARTHI_HOME/plugins/<id>/desktop/plugin.js` — the desktop HALF of a
...
- The Sarthi desktop app (it loads plugins; the CLI/gateway alone does not).
- Write access to `$SARTHI_HOME/desktop-plugins/` (usually
  `~/.sarthi/desktop-plugins/`).

## How to Run

1. Create `$SARTHI_HOME/desktop-plugins/<name>/plugin.js` from
   `templates/plugin.js` (in this skill directory) — that's
   `~/.sarthi/...` by default, or `~/.sarthi/profiles/<profile>/...` under a
...
The ONLY import surface is `@sarthi/plugin-sdk` (plus `react` /
...
- `ctx.os` — the curated OS door, attributed to your plugin:
  ...
  posts a native OS notification. Fires only while the user is away from Sarthi
  ...
  (`sarthi://index-network/intent/1`), a hash path (`/index-network/intent/1`),
...
- Backend: if the plugin ships a Python `plugin_api.py` (under
  `~/.sarthi/plugins/<id>/dashboard/`, manifest `"api": "plugin_api.py"`), reach
...
- Do not import anything except `@sarthi/plugin-sdk`, `react`, and
