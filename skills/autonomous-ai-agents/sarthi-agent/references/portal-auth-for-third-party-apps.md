...
## Layer 1 — Is this thing a Sarthi plugin, or a separate app?
...
|---|---|---|
| **OpenViking memory plugin** (`plugins/memory/openviking/`) | Code that runs **inside the Sarthi process**. Its LLM calls go through Sarthi's already-configured provider. | Already uses Portal if user's Sarthi is configured for Portal. Nothing extra needed. `OPENVIKING_API_KEY` is the OpenViking *server's* own auth, not LLM auth. |
...
plugin that already runs inside Sarthi. That LLM call is already authenticated
via Sarthi's provider config. The plugin's own server auth (e.g.
...
What Sarthi Agent has that *feels* like OAuth — `sarthi login --provider nous`
opening a browser, user signs in, token lands in `~/.sarthi/auth.json` — is a
**Sarthi-specific browser flow**. Under the hood it produces a credential
Sarthi uses as a bearer. It is not a public OAuth provider that Karakeep et al.
...
1. Read Sarthi's existing Portal credential out of `~/.sarthi/auth.json`.
...
rides on the credential Sarthi already holds.

Where this could live in Sarthi:
...
- **Per-key rate limits** so a noisy app can be capped without eating the
  user's headroom for Sarthi itself.
...
1. First decide: Sarthi plugin (runs inside Sarthi) or separate app? If plugin,
   it already uses Portal via Sarthi's provider config — done.
...
4. Mixed setup ("Portal for some things, OpenRouter/Ollama Cloud for the
   Sarthi agent itself") is fully supported. Sarthi treats agent
...
pitch in the Tool Gateway announcement is specifically about Sarthi Agent's
*tools* (web search, browser, image gen, TTS) flowing through the Portal
subscription when Sarthi is configured to use Portal as its provider. It is
