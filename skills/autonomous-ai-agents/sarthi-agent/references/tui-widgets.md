...
Author widget apps for the Sarthi TUI (`sarthi --tui`): glanceable ambient
...
- The TUI must be in use (`sarthi --tui`). Widgets do not render in the
...
1. Use `write_file` to create `~/.sarthi/tui-widgets/<name>.mjs` (see
...
- A thrown `register()` is logged and skipped; check
  `~/.sarthi/logs/tui_gateway_crash.log` if a widget never appears.
