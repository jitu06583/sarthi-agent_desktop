...
Set via `sarthi model` (picker) or `sarthi setup`. 35+ provider profiles ship as
...
| nous | OAuth device code | `sarthi auth add nous` (or `NOUS_API_KEY`) |
| openai-codex | OAuth | `sarthi auth add openai-codex` |
| qwen-oauth | OAuth | `sarthi auth add qwen-oauth` |
| minimax-oauth | OAuth | `sarthi auth add minimax-oauth` |
...
Multiple credentials per provider pool and rotate automatically (`sarthi auth`).
Fallback chain when the primary fails: `sarthi fallback add|remove|list`.

### User-defined model aliases

Work with `/model <name>` in CLI and every gateway platform. Resolved by
`sarthi_cli/model_switch.py::resolve_alias()`; user aliases are checked BEFORE
...
#   sarthi config set model.aliases.fav openrouter/anthropic/claude-sonnet-4.6
model:
  aliases:
