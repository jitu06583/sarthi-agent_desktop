...
`sarthi config check` reports sections missing from an older config.

### Toolsets

Enable/disable via `sarthi tools` (interactive) or `sarthi tools enable/disable NAME`.
Full enumeration: `TOOLSETS` dict in `toolsets.py` (`_SARTHI_CORE_TOOLS` is the default bundle most platforms inherit).
