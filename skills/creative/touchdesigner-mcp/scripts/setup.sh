...
SARTHI_HOME_DIR="${SARTHI_HOME:-$HOME/.sarthi}"
SARTHI_CFG="${SARTHI_HOME_DIR}/config.yaml"
MCP_PORT=40404
...
# ── 3. Ensure Sarthi config has twozero_td MCP entry ──
if [[ ! -f "$SARTHI_CFG" ]]; then
    echo -e " ${FAIL} Sarthi config not found at ${SARTHI_CFG}"
    manual_steps+=("Create ${SARTHI_CFG} with twozero_td MCP server entry")
elif grep -q 'twozero_td' "$SARTHI_CFG" 2>/dev/null; then
    echo -e " ${OK} twozero_td MCP entry exists in Sarthi config"
else
    echo -e " ${WARN} Adding twozero_td MCP entry to Sarthi config..."
    python3 -c "
...
" 2>/dev/null && echo -e " ${OK} twozero_td MCP entry added to config" \
              || { echo -e " ${FAIL} Could not update config (is PyYAML installed?)"; \
                   manual_steps+=("Add twozero_td MCP entry to ${SARTHI_CFG} manually"); }
    manual_steps+=("Restart Sarthi session to pick up config change")
fi
