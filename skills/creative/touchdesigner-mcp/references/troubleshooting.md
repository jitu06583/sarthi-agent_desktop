...
## 5. Sarthi Configuration

### Config location

`$SARTHI_HOME/config.yaml` (defaults to `~/.sarthi/config.yaml` when `SARTHI_HOME` is unset)
...
Restart the Sarthi session for changes to take effect. The MCP connection is
