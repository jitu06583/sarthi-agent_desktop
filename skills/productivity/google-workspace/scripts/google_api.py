#!/usr/bin/env python3
"""Google Workspace API CLI for Sarthi Agent.

Uses the Google Workspace CLI (`gws`) when available, but preserves the
existing Sarthi-facing JSON contract and falls back to the Python client
...
from _sarthi_home import get_sarthi_home
...
def _gws_binary() -> str | None:
    override = os.getenv("SARTHI_GWS_BIN")
    if override:
        return override
...
def main():
    parser = argparse.ArgumentParser(description="Google Workspace API for Sarthi Agent")
