#!/usr/bin/env bash
# GitHub environment detection helper for Sarthi Agent skills.
#
...
if command -v gh &>/dev/null && gh auth status &>/dev/null 2>&1; then
    ...
    GH_AUTH_METHOD="curl"
elif _sarthi_env="${SARTHI_HOME:-$HOME/.sarthi}/.env"; [ -f "$_sarthi_env" ] && grep -q "^GITHUB_TOKEN=" "$_sarthi_env" 2>/dev/null; then
