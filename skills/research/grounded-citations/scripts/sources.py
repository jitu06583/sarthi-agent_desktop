...
Ledger path resolution (first wins):
  --ledger PATH
  $SARTHI_CITATION_LEDGER
  $SARTHI_HOME/cache/citations/ledger.json
"""
...
from _sarthi_home import get_sarthi_home  # noqa: E402
...
def resolve_ledger_path(explicit: str | None = None) -> Path:
    ...
    env = os.environ.get("SARTHI_CITATION_LEDGER", "").strip()
    if env:
        return Path(env).expanduser()
    return get_sarthi_home() / "cache" / "citations" / "ledger.json"
