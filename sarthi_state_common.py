...
Extracted verbatim from sarthi_state.py so the SessionDB mixin modules
(sarthi_state_search / sarthi_state_schema / sarthi_state_portability) can
reference them without importing sarthi_state (which would be a cycle).
sarthi_state re-imports every name here for backward compatibility.
"""
...
# optimized via ``sarthi sessions optimize-storage``. A legacy DB sits at
...
# `sarthi sessions optimize-storage` rebuilds it on a capable host.
...
# triggers repairable UNTIL the user opts into `sarthi db optimize`. This is
