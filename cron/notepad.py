...
Write path is the CLI (``sarthi cron notepad <job_id> set <key> <value>``),
...
NOTEPAD_FILE = get_sarthi_home().resolve() / "cron" / "notepad.db"
...
def render_notepad_section(job_id: str) -> str:
    ...
    return (
        ...
        f"`sarthi cron notepad {job_id} set <key> <value>` "
        f"(also: get/delete/list; `sarthi cron notepad {job_id} delete "
