...
def fire_overdue_jobs(
    ...
    *,
    ...
    from cron.jobs import _ensure_aware, _sarthi_now, is_job_runnable, load_jobs
...
class InProcessCronScheduler(CronScheduler):
    ...
    def start(
        ...
        *,
        ...
        # only the process-global SARTHI_HOME (the default profile) is ticked.
        # Heartbeats and recovery are also scoped per profile so `sarthi cron
        ...
        # Heartbeat once before the first sleep so `sarthi cron status` sees a
        ...
        while not stop_event.is_set():
            ...
            try:
                ...
                # `sarthi cron status`/`list` (separate processes) can show
    ...
    def _start_multiplex(
        ...
        *,
        ...
        Each profile uses ``set_sarthi_home_override()`` + ``use_cron_store()``
        ...
        for entry in profile_homes:
            ...
            home_token = set_sarthi_home_override(str(home))
            try:
            ...
            for entry in profile_homes:
                ...
                try:
                    with use_cron_store(home):
                        ...
                        # so `sarthi cron status` can show WHY ticks fail
                        # (#68483).
                        if ok:
                            clear_ticker_error()
                        elif _tick_error:
