...
from sarthi_cli.config import load_config, _expand_env_vars
...
def run_job(
    ...
    os.environ["SARTHI_CRON_SESSION"] = "1"
    ...
    try:
        ...
        # Model resolution precedence: per-job override > SARTHI_MODEL env >
        ...
        _raw_cron_timeout = os.getenv("SARTHI_CRON_TIMEOUT", "").strip()
        if _raw_cron_timeout:
            try:
                _cron_timeout = float(_raw_cron_timeout)
            except (ValueError, TypeError):
                logger.warning(
                    "Invalid SARTHI_CRON_TIMEOUT=%r; using default 600s",
                    _raw_cron_timeout,
                )
                _cron_timeout = 600.0
        else:
