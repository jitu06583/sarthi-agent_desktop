...
``sarthi gateway restart`` (or ``launchctl kickstart ai.sarthi.gateway``
or ``systemctl restart sarthi-gateway``).  When the cron fires, the
...
``sarthi cron create`` CLI subcommand AND the agent's ``cronjob`` model
...
command identifier (``sarthi gateway``, ``launchctl ... sarthi-gateway``,
``systemctl ... sarthi-gateway``, ``pkill`` against the gateway) so it
...
commands and shell scripts they reference when ``_SARTHI_GATEWAY=1``. It also
rejects ``launchctl submit`` in gateway sessions because launchd treats that
primitive as a persistent KeepAlive job, not a one-shot task. ``sarthi gateway
...
_GATEWAY_LIFECYCLE_PATTERN = re.compile(
    r"(?i)"
    # Branch A: `sarthi gateway restart|stop` — the canonical foot-gun.
    # `start` is intentionally excluded: starting a gateway from inside a
    # gateway is benign (a no-op or "already running" error), and a
    # legitimate cron job might start a sibling profile's gateway.
    r"(?:sarthi\s+gateway\s+(?:restart|stop))"
    # Branch B: launchctl ops on a sarthi-gateway label. macOS launchd
    # labels look like `ai.sarthi.gateway` / `sarthi-gateway`. Requiring the
    # gateway identifier prevents blocking unrelated sarthi services (e.g.
    # `launchctl unload ai.sarthi.update-checker.plist`).
    # `submit` and `bootstrap` are included alongside the direct verbs
    # (kickstart/etc.): `launchctl submit -l ai.sarthi.gateway-<suffix> --
    ...
    # Branch C: systemctl ops on a sarthi-gateway unit.
    r"|(?:systemctl\s+(?:-\S+\s+)*(?:restart|stop|start)\b[^\n]*\bsarthi[.\-]?gateway)"
    # Branch D: pkill / kill targeting the sarthi gateway process. Both
...
# lines (e.g. `launchctl submit \` / `  -l ai.sarthi.gateway-... \` / `  -- ...`,
...
# for `systemctl restart sarthi-gateway` in syslog, a SQL LIKE literal over a
...
# to execution (`grep 'systemctl restart sarthi-gateway' f | sh`); never mask
# such a line.
...
def contains_launchctl_submit_command(command: str) -> bool:
    ...
    chosen by whoever writes it, so a neutral name (``ai.sarthi.svc-reload-tmp``)
...
def _mask_data_sink_arguments(text: str) -> str:
    ...
    ``systemctl restart sarthi-gateway`` from the same characters appearing
    ...
    (false positives blocking legitimate cron prompts), e.g.::
        ...
        sqlite3 db "SELECT msg FROM log WHERE msg LIKE '%systemctl restart sarthi-gateway%'"
...
def _iter_referenced_shell_scripts(
    ...
    *,
    ...
    for segment in _iter_command_segments(command):
        ...
        # (e.g. `Path.home() / ".sarthi"`), not an executable reference.
...
def _resolve_script_path(script_path: str) -> Optional[Path]:
    ...
    under ``<SARTHI_HOME>/scripts/`` and only accepts absolute paths as-is.
    ...
    (``~/.sarthi/scripts/restart.sh``) but is passed as the bare name
    ...
    if raw.is_absolute():
        return raw
    try:
        return get_sarthi_home() / "scripts" / raw
    except (RuntimeError, OSError):
        # get_sarthi_home() falls back to Path.home(), which raises when
        # neither SARTHI_HOME nor HOME is resolvable (launchd/systemd
...
def check_gateway_lifecycle(
        ...
        if resolved_script is not None:
            ...
            if _is_cloud_placeholder_path(resolved_script) or _is_cloud_placeholder_path(
                ...
                raise GatewayLifecycleBlocked(
                    ...
                    "(e.g. ~/.sarthi/scripts/) and recreate the job."
                )
    ...
    if python_script:
        ...
        # `sarthi gateway restart` embedded in a .py script is still
    ...
    if unsafe:
        raise GatewayLifecycleBlocked(
            ...
            "(#30719). Run `sarthi gateway restart` from a shell outside "
