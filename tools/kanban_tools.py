...
def _handle_create(args: dict, **kw) -> str:
    ...
    session_id = args.get("session_id") or os.environ.get("SARTHI_SESSION_ID")
    priority = args.get("priority")
    # Resolve workspace. If the caller passed one explicitly, honor it.
    # Otherwise, a dispatcher-spawned worker (SARTHI_KANBAN_TASK set)
    ...
    # (kanban toolset, no SARTHI_KANBAN_TASK) and CLI/dashboard callers
...
def _maybe_auto_subscribe(conn: Any, task_id: str) -> bool:
    ...
    - **Gateway** (telegram/discord/slack/etc): ``SARTHI_SESSION_PLATFORM``
      and ``SARTHI_SESSION_CHAT_ID`` are set in ContextVars by the
