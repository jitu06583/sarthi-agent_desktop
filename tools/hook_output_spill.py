...
* Never raises. Any I/O error (disk full, permission denied, missing
  SARTHI_HOME, etc.) falls back to a byte-length truncation with an
...
def _resolve_spill_dir(directory_override: Optional[str], session_id: Optional[str]) -> Path:
    ...
    if directory_override:
        ...
        try:
            ...
            base = Path(get_sarthi_home()) / "hook_outputs"
        except Exception:
            # Last-resort fallback: SARTHI_HOME env var, then ~/.sarthi
            home = os.environ.get("SARTHI_HOME") or os.path.expanduser("~/.sarthi")
