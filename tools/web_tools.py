...
Backend is selected during ``sarthi tools`` setup (web.backend in config.yaml).
When available, Sarthi can route Firecrawl calls through a Nous-hosted tool-gateway
...
def _env_value(name: str) -> str:
    """Resolve ``name`` via Sarthi config-aware env, falling back to process env.

    Mirrors the SearXNG provider's ``_searxng_url()`` so that values set
    through Sarthi' config/.env layer (``sarthi config set``, ``sarthi tools``)
...
def _load_web_config() -> dict:
    """Load the ``web:`` section from ~/.sarthi/config.yaml."""
    try:
...
def _get_backend() -> str:
    ...
    Reads ``web.backend`` from config.yaml (set by ``sarthi tools``). A
...
def _is_backend_available(backend: str) -> bool:
    ...
    if backend == "xai":
        ...
        # runs on every web_search dispatch + every `sarthi tools` repaint.
        try:
            ...
            if provider is None and backend and selection_exists("web"):
                ...
                if disabled_key:
                    ...
                    error_text = (
                        ...
                        f"Re-enable it with `sarthi plugins enable {disabled_key}` "
            ...
            if disabled_key:
                ...
                response_data = {
                    ...
                    "error": (
                        ...
                        "Run `sarthi tools` to set one up."
                    ),
                }
        else:
            logger.info(
                ...
                if backend and selection_exists("web"):
                    ...
                    if disabled_key:
                        ...
                        error_text = (
                            ...
                            f"config. Re-enable it with `sarthi plugins "
                        ...
                        return json.dumps(
                            {
                                ...
                                "error": (
                                    ...
                                    f"`sarthi plugins enable {disabled_key}` "
                                    "(or remove it from plugins.disabled)."
                                ),
                            },
                            ensure_ascii=False,
...
def _provider_is_ready(provider) -> bool:
    ...
    require a true availability probe — otherwise ``sarthi doctor`` paints a
...
if __name__ == "__main__":
    ...
    from sarthi_cli.config import get_env_value as _gev
