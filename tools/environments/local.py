...
def _sanitize_subprocess_env(base_env: dict | None, extra_env: dict | None = None) -> dict:
    ...
    for key, value in (base_env or {}).items():
        ...
        if _is_sarthi_internal_secret(key):
            continue
        if key not in _SARTHI_PROVIDER_ENV_BLOCKLIST or _is_passthrough(key):
            sanitized[key] = value
    ...
    for key, value in (extra_env or {}).items():
        if key.startswith(_SARTHI_PROVIDER_ENV_FORCE_PREFIX):
            ...
            continue
        elif key not in _SARTHI_PROVIDER_ENV_BLOCKLIST or _is_passthrough(key):
            sanitized[key] = value
...
def _make_run_env(env: dict) -> dict:
    ...
    for k, v in merged.items():
        if k.startswith(_SARTHI_PROVIDER_ENV_FORCE_PREFIX):
            ...
            continue
        elif k not in _SARTHI_PROVIDER_ENV_BLOCKLIST or _is_passthrough(k):
            run_env[k] = v
