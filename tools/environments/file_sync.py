...
_FORCE_SYNC_ENV = "SARTHI_FORCE_FILE_SYNC"
...
def iter_sync_files(container_base: str = "/root/.sarthi") -> list[tuple[str, str]]:
    ...
    the hardcoded /root/.sarthi to *container_base* because the remote
...
class FileSyncManager:
    ...
    def sync(self, *, force: bool = False) -> None:
        ...
        or ``SARTHI_FORCE_FILE_SYNC=1`` is set.
    ...
    def sync_back(self, sarthi_home: Path | None = None) -> None:
        """Pull remote changes back to the host filesystem.

        Downloads the remote ``.sarthi/`` directory as a tar archive,
        ...
        """
        with self._transaction_lock:
            self._sync_back_transaction(sarthi_home=sarthi_home)

    def _sync_back_transaction(self, sarthi_home: Path | None = None) -> None:
    ...
    def _sync_back_impl(self) -> None:
        ...
        with tempfile.NamedTemporaryFile(suffix=".tar") as tf:
            ...
            with tempfile.TemporaryDirectory(prefix="sarthi-sync-back-") as staging:
    ...
    def _infer_host_path(self, remote_path: str,
        ...
        For example, if the mapping has ``/root/.sarthi/skills/a.md`` →
        ``~/.sarthi/skills/a.md``, a new remote file at
        ``/root/.sarthi/skills/b.md`` maps to ``~/.sarthi/skills/b.md``.
        """
        mapping = file_mapping if file_mapping is not None else []
