...
_EGRESS_LABEL_KEY = "sarthi-egress"
...
def _load_sarthi_env_vars() -> dict[str, str]:
    """Load ~/.sarthi/.env values without failing Docker command execution."""
    try:
...
def _get_active_profile_name() -> str:
    """Return the active Sarthi profile name, or ``"default"`` on any error.
...
def reap_orphan_containers(
    *,
    ...
    """Remove stale sarthi-tagged containers left behind by prior processes.

    Targets containers that match all of:

    * ``label=sarthi-agent=1`` (created by this codebase)
    * ``status=exited`` (running containers are NEVER reaped — they may
      belong to a sibling Sarthi process whose reuse path will pick them
      up; killing them would crash the sibling mid-command)
    * (optional) ``label=sarthi-profile=<profile_filter>`` (sweep only the
      caller's profile by default; a sarthi process in profile A must not
    ...
    * ``State.FinishedAt`` older than *max_age_seconds* ago (so a sibling
    ...
    even with the cleanup-fix in the prior commit, a hard-killed Sarthi
    process leaves its container behind permanently because there's no
    subsequent Sarthi process scheduled to reuse that exact (task, profile)
    pair.
    """
    docker = docker_exe or find_docker() or "docker"
    filters = ["--filter", "label=sarthi-agent=1", "--filter", "status=exited"]
    if profile_filter:
        filters.extend(["--filter", f"label=sarthi-profile={_sanitize_label_value(profile_filter)}"])
...
def find_docker() -> Optional[str]:
    ...
    Resolution order:
    1. ``SARTHI_DOCKER_BINARY`` env var — explicit override (e.g. ``/usr/bin/podman``)
    ...
    override = os.getenv("SARTHI_DOCKER_BINARY")
    if override and os.path.isfile(override) and os.access(override, os.X_OK):
        _docker_executable = override
        logger.info("Using SARTHI_DOCKER_BINARY override: %s", override)
        return override
...
#   SETUID/SETGID - the image's init drops from root to the 'sarthi'
...
def _egress_proxy_args_for_docker() -> tuple[list[str], dict[str, str], list[str]]:
    ...
    * ``env_overrides`` — env vars to set on container creation: ``HTTPS_PROXY``,
      ...
      paths, and one ``SARTHI_PROXY_TOKEN_<NAME>`` per minted mapping
    * ``host_args`` — extra ``--add-host`` flags so the container can reach the
    ...
    if not status.configured:
        msg = (
            ...
            "Run `sarthi egress setup` to mint tokens and write proxy.yaml."
        )
        if enforce:
    ...
    if not (status.pid and status.listening):
        msg = (
            ...
            "Start it with `sarthi egress start`."
        )
        if enforce:
            raise RuntimeError(msg)
        logger.warning("%s — continuing without proxy (enforce_on_docker=false).", msg)
    ...
    if status.ca_cert_path is None or not status.ca_cert_path.exists():
        ...
        msg = (
            ...
            "Re-run `sarthi egress setup` to regenerate it."
        )
        if enforce:
            raise RuntimeError(msg)
        logger.warning("%s — continuing without proxy (enforce_on_docker=false).", msg)
    ...
    if not mappings:
        msg = (
            ...
            "corrupt.  Re-run `sarthi egress setup` to mint provider "
    ...
    container_ca = "/etc/ssl/certs/sarthi-egress-ca.crt"
    ...
    # through the proxy.  Keep the SARTHI_PROXY_TOKEN_* aliases for
    # diagnostics.
    for m in mappings:
        env_overrides[m.real_env_name] = m.proxy_token
        env_overrides[f"SARTHI_PROXY_TOKEN_{m.real_env_name}"] = m.proxy_token
...
def _image_uses_init_entrypoint(docker_exe: str, image: str) -> bool:
    ...
    ``sarthi-agent:latest``) already provide their own PID-1 init and execute
...
class DockerEnvironment(BaseEnvironment):
    ...
    def __init__(
        ...
        # (TERMINAL_SANDBOX_DIR, default ~/.sarthi/sandboxes/). Non-persistent
        ...
        if egress_env_overrides:
            try:
                from sarthi_cli.config import load_config as _load_cfg_for_collision
        ...
        try:
            from sarthi_cli.config import load_config as _load_cfg_for_precedence
        ...
        # ``_SARTHI_EGRESS_NODE_OPTIONS_APPEND`` and merge here.
        _egress_node_append = merged_env.pop(
            "_SARTHI_EGRESS_NODE_OPTIONS_APPEND", None,
        )
        ...
        # s6-overlay images (e.g. sarthi-agent:latest) already use /init as PID 1
        ...
        container_name = f"sarthi-{uuid.uuid4().hex[:8]}"
        # Labels make sarthi-created containers identifiable to:
        #   * the orphan reaper (`sarthi-agent=1` for the global sweep filter)
        #   * future cross-process reuse (`sarthi-task-id`, `sarthi-profile`)
        #   * operators running `docker ps --filter label=sarthi-agent=1`
        # Values are limited to the safe character set defined by
        # _sanitize_label_value(); the active Sarthi profile is captured at
        ...
        label_args = [
            "--label", "sarthi-agent=1",
            "--label", f"sarthi-task-id={task_label}",
            "--label", f"sarthi-profile={profile_name}",
        ...
        self._labels = {
            "sarthi-agent": "1",
            "sarthi-task-id": task_label,
            "sarthi-profile": profile_name,
            _EGRESS_LABEL_KEY: egress_label,
        }

        # Cross-process container reuse (issue #20561 — docs claim "ONE long-lived
        # container shared across sessions").  If a prior Sarthi process
    ...
    def _resolve_passthrough_env(self) -> tuple[dict[str, str], set[str]]:
        ...
        # win over the generic Sarthi secret blocklist. Only implicit passthrough
        # keys are filtered. Also strip Sarthi-internal dynamic secrets
        # (AUXILIARY_*_API_KEY / _BASE_URL, GATEWAY_RELAY_* auth) that the
        # name-based blocklist doesn't cover — see _is_sarthi_internal_secret.
        _implicit_forward = {
            k for k in passthrough_keys if not _is_sarthi_internal_secret(k)
        }
        forward_keys = explicit_forward_keys | (_implicit_forward - _SARTHI_PROVIDER_ENV_BLOCKLIST)
        sarthi_env = _load_sarthi_env_vars() if forward_keys else {}
        unset_names: set[str] = set()
        for key in sorted(forward_keys):
            value = os.getenv(key) or sarthi_env.get(key)
    ...
    def _recreate_container(self) -> bool:
        ...
        task_label = self._labels.get("sarthi-task-id", "")
        profile_label = self._labels.get("sarthi-profile", "")
        existing = self._find_reusable_container(
        ...
        if not self._container_id:
            ...
            try:
                ...
                new_name = f"sarthi-{_uuid.uuid4().hex[:8]}"
    ...
    def _find_reusable_container(
        ...
        matches containers that happened to be named ``sarthi-*`` but were
        started by some other tool.
        """
        try:
            filters = [
                "--filter", "label=sarthi-agent=1",
                "--filter", f"label=sarthi-task-id={task_label}",
                "--filter", f"label=sarthi-profile={profile_label}",
            ]
            if egress_label != "off":
                ...
                # sarthi-egress label is present and not "off".  Without
                # this, a container created with egress=on can be silently
                # reused after the operator runs "sarthi egress disable",
        ...
        # container) but can happen if a previous Sarthi process crashed
    ...
    def cleanup(self, *, force_remove: bool = False):
        ...
        container shared across sessions" and stopping it on every Sarthi exit
        breaks that promise:
        ...
        * The user-visible difference between "ONE long-lived container" and
        ...
        Sarthi process touches a labeled container for ``2 × lifetime_seconds``
        it gets ``docker rm -f``'d at the next Sarthi startup. That covers the
        ...
        # outlives Sarthi processes, processes inside it stay alive, and
        ...
        import threading
        t = threading.Thread(target=_do_cleanup, daemon=True, name=f"sarthi-cleanup-{log_id}")
        t.start()
    ...
    def wait_for_cleanup(self, timeout: float = 30.0) -> bool:
        ...
        before the Python process exits — without this, ``sarthi /quit``
