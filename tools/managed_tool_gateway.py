...
def auth_json_path():
    """Return the Sarthi auth store path, respecting SARTHI_HOME overrides."""
    return get_sarthi_home() / "auth.json"
...
def peek_nous_access_token() -> Optional[str]:
    ...
    Availability scans (`sarthi tools`, banner/status paint, provider
...
# managed vendor's gateway URL is pinned: adding one is a Sarthi release, and
