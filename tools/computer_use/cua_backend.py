...
etc.) surface as specific blocked checks via `sarthi computer-use doctor`
...
# fetches the latest release, so a `SARTHI_CUA_DRIVER_VERSION` env var would
# only have *looked* like it pinned. For a reproducible version, point
# `SARTHI_CUA_DRIVER_CMD` at a specific binary instead.

_CUA_DRIVER_CMD_ENV = "SARTHI_CUA_DRIVER_CMD"
...
def _cua_no_overlay() -> bool:
    """True when Sarthi should pass ``--no-overlay`` to cua-driver.
...
def _cua_telemetry_disabled() -> bool:
    """True when Sarthi should disable cua-driver telemetry for this user.
...
def _cua_configured_permission_mode() -> str:
    ...
    per-session Sarthi YOLO toggle so a stale config line can never
...
def _cua_grant_existing_profile() -> bool:
    ...
    cua-driver's trusted-launcher grant. Sarthi passes
...
def _standard_runtime_launch_args(
    ...
    *,
    ...
    private_socket = socket_path or os.path.join(
        tempfile.gettempdir(), f"sarthi-cua-standard-{uuid.uuid4().hex[:12]}.sock"
    )
...
def _empty_discovery_reason() -> str:
    ...
    return (
        "window discovery returned no windows; run `sarthi computer-use "
...
def _wsl_windows_path_to_posix(path: str) -> str:
    """Translate a Windows absolute manifest command when Sarthi runs in WSL.

    Windows cua-driver manifests can report ``C:\\Users\\...\\cua-driver.exe``
    even though the Sarthi process uses POSIX subprocess spawning inside WSL.
...
class _EmbeddedCuaDaemon:
    ...
    machine-wide daemon would therefore let one Sarthi session's YOLO choice
    ...
    * ``unrestricted`` — explicit Sarthi YOLO; launch-time risk
    ...
    * ``bounded`` — a user-reviewed capability manifest
    ...
    def __init__(
        ...
        if sys.platform == "win32":
            self.socket_path = rf"\\.\pipe\sarthi-cua-{token}"
        else:
...
def _resolve_mcp_invocation(
    ...
    *,
    ...
    # A Windows-installed cua-driver can hand a WSL-hosted Sarthi an absolute
...
def _candidate_cua_driver_commands(override: Optional[str] = None) -> List[str]:
    ...
    ``SARTHI_CUA_DRIVER_CMD`` is authoritative; only when neither is set do we
    ...
    Sarthi Desktop/TUI session can otherwise filter out the `computer_use`
    tool even though `sarthi computer-use doctor` succeeds from a login shell.
    """
    ...
    if sys.platform == "win32":
        ...
        candidates.extend([
            ...
            # misses it until every Sarthi process is restarted.
            os.path.join(
...
def resolve_cua_driver_cmd(override: Optional[str] = None) -> Optional[str]:
    ...
    A supplied override (or ``SARTHI_CUA_DRIVER_CMD``) is never silently
...
def cua_driver_runtime_contract_status(binary: Optional[str] = None) -> Dict[str, Any]:
    """Report whether a local driver can host Sarthi' 0.20 integration."""
    ...
    if version < _CUA_DRIVER_RUNTIME_CONTRACT_MIN:
        return {
            ...
            "binary": resolved,
            "version": raw_version,
            "reason": "Sarthi computer use requires cua-driver 0.20.0 or newer",
        }
...
def cua_driver_update_nudge() -> Optional[str]:
    ...
    return (
        ...
        f"update with `sarthi computer-use install --upgrade`."
    )
...
def _maybe_repair_runtime_contract(contract: Dict[str, Any]) -> Dict[str, Any]:
    ...
    ``SARTHI_CUA_DRIVER_CMD`` override is authoritative even when broken, and
...
def cua_driver_install_hint() -> str:
    ...
    return (
        ...
        f"{installer}\n"
        "Or run `sarthi tools` and enable the Computer Use toolset to install it automatically."
    )
...
class _CuaDriverSession:
    ...
    async def _lifecycle_coro(self) -> None:
        ...
        try:
            ...
            params = StdioServerParameters(
                ...
                # sanitize Sarthi-managed secrets out of the child env.
    ...
    def _start_lifecycle_locked(self) -> None:
        ...
        if not self._ready_event.wait(timeout=30.0):
            ...
            raise RuntimeError(
                ...
                "Run `sarthi computer-use doctor` and check "
                f"{display_sarthi_home()}/logs/agent.log for the phase timings."
            )
    ...
    def _call_tool_via_cli(self, name: str, args: Dict[str, Any], timeout: float) -> Dict[str, Any]:
        ...
        try:
            for attempt in range(attempts):
                ...
                # Sarthi talks to the direct `cua-driver mcp` runtime
    ...
    def _unknown_transport_outcome(name: str, exc: Exception) -> Dict[str, Any]:
        message = (
            ...
            "unknown, so Sarthi did not replay it. Take fresh state before "
...
class CuaDriverBackend(ComputerUseBackend):
    ...
    def __init__(self, permission_mode: str = "standard") -> None:
        ...
        #   - Gets a distinct agent-cursor color per Sarthi run, with
    ...
    def start(self) -> None:
        ...
        if not contract.get("ready"):
            # An installed-but-incompatible driver (e.g. predating a Sarthi
        ...
        if not contract.get("ready"):
            ...
            if os.environ.get(_CUA_DRIVER_CMD_ENV, "").strip():
                repair = (
                    "Update the binary selected by SARTHI_CUA_DRIVER_CMD or "
                    "remove that override."
                )
            else:
                repair = "Run `sarthi computer-use install` to repair it."
            raise RuntimeError(f"cua-driver is not ready: {reason}. {repair}")
        _maybe_nudge_update()
        # The MCP client SDK (`mcp`) is an optional dependency (the
        # `computer-use` / `mcp` extras), not part of Sarthi' minimal core.
    ...
    def capture(
        ...
        Maps sarthi `capture(mode, app)` → cua-driver `list_windows` +
    ...
    def focus_app(self, app: str, raise_window: bool = False) -> ActionResult:
        ...
        separately approved by the Sarthi adapter, and uses cua-driver's
