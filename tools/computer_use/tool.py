...
`health_report` (surfaced by `sarthi computer-use doctor`) reports the
...
# regardless of approval level (e.g. logout kills the session Sarthi runs in).
_BLOCKED_KEY_COMBOS = {
...
# Per-Sarthi-session cached backends. Each backend owns its own cua-driver
...
def _cua_permission_mode(session_id: str) -> str:
    """Map Sarthi's explicit approval bypass onto Cua's immutable mode.

    Sarthi has TWO session-identity namespaces: the tool-dispatch path passes
        ...
        with _backend_lock:
            ...
            if cached is not None:
                ...
                backend_name = os.environ.get(
                    "SARTHI_COMPUTER_USE_BACKEND", "cua"
                ).lower()
                if backend_name in {"cua", "cua-driver", ""}:
                    ...
                    raise RuntimeError(
                        f"Unknown SARTHI_COMPUTER_USE_BACKEND={backend_name!r}"
                    )
...
def release_computer_use_session(session_id: str) -> bool:
    ...
    Sarthi YOLO selected unrestricted mode. Approval state is cleared even
    ...
    try:
        ...
        # while waiting: unrelated Sarthi sessions remain independent.
...
def _shutdown_backend_atexit() -> None:
    ...
    without this a driver can survive the Sarthi process that spawned it
...
def handle_computer_use(args: Dict[str, Any], **kwargs) -> Any:
    ...
    try:
        ...
        return json.dumps({
            ...
            "hint": "If the cua-driver binary is missing, run `sarthi computer-use install`. "
...
def _dispatch(backend: ComputerUseBackend, action: str, args: Dict[str, Any]) -> Any:
    ...
    # none of those capabilities can be supplied across Sarthi sessions.
...
# "foreground" (it doesn't know Sarthi has a typed page route), so the model
...
def _route_capture_through_aux_vision(
    ...
    *,
    ...
    The captured base64 PNG is materialised to ``$SARTHI_HOME/cache/vision/``
    ...
    try:
        ...
        if _mime_for_ext == "image/jpeg" or (not _mime_for_ext and cap.png_b64[:8].startswith("/9j/")):
            ...
            ext = ".png"
        cache_dir = get_sarthi_dir("cache/vision", "temp_vision_images")
...
def _persist_capture_image(cap: CaptureResult) -> Optional[str]:
    """Save a capture in Sarthi' media cache and return its absolute path.
    ...
    try:
        ...
        cache_dir = get_sarthi_dir("cache/images", "image_cache")
        cache_dir.mkdir(parents=True, exist_ok=True)
        try:
            captures = sorted(
...
def _spill_elements_to_file(cap: CaptureResult) -> Optional[str]:
    ...
    try:
        ...
        cache_dir = get_sarthi_dir("cache/computer_use", "computer_use_cache")
        cache_dir.mkdir(parents=True, exist_ok=True)
...
def check_computer_use_requirements() -> bool:
    ...
    `sarthi computer-use doctor` if their session is incomplete (e.g. no
    DISPLAY set).
    """
