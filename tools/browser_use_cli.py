...
_PRIVATE_BROWSER_SENTINEL = "_SARTHI_BU_PRIVATE_BROWSER"
...
_OWN_TAB_PREAMBLE = """\
# sarthi: pin this named session to its own tab (once per daemon process)
def _sarthi_ensure_own_tab():
    ...
    _marker = _os.path.join(
        _tf.gettempdir(), "sarthi-bu-owntab-%s-%s-%s" % (_uid, _name, _dpid)
    )
    ...
    try:
        ...
        pass
_sarthi_ensure_own_tab()
del _sarthi_ensure_own_tab
"""
...
def _base_subprocess_env() -> dict:
    ...
    # may differ from Sarthi's venv Python. PYTHONPATH/PYTHONHOME inherited
    # from the agent process point at Sarthi's venv site-packages, and a
...
def default_downgrade_notice() -> Optional[str]:
    ...
    try:
        ...
        stamp = Path(get_sarthi_home()) / "cache" / _NOTICE_STAMP_NAME
        try:
        ...
        return (
            ...
            "Run `sarthi tools` (Browser Automation → Browser Use) to install it, "
...
def _managed_bin_dir() -> Optional[str]:
    """Sarthi' own bin dir ($SARTHI_HOME/bin) — where install.sh puts uv/uvx
    and where install_cli() links the browser-use binary."""
    try:
        from sarthi_constants import get_sarthi_home

        return str(Path(get_sarthi_home()) / "bin")
...
def install_cli(timeout_s: int = 600) -> Tuple[bool, str]:
    ...
    Resolution order for uv: Sarthi' managed uv (bootstrapped on demand via
    ``sarthi_cli.managed_uv.ensure_uv``) → uv on PATH. The binary is linked
    into ``$SARTHI_HOME/bin`` (``UV_TOOL_BIN_DIR``) so ``_find_cli()``
    ...
    # prevent provisioning the canonical Sarthi-managed copy, or resolution
    # stays pinned to a binary we don't control (version drift, no updates
    # through sarthi tools).
...
def _workspace_dir(task_id: Optional[str]) -> Optional[str]:
    ...
    try:
        ...
        path = Path(get_sarthi_home()) / "cache" / "browser-use" / "workspace" / safe
...
def _resolve_backend_cdp(
    ...
    try:
        ...
        return (
            ...
            "switch backends via `sarthi tools` → Browser Automation."
        )
