...
def _expand_tilde(path: str) -> str:
    ...
    mirrors ``sarthi_constants.get_subprocess_home()`` so that ``~`` resolves
...
def _truncate_to_char_budget(content: str, max_chars: int) -> tuple[str, int, bool]:
    ...
    ``read_file``). Where sarthi previously hard-rejected an oversized read
...
def _file_ops_uses_host_paths(file_ops) -> bool:
    """Return True when *file_ops* targets the same host filesystem as Sarthi.
...
_sarthi_config_resolved: str | None = None
_sarthi_config_resolved_loaded = False


def _get_sarthi_config_resolved() -> str | None:
    """Return the resolved absolute path of the Sarthi config file (cached)."""
    global _sarthi_config_resolved, _sarthi_config_resolved_loaded
    if _sarthi_config_resolved_loaded:
        return _sarthi_config_resolved
    _sarthi_config_resolved_loaded = True
    try:
        from sarthi_cli.config import get_config_path
        _sarthi_config_resolved = str(get_config_path().resolve())
    except Exception:
        try:
            _sarthi_config_resolved = str(Path(_expand_tilde("~/.sarthi/config.yaml")).resolve())
        except Exception:
            _sarthi_config_resolved = None
    return _sarthi_config_resolved
...
def _check_sensitive_path(filepath: str, task_id: str = "default") -> str | None:
    ...
    if resolved in _SENSITIVE_EXACT_PATHS or normalized in _SENSITIVE_EXACT_PATHS:
        return _err
    # Prevent agents from modifying the Sarthi config file directly.
    # approvals.mode and other security settings live here; a malicious or
    # prompt-injected agent could silently disable exec approval by writing to
    # this file.
    sarthi_config = _get_sarthi_config_resolved()
    if sarthi_config and (resolved == sarthi_config or normalized == sarthi_config):
        return (
            f"Refusing to write to Sarthi config file: {filepath}\n"
            "Agent cannot modify security-sensitive configuration. "
            "Edit ~/.sarthi/config.yaml directly or use 'sarthi config' instead."
        )
    return None
...
# .cursorrules (or a project-local .sarthi config tree) outlives the current
...
_real_sarthi_home_cached: str | None = None
_real_sarthi_home_loaded = False


def _get_real_sarthi_home() -> str | None:
    """Return the realpath of the authoritative Sarthi home (cached)."""
    global _real_sarthi_home_cached, _real_sarthi_home_loaded
    if _real_sarthi_home_loaded:
        return _real_sarthi_home_cached
    _real_sarthi_home_loaded = True
    try:
        from sarthi_constants import get_sarthi_home
        _real_sarthi_home_cached = os.path.realpath(str(get_sarthi_home()))
    except Exception:
        try:
            _real_sarthi_home_cached = os.path.realpath(_expand_tilde("~/.sarthi"))
        except Exception:
            _real_sarthi_home_cached = None
    return _real_sarthi_home_cached
...
def _protected_instruction_config() -> tuple[bool, list[str]]:
    ...
    """
    try:
        from sarthi_cli.config import load_config, cfg_get
        cfg = load_config()
...
def _protected_instruction_reason(filepath: str, task_id: str = "default",
    ...
    # The authoritative ~/.sarthi home is governed by its own guards
    ...
    # ``.sarthi`` component rule below, which would otherwise match the
    # home directory itself.
    real_home = _get_real_sarthi_home()
    ...
    for candidate in (normalized, resolved):
        ...
        for pattern in extra_patterns:
            if fnmatch.fnmatch(base_lower, pattern.lower()):
                return base
        # Project-local .sarthi config dirs (e.g. <repo>/.sarthi/config.yaml)
        # are loaded as project context and steer behavior the same way.
        # Scope: the file's IMMEDIATE parent must be ``.sarthi`` — matching
        # any ancestor named .sarthi would gate every write inside a
        # checkout that happens to live under ~/.sarthi (e.g. the
        # sarthi-agent repo itself at ~/.sarthi/sarthi-agent).
        parts = candidate.replace("\\", "/").rstrip("/").split("/")
        if len(parts) >= 2 and parts[-2] == ".sarthi":
            return candidate
    return None
...
def _get_container_mirror_prefix_for_task(task_id: str = "default") -> str | None:
    """Return the container-side Sarthi mirror prefix for Docker file tools."""
    try:
        from tools.terminal_tool import (
            _active_environments,
            _env_lock,
...
def _check_cross_profile_path(filepath: str, task_id: str = "default") -> str | None:
    """Return a soft-guard warning when ``filepath`` lands in another Sarthi
    profile's scoped area, a host-side sandbox-mirror of authoritative profile
    state, or the Docker container's sandbox mirror of Sarthi state.
    ...
    * sandbox-mirror (#32049) — writes that hit the
      ``…/sandboxes/<backend>/<task>/home/.sarthi/…`` mirror created by a
      non-local terminal backend (Docker, Daytona, etc.), where the host
      Sarthi process never reads the mirror and the authoritative file is
      left untouched.
    * container-mirror (#32049 follow-up) — writes from inside a Docker
      ...
      the agent sees a plain ``/root/.sarthi/…`` path.

    Returns ``None`` when the write is in-scope or outside Sarthi scope.
    ...
    # in a session that cd'd into ``~/.sarthi/profiles/other/`` is
...
def read_file_tool(path: str, offset: int = 1, limit: int = 2000, task_id: str = "default") -> str:
    ...
    try:
        ...
        # ── Sarthi internal path guard ────────────────────────────────
        # Prevent prompt injection via catalog or hub metadata files,
        # and block credential stores under SARTHI_HOME.  Pass the
        # already-resolved path so a relative-path read against
        # TERMINAL_CWD == SARTHI_HOME (e.g. "auth.json") still hits the
...
def write_file_tool(path: str, content: str, task_id: str = "default",
    ...
    ``cross_profile`` opts out of the soft cross-Sarthi-profile guard. The
...
def patch_tool(mode: str = "replace", path: str = None, old_string: str = None,
    ...
    ``cross_profile`` opts out of the soft cross-Sarthi-profile guard for
...
WRITE_FILE_SCHEMA = {
    ...
    "parameters": {
        ...
        "properties": {
            ...
            "cross_profile": {
                "type": "boolean",
                "description": "Opt out of the cross-profile soft guard. Defaults to false. Set true ONLY after explicit user direction to edit another Sarthi profile's skills/plugins/cron/memories — by default these writes are blocked with a warning because they affect a different profile than the one this session is running under.",
                "default": False,
            },
        },
...
PATCH_SCHEMA = {
    ...
    "parameters": {
        ...
        "properties": {
            ...
            "cross_profile": {
                "type": "boolean",
                "description": "Opt out of the cross-profile soft guard. Defaults to false. Set true ONLY after explicit user direction to edit another Sarthi profile's skills/plugins/cron/memories.",
                "default": False,
            },
        },
        "required": ["mode"],
...
def _handle_write_file(args, **kw):
    ...
    if "content" not in args:
        return tool_error(
            ...
            "payload, or use execute_code with sarthi_tools.write_file() for very "
            "large files."
        )
