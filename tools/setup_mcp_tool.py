...
``sarthi mcp install <name>`` in the terminal.
"""
...
def setup_mcp_tool(
    ...
    if callback is None:
        return tool_error(
            "setup_mcp is only available in the Sarthi desktop app. Use the "
            "terminal instead: `sarthi mcp install <name>` for catalog entries, "
            "`sarthi mcp login <name>` for OAuth."
        )
...
SETUP_MCP_SCHEMA = {
    ...
    "description": (
        ...
        "Sarthi desktop chat. The card lets them install a catalog entry, "
        ...
        "continue without the server. Catalog names: run `sarthi mcp catalog` "
