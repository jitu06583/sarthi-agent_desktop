...
teammate agent (another Sarthi profile on this install, or an agent on a
...
DM each other via a prompt-injected ``sarthi -p <bot> chat ...`` shellout.
...
- Everything here is additive. The legacy protocol transports
  (``sarthi -p`` / ``sarthi peer dm``) keep working for older prompts.

The transports themselves are unchanged and proven:
- local teammate  → ``sarthi -p <name> chat --in ~ -c "Bot Chat"
  --create-if-missing -Q --query-file <tmp>`` (one turn, reply on stdout)
- peer teammate   → ``sarthi peer dm <peer>[/<name>] < <tmp>``
                ...
                "properties": {
                    "target": {
                        ...
                        "description": (
                            ...
                            "('researcher', 'sarthi' for the default agent), or "
...
def _resolve_local_name(target: str, roster: list[str]) -> Optional[str]:
    """Map a target handle to a profile name ('sarthi' → 'default')."""
    want = target.strip()
    if not want:
        return None
    if want.lower() == "sarthi":
...
def message_agent_tool(
    ...
    if peer_match or bare_peer:
        ...
        command = f"sarthi peer dm {shlex.quote(dm_target)} < {shlex.quote(_write_dm_file(prefix + body))}"
    ...
    command = (
        f"sarthi -p {shlex.quote(resolved)} chat --in ~ -c \"Bot Chat\" "
...
def _write_dm_file(content: str) -> str:
    ...
    fd, path = tempfile.mkstemp(prefix="sarthi-dm-", suffix=".txt", text=True)
...
def _agent_home(agent: Any) -> str:
    ...
    try:
        ...
        pass
    return os.getenv("SARTHI_HOME") or os.path.expanduser("~/.sarthi")
