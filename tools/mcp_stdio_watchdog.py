...
<url>``) is spawned as a direct child of the Sarthi process. Sarthi's own
...
Sarthi process dies hard — ``kill -9``, an OS-level crash, a force-quit of
...
parent dies, so nothing reaps these until the next Sarthi startup's opt-in
