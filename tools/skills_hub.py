#!/usr/bin/env python3
"""
Skills Hub — Source adapters and hub state management for the Sarthi Skills Hub.
...
Used by sarthi_cli/skills_hub.py for CLI commands and the /skills slash command.
"""
...
def _skills_dir() -> Path:
    ...
    return Path(forced) if forced is not None else _sarthi_home() / "skills"
...
class GitHubSource(SkillSource):
    ...
    def inspect(self, identifier: str) -> Optional[SkillMeta]:
        ...
        if isinstance(metadata, dict):
            sarthi_meta = metadata.get("sarthi", {})
            if isinstance(sarthi_meta, dict):
                tags = sarthi_meta.get("tags", [])
        if not tags:
...
class OptionalSkillSource(SkillSource):
    ...
    def _fetch_from_live_repo(self, rel: str) -> Optional[SkillBundle]:
        ...
        ``sarthi update``. Rather than telling them to update first, resolve
    ...
    def _scan_all(self) -> List[SkillMeta]:
        ...
        for skill_md in sorted(self._optional_dir.rglob("SKILL.md")):
            ...
            if isinstance(meta_block, dict):
                sarthi_meta = meta_block.get("sarthi", {})
                if isinstance(sarthi_meta, dict):
                    tags = sarthi_meta.get("tags", [])
...
def _category_skill_dirs(directory: Path) -> List[str]:
    ...
    ``sarthi_cli.skills_hub._existing_categories``.
    """
    skill_dirs: List[str] = []
...
# ---------------------------------------------------------------------------
# Sarthi centralized index source
# ---------------------------------------------------------------------------

SARTHI_INDEX_URL = "https://hermes-agent.nousresearch.com/docs/api/skills-index.json"
SARTHI_INDEX_TTL = 6 * 3600  # 6 hours


def _sarthi_index_cache_file() -> Path:
    return _index_cache_dir() / "sarthi-index.json"


def _load_sarthi_index() -> Optional[dict]:
    """Fetch the centralized skills index, with local cache.

    The index is a JSON file hosted on the docs site, rebuilt daily by CI.
    We cache it locally for SARTHI_INDEX_TTL seconds to avoid repeated
    downloads within a session.
    """
    # Check local cache
    sarthi_index_cache_file = _sarthi_index_cache_file()
    if sarthi_index_cache_file.exists():
        try:
            age = time.time() - sarthi_index_cache_file.stat().st_mtime
            if age < SARTHI_INDEX_TTL:
                return json.loads(sarthi_index_cache_file.read_text(encoding="utf-8"))
        except (OSError, json.JSONDecodeError):
            pass
    ...
    for accept_encoding in ("gzip, deflate", "identity"):
        try:
            ...
            if resp.status_code != 200:
                logger.debug("Sarthi index fetch returned %d", resp.status_code)
            ...
            logger.debug(
                "Sarthi index decode failed (Accept-Encoding=%s): %s",
                accept_encoding,
                e,
            )
            continue
        except (httpx.HTTPError, json.JSONDecodeError) as e:
            logger.debug("Sarthi index fetch failed: %s", e)
            return _load_stale_index_cache()
    ...
    # Cache locally
    try:
        sarthi_index_cache_file.parent.mkdir(parents=True, exist_ok=True)
        sarthi_index_cache_file.write_text(json.dumps(data), encoding="utf-8")
    except OSError:
        pass
...
class SarthiIndexSource(SkillSource):
    """Skill source backed by the centralized Sarthi Skills Index.
    ...
    def _ensure_loaded(self) -> dict:
        if not self._loaded:
            self._index = _load_sarthi_index()
            self._loaded = True
    ...
    def fetch(self, identifier: str) -> Optional[SkillBundle]:
        ...
        if resolved:
            ...
            if bundle:
                bundle.source = entry.get("source", "sarthi-index")
    ...
    def _to_meta(entry: dict) -> SkillMeta:
        return SkillMeta(
            name=entry.get("name", ""),
            description=entry.get("description", ""),
            source=entry.get("source", "sarthi-index"),
            identifier=entry.get("identifier", ""),
...
def create_source_router(auth: Optional[GitHubAuth] = None) -> List[SkillSource]:
    ...
    sources: List[SkillSource] = [
        ...
        SarthiIndexSource(auth=auth), # Centralized index (search + resolved install paths)
