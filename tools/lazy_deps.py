"""
Lazy dependency installer for opt-in Sarthi Agent backends.

Many Sarthi features (Mistral TTS, ElevenLabs TTS, Honcho memory, Bedrock,
...
(``sarthi-agent[all]``) and install them eagerly at setup time. That has
two problems:
...
remediation hint pointing at ``sarthi tools`` or the manual pip command.
...
* **Durable-target mode (immutable images).** When the deployment seals the
  agent's own venv (the Docker image sets ``SARTHI_DISABLE_LAZY_INSTALLS=1``
  and makes ``/opt/sarthi`` read-only), setting
  ``SARTHI_LAZY_INSTALL_TARGET`` redirects lazy installs to a writable
  ...
  installed package cannot brick Sarthi, which is what made it safe to seal
...
LAZY_DEPS: dict[str, tuple[str, ...]] = {
    ...
    # (SARTHI_DISABLE_LAZY_INSTALLS=1) and lazy installs are redirected to the
    ...
    # HF Agent Trace Viewer upload (sarthi trace upload / /upload-trace).
    #
    ...
    # presence, the `sarthi update` lazy-refresh pass re-asserts THIS pin on
...
_LAZY_TARGET_ENV = "SARTHI_LAZY_INSTALL_TARGET"
...
def _allow_lazy_installs() -> bool:
    ...
    2. ``SARTHI_DISABLE_LAZY_INSTALLS=1`` seals the *agent venv* (set by the
    ...
    if os.environ.get("SARTHI_DISABLE_LAZY_INSTALLS") == "1":
...
def _unsupported_feature_reason(feature: str) -> Optional[str]:
    ...
    ``sarthi update`` lazy-refresh pass.
    """
    if sys.platform == "win32" and feature == "platform.matrix":
        return (
            ...
            "from sdist. Run Sarthi under WSL to use Matrix on Windows."
        )
    return None
...
def _is_satisfied(spec: str) -> bool:
    ...
    ``sarthi update`` propagate pin bumps in :data:`LAZY_DEPS` to already-
...
def _core_constraints_file() -> Optional[Path]:
    ...
    * A backend that *requires* a version conflicting with core fails loudly
    ...
    try:
        ...
        if not lines:
            return None
        fd, path = tempfile.mkstemp(prefix="sarthi-core-constraints-", suffix=".txt")
...
def _venv_pip_install(specs: tuple[str, ...], *, timeout: int = 300) -> _InstallResult:
    ...
    * **Durable-target.** When :data:`_LAZY_TARGET_ENV` is set, installs into
    ...
    Mirrors the strategy in ``sarthi_cli.tools_config._pip_install`` but
    ...
    try:
        ...
        uv_env = sarthi_subprocess_env(inherit_credentials=False)
        uv_env["VIRTUAL_ENV"] = str(venv_root)

        # Tier 1: uv (preferred — fast, doesn't need pip in the venv)
        # Managed uv first: $SARTHI_HOME/bin is never on PATH, so a bare
        # which() misses the uv Sarthi installed and falls through to the
...
def ensure(feature: str, *, prompt: bool = True) -> None:
    ...
    # Package-manager installs (NixOS, and any other distro that ships Sarthi
    ...
    # deployment sets SARTHI_MANAGED=true *and* SARTHI_LAZY_INSTALL_TARGET
    ...
    if _lazy_install_target() is None:
        try:
            from sarthi_cli.config import get_managed_system
        ...
        if managed_by:
            raise FeatureUnavailable(
                ...
                f"packages come from {managed_by}, so Sarthi cannot install "
                f"them at runtime. Add the dependencies for {feature!r} via "
                f"{managed_by} (or run a pip/uv install of Sarthi instead)."
            )
...
def feature_install_command(feature: str, *, venv_pip: bool = False) -> Optional[str]:
    ...
    (default install, ``SARTHI_HOME`` overrides, profile installs) and
...
def install_specs(specs: list[str] | tuple[str, ...], *, timeout: int = 300) -> InstallSpecsResult:
    ...
    * **Durable-target on immutable images** — when the deployment seals the
      agent venv (``SARTHI_DISABLE_LAZY_INSTALLS=1``) and sets
      ``SARTHI_LAZY_INSTALL_TARGET``, installs are redirected to the writable
...
def active_features() -> list[str]:
    ...
    Used by ``sarthi update`` to figure out which lazy backends need a
...
def refresh_active_features(*, prompt: bool = False) -> dict[str, str]:
    ...
    Intended for ``sarthi update``. Never raises; lazy-install failures
