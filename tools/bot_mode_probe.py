...
``ui_meta['sarthi-bots']`` block in its profile.yaml), a bot's canonical
...
def _sarthi_root(home: Path) -> Path:
    """Root ~/.sarthi for both the default profile and named profiles."""
...
def _is_bot_managed(profile_dir: Path) -> bool:
    """True when profile.yaml carries a ui_meta['sarthi-bots'] block.
    ...
    try:
        ...
        if "sarthi-bots" not in raw:
            return False
        import yaml

        data = yaml.safe_load(raw)
        ui_meta = data.get("ui_meta") if isinstance(data, dict) else None
        return isinstance(ui_meta, dict) and isinstance(ui_meta.get("sarthi-bots"), dict)
    except Exception:
        return False
...
def _handle(name: str) -> str:
    # The mention middleware aliases the default profile as @sarthi.
    return "sarthi" if name == "default" else name


def _profile_role(profile_dir: Path) -> str:
    """A teammate's role line: Bot Mode title, else profile description.

    The ui_meta['sarthi-bots'].title is the name the user gave the bot in
    ...
    try:
        ...
        if isinstance(ui_meta, dict) and isinstance(ui_meta.get("sarthi-bots"), dict):
            title = str(ui_meta["sarthi-bots"].get("title") or "").strip()
            if title:
                parts.append(title)
...
def _peers(root: Path) -> list[str]:
    """Registered peer gateway names (``sarthi peer``), for the protocol text.
...
def _peer_paragraph(root: Path) -> str:
    ...
    return (
        ...
        "for the peer's main agent). Run `sarthi peer list` for the live "
        "peer list."
    )


def _build_section(home: Path) -> str:
    root = _sarthi_root(home)
    ...
    return (
        f"{_PROTOCOL_HEADING}\n"
        "This install runs Bot Mode: each Sarthi profile is an agent teammate with "
...
def get_bot_mode_protocol_section(home: str | os.PathLike | None = None, *, force_refresh: bool = False) -> str:
    ...
    not the ambient SARTHI_HOME — build threads can lose the ContextVar
    override and the env var would then name the wrong profile.
    """
    resolved = str(home) if home else (os.getenv("SARTHI_HOME") or os.path.expanduser("~/.sarthi"))
    with _lock:
...
def capability_fingerprint(home: str | os.PathLike | None = None) -> str:
    ...
    resolved = Path(str(home) if home else (os.getenv("SARTHI_HOME") or os.path.expanduser("~/.sarthi")))
    surface: dict = {}
    try:
        ...
        token = set_sarthi_home_override(str(resolved))
        try:
    ...
    try:
        ...
        surface["peers"] = _peers(_sarthi_root(resolved))
    except Exception:
        surface["peers"] = []
    try:
