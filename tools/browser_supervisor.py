...
One ``CDPSupervisor`` runs per Sarthi ``task_id`` that has a reachable CDP
...
def _redact_cdp_error_text(exc: object) -> str:
    ...
    those credentials never reach Sarthi logs or tracebacks. Falls back to a
...
DIALOG_BRIDGE_HOST = "sarthi-dialog-bridge.invalid"
...
(() => {
  if (window.__sarthiDialogBridgeInstalled) return;
  window.__sarthiDialogBridgeInstalled = true;
  const ENDPOINT = "http://sarthi-dialog-bridge.invalid/";
