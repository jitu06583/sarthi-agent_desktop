...
`sarthi skills publish` for sharing. No new source type, no new store, no new
...
@dataclass
class BlueprintSpec:
    """Parsed ``metadata.sarthi.blueprint`` automation spec for a skill."""
...
def parse_blueprint(skill_md_text: str) -> Optional[BlueprintSpec]:
    ...
    A skill is a blueprint iff ``metadata.sarthi.blueprint`` is a mapping containing
    ...
    sarthi = meta.get("sarthi") if isinstance(meta, dict) else None
    blueprint = sarthi.get("blueprint") if isinstance(sarthi, dict) else None
    if blueprint is None:
        return None
    if not isinstance(blueprint, dict):
        raise BlueprintError("metadata.sarthi.blueprint must be a mapping")
...
def export_blueprint(job: Dict[str, Any], body: str, *, blueprint_name: Optional[str] = None) -> str:
    ...
    and emit a SKILL.md (with a ``metadata.sarthi.blueprint`` block) they can hand
    to ``sarthi skills publish`` to share. ``body`` is the plain-language
