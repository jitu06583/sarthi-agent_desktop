...
Design contract (matches the Sarthi "no lazy-reading escape hatches / don't
...
def _check_metadata_block(frontmatter: Dict[str, Any]) -> List[LintFinding]:
    ...
    sarthi_meta = meta.get("sarthi") if isinstance(meta, dict) else None
    if not isinstance(sarthi_meta, dict):
        findings.append(
            LintFinding(
                WARNING,
                "missing-metadata",
                "frontmatter is missing metadata.sarthi.{tags, related_skills}.",
            )
        )
    else:
        if "tags" not in sarthi_meta:
            findings.append(
                LintFinding(
                    WARNING, "missing-metadata", "metadata.sarthi.tags is missing."
                )
            )
    author = str(frontmatter.get("author", ""))
    if author and author.strip().lower() in ("sarthi", "agent", "sarthi agent") and (
        author != "Sarthi Agent"
    ):
        findings.append(
            LintFinding(
                WARNING,
                "author-caps",
                f"author '{author}' should be 'Sarthi Agent' (proper caps) "
