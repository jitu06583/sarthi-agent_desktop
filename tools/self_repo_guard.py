...
def _block_message(operation: str, root: Path) -> str:
    ...
    return (
        f"Blocked: `{operation}` would rewrite Sarthi's live source checkout "
        ...
        "checkout, stop Sarthi, run the command externally, then restart "
        "Sarthi."
    )
...
def _scratch_dir_hint() -> str:
    ...
    base = Path(sarthi_home).expanduser() if sarthi_home else Path.home() / ".sarthi"
