...
URL of Sarthi' published Client ID Metadata Document (CIMD) as the
...
This module provides the glue:
    - ``SarthiTokenStorage``: persists tokens/client-info to disk so they
    ...
    mcp_servers:
      my_server:
        ...
        oauth:                                  # all fields optional
          ...
          client_name: "My Custom Client"       # default: "Sarthi Agent"
...
from sarthi_constants import secure_parent_dir
...
_USER_SKIPPED_SENTINEL = "__sarthi_user_skipped__"
...
def _get_token_dir(sarthi_home: str | Path | None = None) -> Path:
    """Return the directory for MCP OAuth token files.

    Uses SARTHI_HOME so each profile gets its own OAuth tokens.
    Layout: ``SARTHI_HOME/mcp-tokens/``
    """
    from sarthi_constants import get_sarthi_home

    base = Path(sarthi_home) if sarthi_home is not None else Path(get_sarthi_home())
...
def _cached_redirect_port(storage: "SarthiTokenStorage | None") -> int | None:
    ...
    redirect URI that was registered with it. If Sarthi restarts and chooses a
...
def _cached_redirect_uri(storage: "SarthiTokenStorage | None") -> str | None:
...
def _raise_if_non_interactive(lead: str) -> None:
    ...
    shared, actionable ``sarthi mcp login`` next-step so the guidance wording
    lives in one place across every non-interactive OAuth boundary (#57836).
    """
    if not _is_interactive():
        raise OAuthNonInteractiveError(
            f"{lead} "
            "Run `sarthi mcp login <server>` interactively to (re)authorize, "
...
# ---------------------------------------------------------------------------
# SarthiTokenStorage -- persistent token/client-info on disk
# ---------------------------------------------------------------------------


class SarthiTokenStorage:
    """Persist OAuth tokens and client registration to JSON files.

    File layout::

        SARTHI_HOME/mcp-tokens/<server_name>.json         -- tokens
        SARTHI_HOME/mcp-tokens/<server_name>.client.json   -- client info
        SARTHI_HOME/mcp-tokens/<server_name>.meta.json     -- oauth server metadata
        SARTHI_HOME/mcp-tokens/<server_name>.cimd-off      -- CIMD refused here
    """

    def __init__(self, server_name: str, *, sarthi_home: str | Path | None = None):
        self._server_name = _safe_filename(server_name)
        self._sarthi_home = Path(sarthi_home) if sarthi_home is not None else None

    def _tokens_path(self) -> Path:
        return _get_token_dir(self._sarthi_home) / f"{self._server_name}.json"

    def _client_info_path(self) -> Path:
        return _get_token_dir(self._sarthi_home) / f"{self._server_name}.client.json"

    def _meta_path(self) -> Path:
        return _get_token_dir(self._sarthi_home) / f"{self._server_name}.meta.json"

    def _cimd_rejected_path(self) -> Path:
        return _get_token_dir(self._sarthi_home) / f"{self._server_name}.cimd-off"
    ...
    async def get_tokens(self) -> "OAuthToken | None":
        ...
        if OAuthToken is None and not _ensure_sdk_loaded():
            return None
        # Sarthi records an absolute wall-clock ``expires_at`` alongside the
    ...
    def mark_cimd_rejected(self) -> None:
        ...
        refused. Cleared by ``remove()``, i.e. by ``sarthi mcp login`` /
        ``sarthi mcp remove``, so a fixed document gets another chance.
        """
    ...
    def restore(self, snapshot: dict[str, bytes], *, only_if_absent: bool = False) -> None:
        ...
        if not snapshot:
            return
        token_dir = _get_token_dir(self._sarthi_home)
    ...
    class _Handler(BaseHTTPRequestHandler):
        def do_GET(self) -> None:  # noqa: N802
            ...
            body = (
                ...
                "<p>You can close this tab and return to Sarthi.</p></body></html>"
            ) if code else (
        ...
        if result["auth_code"] is None:
            ...
            if cimd_url:
                hint = (
                    ...
                    f"Sarthi' Client ID Metadata Document ({cimd_url}); set "
...
def _paste_callback_reader(result: dict) -> None:
    ...
    if line.lower() in _SKIP_TOKENS:
        ...
        print(
            "  OAuth skipped. Run `sarthi mcp login <server>` later to "
...
SarthiOAuthClientProvider: Any = None


def _get_sarthi_oauth_provider_class() -> type | None:
    global SarthiOAuthClientProvider
    if SarthiOAuthClientProvider is not None:
        return SarthiOAuthClientProvider
    if not _ensure_sdk_loaded():
        return None

    class _SarthiOAuthClientProvider(OAuthClientProvider):
        ...
        def __init__(self, *args: Any, token_user_agent: "str | None" = None, **kwargs: Any):
            ...
            self._sarthi_token_user_agent = token_user_agent

        def _stamp_token_user_agent(self, request):
            ua = getattr(self, "_sarthi_token_user_agent", None)
            if ua:
    ...
    _SarthiOAuthClientProvider.__name__ = "SarthiOAuthClientProvider"
    _SarthiOAuthClientProvider.__qualname__ = "SarthiOAuthClientProvider"
    SarthiOAuthClientProvider = _SarthiOAuthClientProvider
    return SarthiOAuthClientProvider
...
def remove_oauth_tokens(
    ...
    *,
    ...
    storage = SarthiTokenStorage(server_name, sarthi_home=sarthi_home)
    storage.remove()
...
# 2026-07-28. The SDK does the protocol work; Sarthi only decides whether a
...
_CIMD_CLIENT_METADATA_URL = (
    "https://nousresearch.github.io/sarthi-agent/docs/oauth/client-metadata.json"
)
...
# (section 4.2), so a CIMD flow cannot use the ephemeral port Sarthi picks
...
def _has_cached_client_info(storage: "SarthiTokenStorage | None") -> bool:
...
def _server_declined_cimd(storage: "SarthiTokenStorage | None") -> bool:
    ...
    Sarthi has to fix the redirect URI. Cached authorization-server metadata
...
def _maybe_use_cimd(
    cfg: dict,
    storage: "SarthiTokenStorage | None" = None,
) -> "tuple[str, int] | None":
    """Return ``(client_id URL, pinned callback port)``, or None to use DCR.

    Every early return below is a case where the redirect URI Sarthi would
...
#   "Claude Code" → 200
#   "Codex"       → 200
#   "Sarthi Agent" / "Sarthi" / "Cursor" / "VS Code" / … → 403
...
def _build_client_metadata(cfg: dict) -> "OAuthClientMetadata":
    ...
    if OAuthClientMetadata is None:
        _ensure_sdk_loaded()
    client_name = cfg.get("client_name", "Sarthi Agent")
    scope = cfg.get("scope")
    ...
    metadata_kwargs: dict[str, Any] = {
        ...
        # rejecting loopback redirect_uris. Sarthi is a CLI/desktop app
...
def _invalidate_tokens_on_client_change(
    ...
    if removed:
        logger.warning(
            ...
            "Re-authorize with: sarthi mcp login %s",
...
def humanize_oauth_registration_error(
    ...
    *,
    ...
    Figma's remote MCP gates DCR on exact ``client_name``. Sarthi auto-sets
    ``Claude Code`` (known-good); this message fires when the user overrode
    that with something Figma still rejects, or an older Sarthi is running.
    """
    msg = str(exc)
    lowered = msg.lower()
...
def build_oauth_auth(
    ...
    storage = SarthiTokenStorage(server_name)
    ...
    if not _is_interactive() and not storage.has_cached_tokens():
        raise OAuthNonInteractiveError(
            ...
            f"`sarthi mcp login {server_name}` interactively first to complete "
    ...
    provider_class = _get_sarthi_oauth_provider_class()
