...
it is automatically downloaded from GitHub releases to $SARTHI_HOME/bin/tirith.
...
def _get_sarthi_home() -> str:
    """Return the Sarthi home directory, respecting SARTHI_HOME env var."""
    return str(get_sarthi_home())


def _failure_marker_path() -> str:
    """Return the path to the install-failure marker file."""
    return os.path.join(_get_sarthi_home(), ".tirith-install-failed")
...
def _sarthi_bin_dir() -> str:
    """Return $SARTHI_HOME/bin, creating it if needed."""
    d = os.path.join(_get_sarthi_home(), "bin")
...
def _install_tirith(*, log_failures: bool = True) -> tuple[str | None, str]:
    """Download and install tirith to $SARTHI_HOME/bin/tirith.
    ...
    try:
        ...
        dest = os.path.join(_sarthi_bin_dir(), "tirith")
        try:
...
def _resolve_tirith_path(configured_path: str) -> str:
    ...
    2. $SARTHI_HOME/bin/tirith (previously auto-installed)
    3. Auto-install from GitHub releases → $SARTHI_HOME/bin/tirith
    ...
    sarthi_bin = os.path.join(_sarthi_bin_dir(), "tirith")
    if os.path.isfile(sarthi_bin) and os.access(sarthi_bin, os.X_OK):
        _resolved_path = sarthi_bin
        _install_failure_reason = ""
        _clear_install_failed()
        return sarthi_bin
