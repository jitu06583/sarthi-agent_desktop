#!/usr/bin/env python3
"""Run a guided tour (highlight + narrate UI elements) in the Sarthi desktop GUI.
...
- ``surface="app"`` — the Sarthi desktop app's own DOM (tours of Sarthi itself).
...
def tour_tool(
    ...
    if callback is None:
        return tool_error("tour is only available in the Sarthi desktop app.")
...
TOUR_SCHEMA = {
    "name": "tour",
    "description": (
        "Give a live guided tour in the Sarthi desktop GUI: dim the screen, "
        "highlight an element, and attach a popover with your own title/text. "
        "Works on two surfaces — 'app' (the Sarthi app itself) and 'preview' "
    ...
    "parameters": {
        ...
        "properties": {
            ...
            "surface": {
                "type": "string",
                "enum": list(SURFACES),
                "description": "Where the tour runs: 'app' (Sarthi desktop UI, default) or 'preview' (the page in the in-app browser pane).",
            },
            "selector": {
                "type": "string",
