"""
Sarthi MCP Server — expose messaging conversations as MCP tools.
...
Plus: channels_list (Sarthi-specific extra)

Usage:
    sarthi mcp serve
    sarthi mcp serve --verbose
...
logger = logging.getLogger("sarthi.mcp_serve")
...
def _get_sessions_dir() -> Path:
    """Return the sessions directory using SARTHI_HOME."""
    try:
        from sarthi_constants import get_sarthi_home
        return get_sarthi_home() / "sessions"
    except ImportError:
        return Path(os.environ.get("SARTHI_HOME", Path.home() / ".sarthi")) / "sessions"
...
def _load_channel_directory() -> dict:
    ...
    try:
        from sarthi_constants import get_sarthi_home
        directory_file = get_sarthi_home() / "channel_directory.json"
    except ImportError:
        directory_file = Path(
            os.environ.get("SARTHI_HOME", Path.home() / ".sarthi")
...
class EventBridge:
    ...
    This is the Sarthi equivalent of OpenClaw's WebSocket gateway bridge.
    ...
    def _poll_once(self, db):
        ...
        try:
            from sarthi_constants import get_sarthi_home
            db_file = get_sarthi_home() / "state.db"
        except ImportError:
            db_file = Path(os.environ.get("SARTHI_HOME", Path.home() / ".sarthi")) / "state.db"
...
def create_mcp_server(event_bridge: Optional[EventBridge] = None) -> "FastMCP":
    """Create and return the Sarthi MCP server with all tools registered."""
    ...
    mcp = FastMCP(
        "sarthi",
        instructions=(
            "Sarthi Agent messaging bridge. Use these tools to interact with "
...
def run_mcp_server(verbose: bool = False) -> None:
    """Start the Sarthi MCP server on stdio."""
    if not _MCP_SERVER_AVAILABLE:
        print(
