...
When running Sarthi inside Docker, the default `network_mode: host` gives the
...
The Sarthi [SECURITY.md](../../SECURITY.md) §2 defines the trust model. The
...
#
# Usage:
#   SARTHI_UID=$(id -u) SARTHI_GID=$(id -g) docker compose up -d
#
...
services:
  gateway:
    ...
    environment:
      ...
      - NO_PROXY=sarthi,sarthi-dashboard,localhost
...
docker compose exec gateway \
  curl -sf --max-time 5 http://sarthi-dashboard:9119/health && echo "OK: internal reachable" || echo "FAIL"
...
- [SECURITY.md](../../SECURITY.md) — Sarthi trust model and vulnerability reporting
