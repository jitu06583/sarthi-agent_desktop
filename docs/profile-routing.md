...
`sarthi config set gateway.profile_routes ...` writes).
...
| `profile` | yes | Target profile name (must exist under `~/.sarthi/profiles/<name>`). |
...
activates the per-profile runtime scope (per-profile `SARTHI_HOME`, secret scope, and
