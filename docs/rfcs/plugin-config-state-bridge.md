...
or change arbitrary Sarthi configuration.
...
small host fact requested by the RFC. Settings writes use Sarthi'
...
`SARTHI_HOME`, so one globally loaded plugin context follows multiplexed
...
```text
<SARTHI_HOME>/plugin-data/<plugin-data-namespace>/state.json
```
...
- **Profile isolation:** the data root resolves from the active context-local
  Sarthi home on every operation.
...
The implementation is covered with real temporary-Sarthi-home tests for:
