...
This document is the formal interface between the **Sarthi gateway** (Python,
...
- **Registration.** The gateway registers a **wake URL** at enroll/provision —
  ...
  a dashboard host). Self-hosted: `sarthi gateway enroll --wake-url <url>` (or
...
**per-gateway secret** (`sarthi gateway enroll` → connector `/relay/enroll`, or
...
driven by the managed Portal / `sarthi` CLI.
