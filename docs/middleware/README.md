...
Sarthi middleware is the behavior-changing companion to observer hooks.
...
selection, or handoff to runtimes such as NeMo Relay without changing Sarthi'
planner, model provider adapters, tool registry, memory, or CLI UX.

With middleware enabled, plugins can:

- Rewrite LLM provider request kwargs before Sarthi calls the provider.
- Rewrite tool arguments before guardrails, approval checks, hooks, and tool
  execution see them.
- Wrap the actual LLM execution callback while preserving Sarthi retry,
  streaming, interrupt, and hook behavior.
- Wrap the actual tool execution callback while preserving Sarthi guardrails,
...
- `telemetry_schema_version`: currently `sarthi.observer.v1`
- `middleware_schema_version`: currently `sarthi.middleware.v1`
...
Sarthi stores those trace entries in later observer hook payloads as
`middleware_trace`.
...
If multiple plugins register the same execution middleware kind, Sarthi runs
them as a nested chain in registration order. Middleware failures are fail-open:
Sarthi logs a warning and continues with the next middleware or the base
runtime path.
...
For each provider request, Sarthi applies middleware in this order:
...
For each tool call, Sarthi applies middleware in this order:

1. Parse and coerce model-provided tool arguments.
2. Apply `tool_request` middleware.
3. Run the normal Sarthi pre-execution path against the effective arguments:
...
For isolated local testing, use one `SARTHI_HOME` for plugin enablement and the
agent run:

```bash
export SARTHI_HOME=/tmp/sarthi-middleware-test
mkdir -p "$SARTHI_HOME"
sarthi plugins enable <plugin-name>
sarthi chat --query 'Reply exactly ok'
```
...
```bash
uv sync
uv run sarthi plugins enable <plugin-name>
uv run sarthi chat --query 'Reply exactly ok'
```
...
def tag_llm_request(**kwargs):
    ...
    extra_body.setdefault("metadata", {})["sarthi_middleware_demo"] = True
...
def normalize_terminal_workdir(**kwargs):
    ...
    args.setdefault("workdir", "/tmp/sarthi-middleware-demo")
    return {
        "args": args,
        "source": "middleware-demo",
...
Return the same response shape Sarthi expects from the provider adapter. Do not
...
- If execution middleware raises before calling `next_call(...)`, Sarthi treats
...
- If execution middleware calls `next_call(...)` successfully and then raises
  during post-processing, Sarthi preserves the downstream result and does not
  run the provider or tool a second time.
- If downstream provider or tool execution fails, middleware may let that error
  propagate or translate it deliberately. Sarthi does not convert downstream
