...
The canonical transcript store is SQLite via `SessionDB` (from `sarthi_state`). The
...
Counts consecutive restarts via a JSON file (`{SARTHI_HOME}/restart_counts.json`). If a
...
This prevents unwanted auto-resets after `sarthi update`, `sarthi gateway restart`,
or `/restart`.
