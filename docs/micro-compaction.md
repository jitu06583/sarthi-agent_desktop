...
has to be thrown away or summarized. Sarthi has always done this in one batch:
...
Sarthi folds the single oldest un-absorbed exchange into a running summary. The
...
The in-memory splice alone isn't enough. Sarthi's normal session flush is
...
With `micro_compact` unset or `false` Sarthi behaves exactly as it always has:
...
Defaults to `$SARTHI_HOME/logs/agent.log`. It reports passes, outcome mix, net
