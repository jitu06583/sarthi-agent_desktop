...
Sarthi includes NeMo Relay as a normal runtime dependency on platforms for
which Relay publishes a native wheel. The shared-metrics integration is built
into Sarthi and does not require a Sarthi observability plugin. Sarthi remains
importable without Relay on other native targets. Those targets use an
explicit reduced-capability no-op host:
Sarthi execution remains available, while Relay scopes, middleware, plugins,
and subscribers are unavailable. The `sarthi-agent[nemo-relay]` extra remains
as a no-op compatibility alias for existing installation commands.

> [!WARNING]
> This removes the Sarthi `observability/nemo_relay` plugin. Existing users
...
> `plugins.toml` selected with `SARTHI_NEMO_RELAY_PLUGINS_TOML`. The legacy
> `SARTHI_NEMO_RELAY_ATOF_*` and `SARTHI_NEMO_RELAY_ATIF_*` variables no
> longer activate exporters. Without the new variable, Sarthi does not run
> Relay plugin discovery, configuration layering, middleware, or exporters.

Sarthi requires NeMo Relay 0.7.1 or later within the 0.7 release line. That
...
Sarthi installs the platform-specific `nemo-relay` native wheel from the
...
through that native module in the Sarthi process so configured interceptors can
...
Collection remains off unless Sarthi policy enables it:
...
opt-in. Set `SARTHI_NEMO_RELAY_PLUGINS_TOML` to a selected `plugins.toml` to
activate configured middleware, exporters, or dynamic plugins. When the
variable is unset, Sarthi does not invoke Relay's plugin initializer, so Relay
...
from the selected file only. If the selected file cannot be loaded, Sarthi
...
retain the same `session_id` and add `sarthi.session.segment` plus
`sarthi.session.segment_reason` (`compaction` or `max_turns`).

## Process-Wide Plugin Policy and Profile Isolation

Relay plugin configuration is a process-level deployment choice, not a Sarthi
profile setting. The first hosted profile triggers lazy initialization, and
every additional profile hosted by that Sarthi process shares the resulting
static middleware, dynamic plugins, subscribers, exporters, and guardrail
policy. After initialization succeeds, Sarthi logs:

```text
Relay plugins are active process-wide and apply to all profiles hosted by this Sarthi process.
```
...
profile's Relay scope stack. Native dynamic plugins are loaded into the Sarthi
process and share the same policy boundary.

Run profiles in separate Sarthi processes when they require different trust
...
Sarthi core owns one Relay host and one isolated Relay session scope per Sarthi
session. Core lifecycle producers use
`sarthi_cli.observability.relay_runtime` to obtain the shared session handle or
run Relay scope, LLM, tool, and mark APIs in that session context. New product
marks do not require Sarthi plugin registration. Shared-metrics marks must
...
```text
Sarthi turn, API, tool, and approval hooks
  -> Relay session, task, LLM, tool, and mark lifecycle
  -> Sarthi shared-metrics subscriber
  -> SQLite counters
  -> immutable JSON delta package
```

Sarthi sends an empty `LLMRequest` into the metrics-owned lifecycle. This does
...
provider route that Sarthi used for the logical call, such as
...
New calls use `sarthi.model_route.count`. The previous
`sarthi.model_call.count` contract remains readable only so pending local
counters created by older builds can be exported without losing data.

The first consented session start emits an empty `sarthi.client.active` Relay
...
counter in any rolling 24-hour window. The metric has no dimensions; Sarthi
version, OS family, architecture, and install method remain bounded package
resources. Concurrent Sarthi processes share the SQLite latch, so simultaneous
...
Each task run is a Relay `Function` scope named `sarthi.task_run`, parented to
the owning Sarthi session. The start counter contains only bounded execution
...
provider attempts for the same Sarthi API request ID; they do not inflate the
logical model-call count. Tool calls are deduplicated by their Sarthi tool-call
...
cancellations. Active task ownership follows the task ID if Sarthi rotates its
conversation session during context compression.

Each tool invocation is represented by a Relay tool lifecycle named
`sarthi.tool_call`. The terminal counter contains only bounded tool category,
outcome, approval outcome, latency, and explicit retry-count buckets. Sarthi
...
tool or plugin names. Sarthi does not infer retries from repeated tool names or
adjacent calls; when the
hook does not provide an explicit retry relationship, the retry bucket is
`unknown`. Approval decisions are emitted as `sarthi.tool_approval` marks and
...
Successful skill mutations emit `sarthi.skill.lifecycle` marks with only a
bounded action and provenance. Successful loads emit `sarthi.skill.load`
marks with bounded provenance, first-use or reuse state, reuse-after-patch
state, and a use-count bucket. Sarthi derives reuse and patch-generation
...
```text
$SARTHI_HOME/telemetry/shared_metrics/metrics.sqlite3
$SARTHI_HOME/telemetry/shared_metrics/outbox/*.json
```
...
are written with atomic replacement. Each package records the Sarthi version,
...
schema field name, its current scope is one `SARTHI_HOME`, so it is more
...
Deleting `$SARTHI_HOME/telemetry/shared_metrics` resets the identifier together
...
The install identity is scoped to one `SARTHI_HOME`. To reset it, stop Sarthi
processes and remove `$SARTHI_HOME/telemetry/shared_metrics`. This deliberately
...
Run a real Sarthi CLI turn against the deterministic local model server:
