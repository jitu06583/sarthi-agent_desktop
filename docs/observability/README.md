# Sarthi Observer Hooks

Sarthi observer hooks are the read-only telemetry contract for plugins that
...
They do not replace Sarthi' planner, model providers, memory, tool registry,
...
Sarthi also has a first-party NeMo Relay shared-metrics path. It uses these
...
Hook callbacks are fail-open. Sarthi catches callback exceptions, logs a
...
payload construction is gated behind `has_hook(...)`, so Sarthi only builds
...
The native NeMo Relay SDK integration maps Sarthi session, turn, LLM, and tool
