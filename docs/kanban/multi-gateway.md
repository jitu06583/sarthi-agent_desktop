...
Sarthi supports multiple gateway processes running concurrently — one per profile
...
needed. On every other profile gateway, add to `~/.sarthi/config.yaml`:
...
Or set the env var: `SARTHI_KANBAN_DISPATCH_IN_GATEWAY=false`
