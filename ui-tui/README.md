...
React + Ink terminal UI for Sarthi. TypeScript owns the screen. Python owns sessions, tools, model calls, and most command logic.
...
Interpreter resolution order is: `SARTHI_PYTHON` → `PYTHON` → `$VIRTUAL_ENV/bin/python` → `./.venv/bin/python` → `./venv/bin/python` → `python3` (or `python` on Windows).
...
- `src/app/setupHandoff.ts` — launches external `sarthi setup`, suspends Ink while it runs, opens a new session on success
...
- Input history is stored in `~/.sarthi/.sarthi_history` or under `SARTHI_HOME`.
...
### Setup (`setup.ts`)
`/setup` — launches external `sarthi setup` wizard, suspends Ink while it runs
...
```text
ui-tui/
  packages/sarthi-ink/   forked Ink renderer (local dep)
  src/
    ...
    app/
      ...
      setupHandoff.ts               launches external sarthi setup, suspends Ink while it runs
    ...
    lib/
      ...
      parentLog.ts               append-only log to ~/.sarthi/tui-parent.log
    ...
    types/
      sarthi-ink.d.ts            type declarations for @sarthi/ink
...
tui_gateway/
  ...
  slash_worker.py        persistent SarthiCLI subprocess for slash commands
```
