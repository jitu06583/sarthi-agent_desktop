 ...
 * sarthi-ink sets ``key.meta`` for plain Alt/Option on every platform
 ...
 * (``sarthi_cli/voice.py::normalize_voice_record_key_for_prompt_toolkit``)
 ...
 * modifier — but sarthi-ink reports Alt as ``key.meta`` on many
...
export const parseVoiceRecordKey = (raw: unknown): ParsedVoiceRecordKey => {
  ...
  // On macOS sarthi-ink reports Alt as ``key.meta``, which ``isActionMod``
...
export const isVoiceToggleKey = (
  ...
  switch (configured.mod) {
    case 'alt':
      ...
      // alt binding.
      //
      // Bare Escape on sarthi-ink can arrive as ``key.meta=true`` on some
    ...
    case 'ctrl':
      ...
      // ``key.meta`` is sarthi-ink's Alt signal and accepting it would
