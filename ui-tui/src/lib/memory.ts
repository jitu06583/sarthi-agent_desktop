...
export interface HeapDumpResult {
  ...
  // the heavy snapshot because SARTHI_AUTO_HEAPDUMP was not enabled (#21767).
  suppressed?: boolean
  success: boolean
}
...
export async function performHeapDump(trigger: MemoryTrigger = 'manual'): Promise<HeapDumpResult> {
  try {
    ...
    const dir = process.env.SARTHI_HEAPDUMP_DIR?.trim() || join(homedir() || tmpdir(), '.sarthi', 'heapdumps')

    await mkdir(dir, { recursive: true })

    const base = `sarthi-${new Date().toISOString().replace(/[:.]/g, '-')}-${process.pid}-${trigger}`
    ...
    const autoEnabled = /^(?:1|true|yes|on)$/i.test((process.env.SARTHI_AUTO_HEAPDUMP ?? '').trim())
...
async function pruneHeapdumps(dir: string): Promise<void> {
  const raw = process.env.SARTHI_HEAPDUMP_MAX_BYTES?.trim()
