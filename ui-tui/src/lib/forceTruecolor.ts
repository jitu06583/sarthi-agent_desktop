...
export function shouldForceTruecolor(env: NodeJS.ProcessEnv = process.env): boolean {
  const override = (env.SARTHI_TUI_TRUECOLOR ?? '').trim()
...
if (shouldForceTruecolor()) {
  ...
  // incorrectly. Keep Sarthi on the safer TERM-driven 256-color path unless
  // users explicitly opt back in via SARTHI_TUI_TRUECOLOR=1.
