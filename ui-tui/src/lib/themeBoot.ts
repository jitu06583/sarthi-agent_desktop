 ...
 * `sarthi-boot-background` / `sarthi-boot-color-scheme` localStorage keys.
 *
...
// Profile-aware: the Python launcher exports SARTHI_HOME (set by
// _apply_profile_override) before spawning the TUI. Falling back to
// ~/.sarthi matches get_sarthi_home()'s default.
const bootFilePath = () => join(process.env.SARTHI_HOME ?? join(homedir(), '.sarthi'), 'tui-theme-boot.json')

// Never touch the user's real ~/.sarthi from test runs (the TS suite has no
// SARTHI_HOME isolation fixture).
...
export interface BootSeedResult {
  ...
  /** True when the cached config pin was seeded into SARTHI_TUI_THEME. */
  seededPin: boolean
}
...
export function seedBootEnvironment(boot: BootTheme | null, env: NodeJS.ProcessEnv): BootSeedResult {
  ...
  if (!boot || env.SARTHI_TUI_THEME || env.SARTHI_TUI_LIGHT) {
    return result
  }
  ...
  if (boot.mode) {
    env.SARTHI_TUI_THEME = boot.mode
  ...
  if (
    ...
    !env.SARTHI_TUI_BACKGROUND
  ) {
    env.SARTHI_TUI_BACKGROUND = boot.background
...
export function invalidateBootBackground(env: NodeJS.ProcessEnv = process.env): boolean {
  if (!seeded.seededBackground || env.SARTHI_TUI_BACKGROUND !== seeded.seededBackground) {
    return false
  }

  delete env.SARTHI_TUI_BACKGROUND
...
/** True when this boot replayed a cached config pin into SARTHI_TUI_THEME.
