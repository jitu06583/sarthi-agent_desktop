...
import { terminalBackgroundHex } from '@sarthi/ink'
...
export const debugCommands: SlashCommand[] = [
  ...
  {
    help: 'rescan $SARTHI_HOME/tui-widgets and (re)register user widget apps',
    name: 'widgets-reload',
    run: (_arg, ctx) => {
  ...
  {
    help: 'write a V8 heap snapshot + memory diagnostics (see SARTHI_HEAPDUMP_DIR)',
    name: 'heapdump',
    run: (_arg, ctx) => {
      ...
      ctx.transcript.panel('Theme', [
        {
          rows: [
            ...
            ['SARTHI_TUI_BACKGROUND', process.env.SARTHI_TUI_BACKGROUND ?? '(unset)'],
            ['SARTHI_TUI_THEME', process.env.SARTHI_TUI_THEME ?? '(unset)'],
