import type { RunExternalProcess } from '@sarthi/ink'
...
export async function runExternalSetup({ args, ctx, done, launcher, suspend }: RunExternalSetupOptions) {
  ...
  transcript.sys(`launching \`sarthi ${args.join(' ')}\`…`)
  ...
  if (result.error) {
    transcript.sys(`error launching sarthi: ${result.error}`)
  ...
  if (result.code !== 0) {
    transcript.sys(`sarthi ${args[0]} exited with code ${result.code}`)
    patchUiState({ status: 'setup required' })
