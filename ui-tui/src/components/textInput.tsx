import type { InputEvent, Key } from '@sarthi/ink'
import * as Ink from '@sarthi/ink'
 ...
 * but that accepts `key.meta` on macOS — and sarthi-ink reports Option as
...
export function supportsFastEchoTerminal(env: NodeJS.ProcessEnv = process.env): boolean {
  ...
  if (isTermuxTuiMode(env)) {
    const override = String(env.SARTHI_TUI_TERMUX_FAST_ECHO ?? '')
      .trim()
      .toLowerCase()
  ...
  useInput(
    (inp: string, k: Key, event: InputEvent) => {
      ...
      if (actionHome) {
        ...
        // ESC d, which sarthi-ink decodes as meta+'d'; without this branch it
