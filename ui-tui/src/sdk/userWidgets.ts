...
import { Box, Text } from '@sarthi/ink'
...
/**
 * User widget apps — Sarthi authors its own TUI widgets, mirroring the
 * Python plugin contract: drop `<name>.mjs` into `$SARTHI_HOME/tui-widgets/`,
 ...
 * React.createElement.
 *
 * Trust model matches `~/.sarthi/plugins/`: files under SARTHI_HOME execute
...
const widgetsDir = () => join(process.env.SARTHI_HOME?.trim() || join(homedir(), '.sarthi'), 'tui-widgets')
 ...
 *  every change, so a widget Sarthi writes appears within ~a second — no
...
export function watchUserWidgets(dir = widgetsDir()): void {
  ...
  if (!attach()) {
    ...
    // user (or Sarthi) ever writes must hot-load too — a 10s poll here read
