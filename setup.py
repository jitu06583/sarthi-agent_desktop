...
Sarthi Agent (see website/docs/getting-started/platform-support.md). The
...
build sandbox. ``nix/python.nix`` sets ``SARTHI_NIX_BUILD=1`` on the
Sarthi package derivation, so only that build may create an artifact.
...
_IN_NIX_BUILD = os.environ.get("SARTHI_NIX_BUILD") == "1"

_BLOCK_MESSAGE = (
    "Building wheels or sdists for sarthi-agent is not supported.\n"
    "Sarthi is distributed via the shell installer, Docker image, or Nix.\n"
    ...
    "the Sarthi Nix derivation sets SARTHI_NIX_BUILD=1. If it does, file a bug."
)
